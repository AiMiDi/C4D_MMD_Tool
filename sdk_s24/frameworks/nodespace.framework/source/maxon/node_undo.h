...
public:
	...
	{
		_previous = graph.GetRoot().GetValue<decltype(NimbusUndoAttribute)>().GetValueOrNull() iferr_ignore("no error handling possible here");
		graph.GetRoot().SetValue<decltype(NimbusUndoAttribute)>(true) iferr_ignore("no error handling possible here");
	}
	...
	{
		if (_graph)
			_graph.GetRoot().SetValue<decltype(NimbusUndoAttribute)>(_previous) iferr_ignore("no error handling possible here");
	}
