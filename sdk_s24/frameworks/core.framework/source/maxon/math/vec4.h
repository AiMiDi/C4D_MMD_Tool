...
#endif
{
	...
	/// Reinterprets the vector as color.
	constexpr Col4<T, STRIDE>& GetColor() { return *(Col4<T, STRIDE>*) this; }
	...
	/// Adds two vectors
	template <typename T2, Int S2> constexpr Vec4& operator +=(const Vec4<T2, S2>& v)
	{
		x += v.x;
		y += v.y;
		z += v.z;
	...
	//----------------------------------------------------------------------------------------
	friend constexpr Unstrided Abs(const Vec4& v1)
	{
	...
	/// Set the minimum of each component.
	friend constexpr void SetMin(Vec4& a, const Vec4& other)
	{
		a.ClampMax(other);
	}

	/// Set the maximum of each component.
	friend constexpr void SetMax(Vec4& a, const Vec4& other)
	{
		a.ClampMin(other);
	}
	...
	friend constexpr T GetAngle(const Vec4& v1, const Vec4& v2)
	{
	...
	template <typename V> constexpr Vec4& operator *=(const SqrMat4<V>& m)
	{
