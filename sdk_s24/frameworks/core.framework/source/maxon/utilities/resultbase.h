...
public:
	...
	ResultBase() : _value(STD_IS_REPLACEMENT(const, T) ? (T*) &NullValue<typename std::conditional<STD_IS_REPLACEMENT(const, T), T&, const Int&>::type>() : const_cast<T*>(&GetZeroRef<T>())) { _error._error = nullptr; }
