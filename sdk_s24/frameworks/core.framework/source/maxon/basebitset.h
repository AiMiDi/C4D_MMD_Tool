...
public:
	...
	const IntType* GetRawData() const
	{
		return _bits.GetFirst();
	}

	static Bool IsSetUnchecked(const IntType* raw, Int index)
	{
		const Int intIndex = (Int)(index >> INT_WIDTH_EXPONENT);
		return (raw[intIndex] & (IntType(1) << (index & INT_MASK))) != 0;
	}
