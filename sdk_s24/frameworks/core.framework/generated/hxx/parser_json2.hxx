...
#if 1
constexpr const maxon::Char* JsonParserInterface::PrivateGetCppName() { return "maxon::JsonParserRef"; }
...
struct JsonParserInterface::MTable
{
	...
	Result<void> (*_JsonParserInterface_WriteFile) (JsonParserInterface* this_, UrlOrOutputStream&& out, const StringEncodingRef& stringEncoding, const DataDictionary& dict, Bool niceFormatting);
	template <typename IMPL> void Init()
	{
		...
		_JsonParserInterface_WriteFile = &IMPL::_JsonParserInterface_WriteFile;
	}
};
...
struct JsonParserInterface::Hxx2
{
	...
	public:
		...
		static Result<void> _JsonParserInterface_Write(JsonParserInterface* this_, const DataDictionary& jsonObject, String& jsonString, Bool niceFormatting) { return C::Get(this_)->Write(jsonObject, jsonString, niceFormatting); }
		static Result<void> _JsonParserInterface_Read(JsonParserInterface* this_, const String& jsonString, JSONPARSERFLAGS flags, const ValueReceiver<const DataDictionary&>& jsonObjects) { return C::Get(this_)->Read(jsonString, flags, jsonObjects); }
		static Result<void> _JsonParserInterface_Read_1(JsonParserInterface* this_, UrlOrInputStream&& nameOrStream, JSONPARSERFLAGS flags, const StringDecodingRef& stringDecoding, const ValueReceiver<const DataDictionary&>& jsonObjects) { return C::Get(this_)->Read(std::forward<UrlOrInputStream>(nameOrStream), flags, stringDecoding, jsonObjects); }
		static Result<void> _JsonParserInterface_WriteFile(JsonParserInterface* this_, UrlOrOutputStream&& out, const StringEncodingRef& stringEncoding, const DataDictionary& dict, Bool niceFormatting) { return C::Get(this_)->WriteFile(std::forward<UrlOrOutputStream>(out), stringEncoding, dict, niceFormatting); }
	};
...
MAXON_ATTRIBUTE_FORCE_INLINE auto JsonParserInterface::WriteFile(UrlOrOutputStream&& out, const StringEncodingRef& stringEncoding, const DataDictionary& dict, Bool niceFormatting) -> Result<void>
{
	return MTable::_instance._JsonParserInterface_WriteFile(this, std::forward<UrlOrOutputStream>(out), stringEncoding, dict, niceFormatting);
}
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto JsonParserInterface::Hxx1::ReferenceFunctionsImpl<S>::WriteFile(UrlOrOutputStream&& out, const StringEncodingRef& stringEncoding, const DataDictionary& dict, Bool niceFormatting) const -> Result<void>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, Result<void>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); JsonParserInterface* o_ = (JsonParserInterface*) this->GetPointer(); if (!(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR)) && MAXON_UNLIKELY(!o_)) { return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); } 
	return (MTable::_instance._JsonParserInterface_WriteFile(o_, std::forward<UrlOrOutputStream>(out), stringEncoding, dict, niceFormatting));
}
