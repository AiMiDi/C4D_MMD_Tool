...
public:
	...
	/// (after observers have been informed). You can override this method in subclasses
	...
	//----------------------------------------------------------------------------------------
	/// This method gets called immediately before a TransactionEnded() of a commit.
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD const NodeSystem& GetNodeSystem();
	...
	/// @return												A GraphNode referencing the added child.
	//----------------------------------------------------------------------------------------
	MAXON_FUNCTION Result<GraphNode> AddChild(const Id& childId, const NodeTemplate& templ, const TemplateArguments& args = maxon::NullValue<const TemplateArguments&>());
	...
	/// @param[in] child							The child node which shall be replaced.
	/// @param[in] base								Node system to use for the replacement.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> ReplaceChild(const GraphNode& child, const NodeSystem& base);

	//----------------------------------------------------------------------------------------
	/// Groups the selection of nodes of this graph into a new group node.
	/// groupRoot is used to set up the new group node. You can leave this empty, then a new
	/// node system will be created as an instance of a group asset which fits to the selection
	/// (group assets already used by the selection are considered). But you can also provide
	/// a valid node system.
	///
	...
	/// @param[in] groupRoot					The MutableRoot of another (usually empty) node system which will become the group, can be empty.
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<GraphNode> MoveToGroup(MutableRoot& groupRoot, const Id& groupId, const Block<const GraphNode>& selection);
	...
	/// @private
	[[deprecated("Use the other overload")]] MAXON_METHOD Result<ForwardRef<UserStateDerivationRef>> CreateUserStateDerivation();
	
	/// @private
	[[deprecated("Use the other overload")]] MAXON_METHOD Result<ForwardRef<UserStateDerivationRef>> CreateUserStateDerivation(const Block<const Id>& categories);

	MAXON_METHOD const GNodeHandle& PrivateGetHandle(const GraphNode& node);

	static MAXON_METHOD Result<GNode> PrivateToGNode(const GraphNode& node, NODE_KIND mask);

	static MAXON_METHOD Result<MutableGNode> PrivateToMutableGNode(const GraphNode& node, NODE_KIND mask);
...
public:
	...
	// #LEGACYCOMPATIBILITY: (Ole) improve method order
	MAXON_METHOD const NodeSystem& GetBase(const GraphNode& node);

	/// @copydoc GraphModelInterface::AddChild
...
{
	Result<GNode> res = NodesGraphModelInterface::PrivateToGNode(node, NODE_KIND::NODE);
...
{
	Result<GNode> res = NodesGraphModelInterface::PrivateToGNode(node, NODE_KIND::PORTLIST_MASK | NODE_KIND::PORT_MASK);
...
{
	Result<GNode> res = NodesGraphModelInterface::PrivateToGNode(node, NODE_KIND::PORT_MASK);
...
{
	Result<MutableGNode> res = NodesGraphModelInterface::PrivateToMutableGNode(node, NODE_KIND::NODE);
...
{
	Result<MutableGNode> res = NodesGraphModelInterface::PrivateToMutableGNode(node, NODE_KIND::PORTLIST_MASK | NODE_KIND::PORT_MASK);
...
{
	Result<MutableGNode> res = NodesGraphModelInterface::PrivateToMutableGNode(node, NODE_KIND::PORT_MASK);
