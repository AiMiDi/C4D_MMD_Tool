...
//		ARRAY_OUTPUT_HEAD = CONTAINER_OUTPUT_HEAD | DEFINITE_FACTOR_HEAD_MASK,
	...
	enum class FLAGS
	{
		...
		CURRENT_ITV = 1 << 6,
		NEXT_ITV = 1 << 7,
		CONTAINER_MEMBER = 1 << 8,
		COMMUTATIVE = 1 << 9,
		STATE_MAPPER = 1 << 10,
		PARAMETER = 1 << 12,
		INPUT_WITHOUT_VALUE = 1 << 13,
		OUTER_DOMAIN = 1 << 14,
		NO_FACTORS = 1 << 15, // flag for CreatePort
		SUSPEND = 1 << 16,
		KEEP_LETTER_CASE = 1 << 17,
		INDEPENDENT = 1 << 18,
		INCLUDE_IN_PREDECESSORS = 1 << 19,
		FACTOR_REFERENCE = 1 << 20,
		OPTIONAL_INPUT = 1 << 21,
		CURRENT_ITV_GROUP = 1 << 22,
		NEXT_ITV_GROUP = 1 << 23,
		HIDDEN = 1 << 24,
		NEW_UNIQUE_NAME = 1 << 25,
		TIME_LOOP = 1 << 26,
		ITV_MASK = CURRENT_ITV | NEXT_ITV,
		ITV_GROUP_MASK = CURRENT_ITV_GROUP | NEXT_ITV_GROUP,
		PORT_MODIFIERS = PERMANENT | CACHED | DONT_WRAP | ITV_MASK | KEEP_LETTER_CASE | OPTIONAL_INPUT | HIDDEN,
