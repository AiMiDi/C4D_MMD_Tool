...
{
	BrowserCallSR(GetStaticIcon, nullptr)(id, pref_wh);
}
...
{
	BrowserCallSR(GetIcon, nullptr)(url, plugin, flags, preview_time);
}
...
{
	BrowserCallSR(GetPreviewIcon, nullptr)(node, pref_wh, flags);
}

SDKBrowserIcon	*SDKBrowser::CreateIcon(BaseBitmap *bm, Bool dont_free)
{
	BrowserCallSR(CreateIcon, nullptr)(bm, dont_free);
}
...
{
	BrowserCallSR(GetDefaultObject, nullptr)(id, flags);
}
...
{
	BrowserCallSR(AllocPresetBrowser, nullptr)(db_index);
}
...
{
	BrowserContentNodeCallSR(GetNodeInfo, nullptr)(this, plugin_id);
}
...
{
	BrowserContentNodeCallR(GetPlugin, nullptr)(flags);					// Return pointer to the plugin that can deal with this file (see GetFileKind)
}
...
{
	BrowserContentNodeCallR(GetIcon, nullptr)(flags, pref_wh);	// Return an icon for this kind of file (does not have to be a preview for this specific file)	pref_wh is the preferred icon size
}
...
Result<BaseArray<Url>> BrowserReceiveUrls(void* dat)
{
	...
	CINEWARE_NAMESPACE::SDKBrowserDragInfo* bdi;

	bdi = (CINEWARE_NAMESPACE::SDKBrowserDragInfo*)dat;
	if (!bdi)
	...
	{
		CINEWARE_NAMESPACE::SDKBrowserContentNodeRef item;
		CINEWARE_NAMESPACE::SDKBrowserPluginRef			 plugin;
		CINEWARE_NAMESPACE::SDKBrowserURL						 url;

		item = bdi->GetItem(idx);

		if (item->IsLink()) // is this a link to another node?
		{
			item = CINEWARE_NAMESPACE::SDKBrowser::FindNode(item->GetNodeURL(LinkThrough)); // try to find the linked node
			if (!item)
				continue;
		}
		...
		if (type == CINEWARE_NAMESPACE::CBPluginTypeFilePreset)
		{
			// preset://
			CINEWARE_NAMESPACE::Filename fileName = url.GetString();
			Url			 u = MaxonConvert(fileName, CINEWARE_NAMESPACE::MAXONCONVERTMODE::NONE);
			urlArray.Append(u) iferr_return;
		}
		else
		{
			// file:// etc.
			CINEWARE_NAMESPACE::Filename fileName = url.GetFilename();
			Url			 u = MaxonConvert(fileName, CINEWARE_NAMESPACE::MAXONCONVERTMODE::NONE);
			urlArray.Append(u) iferr_return;
		}
	}
