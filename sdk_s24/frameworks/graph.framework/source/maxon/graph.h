...
enum class WIRE_MODE : UChar
{
	...
	AMBIGUOUS = 61,								///< Used for shortcut wires when the sources are ambiguous.
...
public:
	...
	enum class GET_CONNECTIONS_MODE
	{
		...
		SHORTCUTS = 2,							///< Report shortcut connections.
		EXTERNAL_CONNECTIONS = 4,		///< Only report connections which leave the node to which the port belongs (so they are external to the node).
		WITHOUT_INHIBITED = 8,			///< Don't report inhibited connections (those where Wires::inhibit is present).
		INDUCED_DEPENDENCIES = 16,	///< Report induced dependencies (due to loop dependencies or sequencing).
...
protected:
	...
	MAXON_METHOD Result<Bool> GetChildren(const GraphNode& node, const ValueReceiver<const GraphNode&>& callback, NODE_KIND filter);
	...
	MAXON_METHOD Result<GraphNode> FindChild(const GraphNode& node, const InternedId& name, NODE_KIND kind);
	...
	MAXON_METHOD Result<Bool> GetConnections(const GraphNode& node, PORT_DIR dir, const ValueReceiver<const GraphConnection&>& callback, Wires mask, GET_CONNECTIONS_MODE mode);

	MAXON_METHOD Result<Wires> GetWires(const GraphNode& source, const GraphNode& target);

	MAXON_METHOD Result<void> Connect(const GraphNode& source, const GraphNode& target, Wires wires);
	...
	[[deprecated]] MAXON_METHOD Result<Int> BeginTransactionImpl();
	...
	MAXON_METHOD Result<Bool> IsConnectedTo(const GraphNode& node, const String& portName);

	MAXON_METHOD Result<Bool> IsConnectedTo(const GraphNode& node, const Id& portID);

	MAXON_METHOD Result<void> Connect(const GraphNode& target, const String& portName);

	MAXON_METHOD Result<void> Connect(const GraphNode& target, const Id& portId);

	MAXON_METHOD Result<void> MuteConnectionWith(const GraphNode& src, const String& portName);

	MAXON_METHOD Result<void> MuteConnectionWith(const GraphNode& node, const Id& portId);
	
	MAXON_METHOD Result<void> UnmuteConnectionWith(const GraphNode& node, const String& portName);

	MAXON_METHOD Result<void> UnmuteConnectionWith(const GraphNode& node, const Id& portId);

	MAXON_METHOD Result<Bool> IsSelected(const GraphNode& node) const;

	MAXON_METHOD Result<GraphNode> CreateOutputPort(const GraphNode& node, const String& name, const CString id = CString("output"));

	MAXON_METHOD Result<GraphNode> CreateInputPort(const GraphNode& node, const String& name, const CString& id = CString("input"));

	MAXON_METHOD Result<void> GetDirectPredecessors(const GraphNode& node, const ValueReceiver<const GraphNode&>& callback, const NODE_KIND& filter);

	MAXON_METHOD Result<void> GetAllPredecessors(const GraphNode& node, const ValueReceiver<const GraphNode&>& callback, const NODE_KIND& filter);

	MAXON_METHOD Result<void> GetDirectSuccessors(const GraphNode& node, const ValueReceiver<const GraphNode&>& callback, const NODE_KIND& filter);

	MAXON_METHOD Result<void> GetAllSuccessors(const GraphNode& node, const ValueReceiver<const GraphNode&>& callback, const NODE_KIND& filter);

	MAXON_METHOD Result<Bool> IsConnectedTo(const GraphNode& node, const GraphNode& port);

	MAXON_METHOD Result<void> MuteConnectionWith(const GraphNode& node, const GraphNode& port);

	MAXON_METHOD Result<void> MuteAllConnections(const GraphNode& node, const PORT_DIR& direction);

	MAXON_METHOD Result<void> UnmuteConnectionWith(const GraphNode& node, const GraphNode& port);

	MAXON_METHOD Result<void> UnmuteAllConnections(const GraphNode& node, const PORT_DIR& direction);

	MAXON_METHOD Result<Bool> RemoveAllConnections(const GraphNode& node);
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> SelectWire(const NodePath& src, const NodePath& dst);
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> DeselectWire(const GraphNode& src, const GraphNode& dst);
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> GetSelectedWires(const ValueReceiver<const GraphNode&, const GraphNode&, const Wires&>& callback);
	
	//----------------------------------------------------------------------------------------
	/// Deselects the True Node passed as an argument.
	/// @param[in] node								The True Node to Deselect
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> DeselectTrueNode(const GraphNode& node);

	//----------------------------------------------------------------------------------------
	/// Deselects the Port passed as an argument.
	/// @param[in] port								The Port to Deselect
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> DeselectPort(const GraphNode& port);

	//----------------------------------------------------------------------------------------
	/// Selects a node or a port.
	/// @param[in] node 						the node or the port that have to be selected.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> SelectNode(const GraphNode& node);

	//----------------------------------------------------------------------------------------
	/// Deselects a node or a port.
	/// @param[in] node 						the node or the port that have to be Deselected.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> DeselectNode(const GraphNode& node);
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> ListAllNodes(const ValueReceiver<const GraphNode&>& callback, const DataDictionary& matchingData);
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<Bool> IsTrueNodeSelected(const GraphNode& node) const;
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<Bool> IsPortSelected(const GraphNode& port) const;
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<Bool> IsWireSelected(const NodePath& src, const NodePath& dst);
  ...
  //----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> GetSelectedNodes(const ValueReceiver<const GraphNode&>& callback, const NODE_KIND &kind);
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> DeselectAll(const NODE_KIND &kind);
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> RemoveAllConnections(const GraphNode &src, const GraphNode &dst);
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Commits (end ends) this transaction. The changes made within this transaction become
...
public:
	...
	/// @return												The attribute value of the connection, may be an empty Opt.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> Result<Opt<const typename ATTR::ValueType&>> GetConnectionValue(const GraphNode& target, CONNECTION_POSITION position, Bool reverse = false) const
	{
		iferr_scope;
		ConstDataPtr ptr = GetConnectionValue(target, position, ATTR::GetInstance(), maxon::GetDataType<typename ATTR::ValueType>(), reverse) iferr_return;
	...
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->SetConnectionValue(this->GetGraphNode(), target, position, attr, value);
	}
	...
	/// @return												True if the value has been changed.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> Result<Bool> SetConnectionValue(const GraphNode& target, CONNECTION_POSITION position, const typename ATTR::ValueType& value) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->SetConnectionValue(this->GetGraphNode(), target, position, ATTR::GetInstance(), ConstDataPtr(value));
	}
	...
	/// @return												True if the value has been changed.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> Result<Bool> SetConnectionValue(const GraphNode& target, CONNECTION_POSITION position, typename ATTR::ValueType&& value) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->SetConnectionValue(this->GetGraphNode(), target, position, ATTR::GetInstance(), MoveDataPtr(std::move(value)));
	}
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	Result<void> Connect(const GraphNode& target, Wires modes = WIRE_MODE::NORMAL, Bool reverse = false) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @copydoc Connect
	Result<void> Connect(const Result<typename SFINAEHelper<GraphNode, BASE>::type>& target, Wires modes = WIRE_MODE::NORMAL, Bool reverse = false) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		if (target == FAILED)
			return target;
	...
	//----------------------------------------------------------------------------------------
	Result<void> RemoveConnections(PORT_DIR dir, Wires mask) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return												True if the value has been changed.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> Result<Bool> SetValue(const typename ATTR::ValueType& value, Bool checkAndInvalidate = true) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->SetValue(this->GetGraphNode(), ATTR::GetInstance(), ConstDataPtr(value), checkAndInvalidate);
	}
	...
	/// @return												True if the value has been changed.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> Result<Bool> SetValue(typename ATTR::ValueType&& value, Bool checkAndInvalidate = true) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->SetValue(this->GetGraphNode(), ATTR::GetInstance(), MoveDataPtr(std::move(value)), checkAndInvalidate);
	}
	...
	//----------------------------------------------------------------------------------------
	Result<typename SFINAEHelper<GraphNode, BASE>::type> CreateOutputPort(const String& name, const CString& id = CString("output"))
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->CreateOutputPort(this->GetGraphNode(), name, id);
	}
	...
	/// @return												The created port.
	//----------------------------------------------------------------------------------------
	Result<typename SFINAEHelper<GraphNode, BASE>::type> CreateInputPort(const String& name, const CString& id = CString("input"))
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->CreateInputPort(this->GetGraphNode(), name, id);
	}
	...
	//----------------------------------------------------------------------------------------
	Result<void> GetDirectPredecessors(const ValueReceiver<const GraphNode&>& callback, NODE_KIND filter = NODE_KIND::NODE) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @param[in] filter										The NODE_KIND that will be included
		//----------------------------------------------------------------------------------------
	Result<void> GetAllPredecessors(const ValueReceiver<const GraphNode&>& callback, NODE_KIND filter = NODE_KIND::NODE) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @param[in] filter										The NODE_KIND that will be included.
	//----------------------------------------------------------------------------------------
	Result<void> GetDirectSuccessors(const ValueReceiver<const GraphNode&>& callback, NODE_KIND filter = NODE_KIND::NODE) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @param[in] callback								The callback that will receive all connected nodes.
	/// @param[in] filter										The NODE_KIND that will be filtered out.
	//----------------------------------------------------------------------------------------
	Result<void> GetAllSuccessors(const ValueReceiver<const GraphNode&>& callback, NODE_KIND filter = NODE_KIND::NODE) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	//----------------------------------------------------------------------------------------
	Result<Bool> IsConnectedTo(const GraphNode& port) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														@trueIfOtherwiseFalse{ports are connected}
	//----------------------------------------------------------------------------------------
	Result<Bool> IsConnectedTo(const String& portName) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														@trueIfOtherwiseFalse{ports are connected}
	//----------------------------------------------------------------------------------------
	Result<Bool> IsConnectedTo(const Id& portId) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	//----------------------------------------------------------------------------------------
	Result<void> Connect(const GraphNode& dst, const String &portName )
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	//----------------------------------------------------------------------------------------
	Result<void> Connect(const GraphNode& dst, const Id& portId)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> MuteConnectionWith(const GraphNode& port)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> MuteConnectionWith(const String& portName)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> MuteConnectionWith(const Id& portId)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> MuteAllConnections(PORT_DIR direction)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> UnmuteConnectionWith(const GraphNode& port)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> UnmuteConnectionWith(const String& portName)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> UnmuteConnectionWith(const Id& portId)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	/// @return														if error
	//----------------------------------------------------------------------------------------
	Result<void> UnmuteAllConnections(PORT_DIR direction)
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
	...
	//----------------------------------------------------------------------------------------
	Result<Bool> RemoveAllConnections()
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
