...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD const PathBlock& ToBlock() const;

	/// @copydoc ToBlock
	MAXON_FUNCTION operator const PathBlock&() const
	{
		return ToBlock();
	}
	...
	/// or the port itself it this is already a direct child of a port list. For example in the node
	/// @graph
	/// +---------+
	/// o x       |
	///  o a      |
	...
	{
		return ToBlock().Slice(GetNodeDepth() + 1);
	}
