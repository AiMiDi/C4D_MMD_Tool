...
struct FFTInterface::MTable
{
	...
	{
		if (PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, FFTInterface, Transform1D))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, FFTInterface, Transform1D), W, MTable>::type::_FFTInterface_Transform1D;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, FFTInterface, Transform2D))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, FFTInterface, Transform2D), W, MTable>::type::_FFTInterface_Transform2D;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, FFTInterface, GetSupportOptions))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, FFTInterface, GetSupportOptions), W, MTable>::type::_FFTInterface_GetSupportOptions;
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto FFTInterface::Hxx1::ConstReferenceFunctionsImpl<S>::Transform1D(const ArrayInterface<Generic>& in, WritableArrayInterface<Generic>& out, FFT_FLAGS fftFlags) const -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto FFTInterface::Hxx1::ConstReferenceFunctionsImpl<S>::Transform2D(const ConstMatrixNxMInterface& in, const MatrixNxMInterface& out, FFT_FLAGS fftFlags) const -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto FFTInterface::Hxx1::ConstReferenceFunctionsImpl<S>::GetSupportOptions() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<FFT_SUPPORT>, FFT_SUPPORT>::type
{
