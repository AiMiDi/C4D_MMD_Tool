...
/// @return												Return an instance to a MediaSessionRef for further reading if necessary.
		static inline Result<ImageTextureRef> LoadTexture(const Url& name, const TimeValue& targetTime = TimeValue(), MEDIASESSIONFLAGS flags = MEDIASESSIONFLAGS::NONE, const Class<ImagePixelStorage>& pixelStorageClass = ImagePixelStorageClasses::Normal(), const LayerSetInterface* layerSet = nullptr);
...
/// @param[in] layerSet						Optional LayerSet or nullptr.
/// @return												Return an instance to a MediaSessionRef for further reading if necessary.
		inline Result<MediaSessionRef> Load(const Url& url, const TimeValue& targetTime, MEDIASESSIONFLAGS flags, const Class<ImagePixelStorage>& pixelStorageClass = ImagePixelStorageClasses::Normal(), const LayerSetInterface* layerSet = nullptr) const;
	};
	/// Intermediate helper class for ImageTextureInterface.
	template <typename S> class COWReferenceFunctionsImpl : public ConstReferenceFunctionsImpl<S>
	{
...
/// @param[in] layerSet						Optional LayerSet or nullptr.
/// @return												Return an instance to a MediaSessionRef for further reading if necessary.
		inline Result<MediaSessionRef> Load(const Url& url, const TimeValue& targetTime, MEDIASESSIONFLAGS flags, const Class<ImagePixelStorage>& pixelStorageClass = ImagePixelStorageClasses::Normal(), const LayerSetInterface* layerSet = nullptr);
	};
