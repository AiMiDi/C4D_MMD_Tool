...
{
	return CheckLib<CustomGuiSplineLib>(CUSTOMGUI_SPLINE, offset, lib_SplineControl);
}

#define SplineCall(b)					cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iSpline*>(this)->*lib->b)

#define SplineCallR(a, b)     cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iSpline*>(this)->*lib->b)

#define SplineCallRC(a, b)    cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const cinema::iSpline*>(this)->*lib->b)

#define SplineGuiCall(b)     cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iSplineCustomGui*>(this)->*lib->b)

#define SplineGuiCallC(b)     cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const cinema::iSplineCustomGui*>(this)->*lib->b)

#define SplineGuiCallR(a, b) cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iSplineCustomGui*>(this)->*lib->b)

#define SplineGuiCallRC(a, b) cinema::CustomGuiSplineLib *lib = cinema::CheckCustomGuiSplineLib(LIBOFFSET(cinema::CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const cinema::iSplineCustomGui*>(this)->*lib->b)


Int32 SplineData::GetKnotCount() const { SplineCallRC(-1, GetKnotCount)(); }
CustomSplineKnot* SplineData::GetKnot(Int32 l) { SplineCallR(nullptr, GetKnot)(l); }
const CustomSplineKnot* SplineData::GetKnot(Int32 l) const { return (const CustomSplineKnot*)const_cast<SplineData*>(this)->GetKnot(l); }
Bool SplineData::SetKnot(Int32 l, const CustomSplineKnot& knot) { SplineCallR(false, SetKnot)(l, knot); }
Vector SplineData::GetPoint(Float r) const { SplineCallRC(Vector(0, 0, 0), GetPoint)(r); }
...
Int32 SplineData::GetSelectCount() const { SplineCallRC(0, GetSelectCount)(); }
void SplineData::SetUserCallback(SplineDataCallback fn, void* pData) { SplineCall(SetUserCallback)(fn, pData); }
SplineDataCallback SplineData::GetUserCallback(void *&pData) const { SplineCallRC(nullptr, GetUserCallback)(pData); }
...
Bool SplineData::CopyTo(SplineData* pDest) const { SplineCallRC(false, CopyTo)(pDest); }
maxon::Result<void> SplineData::GetSplineMapper(maxon::SplineMapper& splineMapper) const { SplineCallRC(maxon::FunctionNotImplementedError(MAXON_SOURCE_LOCATION), GetSplineMapper)(splineMapper); }

void SplineData::SetTension(Float tension) { SplineCall(SetTension)(tension); }
Float SplineData::GetTension() const { SplineCallRC(0.0, GetTension)(); }
...
void SplineCustomGui::SetMessageFunctions(SplineControlMessages* pFuncs)
{
	...
	(reinterpret_cast<iSplineCustomGui*>(this)->*lib->SetMessageFunctions)(f, sizeof(SplineControlFunctionTable), &SplineControlFunctions, pFuncs);
}


SplineData* SplineCustomGui::GetSplineData() { SplineGuiCallR(nullptr, GetSplineData)(); }
...
Bool SplineData::GetRange(Float* xmin, Float* xmax, Float* xsteps, Float* ymin, Float* ymax, Float* ysteps) const
	{ SplineCallRC(false, GetRange)(xmin, xmax, xsteps, ymin, ymax, ysteps); }
