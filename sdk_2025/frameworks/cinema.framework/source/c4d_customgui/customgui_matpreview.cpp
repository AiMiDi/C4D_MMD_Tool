...
{
	return CheckLib<CustomGuiMatPreviewLib>(CUSTOMGUI_MATPREVIEW, offset, lib_MatPreview);
}

#define MatPreviewCall(b) 			cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																			if (!lib || !lib->b) return; \
																			(reinterpret_cast<cinema::iMatPreviewData*>(this)->*lib->b)

#define MatPreviewCallR(a, b) 		cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																			if (!lib || !lib->b) return a; \
																			return (reinterpret_cast<cinema::iMatPreviewData*>(this)->*lib->b)

#define MatPreviewCallRC(a, b) 		cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																			if (!lib || !lib->b) return a; \
																			return (reinterpret_cast<const cinema::iMatPreviewData*>(this)->*lib->b)

#define MatPreviewGuiCall(b) 		cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																			if (!lib || !lib->b) return; \
																			(reinterpret_cast<cinema::iMatPreviewCustomGui*>(this)->*lib->b)

#define MatPreviewGuiCallR(a, b) cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																		if (!lib || !lib->b) return a; \
																		return (reinterpret_cast<cinema::iMatPreviewCustomGui*>(this)->*lib->b)

#define MatPreviewCallFn(b)     cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																		if (!lib || !lib->b) return; \
																		lib->b

#define MatPreviewCallFnR(a, b) cinema::CustomGuiMatPreviewLib *lib = cinema::CheckGuiMatPreviewLib(LIBOFFSET(cinema::CustomGuiMatPreviewLib, b)); \
																		if (!lib || !lib->b) return a; \
																		return lib->b
...
Bool MaterialPreviewData::GetRegion(Float &l, Float &t, Float &r, Float &b) const
{
	MatPreviewCallRC(false, GetRegion)(l, t, r, b);
}
...
void MaterialPreviewData::CopyLinearWorkflow(const BaseDocument *src, BaseDocument *previewDocument)
{
	MatPreviewCallFn(CopyLinearWorkflow)(src, previewDocument);
}
