...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE TP_BaseCollision {};
...
#if defined(MAXON_COMPILER_CLANG) && MAXON_COMPILER_CLANG < 1200
...
#if defined(MAXON_COMPILER_CLANG) && MAXON_COMPILER_CLANG < 1200
	#pragma clang diagnostic enabled "-Wdocumentation"
#endif
};
...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE TP_BaseShape {};
...
{
	Int32		(TP_MasterSystem::*AllocParticle				)();
	Int32		(TP_MasterSystem::*AllocParticles				)(Int32 num, Int32 *ids);
	void		(TP_MasterSystem::*FreeParticle					)(Int32 pid);
	void		(TP_MasterSystem::*FreeAllParticles			)();

	TP_PGroup*				(TP_MasterSystem::*AllocParticleGroup					)();
	...
	BaseObject*				(TP_MasterSystem::*GetVirtualObjects					)(TP_PGroup *ingroup, Bool inRender, Bool subgroups, const HierarchyHelp *hh);
	TP_PGroupInfo*		(TP_MasterSystem::*GetGroupInfo								)(TP_PGroup *group);
	TP_PGroup*				(TP_MasterSystem::*GetGroupFromInfo						)(TP_PGroupInfo *info);

	Int32						(TP_MasterSystem::*NumParticles			)() const;
	Bool						(TP_MasterSystem::*Alive						)(Int32 pid) const;
	Bool						(TP_MasterSystem::*IsBorn						)(Int32 pid) const;
	Bool						(TP_MasterSystem::*IsDie						)(Int32 pid) const;
	Bool						(TP_MasterSystem::*EntersGroup			)(Int32 pid) const;
	TP_PGroup*			(TP_MasterSystem::*Group						)(Int32 pid);
	Vector					(TP_MasterSystem::*Position					)(Int32 pid) const;
	Vector					(TP_MasterSystem::*Velocity					)(Int32 pid) const;
	Float						(TP_MasterSystem::*Mass							)(Int32 pid) const;
	TP_Spin					(TP_MasterSystem::*Spin							)(Int32 pid) const;
	Float						(TP_MasterSystem::*Size							)(Int32 pid) const;
	Vector					(TP_MasterSystem::*Scale						)(Int32 pid) const;
	BaseTime				(TP_MasterSystem::*Age							)(Int32 pid) const;
	BaseTime				(TP_MasterSystem::*Life							)(Int32 pid) const;
	Matrix					(TP_MasterSystem::*Alignment				)(Int32 pid) const;
	Int32						(TP_MasterSystem::*Randomseed				)(Int32 pid) const;
	TP_PShapePtr		(TP_MasterSystem::*Shape						)(Int32 pid) const;
	Matrix					(TP_MasterSystem::*Transform				)(Int32 pid) const;
	UInt16					(TP_MasterSystem::*Flags						)(Int32 pid) const;
	Float						(TP_MasterSystem::*DTFactor					)(Int32 pid) const;
	...
	Int32				(TP_MasterSystem::*NumDataChannels					)();
	...
	UInt32		(TP_MasterSystem::*GetDirty			)() const;
	void			(TP_MasterSystem::*SetDirty			)();
	...
	BaseObject*			(TP_MasterSystem::*GetShapeVirtualObjects			)(const TP_PShapePtr &shape , Int32 pid, BaseDocument *doc, const HierarchyHelp *hh);
...
{
	Int32								(TP_PGroup::*GetLevel						)();
	TP_ParticleNode*		(TP_PGroup::*GetFirstNode				)();
	TP_ParticleNode*		(TP_PGroup::*GetLastNode				)();
	Int32								(TP_PGroup::*NumParticles				)();
	Bool								(TP_PGroup::*IsSubGroup					)(TP_PGroup *group);
	Int32								(TP_PGroup::*GetGroupID					)();
	const String				(TP_PGroup::*GetTitle						)();
	void								(TP_PGroup::*SetTitle						)(const maxon::String &title);
	void								(TP_PGroup::*SetViewType				)(Int32 type);
	Int32								(TP_PGroup::*GetViewType				)();
	void								(TP_PGroup::*SetShowObjects			)(Bool show);
	Bool								(TP_PGroup::*GetShowObjects			)();
	void								(TP_PGroup::*SetColor						)(Vector &col);
	Vector							(TP_PGroup::*GetColor						)();
	Bool								(TP_PGroup::*EditSettings				)();
	Bool								(TP_PGroup::*IsSelected					)();
	Bool								(TP_PGroup::*IsOpened						)();
	void								(TP_PGroup::*Select							)(Int32 mode);
	void								(TP_PGroup::*Open								)(Bool onoff);
	void								(TP_PGroup::*SetUseColor				)(Bool useCol);
	Bool								(TP_PGroup::*GetUseColor				)();
	void								(TP_PGroup::*Cache							)(Bool onoff);
	Bool								(TP_PGroup::*IsCache						)();
};

struct C4D_TP_PARTICLENODE
{
	Int32								(TP_ParticleNode::*GetParticleID			)() const;
	TP_ParticleNode*		(TP_ParticleNode::*GetNext						)();
	TP_ParticleNode*		(TP_ParticleNode::*GetPrev						)();
};
...
#define TpCall(fnc) (this->*cinema::TPOS->MSYS->fnc)
...
public:
	...
	//----------------------------------------------------------------------------------------
	Int32 AllocParticle() { return TpCall(AllocParticle)(); }
	...
	//----------------------------------------------------------------------------------------
	void FreeAllParticles() { TpCall(FreeAllParticles)(); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the number of allocated particles. All particles IDs are less than this value.
	/// @return												The number of particles.
	//----------------------------------------------------------------------------------------
	Int32 NumParticles() const { return TpCall(NumParticles)(); }
	...
	//----------------------------------------------------------------------------------------
	TP_PGroup* AllocParticleGroup() { return TpCall(AllocParticleGroup)(); }
	...
	{
		return const_cast<TP_MasterSystem*>(this)->GetParticleGroups(const_cast<TP_PGroup*>(ingroup), reinterpret_cast<TP_PGroupArray*>(gtab), mode, subgroups);
	}
	...
	//----------------------------------------------------------------------------------------
	BaseObject* GetVirtualObjects(TP_PGroup *ingroup, Bool inRender = true, Bool subgroups = true, const HierarchyHelp *hh = nullptr) { return TpCall(GetVirtualObjects)(ingroup, inRender, subgroups, hh); }
	...
	//----------------------------------------------------------------------------------------
	Bool Alive(Int32 pid) const { return TpCall(Alive)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsBorn(Int32 pid) const { return TpCall(IsBorn)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsDie(Int32 pid) const { return TpCall(IsDie)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Bool EntersGroup(Int32 pid) const { return TpCall(EntersGroup)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Vector Position(Int32 pid) const { return TpCall(Position)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Vector Velocity(Int32 pid) const { return TpCall(Velocity)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Float Mass(Int32 pid) const { return TpCall(Mass)(pid); }
	...
	//----------------------------------------------------------------------------------------
	TP_Spin Spin(Int32 pid) const { return TpCall(Spin)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Float Size(Int32 pid) const { return TpCall(Size)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Vector Scale(Int32 pid) const { return TpCall(Scale)(pid); }
	...
	//----------------------------------------------------------------------------------------
	BaseTime Age(Int32 pid) const { return TpCall(Age)(pid); }
	...
	//----------------------------------------------------------------------------------------
	BaseTime Life(Int32 pid) const { return TpCall(Life)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Matrix Alignment(Int32 pid) const { return TpCall(Alignment)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Int32 Randomseed(Int32 pid) const { return TpCall(Randomseed)(pid); }
	...
	//----------------------------------------------------------------------------------------
	TP_PShapePtr Shape(Int32 pid) const { return TpCall(Shape)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Matrix Transform(Int32 pid) const { return TpCall(Transform)(pid); }
	...
	//----------------------------------------------------------------------------------------
	UInt16 Flags(Int32 pid) const { return TpCall(Flags)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Float DTFactor(Int32 pid) const { return TpCall(DTFactor)(pid); }
	...
	//----------------------------------------------------------------------------------------
	Int32 NumDataChannels() { return TpCall(NumDataChannels)(); }
	...
	//----------------------------------------------------------------------------------------
	UInt32 GetDirty() const { return TpCall(GetDirty)(); }
	...
	/// @return												The particle shape object. @theOwnsPointed{particle shape handler,object}
	//----------------------------------------------------------------------------------------
	BaseObject* GetShapeVirtualObjects(const TP_PShapePtr &shape , Int32 pid, BaseDocument *doc, const HierarchyHelp *hh) { return TpCall(GetShapeVirtualObjects)(shape, pid, doc, hh); }
...
#define PartCall(fnc) (this->*cinema::TPOS->PART->fnc)
...
public:
	...
	//----------------------------------------------------------------------------------------
	Int32 GetParticleID() const { return PartCall(GetParticleID)(); }
	...
	//----------------------------------------------------------------------------------------
	TP_ParticleNode* GetNext() { return PartCall(GetNext)(); }

	const TP_ParticleNode* GetNext() const { return const_cast<TP_ParticleNode*>(this)->GetNext(); }
	...
	//----------------------------------------------------------------------------------------
	TP_ParticleNode* GetPrev() { return PartCall(GetPrev)(); }
};


#define PGroupCall(fnc) (this->*cinema::TPOS->GROUP->fnc)
...
public:
	...
	//----------------------------------------------------------------------------------------
	Int32 GetLevel() { return PGroupCall(GetLevel)(); }

	//----------------------------------------------------------------------------------------
	/// Gets the first node in the particle list of the group.
	/// @return												The first node. @theOwnsPointed{list,TP_ParticleNode}
	//----------------------------------------------------------------------------------------
	TP_ParticleNode* GetFirstNode() { return PGroupCall(GetFirstNode)(); }

	const TP_ParticleNode* GetFirstNode() const { return const_cast<TP_PGroup*>(this)->GetFirstNode(); }

	//----------------------------------------------------------------------------------------
	/// Gets the last node in the particle list of the group.
	/// @return												The last node. @theOwnsPointed{list,TP_ParticleNode}
	//----------------------------------------------------------------------------------------
	TP_ParticleNode* GetLastNode() { return PGroupCall(GetLastNode)(); }

	//----------------------------------------------------------------------------------------
	/// Calculates the number of particles in the group.
	/// @return												The particle count.
	//----------------------------------------------------------------------------------------
	Int32 NumParticles() { return PGroupCall(NumParticles)(); }
	...
	/// @return												The group ID.
	//----------------------------------------------------------------------------------------
	Int32 GetGroupID() { return PGroupCall(GetGroupID)(); }

	//----------------------------------------------------------------------------------------
	/// Gets the group title.
	/// @return												The group title.
	//----------------------------------------------------------------------------------------
	const String GetTitle() { return PGroupCall(GetTitle)(); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetViewType() { return PGroupCall(GetViewType)(); }
	...
	//----------------------------------------------------------------------------------------
	Bool GetShowObjects() { return PGroupCall(GetShowObjects)(); }
	...
	//----------------------------------------------------------------------------------------
	Vector GetColor() { return PGroupCall(GetColor)(); }
...
//----------------------------------------------------------------------------------------
inline Bool InitThinkingParticles()
{
	return nullptr != CheckLib(ID_THINKINGPARTICLES, 0, TPOS);
}
