...
MAXON_CONFIGURATION_BOOL(g_enableTrackFieldLayer, true, maxon::CONFIGURATION_CATEGORY::DEVELOPMENT, "Enable the solid track field layer.");
...
FieldInput FieldInput::GetSubBlock(Int offset, Int blockSize) const
{
	...
	if (_uniqueId.IsPopulated())
		result._uniqueId.Set(&_uniqueId[offset], result._blockCount);
...
Bool FieldInput::IsValid() const
{
	...
	if (MAXON_UNLIKELY(_uniqueId.IsPopulated() && (_uniqueId.GetCount() < GetCount())))
		return false;
