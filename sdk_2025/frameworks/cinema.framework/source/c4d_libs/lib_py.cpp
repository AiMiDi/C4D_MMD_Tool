...
{
	return CheckLib<PyLibrary>(LIBRARY_PY, offset, lib_py);
}

#define PyLibCall(b) 		cinema::PyLibrary *lib = cinema::CheckPyLib(LIBOFFSET(cinema::PyLibrary, b)); \
															if (!lib || !lib->b) return; \
															(lib->b)

#define PyLibCallR(a, b)  cinema::PyLibrary *lib = cinema::CheckPyLib(LIBOFFSET(cinema::PyLibrary, b)); \
															if (!lib || !lib->b) return a; \
															return (lib->b)

#define PyPyRCall(a, b, c)	cinema::PyLibrary *lib = cinema::CheckPyLib(LIBOFFSET(cinema::PyLibrary, a)); \
														if (!lib || !lib->a) return c; \
														return (reinterpret_cast<b*>(this)->*(lib->a))

#define PyPyCall(a, b)	cinema::PyLibrary *lib = cinema::CheckPyLib(LIBOFFSET(cinema::PyLibrary, a)); \
												if (!lib || !lib->a) return; \
												return (reinterpret_cast<b*>(this)->*(lib->a))
