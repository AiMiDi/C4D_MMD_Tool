...
{
	return CheckLib<GeClipMapLib>(LIB_CLIPMAP, offset, lib_GeClipMap);
}

#define GeClipMapCall(b)      cinema::GeClipMapLib *lib = cinema::CheckLib(LIBOFFSET(cinema::GeClipMapLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iGeClipMap*>(this)->*lib->b)

#define GeClipMapCallC(b)      cinema::GeClipMapLib *lib = cinema::CheckLib(LIBOFFSET(cinema::GeClipMapLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const cinema::iGeClipMap*>(this)->*lib->b)

#define GeClipMapCallR(a, b)   cinema::GeClipMapLib *lib = cinema::CheckLib(LIBOFFSET(cinema::GeClipMapLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iGeClipMap*>(this)->*lib->b)

#define GeClipMapCallRC(a, b)   cinema::GeClipMapLib *lib = cinema::CheckLib(LIBOFFSET(cinema::GeClipMapLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const cinema::iGeClipMap*>(this)->*lib->b)
...
{
	GeClipMapCallC(GetDim )(w, h);
}

Int32 GeClipMap::GetBw() const
{
	GeClipMapCallRC(NOTOK, GetBw )();
}

Int32 GeClipMap::GetBh() const
{
	GeClipMapCallRC(NOTOK, GetBh )();
}
...
{
	GeClipMapCallRC(false, SupportsDrawBezierSegment)();
}
...
const BaseContainer& GeClipMap::GetFontListHierarchical()
{
	GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, GetFontListHierarchical));
	if (lib && lib->GetFontListHierarchical)
		return lib->GetFontListHierarchical();

	return maxon::GetZeroRef<BaseContainer>();
}

const BaseContainer& GeClipMap::GetFontListFlat()
{
	GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, GetFontListFlat));
	if (lib && lib->GetFontListFlat)
		return lib->GetFontListFlat();

	return maxon::GetZeroRef<BaseContainer>();
}

void GeClipMap::ClearFontListCache()
{
	GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, ClearFontListCache));
	if (lib && lib->ClearFontListCache)
		lib->ClearFontListCache();
}

Bool GeClipMap::IsFontInstalled(const BaseContainer& fontDescription, BaseContainer* installedFontDescription)
{
	GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, IsFontInstalled));
	if (lib && lib->IsFontInstalled)
		return lib->IsFontInstalled(fontDescription, installedFontDescription);

	return false;
}

BaseContainer GeClipMap::GetInstalledFont(const BaseContainer& fontDescription, Bool* isFontInstalled)
{
	GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, GetInstalledFont));
	if (lib && lib->GetInstalledFont)
		return lib->GetInstalledFont(fontDescription, isFontInstalled);

	return BaseContainer();
}

String GeClipMap::GetRawFontName(const BaseContainer& fontDescription)
{
	GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, GetRawFontName));
	if (lib && lib->GetRawFontName)
		return lib->GetRawFontName(fontDescription);
