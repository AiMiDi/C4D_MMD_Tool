...
{
	return CheckLib<SculptLibrary>(LIBRARY_SCULPT, offset, lib_sculpt);
}

#define SculptLibCall(b) 		cinema::SculptLibrary *lib = cinema::CheckSculptLib(LIBOFFSET(cinema::SculptLibrary, b)); \
	if (!lib || !lib->b) return; \
	(lib->b)

#define SculptLibCallR(a, b)  cinema::SculptLibrary *lib = cinema::CheckSculptLib(LIBOFFSET(cinema::SculptLibrary, b)); \
