...
{
	return CheckLib<BrowserLib>(BROWSER_LIB_ID, offset, lib_browser);
}

//////////////////////////////////////////////////////////////////////////
#define BrowserCallSR(method, error_value) \
	cinema::BrowserLib* lib = cinema::CheckBrowserLib(LIBOFFSET(cinema::BrowserLib, browser)); \
	if (!lib || !lib->browser) \
		return error_value; \
...
#define ReferenceObjectCall( method)	\
	cinema::BrowserLib *lib = cinema::CheckBrowserLib( LIBOFFSET( cinema::BrowserLib, reference_object.method)); \
	if (!lib || !lib->reference_object.method)	\
		return; \
	(reinterpret_cast<ReferenceObject*>(this)->*lib->reference_object.method)

void	SDKReferenceObject::add_reference()
{
	ReferenceObjectCall(add_reference )();
}

void	SDKReferenceObject::remove_reference()
{
