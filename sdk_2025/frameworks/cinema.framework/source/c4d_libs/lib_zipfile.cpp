...
{
	return CheckLib<ZipFileLib>(LIBRARY_ZIPFILE, offset, lib_zipfile);
}

#define ZipLibCall(b) 		cinema::ZipFileLib *lib = cinema::CheckZipLib(LIBOFFSET(cinema::ZipFileLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iZipFile*>(this)->*lib->b)

#define ZipLibCallR(a, b) cinema::ZipFileLib *lib = cinema::CheckZipLib(LIBOFFSET(cinema::ZipFileLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iZipFile*>(this)->*lib->b)

#define ZipLibCallRC(a, b) cinema::ZipFileLib *lib = cinema::CheckZipLib(LIBOFFSET(cinema::ZipFileLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const cinema::iZipFile*>(this)->*lib->b)
...
{
	ZipLibCallRC(0, GetCompressionLevel)();
}
