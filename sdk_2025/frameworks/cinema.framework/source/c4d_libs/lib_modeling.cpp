...
{
	return CheckLib<ModelingLib>(LIBRARY_MODELLING, offset, lib_modeling);
}

#define ModelingLibCall(b) 		cinema::ModelingLib *lib = cinema::CheckModelingLib(LIBOFFSET(cinema::ModelingLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iModeling*>(this)->*lib->b)

#define ModelingLibCallR(a, b)  cinema::ModelingLib *lib = cinema::CheckModelingLib(LIBOFFSET(cinema::ModelingLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iModeling*>(this)->*lib->b)
