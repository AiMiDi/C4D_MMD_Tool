...
#define PaintLibCall(a, b, c)	cinema::PaintLibrary *lib = cinema::CheckPaintLib(LIBOFFSET(cinema::PaintLibrary, b)); \
	if (!lib || !lib->b) return a; \
	return (((c)this)->*(lib->b))

#define PaintLibMethodCall(b) 		cinema::PaintLibrary *lib = cinema::CheckPaintLib(LIBOFFSET(cinema::PaintLibrary, b)); \
	if (!lib || !lib->b) return; \
	(lib->b)

#define PaintLibMethodCallR(a, b)  cinema::PaintLibrary *lib = cinema::CheckPaintLib(LIBOFFSET(cinema::PaintLibrary, b)); \
	if (!lib || !lib->b) return a; \
	return (lib->b)
...
{
	return CheckLib<PaintLibrary>(C4D_PAINT_LIBRARY_ID, offset, lib_paint);
}
...
Bool										BPColorSettingsHelpers::GetMultiMode() { PaintLibMethodCallR(false, bpcolorsettingshelpersGetMultiMode)(); }
void										BPColorSettingsHelpers::SetMultiMode(Bool multi) { PaintLibMethodCall(bpcolorsettingshelpersSetMultiMode)(multi); }
Bool										BPColorSettingsHelpers::ToggleFgBg() { PaintLibMethodCallR(false, bpcolorsettingshelpersToggleFgBg)(); }
