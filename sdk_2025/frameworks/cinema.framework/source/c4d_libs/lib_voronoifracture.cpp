...
{
	return CheckLib<VoronoiFractureLibrary>(VORONOIFRACTURELIB, offset, lib_voronoifracture);
}


#define VoronoiFractureLibCall(b) 		cinema::VoronoiFractureLibrary *lib = cinema::CheckVoronoiFractureLib(LIBOFFSET(cinema::VoronoiFractureLibrary, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iVoronoiFracture*>(this)->*lib->b)

#define VoronoiFractureLibCallR(a, b) cinema::VoronoiFractureLibrary *lib = cinema::CheckVoronoiFractureLib(LIBOFFSET(cinema::VoronoiFractureLibrary, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iVoronoiFracture*>(this)->*lib->b)
