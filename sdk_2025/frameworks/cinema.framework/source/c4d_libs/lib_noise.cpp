...
{
	return CheckLib<NoiseLib>(NOISE_LIB, offset, lib_noise);
}

#define NoiseLibCall(b) 		cinema::NoiseLib *lib = cinema::CheckLibNoise(LIBOFFSET(cinema::NoiseLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iNoise*>(this)->*lib->b)

#define NoiseLibCallR(a, b)  cinema::NoiseLib *lib = cinema::CheckLibNoise(LIBOFFSET(cinema::NoiseLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iNoise*>(this)->*lib->b)
