...
{
	return CheckLib<SDSObjectLib>(LIBRARY_SDS, offset, lib_sds);
}

#define SDSObjectLibCall(b) 		cinema::SDSObjectLib *lib = cinema::CheckSDSObjectLib(LIBOFFSET(cinema::SDSObjectLib, b)); \
															if (!lib || !lib->b) return; \
															(lib->b)

#define SDSObjectLibCallR(a, b)  cinema::SDSObjectLib *lib = cinema::CheckSDSObjectLib(LIBOFFSET(cinema::SDSObjectLib, b)); \
...
Int32 SDSObject::GetPointCount(const BaseTag *pTag)
{
	SDSObjectLibCallR(0, GetPointCount)(pTag);
}
...
const Float32 *SDSObject::GetPointWeights(const BaseTag *pTag)
{
	SDSObjectLibCallR(nullptr, GetPointWeightsC)(pTag);
}

Int32 SDSObject::GetPolygonCount(const BaseTag *pTag)
{
	SDSObjectLibCallR(0, GetPolygonCount)(pTag);
}
...
const PolyWeight *SDSObject::GetEdgeWeights(const BaseTag *pTag)
{
	SDSObjectLibCallR(nullptr, GetEdgeWeightsC)(pTag);
}
