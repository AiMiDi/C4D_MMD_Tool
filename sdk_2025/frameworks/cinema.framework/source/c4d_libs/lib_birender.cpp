...
{
	return CheckLib<BiRenderLib>(LIBRARY_BIRENDER, offset, lib_birender);
}

#define BiRenderLibCall(b) 	cinema::BiRenderLib *lib = cinema::CheckBiRenderLib(LIBOFFSET(cinema::BiRenderLib, b)); \
															if (!lib || !lib->b) return; \
															(lib->b)

#define BiRenderLibCallR(a, b)  cinema::BiRenderLib *lib = cinema::CheckBiRenderLib(LIBOFFSET(cinema::BiRenderLib, b)); \
