...
maxon::Result<void> InstanceObject::SetLocalInstanceMatrices(const maxon::BaseArray<Matrix>& matrices)
{
	InstanceObjectLibrary* lib = CheckInstanceObjectLibrary(LIBOFFSET(InstanceObjectLibrary, SetInstanceMatrices));
	if (!lib || !lib->SetLocalInstanceMatrices)
		return maxon::UnresolvedError(MAXON_SOURCE_LOCATION, "Function not defined in Library or Library doesn't exist"_s);

	return (reinterpret_cast<iInstanceObject*>(this)->*(lib->SetLocalInstanceMatrices))(matrices);
}

maxon::Result<void> InstanceObject::SetLocalInstanceMatrices(maxon::BaseArray<Matrix>&& matrices)
{
	InstanceObjectLibrary* lib = CheckInstanceObjectLibrary(LIBOFFSET(InstanceObjectLibrary, SetInstanceMatricesMove));
	if (!lib || !lib->SetLocalInstanceMatricesMove)
		return maxon::UnresolvedError(MAXON_SOURCE_LOCATION, "Function not defined in Library or Library doesn't exist"_s);

	return (reinterpret_cast<iInstanceObject*>(this)->*(lib->SetLocalInstanceMatricesMove))(std::move(matrices));
}
