...
{
	return CheckLib<IntersectLibrary>(LIBRARY_INTERSECT, offset, lib_intersect);
}

#define IntersectLibCall(b) 		cinema::IntersectLibrary *lib = cinema::CheckIntersectLib(LIBOFFSET(cinema::IntersectLibrary, b)); \
	if (!lib || !lib->b) return; \
	(lib->b)

#define IntersectLibCallR(a, b)  cinema::IntersectLibrary *lib = cinema::CheckIntersectLib(LIBOFFSET(cinema::IntersectLibrary, b)); \
