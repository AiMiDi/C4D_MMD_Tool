...
namespace maxon
{
	...
	PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(DataDescriptionDefinitionInterface);
	...
	public:
		...
		DataDictionary GetInfo() const 
		{
			...
			DataDictionary proxy_result = HXXRET3(NULLIMPL, DataDictionary);
		...
		Result<BaseArray<DataDescriptionEntry>> GetEntries() const 
		{
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<DataDescriptionDefinitionInterface>();
			Result<BaseArray<DataDescriptionEntry>> proxy_result = HXXRET1(NULLIMPL);
		...
		Result<DataDescriptionEntry> GetEntry(Int idx) const 
		{
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<DataDescriptionDefinitionInterface>();
			Result<DataDescriptionEntry> proxy_result = HXXRET1(NULLIMPL);
	...
	static maxon::Result<void> RegisterReflection_DataDescriptionDefinitionInterface_Static(maxon::reflection::Class* cls, maxon::reflection::FunctionRegistrationCallback callback)
	{
		maxon::reflection::FunctionInfo functions[] =
		{
			...
			maxon::reflection::GetFunctionInfo<decltype(DataDescriptionDefinitionInterface::Hxx2::StaticMTable::_instance.DataDescriptionDefinitionInterface_DefaultValuePtr)>(nullptr, maxon::reflection::Member::KIND::FUNCTION, "", 0LL, nullptr),
		};
