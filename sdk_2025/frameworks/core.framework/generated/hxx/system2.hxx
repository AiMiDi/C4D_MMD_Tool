...
struct System::MTable
{
	...
	PRIVATE_MAXON_SF_POINTER(System_AddEpochObserver, AddEpochObserver, MAXON_EXPAND_VA_ARGS, (void), void* data, maxon::details::EpochObserver observer);
	...
	PRIVATE_MAXON_SF_POINTER(System_PrivateHasherFinalize, PrivateHasherFinalize, MAXON_EXPAND_VA_ARGS, (UniqueHash), const UniqueHasher& state);
	...
	template <typename IMPL> void Init()
	{
		...
		System_AddEpochObserver = System_AddEpochObserver_GetPtr<IMPL>(true);
		...
		System_PrivateHasherFinalize = System_PrivateHasherFinalize_GetPtr<IMPL>(true);
...
struct System::Hxx2
{
	...
	public:
		...
		static void System__ConsoleOutput(const Char* s, const SourceLocation& loc, OUTPUT flags) { return S::_ConsoleOutput(s, loc, flags); }
		static OUTPUT System_GetConsoleOutputType() { return S::GetConsoleOutputType(); }
		static void* System_Alloc(Int size, const SourceLocation& location) { return S::Alloc(size, location); }
		static void* System_AllocClear(Int size, const SourceLocation& location) { return S::AllocClear(size, location); }
		static void* System_Realloc(void* data, Int size, const SourceLocation& location) { return S::Realloc(data, size, location); }
		static void System_Free(const void* data) { return S::Free(data); }
		static void System_AddEpochObserver(void* data, maxon::details::EpochObserver observer) { return S::AddEpochObserver(data, std::forward<maxon::details::EpochObserver>(observer)); }
		static Int System_GetAllocSize(void* data) { return S::GetAllocSize(data); }
		static Int System_MemoryDiagnostics(MEMORYDIAGNOSTICSMODE mode, Int* totalAllocation, Int* freePhysicalMemoryEstimate) { return S::MemoryDiagnostics(mode, totalAllocation, freePhysicalMemoryEstimate); }
		static Bool System_MemoryAddObserver(MEMORYOBSERVERMODE mode, Delegate<void()>&& observer) { return S::MemoryAddObserver(mode, std::forward<Delegate<void()>>(observer)); }
		static void System_SetMemoryAllocationThreshold(UInt warnMemoryAllocationThreshold) { return S::SetMemoryAllocationThreshold(warnMemoryAllocationThreshold); }
		static Float64 System_GetCustomTimer() { return S::GetCustomTimer(); }
		static Int System_GetConfigurationCount() { return S::GetConfigurationCount(); }
		static Bool System_GetConfiguration(Int index, ConfigInit& config) { return S::GetConfiguration(index, config); }
		static Bool System_ConfigurationRegister(const ConfigInit& config) { return S::ConfigurationRegister(config); }
		static Char* System_GetCriticalLog(Int index, Int* count) { return S::GetCriticalLog(index, count); }
		static Int System_GetDefinitionCount(EntityBase::TYPE type) { return S::GetDefinitionCount(type); }
		static const InterfaceReference* System_GetVirtualInterfaceReference(const Id& idWithoutHash) { return S::GetVirtualInterfaceReference(idWithoutHash); }
		static EntityDefinition* System_FindDefinition(EntityBase::TYPE type, const Id& eid) { return S::FindDefinition(type, eid); }
		static void* System_SetDebugErrorObserver(Delegate<void(const ErrorInterface*)>&& observer) { return S::SetDebugErrorObserver(std::forward<Delegate<void(const ErrorInterface*)>>(observer)); }
		static void System_ResetDebugErrorObserver(void* observer) { return S::ResetDebugErrorObserver(observer); }
		static const Error* System_SetCurrentError(const Error& error) { return S::SetCurrentError(error); }
		static const Error* System_SetCurrentError_1(Error&& error) { return S::SetCurrentError(std::forward<Error>(error)); }
		static const Error* System_GetLastError() { return S::GetLastError(); }
		static void System_SetDebugError(const Error* preallocated, const Error& error) { return S::SetDebugError(preallocated, error); }
		static const ErrorInterface* System_GetDebugError(const Error* preallocated) { return S::GetDebugError(preallocated); }
		static Result<const Id*> System_PrivateGetInternedId(const Id& value) { return S::PrivateGetInternedId(value); }
		static void System_PrivateFreeInternedId(const Id* ptr) { return S::PrivateFreeInternedId(ptr); }
		static void System_PrivateShowSynchronizedHashMapDiagnostics(Int size, Int capacity, const Char* signature) { return S::PrivateShowSynchronizedHashMapDiagnostics(size, capacity, signature); }
		static const Char* System_GetPreferencesPathSuffix() { return S::GetPreferencesPathSuffix(); }
		static Id System_GetApplicationId() { return S::GetApplicationId(); }
		static void System_SetApplicationType(const Char* dna) { return S::SetApplicationType(dna); }
		static Bool System_PrivateInitExternalDependency(InitMTable initSystemTable, Binary* externalDependency) { return S::PrivateInitExternalDependency(std::forward<InitMTable>(initSystemTable), externalDependency); }
		static Bool System_PrivateResetExternalDependency(const Block<Tuple<Id, const char*, void*>>& safeShutdownMethods, Binary* externalDependency) { return S::PrivateResetExternalDependency(safeShutdownMethods, externalDependency); }
		static UniqueHash System_PrivateGetUniqueHashCode(const Byte* ptr, Int count) { return S::PrivateGetUniqueHashCode(ptr, count); }
		static void System_PrivateHasherInitialize(UniqueHasher& state) { return S::PrivateHasherInitialize(state); }
		static void System_PrivateHasherCombine(UniqueHasher& state, const Byte* ptr, Int count) { return S::PrivateHasherCombine(state, ptr, count); }
		static void System_PrivateHasherCombine_1(UniqueHasher& state, const UniqueHash& hash) { return S::PrivateHasherCombine(state, hash); }
		static UniqueHash System_PrivateHasherFinalize(const UniqueHasher& state) { return S::PrivateHasherFinalize(state); }
		static void System_PrivateFreeWithDestructor(const void* object, DestructorPointer destruct) { return S::PrivateFreeWithDestructor(object, std::forward<DestructorPointer>(destruct)); }
	};
...
MAXON_ATTRIBUTE_FORCE_INLINE auto System::AddEpochObserver(void* data, maxon::details::EpochObserver observer) -> void
{
	return MTable::_instance.System_AddEpochObserver(data, std::forward<maxon::details::EpochObserver>(observer));
}
...
MAXON_ATTRIBUTE_FORCE_INLINE auto System::PrivateHasherFinalize(const UniqueHasher& state) -> UniqueHash
{
	return MTable::_instance.System_PrivateHasherFinalize(state);
}
