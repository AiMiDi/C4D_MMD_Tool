...
struct DataDictionaryObjectInterface::HasBaseDetector { template <typename I> struct Check { static const maxon::Bool value = std::is_same<I, DataDictionaryObjectInterface>::value || ObjectInterface::HasBaseDetector::template Check<I>::value; static I* Cast(DataDictionaryObjectInterface* ptr) { return reinterpret_cast<I*>(ptr); } }; };
struct DataDictionaryObjectInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = DataDictionaryObjectInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(DataDictionaryObjectInterface**);
...
/// @return												True if existent.
		template <typename KEY> inline HXXADDRET2(Bool) Contains(KEY&& key) const;
...
/// @return												Data converted to the right type if found in the dictionary, otherwise the default value.
		template <typename T, typename KEY> inline HXXADDRET2(typename std::conditional<IsFidClass<KEY>::value&&GetCollectionKind<T>::value!=COLLECTION_KIND::ARRAY, typename IsFidClass<KEY>::type, T>::type) Get(KEY&& key, const T& defaultValue) const;
/// Get data stored under a specific key. If the key is not found the given default value will be returned.
/// This functions offers 2 possible calls. First using an FId "dict.Get(MAXCHINEINFO::COMPUTERNAME, String())" or second using any type directly together with the result type "dict.Get(Int32(5), String())".
/// The data type needs to be registered.
/// @param[in] key								Key under which the data is stored.
/// @param[in] defaultValue				Default value which should be returned if the key cannot be found.
/// @return												Data converted to the right type if found in the dictionary, otherwise the default value.
		template <typename T, typename KEY> inline HXXADDRET2(typename std::conditional<IsFidClass<KEY>::value&&GetCollectionKind<T>::value!=COLLECTION_KIND::ARRAY, typename IsFidClass<KEY>::type, T>::type) Get(KEY&& key, T&& defaultValue) const;
/// Get data stored under a specific key. If the key is not found the given default value will be returned.
/// This functions offers 2 possible calls. First using an FId "dict.Get(MAXCHINEINFO::COMPUTERNAME, String())" or second using any type directly together with the result type "dict.Get(Int32(5), String())".
/// The data type needs to be registered.
/// @param[in] key								Key under which the data is stored.
/// @return												Data converted to the right type if found in the dictionary, otherwise the default value.
		template <typename T = void, typename KEY> inline HXXADDRET2(typename std::conditional<std::is_void<T>::value, typename IsFidClass<KEY>::type, T>::type) GetOrDefault(KEY&& key) const;
/// Helper functions for iterator.
		inline HXXADDRET2(void) InitIterator(DataDictionaryIteratorInterface* iterator, Bool end) const;
/// Returns the begin iterator of the DataDictionary. You can use the Iterator to run through all elements of the DataDictionary.
		inline HXXADDRET2(ConstIterator) Begin() const;
/// Returns the end iterator of the DataDictionary. You can use the Iterator to run through all elements of the DataDictionary.
		inline HXXADDRET2(ConstIterator) End() const;
	};
	/// Intermediate helper class for DataDictionaryObjectInterface.
	...
	public:
		...
		using PrivateInterface = DataDictionaryObjectInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(DataDictionaryObjectInterface**);
...
/// @return												OK on success. This function doesn't check if the dictionary contained the key.
	...
	public:
		...
		using PrivateInterface = DataDictionaryObjectInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(DataDictionaryObjectInterface**);
...
#ifdef DOXYGEN
public FnDoxy<DataDictionaryObjectRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<DataDictionaryObjectInterface, maxon::StrongRefHandler, DataDictionaryObjectRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<DataDictionaryObjectInterface, maxon::StrongRefHandler, DataDictionaryObjectRef>>>>);
...
public:
	...
	using ConstPtr = typename DataDictionaryObjectInterface::ConstPtr;
};

class DataDictionaryReferenceObjectRef;

struct DataDictionaryReferenceObjectInterface::HasBaseDetector { template <typename I> struct Check { static const maxon::Bool value = std::is_same<I, DataDictionaryReferenceObjectInterface>::value || DataDictionaryObjectInterface::HasBaseDetector::template Check<I>::value; static I* Cast(DataDictionaryReferenceObjectInterface* ptr) { return reinterpret_cast<I*>(ptr); } }; };
struct DataDictionaryReferenceObjectInterface::Hxx1
{
	...
	public:
		...
		using PrivateBaseClass = S;
		using PrivateBaseClass::PrivateBaseClass;
		using PrivateInterface = DataDictionaryReferenceObjectInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(DataDictionaryReferenceObjectInterface**);
		inline HXXADDRET2(const DataDictionary&) GetDataContainer() const;
	};
	/// Intermediate helper class for DataDictionaryReferenceObjectInterface.
	template <typename S> class Fn : public ConstFn<S>
	{
	public:
		...
		using PrivateInterface = DataDictionaryReferenceObjectInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(DataDictionaryReferenceObjectInterface**);
	...
	public:
		...
		using PrivateInterface = DataDictionaryReferenceObjectInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(DataDictionaryReferenceObjectInterface**);
...
#ifdef DOXYGEN
public FnDoxy<DataDictionaryReferenceObjectRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<DataDictionaryReferenceObjectInterface, maxon::StrongRefHandler, DataDictionaryReferenceObjectRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<DataDictionaryReferenceObjectInterface, maxon::StrongRefHandler, DataDictionaryReferenceObjectRef>>>>);
	using NonConst = typename Super::ReferenceClass;
};
...
public:
	...
	using ConstPtr = typename DataDictionaryReferenceObjectInterface::ConstPtr;
};
