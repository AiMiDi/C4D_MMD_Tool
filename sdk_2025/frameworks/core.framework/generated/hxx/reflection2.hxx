...
struct Proxy::Hxx2
{
	...
	public:
		...
		static Result<Int> Proxy_AddComponents(ClassInterface* cls, Bool classProxy, const Block<const InterfaceReference* const>& interfaces, Bool includeBaseInterfaces) { return S::AddComponents(cls, classProxy, interfaces, includeBaseInterfaces); }
		static Result<void> Proxy_SetHandler(ObjectInterface* object, Int component, ProxyHandler&& handler) { return S::SetHandler(object, component, std::forward<ProxyHandler>(handler)); }
		static Result<void> Proxy_SetHandler_1(ClassInterface* cls, Int component, ProxyHandler&& handler) { return S::SetHandler(cls, component, std::forward<ProxyHandler>(handler)); }
		static Result<maxon::Class<>> Proxy_GetClass(const Block<const InterfaceReference* const>& interfaces) { return S::GetClass(interfaces); }
		static Result<ObjectRef> Proxy_Create(const maxon::Class<>& cls, ProxyHandler&& handler) { return S::Create(cls, std::forward<ProxyHandler>(handler)); }
	};
