...
namespace maxon
{
	namespace reflection
	{
		...
		"GetType@b3e21faa3ec8b623\0" // const DataType& GetType() const
		...
		public:
			...
			const DataType& GetType() const { if (maxon::NonvirtualInterfaceReference::PrivateResolve(&_interface, true)) return ((const Class*) this)->GetType(); return HXXRET3(UNRESOLVED, const DataType&);}
		...
		maxon::Bool Class::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
		{
			...
			#endif
			#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
			if constexpr (maxon::details::UnresolvedReturnType<const DataType&>::value == maxon::UNRESOLVED_RETURN_TYPE::OTHER) tbl->Class_GetType = PRIVATE_MAXON_MF_CAST(decltype(Class_GetType), &Hxx2::Wrapper<Hxx2::Unresolved>::Class_GetType);
			#else
			tbl->Class_GetType = PRIVATE_MAXON_MF_CAST(decltype(Class_GetType), &Hxx2::Wrapper<Hxx2::Unresolved>::Class_GetType);
			#endif
			#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
