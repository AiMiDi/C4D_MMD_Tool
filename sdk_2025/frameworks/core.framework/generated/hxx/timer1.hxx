...
struct TimerInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = TimerInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(TimerInterface**);
...
/// Notifies the observers that the timer has started.
		inline HXXADDRET2(maxon::ObservableRef<ObservableTimerStartedDelegate>) ObservableTimerStarted(maxon::Bool create) const;
/// Notifies the observers that the timer has finished.
		inline HXXADDRET2(maxon::ObservableRef<ObservableTimerFinishedDelegate>) ObservableTimerFinished(maxon::Bool create) const;
...
/// param[in] maxDuration				Maximum time that was suggested.
		inline HXXADDRET2(maxon::ObservableRef<ObservableTimerOverloadDelegate>) ObservableTimerOverload(maxon::Bool create) const;
/// @cond IGNORE
...
/// @endcond
	...
	public:
		...
		using PrivateInterface = TimerInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(TimerInterface**);
...
/// @param[in] queue							The queue, use JOBQUEUE_CURRENT for the current queue or JOBQUEUE_NONE if the job should be executed immediately.
/// @return												OK on success.
		template <typename FN> inline HXXADDRET2(DISABLE_IF_JOBREF(FN,Result<void>)) Start(TimeValue interval, TimeValue delay, TimeValue tolerance, FN&& src, JobQueueInterface* queue) const;
...
/// Can be called from within the timer job.
		inline HXXADDRET2(void) Cancel() const;
/// @cond IGNORE
...
/// @endcond
	...
	public:
		...
		using PrivateInterface = TimerInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(TimerInterface**);
...
#ifdef DOXYGEN
public FnDoxy<TimerRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<TimerInterface, maxon::StrongRefHandler, TimerRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<TimerInterface, maxon::StrongRefHandler, TimerRef>>>>);
...
public:
	...
	using ConstPtr = typename TimerInterface::ConstPtr;
};
