...
struct BackgroundEntryInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = BackgroundEntryInterface;
...
/// GetStartTime returns the time when the entry has been created (so it is the start time).
		inline HXXADDRET2(UniversalDateTime) GetStartTime() const;
/// GetDuration returns the duration of the entry.
		inline HXXADDRET2(TimeValue) GetDuration() const;
...
/// @return                       OK on success.
		inline HXXADDRET2(Int) GetStateCount() const;
	};
	/// Intermediate helper class for BackgroundEntryInterface.
...
/// GetState returns the overall state of the entry. The highest level (e.g. error) defines the state.
		inline HXXADDRET2(STATE) GetState() const;
...
/// GetState returns the overall state of the entry. The highest level (e.g. error) defines the state.
		inline HXXADDRET1(STATE) GetState();
/// AddState adds a new state message to the entry.
/// @param[in] state							see STATE.
/// @param[in] message						Text of the message.
/// @return                       OK on success.
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<BackgroundEntryInterface, maxon::StrongRefHandler, BackgroundEntryRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<BackgroundEntryInterface, maxon::StrongRefHandler, BackgroundEntryRef>>>>);
...
struct BackgroundProgressInterface::Hxx1
{
	...
	public:
		...
		using PrivateBaseClass = S;
		using PrivateBaseClass::PrivateBaseClass;
		using PrivateInterface = BackgroundProgressInterface;
		using PrivateBaseClass::PrivateLookupFn;
...
/// GetActiveCount returns the number of active elements.
		inline HXXADDRET2(Int) GetActiveCount() const;
...
/// GetHistoryCount returns the number of history entries.
		inline HXXADDRET2(Int) GetHistoryCount() const;
/// ObservableStateChanged observable is called if the object adds or removes entries.
		inline HXXADDRET2(maxon::ObservableRef<ObservableStateChangedDelegate>) ObservableStateChanged(maxon::Bool create) const;
/// ObservableTotalProgressNotification observable is called if the overall progress of one of the elements changed.
		inline HXXADDRET2(maxon::ObservableRef<ObservableTotalProgressNotificationDelegate>) ObservableTotalProgressNotification(maxon::Bool create) const;
	};
	/// Intermediate helper class for BackgroundProgressInterface.
	template <typename S> class Fn : public ConstFn<S>
	{
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<BackgroundProgressInterface, maxon::StrongRefHandler, BackgroundProgressRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<BackgroundProgressInterface, maxon::StrongRefHandler, BackgroundProgressRef>>>>);
	using NonConst = typename Super::ReferenceClass;
};
