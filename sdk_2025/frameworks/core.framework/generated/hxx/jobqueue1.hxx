...
struct JobQueueInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = JobQueueInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(JobQueueInterface**);
		using Current = JobQueueInterface::Current;
		using None = JobQueueInterface::None;
/// @brief Returns the number of threads used for the queue.
/// THREADSAFE.
/// @return												Number of threads.
		inline HXXADDRET2(Int) GetThreadCount() const;
/// @brief Returns the number of threads to be used.
/// THREADSAFE.
...
/// @brief Returns the main thread job queue. If you want to run a job in the main thread's
...
/// @brief Returns a queue for asynchronous I/O and service jobs.
/// THREADSAFE.
...
/// @endcond
	...
	public:
		...
		using PrivateInterface = JobQueueInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(JobQueueInterface**);
...
/// @brief Cancels execution of all jobs in the queue (including jobs which are enqueued after
...
/// @param[in] ownerId						Owner id of the queue (only the owner is allowed to cancel a queue).
		inline HXXADDRET2(void) Cancel(UInt ownerId) const;
/// @brief Cancels execution of all jobs in the queue (including jobs which are enqueued after
...
/// THREADSAFE.
/// @param[in] ownerId						Owner id of the queue (only the owner is allowed to cancel a queue).
		inline HXXADDRET2(void) CancelAndWait(UInt ownerId) const;
/// @brief Enables support for job groups with thread affinity.
...
/// @param[in] ownerId						Owner id of the queue.
		inline HXXADDRET2(void) EnableThreadAffinity(UInt ownerId) const;
/// @brief Enables using the current thread for EnqueueAndWait even though it belongs to a different queue context (useful for alien threads).
/// Only the owner of a queue can opt in to use the current thread for computations (requires the id returned by Init()).
/// THREADSAFE.
/// @param[in] ownerId						Owner id of the queue.
		inline HXXADDRET2(void) AlwaysUseCurrentThread(UInt ownerId) const;
...
/// THREADSAFE.
/// @param[in] ownerId						Owner id of the queue.
		inline HXXADDRET2(void) SetIdleSpinTime(UInt ownerId, TimeValue spinTime) const;
/// @cond IGNORE
...
/// @param[in] timeout						Maximum execution interval (or TIMEVALUE_INFINITE for no time-out).
		inline HXXADDRET2(void) ExecuteJobs(TimeValue timeout = TIMEVALUE_INFINITE) const;
/// @endcond
	...
	public:
		...
		using PrivateInterface = JobQueueInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(JobQueueInterface**);
...
#ifdef DOXYGEN
public FnDoxy<JobQueueRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<JobQueueInterface, maxon::StrongRefHandler, JobQueueRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<JobQueueInterface, maxon::StrongRefHandler, JobQueueRef>>>>);
	using NonConst = typename Super::ReferenceClass;
/// @brief Creates and initializes a job queue.
