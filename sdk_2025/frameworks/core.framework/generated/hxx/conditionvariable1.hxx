...
struct ConditionVariableInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = ConditionVariableInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(ConditionVariableInterface**);
/// @brief Waits until the condition has been set or a certain amount of time has passed.
/// @note Executes enqueued jobs while the thread is waiting. The pre 2025 behaviour was always blocking which lead
/// to a number of freezes and deadlocks due to careless use. Explicitly specify WAITMODE::PRIVATE_BLOCKING  if you really 
/// need a blocking wait (works fine with old releases). Futhermore auto-clear only works with WAITMODE::PRIVATE_BLOCKING   as well.
/// THREADSAFE.
...
/// @return												True if the condition has been set, false for time out or error.
		inline HXXADDRET2(Bool) Wait(const TimeValue& timeout = TIMEVALUE_INFINITE, WAITMODE mode = WAITMODE::DEFAULT) const;
	};
	/// Intermediate helper class for ConditionVariableInterface.
	...
	public:
		...
		using PrivateInterface = ConditionVariableInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(ConditionVariableInterface**);
...
/// @brief Clears the condition variable and sets the number of dependencies.
...
/// @param[in] dependencyCnt			The number of times Set() has to be called before the waiting threads will be woken up.
		inline HXXADDRET2(void) Clear(Int32 dependencyCnt = 1) const;
...
/// THREADSAFE.
		inline HXXADDRET2(void) AddDependency() const;
/// @brief Wakes up all threads waiting for this condition (if the dependency count reaches 0, see Clear()).
/// THREADSAFE.
/// @return												True if successful.
		inline HXXADDRET2(Bool) Set() const;
/// @brief ObservableFinished is an observable that is triggered after the condition is set.
/// Is not supported for auto-clear because it requires a sticky state.
/// THREADSAFE.
/// @return												Custom observable.
		inline HXXADDRET2(ObservableFinishedBase<ConditionVariableInterface>) ObservableFinished() const;
	};
	/// Intermediate helper class for ConditionVariableInterface.
	...
	public:
		...
		using PrivateInterface = ConditionVariableInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(ConditionVariableInterface**);
...
#ifdef DOXYGEN
public FnDoxy<ConditionVariableRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<ConditionVariableInterface, maxon::StrongRefHandler, ConditionVariableRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<ConditionVariableInterface, maxon::StrongRefHandler, ConditionVariableRef>>>>);
...
public:
	...
	using ConstPtr = typename ConditionVariableInterface::ConstPtr;
};
