...
namespace maxon
{
	const maxon::Char* const DllInterface::MTable::_ids = 
		...
		"GetLoadedDlls@2b0a9ff4d77d8105\0" // Result<Bool> GetLoadedDlls(const ValueReceiver<const DllRef&>& dlls)
	...
	public:
		...
		static Result<Bool> GetLoadedDlls(const ValueReceiver<const DllRef&>& dlls) { if (maxon::NonvirtualInterfaceReference::PrivateResolve(&_interface, false)) return DllInterface::GetLoadedDlls(dlls); return HXXRET1(UNRESOLVED);}
	...
	maxon::Bool DllInterface::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
	{
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if constexpr (maxon::details::UnresolvedReturnType<Result<Bool>>::value == maxon::UNRESOLVED_RETURN_TYPE::OTHER) tbl->DllInterface_GetLoadedDlls = PRIVATE_MAXON_MF_CAST(decltype(DllInterface_GetLoadedDlls), &Hxx2::Wrapper<Hxx2::Unresolved>::DllInterface_GetLoadedDlls);
		#else
