...
struct DataTypeBuilderInterface::MTable
{
	...
	template <typename S> using CombinedMTable = maxon::PrivateCombinedMTable<DataTypeBuilderInterface, typename ObjectInterface::MTable::template CombinedMTable<S>>;
...
struct DataTypeBuilderInterface::Hxx2
{
	...
	PRIVATE_MAXON_SF_POINTER(DataTypeBuilderInterface_GetDataTypeName, GetDataTypeName, MAXON_EXPAND_VA_ARGS, (Result<String>), const DataType& dt, DATATYPEBUILDERNAME category);
	PRIVATE_MAXON_SF_POINTER(DataTypeBuilderInterface_RecurseDataType, RecurseDataType, MAXON_EXPAND_VA_ARGS, (Result<Tuple<String, Bool>>), DATATYPEBUILDERNAME category, const Member& dt, const ParametricTypeInterface* parentTypeId, Int& globalIdentifier, BaseArray<DataTypeComponent>& components);
		StaticMTable() = default;
		template <typename IMPL> static void Init(StaticMTable* tbl)
		{
		tbl->DataTypeBuilderInterface_GetDataTypeName = DataTypeBuilderInterface_GetDataTypeName_GetPtr<IMPL>(true);
		tbl->DataTypeBuilderInterface_RecurseDataType = DataTypeBuilderInterface_RecurseDataType_GetPtr<IMPL>(true);
		}
	};
