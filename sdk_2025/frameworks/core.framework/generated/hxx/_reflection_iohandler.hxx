...
namespace maxon
{
	...
	PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(IoHandlerInterface);
	...
	public:
		...
		const Id& GetUrlScheme() const 
		{
			...
			const Id* proxy_result = &HXXRET3(NULLIMPL, const Id&);
		...
		Result<String> GetSystemPath(const UrlInterface& url) const 
		{
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<IoHandlerInterface>();
			Result<String> proxy_result = HXXRET1(NULLIMPL);
		...
		Result<IoConnectionRef> OpenConnection(const Url& name) const 
		{
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<IoHandlerInterface>();
			Result<IoConnectionRef> proxy_result = HXXRET1(NULLIMPL);
		...
		Result<String> ConvertToUiName(const UrlInterface& url, CONVERTTOUINAMEFLAGS flags, const AssetRepositoryRef& lookupRepository) const 
		{
			const maxon::ComponentInfo* proxy_info;
			maxon::ObjectInterface* o_;
			const maxon::reflection::ProxyHandler* handler_;
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<IoHandlerInterface>();
			Result<String> proxy_result = HXXRET1(NULLIMPL);
			maxon::reflection::Argument proxy_args[] = {maxon::reflection::Argument::ByValue(reinterpret_cast<maxon::GenericComponent*>(o_) + mt_.IoHandlerInterface_ConvertToUiName_Offset), maxon::reflection::Argument::ByReference(url), maxon::reflection::Argument::ByValue(flags), maxon::reflection::Argument::ByReference(lookupRepository)};
			(*handler_)(_fn_IoHandlerInterface_ConvertToUiName, o_, maxon::reflection::ProxyInvocation(std::addressof(proxy_result), proxy_args, PRIVATE_MAXON_MF_REFLECTION_INVOCATION((Result<String>), const UrlInterface&, CONVERTTOUINAMEFLAGS, const AssetRepositoryRef&), reinterpret_cast<const void* const&>(mt_.IoHandlerInterface_ConvertToUiName)));
			return proxy_result;
		}
	...
	static maxon::Result<void> RegisterReflection_IoHandlerInterface(maxon::reflection::Class* cls, maxon::reflection::FunctionRegistrationCallback callback)
	{
		maxon::reflection::FunctionInfo functions[] =
		{
			...
			MAXON_GET_VIRTUAL_METHOD_INFO(CONST_INSTANCE_FUNCTION, "url\0flags\0lookupRepository\0", 21LL, IoHandlerInterface, IoHandlerInterface_ConvertToUiName),
		};
