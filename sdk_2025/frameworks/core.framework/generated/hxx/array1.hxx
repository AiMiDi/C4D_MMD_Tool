...
struct PersistentIdGeneratorInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = PersistentIdGeneratorInterface;
		...
		inline HXXADDRET2(void) FreeBuffer(PersistentIdBuffer& buffer) const;
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const PersistentIdGeneratorInterface, maxon::StrongCOWRefHandler, PersistentIdGenerator>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const PersistentIdGeneratorInterface, maxon::StrongCOWRefHandler, PersistentIdGenerator>>>>);
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<PersistentIdGeneratorInterface, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<PersistentIdGeneratorInterface, maxon::StrongRefHandler, NonConstRef>>>>);
};
...
/// @return												Start index of the block. I.e., the requested element can be found within the block at @p index - start index.
		inline HXXADDRET2(Int) GetBlock(Int index, SimdBlock<const TYPE>& block) const;
...
/// @return												DataType of the elements or nullptr.
		...
		inline HXXADDRET2(const PersistentIdGenerator&) GetPersistentIdGenerator() const;
...
/// @return												Iterator for the first element (equal to End() if the array is empty).
		inline HXXADDRET2(typename StaticArrayInterface<TYPE>::ConstIterator) Begin() const;
...
/// @return												Iterator for the array end, this is one behind the last element.
		inline HXXADDRET2(typename StaticArrayInterface<TYPE>::ConstIterator) End() const;
	};
	/// Intermediate helper class for StaticArrayInterface.
...
/// @param[in] index							Element index.
/// @return												Array element.
		inline HXXADDRET2(ResultRef<TYPE>) GetWritable(Int index) const;
		template <typename A> inline HXXADDRET2(ResultMem) Set(Int index, A&& value) const;
...
/// @param[in] index							Element index.
/// @return												Array element.
		inline HXXADDRET1(ResultRef<TYPE>) GetWritable(Int index);
		template <typename A> inline HXXADDRET1(ResultMem) Set(Int index, A&& value);
/// Determines a contiguous, possibly strided writable block of array elements which contains the element at @p index.
/// The returned blocks are guaranteed to form a partition of the array,
/// i.e., no two blocks overlap, and they cover the whole array.
///
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const StaticArrayInterface<TYPE>, maxon::StrongCOWRefHandler, StaticArray<TYPE>>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const StaticArrayInterface<TYPE>, maxon::StrongCOWRefHandler, StaticArray<TYPE>>>>>);
	using NonConst = Hxx1::NonConstRef;
};
...
public FnDoxy<NonConstRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<StaticArrayInterface<TYPE>, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<StaticArrayInterface<TYPE>, maxon::StrongRefHandler, NonConstRef>>>>);
};
...
template <typename TYPE> struct ArrayInterface<TYPE>::Hxx1
{
	...
	public:
		...
		using PrivateBaseClass = S;
		using PrivateBaseClass::PrivateBaseClass;
		using PrivateInterface = ArrayInterface;
		using PrivateBaseClass::PrivateLookupFn;
...
/// Returns an iterator pointing to the first array element.
/// @return												Iterator for the first element (equal to End() if the array is empty).
		inline HXXADDRET2(typename ArrayInterface<TYPE>::ConstIterator) Begin() const;
/// Returns an iterator pointing one behind the last array element.
/// @return												Iterator for the array end, this is one behind the last element.
		inline HXXADDRET2(typename ArrayInterface<TYPE>::ConstIterator) End() const;
#ifdef CPP_853_HACK
#ifdef MAXON_COMPILER_GCC
#else
#endif
...
/// @return												False if allocation failed.
		inline HXXADDRET2(ResultMem) Resize(Int count, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::DEFAULT) const;
...
/// @return												False if allocation failed.
		inline HXXADDRET2(ResultMem) SetCapacityHint(Int requestedCapacity, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::ON_GROW_RESERVE_CAPACITY) const;
...
/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
		inline HXXADDRET2(ResultRef<TYPE>) Insert(Int index) const;
...
/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
		inline HXXADDRET2(ResultRef<TYPE>) Insert(Int index, const TYPE& value) const;
/// Inserts a new element at index position and initializes it with #value.
/// @param[in] index							Insert index (the array size will increase and the existing elements are moved).
/// @param[in] value							Value to be inserted.
/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
		inline HXXADDRET2(ResultRef<TYPE>) Insert(Int index, TYPE&& value) const;
...
/// @return												False if insert failed.
		inline HXXADDRET2(ResultMem) Insert(Int index, const Block<const TYPE>& values) const;
/// Inserts a number of new elements at @p index. The elements will be moved.
/// @param[in] index							Insertion index (the array size will increase and the existing elements are moved).
/// @param[in] values							Block with values to be moved.
/// 															If the block points to nullptr, only its count is used, and you have to call the constructor of the new elements manually.
/// @return												False if insert failed.
		inline HXXADDRET2(ResultMem) Insert(Int index, const MoveBlock<TYPE>& values) const;
		inline HXXADDRET2(ResultMem) InsertBlock(Int index, const Block<const TYPE>& values) const;
		inline HXXADDRET2(ResultMem) InsertBlock(Int index, const MoveBlock<TYPE>& values) const;
...
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		inline HXXADDRET2(ResultRef<TYPE>) Append() const;
/// Adds a new element at the end of the array and initializes it with #value.
/// @param[in] value							Value to be appended.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		inline HXXADDRET2(ResultRef<TYPE>) Append(const TYPE& value) const;
/// Adds a new element at the end of the array and initializes it with #value.
/// @param[in] value							Value to be appended.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		inline HXXADDRET2(ResultRef<TYPE>) Append(TYPE&& value) const;
/// Adds a new element at the end of the array and initializes it with #value.
/// @param[in] value							Value to be appended.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		template <typename A> inline HXXADDRET2(ResultRef<TYPE>) Append(A&& value) const;
/// Deletes the last element.
/// @return												True if there was at least one element.
		inline HXXADDRET2(Bool) Pop() const;
/// Deletes the last element.
/// @param[out] dst								Nullptr or pointer to return value.
/// @return												True if there was at least one element.
		inline HXXADDRET2(Bool) Pop(TYPE* dst) const;
...
/// @return												True if operation was successul.
		inline HXXADDRET2(ResultMem) Erase(Int index, Int count = 1) const;
...
/// @param[in] count							Number of elements to be erased (if eraseCnt is higher than what is available at position Erase() will succeed, but remove only the number of available elements).
/// @return												True if operation was successul.
		inline HXXADDRET2(ResultMem) SwapErase(Int index, Int count = 1) const;
...
/// @return												Success of operation.
		...
		inline HXXADDRET2(void) SetPersistentIdGenerator(const PersistentIdGenerator& generator) const;
#ifdef MAXON_COMPILER_INTEL
#endif
#ifdef MAXON_COMPILER_INTEL
#endif
...
/// @param[in] resizeFlags				See COLLECTION_RESIZE_FLAGS.
/// @return												False if allocation failed.
		inline HXXADDRET1(ResultMem) Resize(Int count, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::DEFAULT);
/// Prepare the internal array so that it can hold at least the given number of elements with
/// as few further memory allocations as possible.
/// @param[in] requestedCapacity	The desired internal capacity.
/// @param[in] resizeFlags				If ON_GROW_FIT_TO_SIZE is set, the collection will use only as much memory as needed to hold the data.
/// @return												False if allocation failed.
		inline HXXADDRET1(ResultMem) SetCapacityHint(Int requestedCapacity, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::ON_GROW_RESERVE_CAPACITY);
/// Inserts a new element at @p index. The element will be default-constructed.
/// @param[in] index							Insertion index (the array size will increase and the existing elements are moved).
/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
		inline HXXADDRET1(ResultRef<TYPE>) Insert(Int index);
/// Inserts a new element at index position and initializes it with #value.
/// @param[in] index							Insert index (the array size will increase and the existing elements are moved).
/// @param[in] value							Value to be inserted.
/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
		inline HXXADDRET1(ResultRef<TYPE>) Insert(Int index, const TYPE& value);
/// Inserts a new element at index position and initializes it with #value.
/// @param[in] index							Insert index (the array size will increase and the existing elements are moved).
/// @param[in] value							Value to be inserted.
/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
		inline HXXADDRET1(ResultRef<TYPE>) Insert(Int index, TYPE&& value);
/// Inserts a number of new elements at @p index. The elements will be copied.
/// @param[in] index							Insertion index (the array size will increase and the existing elements are moved).
/// @param[in] values							Block with values to be copied.
/// 															If the block points to nullptr, only its count is used, and you have to call the constructor of the new elements manually.
/// @return												False if insert failed.
		inline HXXADDRET1(ResultMem) Insert(Int index, const Block<const TYPE>& values);
/// Inserts a number of new elements at @p index. The elements will be moved.
/// @param[in] index							Insertion index (the array size will increase and the existing elements are moved).
/// @param[in] values							Block with values to be moved.
/// 															If the block points to nullptr, only its count is used, and you have to call the constructor of the new elements manually.
/// @return												False if insert failed.
		inline HXXADDRET1(ResultMem) Insert(Int index, const MoveBlock<TYPE>& values);
		inline HXXADDRET1(ResultMem) InsertBlock(Int index, const Block<const TYPE>& values);
		inline HXXADDRET1(ResultMem) InsertBlock(Int index, const MoveBlock<TYPE>& values);
/// Adds a new element at the end of the array. The element will be default-constructed.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		inline HXXADDRET1(ResultRef<TYPE>) Append();
/// Adds a new element at the end of the array and initializes it with #value.
/// @param[in] value							Value to be appended.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		inline HXXADDRET1(ResultRef<TYPE>) Append(const TYPE& value);
/// Adds a new element at the end of the array and initializes it with #value.
/// @param[in] value							Value to be appended.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		inline HXXADDRET1(ResultRef<TYPE>) Append(TYPE&& value);
/// Adds a new element at the end of the array and initializes it with #value.
/// @param[in] value							Value to be appended.
/// @return												Element reference or OutOfMemoryError if the allocation failed.
		template <typename A> inline HXXADDRET1(ResultRef<TYPE>) Append(A&& value);
/// Deletes the last element.
/// @return												True if there was at least one element.
		inline HXXADDRET1(Bool) Pop();
/// Deletes the last element.
/// @param[out] dst								Nullptr or pointer to return value.
/// @return												True if there was at least one element.
		inline HXXADDRET1(Bool) Pop(TYPE* dst);
/// Erases (removes and deletes) elements.
/// @param[in] index							Erase index.
/// @param[in] count							Number of elements to be erased (if eraseCnt is higher than what is available at position Erase() will succeed, but remove only the number of available elements).
/// @return												True if operation was successul.
		inline HXXADDRET1(ResultMem) Erase(Int index, Int count = 1);
/// Erases elements within the array and moves elements from the end to the erased gap.
/// This is generally faster than Erase because at most count elements have to be moved,
/// but it changes the order of elements.
/// @param[in] index							Erase index.
/// @param[in] count							Number of elements to be erased (if eraseCnt is higher than what is available at position Erase() will succeed, but remove only the number of available elements).
/// @return												True if operation was successul.
		inline HXXADDRET1(ResultMem) SwapErase(Int index, Int count = 1);
/// Deletes all elements (calls destructors and frees memory).
...
/// @return												Success of operation.
		...
		inline HXXADDRET1(void) SetPersistentIdGenerator(const PersistentIdGenerator& generator);
#ifdef MAXON_COMPILER_INTEL
#endif
#ifdef MAXON_COMPILER_INTEL
#endif
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const ArrayInterface<TYPE>, maxon::StrongCOWRefHandler, Array<TYPE>>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const ArrayInterface<TYPE>, maxon::StrongCOWRefHandler, Array<TYPE>>>>>);
	using NonConst = Hxx1::NonConstRef;
};
...
public FnDoxy<NonConstRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<ArrayInterface<TYPE>, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<ArrayInterface<TYPE>, maxon::StrongRefHandler, NonConstRef>>>>);
};
