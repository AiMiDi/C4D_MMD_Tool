...
template <typename K, typename V> struct MapInterface<K, V>::Hxx1
{
	...
	public:
		...
		using PrivateInterface = MapInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(MapInterface**);
...
/// @return												Pointer to value for the given key, or nullptr if no entry exists for the key.
		inline HXXADDRET2(const V*) FindValue(const K& key) const;
...
/// @return												Pointer to the new map object, nullptr if allocation or copying failed.
		inline Result<MapInterface*> Clone(Bool copyContent = true) const;
...
/// @return												DataType of the keys or nullptr.
		inline HXXADDRET2(const DataType&) GetKeyDataType() const;
...
HashInt GetHashCode() const { CriticalStop("Not implemented."); return 0; };
		public:
		template <typename KT, typename VT> inline HXXADDRET2(Bool) HasType() const;
...
/// @tparam VT										Type of values to check.
		template <typename KT, typename VT> inline HXXADDRET2(void) AssertType() const;
#ifdef MAXON_COMPILER_INTEL
#endif
#ifdef MAXON_COMPILER_INTEL
#endif
	...
	public:
		...
		using PrivateInterface = MapInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(MapInterface**);
...
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		inline HXXADDRET2(ResultRef<V>) InsertKey(const K& key, Bool& created = BoolLValue()) const;
/// Finds the value associated with the given key, or creates a corresponding entry if it doesn't exist yet.
/// The value of a new entry has to be initialized afterwards (but its default constructor has already been invoked).
...
/// @param[out] created						This will be set to true if a new entry has been created successfully, otherwise it will be set to false.
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		inline HXXADDRET2(ResultRef<V>) InsertKey(K&& key, Bool& created = BoolLValue()) const;
...
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		template <typename VALUE> inline HXXADDRET2(ResultRef<V>) Insert(const K& key, VALUE&& value, Bool& created = BoolLValue()) const;
/// Associates the given value with the given key. This adds a new entry for key if necessary,
/// and then sets its value to the given value, whether the entry existed before or not.
/// @param[in] key								Key of the entry to find or create. If a new entry is created, its key will be constructed by move-semantics if possible.
/// @param[in] value							Value to which the key shall map.
/// @param[out] created						This will be set to true if a new entry has been created, otherwise it will be set to false.
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		template <typename VALUE> inline HXXADDRET2(ResultRef<V>) Insert(K&& key, VALUE&& value, Bool& created = BoolLValue()) const;
/// Finds the value associated with the given key in this map.
/// @param[in] key								Key to search for.
/// @return												Pointer to value for the given key, or nullptr if no entry exists for the key.
		inline HXXADDRET2(V*) FindValue(const K& key) const;
...
/// @return												Success of operation.
	...
	public:
		...
		using PrivateInterface = MapInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(MapInterface**);
...
/// @param[out] created						This will be set to true if a new entry has been created successfully, otherwise it will be set to false.
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		inline HXXADDRET1(ResultRef<V>) InsertKey(const K& key, Bool& created = BoolLValue());
/// Finds the value associated with the given key, or creates a corresponding entry if it doesn't exist yet.
/// The value of a new entry has to be initialized afterwards (but its default constructor has already been invoked).
/// @param[in] key								Key of the entry to find or create. If a new entry is created, its key will be constructed by move-semantics if possible.
/// @param[out] created						This will be set to true if a new entry has been created successfully, otherwise it will be set to false.
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		inline HXXADDRET1(ResultRef<V>) InsertKey(K&& key, Bool& created = BoolLValue());
/// Associates the given value with the given key. This adds a new entry for key if necessary,
/// and then sets its value to the given value, whether the entry existed before or not.
/// @param[in] key								Key which shall map to the value.
/// @param[in] value							Value to which the key shall map.
/// @param[out] created						This will be set to true if a new entry has been created, otherwise it will be set to false.
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		template <typename VALUE> inline HXXADDRET1(ResultRef<V>) Insert(const K& key, VALUE&& value, Bool& created = BoolLValue());
/// Associates the given value with the given key. This adds a new entry for key if necessary,
/// and then sets its value to the given value, whether the entry existed before or not.
/// @param[in] key								Key of the entry to find or create. If a new entry is created, its key will be constructed by move-semantics if possible.
/// @param[in] value							Value to which the key shall map.
/// @param[out] created						This will be set to true if a new entry has been created, otherwise it will be set to false.
/// @return												Pointer to value for the given key, or nullptr if an entry didn't exist and allocation of a new entry failed.
		template <typename VALUE> inline HXXADDRET1(ResultRef<V>) Insert(K&& key, VALUE&& value, Bool& created = BoolLValue());
/// Removes an entry with the given key from this map (if possible).
/// @param[in] key								Key of the map entry to be be removed.
/// @return												True if an entry was found and removed for #key, otherwise false or an error if a memory allocation failed.
...
#ifdef DOXYGEN
public COWFnDoxy<Map<K, V>>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const MapInterface<K, V>, maxon::StrongCOWRefHandler, Map<K, V>>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const MapInterface<K, V>, maxon::StrongCOWRefHandler, Map<K, V>>>>>);
	using NonConst = Hxx1::NonConstRef;
};

template <typename K, typename V> class MapInterface<K, V>::Hxx1::NonConstRef :
#ifdef DOXYGEN
public FnDoxy<NonConstRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<MapInterface<K, V>, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<MapInterface<K, V>, maxon::StrongRefHandler, NonConstRef>>>>);
};
