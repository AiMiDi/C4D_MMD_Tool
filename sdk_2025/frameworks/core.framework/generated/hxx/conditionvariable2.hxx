...
struct ConditionVariableInterface::Hxx2
{
	...
	public:
		...
		static void ConditionVariableInterface_Free(const ConditionVariableInterface* object) { return S::Free(object); }
		static ConditionVariableInterface* ConditionVariableInterface_Alloc(const maxon::SourceLocation& allocLocation) { return S::Alloc(allocLocation); }
		static ConditionVariableInterface* ConditionVariableInterface_Alloc_1(const maxon::SourceLocation& allocLocation, Bool isAutoClear) { return S::Alloc(allocLocation, isAutoClear); }
		PRIVATE_MAXON_MF_WRAPPER(ConditionVariableInterface_Clear, ConditionVariableInterface,, (void), Int32 dependencyCnt) { return S::Get(PRIVATE_MAXON_MF_THIS(ConditionVariableInterface))->Clear(dependencyCnt); }
		PRIVATE_MAXON_MF_WRAPPER(ConditionVariableInterface_AddDependency, ConditionVariableInterface,, (void)) { return S::Get(PRIVATE_MAXON_MF_THIS(ConditionVariableInterface))->AddDependency(); }
		PRIVATE_MAXON_MF_WRAPPER(ConditionVariableInterface_Set, ConditionVariableInterface,, (Bool)) { return S::Get(PRIVATE_MAXON_MF_THIS(ConditionVariableInterface))->Set(); }
		PRIVATE_MAXON_MF_WRAPPER(ConditionVariableInterface_Wait, ConditionVariableInterface, const, (Bool), const TimeValue& timeout, WAITMODE mode) { return S::Get(PRIVATE_MAXON_MF_THIS(const ConditionVariableInterface))->Wait(timeout, mode); }
		PRIVATE_MAXON_MF_WRAPPER(ConditionVariableInterface_PrivateAddFinishedObserver, ConditionVariableInterface,, (Result<void>), const JobInterfacePOD& observerPod, JobQueueInterface* observerQueue) { return S::Get(PRIVATE_MAXON_MF_THIS(ConditionVariableInterface))->PrivateAddFinishedObserver(observerPod, observerQueue); }
	};
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ConditionVariableInterface::Hxx1::Fn<S>::Clear(Int32 dependencyCnt) const -> HXXADDRET2(void)
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ConditionVariableInterface::Hxx1::Fn<S>::AddDependency() const -> HXXADDRET2(void)
{
	HXXRETURN0(HXXADDRET2(void)); HXXRES; HXXNONCONST(DebugStop(), , false, maxon::PrivateLogNullptrError()) 
	PRIVATE_MAXON_MF_INVOKE((o_), MTable::_instance.ConditionVariableInterface_AddDependency);
	return (typename ReturnHelper::ReturnType) maxon::OK;
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ConditionVariableInterface::Hxx1::Fn<S>::Set() const -> HXXADDRET2(Bool)
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ConditionVariableInterface::Hxx1::ConstFn<S>::Wait(const TimeValue& timeout, WAITMODE mode) const -> HXXADDRET2(Bool)
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ConditionVariableInterface::Hxx1::Fn<S>::ObservableFinished() const -> HXXADDRET2(ObservableFinishedBase<ConditionVariableInterface>)
{
	HXXRETURN0(HXXADDRET2(ObservableFinishedBase<ConditionVariableInterface>)); HXXRES; HXXNONCONST(, , false, HXXRET3(NULLPTR, ObservableFinishedBase<ConditionVariableInterface>)) 
