...
struct UrlInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = UrlInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(UrlInterface**);
...
/// @return												Returns the UrlScheme for the Url (e.g. "file", "http"). If no scheme is set the returned value is UrlScheme().
		inline HXXADDRET2(const UrlScheme&) GetScheme() const;
...
/// @return												The embedded Url or if there was none Url().
		inline HXXADDRET2(Url) GetEmbeddedUrl() const;
...
/// @return												The directory Url.
		inline HXXADDRET2(Url) GetDirectory() const;
...
/// @return												The path component of the Url (with no authority and scheme).
		inline HXXADDRET2(const String&) GetPath() const;
...
/// @return												The Url as a string.
		inline HXXADDRET2(String) GetUrl() const;
...
/// @return												The Url as a string. All spaces and special characters are url encoded. E.g. ' ' becomes '%20'.
		inline HXXADDRET2(String) GetUrlEncoded() const;
...
/// 															Otherwise it defines an embedded file.
		inline HXXADDRET2(const Url&) GetAuthority() const;
...
/// @return												The suffix without dot.
		inline HXXADDRET2(String) GetSuffix() const;
...
/// @return												True if the suffix matches.
		inline HXXADDRET2(Bool) CheckSuffix(const String& suffix) const;
...
/// are not the same. To check if Urls point to the identical item use @ref maxon::UrlInterface::IoNormalize "IoNormalize" instead.
/// @param[in] name								Comparison object.
/// @param[in] mode								Comparision mode.
/// @return												The comparison result.
		inline HXXADDRET2(COMPARERESULT) Compare(const UrlInterface* name, URLCOMPARE mode = URLCOMPARE::DEFAULT) const;
...
/// @return												IODETECT of the given Url.
		inline HXXADDRET2(IODETECT) IoDetect() const;
...
/// @return												The pointer to the IoConnectionInterface. The return value needs to be checked against nullptr.
		inline typename maxon::SFINAEHelper<Result<IoConnectionRef>, S>::type OpenConnection() const;
...
/// @return												The pointer to the InputStreamInterface. The return value needs to be checked against nullptr.
		inline typename maxon::SFINAEHelper<Result<InputStreamRef>, S>::type OpenInputStream(OPENSTREAMFLAGS flags = OPENSTREAMFLAGS::NONE) const;
...
/// @return												The pointer to the OutputStreamInterface. The return value needs to be checked against nullptr.
		inline typename maxon::SFINAEHelper<Result<OutputStreamRef>, S>::type OpenOutputStream(OPENSTREAMFLAGS flags = OPENSTREAMFLAGS::NONE) const;
...
/// @return												The pointer to the InOutputStreamInterface. The return value needs to be checked against nullptr.
		inline typename maxon::SFINAEHelper<Result<InOutputStreamRef>, S>::type OpenInOutputStream(OPENSTREAMFLAGS flags = OPENSTREAMFLAGS::NONE) const;
...
/// @return												IoBrowseIterator object.
		inline typename maxon::SFINAEHelper<Result<IoBrowseRef>, S>::type GetBrowseIterator(GETBROWSEITERATORFLAGS flags) const;
...
/// @return												Returns the DateTime or an error.
		inline typename maxon::SFINAEHelper<Result<UniversalDateTime>, S>::type IoGetTime(IOTIMEMODE mode) const;
...
/// @return												OK on success.
		inline Result<void> IoCopyFile(const Url& destName, Bool overwrite, Bool removeReadOnly, const IoProgressDelegate& progress = IoProgressDelegate()) const;
...
/// Returns a readable string of the content.
/// @return												The converted result.
		inline HXXADDRET2(CString) ToCString() const;
...
/// @return                       String on success.
		inline Result<String> ConvertToUiName(CONVERTTOUINAMEFLAGS flags, const AssetRepositoryRef& lookupRepository) const;
...
/// @return                       Url on success. Please note that the url might be translated into another scheme (e.g. assetdb:// -> asset://).
		static inline Result<Url> ConvertFromUiName(const String& str, const AssetRepositoryRef& lookupRepository);
	};
	/// Intermediate helper class for UrlInterface.
	...
	public:
		...
		using PrivateInterface = UrlInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(UrlInterface**);
...
/// @return												OK on success.
		inline Result<void> SetPath(const String& path, Bool convertWindowsBackslashes = true) const;
...
/// E.g. if the Url was "file:///c:/directory/file.txt" a call will result in "file:///c:/directory/file".
		inline HXXADDRET2(void) ClearSuffix() const;
...
/// @param[in] persistent					With the default value of true the attribute value is serialized. Use false for a transient attribute (one which shall not be serialized).
/// @return                       OK on success.
		inline Result<void> EraseData(ForwardingDataPtr&& key, Bool persistent = true) const;
	};
	/// Intermediate helper class for UrlInterface.
	...
	public:
		...
		using PrivateInterface = UrlInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(UrlInterface**);
...
/// @param[in] convertWindowsBackslashes	True to convert backslashes under windows (the default).
/// @return												OK on success.
		inline Result<void> SetPath(const String& path, Bool convertWindowsBackslashes = true);
/// Stores a system path in a Url.
/// This call needs to be done after the appropriate scheme has been set.
/// If no scheme is set URLSCHEME_FILESYSTEM will be automatically set.
/// The system path may be converted to a different internal representation, e.g. split
...
/// Deletes the suffix of the Url.
/// E.g. if the Url was "file:///c:/directory/file.txt" a call will result in "file:///c:/directory/file".
		inline HXXADDRET1(void) ClearSuffix();
/// Modifies or sets a Url attribute. See maxon::URLFLAGS for details.
/// @param[in] key								The id of the property to set. The possible values for id depend on the scheme.
/// @param[in] data								Data to be set.
/// @param[in] persistent					With the default value of true the attribute value is serialized. Use false for a transient attribute (one which shall not be serialized).
...
/// @param[in] persistent					With the default value of true the attribute value is serialized. Use false for a transient attribute (one which shall not be serialized).
/// @return                       OK on success.
		inline Result<void> EraseData(ForwardingDataPtr&& key, Bool persistent = true);
...
#ifdef DOXYGEN
public COWFnDoxy<Url>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const UrlInterface, maxon::StrongCOWRefHandler, Url>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const UrlInterface, maxon::StrongCOWRefHandler, Url>>>>);
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<UrlInterface, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<UrlInterface, maxon::StrongRefHandler, NonConstRef>>>>);
};
