...
>
inline auto ConstructElements(ITERATOR first, SENTINEL sentinel, const ARGS& ... args) -> ITERATOR
{
...
>
inline auto ConstructElements(ITERABLE&& iterable, const ARGS& ... args) -> decltype(begin(iterable))
{
	static_assert(IsIterable<ITERABLE>::value, "iterable must support begin(iterable) and end(iterable)");

	return ConstructElements(begin(iterable), end(iterable), args ...);
}
...
<
	...
	typename = typename std::enable_if<!IsIterable<ITERATOR>::value>::type
>
inline auto DestructElements(ITERATOR first, SENTINEL sentinel) -> ITERATOR
{
	using ValueType = typename std::iterator_traits<ITERATOR>::value_type;
...
<
	...
	typename = typename std::enable_if<IsIterable<ITERABLE>::value>::type
>
inline auto DestructElements(ITERABLE&& iterable) -> decltype(begin(iterable))
{
	static_assert(IsIterable<ITERABLE>::value, "iterable must support begin(iterable) and end(iterable)");

	return DestructElements(begin(iterable), end(iterable));
}
...
//----------------------------------------------------------------------------------------
/// Re-arranges elements in a sequence so that elements matching the given value are removed and the container resized accordingly.
/// Relative order of the elements is preserved.
///
/// @param[in,out] iterable				The container to iterate upon. Will be resized after remove operation.
/// @param[in] value							The value to compare with while iterating the container.
/// @return												Number of removed elements.
//----------------------------------------------------------------------------------------
template <typename ITERABLECONTAINER, typename VALUETYPE>
Result<Int> Erase(ITERABLECONTAINER& iterable, VALUETYPE&& value)
{
	iferr_scope;
	static_assert(IsIterable<ITERABLECONTAINER>::value, "ITERABLECONTAINER argument must support begin(iterable) and end(iterable)");

	const Int prevLen = iterable.GetCount();
	auto	itNewEnd = Remove(iterable, value);
	const Int newLen = GetSequenceLength(begin(iterable), itNewEnd);
	iterable.Resize(newLen) iferr_return;
	return prevLen - newLen;
}

//----------------------------------------------------------------------------------------
/// Re-arranges elements in a sequence so that elements matching the given predicate are removed and the container resized accordingly.
/// Relative order of the elements is preserved.
/// The predicate function should have a signature equivalent to
/// @code
/// Bool func(const ElementType&);
...
/// @return												Number of removed elements.
//----------------------------------------------------------------------------------------
template <typename ITERABLECONTAINER, typename PREDICATE>
Result<Int> EraseIf(ITERABLECONTAINER& iterable, PREDICATE&& pred)
{
	iferr_scope;
	static_assert(IsIterable<ITERABLECONTAINER>::value, "ITERABLECONTAINER argument must support begin(iterable) and end(iterable)");

	const Int prevLen = iterable.GetCount();
	auto	itNewEnd = RemoveIf(iterable, pred);
	const Int newLen = GetSequenceLength(begin(iterable), itNewEnd);
	iterable.Resize(newLen) iferr_return;
	return prevLen - newLen;
}
...
//----------------------------------------------------------------------------------------
/// @copydoc EraseIf()
/// Same as EraseIf with a predicate argument, but with PredicateMatcher support.
/// @code
/// maxon::BaseArray<maxon::Tuple<Int, String>> intMap;
/// // ...
/// intMap.Append(0, "0"_s) iferr_return;
/// intMap.Append(1, "1"_s) iferr_return;
/// intMap.Append(3, "3"_s) iferr_return;
/// maxon::EraseIf(intMap, maxon::ByValue(3, maxon::TupleElementEquals::ByIndex<0>()) iferr_return;
/// @endcode
//----------------------------------------------------------------------------------------
template <typename ITERABLECONTAINER, template<typename> class MATCHER, typename VALUEPREDICATE>
Result<Int> EraseIf(ITERABLECONTAINER& iterable, MATCHER<VALUEPREDICATE>&& matcher)
{
	return EraseIf(iterable, matcher.pred);
}
...
/// @endcode
///
/// @param[in] indexable					Indexable input container.
/// @param[in] matcher						Pattern matcher.
/// @return												Match containing index of the first match and total match count.
//----------------------------------------------------------------------------------------
template <typename INDEXABLE, typename MATCHER> static
auto CountIfMatch(INDEXABLE&& indexable, MATCHER matcher) -> Match<Int>
{
	IndexPair result{0, InvalidArrayIndex};
	Int index = 0;
	const Int end = GetSequenceLength(indexable);
	for (; index < end; ++index)
	{
		if (matcher.pred(indexable[index]))
		{
			if (result.first == 0)
			{
				result.second = index;
	...
	if (result.first > 0)
	{
		// Return {first index, length}, see Match<Int>.
		return Match<Int>{result.second, result.first};
	}
	return NO_MATCH;
}
...
/// If N > COUNT, the last element countains the additional N + COUNT parts (including separators).
///
/// To create the parts, the ITERABLE type must be constructible with
/// @code
/// using IteratorType = decltype(begin(iterable));
...
//----------------------------------------------------------------------------------------
/// Finds the values in the first iterable container that are not contained in the second iterable container and
...
inline auto SetDifference(const ITERABLE1& firstSet, const ITERABLE2& secondSet) -> Result<ITERABLE1>
{
	...
	SetDifference(firstSet, secondSet, diff) iferr_return;
...
inline auto SetDifference(const ITERABLE1& firstSet, const ITERABLE2& secondSet, APPENDABLE& outContainer) -> Result<void>
{
	...
	auto it1 = begin(firstSet);
	auto end1 = end(firstSet);
	auto it2 = begin(secondSet);
	auto end2 = end(secondSet);
	while (it1 != end1)
	{
		if (it2 == end2)
		{
			do
			{
				outContainer.Append(*it1) iferr_return;
				++it1;
			} while (it1 != end1);
...
inline auto UnorderedSetDifference(const ITERABLE& firstSet, const FINDABLE& secondSet) -> Result<ITERABLE>
{
	...
	UnorderedSetDifference(firstSet, secondSet, diff) iferr_return;
...
inline auto UnorderedSetDifference(const ITERABLE& firstSet, const FINDABLE& secondSet, APPENDABLE& outputContainer) -> Result<void>
{
	...
	for (const auto& value : firstSet)
	{
		...
		{
			outputContainer.Append(value) iferr_return;
		}
	}
...
inline auto ExcludeSetFromSet(ERASABLE& firstSet, const ITERABLE& secondSet) -> Result<void>
{
	...
	for (const auto& value: secondSet)
	{
		firstSet.Erase(value) iferr_return;
	}
	return OK;
}
