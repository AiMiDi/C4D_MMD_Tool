...
template <typename T> class ParallelInvokeJob : public ParallelInvokeJobWithResult<T, typename std::invoke_result<T>::type>
{
	using Super = ParallelInvokeJobWithResult<T, typename std::invoke_result<T>::type>;
public:
	explicit ParallelInvokeJob(T&& obj) : Super(std::forward<T>(obj), JobInterface::GetJumpTable<ParallelInvokeJob<T>, false>())
	{
