...
class System
{
	MAXON_INTERFACE_NONVIRTUAL(System, MAXON_REFERENCE_STATIC, "net.maxon.interface.system", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	/// Invokes the observer method for object when all threads have agreed on a newer time stamp than the current one.
	static MAXON_METHOD void AddEpochObserver(void* data, maxon::details::EpochObserver observer);
...
UniqueHash UniqueHash::GetUniqueHashCode(const Byte* ptr, Int count)
{
	return System::PrivateGetUniqueHashCode(ptr, count);
}

UniqueHash UniqueHash::GetUniqueHashCode(const Block<const Byte>& bytes)
{
	return System::PrivateGetUniqueHashCode(bytes.GetFirst(), bytes.GetCount());
}

UniqueHasher::UniqueHasher()
{
	System::PrivateHasherInitialize(*this);
}

void UniqueHasher::Combine(const Byte* ptr, Int count)
{
	System::PrivateHasherCombine(*this, ptr, count);
}

void UniqueHasher::Combine(const Block<const Byte>& bytes)
{
	System::PrivateHasherCombine(*this, bytes.GetFirst(), bytes.GetCount());
}

void UniqueHasher::Combine(const UniqueHash& hash)
{
	System::PrivateHasherCombine(*this, hash);
}

UniqueHash UniqueHasher::Finalize() const
{
	return System::PrivateHasherFinalize(*this);
}

UniqueHasher::operator UniqueHash() const
{
	return System::PrivateHasherFinalize(*this);
}
...
template <typename T> void PrivateFreeWithDestructor(const T* obj)
{
	...
	{
		DestructorPointer ptr = GetDestructorPointer<T>();
		System::PrivateFreeWithDestructor(obj, ptr);
	}
}
