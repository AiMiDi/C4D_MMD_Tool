...
public:
	...
	StackBufferAllocator(StackBufferAllocator&& other)
	{
		...
		_allocCount = std::move(other._allocCount);
	...
	void* Alloc(Int size, MAXON_SOURCE_LOCATION_DECLARATION)
	{
		...
		nextBlock:
			...
			if (MAXON_UNLIKELY(index == _memory.GetCount()))
			{
				...
				base = mem + SIZEOF(Int);
				*(reinterpret_cast<Int*>(mem)) = sz;
				end = mem + sz;
			}
			else
			{
				base = _memory[index];
				end = base + *(reinterpret_cast<Int*>(base));
				base += SIZEOF(Int);
			}
		}
		if (LIFO)
		{
			reinterpret_cast<Header*>(p)[-1] = _header;
		}
		_header.index = index;
...
#endif
			Int i = _header.index;
			_header = static_cast<const Header*>(ptr)[-1];
	...
	//----------------------------------------------------------------------------------------
	/// Marks all previously made allocations as freed. This is a really quick operation which removes the need to call Free for each
	/// allocated pointer individually. However, it doesn't free the memory blocks themselves, they will be re-used for the next allocations.
	//----------------------------------------------------------------------------------------
	void Flush()
	{
		_header.index = -1;
