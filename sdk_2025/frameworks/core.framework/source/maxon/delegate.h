...
public:
	...
	//----------------------------------------------------------------------------------------
	template <typename STUBPTR> DelegateBase(void* objectPtr, STUBPTR stubPtr, Callback callback) : _objectPtr(objectPtr), _stubPtr(reinterpret_cast<Stub>(stubPtr)), _callback(callback)
	{
	}

	/// Assign when T is not a Delegate, but a function pointer, ptmf or a PTMFWrapper<OBJECT*, RESULT (OBJECT::*)(ARGS...)>.
	template <typename FN, typename STUBPTR> DelegateBase(FN&& fn, STUBPTR stub, Callback callback) : _objectPtr(&_storage[0]), _stubPtr(reinterpret_cast<Stub>(stub)), _callback(callback)
	{
	...
	template <typename FN, typename STUBPTR> ResultMem CopyFrom(FN&& f, STUBPTR stub, Callback callback)
	{
		...
		_stubPtr = reinterpret_cast<Stub>(stub);
		_callback = callback;
...
template <typename RESULT, typename OBJECT, typename... ARGS> struct MethodDelegateType<RESULT (OBJECT::*)(ARGS...) const>
{
	using type = Delegate<RESULT(ARGS...)>;
...
template <typename I, typename METHOD, typename T> inline typename MethodDelegateType<METHOD>::type GetMethodDelegate(T&& object, Int offset)
{
	...
	if (static_cast<const ReferencedType*>(object))
	{
		return typename MethodDelegateType<METHOD>::type(true, std::forward<T>(object), *(METHOD*) ((const Char*) MethodDelegateMTable<I>(static_cast<const ReferencedType*>(object)) + offset));
	}
	else if (STD_IS_REPLACEMENT(const, typename MethodDelegateType<METHOD>::object))
	{
		// For a null object and a const method, we use the DefaultValuePtr() of the interface.
		const ReferencedType* defaultValue = ReferencedType::DefaultValuePtr();
		if (defaultValue)
		{
			// We use the delegate with pointer semantics as the default value of an interface is already owned by the interface.
			return typename MethodDelegateType<METHOD>::type(true, const_cast<ReferencedType*>(defaultValue), *(METHOD*) ((const Char*) MethodDelegateMTable<I>(defaultValue) + offset));
		}
	}
...
template <typename REF> struct MethodDelegateThisType
{
	...
	static const Bool VIRTUAL = STD_IS_REPLACEMENT(base_of, ObjectInterface, ReferencedType);
...
#define MAXON_METHOD_DELEGATE(obj, interface, method) \
	maxon::details::GetMethodDelegate<interface, decltype(interface::MTable::interface##_##method)>(obj, offsetof(interface::MTable, interface##_##method))

//----------------------------------------------------------------------------------------
/// @tparam RESULT								Return value type of the callable.
/// @note When you pass a delegate to a method you should move it or use an rvalue to avoid unnecessary copies and error handling.
/// @code
///	static void MethodWithADelegate(Bool dummy, Delegate<void()>&& observer) { ... }
///	MethodWithADelegate(true, []() { lambda is passed as rvalue });
/// @endcode
//----------------------------------------------------------------------------------------
...
public:
#ifdef PRIVATE_MAXON_MTABLE_PTMF
	using StubPtrType = RESULT (Delegate::*)(ARGS...);
	static_assert(SIZEOF(StubPtrType) == SIZEOF(void*));
#else
...
private:
	...
	static StubPtrType CheckStub(StubPtrType ptr) { return ptr; }

public:
	using ResultValueType = RESULT;

	static const Delegate& DefaultValueRef() {return GetZeroRef<Delegate>(); }
	...
	//----------------------------------------------------------------------------------------
	explicit Delegate(RESULT (*functionPtr)(ARGS...)) : DelegateBase(reinterpret_cast<void*>(functionPtr), CheckStub(&Delegate::FunctionIndirectStub<RESULT (*)(ARGS...)>), nullptr)
	{
	}
...
#ifdef PRIVATE_MAXON_MTABLE_PTMF
	template <typename REF, typename OBJECT> Delegate(Bool marker, REF&& reference, RESULT (OBJECT::* &methodPtr)(ARGS...))
		: DelegateBase((void*) nullptr, (StubPtrType) methodPtr, GetReferenceUtilityStub<typename maxon::details::MethodDelegateThisType<REF>::BaseRefType>((typename maxon::details::MethodDelegateThisType<REF>::Handler*) nullptr))
	{
		...
		static_assert(!STD_IS_REPLACEMENT(const, typename MDThisType::type), "Cannot bind a non-const method to a const object.");
		static_assert(MDThisType::VIRTUAL == STD_IS_REPLACEMENT(same, const OBJECT, const GenericComponent) && (MDThisType::VIRTUAL || STD_IS_REPLACEMENT(base_of, OBJECT, typename MDThisType::type)), "The type of the method object must be a base of the type of the object.");
	...
	template <typename REF, typename OBJECT> Delegate(Bool marker, REF&& reference, RESULT (OBJECT::* &methodPtr)(ARGS...) const)
		: DelegateBase((void*) nullptr, (StubPtrType) methodPtr, GetReferenceUtilityStub<typename maxon::details::MethodDelegateThisType<REF>::BaseRefType>((typename maxon::details::MethodDelegateThisType<REF>::Handler*) nullptr))
	{
		using MDThisType = maxon::details::MethodDelegateThisType<REF>;
		static_assert(MDThisType::VIRTUAL == STD_IS_REPLACEMENT(same, const OBJECT, const GenericComponent) && (MDThisType::VIRTUAL || STD_IS_REPLACEMENT(base_of, OBJECT, typename MDThisType::type)), "The type of the method object must be a base of the type of the object.");
...
#else
	template <typename REF, typename OBJECT> Delegate(Bool marker, REF&& reference, RESULT (*const &methodPtr)(OBJECT*, ARGS...))
		: DelegateBase((void*) nullptr, (StubPtrType) methodPtr, GetReferenceUtilityStub<typename maxon::details::MethodDelegateThisType<REF>::BaseRefType>((typename maxon::details::MethodDelegateThisType<REF>::Handler*) nullptr))
	{
		using MDThisType = maxon::details::MethodDelegateThisType<REF>;
		static_assert(STD_IS_REPLACEMENT(const, OBJECT) || !STD_IS_REPLACEMENT(const, typename MDThisType::type), "Cannot bind a non-const method to a const object.");
		static_assert(MDThisType::VIRTUAL == STD_IS_REPLACEMENT(same, const OBJECT, const GenericComponent) && (MDThisType::VIRTUAL || STD_IS_REPLACEMENT(base_of, OBJECT, typename MDThisType::type)), "The type of the method object must be a base of the type of the object.");
		// Set _storage[0] here and not in the constructor of DelegateBase to obtain the correct reference semantics.
		new (&this->_storage[0]) typename std::decay<REF>::type(std::forward<REF>(reference));
		_objectPtr = this->_storage[0];
	...
	//----------------------------------------------------------------------------------------
	template <typename FN, typename = typename std::enable_if<!std::is_convertible<Delegate, typename std::decay<FN>::type>::value && (GetCollectionKind<FN>::value == COLLECTION_KIND::NONE) && !std::is_base_of<InternedIdFidBase, typename std::decay<FN>::type>::value>::type>
	...
	ResultMem CopyFrom(FN&& fn)
	{
		...
		return DelegateBase::CopyFrom(std::forward<FN>(fn), CheckStub(&Delegate::FunctorStub<FunctorType>), UtilityStub<FunctorType>);
	}
	...
	template <RESULT (*FUNCTION)(ARGS...)> static ResultOk<Delegate> Create()
	{
		...
		return Delegate(reinterpret_cast<void*>(FUNCTION), &Delegate::FunctionStub<FUNCTION>, nullptr);
	}
	...
	{
		return Delegate(objectPtr, &Delegate::MethodStub<OBJECT, METHOD_PTR>, PTMFUtilityStub);
	}
	...
	{
		return Delegate(const_cast<OBJECT*>(objectPtr), &Delegate::ConstMethodStub<OBJECT, METHOD_PTR>, PTMFUtilityStub);
	}
	...
	{
		return Delegate(&object, &Delegate::MethodStub<OBJECT, METHOD_PTR>, PTMFUtilityStub);
	}
	...
	{
		return Delegate(const_cast<OBJECT*>(&object), &Delegate::ConstMethodStub<OBJECT, METHOD_PTR>, PTMFUtilityStub);
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Bool operator <(const Delegate& rhs) const
	{
		return (_objectPtr < rhs._objectPtr) || ((_objectPtr == rhs._objectPtr) && (reinterpret_cast<const Int&>(_stubPtr) < reinterpret_cast<const Int&>(rhs._stubPtr)));
	}
...
private:
	...
	template <typename FN> Delegate& ConstructFrom(FN&& fn)
	{
		...
		new (this) DelegateBase(std::forward<FN>(fn), CheckStub(&Delegate::FunctorStub<FunctorType>), UtilityStub<FunctorType>);
		return *this;
	}

#ifdef PRIVATE_MAXON_MTABLE_PTMF
	#define PRIVATE_MAXON_STUB_STATIC
	#define PRIVATE_MAXON_STUB_THIS_PARAM
	#define PRIVATE_MAXON_STUB_THIS_PARAM_COMMA
	#define PRIVATE_MAXON_STUB_THIS this
#else
	#define PRIVATE_MAXON_STUB_STATIC static
	#define PRIVATE_MAXON_STUB_THIS_PARAM void* objectPtr
	#define PRIVATE_MAXON_STUB_THIS_PARAM_COMMA void* objectPtr,
	#define PRIVATE_MAXON_STUB_THIS objectPtr
#endif

	/// Invokes a C style function via direct call (inlines it if possible).
	template <RESULT (*FUNCTION)(ARGS...)> PRIVATE_MAXON_STUB_STATIC MAXON_ATTRIBUTE_FORCE_INLINE RESULT FunctionStub(PRIVATE_MAXON_STUB_THIS_PARAM_COMMA ARGS... args)
	{
	...
	template <typename T> PRIVATE_MAXON_STUB_STATIC MAXON_ATTRIBUTE_FORCE_INLINE RESULT FunctionIndirectStub(PRIVATE_MAXON_STUB_THIS_PARAM_COMMA ARGS... args)
	{
		T fn = reinterpret_cast<T>(PRIVATE_MAXON_STUB_THIS);
		return fn(std::forward<ARGS>(args)...);
	}

	/// Invokes a virtual member function.
	template <typename OBJECT, RESULT (OBJECT::*METHOD_PTR)(ARGS...)> PRIVATE_MAXON_STUB_STATIC MAXON_ATTRIBUTE_FORCE_INLINE RESULT MethodStub(PRIVATE_MAXON_STUB_THIS_PARAM_COMMA ARGS... args)
	{
		return (reinterpret_cast<OBJECT*>(PRIVATE_MAXON_STUB_THIS)->*METHOD_PTR)(std::forward<ARGS>(args)...);
	}

	/// Invokes a const virtual member function.
	template <typename OBJECT, RESULT (OBJECT::*METHOD_PTR)(ARGS...) const> PRIVATE_MAXON_STUB_STATIC MAXON_ATTRIBUTE_FORCE_INLINE RESULT ConstMethodStub(PRIVATE_MAXON_STUB_THIS_PARAM_COMMA ARGS... args)
	{
		return (reinterpret_cast<const OBJECT*>(PRIVATE_MAXON_STUB_THIS)->*METHOD_PTR)(std::forward<ARGS>(args)...);
	}

	template <typename T> PRIVATE_MAXON_STUB_STATIC MAXON_ATTRIBUTE_FORCE_INLINE RESULT FunctorStub(PRIVATE_MAXON_STUB_THIS_PARAM_COMMA ARGS... args)
	{
		return (*reinterpret_cast<T*>(PRIVATE_MAXON_STUB_THIS))(std::forward<ARGS>(args)...);
	}
};
...
#define yield_break % yieldResult_; if ((yieldResult_ == maxon::FAILED) || !yieldResult_.GetValue()) break;
