...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Copy-from-value constructor.
	/// Initializes the contained value with a copy of v and constructs the internal lock from l.
	//----------------------------------------------------------------------------------------
	SynchronizedValue(const T& v, LOCKTYPE&& l) : _value(v), _lock(std::move(l)) { }

	//----------------------------------------------------------------------------------------
	/// Move-from-value constructor.
	/// Initializes the contained value by moving from v and constructs the internal lock from l.
	//----------------------------------------------------------------------------------------
	SynchronizedValue(T&& v, LOCKTYPE&& l) : _value(std::move(v)), _lock(std::move(l)) { }
	...
	//----------------------------------------------------------------------------------------
	/// Executes a given function that can safely modify the inner value, potentially in an asynchronous way.
	...
	private:
		...
		SynchronizedValue* _synced = nullptr;
...
// specialization of SynchronizedValue for Serializer which doesn't provide Lock/Unlock functions
template <typename T> class SynchronizedValue<T, Serializer>
{
private:
	T _value;
	mutable typename SFINAEHelper<Serializer, T>::type _serializer;
...
public:
	...
	template <typename FN> void WriteAsync(FN&& fn)
	{
		_serializer.Enqueue([this, callback = std::forward<FN>(fn)] () -> void { callback(_value); });
	}

	template <typename FN> auto Write(FN&& fn) -> decltype(fn(_value))
	{
		return _serializer.EnqueueAndWait([this, &fn] () { return fn(_value); });
	}

	template <typename FN> auto Read(FN&& fn) const -> decltype(fn(_value))
	{
		return _serializer.EnqueueAndWait([this, &fn] () { return fn(_value); });
	}
	...
	T Get() const
	{
		return _serializer.EnqueueAndWait([this] () -> T { return _value; });
	}
