...
// Forward declare is needed on osx for NumToString.
...
public:
	...
	~InternedId()
	{
		...
		_id = reinterpret_cast<const Id*>(0xdead);
#endif
	}
	...
	static Result<InternedId> Create(INPUTTYPE&& inputId)
	{
		iferr_scope;
		InternedId iid;
		iid.Init(std::forward<INPUTTYPE>(inputId)) iferr_return;
		return iid;
	}
	...
	{
		using NumToStringHelper = typename SFINAEHelper<NumToString<NUMBER>>::type;
		// Using Id ctor to avoid copying the constexpr NumToStringHelper string.
		return InternedId::Create(Id{NumToStringHelper::value});
	}
	...
	const Id& Get() const
	{
		return _id ? *_id : Id::DefaultValue();
	}
	...
	{
		return _id ? _id->GetHashCode() : DefaultHasher::EMPTY_HASH;
	}

	/// @copydoc DefaultDoc::GetUniqueHashCode
	const UniqueHash& GetUniqueHashCode() const
	{
		return _id ? reinterpret_cast<const PrivateInfo*>(reinterpret_cast<const Char*>(_id) + INFO_OFFSET)->first : maxon::DefaultValue<const UniqueHash&>();
	}
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const InternedId& DefaultValue()
	{
	...
	static const InternedId DEFAULT_VALUE;

	static void PrivateAddReference(const Id* ptr)
	{
		if (MAXON_UNLIKELY(IsReferenced(ptr)))
			StrongReferenceCounter::Get(ptr).Inc();
	}
	...
	//----------------------------------------------------------------------------------------
	struct FastCompare : private DefaultCompare
	{
		using DefaultCompare::GetHashCode;
...
private:
	...
	{
		return MAXON_UNLIKELY((Int(ptr) & SIZEOF(Int)) == 0) && MAXON_LIKELY(ptr);
	}

	void Release()
	{
		if (MAXON_UNLIKELY(IsReferenced(_id)))
		{
	...
	static constexpr Int INFO_OFFSET = SIZEOF(Id) + SIZEOF(void*) + SIZEOF(HashInt);
...
//----------------------------------------------------------------------------------------
/// InternedIdPtr is layout-compatible with InternedId and can be used for performance reasons
/// instead of InternedId when there's no need to hold a strong reference to the internal Id.
/// You have to make sure that the strong reference is held elsewhere throughout the lifetime
/// of the InternedIdPtr.
///
...
{
public:
	// IMPORTANT: Don't add any constructor here, becauses classes with constructor are returned
	// by reference on Windows.

	// Code copied from InternedId
	...
	const UniqueHash& GetUniqueHashCode() const
	{
		return reinterpret_cast<const InternedId*>(this)->GetUniqueHashCode();
	}

	// This needs to be public so that we can construct a new InternedIdPtr without the need for a constructor.
	const Id* _id = nullptr;
};

template <> struct IsTriviallyEquatable<InternedIdPtr> : public std::true_type
{
};

template <> struct IsZeroInitialized<InternedIdPtr> : public std::true_type
{
};

inline InternedId::operator InternedIdPtr() const
{
	return {_id};
}
...
struct Member
{
	...
	{
		return LexicographicalLessThan(name, other.name, type ? type->GetId() : DefaultValue<const Id&>(), other.type ? other.type->GetId() : DefaultValue<const Id&>());
	}
	...
	//----------------------------------------------------------------------------------------
	/// FastNameCompare can be used instead of DefaultCompare when you need a fast ordered comparison
	...
	//----------------------------------------------------------------------------------------
	struct FastNameCompare : private DefaultCompare
	{
		using DefaultCompare::GetHashCode;

		static Bool IsEqual(const Member& a, const Member& b)
		{
			return InternedId::FastCompare::IsEqual(a.name, b.name);
		}

		static Bool LessThan(const Member& a, const Member& b)
		{
			return InternedId::FastCompare::LessThan(a.name, b.name);
		}
	};
...
class DataTypeLib
{
	MAXON_INTERFACE_NONVIRTUAL(DataTypeLib, MAXON_REFERENCE_STATIC, "net.maxon.interface.datatypelib", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
template <typename... T> class ContainerDataType : public DataType
{
public:
	using DataType::DataType;

	template <typename... T2> Result<const MemberMap*> GetMemberMap(const ContainerDataType<T2...>& runtimeType) const
	{
		return DataTypeLib::GetMemberMap(PrivateGetPointer(), runtimeType.PrivateGetPointer());
	}
	...
	{
		return reinterpret_cast<const GenericType&>(*this);
	}

	operator GenericType() &&
	{
		return std::move(reinterpret_cast<GenericType&>(*this));
	}

	static const ContainerDataType<>& Get(const DataType& derivedType)
	{
		DebugAssert(derivedType->PrivateGetExtraType() && (derivedType->PrivateGetExtraType()->GetValueKind() & VALUEKIND::CONTAINER_MASK) == VALUEKIND::CONTAINER);
		return reinterpret_cast<const ContainerDataType<>&>(derivedType->PrivateGetExtraType());
	}
};
