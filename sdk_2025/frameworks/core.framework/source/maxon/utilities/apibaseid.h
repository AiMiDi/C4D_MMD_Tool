...
protected:
	static const Int RC_STRING_BIT = SIZEOF(Int) * 8 - MAX_LENGTH_EXPONENT - 1; ///< Bit position in _hash of the bit which is set when the Id uses a strong reference on the string.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Returns true if this Id uses a strong reference for the internal string. Otherwise, the Id uses
	...
	template <typename T, typename = typename T::DeclarationHelper> constexpr LiteralId(int dummy, const T& decl) : LiteralId(T::DeclarationHelper::GetId())
	{
		PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(T);
	}

	//----------------------------------------------------------------------------------------
	/// @param[in] str								Must not be nullptr.
	/// @return												Hash valiue.
	//----------------------------------------------------------------------------------------
...
#define PRIVATE_MAXON_ID_HASHCODE(hash, len) (((hash) & maxon::LiteralId::LOW_HASH_MASK) + ((len) << (SIZEOF(maxon::Int) * 8 - maxon::LiteralId::MAX_LENGTH_EXPONENT)))
		state = PRIVATE_MAXON_ID_HASHCODE(state, i);
		return state;
	}

	template <typename T, typename = typename std::enable_if<std::is_base_of<InternedIdFidBase, T>::value>::type> Bool operator ==(const T& other) const
	{
...
protected:
	...
	const Char* _value; ///< C-string identifier. If the #RC_STRING_BIT is set in _hash, the pointee is held by a strong reference.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Sets this Id to str, where str has to be a dynamically allocated string which can be shared
