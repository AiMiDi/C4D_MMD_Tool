...
template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE T& operator %(ResultRef<T> ptr, ThreadReferencedError& err)
{
	...
	else
	{
		// Use CreateErrorPtr() because the compiler can omit the call if the err isn't used.
		err = CreateErrorPtr(MAXON_SOURCE_LOCATION, ERROR_TYPE::OUT_OF_MEMORY);
		// When T is const, return a reference to its DefaultValue.
		// Otherwise, we need a non-const value out of thin air. That's not possible, so return a reference to write-protected memory.
		// Then any write access will crash reliably.
...
#define PRIVATE_MAXON_iferr_ignore0() .PleaseExplainWhyTheErrorIsIgnored() 
#define PRIVATE_MAXON_iferr_ignore1(m1) .UncheckedGetValue()
#define PRIVATE_MAXON_iferr_ignore2(m1, m2) .GetValue()

#define PRIVATE_MAXON_iferr_ignore_C(count, ...) MAXON_EXPAND(PRIVATE_MAXON_iferr_ignore##count(__VA_ARGS__))
#define PRIVATE_MAXON_iferr_ignore_B(count, ...) PRIVATE_MAXON_iferr_ignore_C(count, __VA_ARGS__)
#define PRIVATE_MAXON_iferr_ignore_A(count, ...) PRIVATE_MAXON_iferr_ignore_B(count, __VA_ARGS__)

#define PRIVATE_MAXON_iferr_ignore(...) PRIVATE_MAXON_iferr_ignore_A(MAXON_VA_NARGS(__VA_ARGS__), __VA_ARGS__)
...
#define PRIVATE_iferr_throw(VAR, ...)						do { maxon::Result<void> VAR = __VA_ARGS__; VAR iferr_return; } while(false)
...
/// @endcode
//----------------------------------------------------------------------------------------
#define iferr_throw(ERR) PRIVATE_iferr_throw(MAXON_CONCAT(throw_, __LINE__), ERR)
