...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE constexpr SourceLocation(const Char* f, Int l, const Char* objectName = nullptr) : file(f), lineAndFlags(l), name(objectName)
	{
	}
...
/// You could as well write SourceLocation(MAXON_FILE, __LINE__) in your code, but the purpose of this
/// macro is to be able to easily modify the inner workings of the memory leak detector without having to
/// modify all callers.
//----------------------------------------------------------------------------------------
#ifndef MAXON_DISABLE_SOURCE_LOCATION
	#define	MAXON_SOURCE_LOCATION							maxon::SourceLocation(MAXON_FILE, __LINE__)
#else
	#define	MAXON_SOURCE_LOCATION							maxon::SourceLocation(nullptr, 0)
#endif
...
#define	MAXON_SOURCE_LOCATION_NAME(x)						maxon::SourceLocation(MAXON_FILE, __LINE__, maxon::PrivateGetClassName<x>())


/// MAXON_FUNCTION_SIGNATURE return the name of the function the code is located in
#ifdef MAXON_TARGET_WINDOWS
#define MAXON_FUNCTION_SIGNATURE __FUNCSIG__
#else
#define MAXON_FUNCTION_SIGNATURE __PRETTY_FUNCTION__
#endif
...
#undef MAXON_SOURCE_LOCATION_NAME
	...
	template <StringLiteral FILENAME, int LINE> struct SourceLocationStruct
	{
			static constexpr SourceLocation loc = {FILENAME.value, LINE};
	};

	#define MAXON_SOURCE_LOCATION maxon::SourceLocationStruct<maxon::StringLiteral<maxon::GetStringLengthWith0(MAXON_FILE)>(MAXON_FILE, true), __LINE__>::loc
	...
	#define MAXON_SOURCE_LOCATION \
		...
		{ \
			static constexpr maxon::SourceLocation sourceLocation = maxon::SourceLocation(MAXON_FILE, __LINE__, nullptr); \
