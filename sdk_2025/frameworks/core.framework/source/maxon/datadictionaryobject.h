...
template <typename INTERFACE> struct DataDictionaryKeySet
{
	template <typename CHECK> struct Check
	{
		...
		static const Int mask = (GetRestrict<INTERFACE>::value ? (STD_IS_REPLACEMENT(base_of, typename GetRestrict<INTERFACE>::type, CHECK) ? 3 : 2) : 0)
...
public:
	...
	template <typename REFCLASS, typename T, typename KEY> MAXON_FUNCTION Result<void> Set(KEY&& key, T&& data)
	{
		...
		Data tmp;
		iferr (tmp.SetImpl<TTT>(std::forward<T>(data), OVERLOAD_MAX_RANK))
			return err;
...
{
	MAXON_INTERFACE(DataDictionaryReferenceObjectInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.datadictionaryreferenceobject", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
//----------------------------------------------------------------------------------------
/// Implementation of the DataDictionaryObjectInterface without locking, so it's not threadsafe. Uses a BlockArrayAllocator to store data faster than DataDictionaryObjectClass
...
DataDictionaryIterator::DataDictionaryIterator(const DataDictionaryObjectInterface* dict, Bool ends) : _hasIterator(true), _pair(DefaultValue<const Data&>(), DefaultValue<const Data&>())
{
