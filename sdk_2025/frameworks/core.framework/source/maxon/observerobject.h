...
{
	MAXON_INTERFACE(ObserverObjectInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.observerobject", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
MAXON_DECLARATION(Class<ObserverObjectRef>, ObserverObjectClass, "net.maxon.class.observerobject", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	void Free()
	{
		ObservableRef<T> o = _observable.Get(_notificationObject, _observableId, false);
		if (o)
		{
			o.Free();
		}
		_notificationObject = ObserverObjectRef();
		_observable = ObservableType();
	}

	ObservableRef<T> Get(Bool create)
	{
		return _observable.Get(_notificationObject, _observableId, create);
	}
...
private:
	...
	const char*				_observableId = nullptr;
};
