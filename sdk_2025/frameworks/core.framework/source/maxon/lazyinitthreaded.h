...
/// Init() and after a OperationCancelledError Init() will retry the initialization.
///
...
public:
	...
	{
		decltype(fn()) result = decltype(fn())();
		Int					 value = _state.LoadAcquire();
		Int					 state = value & STATE_MASK;
			...
			while (state == UNINITIALIZED || state == PENDING)
			{
				...
				if ((state == UNINITIALIZED) && _state.TryCompareAndSwap(PENDING, UNINITIALIZED))
				{
					...
					auto umbrella = [&fn, &job, callerJob, options, this]() -> decltype(fn()) {
						...
						const void* previousResource = options & OPTIONS::NO_EXCLUSIVE_RESOURCE ? nullptr : ThreadServices::ThreadedExclusiveResource(1, this);
						auto				fnResult = fn();
			...
			if (ptr)
			{
				...
				iferr (result)
				{
					if ((options & OPTIONS::RETRY_ON_FAILURE) ||
							(options & OPTIONS::RETRY_ON_CANCELLATION && err.IsCancellation()))
					{
				...
				do
				{
					if ((value & CHECKING) == false)
					{
						...
						if (_state.TryCompareAndSwap(value | CHECKING, value))
						{
							...
							// If the initialization has finished successfully the job might be a nullptr.
							if (job)
							{
								const void* previousResource = options & OPTIONS::NO_EXCLUSIVE_RESOURCE ? nullptr : ThreadServices::ThreadedExclusiveResource(1, this);
