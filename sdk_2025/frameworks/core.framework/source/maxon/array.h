...
class ArrayFactory
{
	MAXON_INTERFACE_NONVIRTUAL(ArrayFactory, MAXON_REFERENCE_STATIC, "net.maxon.interface.arrayfactory", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
public:
	//----------------------------------------------------------------------------------------
	...
	/// 															If this is nullptr, the default value of the element type will be used.
	...
	{
		iferr_scope;
		Array<Generic>::NonConst res = NewPagedArrayFromContainerType(maxon::GetDataType<ContainerInterface<T>>(), reinterpret_cast<const Generic*>(&DefaultValue<const T&>()), false) iferr_return;
		return std::move(reinterpret_cast<typename Array<T>::NonConst&>(res));
	}
	...
	template <typename T> static Result<typename Array<T>::NonConst> PrivateNewPagedArrayFromBase(const Array<T>& base)
	{
		...
		if (base.IsPopulated())
		{
			res = PrivateNewPagedArrayFromBase(reinterpret_cast<const Array<Generic>&>(base), reinterpret_cast<const Generic*>(&DefaultValue<const T&>()), false) iferr_return;
		}
		else
		{
			res = NewPagedArrayFromContainerType(maxon::GetDataType<ContainerInterface<T>>(), reinterpret_cast<const Generic*>(&DefaultValue<const T&>()), false) iferr_return;
		}
		return std::move(reinterpret_cast<typename Array<T>::NonConst&>(res));
	}
...
//		DebugAssert(System::GetReferenceCounter(&src) == 0);
	...
	Result<Int> InitIterator(ViewIteratorMember* members, const MemberMap* map, Char* state, PersistentIdBuffer* idBuffer, GenericContainerInterface::ITERATOR_ACCESS access) const
	{
