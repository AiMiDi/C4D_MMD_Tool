...
constexpr const Int PARALLELFOR_DISABLETHREADING = LIMIT<Int>::MAX;
constexpr const Int PARALLELFOR_MAXIMUMDEPTH = 8;
constexpr const Int PARALLELFOR_DEFAULTGRANULARITY = 16;
constexpr const Int PARALLELFOR_MINIMUMGRANULARITY = 1;
[[deprecated("Use ParallelFor::Granularity")]] constexpr const Int PARALLELFOR_USEMAXIMUMTHREADS = 0;
...
#define DISABLE_IF_LOOP_TYPE_IS_INT(TYPE, ...) typename std::enable_if<!STD_IS_REPLACEMENT(convertible, typename std::remove_reference<TYPE>::type, maxon::ParallelFor::Granularity), __VA_ARGS__>::type
...
public:
	...
	struct MinimumGranularity : public Granularity
	{
		constexpr MinimumGranularity() : Granularity(1) { }
	};

	struct DefaultGranularity : public Granularity
	{
		constexpr DefaultGranularity() : Granularity(16) { }
	};

	struct ThreadingDisabled : public Granularity
	{
		constexpr ThreadingDisabled() : Granularity(PARALLELFOR_DISABLETHREADING) { } 
	};
	...
	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline auto Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0) -> decltype(obj(to))
	{
		// Select either NoContext or NoBreakContext based on return value type.
		using Context = typename NoContextSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, granularity, queue, deprecatedThrCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation,  const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, observeForCancellation, granularity, queue, deprecatedThrCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), granularity, queue, deprecatedThrCnt);
	}
	...
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		...
		Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0);

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0);
	...
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, granularity, queue, deprecatedThrCnt);
	}
	...
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		...
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobStatusInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, observeForCancellation, granularity, queue, deprecatedThrCnt);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, reinterpret_cast<JobStatusInterface*>(observeForCancellation), granularity, queue, deprecatedThrCnt);
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline auto Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0) -> decltype(obj(to))
	{
		// Select either NoContext or NoBreakContext based on return value type.
		using Context = typename NoContextSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, granularity, queue, deprecatedThrCnt);
	}
	
	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, observeForCancellation, granularity, queue, deprecatedThrCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), granularity, queue, deprecatedThrCnt);
	}
	...
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static MAXON_ATTRIBUTE_NO_INLINE typename CONTEXT::ResultValueType
		Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0);

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0);
	...
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		static inline DISABLE_IF_LOOP_TYPE_IS_INT(LOOP, typename CONTEXT::ResultValueType)
		Static(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, const Granularity& granularity = Granularity(), JobQueueInterface* queue = JOBQUEUE_CURRENT, Int deprecatedThrCnt = 0)
	{
		return Static<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, granularity, queue, deprecatedThrCnt);
	}
	...
	// Deprecated version of ParallelFor which will be removed.
	
	 template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static inline auto Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT) -> decltype(obj(to))
	{
		// Select either NoContext or NoBreakContext based on return value type.
		using Context = typename NoContextSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static inline Result<void> Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, observeForCancellation, ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static inline Result<void> Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static MAXON_ATTRIBUTE_NO_INLINE typename CONTEXT::ResultValueType
		Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		[[deprecated("Use ParallelFor::Granularity")]]  static inline DISABLE_IF_LOOP_TYPE_IS_INT(LOOP, typename CONTEXT::ResultValueType)
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, ParallelFor::Granularity(granularity), queue, threadCnt);
	}
	...
	template <typename CONTEXT, PARALLELFORFLAGS FLAGS, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
			...
			Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, FLAGS, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, ParallelFor::Granularity(granularity), queue, threadCnt);
	}
	
	template <typename CONTEXT, PARALLELFORFLAGS FLAGS, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		[[deprecated("Use ParallelFor::Granularity")]] static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobStatusInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, FLAGS, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, reinterpret_cast<JobStatusInterface*>(observeForCancellation), ParallelFor::Granularity(granularity), queue, threadCnt);
	}
...
template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		...
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobStatusInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, observeForCancellation, ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		[[deprecated("Use ParallelFor::Granularity")]] static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, reinterpret_cast<JobStatusInterface*>(observeForCancellation), ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static inline auto Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, Int threadCnt, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT) -> decltype(obj(to))
	{
		// Select either NoContext or NoBreakContext based on return value type.
		using Context = typename NoContextSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, ParallelFor::Granularity(minChunkSize), queue, threadCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static inline Result<void> Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, observeForCancellation, ParallelFor::Granularity(minChunkSize), queue, threadCnt);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static inline Result<void> Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobInterface* observeForCancellation, Int threadCnt, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), ParallelFor::Granularity(minChunkSize), queue, threadCnt);
	}


	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static MAXON_ATTRIBUTE_NO_INLINE typename CONTEXT::ResultValueType
		Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Static<CONTEXT, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, ParallelFor::Granularity(granularity), queue, threadCnt);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		[[deprecated("Use ParallelFor::Granularity")]] static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Static<CONTEXT, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), ParallelFor::Granularity(granularity), queue, threadCnt);
	}
	
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		[[deprecated("Use ParallelFor::Granularity")]] static inline DISABLE_IF_LOOP_TYPE_IS_INT(LOOP, typename CONTEXT::ResultValueType)
		Static(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, Int threadCnt, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Static<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, ParallelFor::Granularity(minChunkSize), queue, threadCnt);
	}
	...
	template <typename CONTEXT, PARALLELFORFLAGS FLAGS, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		...
		Static(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, Int threadCnt, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Static<CONTEXT, FLAGS, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, ParallelFor::Granularity(minChunkSize), queue, threadCnt);
	}
...
private:
	...
	public:
		...
		// With NoContextWithResult the loop does not return an error but the loop might be cancelled and therefore ParallelFor returns a Result<void>.
		using ResultValueType = Result<void>;
	...
	/// Helper template for lambdas with Result<void> and without context.
	//----------------------------------------------------------------------------------------
	template <typename RESULTVALUETYPE> class NoContextWithResultSelector
	{
	public:
		using Context = NoBreakContext;
	};
	...
	template <typename USERCONTEXT, PARALLELFORFLAGS FLAGS, typename INDEXTYPE, typename INIT, typename FINALIZE, typename WORKERJOB> struct alignas(MAXON_FALSE_SHARING_SIZE) ForState;
	...
	// Runs single threaded finalizers when requested and destructs the contexts.
	template <typename WORKER, Bool FINALIZE, Bool DESTRUCT> class SingleThreadedFinalize
	{
	public:
		explicit SingleThreadedFinalize(StaticJobArray<WORKER>& jobs) : _jobs(jobs)
		{
		}
		
		void Finalize()
		{
			using Context = typename WORKER::JobContext;
			for (WORKER& job : _jobs)
			{
				Context& localContext = job.GetContext();
				if constexpr (FINALIZE)
					localContext.Finalize();

				// For single-threaded finalizers the context is destructed here.
	...
	// Dummy for all cases where finalize is threaded or should not happen at all.
	template <typename WORKER> class SingleThreadedFinalize<WORKER, false, false>
	{
	public:
		explicit SingleThreadedFinalize(StaticJobArray<WORKER>&) {}
		void Finalize() { }
	};
...
//----------------------------------------------------------------------------------------
/// Helper template for lambdas without return value and without context which may return an error due to cancellation.
//----------------------------------------------------------------------------------------
template <> class ParallelFor::NoContextWithResultSelector<void>
{
public:
	using Context = ParallelFor::NoContextWithResult;
};
