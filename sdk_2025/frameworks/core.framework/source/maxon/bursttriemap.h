...
template <typename K, typename V, Int SIZE> struct BurstTrieBucket
{
	...
	void Remove(Int index, Int end, Int removeCnt)
	{
		while (index + removeCnt < end)
		{
			...
			reinterpret_cast<V*>(values)[index] = std::move(reinterpret_cast<V*>(values)[index + removeCnt]);
			++index;
		}
		while (--removeCnt >= 0)
		{
			reinterpret_cast<V*>(values)[index + removeCnt].~V();
		}
	}
...
{
public:
	static_assert(STD_IS_REPLACEMENT(unsigned, K), "Only unsigned integral types may be used as keys for BurstTrieMap.");
	...
	public:
		...
		MAXON_OPERATOR_COPY_ASSIGNMENT(IteratorBase);
	...
	protected:
#if MAXON_COMPILER_MSVC == 1939 && defined(MAXON_TARGET_RELEASE)
	// MSVC v142 fix, we need to use MAXON_ATTRIBUTE_NO_INLINE otherwise compiler internal error
		...
		ResultMem ChangeKey(BurstTrieMap& map, K newKey)
		{
			...
			if ((pos == this->_rootIndex) && (((this->GetKey() ^ newKey) >> (pos - (this->_top + 1)) * GROUP_BITS) == 0))
			{
				...
				if (!this->_bucket->template Find<SORT>(newKey, cnt, index))
				{
					...
					else if (STD_IS_REPLACEMENT(empty, V))
					{
	...
	ResultMemT<Iterator> InsertEntry(K key, Bool& created = BoolLValue())
	{
		...
		while (true)
		{
			if (!*i)
			{
				...
				it._bucketIndex = 0;
				if (!STD_IS_REPLACEMENT(empty, V))
					PlacementNew<V, true>(&it._bucket->GetValue(0));
				return it;
			}
			if (Pool::IsNode(*i))
			{
			...
			else
			{
				...
				if (cnt < BUCKET_SIZE)
				{
					...
					if (it._bucketIndex == cnt)
					{
						...
						if (!STD_IS_REPLACEMENT(empty, V))
							PlacementNew<V, true>(&it._bucket->GetValue(cnt));
					}
					else
					{
	...
	Result<void> CopyNodes(Index& dest, const BurstTrieMap& srcMap, Index src)
	{
		...
		else
		{
			...
			if (!STD_IS_REPLACEMENT(empty, V))
			{
				...
				for (Int i = 0; i < cnt; ++i)
				{
					PlacementNew<V, true>(&d->GetValue(i));
				}
				for (Int i = 0; i < cnt; ++i)
				{
