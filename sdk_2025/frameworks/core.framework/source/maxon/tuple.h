...
{
	using TT = typename std::conditional<STD_IS_REPLACEMENT(reference, T), void*, T>::type;
	return UInt64(sizeof(TT)) + ((UInt64(alignof(TT) * 2 + STD_IS_REPLACEMENT(reference, T))) << 32);
}
...
public:
	...
	TupleStorageBase()
	{
		DefaultConstruct(static_cast<IndexSequence*>(nullptr));
	}
...
private:
	...
	{
		ForEach{PlacementNew<T, false>(&this->GetMember((typename std::integral_constant<UInt32, I>::type*) nullptr, false))...};
	}
};
...
template <typename... T> class TupleStorage : public TupleStorageBase<And<STD_IS_REPLACEMENT(default_constructible, T)...>::value, And<STD_IS_REPLACEMENT(trivially_destructible, T)...>::value, T...>
{
	...
	using IndexSequence = std::index_sequence_for<T...>;
	static const Bool COPY_CONSTRUCTIBLE = And<STD_IS_REPLACEMENT(copy_constructible, T)...>::value;
	static const Bool COPY_ASSIGNABLE = And<STD_IS_REPLACEMENT(copy_assignable, T)...>::value;
	static const Bool MOVE_CONSTRUCTIBLE = And<STD_IS_REPLACEMENT(move_constructible, T)...>::value;
	static const Bool MOVE_ASSIGNABLE = And<STD_IS_REPLACEMENT(move_assignable, T)...>::value;
...
public:
	...
	constexpr TupleStorage() : first()
	{
	}

	template <typename A1> constexpr TupleStorage(std::index_sequence<0>*, A1&& a1) : first(std::forward<A1>(a1))
	{
	}
...
public:
	...
	constexpr TupleStorage() : first(), second()
	{
	}

	template <typename A1, typename A2> constexpr TupleStorage(std::index_sequence<0, 1>*, A1&& a1, A2&& a2) : first(std::forward<A1>(a1)), second(std::forward<A2>(a2))
	{
	}
	...
	static_assert(!STD_IS_REPLACEMENT(same, typename std::decay<T2>::type, UnitType), "The UnitType specialization of Tuple should be used.");
...
public:
	...
	constexpr TupleStorage() : first(), second(), third()
	{
	}

	template <typename A1, typename A2, typename A3> constexpr TupleStorage(std::index_sequence<0, 1, 2>*, A1&& a1, A2&& a2, A3&& a3) : first(std::forward<A1>(a1)), second(std::forward<A2>(a2)), third(std::forward<A3>(a3))
	{
	}
...
public:
	...
	constexpr TupleStorage() : first(), second(), third(), fourth(), fifth()
	{
	}

	template <typename A1, typename A2, typename A3, typename A4, typename A5> constexpr TupleStorage(std::index_sequence<0, 1, 2, 3, 4>*, A1&& a1, A2&& a2, A3&& a3, A4&& a4, A5&& a5) : first(std::forward<A1>(a1)), second(std::forward<A2>(a2)), third(std::forward<A3>(a3)), fourth(std::forward<A4>(a4)), fifth(std::forward<A5>(a5))
	{
	}
...
public:
	...
	template <typename A1, typename A2> constexpr TupleStorage(std::index_sequence<0, 1>*, A1&& a1, A2&& a2) : first(std::forward<A1>(a1))
	{
	}
...
template <typename T1> class TupleStorage<T1, const UnitType&> : public TupleStorage<T1, UnitType>
{
public:
	...
	template <typename A1, typename A2> constexpr TupleStorage(std::index_sequence<0, 1>* seq, A1&& a1, A2&& a2) : TupleStorage<T1, UnitType>(seq, std::forward<A1>(a1), std::forward<A2>(a2))
	{
	}
};
...
{
	iferr_scope;
	AssignCopy(a1.first, a1.GetSecond()) iferr_return;
...
template <typename T1, typename... T> inline typename SFINAEHelper<String, T1>::type TupleToString(const FormatStatement* fs, const T1& a1, const T&... rest)
{
	...
	{
		return GlobalToString(a1, fs) + CONSTSTRING("/") + TupleToString(fs, rest...);
	}
}
...
{
	return DefaultCompare::IsEqual(a1.first, a1.GetSecond()) && TupleEquals(rest...);
}
...
{
	return CompareTemplate<true>::LessThan(a1.first, a1.GetSecond()) || (sizeof...(T) > 0 && DefaultCompare::IsEqual(a1.first, a1.GetSecond()) && TupleLessThan(rest...));
}

template <typename T1, typename T2> inline TupleStorage<T1&, typename std::conditional<std::is_same<T2, UnitType>::value, UnitType, const T2&>::type> TuplePair(T1& t1, const T2& t2)
{
...
template <typename... T> class Tuple : public maxon::details::TupleStorage<T...>
{
	...
	static const Bool COPYABLE = And<STD_IS_REPLACEMENT(copy_constructible, T)...>::value;
...
public:
	...
	{
		// If there are no padding bytes and each member is trivially hashable, the Tuple is trivially hashable as well.
		return (SIZEOF(Tuple) == (0 + ... + SIZEOF(T))) && (... && IsTriviallyHashable<T>::value);
	}
	...
	<
		typename... U,
		typename = typename std::enable_if<And<STD_IS_REPLACEMENT(constructible, T, const U&)...>::value>::type
	>
	...
	<
		typename... U,
		typename = typename std::enable_if<And<STD_IS_REPLACEMENT(constructible, T, U&&)...>::value>::type
	>
	...
	/// @param[in] x									Values.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT constexpr Tuple(const typename std::conditional<COPYABLE, T, volatile DummyParamType>::type&... x) : Super((IndexSequence*) nullptr, x...)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename HASH = DefaultCompare> HashInt GetHashCode() const
	{
		return GetHashCodeImpl<HASH>(static_cast<IndexSequence*>(nullptr));
	}

	//----------------------------------------------------------------------------------------
	/// Returns the hash value generated from the elements of this tuple.
	//----------------------------------------------------------------------------------------
	template <typename HASH = DefaultCompare> UniqueHash GetUniqueHashCode() const
	{
		if constexpr (TriviallyHashable())
			return UniqueHash::GetUniqueHashCode(reinterpret_cast<const Byte*>(this), SIZEOF(Tuple));
		else
			return GetUniqueHashCodeImpl<HASH>(static_cast<IndexSequence*>(nullptr));
	}
...
private:
	...
	template <typename HASH, size_t... I> HashInt GetHashCodeImpl(std::index_sequence<I...>*) const
	{
		return DefaultHasher::CombineHashes(HASH::GetHashCode(Get<I>())...);
	}

	template <typename HASH, size_t... I> UniqueHash GetUniqueHashCodeImpl(std::index_sequence<I...>*) const
	{
		return HASH::GetCombinedUniqueHash(Get<I>()...);
	}
...
public:
	...
	/// @return												Always "()"_s.
	//----------------------------------------------------------------------------------------
	const String& ToString(const FormatStatement* fmt = nullptr) const { return CONSTSTRING("()"); }

	//----------------------------------------------------------------------------------------
	/// Calculates hash code of an empty tuple.
	/// @return												Always 0.
	//----------------------------------------------------------------------------------------
	template <typename HASH = void> HashInt GetHashCode() const { return DefaultHasher::EMPTY_HASH; }

	template <typename HASH = void> UniqueHash GetUniqueHashCode() const { return UniqueHash(); }
