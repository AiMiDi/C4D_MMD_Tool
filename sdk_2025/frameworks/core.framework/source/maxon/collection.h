...
enum class COLLECTION_RESIZE_FLAGS
{
	...
	ON_RESIZE_MAKE_WRITABLE	= (1 << 5),	///< The resize operation shall ensure that all elements are writable. For most collections, elements are writable anyway, but e.g. for the paged array that's not the case.
...
//----------------------------------------------------------------------------------------
template <typename COLLECTION, typename T> inline typename std::conditional<!STD_IS_REPLACEMENT(reference, COLLECTION) && (GetCollectionKind<COLLECTION>::value != COLLECTION_KIND::NONE), T&&, T&>::type ValueForward(T& x)
{
	return static_cast<typename std::conditional<!STD_IS_REPLACEMENT(reference, COLLECTION) && (GetCollectionKind<COLLECTION>::value != COLLECTION_KIND::NONE), T&&, T&>::type>(x);
};
...
//----------------------------------------------------------------------------------------
template <typename COLLECTION, typename T, Bool STRIDED> inline const Block<T, STRIDED, !STD_IS_REPLACEMENT(reference, COLLECTION) && (GetCollectionKind<COLLECTION>::value != COLLECTION_KIND::NONE)>& BlockForward(const Block<T, STRIDED>& block)
{
	return reinterpret_cast<const Block<T, STRIDED, !STD_IS_REPLACEMENT(reference, COLLECTION) && (GetCollectionKind<COLLECTION>::value != COLLECTION_KIND::NONE)>&>(block);
};
...
public:
...
#else
	template <typename C, typename S> MAXON_ATTRIBUTE_FORCE_INLINE Bool operator ==(const BaseCollection<C, S>& other) const
	{
		static_assert(COLLECTION::KIND == C::KIND, "Can't check collections of different kinds for equality.");
		return static_cast<const COLLECTION*>(this)->IsEqualImpl(static_cast<const C&>(other), DefaultCompare(), OVERLOAD_MAX_RANK);
	}
#endif

	template <typename COMPARE = EqualityCompare, typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value && !STD_IS_REPLACEMENT(same, typename std::decay<COMPARE>::type, EQUALITY), Bool>::type IsEqual(const COLLECTION2& other, COMPARE&& cmp = COMPARE()) const
	{
		static_assert(COLLECTION::KIND == COLLECTION2::KIND, "Can't check collections of different kinds for equality.");
	...
	template <typename COLLECTION2> Bool Intersects(const COLLECTION2& other) const
	{
		...
		{
			for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
			{
				if (maxon::Contains(other, COLLECTION::GetMapKey(*it)))
					return true;
			}
		}
		else
		{
			for (auto&& it : other)
			{
				if (static_cast<const COLLECTION*>(this)->Contains(COLLECTION2::GetMapKey(it)))
					return true;
			}
		}
		return false;
	...
	{
		for (AutoIterator<COLLECTION> it(*static_cast<COLLECTION*>(this)); it; )
		{
			if (!maxon::Contains(std::forward<COLLECTION2>(other), COLLECTION::GetMapKey(*it)))
				it.Erase(*static_cast<COLLECTION*>(this));
			else
				++it;
		}
		return OK;
...
public:
	...
	operator ValueReceiver<typename std::conditional<STD_IS_REPLACEMENT(scalar, VALUETYPE), VALUETYPE, DummyParamType&>::type>()
	{
		return [this] (VALUETYPE&& v) -> Result<Bool>
		{
			iferr_scope;
...
//
// 	operator ValueReceiver<typename std::conditional<alignof(PrivateValueReceiverType) <= alignof(void*) && !STD_IS_REPLACEMENT(abstract, PrivateValueReceiverType), typename std::remove_const<VALUETYPE>::type, EmptyClass>::type>()
// 	{
...
public:
	...
	template <typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value, Bool>::type operator <(const COLLECTION2& other) const
	{
		static_assert(COLLECTION::KIND == COLLECTION2::KIND, "Can't compare collections of different kinds.");
		return static_cast<const COLLECTION*>(this)->CompareImpl(other, DefaultCompare(), OVERLOAD_MAX_RANK) == COMPARERESULT::LESS;
	}

	template <typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value, Bool>::type operator <=(const COLLECTION2& other) const
	{
		static_assert(COLLECTION::KIND == COLLECTION2::KIND, "Can't compare collections of different kinds.");
		static_assert(COMPARERESULT::INCOMPARABLE > COMPARERESULT::EQUAL);
		return static_cast<const COLLECTION*>(this)->CompareImpl(other, DefaultCompare(), OVERLOAD_MAX_RANK) <= COMPARERESULT::EQUAL;
	}

	template <typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value, Bool>::type operator >(const COLLECTION2& other) const
	{
		static_assert(COLLECTION::KIND == COLLECTION2::KIND, "Can't compare collections of different kinds.");
		return static_cast<const COLLECTION*>(this)->CompareImpl(other, DefaultCompare(), OVERLOAD_MAX_RANK) == COMPARERESULT::GREATER;
	}

	template <typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value, Bool>::type operator >=(const COLLECTION2& other) const
	{
		static_assert(COLLECTION::KIND == COLLECTION2::KIND, "Can't compare collections of different kinds.");
		const COMPARERESULT cmp = static_cast<const COLLECTION*>(this)->CompareImpl(other, DefaultCompare(), OVERLOAD_MAX_RANK);
		return cmp == COMPARERESULT::EQUAL || cmp == COMPARERESULT::GREATER;
	}
#else
	template <typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value, Bool>::type operator<(const COLLECTION2& other) const
	{
	...
	template <typename C, typename S> MAXON_ATTRIBUTE_FORCE_INLINE COMPARERESULT operator <=>(const BaseCollection<C, S>& other) const
	{
		static_assert(COLLECTION::KIND == C::KIND, "Can't check collections of different kinds for equality.");
		return static_cast<const COLLECTION*>(this)->CompareImpl(other, DefaultCompare(), OVERLOAD_MAX_RANK);
	}
#endif

	template <typename COMPARE = EqualityCompare, typename COLLECTION2> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<maxon::IsCollection<COLLECTION2>::value, COMPARERESULT>::type Compare(const COLLECTION2& other, COMPARE&& cmp = COMPARE()) const
	{
		static_assert(COLLECTION::KIND == COLLECTION2::KIND, "Can't check collections of different kinds for equality.");
		return static_cast<const COLLECTION*>(this)->CompareImpl(other, cmp, OVERLOAD_MAX_RANK);
	}
	...
	template <typename COLLECTION2> Result<void> InsertAll(Int index, COLLECTION2&& other, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::ON_GROW_RESERVE_CAPACITY)
	{
		...
		else
		{
			...
			if (IS_ARRAY && STD_IS_REPLACEMENT(same, const VALUETYPE, const typename std::decay<ARRAY2>::type::ValueType))
			{
				...
				{
					static_cast<COLLECTION*>(this)->InsertBlock(index, BlockForward<COLLECTION2>(block)) iferr_return;
					index += block.GetCount();
				}
			}
			else
			{
				static_cast<COLLECTION*>(this)->InsertBlock(index, Block<const VALUETYPE>(nullptr, cnt)) iferr_return;
				...
				for (auto&& i : std::forward<COLLECTION2>(other))
				{
					...
					if (res == FAILED)
					{
						...
						while (it != end)
						{
							PlacementNew<VALUETYPE, false>(&*it);
							++it;
						}
						return res;
					}
	...
	template <typename COLLECTION2, typename COMPARE> COMPARERESULT CompareImpl(const COLLECTION2& other, COMPARE&& cmp, OverloadRank0) const
	{
		if (this == (const void*) &other)
		{
		...
		AutoIterator<const COLLECTION> it1(*static_cast<const COLLECTION*>(this));
		for (AutoIterator<const COLLECTION2> it2(other); ; ++it1, ++it2)
		{
			if (!it1)
				return it2 ? COMPARERESULT::LESS : COMPARERESULT::EQUAL;
			if (!it2)
				return COMPARERESULT::GREATER;
			const COMPARERESULT c = cmp.Compare(*it1, *it2);
	...
	HashInt GetHashCode() const
	{
		...
		for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
		{
			hash.Combine(HASH::GetHashCode(*it));
		}
		return hash;
	}
	...
	UniqueHash GetUniqueHashCode() const
	{
		...
		for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
		{
			...
			state.HashAndCombine<HASH>(value);
		}
		return state;
	}
...
public:
	...
	template <typename COLLECTION2, typename COMPARE> Bool IsEqualImpl(const COLLECTION2& other, COMPARE&& cmp, OverloadRank0) const
	{
		// CriticalAssert((STD_IS_REPLACEMENT(same, typename std::decay<COMPARE>::type, DefaultCompare)), "Currently only DefaultCompare is supported.");
		if (this == (const void*) &other)
		{
			return true;
		}
	...
	HashInt GetHashCode() const
	{
		...
		for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
		{
			hash.CombineUnordered(HASH::GetHashCode(*it));
		}
		return hash;
	}
	...
	UniqueHash GetUniqueHashCode() const
	{
		...
		for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
		{
			hash.CombineUnordered(HASH::GetUniqueHashCode(*it));
		}
		return hash;
	}
};
...
public:
	...
	{
		auto value = static_cast<const COLLECTION*>(this)->FindValue(pair.GetKey());
	...
	HashInt GetHashCode() const
	{
		...
		for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
		{
			hash.CombineUnordered(DefaultHasher::CombineHashes(HASH::GetHashCode((*it).GetKey()), HASH::GetHashCode((*it).GetValue())));
		}
		return hash;
	}
	...
	UniqueHash GetUniqueHashCode() const
	{
		...
		for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
		{
			hash.CombineUnordered(UniqueHasher::CombineHashes(HASH::GetUniqueHashCode((*it).GetKey()), HASH::GetUniqueHashCode((*it).GetValue())));
		}
		return hash;
	}
};
