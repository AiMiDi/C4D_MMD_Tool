...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Constructs the object and sets the value d.
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit Vector2f64(Float64 d, Bool setAll = true)
	{
		_val = setAll ? _mm_set1_pd(d) : _mm_set_pd(0.0, d);
	}
	...
	/// @return												A vector consisting of R0=A[ma1], R1=B[mb1]
	//----------------------------------------------------------------------------------------
	template <int mb1, int ma1> static MAXON_ATTRIBUTE_FORCE_INLINE Vector2f64 Shuffle(const Vector2f64& a)
	{
		return Vector2f64(_mm_shuffle_pd(a._val, a._val, _MM_SHUFFLE2(mb1, ma1)));
	}
	...
	//----------------------------------------------------------------------------------------
	/// Unpacks the high values of the input vectors..
	/// @param[in] a									Input vector A.
	/// @param[in] b									Input vector B.
	/// @return												A vector consisting of R0=A[ma1], R1=B[mb1]
	//----------------------------------------------------------------------------------------
	static MAXON_ATTRIBUTE_FORCE_INLINE Vector2f64 UnpackHigh(const Vector2f64& a, const Vector2f64& b)
	{
		return Vector2f64(_mm_unpackhi_pd(a._val, b._val));
	}
...
protected:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector2f64 operator +(const Vector2f64& a, const Vector2f64& b)
	{
		return Vector2f64(a.v[0] + b.v[0], a.v[1] + b.v[1]);
	}
	...
	template <int mb1, int ma1> static MAXON_ATTRIBUTE_FORCE_INLINE Vector2f64 Shuffle(const Vector2f64& a)
	{
		return Vector2f64(a.v[ma1], a.v[mb1]);
	}

protected:
	alignas(16) Float64 v[2];

#endif // C4D_HAS_SSE2_CAPABILITY

};

static_assert(sizeof(Vector2f64) == sizeof(__m128d), "size mismatch");
