...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Constructs a value of this type at @p dest using its default constructor.
	...
	//----------------------------------------------------------------------------------------
	/// Destructs a value of this type at @p dest.
	/// @param[in,out] dest						Pointer to the value.
	//----------------------------------------------------------------------------------------
	...
	/// type which might modify @p src. The blocks may overlap.
	/// @param[in,out] dest						Pointer to the destination array.
	/// @param[in] dstride						Stride in bytes between consecutive destination elements.
	/// @param[in,out] src						Pointer to the source array.
	/// @param[in] sstride						Stride in bytes between consecutive source elements.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Returns true if this type is a valid non-abstract type (so that you can create an object of the type).
	/// @return												True if this type is concrete, false if it is abstract or null.
	//----------------------------------------------------------------------------------------
	Bool IsConcrete() const
	{
		return _ptr && !(_ptr->_valueKind & VALUEKIND::ABSTRACT);
	}
	...
	{
		return IsCastableFromImpl(_ptr, other._ptr);
	}
	...
	{
		return IsCastableFromImpl(other._ptr, _ptr);
	}
...
private:
	...
	static Bool IsCastableFromImpl(const DataTypeImpl* type, const DataTypeImpl* other);
...
public:
	...
	//----------------------------------------------------------------------------------------
	template <typename T> Result<typename std::conditional<GetCollectionKind<T>::value == COLLECTION_KIND::ARRAY || std::is_void_v<T>, T, std::add_lvalue_reference_t<const T>>::type> Get() const
	{
...
private:
	...
	template <typename T> Result<const T&> GetImpl(OverloadRank0) const
	{
		const T* ptr = GetPtr<T>();
		if (!ptr)
