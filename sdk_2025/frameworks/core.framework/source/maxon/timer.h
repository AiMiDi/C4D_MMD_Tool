...
static constexpr Milliseconds MINSLEEPDURATION = Milliseconds(10.0);
class TimerRef;
...
{
	MAXON_INTERFACE_NONVIRTUAL(TimerInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.timer", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
//----------------------------------------------------------------------------------------
/// @brief Helper class to automatically cancel and destroy a timer on shutdown
/// @details This class is useful to automatically cancel and destroy a timer on shutdown.
/// usage: static Global<AutoTimer> g_myTimer;
/// g_myTimer = TimerInterface::AddPeriodicTimer(Seconds(1), []() { ... }, JobQueueInterface::MainThreadQueue());
//----------------------------------------------------------------------------------------
class AutoTimer
{
public:
	AutoTimer() = default;
	AutoTimer(AutoTimer&& src) : _timerRef(std::move(src._timerRef)) { }
	MAXON_IMPLICIT AutoTimer(const TimerRef& timerRef) : _timerRef(timerRef) {}
	MAXON_IMPLICIT AutoTimer(TimerRef&& timerRef) : _timerRef(std::move(timerRef)) { }

	~AutoTimer()
	{
		if (_timerRef)
		{
			_timerRef.CancelAndWait();
			_timerRef = nullptr;
		}
	}
