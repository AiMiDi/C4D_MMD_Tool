...
public:
	...
	~StaticJob()
	{
		...
		// For a non-complex loop always jump through the destructor - it'll be removed by the compiler if the destructor turns out to be trivial. 
		if constexpr (FLAGS == PARALLELFORFLAGS::INITTHREADED_FINALIZETHREADED || FLAGS == PARALLELFORFLAGS::INITSYNC_FINALIZETHREADED || FLAGS == PARALLELFORFLAGS::NOINIT_NOFINALIZE)
	...
	typename CONTEXT::ResultValueType ParallelFor::Static(FROMTYPE tfrom, INDEXTYPE to, const LOOP& obj, const Granularity& granularity, JobQueueInterface* queue, Int threadCnt)
{
	...
	if (MAXON_LIKELY(from < to))
	{
		...
		StaticJobGroupRef<Worker> group = StaticJobGroupRef<Worker>::Create(queue, threadCnt, STATICJOBGROUPFLAGS::ENQUEUEING_THREAD_WAITS | (std::is_trivially_destructible<CONTEXT>::value ? STATICJOBGROUPFLAGS::NO_JOB_DESTRUCTOR : STATICJOBGROUPFLAGS::DEFAULT)).UncheckedGetValue();
	...
	typename CONTEXT::ResultValueType ParallelFor::Static(FROMTYPE tfrom, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, const Granularity& granularity, JobQueueInterface* queue, Int threadCnt)
{
	...
	if (MAXON_LIKELY(from < to))
	{
		...
		StaticJobGroupRef<Worker> group = StaticJobGroupRef<Worker>::Create(queue, threadCnt, STATICJOBGROUPFLAGS::ENQUEUEING_THREAD_WAITS | (USE_SINGLE_THREADED_FINALIZER ? STATICJOBGROUPFLAGS::WILL_RUN_FINALIZER : STATICJOBGROUPFLAGS::DEFAULT) | (std::is_trivially_destructible<CONTEXT>::value ? STATICJOBGROUPFLAGS::NO_JOB_DESTRUCTOR : STATICJOBGROUPFLAGS::DEFAULT)).UncheckedGetValue();
		INDEXTYPE chunkSize = CalculateStaticChunkSize<INDEXTYPE>(UInt(to - from), threadCnt, granularity.value);
		...
		SingleThreadedFinalize<Worker, USE_SINGLE_THREADED_FINALIZER, USE_SINGLE_THREADED_FINALIZER> singleThreaded(jobs);
