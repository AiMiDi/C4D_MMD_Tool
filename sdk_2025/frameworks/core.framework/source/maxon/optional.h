...
public:
	...
	// Copy constructor.
	MAXON_DECLARE_CONDITIONAL_COPY_CONSTRUCTOR(OptDefaultStorage, STD_IS_REPLACEMENT(copy_constructible, T))
	{
		CopyConstruct(src);
	}
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(constructible, T, const U&)>::type
	>
	...
	// Copy assignment.
	MAXON_DECLARE_CONDITIONAL_COPY_ASSIGNMENT(OptDefaultStorage, STD_IS_REPLACEMENT(copy_assignable, T))
	{
		CopyAssign(src);
		return *this;
	}
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(assignable, T, const U&)>::type
	>
	...
	// Move constructor.
	MAXON_DECLARE_CONDITIONAL_MOVE_CONSTRUCTOR(OptDefaultStorage, STD_IS_REPLACEMENT(move_constructible, T))
	{
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(constructible, T, U&&)>::type
	>
	...
	// Move assignment.
	MAXON_DECLARE_CONDITIONAL_MOVE_ASSIGNMENT(OptDefaultStorage, STD_IS_REPLACEMENT(move_assignable, T))
	{
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(assignable, T, U&&)>::type
	>
	...
	{
		new (std::addressof(_valueStorage)) T(std::forward<U>(value));
	}
	...
	{
		new (std::addressof(_valueStorage)) T(std::forward<ARGS>(args) ...);
	}
...
private:
	...
	void CopyConstruct(const OptDefaultStorage<U>& src)
	{
		...
		{
			_hasValue = true;
			new (std::addressof(_valueStorage)) T(src.GetValueRef());
		}
		else
		{
			_hasValue = false;
	...
	void MoveConstruct(OptDefaultStorage<U>&& src)
	{
		...
		{
			_hasValue = true;
			new (std::addressof(_valueStorage)) T(std::move(src.GetValueRef()));
...
public:
	...
	Result<typename SUPER::ValueType::ValueType> GetOrDefault() const
	{
		if (this->_error.GetReferencedError())
			return this->_error;
		return this->UncheckedGetValue().GetOrDefault();
	}

	[[deprecated("use GetOrDefault (since 2024.0)")]] MAXON_ATTRIBUTE_FORCE_INLINE Result<typename SUPER::ValueType::ValueType> GetValueOrNull() const
	{
		return GetOrDefault();
	}
	...
	Result<typename SUPER::ValueType::ValueType> MoveValueOrDefault()
	{
		if (this->_error.GetReferencedError())
			return this->_error;
		return this->UncheckedGetValue().MoveValueOrDefault();
	}
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_DECLARE_CONDITIONAL_COPY_CONSTRUCTOR(Opt, STD_IS_REPLACEMENT(copy_constructible, maxon::details::OptStorageType<T>)) : _storage(src._storage) { }
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(constructible, maxon::details::OptStorageType<T>, const maxon::details::OptStorageType<U>&)>::type
	>
	...
	//----------------------------------------------------------------------------------------
	MAXON_DECLARE_CONDITIONAL_COPY_ASSIGNMENT(Opt, STD_IS_REPLACEMENT(copy_assignable, maxon::details::OptStorageType<T>))
	{
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(assignable, maxon::details::OptStorageType<T>, const maxon::details::OptStorageType<U>&)>::type
	>
	...
	//----------------------------------------------------------------------------------------
	MAXON_DECLARE_CONDITIONAL_MOVE_CONSTRUCTOR(Opt, STD_IS_REPLACEMENT(move_constructible, maxon::details::OptStorageType<T>)) : _storage(std::move(src._storage)) { }
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(constructible, maxon::details::OptStorageType<T>, maxon::details::OptStorageType<U>&&)>::type
	>
	...
	//----------------------------------------------------------------------------------------
	MAXON_DECLARE_CONDITIONAL_MOVE_ASSIGNMENT(Opt, STD_IS_REPLACEMENT(move_assignable, maxon::details::OptStorageType<T>))
	{
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(assignable, maxon::details::OptStorageType<T>, maxon::details::OptStorageType<U>&&)>::type
	>
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(constructible, maxon::details::OptStorageType<T>, const U&)>::type
	>
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(assignable, maxon::details::OptStorageType<T>, const U&)>::type
	>
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(constructible, maxon::details::OptStorageType<T>, U&)>::type
	>
	...
	<
		typename U,
		typename = typename std::enable_if<STD_IS_REPLACEMENT(assignable, maxon::details::OptStorageType<T>, U&)>::type
	>
	Opt& operator =(Opt<U&> src)
	{
		if (src)
	...
	/// Returns the contained value, or a default value if Opt contained no value.
	//----------------------------------------------------------------------------------------
	const T& GetOrDefault() const
	{
		if (!HasValue())
			return DefaultValue<const T&>();
		else
			return GetValueRef();
	}

	[[deprecated("use GetOrDefault (since 2024.0)")]] MAXON_ATTRIBUTE_FORCE_INLINE const T& GetValueOrNull() const
	{
		return GetOrDefault();
	}
	...
	//----------------------------------------------------------------------------------------
	/// Returns the moved contained value, or a default value if Opt contained no value.
	//----------------------------------------------------------------------------------------
	T MoveValueOrDefault()
	{
		if (!HasValue())
	...
	{
		CheckValue();
		return std::addressof(GetValueRef());
	}
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Opt(T& value) : _dataPtr(std::addressof(value)) { }
	...
	//----------------------------------------------------------------------------------------
	template <typename TT, typename = typename std::enable_if<std::is_same<const TT, const T>::value || std::is_base_of<TT, T>::value>::type> operator TT*() const
	{
		// Note: We use a function template because otherwise the conversion function would be used to initialize a Bool from an Opt in scenarios like this:
		// Opt<Bool&> opt;
		// Bool b = opt; // probably not intended!
		// The trick with the function template prevents such a conversion but still allows to write
		// Bool* ptr = opt;
		// The alternative solution with "explicit" keyword would prevent that as well.
		return _dataPtr;
	}
	...
	{
		if (_dataPtr)
			return GlobalToString(*_dataPtr, format);
		else
			return CONSTSTRING("(no value)");
	}
	...
	HashInt GetHashCode() const
	{
		return _dataPtr ? DefaultCompare::GetHashCode(*_dataPtr) : 0;
	}
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Opt(const T& value) : _dataPtr(std::addressof(value)) { }
	...
	/// This is supported for interoperability with legacy code.
	//----------------------------------------------------------------------------------------
	template <typename TT, typename = typename std::enable_if<std::is_same<const TT, const T>::value || std::is_base_of<TT, T>::value>::type> operator const TT*() const
	{
		return _dataPtr;
	}
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Opt(typename std::remove_reference<T>::type&& value) : _dataPtr(std::addressof(value)) { }
