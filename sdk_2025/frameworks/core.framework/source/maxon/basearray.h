...
//----------------------------------------------------------------------------------------
template <typename T, Bool ZEROINITIALIZE, Bool MOVE_AND_COPY_MEMORY> class BaseArrayUtilities
{
public:
	//----------------------------------------------------------------------------------------
	/// @brief Initializes #initCnt elements with the default value, cannot fail.
	...
	static MAXON_ATTRIBUTE_NO_INLINE void Initialize(T* dst, Int initCnt)
	{
		for (; initCnt > 0; initCnt--)
			PlacementNew<T, ZEROINITIALIZE>(dst++);
	}

	//----------------------------------------------------------------------------------------
	/// @brief Destructs #destructCnt elements, cannot fail.
	/// @param[in] dst								Pointer to the destination buffer.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Copies multiple elements (used by BaseArray::Insert()).
	/// @note It is assumed that the elements dst points to are not constructed.
	/// @param[in] src								Pointer to the source buffer.
	/// @param[in] dst								Pointer to the destination buffer.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Tries to reallocate an array buffer of the requested size for PODs or movable data types.
...
/// @tparam ZEROINITIALIZE				Should be false by default, only true for types which require zero initialization before calling the constructor.
//----------------------------------------------------------------------------------------
template <typename T, Bool ZEROINITIALIZE> class BaseArrayUtilities<T, ZEROINITIALIZE, true>
{
public:
	//----------------------------------------------------------------------------------------
	/// @brief Initializes #initCnt elements with the default value, cannot fail.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Moves objects within the array or from one array to another, cannot fail.
	/// It is assumed that the elements dst points to are not constructed, if dst and src
	/// overlap the elements are moved in the right order.
	/// @param[in] src								Pointer to the source buffer.
	/// @param[in] dst								Pointer to the destination buffer.
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE void MoveFromKeepSrc(T* src, T* dst, Int moveCnt)
	{
		memmove((void*)dst, (void*)src, size_t(moveCnt * SIZEOF(T)));
	}
...
template <typename T, Int MINCHUNKSIZE = BASEARRAY_DEFAULT_CHUNK_SIZE, BASEARRAYFLAGS MEMFLAGS = BASEARRAYFLAGS::NONE, typename ALLOCATOR = DefaultAllocator> class BaseArray
	: public ArrayBase<BaseArray<T, MINCHUNKSIZE, MEMFLAGS, ALLOCATOR>, T, BaseArrayData<T, MEMFLAGS, ALLOCATOR, STD_IS_REPLACEMENT(empty, ALLOCATOR)>, DefaultCompare>
{
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief This constructor has to be used if an array should use a custom allocator with member variables.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit BaseArray(const ALLOCATOR& a) : Super(a) { }

	//----------------------------------------------------------------------------------------
	/// @brief This constructor has to be used if an array should move a custom allocator with member variables.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit BaseArray(ALLOCATOR&& a) : Super(std::move(a)) { }

	//----------------------------------------------------------------------------------------
	/// @brief Destructs the array with all its elements.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a Block<const T> which represents the elements of the array.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a Block<T> which represents the elements of the array.
	/// @return												The content of this array as a block.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Converts this BaseArray to a Block<const T> which represents the elements of the array.
	/// @return												The content of this array as a block.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Converts this BaseArray to a Block<T> which represents the elements of the array.
	/// @return												The content of this array as a block.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Converts this BaseArray to a StridedBlock<const T> which represents the elements of the array.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Converts this BaseArray to a StridedBlock<T> which represents the elements of the array.
	/// The block will have the natural stride of T.
	/// @return												The content of this array as a strided block.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Operator for passing a BaseArray to a Block<const Byte> which represents the raw bytes of the array.
	/// This operator is only supported when T is a scalar type.
	/// @return												The content of this array as a raw Byte block.
	//----------------------------------------------------------------------------------------
	template <typename DUMMY = T, typename = typename std::enable_if<STD_IS_REPLACEMENT(scalar, DUMMY)>::type> operator Block<const Byte>() const
	{
		return ToBlock();
	}

	//----------------------------------------------------------------------------------------
	/// @brief Operator for passing a BaseArray to a Block<Byte> which represents the raw bytes of the array.
	/// This operator is only supported when T is a scalar type.
	/// @return												The content of this array as a raw Byte block.
	//----------------------------------------------------------------------------------------
	template <typename DUMMY = T, typename = typename std::enable_if<STD_IS_REPLACEMENT(scalar, DUMMY)>::type> operator Block<Byte>()
	{
		return ToBlock();
	}

	//----------------------------------------------------------------------------------------
	/// @brief Deletes all elements (calls destructors and frees memory).
	//----------------------------------------------------------------------------------------
	void Reset()
	{
		if (_capacity > 0)
	...
	//----------------------------------------------------------------------------------------
	/// @brief Deletes all elements, but doesn't free memory (calls destructors though).
	//----------------------------------------------------------------------------------------
	void Flush()
	{
		if (_cnt > 0)
	...
	//----------------------------------------------------------------------------------------
	/// @brief Gets the number of elements for which memory has been allocated (this is usually bigger than GetCount()).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Array (subscript) operator for const objects.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Array (subscript) operator for non-const objects.
	/// @param[in] idx								Element index (if it's out of bounds you will get an error in debug code only, otherwise it will crash).
	/// @return												Array element.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Appends a new element at the end of the array and constructs it using the forwarded value.
	...
	template <typename ARG> MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<T> Append(ARG&& x)
	{
		...
		T* element = _ptr + _cnt++;
		if (MAXON_UNLIKELY(NewCopy(*element, std::forward<ARG>(x)) == FAILED) && TestForCopyFromMember<T>::isSupported)
		{
	...
	//----------------------------------------------------------------------------------------
	/// @brief Appends a new element at the end of the array and constructs it using the forwarded values.
	...
	template <typename... ARGS> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<sizeof...(ARGS) != 1, ResultRef<T>>::type Append(ARGS&&... args)
	{
		...
		return PlacementNew<T, Bool(MEMFLAGS & BASEARRAYFLAGS::ZEROINITIALIZE)>(_ptr + _cnt++, std::forward<ARGS>(args)...);
	}

	//----------------------------------------------------------------------------------------
	/// @brief Appends new elements at the end of the array.
	...
	MAXON_ATTRIBUTE_FORCE_INLINE ResultPtr<T> AppendBlock(const MoveBlock<T>& values)
	{
		...
		if (MAXON_LIKELY(element != nullptr))
		{
			...
			{
				using Utilities = BaseArrayUtilities<T, Bool(MEMFLAGS & BASEARRAYFLAGS::ZEROINITIALIZE), STD_IS_REPLACEMENT(trivially_copyable, T) || MEMFLAGS == BASEARRAYFLAGS::MOVEANDCOPYOBJECTS>;
				Utilities::MoveFromKeepSrc(values.GetFirst(), element, values.GetCount());
			}
		}
	...
	template <Int CS, BASEARRAYFLAGS MF, typename A> MAXON_IMPLICIT MAXON_ATTRIBUTE_FORCE_INLINE ResultPtr<T> Append(BaseArray<T, CS, MF, A>&& a)
	{
		return AppendBlock(MoveBlock<T>(a.GetFirst(), a.GetCount()));
	} 
	...
	MAXON_ATTRIBUTE_FORCE_INLINE ResultPtr<T> Append(const std::initializer_list<T>& values)
	{
		...
		Int cnt = last - first;
		return AppendBlock(Block<const T>(first, cnt));
	}

	//----------------------------------------------------------------------------------------
	/// @brief Inserts a new element at index position and constructs it using the forwarded value.
	...
	//----------------------------------------------------------------------------------------
	template <typename ARG> MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<T> Insert(Int position, ARG&& x)
	{
		T* element = InsertWithoutConstructor(position);
		if (element)
		{
			if ((NewCopy(*element, std::forward<ARG>(x)) == FAILED) && TestForCopyFromMember<T>::isSupported)
			{
				using Utilities = BaseArrayUtilities<T, Bool(MEMFLAGS & BASEARRAYFLAGS::ZEROINITIALIZE), STD_IS_REPLACEMENT(trivially_copyable, T) || MEMFLAGS == BASEARRAYFLAGS::MOVEANDCOPYOBJECTS>;
	...
	//----------------------------------------------------------------------------------------
	/// @brief Inserts a new element at index position and constructs it using the forwarded values.
	/// @param[in] position						Insert index (the array size will increase and the existing elements are moved).
	/// @param[in] args								Forwarded values to construct the object.
	/// @return												Element reference or OutOfMemoryError if the allocation failed (or position is out of boundaries).
	//----------------------------------------------------------------------------------------
	template <typename... ARGS> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<sizeof...(ARGS) != 1, ResultRef<T>>::type Insert(Int position, ARGS&&... args)
	{
		T* element = InsertWithoutConstructor(position);
		if (element)
			PlacementNew<T, Bool(MEMFLAGS & BASEARRAYFLAGS::ZEROINITIALIZE)>(element, std::forward<ARGS>(args)...);
		return element;
	}
	...
	//----------------------------------------------------------------------------------------
	/// @brief Inserts new elements at index position (all elements from @p position on are moved by the the count of @p values).
	/// @param[in] position						Insert index (the array size will increase and the existing elements are moved).
	/// @param[in] values							Block with values to be moved (a MoveBlock must be used to prevent accidental moves of temporary Blocks).
	/// 															If the block points to nullptr, only its count is used, and you have to call the constructor of the new elements manually.
	...
	template <Int CS, BASEARRAYFLAGS MF, typename A> MAXON_IMPLICIT MAXON_ATTRIBUTE_FORCE_INLINE ResultPtr<T> Insert(Int position, BaseArray<T, CS, MF, A>& a)
	{
		return InsertBlock(position, a.ToBlock());
	} 

	template <Int CS, BASEARRAYFLAGS MF, typename A> MAXON_IMPLICIT MAXON_ATTRIBUTE_FORCE_INLINE ResultPtr<T> Insert(Int position, const BaseArray<T, CS, MF, A>& a)
	{
		return InsertBlock(position, a.ToBlock());
	} 
	...
	template <Int CS, BASEARRAYFLAGS MF, typename A> MAXON_IMPLICIT MAXON_ATTRIBUTE_FORCE_INLINE ResultPtr<T> Insert(Int position, BaseArray<T, CS, MF, A>&& a)
	{
		return InsertBlock(position, MoveBlock<T>(a.GetFirst(), a.GetCount()));
	} 
	...
	ResultPtr<T> Insert(Int position, const std::initializer_list<T>& values)
	{
		...
		const T* last = values.end();
		Int cnt = last - first;
		return InsertBlock(position, Block<const T>(first, cnt));
	}
	...
	template <typename... ARGS> MAXON_ATTRIBUTE_FORCE_INLINE ResultMemT<Iterator> InsertBlock(Iterator position, ARGS&&... args)
	{
		return Iterator(InsertBlock(position - Begin(), std::forward<ARGS>(args)...).GetPointer());
	}
	...
	{
		return Iterator(Insert(position - Begin(), std::forward<ARGS>(args)...).GetPointer());
	}

	//----------------------------------------------------------------------------------------
	/// @brief Erases (removes and deletes) elements.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Erases elements within the array and moves elements from the end to the erased gap.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Determines a contiguous block of array elements which contains the element at @p position.
	/// @note For a BaseArray, this yields the whole array as a block.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Resizes the array to contain newCnt elements.
			...
			if (newCnt > _capacity)
			{
				...
				if (element)
				{
					...
					if (memoryHasBeenMoved == false && _capacity > 0)
					{
						...
						if ((STD_IS_REPLACEMENT(base_of, FixedBufferAllocatorBase, ALLOCATOR) == false) || (element != _ptr))
						{
	...
	//----------------------------------------------------------------------------------------
	/// @brief Gets the index of the element. The element must be part of the array, otherwise (e.g.
	...
	//----------------------------------------------------------------------------------------
	/// @brief If necessary the array capacity is increased to hold at least the given number of elements without further memory allocations.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Prepares the array to hold at least the given number of elements with as few further memory allocations as possible.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Specialization of AppendAllImpl, used by BaseCollection::AppendAll.
	/// @private
	//----------------------------------------------------------------------------------------
	template <typename COLLECTION2> Result<void> AppendAllImpl(COLLECTION2&& other, COLLECTION_RESIZE_FLAGS resizeFlags, Bool overwrite, OverloadRank0)
	{
		if (maxon::IsCollection<COLLECTION2>::value)
		{
			...
			if (IncreaseCapacity(cnt, resizeFlags) == nullptr)
			...
			{
				auto res = NewCopy<T>(*element, ValueForward<COLLECTION2>(i));
				if (res == FAILED)
				{
					_cnt = element - _ptr;
					return res;
	...
	{
		return AppendBlock(typename std::conditional<STD_IS_REPLACEMENT(reference, BASEARRAY) || STD_IS_REPLACEMENT(const, BASEARRAY), Block<const T>, MoveBlock<T>>::type(other.GetFirst(), other.GetCount()));
	}
	...
	{
		return AppendBlock(std::forward<BLOCK>(other));
	}

	using Super::CopyFromImpl;

	//----------------------------------------------------------------------------------------
	/// @brief Specialization of CopyFromImpl for a BaseArray source, used by BaseCollection::CopyFrom.
	/// @private
	//----------------------------------------------------------------------------------------
	template<typename BASEARRAY, typename = typename std::enable_if<STD_IS_REPLACEMENT(same, typename std::decay<BASEARRAY>::type, BaseArray)>::type>
	...
	//----------------------------------------------------------------------------------------
	/// @brief Specialization of CopyFromImpl for a Block source, used by BaseCollection::CopyFrom.
	/// @private
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Swaps elements a and b (equivalent to global Swap(array[a], array[b]).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Calculates the memory usage for this array.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns an iterator for the first element.
	/// @note When you modify the array Begin() might change, it is not a constant value.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns an iterator for the end (End() - 1 is the last element if the array is not empty).
	/// @note When you modify the array End() might change, it is not a constant value.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Moves elements to another (empty) array.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Disconnects the array's memory buffer and returns its content as a block.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Sets the array's memory buffer to the given block. The current content of this array is freed before.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the allocator as reference.
	...
	{
		return ToBlock().GetUniqueHashCode();
	}
...
protected:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Constructs a BaseArray such that it uses the given block as memory buffer.
	/// @param[in] block							Memory block to use for the array. The array takes ownership of the memory.
	/// @param[in] capacity						Capacity of the buffer. If a non-positive value is given, the capacity is assumed to be the same as the size.
	/// @note The memory pointed to by @p block must have been allocated by the allocator that the array is using.
	/// @private
...
private:
	//----------------------------------------------------------------------------------------
	/// @brief BaseArray specific: Appends uninitialized element(s) at the end of the array.
	...
	MAXON_WARN_UNUSED T* IncreaseCapacity(Int increment = 1, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::ON_GROW_RESERVE_CAPACITY)
	{
		...
		if (MAXON_UNLIKELY(requestedCapacity > _capacity))
		{
			...
			if constexpr (STD_IS_REPLACEMENT(base_of, FixedBufferAllocatorBase, ALLOCATOR))
			{
	...
	//----------------------------------------------------------------------------------------
	/// @brief BaseArray specific: Inserts uninitialized element(s) at the specified index.
	/// @note <B> This does not call the constructor! Use Insert() unless you deal with PODs that shall not be
	...
	MAXON_WARN_UNUSED T* InsertWithoutConstructor(Int idx, Int increment = 1)
	{
		...
		if (_cnt + increment > _capacity)
		{
			...
			{
				// If the FixedBufferAllocator is used it might return the already used pointer because the buffer is big enough and the elements don't have to be moved.
				if ((STD_IS_REPLACEMENT(base_of, FixedBufferAllocatorBase, ALLOCATOR) == false) || (newArray != _ptr))
				{
