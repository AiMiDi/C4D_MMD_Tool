...
MAXON_DATATYPE(IDENT_VOID, "net.maxon.ident_void", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
{
	auto tmp = static_cast<T*>(arrayPtr)->ToArray();
...
{
	auto tmp = static_cast<T*>(mapPtr)->ToSet();
...
{
	auto tmp = static_cast<T*>(mapPtr)->ToMap();
...
public:
	...
	static constexpr ValueType NONE									= 0;					///< Default.
	static constexpr ValueType CONDITION_IF					= (1 << 0);		///< If set element additionally describes an if-condition. Can only be done for elementary datatypes.
	static constexpr ValueType CONDITION_ELSE				= (1 << 1);		///< Else/Else If condition, cannot be combined with other flags.
	static constexpr ValueType CONDITION_END				= (1 << 2);		///< End condition, cannot be combined with other flags.
	static constexpr ValueType IS_LEGACY_ELEMENT		= (1 << 3);		///< Mark an element as "legacy". The element will no longer be read/written but also not show up in any warnings.
	static constexpr ValueType DONT_WRITE						= (1 << 4);		///< If set the element will only be read (for conversion purposes), not written.
	static constexpr ValueType ACTION_BEFORE_WRITE	= (1 << 5);		///< If set there will be a call to the helper class WriteAction member function before an element is written.
	static constexpr ValueType ACTION_AFTER_READ		= (1 << 6);		///< If set there will be a call to the helper class ReadAction member function after an element is read.
	static constexpr ValueType REFERS_TO_HELPER			= (1 << 7);		///< If set the description refers to the helper class instead of the base class.
	static constexpr ValueType DELAYED							= (1 << 8);		///< Can only be used for references. Reads/Writes the reference at a later point in time (like WeakRefs), avoiding recursion problems. E.g. this can be used if a node holds its children as StrongRefs.
	static constexpr ValueType TYPE_SET							= (1 << 9);		///< The described element is a Set type.
	static constexpr ValueType TYPE_MAP							= (1 << 10);	///< The described element is a Map type.
	static constexpr ValueType TYPE_ARRAY						= (1 << 11);	///< The described element is an Array type.
	static constexpr ValueType TYPE_UNIQUEREF				= (1 << 12);	///< The described element is an UniqueRef type.
	static constexpr ValueType TYPE_WEAKREF					= (1 << 13);	///< The described element is a WeakRef type.
	static constexpr ValueType TYPE_DATATYPE				= (1 << 14);	///< The described element is a DataTypeImpl*. Only necessary if RECEIVE_DATATYPE is also used.
	static constexpr ValueType RECEIVE_DATATYPE			= (1 << 15);  ///< The described element will receive its datatype from an earlier element of TYPE_DATATYPE.
	static constexpr ValueType APPLY_TO_KEY					= (1 << 16);	///< Can only be used together with TYPE_MAP and RECALL_DATATYPE. If set the map's key will receive the datatype instead of the value.
	static constexpr ValueType WRITE_IF_NOT_EMPTY		= (1 << 17);	///< Only applies to text formats. Can only be used together with TYPE_ARRAY, TYPE_MAP or TYPE_SET. If present the array/set/map swill only be written if it contains at least one element. Note that upon read no cs atiallt to the ReadAction function will happen when the collection hasn't been written. This flag Will sautomatically be set if DescribeDefaults() is called.
	static constexpr ValueType CONDITION_NOWRITE		= (1 << 18);  ///< If CONDITION_IF is set this value will not be written but can function as a pure condition.
...
{
	MAXON_INTERFACE(DataSerializeInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.dataserialize", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define Describe(name, memberName, type, flags) descriptionStream.DescribeElement<DESCRIBECLASS, type, flags>(name, &(static_cast<DESCRIBECLASS*>(nullptr)->memberName), maxon::DESCRIBEFLAGS::NONE)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeLegacy(name) descriptionStream.DescribeElement(name, maxon::DataType::DefaultValue(), 0, nullptr, (maxon::DESCRIBEFLAGS)maxon::DESCRIBEFLAGS::IS_LEGACY_ELEMENT, 0, 0)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeHelper(name, memberName, type, flags) descriptionStream.DescribeElement<IOHELPERCLASS, type, flags>(name, &(static_cast<IOHELPERCLASS*>(nullptr)->memberName), maxon::DESCRIBEFLAGS::REFERS_TO_HELPER)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeIf(name, memberName, type, flags, mask, value) descriptionStream.DescribeElement<DESCRIBECLASS, type, flags>(name, &(static_cast<DESCRIBECLASS*>(nullptr)->memberName), maxon::DESCRIBEFLAGS::CONDITION_IF, mask, value)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeHelperIf(name, memberName, type, flags, mask, value) descriptionStream.DescribeElement<IOHELPERCLASS, type, flags>(name, &(static_cast<IOHELPERCLASS*>(nullptr)->memberName), maxon::DESCRIBEFLAGS::CONDITION_IF | maxon::DESCRIBEFLAGS::REFERS_TO_HELPER, mask, value)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeElse() descriptionStream.DescribeElement(nullptr, maxon::DataType::DefaultValue(), 0, nullptr, (maxon::DESCRIBEFLAGS)maxon::DESCRIBEFLAGS::CONDITION_ELSE, 0, 0)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeElseIf(mask, value) descriptionStream.DescribeElement(nullptr, maxon::DataType::DefaultValue(), 0, nullptr, (maxon::DESCRIBEFLAGS)maxon::DESCRIBEFLAGS::CONDITION_ELSE, mask, value)
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	#define DescribeEndIf() descriptionStream.DescribeElement(nullptr, maxon::DataType::DefaultValue(), 0, nullptr, (maxon::DESCRIBEFLAGS)maxon::DESCRIBEFLAGS::CONDITION_END, 0, 0)
	...
	{
		static_assert(STD_IS_REPLACEMENT(same, T, CMP), "Describe: class does not match member type");
	...
	{
		static_assert(STD_IS_REPLACEMENT(same, typename T::ValueType, CMP), "Describe: class does not match member value type");
	...
	{
		static_assert(STD_IS_REPLACEMENT(same, typename T::KeyType, typename CMP::FirstType), "Describe: Pair::first does not match member key type of Map");
		static_assert(STD_IS_REPLACEMENT(same, typename T::ValueType, typename CMP::SecondType), "Describe:: Pair::second does not match member value type of Map");
...
{
	MAXON_INTERFACE(DataSerializeReaderInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.dataserializereader", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(Class<DataSerializeRef>, DataSerializeBaseClass, "net.maxon.class.dataserializebase", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Default implementation of DataSerializeReaderInterface.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(Class<DataSerializeReaderRef>, DataSerializeReaderClass, "net.maxon.class.dataserializereader", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Default implementation of DataSerializeWriterInterface.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(Class<DataSerializeWriterRef>, DataSerializeWriterClass, "net.maxon.class.dataserializewriter", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
