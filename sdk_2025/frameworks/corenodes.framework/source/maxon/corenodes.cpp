...
const InPortId InPortId::_defaultValue;
const OutPortId OutPortId::_defaultValue;

String PortId::ToString(const FormatStatement* fs) const
{
	return IsEmpty() ? "null"_s : IsIndex() ? String::IntToString(GetIndex()) : _id.ToString(fs);
}
...
void MicroNodeGroupInterface::PrivateAutoCreatePortInfo::CreatePort(const Id& name, MicroNode::TYPE type, MicroNode::FLAGS flags, const DataType& vtype)
{
	if (result == FAILED)
		return;
	...
	result = group->PrivateCreatePort(name, type, flags, typeOverride ? typeOverride : vtype, false);
}
