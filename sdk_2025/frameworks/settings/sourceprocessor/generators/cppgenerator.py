...
class Generator(object):
    ...
    def writeInterfaceUse(self, cls, iname):
        ...
        if cls.interface != Class.SIMPLE_VIRTUAL:
            self.writeUse('s_ui_' + underscoreClass + suffix, iname + '::_interface.GetReference(), &maxon::g_translationUnit, "' + self.header + '"', cls.implementationModule)
        if cls.interface == Class.VIRTUAL:
            if cls.singleImpl:
                self.writeUse('s_ui1_' + underscoreClass + suffix, iname + '::_nonvirtualMTable.GetReference(), &maxon::g_translationUnit, "' + self.header + '"', cls.implementationModule)
            if cls.hasStaticMethods:
                self.writeUse('s_usi_' + underscoreClass + suffix, iname + '::_staticInterface.GetReference(), &maxon::g_translationUnit, "' + self.header + '"', cls.implementationModule)
