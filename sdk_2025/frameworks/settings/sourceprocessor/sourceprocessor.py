...
stylecheck.methodmarker
  checks the correct usage of MAXON_METHOD in component implementations
...
scriptDir, fn = os.path.split(__file__)
...
# Stores macros which must be replaced in code before the source processor runs.
SPECIAL_MACROS = [
    ...
    '}'),
    ('MAXON_IF_SOURCEPROCESSOR', '%1'), ('MAXON_IF_TARGET_DEBUG', '%1'), ('PRIVATE_MAXON_ENABLE_IF_ERROR', '%1'), 
    ('MAXON_DEPRECATED_ENUMVALUE', '%1') ]
# Stores operator symbols.
OPERATORS = { '=', '+=', '-=', '*=', '/=', '%=', '^=', '&=', '|=', '>>=', '<<=', '==', '!=', '<=', '>=', '<=>', '||' }
# Stores custom prefix qualifiers for variables and functions.
FUNCVAR_QUALIFIER_PREFIXES = {
    'constexpr', 'consteval', 'inline', 'static', 'virtual', 'extern', 'thread_local', 'explicit', 'friend', 'MAXON_IMPLICIT', 
    'MAXON_WARN_UNUSED', '__declspec', '@property', 'IBOutlet', 'MAXON_ATTRIBUTE_FORCE_INLINE', 'MAXON_ATTRIBUTE_FORCE_RELEASE_INLINE',
    'MAXON_ATTRIBUTE_NO_INLINE', 'MAXON_ATTRIBUTE_DLL_PUBLIC', 'MAXON_WARN_MUTE_FUNCTION_LENGTH', 
    'MAXON_WARN_MUTE_NAMING', 'MAXON_METHOD', 'MAXON_FUNCTION', 'PIXELFORMAT_INLINE', 'GLAPI_EXTERN', 'GLAPI', 
    'JNIEXPORT', 'FMX_ENTRYPT', 'CONST_2024' }
# Stores custom postfix qualifiers for variables and functions.
FUNCVAR_QUALIFIER_POSTFIXES = { 'WINAPI', 'CALLBACK', 'APIENTRY', 'DNSSD_API', 'noexcept', 'CONST_2024' }
# Stores custom prefix qualifiers for arguments which are passed in parentheses.
PARAMETER_QUALIFIER_PREFIXES = { '__in', '__in_opt', 'AVAHI_GCC_UNUSED' }
...
def isCamelCaseUpper(str):
    ...
    if not (str and str[0].isupper()):
        return False
    return len(str) == 1 or (str.isalnum() and not str.isupper())
...
class Parser(object):
    ...
    def checkCamelCaseUpper(self, scope, name, type):
        if self.args.stylecheck and self.args.stylecheckOptions.get('naming', True) and not isCamelCaseUpper(name):
            ...
            if n and n.owner and n.name != 'std' and n.owner.name != 'std' and n.name != 'cinema':
    ...
    # returns tuple (type as string, bool, param name, forward) where the bool tells if the type is a const-qualified type
    # and forward is 0 when the type contains no types of self.forwards (or all of them are followed by * or &),
    # otherwise forward is 1 when all of the forward types have a header file, otherwise 2. 
    def parseTypeCheckConst(self, firstToken=None, parentheses=True, allowTilde=False, param=False):
        ...
        while True:
            ...
            if t == '*' or t == '&':
                fwd = None
            elif fwd != None and forward != 2:
                forward = 2 if fwd else 1
            if t == '<':
                nest += 1
            elif nest > 0 and t == '>':
                nest -= 1
            ...
            if maybeSuffix and not allowIdent:
                ...
                keyword = t == 'const' or t == 'volatile' or t == 'template' or t == 'typename' or t == 'struct' or t == 'static' or t == 'union'
                ...
                if t == '*':
                    const = False
                fwd = self.forwards.get(t)
            prev = t
            self.lex.accept()

    # parses a type, also handles template types with < > and commas and macros like BASE_INTERFACES(A, B)
    def parseType(self, firstToken=None, parentheses=True):
        type, c, name, dummy = self.parseTypeCheckConst(firstToken, parentheses)
        return type
        ...
        try:
            ...
            while True:
                ...
                if self.args.stylecheck and self.args.stylecheckOptions.get('no-whitespace', True) and noSpace:
                    ...
                    noSpace = False
                if (len(t) == 1 or t.type == Token.POSTFIX_DOC or t == 'MAXON_FORWARD') and stop.find('F' if t == 'MAXON_FORWARD' else t if t.type != Token.POSTFIX_DOC else '/') >= 0:
                ...
                prevToken = t
                if t.type != Token.WHITESPACE and t.type != Token.DIRECTIVE and t != '\n':
                    prevNonWsToken = t
        finally:
            self.lex.reportWhitespace -= 1
            self.lex.ignoreDefine -= 1
    ...
    def parseForward(self, name, owner):
        self.lex.consume('(')
        t = self.lex.getNoEof(owner)
        if t == ')':
            self.forwards[name] = True
        else:
            self.forwards[name] = False
            if t.type == Token.STRING:
                self.root.forwardHeaders.add(t)
            else:
                t.pos.raiseError('Expected header file name in quotes, found ' + t + ' instead.')
            self.lex.consume(')')
    ...
    def parseParams(self, noneIfInvalid=False):
        ...
        try:
            ...
            if self.lex.getNoEof() == ')':
                ...
                while True:
                    t = self.lex.getNoEof()
                    while t in PARAMETER_QUALIFIER_PREFIXES:
                        t = self.lex.getNoEof()
                    type = ''
                    ellipsis = t if t == '...' else None
                    parenthesizedName = None
                    if not ellipsis:
                        type, dummy, parenthesizedName, dummy = self.parseTypeCheckConst(t, True, False, True)
    ...
    def tryParseMethod(self, t, doclist, cls, access):
        ...
        while True:
            ...
            elif not t in self.funcvarQualifierPrefixes:
                self.lex.reject()
                return None
        ...
        if self.lex.next() == 'operator':
            ...
            retType, dummy, dummy, fwd = self.parseTypeCheckConst(None, False)
        else:
            retType, dummy, dummy, fwd = self.parseTypeCheckConst(None, True)
    ...
    def tryParseVarOrFunction(self, t, owner, access):
        ...
        checkNaming = True
        method = False
        while (t in self.funcvarQualifierPrefixes) or (t == '[' and self.lex.next() == '['):
            extern = False
            property = False
            if t == '[':
                self.parseAttributeRest()
                ...
                static = True
                if constexpr and self.args.stylecheck and self.args.stylecheckOptions.get('static-within-type', True):
                    self.styleerrors.append(t.pos, 'Wrong place of static, put it in front of constexpr.')
            elif t == 'constexpr':
                constexpr = True
                ...
                if self.args.stylecheck and self.args.stylecheckOptions.get('virtual', True) and self.args.publicframework:
        ...
        if t.type == Token.IDENT or t == '::': # the type might start with ::
            ...
            if t == '::':
                type = None
            if conversion:
                type, constType, dummy, dummy = self.parseTypeCheckConst(None, False, False)
            else:
                type, constType, dummy, dummy = self.parseTypeCheckConst(t, True, True)
            if type:
                name = conversion if conversion else self.lex.get(owner)
                while name in self.funcvarQualifierPostfixes:
                        ...
                        if params is not None:
                            ...
                            while True:
                                ...
                                if t == 'const':
                                    ...
                                    break
                            limit = self.args.stylecheckOptions.get('max-linecount', 600)
                            if checkLineCount and self.args.stylecheck and lineCount > limit:
                                self.errors.append(name.pos, 'The function ' + name + ' has ' + str(lineCount) + ' lines, this exceeds the limit of ' + str(limit) + ' lines.')
                            if namePrefix:
                                return None
                            if self.args.stylecheck and self.args.stylecheckOptions.get('methodmarker', True):
    ...
    def tryParseObservable(self, t, cls, access):
        ...
        param = Parameter('maxon::Bool', "create")
    ...
    def tryParseNamespaceOrClassDeclaration(self, t, owner, access):
        if t == 'MAXON_REGISTRY' or t == 'MAXON_DECLARATION':
            declToken = t
            ...
            if t == 'namespace':
                ...
                t = self.lex.getNoEof(owner)
                if t == 'MAXON_FORWARD':
                    self.parseForward(name, owner)
                    self.lex.consume(';')
                    return None
                elif t == '=':
                    alias, t = self.parseUntil(owner, None, False, ';F', False, -1)
            ...
            return decl
        elif (t.startswith('MAXON_') or t.find('MACRO') >= 0) and not (t in self.funcvarQualifierPrefixes):
            # skip a macro usage
            t = self.lex.next()
            if t == '(':
                t = self.lex.consume('(')
        ...
        if t == 'class' or t == 'struct' or t == 'union' or t == '@class' or t == '@protocol' or t == '@interface' or t == 'interface':
            ...
            if anonymous:
                ...
                if name == 'MAXON_WARN_UNUSED_CLASS':
                    name = self.lex.get(owner)                  
                if name == 'CINEMA4DC_SINGLEINHERITANCE' or name == 'CINEWARE_SINGLEINHERITANCE': # For the C4D Cinema API the PTMF on Windows must only take 8 bytes.
                    name = self.lex.get(owner)
            ...
            if name.type == Token.IDENT and (punct == '{' or punct == ':' or punct == ';'):
                ...
                if cls.parseAnnotation('iserrorinterface', False):
                    cls.isError = True 
                cls.ambiguousCalls = []
                cls.selfCalls = []
                ...
                if punct != ';':
                    ...
                    while True:
                        ...
                        if t == '}':
                            ...
                            if not cls.interface:
                                self.errors.append(t.pos, t + ' can only be used within an interface.')
                            if addToHxxClass:
                            ...
                            n = self.lex.getNoEof(cls)
                            n.expect([',', ')'])
                            if n == ',':
                                self.lex.consume('MAXON_IMPLEMENTATION_MODULE')
                                self.lex.consume('(')
                                cls.implementationModule = self.lex.getString()
                                self.lex.consume(')')
                                self.lex.consume(')')
                            if cls.qname() == 'maxon.ObjectInterface':
                                cls.implementationModule = '"net.maxon.kernel"'
                        ...
                        elif t == 'MAXON_METHOD_RESERVE':
                            self.lex.consume(';')
                            if cls.interface == Class.SIMPLE_VIRTUAL:
                                Function(t, t + str(len(cls.children)), False, 'void', cls, 'private')
                            else:
                                self.errors.append(t.pos, 'MAXON_METHOD_RESERVE can only be used for simple virtual interfaces.')
                        elif t == 'MAXON_USING' or t == 'MAXON_USING_CONST':
                            if not cls.interface:
                                self.errors.append(t.pos, t + ' can only be used for interfaces.')
                            self.lex.consume('(')
                            # We expect a qualified member name Interface::Function. That's not a type, but still we can use parseType because the same parsing rules apply.
                            function = self.parseType()
                            member = function.rfind('::')
                            if member < 0:
                                self.errors.append(t.pos, t + ' has to contain the qualified name of an inherited function or method.')
                            elif access != 'public':
                                self.errors.append(t.pos, t + ' has to be within a public section.')
                            else:
                                baseInterface = function[:member]
                                usingName = function[member+2:]
                                for b, g in cls.baseInterfaces:
                                    if b == baseInterface:
                                        (cls.usings if t == 'MAXON_USING' else cls.constUsings).append((baseInterface, usingName))
                                        break
                                else:
                                    self.errors.append(t.pos, baseInterface + ' not found in the list of base interfaces of ' + cls.name + '.')
                            self.lex.consume(')')
                            self.lex.consume(';')
                        else:
                    ...
                    if cls.parseAnnotation('nullimpl', False) and cls.interface != Class.NONVIRTUAL:
                        self.errors.append(name.pos, 'nullimpl annotation can only be used for non-virtual interfaces.')
                    if cls.interface:
                        cls.generic = generic
                        if cls.interface != Class.VIRTUAL:
    ...
    def tryParseNamespaceDeclaration(self, t, owner):
        if t.type == Token.INCLUDE:
            ...
            decl.id = id
            return decl
        elif t == 'MAXON_DATATYPE_LOCAL' or t == 'MAXON_INTERNED_ID_LOCAL' or t == 'MAXON_MEMBER_NAME_LOCAL':
            if self.isHeader:
                self.errors.append(t.pos, t + ' must not be used in a header file.')
            self.lex.consume('(')
            self.parseUntil(owner, None, False, ')', False, -1)
            self.lex.consume(';')
        elif t == 'MAXON_INTERNED_ID' or t == 'MAXON_MEMBER_NAME':
            self.lex.consume('(')
            if t == 'MAXON_MEMBER_NAME':
              shortName = self.lex.getIdent()
              # Build c++ variable name
              name = Token('PrivateMemberNameId_' + shortName, Token.IDENT, shortName.pos)
              t = Token('MAXON_INTERNED_ID', Token.IDENT, t.pos)
              # If optional string is supplied, use it as maxon::Id for member, else use member name
              if self.lex.next() == ',':
                self.lex.get()
                id = self.lex.getString()
              else:
                id = Token('"' + shortName + '"', Token.STRING, shortName.pos)
            else:
              param = self.lex.next()
              if param.type == Token.STRING or param.type == Token.IDENT:
                # Support define or macro usage in MAXON_INTERNED_ID, not just literals.
                id, lastToken = self.parseUntil(owner, None, False, ',', False, -1)
                # Comma already consume by parseUntil.
                name = self.lex.getIdent()
              else:
                param.pos.raiseError('Expected string literal or macro, found ' + param + ' instead.')
            self.lex.consume(')')
            decl = CppDeclaration(t, name, owner, 'public')
            decl.id = id
            return decl
        elif t == 'FP_TYPEARG_INTERNED_IDS':
            self.lex.consume('(')
            basename = self.lex.getIdent()
            while self.lex.next() == ',':
                self.lex.consume(',')
                type = self.lex.getIdent()
                decl = CppDeclaration('MAXON_INTERNED_ID', Token(basename + type, Token.IDENT, type.pos), owner, 'public')
                decl.id = '"' + basename + type + '"'
            self.lex.consume(')')
            return decl
    ...
    def tryParseNamespaceMember(self, t, namespace):
        ...
        if t.type == Token.HXX1_INCLUDE:
            ...
            if result:
                ...
                lw = self.lex.leadingWhitespaceOfLine(declStart)
                i = indentLen(lw)
                if lw is not None and (i < namespace.minIndentation or ((namespace.firstMember or t in self.funcvarQualifierPrefixes) and i > namespace.maxIndentation)):
                    self.styleerrors.append(declStart.pos, 'Wrong indentation of namespace member.')
        ...
        if t == 'namespace' or t == 'RESOURCE' or t == 'RESOURCEi':
            ...
            if name.type == Token.IDENT:
                ...
                if next in ['{', '::']:
                    ...
                    # namespace alias found, ignore for now
                    self.lex.accept()
                    self.resetDoc()
                    self.parseUntil(owner, None, False, ';', False, -1)
                    return True
            ...
            try:
                if not args.encoding:
                    ...
                    if sys.version_info.major >= 3:
                        f = open(file, 'r', encoding=enc, newline=None)
                        source = f.read()
                        # remove UTF-8 BOM marker if present (e.g. for a UTF-8 file with BOM when a wrong encoding such as the default latin_1 is used.
                        if len(source) >= 3 and source[0:3] == '\357\273\277':
                            source = source[3:]
                    else:
                        import codecs
                ...
                # Custom callback preprocessing
                if args.callback and os.path.exists(args.callback):
                    pluginmod = None
                    try:
                        pluginmod = loadModule(args.callback)
                    except:
                        sys.stdout.write("Could not load callback plugin "  + args.callback)
                        raise
                    if "Preprocess" not in dir(pluginmod):
                        raise Exception("Callback plugin does not provide a Preprocess function.")
                    source = pluginmod.Preprocess(source, file, args)

                for m, r in SPECIAL_MACROS:
                ...
                parser.forwards = dict()
                parser.root = namespace
                parser.funcvarQualifierPrefixes = copy.deepcopy(args.funcvarQualifierPrefixes)
                parser.funcvarQualifierPostfixes = copy.deepcopy(args.funcvarQualifierPostfixes)
                ...
                if frameworkRegistration and otherMember:
                    errors.append(otherMember.pos, 'A file which includes framework registrations must not contain any additional source.')
...
def loadGenerators(dir, show, quiet):
...
def processHandler(queue, resultQueue, args, stampTime, previousFiles, generators, scriptDir, basedir, summary):
    if not sharesModules():
        generators = loadGenerators(scriptDir, False, args.quiet)
    errorMsg = ''
    exceptions = False
    filesWithErrors = []
    while True:
        ...
        try:
            ...
            sys.stderr.write('warning: Exception occurred in sourceprocessor subprocess:\n' + traceback.format_exc())
            exceptions = True
            break
        if err:
...
def main():
    ...
    parser.add_argument('--callback', action='store', help='A path to an external python script for custom preprocessing of source code.')
    parser.add_argument('--quiet', action='store_true', help='Mute logging of status.')
    ...
    frameworkroot = args.frameworkroot or []
    result = True
    for d in args.directories:
        args.directory = os.path.abspath(d)
        args.frameworkroot = copy.copy(frameworkroot)

        # append frameworks folder which can be found through args.directory
        dir = args.directory
        while True:
            ...
            if not parent or parent == dir:
                break
            dir = parent
            fwDir = os.path.join(dir, 'frameworks')
            if os.path.isdir(fwDir):
                args.frameworkroot.append(fwDir)
                break
...
def process(args):
    ...
    args.funcvarQualifierPrefixes = copy.deepcopy(FUNCVAR_QUALIFIER_PREFIXES)
    args.funcvarQualifierPostfixes = copy.deepcopy(FUNCVAR_QUALIFIER_POSTFIXES)
    ...
    args.frameworks = set()
    args.modules = dict()
    args.encoding = 'latin_1' if sys.version_info.major >= 3 else None
    args.projectId = None
    args.c4d = False
    ...
    for key in keys:
        ...
        if key.startswith('stylecheck.'):
            ...
            elif key.lower() == lexer.cmdRegisterQualifiers:
                items = { n.strip() for n in str(option).split(";") }
                args.funcvarQualifierPrefixes |= items
                args.funcvarQualifierPostfixes |= items
            ...
            for f in option.split(';'):
                f = f.strip()
                if f:
                    args.frameworks.add(getFrameworkId(args, f))
        elif key == 'modules' or key == 'modulesweak' or key == 'modulesoptional':
            args.modules['net.maxon.kernel'] = 'STRONG'
            dep = 'STRONG' if key == 'modules' else key[7:].upper()
            for m in option.split(';'):
                m = m.strip()
                if m:
                    m = getModuleId(m)
                    if m in args.modules:
                        projectErrors.append(projPos, 'Duplicate module dependency ' + m + ' in projectdefinition.txt.')
                    else:
                        args.modules[m] = dep
        elif key == 'stylecheck':
            ...
            parentdir = os.path.basename(os.path.dirname(args.directory))
            if args.projectType in ['app', 'appdll'] or parentdir in ['frameworks', 'modules']:
                args.maxon = option.lower() == 'true'
            else:
                projectErrors.append(projPos, 'The "maxon" setting is not allowed in this context.')
        elif key == 'obfuscate':
        ...
        elif key == 'frameworkroot':
            args.frameworkroot.append(os.path.join(args.directory, option))
    ...
    if not args.projectId:
        ...
        projectErrors.append(projPos, 'Illegal module id ' + args.projectId + ': A module id must not be in the net.maxon domain.')
    ...
    args.generated = os.path.abspath(args.generated)
    ...
    generators = loadGenerators(scriptDir, True, args.quiet)
    ...
    for run in range(0, 2):
        ...
        for fwDir in usedFrameworks(args, args.directory, True, set()):
            ...
            if args.generatedroot:
                # Callers might use the generatedroot and store the generated framework files not under
                # '/frameworks/[name]/generated/' but rather '/generatedroot/[name]/'
                # We ensure that hxx files of dependent frameworks can be found.
                # Note it's important to prefer the hxx files under generatedroot.
                fwName = os.path.basename(os.path.normpath(fwDir))
                summaryGenFile = os.path.join(args.generatedroot, fwName, 'summary', 'framework.summary')
                if os.path.isfile(summaryGenFile):
                    summaryFile = summaryGenFile
                    #print("Loading Summary from Generated Dir: "+str(summaryFile) +" for '" +str(fwName)+"'")
        ...
        try:
            ...
            sys.stderr.write('warning: Sourceprocessor has been cancelled.\n')
            return False
