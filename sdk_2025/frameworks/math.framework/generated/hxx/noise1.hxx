...
struct NoiseInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = NoiseInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(NoiseInterface**);
		operator const typename maxon::RefConversionTarget<S, typename S::Handler, maxon::StrongRefHandler, Noise, false>::type&() const { return reinterpret_cast<const Noise&>(this->GetBaseRef()); }
		operator const typename maxon::RefConversionTarget<S, typename S::Handler, maxon::StrongRefHandler, Noise, false>::type&() { return reinterpret_cast<const Noise&>(this->GetBaseRef()); }
...
/// @return												Seed value of that was passed to Init() or 0 if noise was not initialized.
		inline HXXADDRET2(UInt32) GetSeed() const;
...
/// @return												Value that was passed to Init() or 0 if noise was not initialized.
		inline HXXADDRET2(Int) GetPermutationTablePower() const;
...
/// @return												The permutation table or nullptr if noise was not initialized. Ownership doesn't change - the table is still owned by the Noise class.
		inline HXXADDRET2(const BaseArray<UInt16>*) GetPermutationTable() const;
...
/// @return												OK on success.
		static inline Result<void> GetFbmTable(FbmTableRef table, BaseArray<Float32>& fbm);
...
/// @return												The random table or nullptr if noise was not initialized. Ownership doesn't change - the table is still owned by the Noise class.
		inline HXXADDRET2(const BaseArray<Vector4d32>*) GetRandomTable() const;
...
/// @return												Noise value in the range of [-1..1]
		inline HXXADDRET2(Float32) SNoise(const Vector32& p) const;
/// Calculates an 'Improved Perlin Noise' value for four dimensions.
/// The noise will repeat itself after a distance of (1 << permutationTablePower).
/// @param[in] p									Point for noise calculation.
/// @return												Noise value in the range of [-1..1]
		inline HXXADDRET2(Float32) SNoise(const Vector4d32& p) const;
/// Calculates an 'Improved Perlin Noise' value for four dimensions.
/// The noise will repeat itself after a distance of (1 << permutationTablePower).
...
/// @return												Noise value in the range of [-1..1]
		inline HXXADDRET2(Float32) SNoise(const Vector32& p, Float32 time) const;
...
/// @return												Noise value in the range of [-1..1]
		inline HXXADDRET2(Float32) PeriodicSNoise(const Vector32& p, Int repeatX, Int repeatY, Int repeatZ) const;
...
/// @return												Noise value in the range of [-1..1]
		inline HXXADDRET2(Float32) PeriodicSNoise(const Vector4d32& p, Int repeatX, Int repeatY, Int repeatZ, Int repeatT) const;
...
/// @param[out] index							Nullptr or pointer to an array that will be filled with indices to the noise permutation table that correspond to the distance values. The array must at least have maximumOrder elements, otherwise the routine will crash.
		inline HXXADDRET2(void) Voronoi(const Vector32& p, Int maximumOrder, Float32* distance, Int32* index) const;
...
/// @param[out] distance					Pointer to an array that will be filled with distance values. It is guaranteed that distance[i] <= distance[i + 1]. The array must at least have maximumOrder elements, otherwise the routine will crash.
/// @param[out] index							Nullptr or pointer to an array that will be filled with indices to the noise permutation table that correspond to the distance values. The array must at least have maximumOrder elements, otherwise the routine will crash.
		inline HXXADDRET2(void) Voronoi(const Vector4d32& p, Int maximumOrder, Float32* distance, Int32* index) const;
...
/// @return												Reference to precomputed Fbm coefficients or nullptr if not enough memory.
		static inline ResultMemT<FbmTableRef> InitFbm(Float32 lacunarity, Float32 gain);
...
/// @return												Noise value. The range dependens on the values passed to InitFbm.
		inline HXXADDRET2(Float32) Fbm(FbmTableRef table, Vector32 p, Float32 octaves) const;
/// Calculates Fractal Brownian Motion noise.
/// Note that calling with Vector4d32 and the fourth (w) component set to zero delivers different results as the 4-dimensional tables are different.
/// @param[in] table							Table that was initialized with InitFbm.
/// @param[in] p									Point for noise calculation.
/// @param[in] octaves						Number of octaves to be calculated in the range of [0..15]. The higher the number, the more computationally expensive the function.
/// @return												Noise value. The range dependens on the values passed to InitFbm.
		inline HXXADDRET2(Float32) Fbm(FbmTableRef table, Vector4d32 p, Float32 octaves) const;
...
/// @return												Turbulence value, in the range of [-1.75..1.75]
		inline HXXADDRET2(Float32) Turbulence(Vector32 p, Float32 octaves, Bool absolute) const;
/// Calculates Perlin's Turbulence function.
/// @param[in] p									Point for noise calculation.
/// @param[in] octaves						Number of octaves to be calculated. The higher the number, the more computationally expensive the function.
/// @param[in] absolute						If true the absolute values of each octave are summed.
/// @return												Turbulence value, in the range of [-1.75..1.75]
		inline HXXADDRET2(Float32) Turbulence(Vector4d32 p, Float32 octaves, Bool absolute) const;
...
/// @return												Multifractal value. The range depends on the used parameters.
		inline HXXADDRET2(Float32) RidgedMultifractal(FbmTableRef table, Vector32 p, Float32 octaves, Float32 offset, Float32 threshold) const;
/// Calculates Musgraves Ridged Multifractal function.
/// @param[in] table							Table that was initialized with InitFbm.
/// @param[in] p									Point for noise calculation.
/// @param[in] octaves						Number of octaves to be calculated. The higher the number, the more computationally expensive the function.
...
/// @param[in] threshold					Must be >0.0. The higher the value, the more sharp details / peaks. A good start value is 2.0.
/// @return												Multifractal value. The range depends on the used parameters.
		inline HXXADDRET2(Float32) RidgedMultifractal(FbmTableRef table, Vector4d32 p, Float32 octaves, Float32 offset, Float32 threshold) const;
	};
	/// Intermediate helper class for NoiseInterface.
	...
	public:
		...
		using PrivateInterface = NoiseInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(NoiseInterface**);
...
/// @return												OK on success. IllegalArgumentError is returned if the permutationTablePower was out of range.
	...
	!maxon::Bool(ST::type::Handler::KIND & (maxon::VALUEKIND::COW_MASK | maxon::VALUEKIND::DEEP_CONSTNESS)) && !std::is_const<typename ST::type::PrivateReferencedType>::value,
	Fn<typename ST::type>,
...
#ifdef DOXYGEN
public ConstFnDoxy<Noise>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const NoiseInterface, maxon::StrongRefHandler, Noise>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const NoiseInterface, maxon::StrongRefHandler, Noise>>>>);
	using NonConst = Hxx1::NonConstRef;
/// allocator for common use.
	static MAXON_ATTRIBUTE_FORCE_INLINE maxon::ResultMemT<Noise> Create();
/// Create a new Noise object for later sampling.
/// A permutationTablePower of 10 results in 1024 elements, which is a good compromise between too frequent repetition and memory consumption.
/// @param[in] seed								Start value for the random table generation.
/// @param[in] permutationTablePower	This specifies the size of the permutation table, which will have (2 ^ permutationTablePower) entries. permutationTablePower must be [5..16], otherwise initialization fails.
	static MAXON_ATTRIBUTE_FORCE_INLINE maxon::ResultMemT<Noise> Create(UInt32 seed, Int permutationTablePower);
};

class NoiseInterface::Hxx1::NonConstRef :
#ifdef DOXYGEN
public FnDoxy<NonConstRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<NoiseInterface, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<NoiseInterface, maxon::StrongRefHandler, NonConstRef>>>>);
};

/// Noise is the reference class of NoiseInterface.
class Noise : public NoiseInterface::Hxx1::Reference
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Noise, typename NoiseInterface::Hxx1::Reference);
};
...
struct RenderNoiseInterface::Hxx1
{
	class Reference;
	class NonConstRef;
	using ReferenceClass = RenderNoise;
	...
	public:
		...
		using PrivateBaseClass = S;
		using PrivateBaseClass::PrivateBaseClass;
		using PrivateInterface = RenderNoiseInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static ConstFn PrivateLookupFn(RenderNoiseInterface**);
...
/// @return												Noise value in the range of [0..1]
		inline HXXADDRET2(Float32) Noise(const Vector32& p, Float32 time, Float32 octaves = 6.0, Bool absolute = false, Float32 sampleRadius = 0.0) const;
/// Calculates a noise value.
/// @param[in] p									Point for which noise is calculated.
/// @param[in] time								Time for which noise is calculated.
/// @param[in] octaves						Maximum number of octaves calculated. Does only apply to some noises.
...
/// @param[in] sampleRadius				The radius that determines how much details of the noise will be retained. A value of 0.0 returns the original value. Any higher values reduce details and might blend the result towards an average value.
/// @return												Noise value in the range of [0..1]
		inline HXXADDRET2(Float32) Noise(const Vector2d32& p, Float32 time, Float32 octaves = 6.0, Bool absolute = false, Float32 sampleRadius = 0.0) const;
	};
	/// Intermediate helper class for RenderNoiseInterface.
	template <typename S> class Fn : public ConstFn<S>
	{
	public:
		...
		using PrivateInterface = RenderNoiseInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(RenderNoiseInterface**);
/// Initializes the noise.
/// A permutationTablePower of 10 results in 1024 elements, which is a good compromise between too frequent repetition and memory consumption.
/// @param[in] seed								Start value for the random table generation.
/// @param[in] permutationTablePower	This specifies the size of the permutation table, which will have (2 ^ permutationTablePower) entries. permutationTablePower must be [5..16], otherwise initialization fails.
...
/// @return												OK on success. An error is also returned if the passed noise reference was not yet initialized.
	...
	public:
		...
		using PrivateInterface = RenderNoiseInterface;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(RenderNoiseInterface**);
		operator const typename maxon::RefConversionTarget<S, typename S::Handler, maxon::StrongCOWRefHandler, RenderNoise, false>::type&() const { return reinterpret_cast<const RenderNoise&>(this->GetBaseRef()); }
		operator const typename maxon::RefConversionTarget<S, typename S::Handler, maxon::StrongCOWRefHandler, RenderNoise, false>::type&() { return reinterpret_cast<const RenderNoise&>(this->GetBaseRef()); }
/// Initializes the noise.
/// A permutationTablePower of 10 results in 1024 elements, which is a good compromise between too frequent repetition and memory consumption.
/// @param[in] seed								Start value for the random table generation.
/// @param[in] permutationTablePower	This specifies the size of the permutation table, which will have (2 ^ permutationTablePower) entries. permutationTablePower must be [5..16], otherwise initialization fails.
...
#ifdef DOXYGEN
public COWFnDoxy<RenderNoise>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const RenderNoiseInterface, maxon::StrongCOWRefHandler, RenderNoise>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const RenderNoiseInterface, maxon::StrongCOWRefHandler, RenderNoise>>>>);
	using NonConst = Hxx1::NonConstRef;
};

class RenderNoiseInterface::Hxx1::NonConstRef :
#ifdef DOXYGEN
public FnDoxy<NonConstRef>
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<RenderNoiseInterface, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<RenderNoiseInterface, maxon::StrongRefHandler, NonConstRef>>>>);
};

/// RenderNoise is the reference class of RenderNoiseInterface.
///
...
class RenderNoise : public RenderNoiseInterface::Hxx1::Reference
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(RenderNoise, typename RenderNoiseInterface::Hxx1::Reference);
};
