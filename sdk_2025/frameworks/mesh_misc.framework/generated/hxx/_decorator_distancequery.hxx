...
public:
	MAXON_METHOD Result<void> Init(cinema::PolygonObject* mesh, Bool initVoxelization)
	{
	...
	{
		return maxon::Cast<DistanceQueryRef>(self.GetDecoratedObject()).Init(std::forward<const Block<const Vector>>(points), std::forward<const Block<const SimplePolygon>>(polygons), objectPosition);
	}
