...
#define MAXON_MESHATTRIBUTE(T, Name) \
	...
	{ \
		...
		{ \
			...
			public: \
			TYPE() { } \
			TYPE(TYPE&& src)	{ MoveContent(std::move(src)); } \
...
/// @param[in] attributeType			The mesh attribute type name.
//----------------------------------------------------------------------------------------
...
public:
	...
	MeshAttribute(const MeshAttributeClass& cls, void* valuePtr) : _cls(cls), _valuePtr(valuePtr) {	}
	MeshAttribute(const MeshAttributeClass& cls, const void* valuePtr) : _cls(cls), _valuePtr(const_cast<void*>(valuePtr)) {	}
