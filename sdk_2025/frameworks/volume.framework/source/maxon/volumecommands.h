...
struct VolumeCommandData
{
	...
	BaseArray<cinema::BaseObject*>*				op = nullptr;								// The objects to perform the operation on.
	BaseArray<const cinema::BaseObject*>* reference = nullptr;				// Reference volumes for field conversion and MoSpline reference for spline conversion.
	cinema::BaseDocument*									doc = nullptr;							// The current document.
	cinema::BaseThread*										bt = nullptr;								// A thread reference to test for cancellation.
	BaseArray<cinema::BaseObject*>*				masks = nullptr;						// Optional mask volumes for some operations like filters.
	BaseArray<cinema::BaseObject*>				result;											// The output lists for the command.
	...
	const cinema::SplineData*							splineData = nullptr;				// A spline data pointer used for scaling of spline radii
};
