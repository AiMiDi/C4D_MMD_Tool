...
using PatchC4DDescriptionEntryDelegate = Delegate<Result<void>(cinema::BaseContainer& param)>;
...
public:
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> CreateC4DDescription(const DataType& dataType, cinema::Description& c4dDescription, const LanguageRef& language,
		const DataDictionary& dataEntry, const DataDictionary& guiEntry, const DataDescription& mainDataDescription, const DataDescription& stringDescription, const cinema::DescID& mainId,
		const cinema::DescID& groupId, const PatchC4DDescriptionEntryDelegate& patchEntryFunc, DescTranslation& translateIds,
		const BaseArray<InternedId>& parentIds, const cinema::DescID& parentFoldId, const GetDataCallbackType& getDataCallback, const GetExtraDataCallbackType& getExtraDataDelegate,
		const cinema::BaseDocument* doc) const;
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> ConvertToC4D(cinema::GeData& output, const DataType& dataType, const Data& data, const cinema::DescID& descIdSuffix, const DataDictionary& dataEntry,
		const DataDictionary& guiEntry, const GetExtraDataCallbackType& extraDataDelegate, const cinema::BaseDocument* doc) const;
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<Tuple<Data, Bool>> ConvertToCore(const DataType& dataType, const cinema::GeData& data, const cinema::DescID& descIdSuffix, const DataDictionary& dataEntry,
		const DataDictionary& guiEntry, const Data& oldData, const GetExtraDataCallbackType& extraDataDelegate, const cinema::BaseDocument* doc) const;
...
namespace UiConversions
{
	...
	MAXON_DECLARATION(UiConversions::EntryType, UiColorForUiColors, "net.maxon.ui.colorforuicolors");
	MAXON_DECLARATION(UiConversions::EntryType, UiColorForUiColorsAlpha, "net.maxon.ui.colorforuicolorsalpha");
