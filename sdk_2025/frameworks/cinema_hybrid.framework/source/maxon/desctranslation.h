...
public:
	...
	using DescEntryMap = HashMap<cinema::DescID, DescEntryStruct>;
	using GroupIdMap = HashMap<InternedId, cinema::DescID>;
	...
	//----------------------------------------------------------------------------------------
	const DescEntryStruct* FindTranslationDescID(const cinema::DescID& id) const
	{
	...
	const DescEntryMap::Entry* ReverseFindTranslationDescID(const Block<const InternedId>& emulKey) const
	{
		...
		{
			if (emulKey.IsEqual(transEntry.GetValue()._id))
			{
				return &transEntry;
			}
		}
