...
MAXON_ATTRIBUTE(Int, LIST_VIEW_ICON_SIZE_LEVEL, "net.maxon.assetmanager.settings.listviewiconsize");
MAXON_ATTRIBUTE(Int, GRID_VIEW_ICON_SIZE_LEVEL, "net.maxon.assetmanager.settings.gridviewiconsize");
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD cinema::SubDialog* Get();
...
public:
	...
	/// @param[in] source							Optional parameter to indicate the source of the action, for example "commander".
	/// @return												Bool on success. True if loaded. False indicates another error state.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<Bool> LoadAssets(const AssetRepositoryRef& searchRepository, const HashMap<Id, String>& selectionAndFilterString, const GraphModelPresenterRef& presenter, const nodes::NodesGraphModelRef& graphModelRef, const String& source);

	static MAXON_METHOD Result<void> AddFileAssets(const Block<Url>& urls, const AssetRepositoryRef& lookupRepository, Bool showSettings, const AssetRepositoryRef& targetRepository = {}, const Id& assetCategory = {}, Bool forceAddToAssetCategory = false, Bool revealAsset = true);

	static MAXON_METHOD Result<AssetManagerSubDialogRef> GetAssetManagerSubDialogRef();

	//----------------------------------------------------------------------------------------
	/// GetSelectedAssetsFilename retrieves the filename/url of selected assets in the last active Asset Browser.
	/// This will only return assets of type AssetTypes::File, otherwise only assets which pass the given filter.
	/// @param[in] updateUsage				Update ASSETMETADATA::Usage of the asset, see UpdateAssetUsage()
	/// @param[in] showDownloadDialog	Shows download dialog for not yet downloaded assets.
	/// @param[in] filter							Optional filter callback.
	/// @return												Two arrays, with the first containing the assets Id: Asset ID, Url: asset url, AssetDescription: asset description, String: filter string from asset browser. And the second the assets subtype.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<AssetDragArrayTuple> GetSelectedAssetsFilename(Bool updateUsage, Bool showDownloadDialog, const Filter& filter = GetZeroRef<Filter>());
};
