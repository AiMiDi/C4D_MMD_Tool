...
Result<String> GetPasswordHash(const Block<const Char>& password, const StreamConversionFactory& hashClass, const DataDictionary& settings)
{
	iferr_scope;
	...
	hash.ConvertAll(password, hashValue) iferr_return;
...
Result<String> GetPasswordHash(const String& password, const StreamConversionFactory& hashClass, const DataDictionary& settings)
{
	...
	return GetPasswordHash(passwdchr, hashClass, settings);
}
