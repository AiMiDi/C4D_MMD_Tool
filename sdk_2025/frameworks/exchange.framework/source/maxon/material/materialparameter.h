...
struct TextureBuffer
{
	...
	{
		return _pixels.GetPointer() != nullptr && _pixels->IsPopulated();
	}
};
...
//----------------------------------------------------------------------------------------
/// An atomic modification instruction that controls the material creation process.
/// The definition space is abstracted away from material creator implementation,
/// meaning that each material importer uses its own means to achieve the modified result. 
/// One should consider instructions as an auxiliary (and hence not mandatory)
/// source of information that improves the quality the of created materials.
//----------------------------------------------------------------------------------------
struct InstructionData
{
	InstructionData() = default;

	explicit InstructionData(const Id& id, Data&& value, Bool isForTexture)
		: _id(id),
			_value(std::move(value)),
			_isForTexture(isForTexture)
	{
	}
	...
	Result<void> CopyFrom(const InstructionData& src)
	{
		iferr_scope;
		_id = src._id;
		_isForTexture = src._isForTexture;
	...
	{
		return _id == other._id && _isForTexture == other._isForTexture && _value.IsEqual(other._value);
	}

	HashInt GetHashCode() const
	{
		DefaultHasher hash;
		hash.CombineUnordered(_id.GetHashCode());
		hash.CombineUnordered(_isForTexture);
		hash.CombineUnordered(_value.GetHashCode());
		return hash;
	}

	Id	 _id;									 ///< Modifier type (one of MATERIAL::INSTRUCTIONS::[MODIFIER_ID])
	Data _value;							 ///< Payload value. Must be InstructionGraph (not implemented) if _id == MATERIAL::INSTRUCTIONS::GRAPHMODIFIER
	Bool _isForTexture = true; ///< Is making effect for scalar values or for textures
};
...
struct InstructionSet
{
	...
	{
		return _instructions.CopyFrom(src._instructions);
	}

	Bool IsPopulated() const
	{
		return _instructions.IsPopulated();
	}

	Bool IsEqual(const InstructionSet& other, EQUALITY equality = EQUALITY::DEEP) const
	{
		return _instructions.GetHashCode() == other._instructions.GetHashCode();
	}
	...
	BaseArray<InstructionData> _instructions; ///< A container with the instructions
};
...
struct MaterialExchangeData
{
	...
	/// maxon::MATERIAL::PORTBUNDLE::USDPREVIEWSURFACE (connects node materials with usd preview material).
	/// maxon::MATERIAL::PORTBUNDLE::VIEWPORTMATERIAL (connects node materials with the viewport).
	//----------------------------------------------------------------------------------------
	Id _materialTypeId;
	...
	//----------------------------------------------------------------------------------------
	/// Contains a set of instructions with the auxiliary information for the parameter mapping.
	/// Each parameter in _parameters can be mapped to the instruction set.
	/// Each instruction set contains a set of maxon::material::InstructionData
	//----------------------------------------------------------------------------------------
