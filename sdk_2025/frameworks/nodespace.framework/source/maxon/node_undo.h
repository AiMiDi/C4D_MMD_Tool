...
public:
	...
	{
		_previous = graph.GetViewRoot().GetValue(NimbusUndoAttribute).GetOrDefault() iferr_ignore("no error handling possible here");
		graph.GetViewRoot().SetValue(NimbusUndoAttribute, ConstDataPtr(true), false) iferr_ignore("no error handling possible here");
	}
	...
	{
		if (_graph)
			_graph.GetViewRoot().SetValue(NimbusUndoAttribute, ConstDataPtr(_previous), false) iferr_ignore("no error handling possible here");
	}
