...
// the space after # is necessary so that the source processor ignores the directive
...
{
	MAXON_INTERFACE_NONVIRTUAL(NodePathInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.graph.interface.nodepath", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Converts a node path into its textual notation as an InternedId. See ToCString().
	/// @param[in] path								The path to convert.
	/// @return												The textual notation of the path as an InternedId.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<InternedId> ToInternedId(const PathBlock& path);
	...
	//----------------------------------------------------------------------------------------
	/// Slice the NodePath from #start to GetCount(). Does not make a heap allocation,
	/// only a sub block of the original NodePath is returned.
	/// @param[in] start							The starting depth index in the original NodePath.
	/// @return												The sliced block.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD PathBlock Slice(Int start) const;

	//----------------------------------------------------------------------------------------
	/// Slice the NodePath from #start to #end. Does not make a heap allocation,
	/// only a sub block of the original NodePath is returned.
	/// Slice from 0 to GetNodeDepth() for a node path (without port and port list).
	/// @param[in] start							The starting depth index in the original NodePath.
	/// @param[in] end								The ending depth index in the original NodePath.
	/// @return												The sliced block.
	//----------------------------------------------------------------------------------------
...
/// make use of the precomputed GetFastUniqueHashCode of the NodePath.
//----------------------------------------------------------------------------------------
