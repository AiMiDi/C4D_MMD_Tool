...
/// you have to set ReqOrderIndex instead.
/// This will be used to derive the value for OrderIndex.
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE(Int, OrderIndex, "orderindex", MAXON_ATTRIBUTE_READONLY);
...
///
/// For a NodeSystem-based graph this is a derived attribute which can't be set directly,
/// you have to set ReqInsertIndex instead.
/// This will be used to derive the value for InsertIndex.
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE(Int, InsertIndex, "insertindex", MAXON_ATTRIBUTE_READONLY);

//----------------------------------------------------------------------------------------
/// The PortValue attribute is a derived attribute to query the value of a port, ignoring upstream overrides.
...
/// value isn't set, the values of connected downstream ports. The value may be overridden
/// by upstream connections to the port, and you have to use the EffectivePortValue attribute
/// to query the final value of a port.
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE(Data, PortValue, "value", MAXON_ATTRIBUTE_READONLY);

//----------------------------------------------------------------------------------------
/// The EffectivePortValue attribute is a derived attribute to query the effective value of a port.
/// The attribute value has to be derived by the graph implementation, typically this takes into
/// account the PortValue, upstream port connections and also functional nodes which can be
/// evaluated during validation of the graph when their inputs have known values too.
///
/// If no effective value can be derived for the port, which includes the case of a port with
/// dynamic value, the effective value is an empty Data.
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE(Data, EffectivePortValue, "effectivevalue", MAXON_ATTRIBUTE_READONLY);

[[deprecated("use EffectivePortValue (since 2024.0)")]] static const EffectivePortValue_PrivateHelper& ConstantValue = EffectivePortValue;
...
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE(String, EffectiveName, "effectivename", MAXON_ATTRIBUTE_READONLY);
...
{
	MAXON_INTERFACE(GraphAttributeInterface, MAXON_REFERENCE_CONST, "net.maxon.graph.interface.attribute", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
public:
	//----------------------------------------------------------------------------------------
...
{
	MAXON_INTERFACE(GraphMessageInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.graph.interface.message", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
public:
	//----------------------------------------------------------------------------------------
	...
	/// @copydoc ErrorInterface::FormatMessage
	MAXON_METHOD String Format(const LanguageRef& language, const ErrorInterface::PartFormatter& partFormatter) const;

	//----------------------------------------------------------------------------------------
	/// @brief Allows to read the message's internal data.
	/// The data is valid as long as the message's lifetime. Any graph change will invalidate it.
	/// @return												The data (can be empty if message has no additional data).
	//----------------------------------------------------------------------------------------
	MAXON_METHOD ConstDataPtr GetData() const;
};


//----------------------------------------------------------------------------------------
/// An AttributeValueConversion is passed to methods of NodeSystemInterface and GraphModelInterface
/// to convert the values of all attributes.
/// @param[in] path								The path of the node where the attribute value is set.
/// @param[in] attr								The identifier of the attribute. For template arguments this is empty.
/// @param[in] value							The value of the attribute.
/// @return												The new value to use for the attribute, or an empty optional if the old value shall be kept.
//----------------------------------------------------------------------------------------
using AttributeValueConversion = Delegate<Result<Opt<Data>>(const NodePath& path, const InternedId& attr, const Data& value)>;
