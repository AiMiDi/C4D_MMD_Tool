...
namespace IMAGEPROPERTIES
{
	...
	namespace TEXTURE
	{
		MAXON_ATTRIBUTE(FileFormat,			FILEFORMAT,	"net.maxon.image.fileformat");	///< Optional: fileformat that loaded the texture.
	...
	namespace LAYER
	{
		...
		namespace ADJUSTMENT
		{
			...
			MAXON_ATTRIBUTE(Array<Char>, PSD_DATA,		 "net.maxon.image.layer.psd_data");			///< Adjustment layer data.
		}
	}
}
...
} MAXON_ENUM_FLAGS(TRANSFORM_COLORS_FLAGS);
...
/// @return												Class<ImagePixelStorage> on success.
//----------------------------------------------------------------------------------------
...
{
	MAXON_INTERFACE(ImageTextureInterface, MAXON_REFERENCE_NORMAL, "net.maxon.image.interface.imagetexture", MAXON_IMPLEMENTATION_MODULE("net.maxon.image"));
...
public:
	...
	static MAXON_METHOD Result<ImageTextureRef> LoadTexture(
		...
		const Class<ImagePixelStorage>& pixelStorageClass = GetNormalPixelStorageClass(), const LayerSetInterface* layerSet = nullptr);
	...
	MAXON_METHOD Result<MediaSessionRef> Load(const Url& url, const TimeValue& targetTime, MEDIASESSIONFLAGS flags, 
		const Class<ImagePixelStorage>& pixelStorageClass = GetZeroRef<Class<ImagePixelStorage>>(), const LayerSetInterface* layerSet = nullptr);
...
{
	MAXON_INTERFACE(ImageInterface, MAXON_REFERENCE_NORMAL, "net.maxon.image.interface.image", MAXON_IMPLEMENTATION_MODULE("net.maxon.image"));
...
{
	MAXON_INTERFACE(ImageLayerInterface, MAXON_REFERENCE_NORMAL, "net.maxon.image.interface.imagelayer", MAXON_IMPLEMENTATION_MODULE("net.maxon.image"));
...
{
	MAXON_INTERFACE_NONVIRTUAL(ImageFunctionsInterface, MAXON_REFERENCE_STATIC, "net.maxon.image.interface.imagefunctions", MAXON_IMPLEMENTATION_MODULE("net.maxon.image"));
...
/// @param[in] sourceUrl					Url of the source movie/image sequence.
/// @param[in] destPath						Url of the destination folder.
/// @param[in] destFilePatern			Format string of the destination files.
/// @param[in] exportClass				Export Image class to use.
/// @param[out] resultFiles				Receives the generated files.
/// @return												OK on success.
/// example:
/// Url loadUrl = Application::GetUrl(APPLICATION_URLTYPE::CURRENT_MODULE_RESOURCE_DIR).Append("modules"_s).Append("images"_s).Append("c4d_logo_with_alpha.avi"_s) iferr_return;
/// Url saveUrl = Application::GetUrl(APPLICATION_URLTYPE::USER_DOCUMENTS_DIR) iferr_return;
/// ConvertImageSequence(loadUrl, saveUrl, "_logo_with_alpha@{4'0'}.png"_s, ImageSaverClasses::Png(), nullptr) iferr_return;
/// 
/// or to convert a image sequence
/// Url loadUrl = Application::GetUrl(APPLICATION_URLTYPE::CURRENT_MODULE_RESOURCE_DIR).Append("modules"_s).Append("images"_s).Append("logo_with_alpha@{4'0'}.png"_s) iferr_return;
/// loadUrl.Set(URLFLAGS::IMAGESEQUENCE_FPS, 1.0) iferr_return; Url saveUrl =
/// Url saveUrl = Application::GetUrl(APPLICATION_URLTYPE::USER_DOCUMENTS_DIR) iferr_return;
/// ConvertImageSequence(loadUrl, saveUrl, "_logo_with_alpha@{4'0'}.png"_s, ImageSaverClasses::Png(), nullptr) iferr_return;
//----------------------------------------------------------------------------------------
