  ...
  const Delegate<Result<Bool>(const REFTYPE& child, const REFTYPE& parent, const ConstDataPtr& branch, Int depth)>& func, Int depth = 0)
{
  ...
  {
    Bool continueTraverse = MediaTraverseMeAndChildren(Cast<REFTYPE>(child), t, branch, func, depth + 1) iferr_return;
    if (!continueTraverse)
      return false;
  }
  return true;
  ...
  const Delegate<Result<Bool>(const REFTYPE& child, const REFTYPE& parent, const ConstDataPtr& branch, Int depth)>& func, Int depth = 0)
{
  ...
  {
    continueTraverse = MediaTraverseChildren(t, b, func, depth) iferr_return;
    if (!continueTraverse)
      return false;
  }
...
namespace MEDIAFORMAT
{
	...
	namespace IMAGELAYER
	{
		...
		namespace ADJUSTMENT
		{
			...
			MAXON_ATTRIBUTE(Array<Char>, PSD_DATA, "net.maxon.mediaformat.layer.psd_data");				///< adjustment layer data
		}
	}
	namespace VIDEO
	{
	...
	namespace AUDIO
	{
		...
		MAXON_ATTRIBUTE(Array<DataDictionary>, MARKERARRAY, "net.maxon.mediaformat.audio.markerarray");		///< Array of marker DataDictionaries see MEDIAFORMAT::AUDIO::MARKERS.
		namespace MARKER
		{
...
MAXON_DECLARATION(Class<MediaBaseRef>, MediaBaseClass, "net.maxon.image.class.mediabase", MAXON_IMPLEMENTATION_MODULE("net.maxon.image"));
