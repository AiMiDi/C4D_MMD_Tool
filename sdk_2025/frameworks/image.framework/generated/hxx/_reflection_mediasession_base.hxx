...
namespace maxon
{
	...
	PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(MediaBaseInterface);
	...
	public:
		...
		Result<void> ConnectToSession(const MediaSessionRef& session) 
		{
			...
			(*handler_)(_fn_MediaBaseInterface_ConnectToSession, o_, maxon::reflection::ProxyInvocation(std::addressof(proxy_result), proxy_args, PRIVATE_MAXON_MF_REFLECTION_INVOCATION((Result<void>), const MediaSessionRef&), reinterpret_cast<const void* const&>(mt_.MediaBaseInterface_ConnectToSession)));
			return proxy_result;
		}
		...
		MediaSessionRef GetSession() const 
		{
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<MediaBaseInterface>();
			MediaSessionRef proxy_result = HXXRET3(NULLIMPL, MediaSessionRef);
			...
			(*handler_)(_fn_MediaBaseInterface_GetSession, o_, maxon::reflection::ProxyInvocation(std::addressof(proxy_result), proxy_args, PRIVATE_MAXON_MF_REFLECTION_INVOCATION((MediaSessionRef)), reinterpret_cast<const void* const&>(mt_.MediaBaseInterface_GetSession)));
			return proxy_result;
		}
