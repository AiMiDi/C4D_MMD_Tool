...
namespace maxon
{
	...
	const maxon::Char* const ImageRegionObservableInterface::MTable::_ids = 
		...
		"Check@bdaa1636cff91244\0" // Bool Check(const GenericData& handler) const
	"";
	MAXON_WARNING_PUSH
	...
	public:
		...
		Bool Check(const GenericData& handler) const { if (maxon::NonvirtualInterfaceReference::PrivateResolve(&_interface, true)) return ((const ImageRegionObservableInterface*) this)->Check(handler); return maxon::PrivateLogNullptrError(false);}
	};
	MAXON_WARNING_POP
	...
	maxon::Bool ImageRegionObservableInterface::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
	{
		...
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		#else
		tbl->ImageRegionObservableInterface_Check = PRIVATE_MAXON_MF_CAST(decltype(ImageRegionObservableInterface_Check), &Hxx2::Wrapper<Hxx2::Unresolved>::ImageRegionObservableInterface_Check);
		#endif
	#ifdef PRIVATE_MAXON_ASSEMBLE_UNRESOLVED
