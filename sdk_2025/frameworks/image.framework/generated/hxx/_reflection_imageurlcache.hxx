...
namespace maxon
{
	...
	public:
		...
		maxon::ObservableRef<ObservableFreeCacheDelegate> ObservableFreeCache(maxon::Bool create) const 
		{
		...
		Result<Opt<ImageBaseRef>> GetImage(UpdateCacheDelegate&& updateDelegate, Bool async) const 
		{
			...
			maxon::reflection::Argument proxy_args[] = {maxon::reflection::Argument::ByValue(reinterpret_cast<maxon::GenericComponent*>(o_) + mt_.ImageUrlCacheInterface_GetImage_Offset), maxon::reflection::Argument::ByReference(updateDelegate), maxon::reflection::Argument::ByValue(async)};
			(*handler_)(_fn_ImageUrlCacheInterface_GetImage, o_, maxon::reflection::ProxyInvocation(std::addressof(proxy_result), proxy_args, PRIVATE_MAXON_MF_REFLECTION_INVOCATION((Result<Opt<ImageBaseRef>>), UpdateCacheDelegate&&, Bool), reinterpret_cast<const void* const&>(mt_.ImageUrlCacheInterface_GetImage)));
			return proxy_result;
		}
	...
	static maxon::Result<void> RegisterReflection_ImageUrlCacheInterface(maxon::reflection::Class* cls, maxon::reflection::FunctionRegistrationCallback callback)
	{
		maxon::reflection::FunctionInfo functions[] =
		{
			...
			MAXON_GET_VIRTUAL_METHOD_INFO(CONST_INSTANCE_FUNCTION, "updateDelegate\0async\0", 5LL, ImageUrlCacheInterface, ImageUrlCacheInterface_GetImage),
		};
