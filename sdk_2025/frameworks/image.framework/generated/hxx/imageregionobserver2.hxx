...
struct ImageRegionObservableInterface::MTable
{
	...
	PRIVATE_MAXON_MF_POINTER(ImageRegionObservableInterface_Check, Check, false, ImageRegionObservableInterface, const, (Bool), const GenericData& handler);
	template <typename IMPL> void Init()
	{
		...
		ImageRegionObservableInterface_Check = ImageRegionObservableInterface_Check_GetPtr<IMPL>(0, true).first;
	}
};
...
struct ImageRegionObservableInterface::Hxx2
{
	...
	public:
		...
		PRIVATE_MAXON_MF_WRAPPER(ImageRegionObservableInterface_Check, ImageRegionObservableInterface, const, (Bool), const GenericData& handler) { return S::Get(PRIVATE_MAXON_MF_THIS(const ImageRegionObservableInterface))->Check(handler); }
	};
...
MAXON_ATTRIBUTE_FORCE_INLINE auto ImageRegionObservableInterface::Check(const GenericData& handler) const -> Bool
{
	return PRIVATE_MAXON_MF_INVOKE((this), MTable::_instance.ImageRegionObservableInterface_Check, handler);
}
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ImageRegionObservableInterface::Hxx1::ConstFn<S>::Check(const GenericData& handler) const -> HXXADDRET2(Bool)
{
	HXXRETURN0(HXXADDRET2(Bool)); HXXRES; HXXCONST(maxon::PrivateLogNullptrError(false)) 
	return (PRIVATE_MAXON_MF_INVOKE((o_), MTable::_instance.ImageRegionObservableInterface_Check, handler));
}
#if defined(MAXON_DEPENDENCY_ENABLE) && !defined(PRIVATE_MAXON_REGISTRATION_UNIT) && !defined(MAXON_DEPENDENCY_DISABLE_imageregionobserver)
#ifdef MAXON_USE_REGISTER_FUNCTION
