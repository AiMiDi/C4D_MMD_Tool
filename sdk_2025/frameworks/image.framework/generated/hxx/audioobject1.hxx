...
struct AudioObjectInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = AudioObjectInterface;
...
/// Return the number of bits per channel.
		inline HXXADDRET2(Int) GetBitsPerChannel() const;
/// Return the sample rate in Hz.
		inline HXXADDRET2(Float) GetSampleRate() const;
/// Return the duration/length of the sound in samples.
		inline HXXADDRET2(Int) GetSampleCount() const;
/// Return the length as BaseTime
		inline HXXADDRET2(TimeValue) GetLength() const;
/// Read only data buffer access
		inline HXXADDRET2(Block<const Char>) GetDataPtr() const;
	};
	/// Intermediate helper class for AudioObjectInterface.
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<AudioObjectInterface, maxon::StrongRefHandler, AudioObjectRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<AudioObjectInterface, maxon::StrongRefHandler, AudioObjectRef>>>>);
