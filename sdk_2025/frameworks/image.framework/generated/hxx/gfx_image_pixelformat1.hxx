...
struct PixelFormatInterface::Hxx1
{
	...
	public:
		...
		using PrivateInterface = PixelFormatInterface;
...
/// @return												Block with the description of all channels.
		inline HXXADDRET2(Block<const ImageChannel>) GetChannels() const;
/// Returns the corresponding PixelFormatGroup
		inline HXXADDRET2(const PixelFormatGroup&) GetPixelFormatGroup() const;
...
/// identical to GetBitsPerPixel().
		inline HXXADDRET2(const ChannelOffsets&) GetChannelOffsets() const;
/// Returns the size in bits of each pixel (the sum of all channels).
		inline HXXADDRET2(BITS) GetBitsPerPixel() const;
...
/// @return												The number of bytes for the requested number of bytes with the given alignment.
		inline HXXADDRET2(Int) GetBytesPerLine(Int width, Int lineAlignment) const;
...
/// e.g. yuv encoding packs 2 pixels into 4 bytes (Y0U0Y1V1) but the u and v value is the same for both pixels.
		inline HXXADDRET2(Int) GetPackedPixelCount() const;
/// Returns true if the pixel format supports alpha.
		inline HXXADDRET2(Bool) HasAlpha() const;
...
/// Returns the default color space
		inline HXXADDRET2(const ColorProfile&) GetDefaultColorProfile() const;
...
/// @return												True if the pixels are identical.
		inline HXXADDRET2(Bool) IsIdenticalPixel(const ImageConstBuffer& a, const ImageConstBuffer& b, Int oneTolerance) const;
...
/// @return												String representation of the pixel.
		inline HXXADDRET2(String) PixToString(const Pix* src) const;
...
/// Returns the short name set in the init function.
		inline HXXADDRET2(const Char*) GetShortName() const;
	};
	/// Intermediate helper class for PixelFormatInterface.
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const PixelFormatInterface, maxon::StrongCOWRefHandler, PixelFormat>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<const PixelFormatInterface, maxon::StrongCOWRefHandler, PixelFormat>>>>);
...
#else
public maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<PixelFormatInterface, maxon::StrongRefHandler, NonConstRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(NonConstRef, maxon::RefBase<typename Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<PixelFormatInterface, maxon::StrongRefHandler, NonConstRef>>>>);
};
