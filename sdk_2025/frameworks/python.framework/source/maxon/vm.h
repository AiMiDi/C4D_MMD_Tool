...
public:
	MAXON_IMPLICIT LocalStringStorage(const String& str)
	{
		if (_arr.Resize(str.GetLength() + 1) == OK)
			str.GetCStringBlock(_arr) iferr_cannot_fail("already presized");
	}
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> Init(const LoadRuntime& runtimeInfo, const DataDictionary& settings = DataDictionary::DefaultValue());
...
namespace PythonVms
{
// Same as MAXON_CPYTHON3VM()
#define MAXON_CPYTHONVM() maxon::PythonVms::Get(maxon::Id("net.maxon.python.virtualmachine.c4dpy3"))

#define MAXON_CPYTHON3VM() maxon::PythonVms::Get(maxon::Id("net.maxon.python.virtualmachine.c4dpy3"))
}
