...
template <typename T> inline typename std::enable_if<GetCollectionKind<T>::value != COLLECTION_KIND::ARRAY && !STD_IS_REPLACEMENT(same, T, Data), Result<void>>::type ReadDocument(const Url& url, const Id& id, T& object, const DataDictionary& dict = DataDictionary())
{
	return IoDocument::ReadDocument(url, id, DataPtr(object), dict);
}

template <typename T> inline typename std::enable_if<STD_IS_REPLACEMENT(same, T, Data), Result<void>>::type ReadDocument(const Url& url, const Id& id, T& object, const DataDictionary& dict = DataDictionary())
{
...
template <typename T> inline typename std::enable_if<GetCollectionKind<T>::value != COLLECTION_KIND::ARRAY && !STD_IS_REPLACEMENT(same, T, Data), Result<void>>::type ReadDocument(UrlOrInputStream&& urlOrStream, const Id& id, T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
	return IoDocument::ReadDocument(std::move(urlOrStream), id, DataPtr(object), format, dict);
}

template <typename T> inline typename std::enable_if<STD_IS_REPLACEMENT(same, T, Data), Result<void>>::type ReadDocument(UrlOrInputStream&& urlOrStream, const Id& id, T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
...
template <typename T> inline Result<void> WriteDocument(const Url& url, OPENSTREAMFLAGS flags, const Id& id, const T& object,
	IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
	iferr_scope;
	OutputStreamRef stream = url.OpenOutputStream(flags) iferr_return;
...
template <typename T> inline typename std::enable_if<STD_IS_REPLACEMENT(same, T, Data), Result<void>>::type WriteDocument(const OutputStreamRef& stream, const Id& id, const T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
...
template <typename T> inline typename std::enable_if<GetCollectionKind<T>::value != COLLECTION_KIND::ARRAY && !STD_IS_REPLACEMENT(same, T, Data), Result<void>>::type WriteDocument(const OutputStreamRef& stream, const Id& id, const T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
