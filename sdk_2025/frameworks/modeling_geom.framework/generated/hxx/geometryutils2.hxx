...
struct DegeneratedInputErrorInterface::MTable
{
	...
	template <typename S> using CombinedMTable = maxon::PrivateCombinedMTable<DegeneratedInputErrorInterface, typename ErrorInterface::MTable::template CombinedMTable<S>>;
...
struct GeometryUtilsInterface::Hxx2
{
	...
	public:
		...
		static Float GeometryUtilsInterface_LinearRemapToRange(Float value, Float from1, Float to1, Float from2, Float to2) { return S::LinearRemapToRange(value, from1, to1, from2, to2); }
		static Float GeometryUtilsInterface_GetAngle2D(const Vector2d& direction1, const Vector2d& direction2) { return S::GetAngle2D(direction1, direction2); }
		static Float GeometryUtilsInterface_InterpolatePointOnSegment(const Vector& segmentPoint1, const Vector& segmentPoint2, const Vector& point, Bool clamp) { return S::InterpolatePointOnSegment(segmentPoint1, segmentPoint2, point, clamp); }
		static Float GeometryUtilsInterface_InterpolatePointOnSegment2D(const Vector2d& segmentPoint1, const Vector2d& segmentPoint2, const Vector2d& point, Bool clamp) { return S::InterpolatePointOnSegment2D(segmentPoint1, segmentPoint2, point, clamp); }
		static Bool GeometryUtilsInterface_CalculateBestFitPlane(const Block<const Vector>& pointCloud, Vector& centroid, Vector& normal, Int iterMax) { return S::CalculateBestFitPlane(pointCloud, centroid, normal, iterMax); }
		static Result<void> GeometryUtilsInterface_Points3Dto2D(const Block<const Vector>& points, const Vector& normal, Vector& basisVector0, Vector& basisVector1, Vector& translate, Bool checkDegenerated, WritableArrayInterface<Vector2d>& outPoints, Bool computeBasis) { return S::Points3Dto2D(points, normal, basisVector0, basisVector1, translate, checkDegenerated, outPoints, computeBasis); }
		static Result<void> GeometryUtilsInterface_Points3Dto2D_1(const Block<const Vector>& points, const Vector& normal, Bool checkDegenerated, WritableArrayInterface<Vector2d>& outPoints) { return S::Points3Dto2D(points, normal, checkDegenerated, outPoints); }
		static Vector GeometryUtilsInterface_Point2Dto3D(const Vector2d& point, const Vector& basisVector0, const Vector& basisVector1, const Vector& translate) { return S::Point2Dto3D(point, basisVector0, basisVector1, translate); }
		static Result<void> GeometryUtilsInterface_Points2Dto3D(const Block<const Vector2d>& points, const Vector& basisVector0, const Vector& basisVector1, const Vector& translate, WritableArrayInterface<Vector>& outPoints) { return S::Points2Dto3D(points, basisVector0, basisVector1, translate, outPoints); }
		static Float GeometryUtilsInterface_CalculateTriangleArea(const Vector& a, const Vector& b, const Vector& c) { return S::CalculateTriangleArea(a, b, c); }
		static Float GeometryUtilsInterface_CalculateSignedTriangleArea(const Vector& a, const Vector& b, const Vector& c) { return S::CalculateSignedTriangleArea(a, b, c); }
		static Float GeometryUtilsInterface_CalculateQuadrangleArea(const Vector& a, const Vector& b, const Vector& c, const Vector& d) { return S::CalculateQuadrangleArea(a, b, c, d); }
		static Float GeometryUtilsInterface_CalculateOutlineArea(const Block<const Vector>& outlinePoints) { return S::CalculateOutlineArea(outlinePoints); }
		static Vector GeometryUtilsInterface_CalculateOutlineNormal(const Block<const Vector>& points, const Block<const Int32>& indices, Bool normalized) { return S::CalculateOutlineNormal(points, indices, normalized); }
		static Vector GeometryUtilsInterface_CalculateOutlineNormal_1(const Block<const Vector>& points, Bool normalize) { return S::CalculateOutlineNormal(points, normalize); }
		static Bool GeometryUtilsInterface_IsVertexConvex(const Vector& vertex, const Vector& next, const Vector& pred, const Vector& up) { return S::IsVertexConvex(vertex, next, pred, up); }
		static GEOMETRY_TEST_EXACT GeometryUtilsInterface_IsVertexConvexExact(const Vector& vertex, const Vector& next, const Vector& pred, const Vector& up, Float epsilon) { return S::IsVertexConvexExact(vertex, next, pred, up, epsilon); }
		static Result<void> GeometryUtilsInterface_CalculateMeanValueCoordinatesWeights(const Vector& point, const Block<const Vector>& outline, WritableArrayInterface<Float>& weights) { return S::CalculateMeanValueCoordinatesWeights(point, outline, weights); }
		static Result<void> GeometryUtilsInterface_CalculateMeanValueCoordinatesWeights_1(const Vector& point, const Block<const Vector>& points, const Block<const UInt32>& outlineIndices, const Vector& normal, WritableArrayInterface<Float>& weights) { return S::CalculateMeanValueCoordinatesWeights(point, points, outlineIndices, normal, weights); }
		static Bool GeometryUtilsInterface_IsVertexConvex2D(const Vector2d& vertex, const Vector2d& next, const Vector2d& pred) { return S::IsVertexConvex2D(vertex, next, pred); }
		static GEOMETRY_TEST_EXACT GeometryUtilsInterface_IsVertexConvexExact2D(const Vector2d& vertex, const Vector2d& next, const Vector2d& pred, Float epsilon) { return S::IsVertexConvexExact2D(vertex, next, pred, epsilon); }
		static Vector2d GeometryUtilsInterface_CalculateBarycentricCoordinate(const Vector& point, const Vector& a, const Vector& b, const Vector& c) { return S::CalculateBarycentricCoordinate(point, a, b, c); }
		static Vector GeometryUtilsInterface_CalculateBarycentricCoordinate3D(const Vector& point, const Vector& a, const Vector& b, const Vector& c) { return S::CalculateBarycentricCoordinate3D(point, a, b, c); }
		static Bool GeometryUtilsInterface_PointInTriangle(const Vector& point, const Vector& a, const Vector& b, const Vector& c) { return S::PointInTriangle(point, a, b, c); }
		static Bool GeometryUtilsInterface_PointInTriangle2D(const Vector2d& point, const Vector2d& a, const Vector2d& b, const Vector2d& c) { return S::PointInTriangle2D(point, a, b, c); }
		static Int GeometryUtilsInterface_GetPointInPolygonWindingNumber2D(Vector2d point, const Block<const Vector2d>& outline) { return S::GetPointInPolygonWindingNumber2D(std::forward<Vector2d>(point), outline); }
		static Int GeometryUtilsInterface_GetPointInPolygonWindingNumber2DExact(Vector2d point, const Block<const Vector2d>& outline) { return S::GetPointInPolygonWindingNumber2DExact(std::forward<Vector2d>(point), outline); }
		static Int GeometryUtilsInterface_GetPointInPolygonWindingNumber2D_1(Vector2d point, const Block<const Vector2d>& outline, Int& edgeHit) { return S::GetPointInPolygonWindingNumber2D(std::forward<Vector2d>(point), outline, edgeHit); }
		static Int GeometryUtilsInterface_GetPointInPolygonWindingNumber(Vector point, const Block<const Vector>& outline) { return S::GetPointInPolygonWindingNumber(std::forward<Vector>(point), outline); }
		static Int GeometryUtilsInterface_GetPointInPolygonWindingNumber_1(Vector point, const Block<const Vector>& outline, Int& edgeHit) { return S::GetPointInPolygonWindingNumber(std::forward<Vector>(point), outline, edgeHit); }
		static Bool GeometryUtilsInterface_PointInOutline2D(const Vector& point, const Block<const Vector2d>& outline) { return S::PointInOutline2D(point, outline); }
		static Bool GeometryUtilsInterface_PointInOutlineRobust2D(const Vector& point, const Block<const Vector2d>& outline) { return S::PointInOutlineRobust2D(point, outline); }
		static Vector GeometryUtilsInterface_CalculateBisector(const Vector& v1, const Vector& v2, const Vector& up, Float w1, Float w2) { return S::CalculateBisector(v1, v2, up, w1, w2); }
		static Bool GeometryUtilsInterface_CalculateCircumcenter2D(const Vector2d& a, const Vector2d& b, const Vector2d& c, Vector2d& center, Float& sqrRad, Float tolerance) { return S::CalculateCircumcenter2D(a, b, c, center, sqrRad, tolerance); }
		static Bool GeometryUtilsInterface_CalculateCircumcenter(const Vector& a, const Vector& b, const Vector& c, Vector& center, Float& rad, Float tolerance) { return S::CalculateCircumcenter(a, b, c, center, rad, tolerance); }
		static Bool GeometryUtilsInterface_SegmentsIntersectionTest2D(const Vector2d& segment1Point1, const Vector2d& segment1Point2, const Vector2d& segment2Point1, const Vector2d& segment2Point2) { return S::SegmentsIntersectionTest2D(segment1Point1, segment1Point2, segment2Point1, segment2Point2); }
		static Bool GeometryUtilsInterface_SegmentRayIntersectionTest2D(const Vector2d& segmentPoint1, const Vector2d& segmentPoint2, const Vector2d& rayStartPoint, const Vector2d& rayDirection) { return S::SegmentRayIntersectionTest2D(segmentPoint1, segmentPoint2, rayStartPoint, rayDirection); }
		static Bool GeometryUtilsInterface_IntersectSegments2D(const Vector2d& segment1Point1, const Vector2d& segment1Point2, const Vector2d& segment2Point1, const Vector2d& segment2Point2, Vector2d& intersectionPoint, Float tolerance) { return S::IntersectSegments2D(segment1Point1, segment1Point2, segment2Point1, segment2Point2, intersectionPoint, tolerance); }
		static Vector GeometryUtilsInterface_LineLineDistance(const Vector& line1Point, const Vector& line1Dir, const Vector& line2Point, const Vector& line2Dir, Bool& parallel, Float& param1, Float& param2, Float tolerance) { return S::LineLineDistance(line1Point, line1Dir, line2Point, line2Dir, parallel, param1, param2, tolerance); }
		static Bool GeometryUtilsInterface_PointInSegment2D(const Vector2d& point, const Vector2d& segmentPoint1, const Vector2d& segmentPoint2, Float tolerance) { return S::PointInSegment2D(point, segmentPoint1, segmentPoint2, tolerance); }
		static Bool GeometryUtilsInterface_PointInSegment(const Vector& point, const Vector& segmentPoint1, const Vector& segmentPoint2, Float tolerance) { return S::PointInSegment(point, segmentPoint1, segmentPoint2, tolerance); }
		static Bool GeometryUtilsInterface_PointInHalfLine(const Vector& point, const Vector& halfLineOrigin, const Vector& halfLineDir, Float tolerance) { return S::PointInHalfLine(point, halfLineOrigin, halfLineDir, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLines(const Vector& line1Point, const Vector& line1Dir, const Vector& line2Point, const Vector& line2Dir, Vector& intersectionPoint, Float tolerance) { return S::IntersectLines(line1Point, line1Dir, line2Point, line2Dir, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLineSegment(const Vector& linePoint, const Vector& lineDir, const Vector& segmentPoint1, const Vector& segmentPoint2, Vector& intersectionPoint, Float tolerance) { return S::IntersectLineSegment(linePoint, lineDir, segmentPoint1, segmentPoint2, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectSegments(const Vector& segment1Point1, const Vector& segment1Point2, const Vector& segment2Point1, const Vector& segment2Point2, Vector& intersectionPoint, Float tolerance) { return S::IntersectSegments(segment1Point1, segment1Point2, segment2Point1, segment2Point2, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectHalfLines(const Vector& halfLine1Origin, const Vector& halfLine1Dir, const Vector& halfLine2Origin, const Vector& halfLine2Dir, Vector& intersectionPoint, Float tolerance) { return S::IntersectHalfLines(halfLine1Origin, halfLine1Dir, halfLine2Origin, halfLine2Dir, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLineHalfLine(const Vector& linePoint, const Vector& lineDir, const Vector& halfLineOrigin, const Vector& halfLineDir, Vector& intersectionPoint, Float tolerance) { return S::IntersectLineHalfLine(linePoint, lineDir, halfLineOrigin, halfLineDir, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectHalfLineSegment(const Vector& halfLineOrigin, const Vector& halfLineDir, const Vector& segmentPoint1, const Vector& segmentPoint2, Vector& intersectionPoint, Float tolerance) { return S::IntersectHalfLineSegment(halfLineOrigin, halfLineDir, segmentPoint1, segmentPoint2, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectPlanes(const Vector& plane1Point, const Vector& plane1Normal, const Vector& plane2Point, const Vector& plane2Normal, Vector& resLinePoint, Vector& resLineDirection, Float tolerance) { return S::IntersectPlanes(plane1Point, plane1Normal, plane2Point, plane2Normal, resLinePoint, resLineDirection, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLinePlane(const Vector& linePoint, const Vector& lineDir, const Vector& planePoint, const Vector& planeNormal, Vector& intersectionPoint, Float tolerance) { return S::IntersectLinePlane(linePoint, lineDir, planePoint, planeNormal, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLinePlane_1(const Vector& linePoint, const Vector& lineDir, const Vector& planePoint, const Vector& planeNormal, Vector& intersectionPoint, Float& param, Bool& samePlane, Float tolerance) { return S::IntersectLinePlane(linePoint, lineDir, planePoint, planeNormal, intersectionPoint, param, samePlane, tolerance); }
		static Bool GeometryUtilsInterface_IntersectSegmentPlane(const Vector& segmentPoint1, const Vector& segmentPoint2, const Vector& planePoint, const Vector& planeNormal, Vector& intersectionPoint, Float tolerance) { return S::IntersectSegmentPlane(segmentPoint1, segmentPoint2, planePoint, planeNormal, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectHalfLinePlane(const Vector& halfLinePoint, const Vector& halfLineDir, const Vector& planePoint, const Vector& planeNormal, Vector& intersectionPoint, Float tolerance) { return S::IntersectHalfLinePlane(halfLinePoint, halfLineDir, planePoint, planeNormal, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLineTriangle(const Vector& linePoint, const Vector& lineDir, const Vector& a, const Vector& b, const Vector& c, Vector& intersectionPoint, Vector2d* barycCoords, Float tolerance) { return S::IntersectLineTriangle(linePoint, lineDir, a, b, c, intersectionPoint, barycCoords, tolerance); }
		static Bool GeometryUtilsInterface_IntersectHalfLineTriangle(const Vector& halfLineOrigin, const Vector& halfLineDir, const Vector& a, const Vector& b, const Vector& c, Vector& intersectionPoint, Vector2d* barycCoords, Float tolerance) { return S::IntersectHalfLineTriangle(halfLineOrigin, halfLineDir, a, b, c, intersectionPoint, barycCoords, tolerance); }
		static Bool GeometryUtilsInterface_IntersectLineQuadrangle(const Vector& linePoint, const Vector& lineDir, const Vector& a, const Vector& b, const Vector& c, const Vector& d, Vector& intersectionPoint, Float tolerance) { return S::IntersectLineQuadrangle(linePoint, lineDir, a, b, c, d, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectHalfLineQuadrangle(const Vector& halfLineOrigin, const Vector& halfLineDir, const Vector& a, const Vector& b, const Vector& c, const Vector& d, Vector& intersectionPoint, Float tolerance) { return S::IntersectHalfLineQuadrangle(halfLineOrigin, halfLineDir, a, b, c, d, intersectionPoint, tolerance); }
		static Bool GeometryUtilsInterface_IntersectSegmentTriangle(const Vector& segmentPoint1, const Vector& segmentPoint2, const Vector& a, const Vector& b, const Vector& c, Vector& intersectionPoint, Vector* barycCoords, Float tolerance) { return S::IntersectSegmentTriangle(segmentPoint1, segmentPoint2, a, b, c, intersectionPoint, barycCoords, tolerance); }
		static Bool GeometryUtilsInterface_TrianglesIntersectionTest(const Vector& t1a, const Vector& t1b, const Vector& t1c, const Vector& t2a, const Vector& t2b, const Vector& t2c, Float tolerance) { return S::TrianglesIntersectionTest(t1a, t1b, t1c, t2a, t2b, t2c, tolerance); }
		static Bool GeometryUtilsInterface_CalculatePolygonPointST(const Vector& point, const Vector& pa, const Vector& pb, const Vector& pc, const Vector& pd, Bool isQuad, Float& s, Float& t, Float tolerance) { return S::CalculatePolygonPointST(point, pa, pb, pc, pd, isQuad, s, t, tolerance); }
		static void GeometryUtilsInterface_InverseBilinarInterpolate(const Vector& pa, const Vector& pb, const Vector& pc, const Vector& pd, const Vector& point, Vector2d& bilinarVariables) { return S::InverseBilinarInterpolate(pa, pb, pc, pd, point, bilinarVariables); }
		static void GeometryUtilsInterface_InverseBilinarInterpolateNewton(const Vector& pa, const Vector& pb, const Vector& pc, const Vector& pd, const Vector& point, Int32 iterations, Vector2d& bilinarVariables) { return S::InverseBilinarInterpolateNewton(pa, pb, pc, pd, point, iterations, bilinarVariables); }
		static void GeometryUtilsInterface_InverseBilinarInterpolate2D(const Vector2d& pa, const Vector2d& pb, const Vector2d& pc, const Vector2d& pd, const Vector2d& point, Vector2d& bilinarVariables) { return S::InverseBilinarInterpolate2D(pa, pb, pc, pd, point, bilinarVariables); }
		static void GeometryUtilsInterface_InverseBilinarInterpolate2DNewton(const Vector2d& pa, const Vector2d& pb, const Vector2d& pc, const Vector2d& pd, const Vector2d& point, Int32 iterations, Vector2d& bilinarVariables) { return S::InverseBilinarInterpolate2DNewton(pa, pb, pc, pd, point, iterations, bilinarVariables); }
		static Vector GeometryUtilsInterface_BilinearInterpolate(const Vector& pa, const Vector& pb, const Vector& pc, const Vector& pd, const Float s, const Float t) { return S::BilinearInterpolate(pa, pb, pc, pd, s, t); }
		static Float GeometryUtilsInterface_CalculateSignedTriangleArea_1(const Vector& a, const Vector& b, const Vector& c, const Vector& normal) { return S::CalculateSignedTriangleArea(a, b, c, normal); }
		static Result<void> GeometryUtilsInterface_CalculateMeanValueCoordinatesWeights_2(const Vector& point, const Block<const Vector>& outline, const Vector& normal, WritableArrayInterface<Float>& weights) { return S::CalculateMeanValueCoordinatesWeights(point, outline, normal, weights); }
		static Float GeometryUtilsInterface_CalculatePerimeterLength2D(const Block<const Vector2d>& outlinePoints) { return S::CalculatePerimeterLength2D(outlinePoints); }
		static Result<Bool> GeometryUtilsInterface_IsWoundClockwise2D(const Block<const Vector2d>& outlinePoints) { return S::IsWoundClockwise2D(outlinePoints); }
		static Vector2d GeometryUtilsInterface_CalculateOutlineCentroidAndArea2D(const Block<const Vector2d>& outlinePoints, Float* area, Bool* windingPolarity) { return S::CalculateOutlineCentroidAndArea2D(outlinePoints, area, windingPolarity); }
		static Result<Vector2d> GeometryUtilsInterface_CalculateMultiOutlineCentroidAndAreas2D(const Block<const Vector2d>& loopVertices, const Block<const Int>& loopVertexCounts, Bool includeHolesInCogCalcs, Float* overallArea, BaseArray<Float>* loopSoloAreas, BaseArray<Bool>* windingPolarity) { return S::CalculateMultiOutlineCentroidAndAreas2D(loopVertices, loopVertexCounts, includeHolesInCogCalcs, overallArea, loopSoloAreas, windingPolarity); }
		static Result<BaseArray<Int>> GeometryUtilsInterface_CalculateConvexHull2D(const Block<const Vector2d>& outlinePoints) { return S::CalculateConvexHull2D(outlinePoints); }
		static Result<BaseArray<BaseArray<Int>>> GeometryUtilsInterface_CalculateConvexDecomposition2D(const Block<const Vector2d>& outlinePoints) { return S::CalculateConvexDecomposition2D(outlinePoints); }
		static Result<Bool> GeometryUtilsInterface_CleanLoop2DInPlace(BaseArray<Vector2d>& outlinePoints, Bool maintainStart) { return S::CleanLoop2DInPlace(outlinePoints, maintainStart); }
		static Result<BaseArray<Vector2d>> GeometryUtilsInterface_CalculateNoFitPolygonForConvex(const Block<const Vector2d>& outlinePointsA, const Block<const Vector2d>& outlinePointsB, Bool relativeBToA) { return S::CalculateNoFitPolygonForConvex(outlinePointsA, outlinePointsB, relativeBToA); }
		static Result<BaseArray<Vector2d>> GeometryUtilsInterface_CalculateNoFitPolygonUsingSliding(const Block<const Vector2d>& outlinePointsA, const Block<const Vector2d>& outlinePointsB, Bool relativeBToA) { return S::CalculateNoFitPolygonUsingSliding(outlinePointsA, outlinePointsB, relativeBToA); }
		static Result<BaseArray<Vector2d>> GeometryUtilsInterface_CalculateInnerFitPolygonUsingSliding(const Block<const Vector2d>& outlinePointsCcwA, const Block<const Vector2d>& outlinePointsB) { return S::CalculateInnerFitPolygonUsingSliding(outlinePointsCcwA, outlinePointsB); }
		static Result<BaseArray<Vector2d>> GeometryUtilsInterface_CalculateNoFitPolygonForConvex_1(const Block<const Vector2d>& outlinePointsA, const Block<const Vector2d>& outlinePointsB, const Matrix2d& transformA, const Matrix2d& transformB, Bool relativeBToA) { return S::CalculateNoFitPolygonForConvex(outlinePointsA, outlinePointsB, transformA, transformB, relativeBToA); }
		static Result<BaseArray<Vector2d>> GeometryUtilsInterface_CalculateNoFitPolygonUsingSliding_1(const Block<const Vector2d>& outlinePointsA, const Block<const Vector2d>& outlinePointsB, const Matrix2d& transformA, const Matrix2d& transformB, Bool relativeBToA) { return S::CalculateNoFitPolygonUsingSliding(outlinePointsA, outlinePointsB, transformA, transformB, relativeBToA); }
		static Result<BaseArray<BaseArray<Vector2d>>> GeometryUtilsInterface_CalculateUnionOfPolygons(const Block<const Vector2d>& outlinePointsA, const Block<const Vector2d>& outlinePointsB, Bool includeHolesInResult, Bool createOuterLoop) { return S::CalculateUnionOfPolygons(outlinePointsA, outlinePointsB, includeHolesInResult, createOuterLoop); }
		static Bool GeometryUtilsInterface_IntersectsSegmentsExact2D(const Vector2d& segment1Point1, const Vector2d& segment1Point2, const Vector2d& segment2Point1, const Vector2d& segment2Point2, Vector2d& intersectionPoint, Bool& areParallel, Vector2d* intersectionPoint2, Float tolerance) { return S::IntersectsSegmentsExact2D(segment1Point1, segment1Point2, segment2Point1, segment2Point2, intersectionPoint, areParallel, intersectionPoint2, tolerance); }
		static LOOP_OVERLAP_STATE GeometryUtilsInterface_GetContainmentStateForNonIntersectingLoops(const Block<const Vector2d>& outlinePointsA, const Block<const Vector2d>& outlinePointsB) { return S::GetContainmentStateForNonIntersectingLoops(outlinePointsA, outlinePointsB); }
		static Bool GeometryUtilsInterface_AreLoopsEqualToWithinTolerance(const Block<const Vector2d>& loopA, const Block<const Vector2d>& loopB, Bool allowCyclicRotation, Bool ignoreTranslationOffset, maxon::Float tol) { return S::AreLoopsEqualToWithinTolerance(loopA, loopB, allowCyclicRotation, ignoreTranslationOffset, tol); }
		static void GeometryUtilsInterface_CalculateBoundingBoxIndices(const Block<const Vector2d>& verts, Int& minIdxX, Int& maxIdxX, Int& minIdxY, Int& maxIdxY, Bool findMostCounterClockwise) { return S::CalculateBoundingBoxIndices(verts, minIdxX, maxIdxX, minIdxY, maxIdxY, findMostCounterClockwise); }
		static Bool GeometryUtilsInterface_ArePointsCoLinearWithinTolerance(const Block<const Vector>& points, maxon::Float tol) { return S::ArePointsCoLinearWithinTolerance(points, tol); }
	};
