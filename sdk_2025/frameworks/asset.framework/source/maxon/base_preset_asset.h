...
public:
	...
	/// @see AssetCreationInterface::SetDefaultObject.
...
public:
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> ConvertFromLegacyBrowser(Int32 pluginId, const Block<const Char>& memBlock, const cinema::BaseContainer& settings, const String& name, DataDictionary& metaProperties, AddAssetMetaData& additionalMetaData, AddAssetDepencendyStruct& addDependencyStruct, ResolveAssetDependenciesStruct& resolveAssets);
	...
	//----------------------------------------------------------------------------------------
	/// Reads the content of a preset asset. DataType and content will depend on the preset type and implementation.
	/// Returned typed might differ from GetPresetInternalData.
	/// @return												The preset's exposed data. Valid only as long as the loaded preset is in scope.
	//----------------------------------------------------------------------------------------
	...
	/// @return												The preset's exposed data (copied).
	//----------------------------------------------------------------------------------------
...
/// Internal identifiers used for PresetLoadArgs/PresetSaveArgs. The attributes use by each preset asset type depend on the implementation.
namespace presetargs
{

MAXON_ATTRIBUTE(Bool, Partial, "net.maxon.asset.preset.args.partial");
MAXON_ATTRIBUTE(void*, ClassicSubType, "net.maxon.asset.preset.args.classicsubtype");									///< Pointer to const Block<const Int32>.
MAXON_ATTRIBUTE(Array<Id>, AssetSubType, "net.maxon.asset.preset.args.assetsubtype");
MAXON_ATTRIBUTE(void*, DefaultsBaseContainer, "net.maxon.asset.preset.args.classicdefaultsdata");			///< Pointer to BaseContainer.
...
{
	MAXON_DECLARATION(BasePresetAssetType, CameraSensorPreset, "net.maxon.assettype.preset.camerasensor");
