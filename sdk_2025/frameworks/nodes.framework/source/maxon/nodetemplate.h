...
public:
	...
	enum class KIND : UChar
	{
		...
		// FLAGS
		DEDUCED   = 1 << 0,	///< This flag is set internally (in combination with the other flags) if the value has been deduced from the node graph.
		EXPLICIT  = 1 << 4,	///< This flag is used in combination with the other flags if the value has been explicitly set by code and shall not be deduced or modified by the user.

		// KIND TYPES
		VALUE     = 1 << 1,	///< The template argument is for a port which is marked as TemplateParameter.
		TYPE      = 1 << 2,	///< The template argument is for a port which is marked as TypeTemplateParameter.
		BUNDLE    = 1 << 3,	///< The template argument is for a port which is marked as BundleTemplateParameter.
		VALUEMODE = 1 << 5,	///< The template argument is for a port which is marked as ValueModeTemplateParameter.

		// MASKS
		MASK = UChar(~(DEDUCED | EXPLICIT)),	///< Anything but DEDUCED or EXPLICIT.
	...
	//----------------------------------------------------------------------------------------
	/// Returns the value for the value mode template parameter identified by the given path.
	/// @param[in] path								The path of the template parameter.
	/// @return												The argument for the parameter.
	//----------------------------------------------------------------------------------------
	Opt<const VALUEMODE&> GetValueModeArgument(const NodePath& path) const
	{
		const Data* d = GetImpl(path, KIND::VALUEMODE);
		return d ? d->GetPtr<VALUEMODE>() : nullptr;
	}
...
{
	MAXON_INTERFACE(NodeTemplateInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.node.interface.nodetemplate", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
public:
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Tells if this node template is a group base. Group bases may have some ports
	/// but are otherwise empty or have internal nodes only (e.g. the loop carried value template).
	/// When a group base is added in the node editor it isn't added directly
	/// but as an editable instance of the group base.
	///
	/// @return												True if this node template is a group base, false otherwise.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Bool IsGroupBase() const;
...
{
	MAXON_INTERFACE(NodeSystemBasedNodeTemplateInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.node.interface.nodesystembasednodetemplate", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
public:
	//----------------------------------------------------------------------------------------
