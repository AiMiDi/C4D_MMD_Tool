...
public:
	...
	/// @param[in] convertible				True (the default) if template instantiations shall be user-convertible to editable groups.
	/// @return												A NodeSystemBasedNodeTemplate based on #templ.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<NodeSystemBasedNodeTemplate> CreateTemplateFromNodeSystem(const NodeSystem& templ, const AssetRepositoryRef& lookup, Bool convertible = true);
