...
{
	MAXON_INTERFACE(NodeSystemManagerInterface, MAXON_REFERENCE_NORMAL, "net.maxon.node.interface.nodesystemmanager", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
...
public:
	...
	/// @private
	MAXON_METHOD Result<void> Update(const Block<const UpdateAssetEntry>& updates, const ValueReceiver<const UpdateAssetEntry&>& updatedRecv);
...
{
	MAXON_INTERFACE(NodesGraphModelInterface, MAXON_REFERENCE_NORMAL, "net.maxon.node.interface.nodesgraphmodel", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
...
public:
	...
	/// @return												A GraphNode referencing the added child.
	//----------------------------------------------------------------------------------------
	MAXON_FUNCTION Result<GraphNode> AddChild(const Id& childId, const NodeTemplate& templ, const TemplateArguments& args = GetZeroRef<TemplateArguments>());
	...
	//----------------------------------------------------------------------------------------
	/// @copydoc MoveGroupToAsset
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<AssetDescription> MoveToAsset(const GraphNode& node, const AssetRepositoryRef& repository, const Id& assetId, const Delegate<Result<void>(const MutableRoot& rootNode)>& assetFinalizer);
	...
	/// @param[in] overwrite					True to overwrite last version of the asset with identifier #assetId, false to create a new version.
	/// @param[in] assetFinalizer			This Delegate is called right before the asset is stored.
	/// 															This allows to do some finalization on the node system.
	/// @return												The AssetDescription for the new asset.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<AssetDescription> MoveGroupToAsset(const GraphNode& node, const AssetRepositoryRef& repository, const Id& assetId, Bool overwrite, const Delegate<Result<void>(const MutableRoot& rootNode)>& assetFinalizer);

	//----------------------------------------------------------------------------------------
	/// Changes a NodeTemplate instance into an editable group.
	/// @param[in] node								The node which shall be made an editable group.
	/// @param[in] groupFinalizer			This Delegate is called right before the editable group replaces the original node.
	/// 															This allows to do some finalization on the node system.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> ChangeAssetToEditableGroup(const GraphNode& node, const Delegate<Result<void>(const MutableRoot& rootNode)>& groupFinalizer);
