...
struct ObservableBaseInterface::MTable
{
	...
	Result<void> (*_ObservableBaseInterface_AddObserver) (maxon::GenericComponent* this_, ObserverObjectInterface* observer, const FunctionBaseRef& observerFunction, Int priority);
...
struct ObservableBaseInterface::Hxx2
{
	...
	public:
		...
		static Result<void> _ObservableBaseInterface_AddObserver(maxon::GenericComponent* this_, ObserverObjectInterface* observer, const FunctionBaseRef& observerFunction, Int priority) { return ((typename S::Implementation*) this_)->AddObserver(observer, observerFunction, priority); }
...
{
	const ObservableBaseInterface::MTable& mt_ = PRIVATE_MAXON_VTABLE(ObservableBaseInterface, this); return mt_._ObservableBaseInterface_AddObserver((maxon::GenericComponent*) this + mt_._ObservableBaseInterface_AddObserver_Offset, observer, observerFunction, priority);
}
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ObservableBaseInterface::Hxx1::ReferenceFunctionsImpl<S>::AddObserver(ObserverObjectInterface* observer, const FunctionBaseRef& observerFunction, Int priority) const -> Result<void>
{
	...
	return (mt_._ObservableBaseInterface_AddObserver(reinterpret_cast<maxon::GenericComponent*>(o_) + mt_._ObservableBaseInterface_AddObserver_Offset, observer, observerFunction, priority));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto ObservableBaseInterface::Hxx1::COWReferenceFunctionsImpl<S>::AddObserver(ObserverObjectInterface* observer, const FunctionBaseRef& observerFunction, Int priority) -> Result<void>
{
...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); ObservableBaseInterface* o_ = (ObservableBaseInterface*) this->GetPointer(); if (!(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR)) && MAXON_UNLIKELY(!o_)) { DebugStop(); return maxon::PrivateLogNullptrError(); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(ObservableBaseInterface, o_, ObservableBaseInterface_SetInverseNotification) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<ObservableBaseInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<ObservableBaseInterface>() : PRIVATE_MAXON_VTABLE(ObservableBaseInterface, o_); 
