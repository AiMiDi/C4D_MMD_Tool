...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); UserLoggerTypeInterface* o_ = (UserLoggerTypeInterface*) this->GetPointer(); if (!(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR)) && MAXON_UNLIKELY(!o_)) { DebugStop(); return maxon::PrivateLogNullptrError(); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(UserLoggerTypeInterface, o_, UserLoggerTypeInterface_Reset) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<UserLoggerTypeInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<UserLoggerTypeInterface>() : PRIVATE_MAXON_VTABLE(UserLoggerTypeInterface, o_); 
...
struct LoggerInterface::Hxx2
{
	...
	public:
		...
		static Result<void> _LoggerInterface_AddLoggerType(LoggerInterface* this_, TARGETAUDIENCE ta, const Class<LoggerTypeRef>& loggerTypeCls, const LOGGERTYPEINITCALLBACK& cb) { return C::Get(this_)->AddLoggerType(ta, loggerTypeCls, cb); }
		static Result<void> _LoggerInterface_RemoveLoggerType(LoggerInterface* this_, LoggerTypeRef& loggerType) { return C::Get(this_)->RemoveLoggerType(loggerType); }
		static Result<void> _LoggerInterface_RemoveLoggerTypes(LoggerInterface* this_, const Class<LoggerTypeRef>& loggerTypeCls) { return C::Get(this_)->RemoveLoggerTypes(loggerTypeCls); }
		static Result<void> _LoggerInterface_Write(LoggerInterface* this_, TARGETAUDIENCE ta, const String& str, const maxon::SourceLocation& loc, WRITEMETA level) { return C::Get(this_)->Write(ta, str, loc, level); }
		static String _LoggerInterface_GetName(const LoggerInterface* this_) { return C::Get(this_)->GetName(); }
		static void _LoggerInterface_SetName(LoggerInterface* this_, const String& name) { return C::Get(this_)->SetName(name); }
		static void _LoggerInterface_Enable(LoggerInterface* this_, Bool enable) { return C::Get(this_)->Enable(enable); }
		static Bool _LoggerInterface_IsEnabled(LoggerInterface* this_) { return C::Get(this_)->IsEnabled(); }
		static BaseArray<Pair<LoggerTypeRef, TARGETAUDIENCE>> _LoggerInterface_GetLoggerTypes(const LoggerInterface* this_) { return C::Get(this_)->GetLoggerTypes(); }
