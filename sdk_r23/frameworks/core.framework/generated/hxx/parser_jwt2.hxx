...
#if 1
constexpr const maxon::Char* JwtParserInterface::PrivateGetCppName() { return "maxon::JwtParserRef"; }
...
struct JwtParserInterface::Hxx2
{
	...
	public:
		...
		static Result<void> _JwtParserInterface_Read(JwtParserInterface* this_, UrlOrInputStream&& nameOrStream, JWTPARSERFLAGS flags, const StringDecodingRef& stringDecoding, const Block<const Char>& publicKey, const ValueReceiver<const DataDictionary&>& jsonObjects, BaseArray<Char>* originalToken) { return C::Get(this_)->Read(std::forward<UrlOrInputStream>(nameOrStream), flags, stringDecoding, publicKey, jsonObjects, originalToken); }
		static Result<void> _JwtParserInterface_Read_1(JwtParserInterface* this_, const Block<const Char>& jwtString, JWTPARSERFLAGS flags, const StringDecodingRef& stringDecoding, const Block<const Char>& publicKey, const ValueReceiver<const DataDictionary&>& jsonObjects) { return C::Get(this_)->Read(jwtString, flags, stringDecoding, publicKey, jsonObjects); }
		static Result<CString> _JwtParserInterface_Write(JwtParserInterface* this_, const DataDictionary& jsonObject, const StringEncodingRef& stringEncoding, const Block<const Char>& privateKey) { return C::Get(this_)->Write(jsonObject, stringEncoding, privateKey); }
	};
