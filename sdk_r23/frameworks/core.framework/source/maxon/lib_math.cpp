...
MATH_SQUAREMATRIX VectorToSquareMatrix(const MATH_VECTOR& dirVector, const MATH_VECTOR& upVector)
{
...
template SqrMat3<Vector32> VectorToSquareMatrix(const Vector32&, const Vector32&);
template SqrMat3<Vector64> VectorToSquareMatrix(const Vector64&, const Vector64&);

template <typename FLOAT>
MATH_SQUAREMATRIX CheckedVectorToSquareMatrix(const MATH_VECTOR& dirVector, const MATH_VECTOR& upVector)
{
	MATH_VECTOR checkedUp = upVector;
	if (CompareFloat(Abs(Dot(dirVector, checkedUp)), FLOAT(1.0), FLOAT(1.0e-3)))
	{
		const MATH_SQUAREMATRIX ROTATEX90(MATH_VECTOR(FLOAT(1.0), FLOAT(0.0), FLOAT(0.0)), MATH_VECTOR(FLOAT(0.0), FLOAT(0.0), FLOAT(-1.0)), MATH_VECTOR(FLOAT(0.0), FLOAT(1.0), FLOAT(0.0)));
		checkedUp = ROTATEX90 * checkedUp;
		if (CompareFloat(Abs(Dot(dirVector, checkedUp)), FLOAT(1.0), FLOAT(1.0e-3)))
		{
			const MATH_SQUAREMATRIX ROTATEY90(MATH_VECTOR(FLOAT(0.0), FLOAT(0.0), FLOAT(-1.0)), MATH_VECTOR(FLOAT(0.0), FLOAT(1.0), FLOAT(0.0)), MATH_VECTOR(FLOAT(1.0), FLOAT(0.0), FLOAT(0.0)));
			checkedUp = ROTATEY90 * checkedUp;
			if (CompareFloat(Abs(Dot(dirVector, checkedUp)), FLOAT(1.0), FLOAT(1.0e-3)))
			{
				const MATH_SQUAREMATRIX ROTATEZ90{MATH_VECTOR(FLOAT(0.0), FLOAT(-1.0), FLOAT(0.0)), MATH_VECTOR(FLOAT(1.0), FLOAT(0.0), FLOAT(0.0)), MATH_VECTOR(FLOAT(0.0), FLOAT(0.0), FLOAT(1.0))};
				checkedUp = ROTATEZ90 * checkedUp;
			}
		}
	}
	return VectorToSquareMatrix(dirVector, checkedUp);
}

template SqrMat3<Vector32> CheckedVectorToSquareMatrix(const Vector32&, const Vector32&);
template SqrMat3<Vector64> CheckedVectorToSquareMatrix(const Vector64&, const Vector64&);

template <typename FLOAT>
MATH_MATRIX VectorToMatrix(const MATH_VECTOR& dirVector, const MATH_VECTOR& upVector)
{
	return {MATH_VECTOR{}, VectorToSquareMatrix(dirVector, upVector)};
}

template Mat3<Vector32> VectorToMatrix(const Vector32&, const Vector32&);
template Mat3<Vector64> VectorToMatrix(const Vector64&, const Vector64&);
...
// Legacy code copied from c4d_tools.cpp.
template <typename FLOAT> MATH_VECTOR RGBToHSL(const MATH_COLOR& col)
{
	FLOAT r = col.r;
	FLOAT g = col.g;
	FLOAT b = col.b;
	...
	return MATH_VECTOR(h, s, l);
}

// create instantiation for Float32 and Float64
template Vector32 RGBToHSL(const Color32&);
template Vector64 RGBToHSL(const Color64&);

// @Private Helper for HSLToRGB. 
template <typename FLOAT>
static FLOAT HSLToRGB_value(FLOAT nl, FLOAT n2, FLOAT hue);

// Legacy code copied from c4d_tools.cpp.
template <typename FLOAT>
FLOAT HSLToRGB_value(FLOAT nl, FLOAT n2, FLOAT hue)
{
	if (hue > FLOAT(360.0))
		hue -= FLOAT(360.0);
	...
	if (hue < FLOAT(180.0))
		return n2;
	if (hue < FLOAT(240.0))
		return nl + (n2 - nl) * (FLOAT(240.0) - hue) / FLOAT(60.0);
	return nl;
}

// Legacy code copied from c4d_tools.cpp.
template <typename FLOAT> MATH_COLOR HSLToRGB(const MATH_VECTOR& col)
{
	FLOAT h = col.x * FLOAT(360.0);
	FLOAT s = col.y;
	FLOAT l = col.z;
...
// create instantiation for Float32 and Float64
template Color32 HSLToRGB(const Vector32&);
template Color64 HSLToRGB(const Vector64&);
...
Bool IsMatrixRectangular(const MATRIXTYPE& m, typename MATRIXTYPE::ValueType epsilon)
{
	return Dot(m.sqmat.v1, m.sqmat.v2) < epsilon && Dot(m.sqmat.v1, m.sqmat.v3) < epsilon && Dot(m.sqmat.v2, m.sqmat.v3) < epsilon;
}

template Bool IsMatrixRectangular(const Matrix32&, Float32);
template Bool IsMatrixRectangular(const Matrix64&, Float64);
...
template <typename FLOAT> MATH_VECTOR GetRotationAngles(const MATH_SQUAREMATRIX& m, ROTATIONORDER rotationOrder)
{
	...
	Vector64 v1 = !((Vector64) m.v1), v2 = !((Vector64) m.v2), v3 = !((Vector64) m.v3);
