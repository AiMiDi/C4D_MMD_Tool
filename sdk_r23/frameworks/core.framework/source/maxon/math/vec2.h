...
#endif
{
	...
	/// @return												*this.
	//----------------------------------------------------------------------------------------
	constexpr Vec2& operator =(const typename std::conditional<STRIDE == 1, DummyParamType, Vec2>::type& src)
	{
		x = src.x;
		y = src.y;
		return *this;
	...
	/// Adds two vectors
	template <typename T2, Int S2> constexpr Vec2& operator +=(const Vec2<T2, S2>& v)
	{
		x += v.x;
		y += v.y;
		return *this;
	...
	constexpr Vec2& operator *=(ValueTypeParam s)
	{
		x *= s;
		y *= s;
		return *this;
	...
	/// Multiplies each vector component by a scalar
	template <typename S> constexpr Vec2<typename MultiplicativePromotion<T, S, STD_IS_REPLACEMENT(scalar, S)>::type, 1> operator *(S s) const
	{
	...
	//----------------------------------------------------------------------------------------
	template <typename T2, Int S2> constexpr BoolType operator ==(const Vec2<T2, S2>& v) const
	{
		return x == v.x && y == v.y;
	}
	...
	constexpr BoolType LessThanOrEqual(const Vec2& v) const
 	{
 		return x <= v.x && y <= v.y;
 	}
	...
	// is not necessarily true.
	constexpr BoolType LessThan(const Vec2& v) const
	{
		return x < v.x && y < v.y;
	}
	...
	constexpr BoolType IsEqual(const Vec2& other, ValueTypeParam epsilon) const
	{
	...
	//----------------------------------------------------------------------------------------
	friend constexpr Unstrided Abs(const Vec2& v1)
	{
		return Unstrided(Abs(v1.x), Abs(v1.y));
	}

	/// Calculates dot product of v1 and v2
	template <typename T2, Int S2> friend constexpr typename MultiplicativePromotion<T, T2>::type Dot(const Vec2& v1, const Vec2<T2, S2>& v2)
	{
		return v1.x * v2.x + v1.y * v2.y;
	}
	...
	constexpr void ClampMin(const Vec2& other = Vec2())
	{
	...
	constexpr void ClampMax(const Vec2& other)
	{
	...
	/// Set the minimum of each component.
	friend constexpr void SetMin(Vec2& a, const Vec2& other)
	{
		a.ClampMax(other);
	}

	/// Set the maximum of each component.
	friend constexpr void SetMax(Vec2& a, const Vec2& other)
	{
		a.ClampMin(other);
	}
	...
	friend constexpr T GetAngle(const Vec2& v1, const Vec2& v2)
	{
	...
	template <typename T2> constexpr Vec2& operator *=(const Mat2<T2>& m)
	{
	...
	//----------------------------------------------------------------------------------------
	static constexpr const Vec2& NullValue()
	{
