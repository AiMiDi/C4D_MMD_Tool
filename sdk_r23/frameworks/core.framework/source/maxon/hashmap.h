...
public:
	...
	template <typename KEYHASH = HASH, typename KEY, typename C> Result<Entry*> InsertCtor(KEY&& key, C&& constructor, Bool& created = BoolLValue())
	{
		...
		HashInt hash = KEYHASH::GetHashCode(key);
	...
	public:
		static Bool IsEqual(const HashMap& a, const HashMap& b, VALUEHASH&& hash = VALUEHASH())
		{
			...
			for (Int i = 0; i < n; ++i)
			{
				for (const Entry* e = LoadAcquire((MODE != HASHMAP_MODE::DEFAULT ? a._table + i : a._nonemptyBuckets[i])->list); e; e = e->GetNextInBucket())
				{
		...
		static HashInt GetHashCode(const HashMap& m)
		{
			...
			for (Int i = 0; i < m._nonemptyBucketCount; ++i)
			{
				...
				{
					hash.CombineUnordered(DefaultHash::CombineHashes(e->GetHashCode(), VALUEHASH::GetHashCode(e->GetValue())));
				}
			}
			return hash;
		}
...
public:
	...
	HashInt GetHashCode() const
	{
		...
		for (Int i = 0; i < _nonemptyBucketCount; ++i)
		{
			...
			{
				hash.CombineUnordered(e->GetHashCode());
			}
		}
		return hash;
	}
