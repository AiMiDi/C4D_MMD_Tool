...
struct IoHandlerInterface::MTable
{
	...
	Result<String> (*_IoHandlerInterface_ConvertToUiName) (const maxon::GenericComponent* this_, const UrlInterface& url, CONVERTTOUINAMEFLAGS flags);
	maxon::Int _IoHandlerInterface_ConvertToUiName_Offset;
	...
	if (PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, IoHandlerInterface, ConvertToUiName))
	{
		auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, IoHandlerInterface, ConvertToUiName), W, MTable>::type::_IoHandlerInterface_ConvertToUiName;
		_IoHandlerInterface_ConvertToUiName = reinterpret_cast<const decltype(_IoHandlerInterface_ConvertToUiName)&>(ptr);
		_IoHandlerInterface_ConvertToUiName_Offset = offset;
	}
	}
...
struct IoHandlerInterface::Hxx2
{
	...
	public:
		...
		static Result<String> _IoHandlerInterface_ConvertToUiName(const maxon::GenericComponent* this_, const UrlInterface& url, CONVERTTOUINAMEFLAGS flags) { return ((const typename S::Implementation*) this_)->ConvertToUiName(url, flags); }
...
MAXON_ATTRIBUTE_FORCE_INLINE auto IoHandlerInterface::ConvertToUiName(const UrlInterface& url, CONVERTTOUINAMEFLAGS flags) const -> Result<String>
{
	const IoHandlerInterface::MTable& mt_ = PRIVATE_MAXON_VTABLE(IoHandlerInterface, this); return mt_._IoHandlerInterface_ConvertToUiName((const maxon::GenericComponent*) this + mt_._IoHandlerInterface_ConvertToUiName_Offset, url, flags);
}
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoHandlerInterface::Hxx1::ConstReferenceFunctionsImpl<S>::ConvertToUiName(const UrlInterface& url, CONVERTTOUINAMEFLAGS flags) const -> Result<String>
{
	...
	return (mt_._IoHandlerInterface_ConvertToUiName(reinterpret_cast<const maxon::GenericComponent*>(o_) + mt_._IoHandlerInterface_ConvertToUiName_Offset, url, flags));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoHandlerInterface::Hxx1::ConstReferenceFunctionsImpl<S>::GetHandlerForScheme(const UrlScheme& scheme) -> IoHandler
{
	return (Hxx2::StaticMTable::_instance._IoHandlerInterface_GetHandlerForScheme(scheme));
}
