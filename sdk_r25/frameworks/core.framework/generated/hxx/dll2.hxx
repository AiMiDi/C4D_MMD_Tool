...
struct DllInterface::Hxx2
{
	...
	public:
		...
		static Result<void> _DllInterface_Unload(DllInterface* this_) { return C::Get(this_)->Unload(); }
		static Result<void> _DllInterface_Release(DllInterface* this_) { return C::Get(this_)->Release(); }
		static Result<void*> _DllInterface_FindSymbolPointer(const DllInterface* this_, const CString& symbol) { return C::Get(this_)->FindSymbolPointer(symbol); }
		static Result<Binary*> _DllInterface_GetBinary(const DllInterface* this_) { return C::Get(this_)->GetBinary(); }
		static Result<void> _DllInterface_UpdateSymbolData(DllInterface* this_) { return C::Get(this_)->UpdateSymbolData(); }
		static Result<Url> _DllInterface_GetPath(const DllInterface* this_) { return C::Get(this_)->GetPath(); }
	};
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto DllInterface::Hxx1::ConstReferenceFunctionsImpl<S>::LoadDll(const Url& fileName, Bool registerDll, Bool requireCoreModule, Int systemDependentFlags) -> Result<DllRef>
{
	return (MTable::_instance._DllInterface_LoadDll(fileName, registerDll, requireCoreModule, systemDependentFlags));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto DllInterface::Hxx1::ConstReferenceFunctionsImpl<S>::GetModule(const void* handle, UInt processId) -> Result<DllRef>
{
	return (MTable::_instance._DllInterface_GetModule(handle, processId));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto DllInterface::Hxx1::ConstReferenceFunctionsImpl<S>::AddDllPath(const Url& dllDirectory) -> Result<void>
{
	return (MTable::_instance._DllInterface_AddDllPath(dllDirectory));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto DllInterface::Hxx1::ConstReferenceFunctionsImpl<S>::RemoveDllPath(const Url& dllDirectory) -> Result<void>
{
	return (MTable::_instance._DllInterface_RemoveDllPath(dllDirectory));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto DllInterface::Hxx1::ConstReferenceFunctionsImpl<S>::GetLoadedDlls(const ValueReceiver<const DllRef&>& dlls) -> Result<void>
{
	return (MTable::_instance._DllInterface_GetLoadedDlls(dlls));
}
