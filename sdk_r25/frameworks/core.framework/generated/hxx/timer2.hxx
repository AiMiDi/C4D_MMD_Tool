...
struct TimerInterface::Hxx2
{
	...
	public:
		...
		static void _TimerInterface_CancelAndWait(TimerInterface* this_) { return C::Get(this_)->CancelAndWait(); }
		static void _TimerInterface_Cancel(TimerInterface* this_) { return C::Get(this_)->Cancel(); }
		static maxon::ObservableRef<ObservableTimerStartedDelegate> _TimerInterface_ObservableTimerStarted(const TimerInterface* this_) { return C::Get(this_)->ObservableTimerStarted(); }
		static maxon::ObservableRef<ObservableTimerFinishedDelegate> _TimerInterface_ObservableTimerFinished(const TimerInterface* this_) { return C::Get(this_)->ObservableTimerFinished(); }
		static maxon::ObservableRef<ObservableTimerOverloadDelegate> _TimerInterface_ObservableTimerOverload(const TimerInterface* this_) { return C::Get(this_)->ObservableTimerOverload(); }
/// @cond IGNORE

		static void _TimerInterface_PrivateDisableAllTimers() { return C::PrivateDisableAllTimers(); }
		static Result<void> _TimerInterface_Start(TimerInterface* this_, Float64 interval, Float64 startOffset, Float64 tolerance, const JobInterfacePOD& pod, JobQueueInterface* queue) { return C::Get(this_)->Start(interval, startOffset, tolerance, pod, queue); }
/// @endcond
...
template <typename S> template <typename FN> MAXON_ATTRIBUTE_FORCE_INLINE auto TimerInterface::Hxx1::ConstReferenceFunctionsImpl<S>::AddPeriodicTimer(TimeValue interval, FN&& job, JobQueueInterface* queue) -> Result<TimerRef>
{
	return (TimerInterface::template AddPeriodicTimer<FN>(std::forward<TimeValue>(interval), std::forward<FN>(job), queue));
}
template <typename S> template <typename FN> MAXON_ATTRIBUTE_FORCE_INLINE auto TimerInterface::Hxx1::ConstReferenceFunctionsImpl<S>::AddOneShotTimer(TimeValue delay, FN&& job, JobQueueInterface* queue) -> Result<TimerRef>
{
