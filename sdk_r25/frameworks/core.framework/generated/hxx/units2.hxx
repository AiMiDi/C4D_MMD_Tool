...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto DimensionInterface::Hxx1::ConstReferenceFunctionsImpl<S>::Get(BASE_DIMENSION q) -> const Dimension&
{
	return (Hxx2::StaticMTable::_instance._DimensionInterface_Get(q));
}
...
struct UnitInterface::Hxx2
{
	...
	public:
		...
		static const Id& _UnitInterface_GetId(const UnitInterface* this_) { return C::Get(this_)->GetId(); }
		static const Dimension& _UnitInterface_GetDimension(const UnitInterface* this_) { return C::Get(this_)->GetDimension(); }
		static Float _UnitInterface_GetSIConversionFactor(const UnitInterface* this_) { return C::Get(this_)->GetSIConversionFactor(); }
		static Result<Unit> _UnitInterface_operatorMul(const UnitInterface* this_, const Result<Unit>& u) { return C::Get(this_)->operator *(u); }
		static Result<Unit> _UnitInterface_operatorDiv(const UnitInterface* this_, const Result<Unit>& u) { return C::Get(this_)->operator /(u); }
		static Result<Unit> _UnitInterface_operatorXor(const UnitInterface* this_, Int exponent) { return C::Get(this_)->operator ^(exponent); }
		static Result<Unit> _UnitInterface_Copy(const UnitInterface* this_, const Id& uid) { return C::Get(this_)->Copy(uid); }
		static Result<Unit> _UnitInterface_Copy_1(const UnitInterface* this_, const Dimension& quantity) { return C::Get(this_)->Copy(quantity); }
		static Result<void> _UnitInterface_AddToDimension(const UnitInterface* this_) { return C::Get(this_)->AddToDimension(); }
		static Result<Unit> _UnitInterface_Reduce(const UnitInterface* this_) { return C::Get(this_)->Reduce(); }
		static String _UnitInterface_ToString(const UnitInterface* this_, const FormatStatement* fs) { return C::Get(this_)->ToString(fs); }
	};
