...
public:
	...
	ResultRef<typename std::remove_const<ReferencedType>::type> MakeWritable(Bool resetOnError = !maxon::HasBaseDetector<ReferencedType, ErrorInterface>::value) { return this->GetBaseRef().MakeWritable(resetOnError); }
};
...
{
	const ClassInterface* _class; ///< Owning class object. Although just a simple pointer, this acts as a strong reference due to the internals of object handling.
	void (*_free)(const void*);	///< Destruct&free function.
};
