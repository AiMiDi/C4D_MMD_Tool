...
/// @MAXON_ANNOTATION{errorObject=false}
class RuntimeErrorInterface : MAXON_INTERFACE_BASES(ErrorInterface)
{
	MAXON_INTERFACE(RuntimeErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.corenode.interface.runtimeerror");
public:
...
private:
	FrameData(FrameData* parent, const Frame* frame) : _parent(parent), _frame(frame)
	{
	}
...
public:
	VarAccess(Context& ctx, FrameData& frame, const Int* offsetsMap, const void* userData)
		: _frame(&frame), _offsetsMap(offsetsMap), _context(&ctx), _userData(userData)
	{
	}
