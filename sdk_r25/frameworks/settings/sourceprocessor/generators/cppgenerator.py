    ...
    def processMethod(self, method, cls, freeMethod):
        ...
        if method.kind == 'MAXON_METHOD':
            ...
            if cls.interface == Class.VIRTUAL:
                if method.static:
                    ...
                    self.reflectionFunctions << 'MAXON_GET_VIRTUAL_METHOD_INFO' + ('_NO_PROXY' if cls.singleImpl else '') + '(' + ('CONST_INSTANCE_FUNCTION, ' if method.const else 'INSTANCE_FUNCTION, ') + paramNames + ', ' + str(inOut) + 'LL, ' + maxonComma(self.tclass) + ', _' + fname + '),'
        ...
        if not (freeMethod and cls.derived) and (method.kind == 'MAXON_METHOD'):
            if cls.interface == Class.VIRTUAL:
                ...
                self.init << '_' + cname + ' = GetFreeMethod<IMPL>(OVERLOAD_MAX_RANK);'
                self.pointers << 'template <typename IMPL> auto GetFreeMethod(maxon::OverloadRank1) { return &IMPL::_' + cname + '; }'
                self.pointers << 'template <typename IMPL, typename = typename IMPL::Implementation::PrivateComponentWithBase> auto GetFreeMethod(maxon::OverloadRank2) { return reinterpret_cast<decltype(_' + cname + ')>(&maxon::ObjectInterface::PrivateFreeInstance); }'
            else:
            ...
            if cls.interface == Class.NONVIRTUAL or (cls.interface == Class.VIRTUAL and method.static):
                unresolvedImpl = 'tbl->_' + cname + ' = &Hxx2::' + ('CWrapper<maxon::StaticHelper<Hxx2::Unresolved>' if cls.interface == Class.VIRTUAL else 'Wrapper<Hxx2::Unresolved') + '>::_' + cname + ';'
            ...
            s = 'static ' + deferredRetType + ' _' + cname + '(' + xparams + ') { return '
            if cls.interface == Class.VIRTUAL:
                if method.static:
                ...
                if method.static:
                    ...
                    if method.kind == 'MAXON_FUNCTION':
                        ...
                        if cls.interface == Class.VIRTUAL:
                            ...
                            oPointer = 'reinterpret_cast<' + const + oPointerType + '*>(o_) + mt_._' + cname + '_Offset'
                        s += oPointer
                        s += functionArgs + ')'
                    if returnsThis:
                        if method.result:
    ...
    def processClass(self, cls):
        ...
        if cls.interface:
            ...
            if cls.interface == Class.VIRTUAL:
                ...
                if self.nonstaticMethods or self.implFunctions:
                    ...
                    wrapper = False
                    self.defs << 'template <typename S> using CWrapper = typename ' + wrapperSuper + ';'
            elif cls.interface == Class.NONVIRTUAL:
            ...
            if cls.refKind:
                ...
                for fn in ['Const', '', 'COW']:
                    ...
                    createFunctionsImpl = functions or cls.generic 
                    functionsImpl.append(createFunctionsImpl)
                    if functions or cls.generic:
                        self.decls << '/// Intermediate helper class for ' + cls.name + '.'
                        fnSuper = 'S' if fn == 'Const' or not functionsImpl[0] else 'ConstReferenceFunctionsImpl<S' + genericArgs + '>'
                ...
                constRefFunctionsImpl = 'ConstReferenceFunctionsImpl<' + superRefFunctionsImpl + '>' if functionsImpl[0] else superRefFunctionsImpl
                self.decls << 'template <typename ST' + genericParamsWithDefaults + '> struct ReferenceFunctionsHelper : public std::conditional<'
                if functionsImpl[2]:
                    self.decls << 'maxon::Bool(ST::type::Handler::KIND & (maxon::VALUEKIND::COW_MASK | maxon::VALUEKIND::DEEP_CONSTNESS)),'
                    self.decls << 'COWReferenceFunctionsImpl<' + superRefFunctionsImpl + '>,'
                    if functionsImpl[1]:
                        self.decls << 'typename std::conditional<std::is_const<typename ST::type::PrivateReferencedType>::value, ' \
                            + constRefFunctionsImpl + ', ' \
                            + 'ReferenceFunctionsImpl<' + superRefFunctionsImpl + '>>::type'
                    else:
                    ...
                    self.decls << 'ReferenceFunctionsImpl<' + superRefFunctionsImpl + '>,'
                    self.decls << constRefFunctionsImpl
                else:
