...
public:
	...
	Id _accessNodeResultId;					/// Access node port id to connect to component access bundle (generally the access node's single out for an input component access).
	Id _dummy;											/// Unused
	Bool _allowNested = false;			/// True to allow nested component access (break down a matrix's vectors into distinct float inputs.

	using PortMap = BaseArray<Tuple<Id, Int, Data>>;							// {Port Id, 0 based port index (full IO port index), port type and default value (could just default to 0)}
	BufferedBaseArray<PortMap, (Int)PORT_DIR::END> _accessIo;			// Use [PORT_DIR::INPUT] or [PORT_DIR::OUTPUT].

	/// Delegate that will invoke the CoreNode  within the user node #_nodeId.
	using InvokeType = Delegate<Result<void>(const Block<const Tuple<ConstDataPtr, CORENODE_PORT_FLAGS>>&)>;
