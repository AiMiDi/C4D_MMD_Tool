...
class Generator(object):
    ...
    def writeInterfaceUse(self, cls, iname):
        ...
        if cls.interface != Class.SIMPLE_VIRTUAL:
            self.writeUse('s_ui_' + underscoreClass + suffix, iname + '::_interface.GetReference(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, "' + self.header + '"', cls.implementationModule)
        if cls.interface == Class.VIRTUAL:
            if cls.singleImpl:
                self.writeUse('s_ui1_' + underscoreClass + suffix, iname + '::_nonvirtualMTable.GetReference(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, "' + self.header + '"', cls.implementationModule)
            if cls.hasStaticMethods:
                self.writeUse('s_usi_' + underscoreClass + suffix, iname + '::_staticInterface.GetReference(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, "' + self.header + '"', cls.implementationModule)
    ...
    def writeInterfaceDefinition(self, cls, iname):
        ...
        if cls.implementationModule:
            self.reflection << 'PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(' + iname + ');'
        self.reflection << 'static maxon::InterfaceDefinition s_' + cls.name + suffix + '(' + iname + '::_interface, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, maxon::EntityBase::FLAGS::' \
            + ('NONE' if notMain else 'MAIN_DEFINITION') + ',' + ('"' + cls.owner.qname(None, '::') + '::' + cls.refClass + '"' if cls.refClass else 'nullptr') + ');'
        if notMain:
            templ = iname.find('<') 
            self.reflection << 'static maxon::EntityUse s_umi_' + cls.name + suffix + '(' + cls.qname(None, '::', True) + (iname[templ:] if templ >= 0 else '') + '::_interface.GetReference(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, nullptr, false);'
    ...
    def processMethod(self, method, cls, freeMethod):
        ...
        nonConstRefReturn = method.retType.endswith('&') and not method.retType.startswith('const')
        onlyCow = not method.static and ((nonConstRefReturn and not method.parseAnnotation('refclass', False))  # non-const reference return types can only be handled by COW functions unless overridden by refclass=True
        ...
        if defaultReturn:
            ...
            defaultReturn = 'maxon::PrivateDefaultReturnValue<' + methodRetType + (', true' if nonConstRefReturn else ', false') + '>(maxon::NULL_RETURN_REASON::@, OVERLOAD_MAX_RANK)'
        elif method.forward:
            defaultReturn = '{}'
        else:
    ...
    def processClass(self, cls):
        ...
        if cls.interface:
            ...
            if cls.refKind:
                ...
                for ref in ['Reference', 'NonConstRef']:
                    ...
                    superRef = 'maxon::RefBase<typename Hxx1::FnHelper<' + base + genericArgs + '>>'
                ...
                if buildRefClass:
                    ...
                    self.decls << genericPrefix + 'class ' + ('[[nodiscard]] ' if cls.isError else '') + cls.refClass + ' : public ' + base << '{' << 'public:'
            ...
            if errorObject and cls.parseAnnotation('refclass', True) == True:
                ...
                if cls.implementationModule:
                    rest = ',  MAXON_IMPLEMENTATION_MODULE(' + cls.implementationModule + ');'
                else:
                    rest = ''
                self.decls << 'MAXON_DECLARATION(' + cls.refClass + ', ' + errorObject + ', ' + en + rest + ');'
        ...
        if cls.interface and cls.interface != Class.SIMPLE_VIRTUAL:
            ...
            if cls.interface == Class.VIRTUAL:
                ...
                dest << 'PRIVATE_MAXON_COMPONENT_REGISTER(' + maxonComma(proxy) + ', ' + maxonComma(templatePrefix) + ', typename, maxon::EntityBase::FLAGS::PROXY, ' + ('nullptr' if cls.singleImpl else '&PRIVATE_MAXON_TRANSLATION_SUBUNIT') + ', ::MaxonRegistry, maxon::LiteralId());'
                dest << '#endif'
                if cls.hasStaticMethods:
            ...
            if isinstance(decl, Class):
                ...
                if decl.kind == 'MAXON_DATATYPE':
                    ...
                    if decl.containsResourceId and not self.cpp:
                        ...
                        base = decl.owner.qname(None, '::') + '::RESTRICT' if decl.owner.containsResourceId else 'maxon::RESTRICT'
...
def process(args, file, root):
    ...
    if root:
        ...
        if needHxx12 or not (gen.regs.hasOnlyDirectiveCode() and gen.reflection.hasOnlyDirectiveCode()):
            ...
            for c in sorted(root.forwardHeaders):
                i = '#include ' + c + '\n' + i
...
def done(args, results, sources, msg, err):
    ...
    def writeRegisterFile(i, results):
        ...
        if i > 0:
            base += str(i)
        writeToFile(os.path.join(hxx, base + '.cpp'), '#define PRIVATE_MAXON_GENERATED_UNIT\n#include "' + base + '.hxx"\n')
        writeToFile(os.path.join(hxx, base + '.hxx'), '/*\n\tDON\'T EDIT THIS FILE!\n\tIt has been created and is updated automatically by the source processor.\n' + \
    ...
    if fwr:
        for file, prologue in [('interface_registration.cpp', '#define MAXON_REGISTRATION_EXCLUDE_REFLECTION\n#define MAXON_REGISTRATION_EXCLUDE_REFLECTION_NONVIRTUAL\n'),
            ...
            reg = '#define PRIVATE_MAXON_GENERATED_UNIT\n' + prologue
