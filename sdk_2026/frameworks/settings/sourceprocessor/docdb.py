...
def traverseFiles(args, maindir, framework=None):
    ...
    if maindir.endswith('framework'):
        # For now assume it's a public framework
        b = True
        if b:
            framework = Framework.getFramework(args, maindir)
    ...
    for fr in args.directories:
        ...
        for file, framework in traverseFiles(args, fr):
            if file.endswith('.framework') or file.endswith('.module'):
                # Assuming directory structure created by CMake projects
                dir = os.path.join(file, 'parsed')
...
class Framework(object):
    ...
    def parseAliases(self, aliasFile):
        f = open_23(aliasFile, 'r', newline=None)
        aliases = dict()
        for line in f:
            pos = line.find('=')
            if pos >= 0:
...
#            for x in usedFrameworks(args, file, False, set()):
#                b = Framework.getFramework(args, x)
#                if b:
#                    f.bases.add(b)
...
def main():
    ...
    args.generatedroot = None
    if not args.frameworkroot:
        args.frameworkroot = []
