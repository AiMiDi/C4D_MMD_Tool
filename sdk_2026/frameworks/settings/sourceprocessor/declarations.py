...
def open_23(file, mode, newline=None, *args, **kwargs):
    if sys.version_info.major == 2:
        if newline is None:
            mode += 'U'
        return open(file, mode, *args, **kwargs)
    else:
        return open(file, mode, newline=newline, *args, **kwargs)
...
# Writes string s to file. If the file's content already equals s, nothing is done. Afterwards the file is set to read-only.
def writeToFile(file, s):
    ...
    if current != s:
        try:
            att = os.stat(file)[0]
            os.chmod(file, att | stat.S_IWRITE)
        except:
            pass
        f = open(file, 'w')
        f.write(s)
        f.close()
        att = os.stat(file)[0]
        os.chmod(file, (att & ~stat.S_IWRITE) | stat.S_IREAD)
...
def findFrameworkDir(args, startDir, fwName):
    for dir in args.frameworkroot:
        fwDir = os.path.join(dir, fwName)
        if os.path.isdir(fwDir):
            return fwDir


def getFrameworkId(args, name):
    id = name
    fwDir = findFrameworkDir(args, args.directory, name)
    if fwDir:
...
def getModuleId(name):
    ...
    if name.endswith('.module'):
        ...
        id = 'net.maxon.c4d.' + name
    return id


def usedFrameworks(args, projectDir, selfAndRecursive, visited):
    ...
    if frameworks:
        for fw in frameworks.split(';'):
            fw = fw.strip()
            if fw:
                fwDir = findFrameworkDir(args, projectDir, fw)
                if fwDir:
                    if selfAndRecursive:
                        for x in usedFrameworks(args, fwDir, True, visited):
                            yield x
                    else:
                        yield fwDir
...
# Returns True if str is of the form vXYZ where XYZ is some decimal number
def isVersionIdentifier(str):
    return str and str[0] == 'v' and str[1:].isdigit()
...
class Entity(object):
    ...
    def skipVersionNamespaces(e, enableSkip):
        result = e
        if enableSkip:
            while result and isinstance(result, Namespace) and isVersionIdentifier(result.name):
                result = result.owner
        return result
...
class Function(CppDeclaration):
    ...
    def includeInRefClass(self):
        return (self.name != 'operator =' \
            ...
            # Don't add GetHashCode functions to reference classes, otherwise they would hide the corresponding functions of RefBaseFn.
            and not (self.name in ['GetHashCode', 'GetUniqueHashCode'] and len(self.params) == 0))
...
class Class(TypeDeclaration):
    ...
    def __init__(self, name, template, owner, access, forward):
        ...
        self.usings = []
        self.constUsings = []
        self.implementationModule = None
