...
{
	MAXON_INTERFACE_NONVIRTUAL(RegistryEntryIteratorInterface, MAXON_REFERENCE_NONE, "net.maxon.interface.registryentryiterator", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	MAXON_METHOD const Generic* GetEntry(const DataType& type) const;

	[[deprecated]] MAXON_METHOD ConstDataPtr GetEntry() const;
};
...
{
	MAXON_INTERFACE_NONVIRTUAL(RegistryInterface, MAXON_REFERENCE_NONE, "net.maxon.interface.registry", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	static MAXON_METHOD const Generic* FindEntryValue(const RegistryInterface* registry, const Id& eid, const DataType& type, Id* eidOut);

	[[deprecated]] static MAXON_METHOD const Generic* FindEntryValue(const RegistryInterface* registry, const Id& eid, const DataType& type);

	[[deprecated]] static MAXON_METHOD ConstDataPtr FindEntryValue(const RegistryInterface* registry, const Id& eid);
...
{
public:
	RegistryReference(LiteralId&& rid, TranslationUnit* unit) : _super(EntityBase::TYPE::REGISTRY, std::move(rid), unit)
	{
	}
...
public:
	...
	{
		return reinterpret_cast<const T*>(RegistryInterface::FindEntryValue(R::GetRegistry(), eid, R::GetDataType(), nullptr));
	}
	...
	{
		const T* v = Find(eid);
		return v ? *v : maxon::DefaultValue<const T&>();
	}
...
{
public:
	template <typename REG> explicit RegistryDefinition(REG*, TranslationUnit* unit) : _super(EntityBase::TYPE::REGISTRY, REG::GetId(), unit, nullptr), _type(&REG::GetDataType)
	{
	}
...
#define PRIVATE_MAXON_REGISTRY_DEFINE(R) static maxon::RegistryDefinition g_registryDefinition_##R((R::Registry*) nullptr, &PRIVATE_MAXON_TRANSLATION_SUBUNIT)

//----------------------------------------------------------------------------------------
/// MAXON_DEPENDENCY_ON_REGISTRY adds a dependency of the current module on the specified registry.
...
#define MAXON_DEPENDENCY_ON_REGISTRY(registry) \
	static maxon::EntityUse MAXON_CONCAT(s_regdep_, __LINE__)(registry::Registry::GetId().GetCString(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, nullptr, false);
