...
template <typename T, typename HELPER> class FId<T, InternedId, HELPER> : public InternedIdFidBase
{
public:
	using FidType = T;
...
/// @param[in] ...								Further information about the resource, e.g. MAXON_ATTRIBUTE_READONLY.
...
/// @code
/// MAXON_ATTRIBUTE(Bool, IsGroup, "isgroup", MAXON_ATTRIBUTE_READONLY);
/// @endcode
//----------------------------------------------------------------------------------------
#define MAXON_ATTRIBUTE_READONLY static constexpr maxon::Bool READONLY = true;
...
{
public:
	explicit LiteralIdSwitch(const LiteralId& id) : _id(id) { }
...
private:
	LiteralId _id; // the Id of the switch expression
};
...
inline IdSwitchCheck<LiteralIdSwitch> GetIdSwitch(const LiteralId& id, OverloadRank1) { return LiteralIdSwitch(id); }
...
#define PRIVATE_ID_ENUM_CASE(key, line) key.GetHashCode(): if (privateSwitchId.Check(key)) goto MAXON_CONCAT(lbl, line); break; MAXON_CONCAT(lbl, line)
...
//----------------------------------------------------------------------------------------
/// ID_SWITCH and ID_ENUM_CASE allow to use enum LiteralId values in a switch statement, see #ID_SWITCH.
/// Should be used instead of the usual #ID_CASE whenever an enum is defined using a RESOURCE_DEFINE with a LiteralId type.
/// @param[in] key								The key to be used for the case.
//----------------------------------------------------------------------------------------
#define ID_ENUM_CASE(key) MAXON_EXPAND(PRIVATE_ID_ENUM_CASE(key, __LINE__))
...
//----------------------------------------------------------------------------------------
	...
	{
		constexpr Bool firstTypeResult = ValidKeyValuePairTraitHelper<ILLEGALTYPE>::template IsValueTypeLegal<VALUETYPE, KEYTYPE>();
		constexpr Bool otherTypesResult = ValidKeyValuePairTraitHelper<OTHERILLEGALTYPES...>::template IsValueTypeLegal<VALUETYPE, KEYTYPE>();
		return firstTypeResult && otherTypesResult;
	}
};

// Specialization for single illegal type use case.
template <typename ILLEGALTYPE>
class ValidKeyValuePairTraitHelper<ILLEGALTYPE>
{
private:

	/// @brief Implementation of the type validation.
	template <typename VALUETYPE, typename KEYTYPE>
	static constexpr Bool IsValueTypeLegalImpl()
	{
		constexpr Bool isValidFidKey = IsFidClass<KEYTYPE>::value && !STD_IS_REPLACEMENT(same, typename IsFidClass<KEYTYPE>::type, Data);
		using FIDVALUETYPE = typename std::conditional<isValidFidKey, typename maxon::Substitute<VALUETYPE, typename IsFidClass<KEYTYPE>::type>::type, void>::type;

		// With Fid as a key, ignore T type check. Otherwise, T must not allow LiteralId.
		constexpr Bool nonFidValueTypeOk = isValidFidKey || !STD_IS_REPLACEMENT(same, typename std::decay<VALUETYPE>::type, ILLEGALTYPE);
		// With non Fid key, ignore TT check. Otherwise, FIDVALUETYPE must not allow LiteralId, but VALUETYPE can be LiteralId then and will be converted to FIDVALUETYPE.
		constexpr Bool fidValueTypeOk = !isValidFidKey || !STD_IS_REPLACEMENT(same, typename std::decay<FIDVALUETYPE>::type, ILLEGALTYPE);
		constexpr Bool valueTypeOk = nonFidValueTypeOk && fidValueTypeOk;
		return valueTypeOk;
	}
...
public:
	...
	static constexpr Bool IsValueTypeLegal()
	{
		return IsValueTypeLegalImpl<VALUETYPE, KEYTYPE>();
	}
};
...
public:
	...
	/// @brief Validation using deduced types, i.e. run type validation on lvalue, rvalue, ...
	template <typename VALUETYPE, typename KEYTYPE>
	static constexpr Bool IsValueTypeLegal(const KEYTYPE& key, const VALUETYPE&)
	{
		return IsValueTypeLegal<VALUETYPE, decltype(key)>();
	}

	/// @brief Validation using deduced key type. Allows validation directly using attribute FId variables.
	template <typename VALUETYPE, typename KEYTYPE>
	static constexpr Bool IsValueTypeLegal(const KEYTYPE&)
	{
		return IsValueTypeLegal<VALUETYPE, KEYTYPE>();
	}

	/// @brief Validation using explicit types.
	template <typename VALUETYPE, typename KEYTYPE>
	static constexpr Bool IsValueTypeLegal()
	{
		constexpr Bool otherTypesRes = maxon::details::ValidKeyValuePairTraitHelper<ILLEGALTYPES...>::template IsValueTypeLegal<VALUETYPE, KEYTYPE>();

		return otherTypesRes;
	}

	/// @brief Use "ValidKeyValuePairTrait<ILLEGALTYPES...>::IsValid<VALUETYPE, KEYTYPE>" to get compile time validation of the type pair.
	template <typename VALUETYPE, typename KEYTYPE>
