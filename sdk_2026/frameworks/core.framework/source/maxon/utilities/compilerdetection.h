...
#if !defined(MAXON_TARGET_64BIT) && (__LP64__ || _WIN64)
	#define MAXON_TARGET_64BIT
#endif

#define MAXON_ATTRIBUTE_EMPTY_BASES // MSVC compiler needs special handling for empty bases

#if (defined(_WIN32) || defined(_MSC_VER)) && !defined(MAXON_TARGET_WINDOWS)
	#define MAXON_TARGET_WINDOWS
#elif defined(__APPLE__)
	#include <TargetConditionals.h>
	#if defined(__has_feature)
		#if (__has_feature(address_sanitizer) || __has_feature(undefined_behavior_sanitizer)) && !defined(MAXON_TARGET_SANITIZE)
			#define MAXON_TARGET_SANITIZE
		#endif
	#endif
	
	#if TARGET_OS_MAC && TARGET_OS_SIMULATOR
		#if !defined(MAXON_TARGET_IOS_SIMULATOR)
			#define MAXON_TARGET_IOS_SIMULATOR
		#endif
	...
	// clang windows emulated the msvc behaviour, so even clang windows needs to have this keyword
	#undef MAXON_ATTRIBUTE_EMPTY_BASES
	#define MAXON_ATTRIBUTE_EMPTY_BASES  __declspec(empty_bases)
#endif
	...
	#define MAXON_ASSERT_STANDARD_LAYOUT(X) static_assert(STD_IS_REPLACEMENT(standard_layout, X), #X " must be a standard-layout class!")
	#define MAXON_ASSERT_LEGAL_LAYOUT(X) static_assert(!STD_IS_REPLACEMENT(polymorphic, X), #X " must not contain virtual members!")
	...
	#define MAXON_OFFSETOF_NON_STANDARD_LAYOUT(T, M) (size_t((&reinterpret_cast<const T*>(32)->M)) - size_t(32))	// Use a non-nullptr base address because otherwise the offset for derived classes with virtual methods were always 0.
	...
	#endif
	#define MAXON_WARNING_DISABLE_UNUSED_PRIVATE_FIELD		_Pragma("clang diagnostic ignored \"-Wunused-private-field\"")
	...
	#define MAXON_ADDRESS_SANITIZER_DISABLE(reason) __attribute__((no_sanitize("address"))) 

	#define _HAS_RANGE_BASED_FOR_DIFFERING_TYPES

	#pragma clang diagnostic ignored "-Wundefined-var-template"
	#define PRIVATE_MAXON_FILE __FILE_NAME__
	...
	#define MAXON_WARNING_DISABLE_VARIABLE_SHADOWING		__pragma(warning(disable : 4456)) __pragma(warning(disable : 4457))
	#define MAXON_WARNING_DISABLE_UNUSED_VARIABLES			__pragma(warning(disable : 4101)) __pragma(warning(disable : 4189))
	...
	#define _HAS_RANGE_BASED_FOR_DIFFERING_TYPES
	#define PRIVATE_MAXON_FILE__ __builtin_FILE()
  ...
  #define PRIVATE_MAXON_FILE		__FILE__
	...
	#define STD_IS_REPLACEMENT(name, ...)					std :: is_##name<__VA_ARGS__>::value
#elif defined(__linux__)  && defined(MAXON_COMPILER_CLANG)
	#define STD_IS_REPLACEMENT(name, ...)					std :: is_##name<__VA_ARGS__>::value
#elif defined MAXON_COMPILER_CLANG
	#define __is_copy_constructible(T)						std :: is_copy_constructible<T>::value
	#define __is_default_constructible(T)					std :: is_default_constructible<T>::value
	#define __is_move_constructible(T)						std :: is_move_constructible<T>::value
	#define __is_copy_assignable(...) 						std :: is_copy_assignable<__VA_ARGS__>::value
	#define __is_move_assignable(T)								std :: is_move_assignable<T>::value
	#define __is_trivially_copy_assignable(T)			std :: is_trivially_copy_assignable<T>::value
	#define __is_trivially_copyable(T)						std :: is_trivially_copyable<T>::value

	// Unfortunately as of Xcode 13 this is not part of its clang build.
	#if defined(__APPLE__) && (MAXON_COMPILER_CLANG < 2000)
		#define __is_scalar(...)											std :: is_scalar<__VA_ARGS__>::value
		#define __is_integral(...)										std :: is_integral<__VA_ARGS__>::value
		#define __is_const(...)												std :: is_const<__VA_ARGS__>::value
		#define __is_convertible(...)									std :: is_convertible<__VA_ARGS__>::value
		#define __is_destructible(...)								std :: is_destructible<__VA_ARGS__>::value
		#define __is_enum(...)												std :: is_enum<__VA_ARGS__>::value
		#define __is_floating_point(...)							std :: is_floating_point<__VA_ARGS__>::value
		#define __is_function(...)										std :: is_function<__VA_ARGS__>::value
		#define __is_pointer(...)											std :: is_pointer<__VA_ARGS__>::value
		#define __is_reference(...)										std :: is_reference<__VA_ARGS__>::value
		#define __is_same(...)												std :: is_same<__VA_ARGS__>::value
		#define __is_void(...)												std :: is_void<__VA_ARGS__>::value
	#endif

	#define STD_IS_REPLACEMENT_HELPER(name, ...)	name(__VA_ARGS__)
...
#define MAXON_WARN_UNUSED_CLASS PRIVATE_MAXON_WARN_UNUSED_CLASS
		...
		#define MAXON_TARGET_ABI_WIN_ARM64
	#else
		#define MAXON_TARGET_CPU_INTEL
		#define MAXON_TARGET_CPU_X86
		...
		#endif
	#endif
	#define PRIVATE_MAXON_MTABLE_PTMF
#elif defined(MAXON_TARGET_MACOS) || defined(MAXON_TARGET_LINUX) || defined(MAXON_TARGET_IOS)
		...
		#define MAXON_TARGET_CPU_ARM
		#ifdef MAXON_TARGET_64BIT
			#define MAXON_TARGET_ABI_AARCH64
		#endif
	#endif
#endif
...
#if defined(MAXON_TARGET_ABI_WIN_X64) || defined(MAXON_TARGET_ABI_SYSTEMV_AMD64) || (defined(MAXON_TARGET_ABI_AARCH64) && !defined(MAXON_TARGET_IOS)) || defined(MAXON_TARGET_ABI_WIN_ARM64)
...
#define MAXON_SWITCH_CHECKALLENUMS_END								MAXON_WARNING_POP
...
#else
	#define MAXON_LIKELY(...) (__VA_ARGS__)
	#define MAXON_UNLIKELY(...) (__VA_ARGS__)
#endif
	...
	{
	class EnforceNullptrCheck { };
	}

	inline void* operator new(size_t size, maxon::details::EnforceNullptrCheck, void* ptr) noexcept { return ptr; }
	// Placement new needs a matching deletion function, otherwise warning C4291 is triggered.
	inline void operator delete(void*, maxon::details::EnforceNullptrCheck, void*) noexcept { }
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	#define MAXON_SAFE_PLACEMENT_NEW(PTR)		new (maxon::details::EnforceNullptrCheck(), PTR)
#else
	//----------------------------------------------------------------------------------------
	/// Clang 3.7 introduced a new optimization where a placement new wont check the given ptr for nullptr.
	/// for places where the pointer is already checked outside this is a great optimization.
...
namespace std
{
	...
	template <typename T, typename... Args> using result_of = std::invoke_result<T, Args...>;
#else
	template <typename T, typename... Args> using result_of = typename std::result_of<T(Args...)>;
#endif
}
	...
	// other approaches with constexpr can also find the file start but the compiler will store still the full path.
	// unfortunately this is very slow. compile times are increased by 6x.
	#if 0 // __cplusplus >= 202002L
		#include <maxon/utilities/stringliteral.h>
		// stylecheck.indentation=false
		namespace maxon
		{
			template <StringLiteral NAME> struct PrivateFileNameHelper
			{
				template <size_t N> consteval static size_t GetFileNameLength(const char (&str)[N])
				{
					int idx;
					for (idx = (int)N - 1; idx >= 0 && str[idx] != '/' && str[idx] != '\\'; idx--)
					{
					}
					return N - idx - 1;
				}

				inline static constexpr auto length = GetFileNameLength(NAME.value);
				inline static constexpr auto nameT = StringLiteral<length>(&NAME.value[NAME.SIZE - length], true);
				inline static constexpr const char* name = nameT.value;
			};
		}
		#define PRIVATE_MAXON_FILE maxon::PrivateFileNameHelper<maxon::StringLiteral<maxon::GetStringLengthWith0(PRIVATE_MAXON_FILE__)>(PRIVATE_MAXON_FILE__, true)>::name
	#else
		#define PRIVATE_MAXON_FILE	PRIVATE_MAXON_FILE__
	#endif
#endif

#ifndef MAXON_FILE
	#define MAXON_FILE PRIVATE_MAXON_FILE
#endif
