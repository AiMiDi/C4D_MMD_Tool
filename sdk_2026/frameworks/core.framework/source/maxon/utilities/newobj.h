...
/// @code
/// MyType* x = NewObj(MyType, optional constructor parameters) iferr_return;
/// @endcode
...
/// exception, it returns null if the allocation fails. NewObj does not clear the memory.
/// @code
/// MyType* x = NewObjT<MyType>(optional constructor parameters) iferr_return;
/// @endcode
...
/// @return												Pointer to object (null if allocation failed)
//----------------------------------------------------------------------------------------
#define NewObjClear(T, ...) static_cast<T*>(MAXON_SAFE_PLACEMENT_NEW(maxon::details::NewDelete<T>::AllocatorType::AllocClear(maxon::details::NewDelete<T>::SIZE, MAXON_SOURCE_LOCATION_NAME(T))) typename maxon::details::NewDelete<T>::ConstructorType(__VA_ARGS__))
#endif
