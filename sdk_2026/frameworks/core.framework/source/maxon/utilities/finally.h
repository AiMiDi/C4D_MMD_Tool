...
public:
	...
	// template<std::convertible_to<T> U> std::convertible_to was introduced with C++20 and does not compile against C++17
	template<typename U>
	MAXON_IMPLICIT FinallyWrapper(U&& x) : _lambda(std::forward<U>(x))
	{
	}
...
//----------------------------------------------------------------------------------------
#define finally auto MAXON_CONCAT(private_finally_, __LINE__) = maxon::GetPtrSizedZeroRef<maxon::details::FinallyFactory>() % [&]() mutable -> void
...
public:
	...
	// template<std::convertible_to<T> U> std::convertible_to was introduced with C++20 and does not compile against C++17
	template<typename U>
	MAXON_IMPLICIT FinallyOnce(U&& lambda) : _lambda(std::forward<U>(lambda)), _isEnabled(true)
	{
	}
...
public:
	...
	{
		return FinallyOnce<T>(std::forward<T>(lambda));
	}
};
...
///  or might be invoked manually (to clean up). In any case the block is invoked just once
