...
public:
	...
	constexpr MAXON_IMPLICIT DefaultHasher(HashInt start = EMPTY_HASH) : _state(start)
	{
	}
	...
	template <typename... ARGS> void HashAndCombine(const ARGS&... args)
	{
		ForEach{(Combine(SFINAEHelper<DefaultCompare, ARGS...>::type::GetHashCode(args)), 0)...};
	}

	template <typename... R> static constexpr HashInt CombineHashes(HashInt first, R... rest)
	{
		DefaultHasher hash{first};
		ForEach{(hash.Combine(rest), 0)...};
		return hash;
	}
...
MAXON_WARNING_PUSH
MAXON_WARNING_DISABLE_UNUSED_PRIVATE_FIELD
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Mixes hashes of the given values into the hasher state.
	...
	//----------------------------------------------------------------------------------------
	template <typename HASH = DefaultCompare, typename... ARGS> void HashAndCombine(const ARGS&... values)
	{
		ForEach{(HashAndCombineImpl<HASH, ARGS>(values), 0)...};
	}
...
private:
	template <typename HASH, typename T> void HashAndCombineImpl(const T& value)
	{
