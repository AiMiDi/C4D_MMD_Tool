...
#endif
	MAXON_ATTRIBUTE_FORCE_INLINE BlockArray() : _blocks(ForwardAllocator(this))
	{
	}
...
#pragma warning(pop)
	...
	/// move constructor
	MAXON_ATTRIBUTE_FORCE_INLINE BlockArray(BlockArray&& src) : Super(std::move(src)), MAXON_MOVE_MEMBERS(_blocks)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename... ARGS> MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<T> Append(ARGS&&... args)
	{
		...
		return block->Append(std::forward<ARGS>(args)...);
	}
	...
	{
		return InsertBlock(GetCount(), values);
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename... ARGS> MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<T> Insert(Int position, ARGS&&... args)
	{
		T* element = InsertBlock(position, Block<const T>(nullptr, 1)).GetPointer();
		if (MAXON_LIKELY(element != nullptr))
		{
			if ((NewCopy(*element, std::forward<ARGS>(args)...) == FAILED) && TestForCopyFromMember<T>::isSupported)
			{
				// Copy failed, default construct and erase
				PlacementNew<T, Bool(MEMFLAGS & BLOCKARRAYFLAGS::ZEROINITIALIZE)>(element);
				Erase(position) iferr_ignore("position is valid");
				return nullptr;
			}
		}
		return element;
	...
	//----------------------------------------------------------------------------------------
	template <typename... ARGS> ResultMemT<Iterator> Insert(const Iterator& position, ARGS&&... args)
	{
		ResultMemT<Iterator> element = InsertBlock(position, Block<const T>(nullptr, 1));
		if (MAXON_LIKELY(element == OK))
		{
			T* elementPtr = element.UncheckedGetValue().GetPtr();
			if ((NewCopy(*elementPtr, std::forward<ARGS>(args)...) == FAILED) && TestForCopyFromMember<T>::isSupported)
			{
				// Copy failed, default construct and erase
				PlacementNew<T, Bool(MEMFLAGS & BLOCKARRAYFLAGS::ZEROINITIALIZE)>(elementPtr);
				Erase(element.UncheckedGetValue());
				return FAILED;
			}
		}
		return element;
	...
	Result<void> CopyFromImpl(BASEARRAY&& src, COLLECTION_RESIZE_FLAGS resizeFlags, OverloadRank4)
	{
		...
		while (cnt > 0)
		{
			...
			if (block->InsertBlock(0, ToBlock(&src[i], insertCnt)) == FAILED)
	...
	public:
		...
		MAXON_ATTRIBUTE_FORCE_INLINE explicit IteratorTemplate(CollectionType* a = nullptr, BlockType *block = nullptr, BlockIterator it = BlockIterator(), BlockIterator end = BlockIterator()) : _array(a), _block(block), _iterator(it), _end(end)
		{
		}

		MAXON_ATTRIBUTE_FORCE_INLINE IteratorTemplate(const IteratorTemplate& src) : _array(src._array), _block(src._block), _iterator(src._iterator), _end(src._end)
		{
		}

		MAXON_ATTRIBUTE_FORCE_INLINE IteratorTemplate& operator =(const IteratorTemplate& src)
		{
		...
		MAXON_ATTRIBUTE_FORCE_INLINE operator ConstIterator&()
		{
			return *(ConstIterator*) this;
		}
		...
		MAXON_ATTRIBUTE_FORCE_INLINE Bool operator ==(const IteratorTemplate& b) const
		{
...
private:
	...
	Int Insert(Int blockIdx, Int localIdx, const Block<const T>& values)
	{
		...
		while (insertCnt > 0)
		{
			...
			if (block->InsertBlock(localIdx, ToBlock(x, cnt)) == FAILED)
			{
	...
	public:
		...
		void Flush()
		{
			...
			// Avoid IsEmpty() triggering when Append() is called for a flushed array.
			if (_usedBlockCnt > 0)
				_usedBlockCnt = 1;
			_submittedElementCnt = 0;
			_isContinuous = Int(true);
		...
		ArrayBlock* Append()
		{
			...
			// Is there an empty block following (happens on flush?)
			...
			ArrayBlock* block = _array.AppendBlock(MoveBlock<ArrayBlock>(nullptr, 1)).GetPointer();
			if (block)
			{
	...
	private:
		...
		{
			// Allocate first and then construct because the ArrayBlock element with its allocator must not be moved or copied.
			ArrayBlock* block = _array.InsertBlock(position, MoveBlock<ArrayBlock>(nullptr, 1)).GetPointer();
			if (block)
			{
				new (block) ArrayBlock(_array.GetAllocator());
