...
namespace details
{
		...
		template <UInt64 MODULE_ID_HASH> ModuleInfo ModuleInfoWrapper<MODULE_ID_HASH>::info(MAXON_MODULE_ID, &g_mainTranslationUnit, EntityBase::FLAGS::FRAMEWORK PRIVATE_MAXON_DEFINE_MODULE_INFO_FLAGS);
	#else
		// This is the ModuleInfo used by a module.
		template <UInt64 MODULE_ID_HASH> ModuleInfo ModuleInfoWrapper<MODULE_ID_HASH>::info(MAXON_MODULE_ID, &g_mainTranslationUnit, EntityBase::FLAGS::NONE PRIVATE_MAXON_DEFINE_MODULE_INFO_FLAGS);
	#endif
#endif
...
#define MAXON_REGISTRY_REGISTER(R) namespace R { const maxon::DataType& Registry::GetDataType() { return maxon::GetDataType<EntryType>(); } maxon::RegistryReference Registry::_reference(Registry::GetId(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT); }
...
#define MAXON_INTERFACE_REGISTER(I, IID, NULL_IMPL, FLAGS) \
	...
	maxon::NonvirtualInterfaceReference I::_interface(IID, (maxon::MTableBase*) &I::MTable::_instance, (maxon::MTableBase*) &I::MTable::_unresolvedInstance, I::MTable::_ids, SIZEOF(I::MTable) / SIZEOF(void*), (maxon::NonvirtualInterfaceReference::UnresolvedInitializer) &I::MTable::InitUnresolved, NULL_IMPL, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, I::REFERENCE_KIND | FLAGS)

#define MAXON_DEFAULT_VALUE_INCOMPLETE_GET_DEFINITION \
	namespace maxon \
	{ \
		template <typename T, Bool ALLOW_NONCONST> T DefaultValueTypeHelper<T, ALLOW_NONCONST>::IncompleteGet() { return maxon::PrivateDefaultValueHelper<T>(OVERLOAD_MAX_RANK); } \
		template <typename T, Bool ALLOW_NONCONST> const T& DefaultValueTypeHelper<const T&, ALLOW_NONCONST>::IncompleteGet() { return maxon::PrivateConstRefDefaultValueHelper<T>(OVERLOAD_MAX_RANK); } \
	}
...
{
	return (Int) (void*) &DefaultValueTypeHelper<T, false>::IncompleteGet;
}
