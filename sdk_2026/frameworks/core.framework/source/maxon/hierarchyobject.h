...
{
	MAXON_INTERFACE(HierarchyObjectInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.hierarchyobject", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	HierarchyObjectIterator(const HierarchyObjectIterator& src) : _last(src._last), _parent(src.parent)
	{
	}

	HierarchyObjectIterator(HierarchyObjectIterator&& src) : _last(src._last), _parent(std::move(src._parent))
	{
		src._last = nullptr;
	}
	...
	{
		// Store first parent to later limit the highest possible tree level when searching for a neighbour tree.
		_parent = first.GetParent();

		for (HierarchyObjectInterface* ptr = first; ptr; ptr = ptr->FindNextLeaf(_parent))
		{
			 // filter
			if (ptr->IsInstanceOf<REFTYPE>())
			{
				_last = ptr;
				return;
			}
	...
	{
		return *reinterpret_cast<REFTYPE*>(&_last);
	}
...
private:
	HierarchyObjectInterface* _last = nullptr;
