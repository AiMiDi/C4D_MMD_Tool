...
class RunLoop
{
	MAXON_INTERFACE_NONVIRTUAL(RunLoop, MAXON_REFERENCE_STATIC, "net.maxon.interface.runloop");
...
public:
	...
	/// @markPrivate for legacy c4d, Windows only
	...
	/// @markPrivate for legacy c4d, Linux only
	static MAXON_METHOD void PrivateSetDefaultDisplayAndScreen(void* display, int screen);

	//----------------------------------------------------------------------------------------
	/// Invokes the run loop for the specified duration (rather used for external run loops).
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Bool Invoke(TimeValue duration);

	/// @markPrivate
	static MAXON_METHOD void SetMainThreadExecutionDelegate(Delegate<void()>&& executionDelegate);
};
