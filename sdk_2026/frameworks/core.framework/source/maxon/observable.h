...
template <> struct MAXON_WARN_UNUSED_CLASS GetResultType<void>
{
	...
	static resulttype ReturnData(Data&&)
	{
		return OK;
	}
};
...
#define PRIVATE_MAXON_OBSERVABLE_IMPL(NAME, ID, Interface) \
	maxon::ObservableImplRef<maxon::ObservableClass_PrivateHelper, typename Interface::MAXON_CONCAT(NAME, Delegate)>	MAXON_CONCAT(_, NAME); \
	maxon::ObservableRef<typename Interface::MAXON_CONCAT(NAME, Delegate)> NAME(maxon::Bool create) const { return MAXON_CONCAT(_, NAME).Get(this->self, ID, create); }
...
/// @endcode
//----------------------------------------------------------------------------------------
#define MAXON_OBSERVABLE_IMPL(NAME, ...) PRIVATE_MAXON_OBSERVABLE_IMPL(NAME, MAXON_STRINGIFY(NAME), MAXON_SECONDARG(PRIVATE_MAXON_OBSERVABLE_IMPL_##__VA_ARGS__, __VA_ARGS__))
...
/// @endcode
//----------------------------------------------------------------------------------------
#define OBSERVABLE_IMPL_NONVIRTUAL(NAME, OBJECT) \
	ObservableImplRef<ObservableClass_PrivateHelper, Interface::MAXON_CONCAT(NAME, Delegate)>	MAXON_CONCAT(_, NAME); \
	ObservableRef<Interface::MAXON_CONCAT(NAME, Delegate)> NAME(maxon::Bool create) const { return MAXON_CONCAT(_, NAME).Get(OBJECT, MAXON_STRINGIFY(NAME), create); }
...
{
	MAXON_INTERFACE(FunctionBaseInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.functionbase", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllComponent, "net.maxon.component.observablecombinerrunall", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Implementation of CombinerInterface. Using this combiner ensures that all subscribers are called.
/// The result is combined by an OR operation.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllBoolOrComponent, "net.maxon.component.observablecombinerrunallboolor", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Implementation of CombinerInterface.
/// The result is initialized with true and runs until the first false occurs.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllBoolUntilFalse, "net.maxon.component.observablecombinerrunallbooluntilfalse", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Implementation of CombinerInterface.
/// The result is initialized with false and runs until the first true occurs.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllBoolUntilTrue, "net.maxon.component.observablecombinerrunallbooluntiltrue", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Implementation of CombinerInterface. Using this combiner ensures that all subscribers are called.
/// The result is combined by an OR operation.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllEnumFlagsOrComponent, "net.maxon.component.observablecombinerrunallenumflagsor", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Implementation of CombinerInterface. Using this combiner ensures that all subscribers are called.
/// The result is combined by an OR operation. Use this for 64-bit-wide enums.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllEnumFlags64OrComponent, "net.maxon.component.observablecombinerrunallenumflags64or", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

//----------------------------------------------------------------------------------------
/// Implementation of CombinerInterface. Using this combiner ensures that all subscribers are called.
/// All errors returned by the subscribers will be aggregated and returnd as AggregatedError.
//----------------------------------------------------------------------------------------
MAXON_DECLARATION(ComponentDescriptor, ObservableCombinerRunAllAggregateErrorsComponent, "net.maxon.component.observablecombinerrunallaggregateerrors", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	{
		ObservableBaseInterface::PrivateDisconnectAll(observer, 0);
		ObservableBaseInterface::PrivateDisconnectAll(observer, 1);
	}
	...
	{
		ObservableBaseInterface::PrivateDisconnectAll(nullptr, 0);
		ObservableBaseInterface::PrivateDisconnectAll(nullptr, 1);
	}
...
MAXON_DECLARATION(Class<FunctionBaseRef>, FunctionBaseClass, "net.maxon.class.functionbase", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
MAXON_DECLARATION(Class<ObservableBaseRef>, ObservableClass, "net.maxon.class.observable", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
template <typename RESULT> class ObservableNotifyRef<Delegate<RESULT()>> : public ObservableBaseRef
{
public:
	...
	typename maxon::details::GetResultType<RESULT>::resulttype Notify() const
	{
		if (GetPointer() == nullptr)
			return typename maxon::details::GetResultType<RESULT>::resulttype{};
...
public:
	...
	template <typename FN> Result<FunctionBaseRef> AddObserver(ObserverObjectInterface* observer, FN&& fn, Int priority = ADDOBSERVER_DEFAULTPRIORITY) const
	{
		...
		ObservableBaseRef::AddObserver(observer, func, priority) iferr_return;
	...
	template <typename FN> Result<FunctionBaseRef> AddObserver(ObserverObjectInterface* observer, FN&& fn, const JobQueueRef& queue, Int priority = ADDOBSERVER_DEFAULTPRIORITY) const
	{
		...
		static_assert(STD_IS_REPLACEMENT(same, typename DELEGATE::Super::ResultValueType, void), "Enqueued observers must not return a value.");
...
// 		}
	...
	{
		return AddObserver(nullptr, std::forward<FN>(fn), priority);
	}
	...
	{
		return AddObserver(nullptr, std::forward<FN>(fn), queue, priority);
	}

	//----------------------------------------------------------------------------------------
	/// Helper function for data type registration.
	//----------------------------------------------------------------------------------------
	static ObservableRef<DELEGATE> DefaultValue() { return ObservableRef<DELEGATE>(); }
};
...
{
	MAXON_INTERFACE_NONVIRTUAL(ObservableStaticInterface, MAXON_REFERENCE_STATIC, "net.maxon.interface.observablestatic", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
public:
...
/// @tparam DELEGATE							Delegate of the observable class defined in the MAXON_OBSERVABLE macro. Usually "Delegate" is appended to the observable name, e.g. ObservableMachineStateChangedDelegate.
//----------------------------------------------------------------------------------------
template <typename ObservableClass, typename DELEGATE> class ObservableImplRef
{
public:
	//----------------------------------------------------------------------------------------
	...
	~ObservableImplRef()
	{
		ObservableBaseRef t = _observable.Get();
		if (t)
			t.Free();
	}

	[[deprecated("not required anymore, remove Init()")]] Result<void> Init(ObserverObjectInterface* owner, const Id& observableName)
	{
		return OK;
	}
	...
	void Free()
	{
		...
		{
			t.Free();
			_observable = ObservableBaseRef::DefaultValue();
		}
	}
	...
	ObservableBaseRef InitFirst(const ObserverObjectInterface* owner, const char* observableName) const
	{
		...
		{
			DebugStop();
			return ObservableBaseRef::DefaultValue();
		};

		const ClassInterface* cl = ObservableStaticInterface::GetOrAllocClass(ObservableClass::GetId(), ObservableClass::Get(), &DELEGATE::OBSERVABLECOMBINER::Get());

		ObservableBaseRef observable = Class<ObservableBaseRef>(cl).Create() iferr_return;

		observable.Init(const_cast<ObserverObjectInterface*>(owner), Id(observableName)) iferr_return;

		if (_observable.TryCompareAndSwap(observable, ObservableBaseRef::DefaultValue()))
			return observable;
	...
	{
		ObservableBaseRef t = _observable.Get();
		if (MAXON_UNLIKELY(t == nullptr) && createOnDemand)
			t = InitFirst(owner, observableName);
		return (ObservableRef<DELEGATE>&)t;
	};
	...
	Int GetObserverCount()
	{
		...
		if (!t)
			return 0;
		return t.GetObserverCount();
	}

	template <typename... ARGS> typename maxon::details::GetResultType<typename DELEGATE::ResultValueType>::resulttype Notify(ARGS&&... args) const
	{
		ObservableBaseRef t = _observable.Get();
		if (!t)
			return {}; // TODO: (Tilo) find the right combiner return value
		return ((ObservableRef<DELEGATE>&)t).Notify(std::forward<ARGS>(args)...);
	}

private:
	mutable ThreadSafeRef<ObservableBaseRef> _observable;
};
...
//----------------------------------------------------------------------------------------
	...
	#define MAXON_OBSERVABLE(RETTYPE, NAME, ...) \
		...
		MAXON_METHOD maxon::ObservableRef<MAXON_CONCAT(NAME, Delegate)> NAME(maxon::Bool create) const;

	#define MAXON_OBSERVABLE_STATIC(RETTYPE, NAME, ...) \
		class MAXON_CONCAT(NAME, Delegate); \
		static MAXON_METHOD maxon::ObservableRef<MAXON_CONCAT(NAME, Delegate)> NAME(maxon::Bool create);
#else
	// special defines for doxygen, this simulates a normal function
	#define MAXON_OBSERVABLE(RETTYPE, NAME, ARGUMENTLIST, COMBINER) ObservableRef<> NAME(maxon::Bool create) const -> RETTYPE (*) ARGUMENTLIST
	#define MAXON_OBSERVABLE_STATIC(RETTYPE, NAME, ARGUMENTLIST, COMBINER) ObservableRef<> NAME(maxon::Bool create) const->RETTYPE(*) ARGUMENTLIST
#endif
