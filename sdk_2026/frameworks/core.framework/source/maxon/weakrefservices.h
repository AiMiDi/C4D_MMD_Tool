...
struct FatWeakRefProxy;

class WeakRefServices
{
	MAXON_INTERFACE_NONVIRTUAL(WeakRefServices, MAXON_REFERENCE_STATIC, "net.maxon.interface.weakrefservices", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

public:
	//----------------------------------------------------------------------------------------
	/// @brief Returns true if there currently are weak references to the target. This does not take other metadata into account, just the weak references.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a weak reference to a strongly referenced object.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Copy constructs a weak reference.
	/// @param[in] dst								Destination.
	/// @param[in] src								Source.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Removes all weak references to a strongly referenced object (usually called when it is going to be destructed).
	...
	//----------------------------------------------------------------------------------------
	/// @brief All weak references to oldTarget will be updated and point to newTarget.
	/// The caller holds a strong reference to the old target. After the call there'll be no more weak references pointing to the old target (until new ones are created).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds an observer which will be invoked before a strongly referenced object is destructed.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Erases an observer which was installed with AddObserver. This is only required if the observer or its data
	/// becomes invalid or is to be released before the target object has been released.
	/// @note You must not call this method from inside the observer callback when the target is locked!
	/// @note The callbackData is not erased, it's required to match the value used for AddObserver.
	/// @param[in] target							Pointer to strongly referenced target object, guaranteed to be valid and not nullptr.
	/// @param[in] callback						Pointer to the callback.
	/// @param[in] callbackData				Either nullptr (just the callback pointer is relevant)  or it must match the call to AddObserver.
	/// @return												True on success, false if the observer couldn't be removed (target does not exist anymore or callback/callbackData does not match).
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Bool EraseObserver(const void* target, MemoryObserverCallback callback, void* callbackData);

	//----------------------------------------------------------------------------------------
	/// @brief Adds metadata to a target object.
	/// @param[in] target							Pointer to strongly referenced target object, guaranteed to be valid and not nullptr.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Removes metadata from a target object.
	/// @param[in] target							Pointer to strongly referenced target object, guaranteed to be valid and not nullptr.
	/// @param[in] uniqueId						Unique pointer to an id (either an InternedId or a private static Id).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a proxy object which allows safe access to the metadata.
	/// @param[in] target							Pointer to strongly referenced target object, guaranteed to be valid and not nullptr.
	/// @param[in] uniqueId						Unique pointer to an id (either an InternedId or a private static Id).
	...
	/// @brief private
	static MAXON_METHOD Result<Bool> BrowseAllWeakReferences(const void* oldTarget, const ValueReceiver<Pair<WeakRefBase*, FatWeakRefProxy*>>& receiver);
	static MAXON_METHOD Bool AddWeakReference(FatWeakRefProxy& proxy, const void* target);
	static MAXON_METHOD void RemoveWeakReference(FatWeakRefProxy& weakRef);
	static MAXON_METHOD Bool CopyWeakReference(FatWeakRefProxy& dst, const FatWeakRefProxy& src);
	static MAXON_METHOD Bool CopyWeakReference(FatWeakRefProxy& dst, const WeakRefBase& src);
	static MAXON_METHOD void MoveWeakReference(FatWeakRefProxy& dst, FatWeakRefProxy& src);
	static MAXON_METHOD void MoveWeakReference(FatWeakRefProxy& dst, WeakRefBase& src);
...
//----------------------------------------------------------------------------------------
/// A FatWeakRef is a WeakRef which requires more space but saves you one memory indirection when resolving it to
/// a strong reference. For most cases a simple WeakRef which is smaller will suit you better.
//----------------------------------------------------------------------------------------
...
public:
	...
	MAXON_IMPLICIT FatWeakRef(const REFERENCE& strongReference)
	{
		auto& ptr = GetBaseRef(strongReference);
		if (ptr)
			WeakRefServices::AddWeakReference(*this, ptr.GetPointer());
	}
	...
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT FatWeakRef(const FatWeakRef& src)
	{
	...
	/// @param[in] src								Source weak reference.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT FatWeakRef(const WeakRef<REFERENCE>& src)
	{
		if (WeakRefServices::CopyWeakReference(*this, src) == false)
			DebugStop();
	}
	...
	/// @param[in] src								Source weak reference.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT FatWeakRef(WeakRef<REFERENCE>&& src)
	{
	...
	/// @param[in] src								Source weak reference.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT FatWeakRef(FatWeakRef&& src)
	{
		WeakRefServices::MoveWeakReference(*this, src);
	}

	//----------------------------------------------------------------------------------------
	/// @brief Copies a weak reference.
	/// @param[in] src								Source weak reference.
	/// @return												*this.
	//----------------------------------------------------------------------------------------
	MAXON_OPERATOR_COPY_ASSIGNMENT(FatWeakRef);
	...
	FatWeakRef& operator =(const REFERENCE& strongReference)
	{
		...
		if (ptr)
		{
			if (WeakRefServices::AddWeakReference(*this, ptr.GetPointer()) == false)
				DebugStop();
		}
	...
	//----------------------------------------------------------------------------------------
	/// @brief Removes the weak reference.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a strong reference. If the weak referenced object was already released a
	...
	/// @brief Private
	const WeakRefTargetBase& GetProxyWeakRefTarget() const
	{
	...
	{
		return _hasWeakRefTarget != nullptr;
	}

	/// @brief Private
	operator FatWeakRefProxy&()
	{
		return *reinterpret_cast<FatWeakRefProxy*>(this);
	}

	/// @brief Private
	operator const FatWeakRefProxy&() const
	{
		return *reinterpret_cast<const FatWeakRefProxy*>(this);
	}
...
private:
	...
	mutable void* _hasWeakRefTarget = nullptr;
};

static_assert(sizeof(FatWeakRef<StrongRef<Char>>) == 24, "FatWeakRef should have the same size as FatWeakRefProxy.");
