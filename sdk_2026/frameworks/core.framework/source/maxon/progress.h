...
{
	MAXON_INTERFACE(ProgressInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.progress", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	/// @param[in] jobIndex						Job index returned by AddProgressJob() to cancel an individual sub job, or -1 to cancel all jobs.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> CancelJob(Int jobIndex = -1);

	//----------------------------------------------------------------------------------------
	/// Checks if the job is cancelled.
	/// @param[in] jobIndex						Job index returned by AddProgressJob() to check cancellation of an individual sub job, or -1 to check if the whole progres has been cancelled.
	/// @return												True if job is cancelled, false otherwise.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Bool IsCancelled(Int jobIndex = -1);
};

#include "progress1.hxx"

MAXON_DECLARATION(Class<ProgressRef>, ProgressObject, "net.maxon.image.class.mediasessionprogressobject", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
