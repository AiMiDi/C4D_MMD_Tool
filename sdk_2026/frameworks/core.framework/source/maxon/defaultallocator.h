...
public:
	...
	{
		System::Free(reinterpret_cast<void*>(MAXON_REMOVE_CONST(p)));
		p = nullptr;
	}
	...
	template <typename T> static MAXON_FUNCTION void DeleteInNextEpoch(T* object)
	{
		void (*dp)(T*) = &EpochDelete;
		System::AddEpochObserver((void*) object, (void (*)(void*))dp);
	}

private:
	template <typename T> static void EpochDelete(T* o)
	{
		DeleteObj(o);
	}
};
...
//----------------------------------------------------------------------------------------
#define NewMem(T, cnt) maxon::ResultPtr<T>(static_cast<T*>(maxon::DefaultAllocator::Alloc(maxon::Int64(SIZEOF(T)) * maxon::Int64((cnt)), MAXON_SOURCE_LOCATION_NAME(T))))
...
/// @return												Pointer to memory block or nullptr.
//----------------------------------------------------------------------------------------
#define NewMemClear(T, cnt) maxon::ResultPtr<T>(static_cast<T*>(maxon::DefaultAllocator::AllocClear(maxon::Int64(SIZEOF(T)) * maxon::Int64((cnt)), MAXON_SOURCE_LOCATION_NAME(T))))
...
{
	DefaultAllocator::Free(const_cast<void*&>(p));
}
...
template <typename T> void UniqueRefHandler::RemoveReference(const T* o)
{
	T* p = const_cast<T*>(o);
	DeleteObj(p);
}
