...
static void _PrivateConsoleOutput(const Char* s, const SourceLocation& loc, OUTPUT flags)
{
	if (LoggerInterface::_interface.IsInitialized() && System::_interface.IsInitialized() && ThreadServices::_interface.IsInitialized())
	{
		System::_ConsoleOutput(s, loc, flags);
	}
	else
	{
		printf("%s", s);
...
static const OUTPUT g_enabledOutputDefaults = OUTPUT::DIAGNOSTIC | OUTPUT::WARNING | OUTPUT::CRITICAL | OUTPUT::NOLINEBREAK;


bool g_enableShutdownOutputIgnore = false;

static OUTPUT g_enabledOutput = g_enabledOutputDefaults;
...
static void InitializeAndSetConsoleOutput()
{
	...
	if (hasCheckedDebugger == false)
	{
		...
		g_enabledOutput = g_enabledOutputDefaults;
...
void _ConsoleOutput(OUTPUT flags, const Char* str, Int line, const Char* file)
{
	...
	if (!(g_enabledOutput & flags & (OUTPUT::DIAGNOSTIC | OUTPUT::WARNING | OUTPUT::CRITICAL)) || g_enableShutdownOutputIgnore)
		return;
	...
	*dst++ = 0;
	...
	_PrivateConsoleOutput(result.GetFirst(), SourceLocation(file, line), flags);
}
...
void _ConsoleOutput(OUTPUT flags, const String& str, Int line, const Char* file)
{
	...
	// Safety guard against diagnostic output from a static destructor after MaxonEnd().
	if (str.GetPointer() && g_enableShutdownOutputIgnore == false)
	{
		iferr_scope_handler
		{
			err.DbgStop();
		};
		buf = str.GetCString() iferr_return;
		buf.Append('\0') iferr_return;
	}

	_ConsoleOutput(flags, buf.IsEmpty() ? "" : buf.GetFirst(), line, file);
}
...
void _ApplicationOutput(maxon::TARGETAUDIENCE t, maxon::WRITEMETA lvl, const maxon::StringProxy* proxy, Int line, const Char* file)
{
	...
	{
		_PrivateConsoleOutput("Console output failed", SourceLocation(file, line), OUTPUT::NONE);
	}
}
