...
inline Int Clz64(Int64 x)
{
	...
	#else
	return (x & 0xffffffff00000000LL) ? (_BitScanReverse(&result, (Int32)(x >> 32)), 31 - result) : _BitScanReverse(&result, (Int32)x) ? 63 - result
																																																																		 : 64;
	#endif
}
...
{
	static_assert(STD_IS_REPLACEMENT(integral, T), "Clz can only be used for integral values.");
...
{
	static_assert(STD_IS_REPLACEMENT(integral, T), "Ctz can only be used for integral values.");
