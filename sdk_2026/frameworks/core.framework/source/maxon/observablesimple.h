...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Finds the value associated with the given handle.
	/// handle has to be one of the handles returned by a previous call to the Insert function. Then this
	/// function returns the value which was inserted by that call.
	/// @param[in] handle							A handle (returned by a previous call to Insert).
	/// @return												The value associated with that handle, if any.
	//----------------------------------------------------------------------------------------
	Opt<T> FindValue(const GenericData& handle) const
	{
		if (handle.IsPopulated())
		{
			const UInt id = handle.Get<Remove>()._id;
			auto r = _map.Read();
			const T* const v = r->FindValue(id);
			if (v)
				return *v;
		}
		return {};
