...
public:
	...
	{
		return Append(block.GetFirst(), block.GetCount());
	}
...
{
	MAXON_INTERFACE_NONVIRTUAL(StringInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.string", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	template<typename T> static MAXON_ATTRIBUTE_FORCE_INLINE const String& GetAndCacheConstStringT(const T* chr, Int len, Int isUtf8)
	{
		return GetAndCacheConstString(chr, len, isUtf8);
	}
	...
	/// @return												The converted string.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<String> Prettify(Int indent = 0, const String& indentStr = CONSTSTRING("\t"), Int breakUpThreshold = 100, Bool angularBrackets = false) const;
...
namespace details
{
template <typename T> struct HasBaseHelper<T, StringInterface, 0> : public std::false_type
{
};
}
...
public:
	...
	template <typename T> static MAXON_ATTRIBUTE_FORCE_INLINE const CString& GetAndCacheConstStringT(const T* chr, Int len)
	{
		return GetAndCacheConstString(chr, len);
	}
...
#undef CONSTSTRING
#define CONSTSTRING(x)	(maxon::StringInterface::GetAndCacheConstStringT((const maxon::Char*)(PRIVATE_MAXON_AS_UTF8("") PRIVATE_MAXON_AS_UTF8(x)), SIZEOF(PRIVATE_MAXON_AS_UTF8(x)) - 1, MAXON_CONSTEXPR_EVAL(maxon::Bool, maxon::PrivateIsUtf8(PRIVATE_MAXON_AS_UTF8(x), SIZEOF(PRIVATE_MAXON_AS_UTF8(x))))))
#define CONSTCSTRING(x) (maxon::CStringInterface::GetAndCacheConstStringT("" x, SIZEOF(x) / SIZEOF(x[0]) - 1))
...
public:
	...
	{
		DebugAssert(count < 2 || str[1] != 0, "This looks like using the wrong constructor");
	...
	/// @param[in] stringDecoding			Encoding type.
	//----------------------------------------------------------------------------------------
	explicit String(const Block<const char8_t>& str, const StringDecodingRef& stringDecoding = GetUtf8DefaultDecoder())
	{
		if (SetCString(reinterpret_cast<const Block<const Char>&>(str), stringDecoding) == FAILED)
			*this = nullptr;
	}
#endif
...
{
#ifdef MAXON_TARGET_RELEASE
	if (cnt == 0)
		return String::DefaultValue(); // empty string
#endif
	...
	return StringInterface::GetAndCacheConstString(str, Int(cnt), -1);
}
...
MAXON_ATTRIBUTE_FORCE_INLINE const String& operator ""_s(const wchar_t* str, std::size_t cnt)
{
	...
	if (cnt == 0)
		return String::DefaultValue(); // empty string
#endif
	return StringInterface::GetAndCacheConstString(str, Int(cnt));
}
...
#ifdef MAXON_TARGET_RELEASE
	if (cnt == 0)
		return CString::DefaultValue(); // empty string
#endif
...
template <typename COLLECTION, typename SUPER> String BaseCollection<COLLECTION, SUPER>::ToString(const FormatStatement* formatStatement) const
{
	...
	for (AutoIterator<const COLLECTION> it(*static_cast<const COLLECTION*>(this)); it; ++it)
	{
		...
		if (result.Append(GlobalToString(*it, formatStatement)) == FAILED)
			DebugStop();
	}
	if (result.Append("}"_s) == FAILED)
		DebugStop();
...
template <typename SCR> Result<String> StringInterface::ConvertString(const SCR& converter) const
{
	iferr_scope;
...
#define FormatString(...)                                                                                                                  \
	maxon::ExtractString(                                                                                                                    \
		maxon::ForwardString<!STD_IS_REPLACEMENT(same, std::remove_extent<std::remove_reference<decltype(MAXON_FIRSTARG(__VA_ARGS__))>::type>::type,       \
													 const maxon::Char) ||                                                                                    \
...
inline void AppendStringWithSeparator(STRINGTYPE& base, const STRINGTYPE& add, const STRINGTYPE& delimiter)
{
	if (add.IsPopulated())
	{
		if (base.IsPopulated())
...
{
	STRINGTYPE s = base;
	AppendStringWithSeparator(s, add, delimiter);
	return s;
}
