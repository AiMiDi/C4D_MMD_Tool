...
/// you could use BaseRef<const MyType, StrongCOWRefHandler> which is equivalent to StrongCOWRef<MyType>.
///
...
public:
	...
	//----------------------------------------------------------------------------------------
	typename std::conditional<std::is_void<T>::value, DeleteReturnType01, T>::type& operator *() { return *_object; }
	...
	/// @return												Pointee as a reference.
	//----------------------------------------------------------------------------------------
	typename std::conditional<std::is_void<ConstReferencedType>::value, DeleteReturnType01, T>::type& operator *() const { return *_object; }
	...
	/// @note Not available for T = void.
	const typename std::conditional<std::is_void<T>::value, DeleteReturnType01, T>::type& operator [](Int) const = delete;
	...
	//----------------------------------------------------------------------------------------
	/// Makes the pointee writable. Depending on the argument for the template parameter HANDLER, this may just return the original pointer,
	/// or it may replace the pointer by a new one and return the new pointer. The latter
	/// is the case for copy-on-write semantics if there is more than one reference to the pointee. Unlike MakeWritable this function
	/// doesn't copy the contents of the original pointee if a new object is created. This is useful when you overwrite the
	/// content in the sequel anyway.
	/// @return												Internal pointer, which might be a new pointer, or nullptr if the copy failed.
	//----------------------------------------------------------------------------------------
	ResultRef<typename std::remove_const<T>::type> MakeWritableDontCopy()
	{
		return HANDLER::MakeWritableDontCopy(*this);
	}

	//----------------------------------------------------------------------------------------
	/// Returns whether the referenced object has more than this single reference.
	/// For a null reference it will return false.
	...
	/// @return												True if there's a referenced object and it has more than this single reference, false otherwise.
	//----------------------------------------------------------------------------------------
	Bool HasMultipleReferences() const
	{
		return _object && HANDLER::GetReferenceCounter(_object).HasMultipleReferences();
	}

	/// @note Not available for T = void.
	typename std::conditional<std::is_void<T>::value, DeleteReturnType01, T>::type& GetOrDefault() const
	{
		static_assert(MAXON_IS_COW_KIND(HANDLER::KIND), "GetOrDefault is only supported for copy-on-write references.");
		return _object ? *_object : maxon::DefaultValue<const T&>();
	}
	...
	{
		// We force inlining so that MAXON_SOURCE_LOCATION is more likely to use the current g_mainTranslationUnit.
	...
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!STD_IS_REPLACEMENT(base_of, BaseRef, REF) && maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::value>::type> MAXON_IMPLICIT BaseRef(const REF& src) : _object(maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()))
	{
	...
	template <typename REF, typename = typename std::enable_if<!STD_IS_REPLACEMENT(base_of, BaseRef, REF) && maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::value>::type> MAXON_IMPLICIT BaseRef(REF& src) : _object(maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()))
	{
		static_assert(!(HANDLER::KIND & REF::Handler::KIND & VALUEKIND::UNIQUE_REFERENCE), "Can't copy a BaseRef with unique ownership.");
		static_assert(!(((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::UNIQUE_REFERENCE) && ((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::STRONG_MASK)),
									"Can't copy a BaseRef between unique and strong ownership.");
	...
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!STD_IS_REPLACEMENT(base_of, BaseRef, REF) && maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::value>::type> BaseRef& operator =(const REF& src)
	{
		static_assert(!(HANDLER::KIND & REF::Handler::KIND & VALUEKIND::UNIQUE_REFERENCE), "Can't copy a BaseRef with unique ownership.");
		static_assert(!(((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::UNIQUE_REFERENCE) && ((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::STRONG_MASK)),
									"Can't copy a BaseRef between unique and strong ownership.");
	...
	template <typename REF, typename = typename std::enable_if<!STD_IS_REPLACEMENT(base_of, BaseRef, REF) && maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::value>::type> BaseRef& operator =(REF& src)
	{
		static_assert(!(HANDLER::KIND & REF::Handler::KIND & VALUEKIND::UNIQUE_REFERENCE), "Can't copy a BaseRef with unique ownership.");
		static_assert(!(((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::UNIQUE_REFERENCE) && ((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::STRONG_MASK)),
									"Can't copy a BaseRef between unique and strong ownership.");
	...
	/// @param[in] src								Another BaseRef for the move.
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!STD_IS_REPLACEMENT(const, REF) && !STD_IS_REPLACEMENT(base_of, BaseRef, REF) && maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::value>::type> MAXON_IMPLICIT BaseRef(REF&& src) : _object(maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()))
	{
	...
	/// @param[in] src								BaseRef whose internal pointer shall be moved to this BaseRef.
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!STD_IS_REPLACEMENT(const, REF) && !STD_IS_REPLACEMENT(base_of, BaseRef, REF) && maxon::HasErasedBase<typename REF::ReferencedType, ReferencedType>::value>::type> BaseRef& operator =(REF&& src)
	{
	...
	/// @param[in] b									Reference to compare with.
	/// @return												True when this is less than b, false otherwise.
	//----------------------------------------------------------------------------------------
	Bool operator <(const BaseRef& b) const { return RefCompareType::LessThan(_object, b._object); }
	...
	//----------------------------------------------------------------------------------------
	/// Returns the hash code of the reference. For COW references this is the hash code of the referenced
	/// object, otherwise it's the hash code of the pointer.
	/// @return												The reference's hash code.
	//----------------------------------------------------------------------------------------
	HashInt GetHashCode() const { return RefCompareType::GetHashCode(_object); }

	//----------------------------------------------------------------------------------------
	/// Returns the hash value of the reference. For COW references this is the hash value of the referenced
	/// object, otherwise it's the hash value of the pointer.
	/// @return												The reference's hash value.
	//----------------------------------------------------------------------------------------
	UniqueHash GetUniqueHashCode() const { return RefCompareType::GetUniqueHashCode(_object); }
	...
	static typename std::conditional<Bool(HANDLER::KIND & VALUEKIND::UNIQUE_REFERENCE), BaseRef, const BaseRef&>::type DefaultValue()
	{
		return PrivateDefaultValue((typename std::integral_constant<Bool, Bool(HANDLER::KIND & VALUEKIND::UNIQUE_REFERENCE)>::type*) nullptr);
	}

	//----------------------------------------------------------------------------------------
	/// Sets the internal pointer to nullptr and returns its previous value. This moves the ownership of the object
	/// from this BaseRef to the invoking code, so you have to take care of proper deallocation once the object
	/// isn't needed any longer.
	///
	/// If this BaseRef is a strong reference, the reference count must be exactly 1 before the call,
	/// otherwise a nullptr will be returned.
	/// @return												The pointer to the previously referenced object.
	//----------------------------------------------------------------------------------------
	typename std::conditional<Bool(HANDLER::KIND & VALUEKIND::COW_MASK), typename std::remove_const<T>::type, T>::type* Disconnect()
	{
		T* o = _object;
		...
		_object = nullptr;
		return const_cast<typename std::remove_const<T>::type*>(o);
	}
...
{
public:
	static constexpr VALUEKIND KIND = VALUEKIND::POINTER_FLAGS | VALUEKIND::DEEP_CONSTNESS;
};
...
#define NewObjCopy(T, SRC) (maxon::details::NewDelete<T>::IS_ALLOC_TYPE ? maxon::details::NewDelete<T>::Cast(maxon::details::NewDelete<T>::AllocType::Alloc(maxon::SourceLocation(MAXON_FILE, __LINE__, maxon::PrivateGetClassName<T>()), SRC)) \
														: ResultPtr<T>((T*) PrivateInitCopy(maxon::details::NewDelete<T>::AllocatorType::Alloc(maxon::details::NewDelete<T>::SIZE, maxon::SourceLocation(MAXON_FILE, __LINE__, maxon::PrivateGetClassName<T>())), reinterpret_cast<const typename maxon::details::NewDelete<T>::ConstructorType&>(SRC))))
...
{
public:
	static constexpr VALUEKIND KIND = VALUEKIND::COW_REFERENCE | VALUEKIND::INT_SIZE_OR_LESS | VALUEKIND::ZERO_INITIALIZED | VALUEKIND::REF_OR_POINTER | VALUEKIND::VALUE;

	template <typename T, typename = void> struct MakeWritableHelper
	{
		template <typename R> static MAXON_ATTRIBUTE_NO_INLINE ResultRef<T> PrivateMakeWritable(R& ref, Bool resetOnError)
		{
			T* o = const_cast<T*>(ref.GetPointer());
		...
		{
			T* o = const_cast<T*>(ref.GetPointer());
			if (MAXON_LIKELY(o) && !MAXON_UNLIKELY(GetReferenceCounter(o).HasMultipleReferences()))
				return o;

			return PrivateMakeWritable(ref, resetOnError);
		}
	};
	...
	/// @param[in] resetOnError				If true (the default), the reference will be reset when the copy fails. If false, the reference will stay the same, but the function returns nullptr.
	//----------------------------------------------------------------------------------------
	template <typename R> static MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<typename std::remove_const<typename R::ReferencedType>::type> MakeWritable(R& ref, Bool resetOnError)
	{
		using NonConstT = typename std::remove_const<typename R::ReferencedType>::type;
		return MakeWritableHelper<NonConstT>::MakeWritable(ref, resetOnError);
	}

	template <typename R> static MAXON_ATTRIBUTE_NO_INLINE ResultRef<typename std::remove_const<typename R::ReferencedType>::type> PrivateMakeWritableDontCopy(R& ref)
	{
	...
	template <typename R> static MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<typename std::remove_const<typename R::ReferencedType>::type> MakeWritableDontCopy(R& ref)
	{
		using T = typename std::remove_const<typename R::ReferencedType>::type;
		T* o = const_cast<T*>(ref.GetPointer());
		if (MAXON_LIKELY(o) && !MAXON_UNLIKELY(GetReferenceCounter(o).HasMultipleReferences()))
			return o;
		return PrivateMakeWritableDontCopy(ref);
	}

	template <typename R, typename CREATE, typename COPY> static MAXON_ATTRIBUTE_FORCE_INLINE ResultRef<typename std::remove_const<typename R::ReferencedType>::type> MakeWritable(R& ref, CREATE&& create, COPY&& copy)
	{
		using T = typename std::remove_const<typename R::ReferencedType>::type;
		T* o = const_cast<T*>(ref.GetPointer());
...
{
public:
	static constexpr VALUEKIND KIND = (StrongCOWRefHandler::KIND & ~VALUEKIND::COW_REFERENCE) | VALUEKIND::ACOW_REFERENCE;

	//----------------------------------------------------------------------------------------
	/// @param[in] resetOnError				If true (the default), the reference will be reset when the copy fails. If false, the reference will stay the same, but the function returns nullptr.
	//----------------------------------------------------------------------------------------
	template <typename R> static MAXON_ATTRIBUTE_NO_INLINE ResultRef<typename std::remove_const<typename R::ReferencedType>::type> MakeWritable(R& ref, Bool resetOnError)
	{
		using NonConstT = typename std::remove_const<typename R::ReferencedType>::type;
		NonConstT* o = nullptr;
...
{
public:
	static constexpr VALUEKIND KIND = VALUEKIND::UNIQUE_REFERENCE | VALUEKIND::TRIVIALLY_EQUATABLE | VALUEKIND::TRIVIALLY_HASHABLE | VALUEKIND::INT_SIZE_OR_LESS | VALUEKIND::ZERO_INITIALIZED | VALUEKIND::REF_OR_POINTER | VALUEKIND::DEEP_CONSTNESS;
...
public:
	...
	{
		if (_mem)
			H::RemoveReference(*(RawMem<T>**)&_mem);
	}
	BaseRef& operator =(T* src)
	{
		if (src != _mem)
