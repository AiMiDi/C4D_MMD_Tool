...
public:
	...
	//----------------------------------------------------------------------------------------
	InterfaceReference(const Char* iidWithoutHash, const InterfaceReference* const* baseInterfaces, const Char* methodIds, const METHOD_FLAGS* methodFlags, TranslationUnit* unit, EntityBase::FLAGS refType);
...
private:
	...
	const METHOD_FLAGS* const _methodFlags; ///< Pointer to a block of method flags.
	void* _reserved;
...
{
public:
	// Only needed for MAXON_DECLARATION to initialize a default value.
	ComponentDescriptor();
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const ComponentDescriptor& DefaultValue()
	{
...
private:
	...
	template <typename C> void Construct()
	{
		...
		_destruct = (DestructorPointer) &C::_Wrapper::PrivateWrapperDestruct;
...
public:
	...
	///
	/// @param[in] size								Optional memory size for the allocation. This can be used if the object needs some extra space.
...
/// @param[in] ...								Optional information about the interface, for now only MAXON_IMPLEMENTATION_MODULE can be used here.
/// @see @ref vinterfaces
/// @see MAXON_COMPONENT
/// @see MAXON_METHOD
/// @see MAXON_FUNCTION
...
#define MAXON_INTERFACE(Name, REFKIND, ID, ...) \
	public: \
		...
		static Name* Alloc(const maxon::SourceLocation&, const Name& src) { auto r = src.Clone(); return (r == maxon::FAILED) ? nullptr : static_cast<Name*>(r.GetValue()); } \
		...
		class Reference; \
		using Ptr = maxon::RefBase<ReferenceFunctions<maxon::RefBaseFn<maxon::DirectRef<Name, maxon::PointerHandler, void>>>>; \
		using ConstPtr = maxon::RefBase<ReferenceFunctions<maxon::RefBaseFn<maxon::DirectRef<const Name, maxon::PointerHandler, void>>>>; \
		inline Ptr GetPtr(); \
	...
	protected: \
		~Name(); \
	private: \
		static Name* Alloc(const maxon::SourceLocation&) { MAXON_ASSERT_STANDARD_LAYOUT(Name); return maxon::details::VirtualCreate<Name>::Create(); } \
...
template <typename I, typename = I*(*)()> struct VirtualCreate
{
	...
	{
		if (STD_IS_REPLACEMENT(same, const I*, decltype(I::DefaultValuePtr())))
		{
			const I* ptr = (const I*) I::DefaultValuePtr();
			if (ptr)
...
using ImplementsHelperMemberType = DummyParamType && (*)(Char, DeleteReturnType01&);
#define PRIVATE_MAXON_IMPLEMENTS_HELPER_MEMBER(Name) static maxon::DummyParamType&& Name(maxon::Char, maxon::DeleteReturnType01&)
...
template <typename INTERFACE, typename SUPER> struct PrivateCombinedMTable : public CombinedMTableBase<INTERFACE, SUPER, !STD_IS_REPLACEMENT(void, typename SUPER::template FindInterface<INTERFACE>)>
{
};
...
public:
	...
	using Ptr = maxon::RefBase<ReferenceFunctions<maxon::RefBaseFn<maxon::DirectRef<Interface, maxon::ComponentPointerHandler, PrivateComponentBase>>>>;
	using ConstPtr = maxon::RefBase<ReferenceFunctions<maxon::RefBaseFn<maxon::DirectRef<const Interface, maxon::ComponentPointerHandler, PrivateComponentBase>>>>;
	using SelfPtr = maxon::SelfPtrBase<ReferenceFunctions<maxon::RefBaseFn<maxon::DirectRef<Interface, maxon::SelfPointerHandler, PrivateComponentBase>>>>;
	...
	{
		static maxon::Bool InitMTables(maxon::ClassInfo* info, maxon::Int offset) { return SUPER::template Wrapper<S>::InitMTables(info, offset) && Wrapper::InitMTablesWithBases(info, offset); }
	};
};
...
public:
	...
	{
		// Workaround for linker errors with MSVC when LTCG/LTO is enabled
#if defined(_MSC_VER) && !defined(__clang__)
		template <typename I> const typename I::MTable& GetMTable() const { DebugStop(); return GetZeroRef<typename I::MTable>(); }
#else
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE constexpr std::nullptr_t DefaultValuePtr() { return nullptr; }

	//----------------------------------------------------------------------------------------
	/// Returns the class of this object. Each object holds a strong reference on its class.
	/// @return												Class of this object.
	/// @MAXON_ANNOTATION{refsuperclassParameter=REFSUPERCLASS}
	//----------------------------------------------------------------------------------------
	template <typename REFSUPERCLASS = ObjectRef> MAXON_FUNCTION const maxon::Class<typename REFSUPERCLASS::PrivateErasedReferencedType::Hxx1::ReferenceClass>& GetClass() const
	{
		return reinterpret_cast<const maxon::Class<typename REFSUPERCLASS::PrivateErasedReferencedType::Hxx1::ReferenceClass>&>(_classInfo->_base._class);
	}
	...
	// Workaround for linker errors with MSVC when LTCG/LTO is enabled
#if defined(_MSC_VER) && !defined(__clang__)
	template <typename REF> const ComponentInfo& GetComponentInfoOfSuper(const REF* sptr, OverloadRank0) const { DebugStop(); return GetZeroRef<ComponentInfo>(); }
#else
	...
	MAXON_FUNCTION UniqueHash GetUniqueHashCode() const { return GetUniqueHashCodeImpl(); }
	...
	//----------------------------------------------------------------------------------------
	/// Returns a 128-bit hash value for this object. The implementation has to ensure uniform distribution,
	/// so that one can safely assume that two objects are equal if their hash values are equal.
	/// If you implement this method in a component, make sure to incorporate the result of the super implementation in the result.
...
{
	MAXON_INTERFACE(ErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.error", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
protected:
	...
	{
		*static_cast<typename S::DirectlyReferencedType::Hxx1::ReferenceClass*>(this) = S::DirectlyReferencedType::Hxx1::ErrObj::GetInstance().template Get<PRIVATE_MAXON_TRANSLATION_SUBUNIT>();
	...
	//----------------------------------------------------------------------------------------
	/// A MessageDelegate is called internally by FormatMessage()/GetMessage().
	/// The given callback must use the PartFormatter to translate parts of the message<
	/// into more readable statements.
	/// @param[in] language						The language to use, may be left empty.
	/// @param[in] fmt								The PartFormatter delegate, may be left empty.
	/// @return												The formatted message.
	//----------------------------------------------------------------------------------------
	using MessageDelegate = Delegate<String(const LanguageRef& language, const ErrorInterface::PartFormatter& fmt)>;
	...
	/// @MAXON_ANNOTATION{returnsThis,noResult}
	//----------------------------------------------------------------------------------------
	MAXON_METHOD void SetMessageDelegate(MessageDelegate&& message);
...
// Force dependency on ObjectBaseClass (even when the MAXON_DECLARATION isn't used in the translation unit).
static EntityUse g_dependency_ObjectImpl{ObjectBaseClass.GetReference(), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, "maxon/objectbase.h", false};
#endif
...
private:
	...
	{
		new (component) C; // prevent zero-initialization which can happen with C()
	}
...
template <typename C> inline Result<void> AddComponentToClass(ClassInterface& cls, const DeclarationWrapper<ComponentDescriptor, C>& comp)
{
	return cls.AddComponent(static_cast<const C&>(comp)());
}
...
template <typename T, typename C> inline Result<void> AddComponentToClass(ClassInterface& cls, const DeclarationWrapper<Class<T>, C>& comp)
{
	return cls.AddComponents(static_cast<const C&>(comp)());
}
...
#define PRIVATE_MAXON_COMPONENT_ID(...) maxon::LiteralId(0, MAXON_SECONDARG(__VA_ARGS__, __VA_ARGS__))
...
#define PRIVATE_MAXON_COMPONENT_REGISTER(C, TEMPLATE, TYPENAME, FLAGS, UNIT, REGISTRY, ...) \
	...
	TEMPLATE const maxon::ComponentDescriptor C::_descriptor((TYPENAME C*) nullptr, PRIVATE_MAXON_COMPONENT_ID(__VA_ARGS__), UNIT, MAXON_FILE, (FLAGS) PRIVATE_MAXON_COMPONENT_EXTRA_FLAGS, (REGISTRY*) nullptr)
...
#define PRIVATE_MAXON_CLASS_REGISTER(C, TEMPLATE, REGISTRY, TYPE, ...) \
	PRIVATE_MAXON_COMPONENT_REGISTER(C, TEMPLATE,, maxon::EntityBase::FLAGS::NONE, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, ::MaxonRegistry, __VA_ARGS__); \
	template <typename> extern maxon::ObjectDefinition MAXON_CONCAT(private_class, __LINE__); \
	template <> maxon::ObjectDefinition MAXON_CONCAT(private_class, __LINE__)<C>{PRIVATE_MAXON_COMPONENT_ID(__VA_ARGS__), &C::PrivateConstructClass<TYPE>, (REGISTRY*) nullptr, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, MAXON_FILE, maxon::EntityBase::FLAGS::NONE}; \
	TEMPLATE const maxon::Class<>& C::PrivateGetClass() \
	{ \
		const void* generic = MAXON_CONCAT(private_class, __LINE__)<C>.PrivateGet(); \
...
//----------------------------------------------------------------------------------------
#define MAXON_COMPONENT_ONLY_REGISTER(C, ...) PRIVATE_MAXON_COMPONENT_REGISTER(C,,, maxon::EntityBase::FLAGS::NONE, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, PRIVATE_MAXON_COMPONENT_REGISTRY(__VA_ARGS__), __VA_ARGS__)
...
//----------------------------------------------------------------------------------------
#define MAXON_COMPONENT_ONLY_REGISTER_TEMPLATE(C, ...) PRIVATE_MAXON_COMPONENT_REGISTER(C, template <>,, maxon::EntityBase::FLAGS::NONE, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, PRIVATE_MAXON_COMPONENT_REGISTRY(__VA_ARGS__), __VA_ARGS__)
...
#define MAXON_COMPONENT_OBJECT_REGISTER(C, ...) PRIVATE_MAXON_CLASS_REGISTER(C,, ::MaxonRegistry, maxon::Class<>, __VA_ARGS__) \
	static maxon::ObjectDefinition g_object_##C(PRIVATE_MAXON_COMPONENT_ID(__VA_ARGS__), &C::PrivateCreateInstance<PRIVATE_MAXON_COMPONENT_TYPE(typename C::Interface::Hxx1::ReferenceClass, __VA_ARGS__)>, (PRIVATE_MAXON_COMPONENT_REGISTRY(__VA_ARGS__)*) nullptr, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, MAXON_FILE, maxon::EntityBase::FLAGS::NONE); \
...
public:
	...
	/// component has non-copyable members (members without copy assignment operator) and shall support copying.
	//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
/// Virtual interfaces can have static methods. Of course there can't be more than one implementation of these methods
/// (static methods are non-virtual per se). The implementation is registered similar to MAXON_IMPLEMENTATION,
/// but the macro to use is MAXON_STATIC_REGISTER.
///
/// @code
/// class SubTypeAssetImpl : public Component<SubTypeAssetImpl, SubTypeAssetInterface>
/// {
/// 	...
/// };
///
/// MAXON_COMPONENT_CLASS_REGISTER(SubTypeAssetImpl, "org.my.class.subtypeasset");
/// MAXON_STATIC_REGISTER(SubTypeAssetImpl);
/// @endcode
///
/// @param[in] C									Name of the implementation class.
//----------------------------------------------------------------------------------------
#define MAXON_STATIC_REGISTER(C, ...) PRIVATE_MAXON_STATIC_REGISTER_A(C, MAXON_CONCAT(__LINE__, MAXON_CONCAT(_, __COUNTER__)), __VA_ARGS__)

#define PRIVATE_MAXON_STATIC_REGISTER_A(C, ID, ...) PRIVATE_MAXON_STATIC_REGISTER_B(C, ID, __VA_ARGS__)
#define PRIVATE_MAXON_STATIC_REGISTER_B(C, ID, ...) PRIVATE_MAXON_STATIC_REGISTER_C(C, ID, __VA_ARGS__)
#define PRIVATE_MAXON_STATIC_REGISTER_C(C, ID, ...) \
	PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(C::PrivateInterface); \
	static maxon::NonvirtualInterfaceImplementation MAXON_CONCAT(private_implementation_, ID)((maxon::StaticHelper<C>*) nullptr, #C, nullptr, C::PrivateInterface::_staticInterface, (maxon::NonvirtualInterfaceReference::MTableInitializer) &C::PrivateInterface::Hxx2::StaticMTable::Init<C::PrivateInterface::Hxx2::CWrapper<maxon::StaticHelper<C>>>, C::PrivateInterface::REFERENCE_KIND | maxon::EntityBase::FLAGS(__VA_ARGS__), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, MAXON_FILE)
...
public:
	...
	static C* GetOrNull(ObjectInterface* object)
	{
		if (MAXON_UNLIKELY(object == nullptr))
			return nullptr;
		const ComponentInfo* ci = reinterpret_cast<const ObjectSuper*>(object)->_classInfo->GetComponentInfo(C::_descriptor);
...
// Because the initialization of explicit instantiations is unordered, PRIVATE_MAXON_TRANSLATION_SUBUNIT might not have been initialized when MAXON_INTERFACE_REGISTER_VIRTUAL is done for interface template instantiations.
// Therefore call TranslationUnit::PrivateInitModule to ensure a valid ModuleInfo pointer for PRIVATE_MAXON_TRANSLATION_SUBUNIT.
#define PRIVATE_MAXON_INITIALIZED_TRANSLATION_UNIT(...) ((MAXON_EXPAND(MAXON_VA_NARGS(__VA_ARGS__)) > 0) ? PRIVATE_MAXON_TRANSLATION_SUBUNIT.PrivateInitModule(&PRIVATE_MAXON_MODULE) : &PRIVATE_MAXON_TRANSLATION_SUBUNIT)
...
#define PRIVATE_MAXON_INTERFACE_REGISTER_VIRTUAL(I, POSTFIX, TARGS, TEMPL, IID, ...) \
	...
	TEMPL maxon::InterfaceReference I::_interface(IID, g_baseInterfaces_##POSTFIX TARGS, I::MTable::_ids, I::MTable::_flags, PRIVATE_MAXON_INITIALIZED_TRANSLATION_UNIT(TEMPL), I::REFERENCE_KIND | I::INTERFACE_FLAGS)
...
#define PRIVATE_MAXON_INTERFACE_REGISTER_VIRTUAL_SINGLE_BASE(I, TEMPL, IID, BASE) \
	TEMPL maxon::InterfaceReference I::_interface(IID, BASE, I::MTable::_ids, I::MTable::_flags, PRIVATE_MAXON_INITIALIZED_TRANSLATION_UNIT(TEMPL), I::REFERENCE_KIND | I::INTERFACE_FLAGS)
...
template <typename T, typename = void> class UnresolvedReturnType
{
public:
	static const UNRESOLVED_RETURN_TYPE value
		= STD_IS_REPLACEMENT(void, T) ? UNRESOLVED_RETURN_TYPE::VOID_ZERO
		: (STD_IS_REPLACEMENT(integral, T) || STD_IS_REPLACEMENT(pointer, T) || STD_IS_REPLACEMENT(enum, T)) ? UNRESOLVED_RETURN_TYPE::INTEGRAL_ZERO
		: STD_IS_REPLACEMENT(floating_point, T) ? (STD_IS_REPLACEMENT(same, const float, const T) ? UNRESOLVED_RETURN_TYPE::FLOAT_ZERO : UNRESOLVED_RETURN_TYPE::DOUBLE_ZERO)
		: (STD_IS_REPLACEMENT(lvalue_reference, T) && STD_IS_REPLACEMENT(const, typename std::remove_reference<T>::type)
			 && GetDirectlyReferencedType<typename std::remove_reference<T>::type>::value) ? UNRESOLVED_RETURN_TYPE::POINTER_TO_ZERO
		: UNRESOLVED_RETURN_TYPE::OTHER;
};

template <> class UnresolvedReturnType<UniqueHash>
{
public:
	static const UNRESOLVED_RETURN_TYPE value = UNRESOLVED_RETURN_TYPE::INTEGRAL2X_ZERO;
};

// Ref with non-trivial destructor is passed via stack
template <typename T> class UnresolvedReturnType<T, typename std::enable_if<GetDirectlyReferencedType<T>::value && !(T::Handler::KIND & VALUEKIND::POINTER)>::type>
{
public:
	static const UNRESOLVED_RETURN_TYPE value = UNRESOLVED_RETURN_TYPE::STACK_ZERO;
};

template <typename T> class UnresolvedReturnType<Result<T>>
{
public:
	static const UNRESOLVED_RETURN_TYPE value
		= STD_IS_REPLACEMENT(void, T) ? UNRESOLVED_RETURN_TYPE::RESULT_VOID
		: (UnresolvedReturnType<T>::value == UNRESOLVED_RETURN_TYPE::INTEGRAL_ZERO) ? UNRESOLVED_RETURN_TYPE::RESULT_INTEGRAL_ZERO
		: (UnresolvedReturnType<T>::value == UNRESOLVED_RETURN_TYPE::STACK_ZERO) ? UNRESOLVED_RETURN_TYPE::RESULT_STACK_ZERO
		: UNRESOLVED_RETURN_TYPE::OTHER;
};
