...
{
	MAXON_INTERFACE(FileFormatHandlerInterface, MAXON_REFERENCE_CONST, "net.maxon.interface.fileformathandler", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
{
	MAXON_INTERFACE_NONVIRTUAL(FileFormatDetectionInterface, MAXON_REFERENCE_STATIC, "net.maxon.interface.fileformatdetection", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
template <typename... LIMITTYPES> Result<BaseArray<FileFormatHandler>> FileFormatDetectionInterface::DetectAll(const Url& url, FILEFORMATDETECTIONFLAGS flags, const DetectedCallback& detectedCallback)
{
	DataType array[] = { DataType::DefaultValue(), GetDataType<LIMITTYPES>()... }; // add the nullptr infront to allow 0 template arguments.
	return DetectAll(url, ToBlock(array).Slice(1), flags, detectedCallback);
}

template <typename... LIMITTYPES> Result<FileFormatHandler> FileFormatDetectionInterface::Detect(const Url& url, FILEFORMATDETECTIONFLAGS flags)
{
	iferr_scope;

	BaseArray<FileFormatHandler> handler = DetectAll<LIMITTYPES...>(url, flags,
