...
{
	MAXON_INTERFACE(JobErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.joberror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};

/// @cond INTERNAL

/// Partition suggestions for worker threads (ParallelFor with asymmetric multiprocessing aka E and P cores).
struct WorkerThreadPartitions
{
	UInt pCorePartitionSize;			///< Suggested partition size for a thread running on a performance core.
	UInt eCorePartitionSize;			///< Suggested partition size for a thread running on an efficiency core.
	UInt32 pCoreGranularity;			///< Suggested granularity (inner loop size) for a thread running on a performance core.
	UInt32 eCoreGranularity;			///< Suggested granularity (inner loop size) for a thread running on an efficiency core.
	Int16 firstECoreThreadIndex;	///< Index of the first worker thread which runs on an efficiency core. 
	Int16 lastECoreThreadIndex;		///< Index of the last worker thread which runs on an efficiency core.
};
...
using MainThreadYieldDelegate = Bool (*)();
...
class ThreadServices
{
	...
	friend class StackAllocUtilities;
	friend class BaseThreadUtilities;
	
	MAXON_INTERFACE_NONVIRTUAL(ThreadServices, MAXON_REFERENCE_STATIC, "net.maxon.interface.threadservices", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
private:
	...
	//----------------------------------------------------------------------------------------
	/// Creates a CoreJobGroup with a job for each worker thread of the specified queue.
	/// THREADSAFE.
	...
	/// @return												CoreJobGroup pointer or nullptr if allocation failed.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD CoreJobGroup* CreateGroupForQueue(JobQueueInterface* queue, Int& cnt, Int jobSize, UInt flags);
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<void> AddJob(CoreJobGroup& group, const JobInterfacePOD& pod, UInt flags);
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<void> AddJobs(CoreJobGroup& group, void** memory, Int cnt, Int jobPODOffset, UInt flags);
	...
	//----------------------------------------------------------------------------------------
	/// Adds a reference to a job which might be part of a group (retrieved via GetCurrentJob).
	/// THREADSAFE.
	/// @param[in] pod								Job structure with jump table (CoreJob pointer of the JobInterface is guaranteed to be valid).
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Removes a reference to a job might be part of a group (retrieved via GetCurrentJob).
	/// THREADSAFE.
	/// @param[in] pod								Job structure with jump table (CoreJob pointer of the JobInterface is guaranteed to be valid).
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Transfers a strong reference to a job's predecessor to the caller (only for continuations). The caller must release the reference.
	/// THREADSAFE.
	/// @param[in] pod								Job structure with jump table (CoreJob pointer of the JobInterface is guaranteed to be valid).
	/// @param[out] prev							Pointer to the predecessor job (reference counted) or nullptr (no predecessor).
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD void DisconnectPredecessor(const JobInterfacePOD& pod, JobInterface** prev);

	//----------------------------------------------------------------------------------------
	/// Calculates the suggested sizes for the partitions and the granularity for ParallelFor (asymmetric multiprocessing aka E and P cores).
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Int CalculatePartitions(UInt cnt, Int threadCnt, Int minChunkSize, WorkerThreadPartitions& partitions);

	static MAXON_METHOD void SetMainThreadYieldDelegate(MainThreadYieldDelegate d);
};
/// @endcond
