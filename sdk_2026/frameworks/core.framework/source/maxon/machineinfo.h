...
namespace MACHINEINFO
{
	...
	/// special case for Linux: it can generate more than one (unique) identifier, so store all of them
	MAXON_ATTRIBUTE(Array<String>, LINUXSYSTEMIDS, "net.maxon.machineinfo.linuxsystemids");
#endif
	...
	MAXON_ATTRIBUTE(Id, UNIQUEOSIDENTIFIERSOURCE, "net.maxon.machineinfo.uniqueosidentifiersource",
		...
		RESOURCE_DEFINE(ENUM_TPM, LiteralId("tpm"))
		RESOURCE_DEFINE(ENUM_UEFI, LiteralId("uefi"))
		RESOURCE_DEFINE(ENUM_REGISTRY, LiteralId("registry"))
		RESOURCE_DEFINE(ENUM_CRYPTOGRAPHYID, LiteralId("cryptographyid"))
		RESOURCE_DEFINE(ENUM_CRYPTOGRAPHYID_FALLBACK, LiteralId("cryptographyid_fallback"))
		RESOURCE_DEFINE(ENUM_SERIALKEY, LiteralId("serialkey"))
