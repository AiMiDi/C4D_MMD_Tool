...
// always enabled
#if defined(MAXON_TARGET_CPU_X86) && defined(MAXON_TARGET_WINDOWS) && defined(MAXON_COMPILER_CLANG)
...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE void UpdateUInt64(UInt64 u)
	{
		...
		if constexpr (MAXON_LIKELY(g_hasSSE4_2))
		{
			UInt64 c = _crc;
			c = _mm_crc32_u64(c, u);
			_crc = (UInt32)c;
	...
	/// @tparam T											Type of the const data block.
	/// @param[in] mem								A raw memory buffer.
	//----------------------------------------------------------------------------------------
	template <Int FORCEALIGNMENT = 0, typename T, size_t SIZE> MAXON_ATTRIBUTE_FORCE_RELEASE_INLINE void Update(const T (&mem)[SIZE])
	{
		return Update<FORCEALIGNMENT>(Block<const T>(mem, SIZE));
	}
	...
	template <Int FORCEALIGNMENT = 0, typename ARRAY> MAXON_ATTRIBUTE_FORCE_RELEASE_INLINE void Update(const ARRAY& mem)
	{
		...
		constexpr Int ALIGNMENT = FORCEALIGNMENT == 0 ? (Int)alignof(T) : FORCEALIGNMENT;
		...
		{
			MAXON_ASSUME_ALIGNED(d, ALIGNMENT);
			d = (const UChar*)((UInt)d & ~(UInt)(ALIGNMENT - 1));
		}
		Int n = mem.GetCount() * ELEMENTSIZE;
		...
		if constexpr (MAXON_LIKELY(g_hasSSE4_2))
		{
			...
			c = _crc;
#if 0
			// the crc32 instruction works on unaligned memory. my speedtests could not find a speedhit when accessing the unaligned memory
			x = UInt(d) & 3;
			...
			if constexpr (ALIGNMENT < 8)
			{
				...
				if ((UInt(d) & 7) && n >= 4)
				{
					c = _mm_crc32_u32(c, *reinterpret_cast<const UInt32*>(d));
					n -= 4;
					d += 4;
				}
			}
			...
			UInt64 c64 = c;
			// process in 8 byte chunks as long as enough data is available
			for (; n >= 8; n -= 8, d += 8)
			{
				c64 = _mm_crc32_u64(c64, *reinterpret_cast<const UInt64*>(d));
			}
			c = (UInt32)c64;

			// calculate the remaining bytes
			if constexpr ((ELEMENTSIZE % 8) != 0)
			{
				if (MAXON_LIKELY(n >= 4))
				{
					c = _mm_crc32_u32(c, *reinterpret_cast<const UInt32*>(d));
				...
				if constexpr ((ELEMENTSIZE % 4) != 0)
				{
					if (MAXON_LIKELY(n >= 2))
					{
						c = _mm_crc32_u16(c, *reinterpret_cast<const UInt16*>(d));
						n -= 2;
						d += 2;
					}
...
#endif
		{
			...
			if ((UInt(d) & 7) && n >= 4)
			{
				u32 = *reinterpret_cast<const UInt32*>(d);
	...
	MAXON_ATTRIBUTE_FORCE_INLINE void UpdateVector32(const Vector32& r)
	{
		...
		{
			c.f = r.x;
			UInt32 crc = _mm_crc32_u32(_crc, c.GetUInt());
			c.f = r.y;
			crc = _mm_crc32_u32(crc, c.GetUInt());
			c.f = r.z;
			_crc = _mm_crc32_u32(crc, c.GetUInt());
		}
		else
#endif
		{
