...
static_assert(STD_IS_REPLACEMENT(trivially_destructible, TranslationUnit), "TranslationUnit has to be trivially destructible.");
static_assert(STD_IS_REPLACEMENT(trivially_destructible, EntityReference), "EntityReference has to be trivially destructible.");
static_assert(STD_IS_REPLACEMENT(trivially_destructible, EntityDefinition), "EntityDefinition has to be trivially destructible.");
static_assert(STD_IS_REPLACEMENT(trivially_destructible, EntityUse), "EntityUse has to be trivially destructible.");
...
class ConcatenatedIdMap : public HashMap<CharTriple, ConcatenatedIdData, DefaultCompare, HashMapKeyValuePair, CStdLibAllocator, HASHMAP_MODE::SYNCHRONIZED>
{
public:
	ConcatenatedIdMap() : HashMap<CharTriple, ConcatenatedIdData, DefaultCompare, HashMapKeyValuePair, CStdLibAllocator, HASHMAP_MODE::SYNCHRONIZED>()
	{
...
public:
	explicit ConcatenatedIdData(const CharTriple& triple)
	{
		...
		_id.Init(_buffer, false) iferr_cannot_fail("String is not copied and hence Init must succeed");
	}
...
{
	DebugAssert(a && b && c);
	// g_concatenatedIds must not be declared outside of this method because GetConcatenated() might be called before it was constructed.
...
InternedIdInitializer::InternedIdInitializer(InternedId* iid, const Char* str) : InternedIdInitializer(iid, str, g_maxon.GetInternedIdInitializer())
{
}

InternedIdInitializer::InternedIdInitializer(InternedId* iid, const Char* str, InternedIdInitializer*& link) : _id(iid), _str(str)
{
	CheckInitialState();
	_nextOfBinary = link;
	link = this;
...
// We have to do this check in an extra function, otherwise compilers warn that _nextOfBinary is uninitialized.
// In practice that's not the case because all InternedIdInitializers are global variables, and these are zero-initialized
// before their constructors are called. The use of an extra MAXON_ATTRIBUTE_NO_INLINE function prevents the compilers
// from being too clever.
void InternedIdInitializer::CheckInitialState()
{
	if (_nextOfBinary)
	{
...
EntityBase::EntityBase(TYPE type, LiteralId&& idValue, TranslationUnit* unit, FLAGS flags) : _id(DONT_INITIALIZE)
{
	...
	if ((type != TYPE::MODULE) || !(flags & FLAGS::DEFINITION) || !_id.GetCString()) // id of ModuleInfo may have been set already by register.h, then don't override it
	{
		...
		if (idValue.UsesStrongReference())
			__debugbreak();
...
TranslationUnit::TranslationUnit(const ModuleInfo* module, const Char* file, EntityBase::FLAGS flags)
	: _super(EntityBase::TYPE::TRANSLATION_UNIT, Id(file), this, flags | EntityBase::FLAGS::DEFINITION),
		_module(module), _currentSubUnit(nullptr), _previousSubUnit(nullptr)
{
...
TranslationUnit::TranslationUnit(TranslationUnit* main, const Char* unityId, EntityBase::FLAGS flags) : TranslationUnit(main->GetModule(), unityId, flags)
{
	// This constructor is called for subunits of a unity build. Chain the subunits via _previousSubUnit pointer.
	_previousSubUnit = main->PrivateGetCurrentUnit();
	main->_currentSubUnit = this;
}

const Char* TranslationUnit::GetName() const
{
	return GetFilename(_super.GetId().GetCString());
}
	...
	: _super(type, std::move(idValue), unit, flags | EntityBase::FLAGS::DEFINITION), _nextOfUnit(nullptr), _references(nullptr), _moduleReference(nullptr)
{
	if (unit)
	{
		...
		// Update meaningless filename if we have a better one.
		if (file && ((unit->_super._id == LiteralId(PRIVATE_MAXON_PLACEHOLDER_FILENAME))
								 || unit->_super._id.ToBlock().StartsWith(CharToBlock("unity_"))))
		{
			reinterpret_cast<Id&>(unit->_super._id) = Id(file);
		}
	}
}
...
// For technical reasons, GlobalBase always receives g_mainTranslationUnit of the current unity build as TranslationUnit.
// With PrivateGetCurrentUnit, we get the current subunit.
GlobalBase::GlobalBase(const Char* name, Result<void> (*init) (GlobalBase* global), void (*free) (GlobalBase* global), TranslationUnit* main)
	: _super(EntityBase::TYPE::GLOBAL, Id(GetFilename(name)), main->PrivateGetCurrentUnit(), nullptr, EntityBase::FLAGS::HIDDEN), _init(init), _free(free)
{
}
...
Block<const Tuple<const Char*, ModuleInfo::DEPENDENCY>> ModuleInfo::GetExpectedModuleDependencies() const
{
	return ToBlock(_expectedModuleDependencies, _expectedModuleDependenciesCount);
}

Binary::Binary() : 
	_abiVersion(MAXON_API_ABI_VERSION),
	_coreFrameworkVersion(MAXON_CORE_FRAMEWORK_VERSION)
{
}
...
EntityReference::EntityReference(EntityBase::TYPE type, LiteralId&& idValue, TranslationUnit* unit, ModuleInfo* module)
	: _super(type, std::move(idValue), unit, EntityBase::FLAGS::NONE)
{
	_nextOfModule = module->_references;
	module->_references = this;
	_nextOfDefinition = nullptr;
	_definition = nullptr;
}
...
const void* EntityReference::InitializeDeclaration(const void* def) const
{
	DebugAssert(def && g_maxon.GetInitializer());
	(*g_maxon.GetInitializer())(const_cast<EntityBase*>(&_definition->GetBase()), _super.GetId()) iferr_ignore("_object checked afterwards.");
	// this can be any Declaration<T, C>-type, because they share a common layout
	const void* result = reinterpret_cast<const ObjectBaseClass_PrivateHelper::DeclarationType*>(this)->_object;
	if (MAXON_LIKELY(result))
		return result;
	if (_super.GetType() == EntityBase::TYPE::DATATYPE)
...
EntityUse::EntityUse(const Char* idValue, TranslationUnit* unit, EntityDefinition* def, Bool weak)
{
	Construct(idValue, unit, nullptr, def, nullptr, weak);
}
...
// if false is returned the calling method of the Unresolved class will return the appropriate default value (false in case of a Bool or nullptr for any kind of pointer)
Bool NonvirtualInterfaceReference::PrivateResolve(NonvirtualInterfaceReference* this_, Bool showMessage)
{
	Result<void> r;

	// For ALLOW_MISSING_METHODS
	if (this_->_super.GetBase().GetFlags() & EntityBase::FLAGS::INITIALIZED && this_->_super.GetBase().GetFlags() & EntityBase::FLAGS::ALLOW_MISSING_METHODS)
		return false;
...
InterfaceReference::InterfaceReference(const Char* iidWithoutHash, const InterfaceReference* const* baseInterfaces, const Char* methodIds, const METHOD_FLAGS* methodFlags, TranslationUnit* unit, EntityBase::FLAGS refType)
	: _super(EntityBase::TYPE::INTERFACE, iidWithoutHash, unit, refType), _infoOffset(LIMIT<Int>::MIN), _type(), _basesPtr(baseInterfaces), _methodIds(methodIds), _idWithoutHash(iidWithoutHash), _version(nullptr), _depth(-1), _methodFlags(methodFlags)
{
}

InterfaceReference::InterfaceReference(const Char* iidWithoutHash, const InterfaceReference* baseInterface, const Char* methodIds, const METHOD_FLAGS* methodFlags, TranslationUnit* unit, EntityBase::FLAGS refType)
	: InterfaceReference(iidWithoutHash, (const InterfaceReference* const*) baseInterface, methodIds, methodFlags, unit, refType | EntityBase::FLAGS::SINGLE_BASE)
{
}
...
InterfaceDefinition::InterfaceDefinition(const InterfaceReference& ref, TranslationUnit* unit, EntityBase::FLAGS flags, const Char* cppName)
	: _super(ref._super, unit, nullptr, flags),
		_infoOffset(LIMIT<Int>::MIN),
		_cppName(cppName)
{
}
...
const ComponentInfo& ClassInfo::PrivateGetComponentInfo(const ComponentDescriptor& d) const
{
	...
	{
		DebugAssert(e->_descriptor && (e->_descriptor->_hashCode == d._hashCode));
...
{
	iferr_scope;
	iferr (ObjectInterface* c = GetClass().GetPointer()->CreatePointer(System::GetAllocSize(reinterpret_cast<void*>(MAXON_REMOVE_CONST(this)))))
		return err;
	iferr (c->CopyFrom(this))
	{
		PrivateFreeInstance(c);
		return err;
	}
	Result<void> result;
...
void ObjectInterface::PrivateFreeInstance(const ObjectInterface* o)
{
	System::PrivateFreeWithDestructor(o, DestructorPointer(ClassInterface::MTable::_instance.ClassInterface_DestructInstance));
}
...
Result<void> ObjectDefinition::AddClassExtension(const void* ptr)
{
	// Return if the component only (and no class object) was to be registered.
	if (ptr == nullptr)
		return OK;

	const_cast<EntityBase&>(_super.GetBase()).PrivateAddFlags(EntityBase::FLAGS::CLASS_DEFINITION);

	const Class<>& cls = reinterpret_cast<const Class<>&>(ptr);
	return RegistryInterface::InsertEntry(Classes::Registry::GetRegistry(), _super.GetBase().GetId(), ConstDataPtr(cls), nullptr);
}
...
// 32-byte-aligned block of ZERO_MEMORY_SIZE (1024) zero bytes. Typically, string constants are stored in write-protected memory.
...
Bool ErrorInterface::IsCancellation() const
{
	return IsErrorOfType<OperationCancelledError>(this);
}
...
public:
	InitSystemFree()
	{
		...
		System::MTable::_instance.System_Free = &SystemFreeDummy;
		// Same for WeakRefServices::RemoveWeakReference and DataTypeLib::PrivateHandleUnreferencedType
		WeakRefServices::MTable::_instance.WeakRefServices_RemoveWeakReference = &WeakRefServicesRemoveWeakReferenceDummy;
		DataTypeLib::MTable::_instance.DataTypeLib_PrivateHandleUnreferencedType = &DataTypeLibPrivateHandleUnreferencedTypeDummy;
	}
};
...
AggregatedError::AggregatedError(MAXON_SOURCE_LOCATION_DECLARATION)
{
