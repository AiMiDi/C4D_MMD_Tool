...
ResultRef<Generic> GenericBaseArray::Append(const DataType& elementType)
{
	...
	Generic* ptr = GetPointerSlow(elementType, newPosition);
...
ResultMem GenericBaseArray::Resize(const DataType& elementType, Int count, COLLECTION_RESIZE_FLAGS resizeFlags)
{
	...
	else if (count < _cnt)
	{
		...
		elementType->Destruct(GetPointerSlow(elementType, count), sz, _cnt - count);
		if (resizeFlags & COLLECTION_RESIZE_FLAGS::ON_SHRINK_FIT_TO_SIZE)
		{
			fit = true;
		}
	...
	else if (count <= _capacity)
	{
		if (!(resizeFlags & COLLECTION_RESIZE_FLAGS::ON_GROW_UNINITIALIZED
			...
			elementType->Construct(GetPointerSlow(elementType, _cnt), sz, count - _cnt);
...
ResultRef<Generic> GenericBaseArray::Insert(const DataType& elementType, Int index)
{
	if (Insert(elementType, index, MoveBlock<ValueType>(nullptr, 1)) == FAILED)
		return nullptr;
	Generic* ptr = GetPointerSlow(elementType, index);
	elementType->Construct(ptr);
	return ptr;
}
...
ResultMem GenericBaseArray::Insert(const DataType& elementType, Int index, const MoveBlock<ValueType>& values)
{
	...
	else
	{
		...
		elementType->MoveFrom(GetPointerSlow(elementType, index + values.GetCount()), sz, GetPointerSlow(elementType, index), sz, _cnt - values.GetCount() - index);
		elementType->Destruct(GetPointerSlow(elementType, index), sz, values.GetCount());
	}
	if (values.GetFirst())
	{
		elementType->MoveConstruct(GetPointerSlow(elementType, index), sz, values.GetFirst(), sz, values.GetCount());
	}
	return OK;
}

ResultMem GenericBaseArray::Erase(const DataType& elementType, Int index, Int eraseCnt)
{
	if (MAXON_UNLIKELY(UInt(index) > UInt(_cnt)))
	{
		return FAILED;
	}

	const Int availableCnt = _cnt - index;
	...
	_cnt -= eraseCnt;
	Int sz = elementType->GetSize();
	elementType->MoveFrom(GetPointerSlow(elementType, index), sz, GetPointerSlow(elementType, index + eraseCnt), sz, _cnt - index);
	elementType->Destruct(GetPointerSlow(elementType, _cnt), sz, eraseCnt);
	return OK;
}

ResultMem GenericBaseArray::SwapErase(const DataType& elementType, Int index, Int eraseCnt)
{
	if (MAXON_UNLIKELY(UInt(index) > UInt(_cnt)))
	{
		return FAILED;
	...
	Int moveStartIdx = Max<Int>(_cnt - eraseCnt, index + eraseCnt);
	Int sz = elementType->GetSize();
	elementType->MoveFrom(GetPointerSlow(elementType, index), sz, GetPointerSlow(elementType, moveStartIdx), sz, _cnt - moveStartIdx);
	_cnt -= eraseCnt;
	elementType->Destruct(GetPointerSlow(elementType, _cnt), sz, eraseCnt);
	return OK;
}
...
{
	const Int size = elementType->GetSize();
	const Char* ptr = _ptr;
	DefaultHasher hash;
	for (Int i = 0; i < _cnt; ++i)
	{
...
UniqueHash GenericBaseArray::GetUniqueHashCode(const DataType& elementType) const
{
	const Int size = elementType->GetSize();
	const Char* ptr = _ptr;
	const Bool trivial = Bool(elementType->GetValueKind() & VALUEKIND::TRIVIALLY_HASHABLE);
	if (trivial)
	{
		return UniqueHash::GetUniqueHashCode(reinterpret_cast<const Byte*>(ptr), size * _cnt);
	}
	else
	{
		UniqueHasher state;
		for (Int i = 0; i < _cnt; ++i)
		{
			state.Combine(elementType->GetUniqueHashCode(ptr));
			ptr += size;
		}
