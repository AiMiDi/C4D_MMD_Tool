...
#define MAXON_DEPENDENCY_ALWAYS_WEAK(headerOrEntity) static maxon::WeakUse MAXON_CONCAT(s_weakUse_, __COUNTER__)(&PRIVATE_MAXON_TRANSLATION_SUBUNIT, headerOrEntity)

#ifndef PRIVATE_MAXON_UNITY_BUILD_ID
...
//----------------------------------------------------------------------------------------
#define MAXON_DEPENDENCY_WEAK(headerOrEntity) MAXON_DEPENDENCY_ALWAYS_WEAK(headerOrEntity)

#else

#define MAXON_DEPENDENCY_WEAK(headerOrEntity) int MAXON_CONCAT(dummy, __COUNTER__) = MAXON_DEPENDENCY_WEAK_not_allowed_in_unity_builds();
...
template <TranslationUnit& UNIT, Bool WEAK, typename NAME> const maxon::EntityUse g_PrivateDependencyOnModule{NAME::value, &UNIT, nullptr, WEAK};
...
#define PRIVATE_MAXON_DEPENDENCY_ON_MODULE_C(module, id) \
	...
	inline const maxon::EntityUse* const s_moddep_##id \
		= &maxon::g_PrivateDependencyOnModule<maxon::g_mainTranslationUnit, false, maxon::PrivateFixedSizeArrayBuilder<PrivateModuleDependency##id, std::make_index_sequence<SIZEOF(module)>>::type>; \
