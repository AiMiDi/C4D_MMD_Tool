...
struct WeakRefTargetBase
{
	...
	const void* Lock() const
	{
		...
		if (MAXON_UNLIKELY((value & 1) || _target.TryCompareAndSwap(value | 1, value) == false))
		{
			CpuYield	wait(this);
	...
	//----------------------------------------------------------------------------------------
	/// @brief Unlocks the WeakRefTarget.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if there currently is a target.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the current target pointer.
	/// @note Use Lock() and Unlock() to get a thread-safe pointer.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Unlocks the current target.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Swaps old and new target.
	...
	{
		if (MAXON_UNLIKELY(_target.TryCompareAndSwap(UInt(newTarget), UInt(oldTarget)) == false))
		{
			CpuYield	wait(this);
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a weak reference. If memory allocation fails the weak reference will be nullptr.
	...
	/// @param[in] src								Source weak reference.
	//----------------------------------------------------------------------------------------
	WeakRef(const WeakRef& src) = default;
	...
	//----------------------------------------------------------------------------------------
	/// @brief Moves a weak reference.
	/// @param[in] src								Source weak reference, will be nulled.
	//----------------------------------------------------------------------------------------
	WeakRef(WeakRef&& src) = default;
	...
	/// @return												*this.
	//----------------------------------------------------------------------------------------
	WeakRef& operator =(const WeakRef& src) = default;
	...
	/// @return												*this.
	//----------------------------------------------------------------------------------------
	WeakRef& operator =(WeakRef&& src) = default;
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a strong reference. If the weak referenced object was already released a null reference will be returned.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Compares WeakRefs for equality.
...
#else
	//----------------------------------------------------------------------------------------
	/// @brief Compares WeakRefs for inequality.
	/// @param[in] other							The other comparand.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if the weak reference points to a strong reference (non-blocking check).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if the weak reference is empty.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if this weak reference contains anything.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Computes hash code based on the targeted object's pointer. WeakRefs pointing to the same target will have the same hash code.
	/// When the target object is removed the hash code will change to the hash code of nullptr, this means two WeakRefs
	/// which initially pointed to different objects will have the same hash code after the target
	/// objects were destructed.
	/// @return												Hash code of the pointer of the referenced object, or the hash code of nullptr if the object doesn't exist.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Computes hash value based on the targeted object's pointer. WeakRefs pointing to the same target will have the same hash value.
	/// When the target object is removed the hash value will change to the hash value of nullptr, this means two WeakRefs
	/// which initially pointed to different objects will have the same hash value after the target
	/// objects were destructed.
	/// @return												Hash value of the pointer of the referenced object, or the hash value of nullptr if the object doesn't exist.
	//----------------------------------------------------------------------------------------
	UniqueHash GetUniqueHashCode() const
	{
		const void* target = GetCurrent();
		return DefaultCompare::GetUniqueHashCode(target);
	}
	...
	{
		return data ? *data : GetZeroRef<WeakRefTargetBase>();
	}
