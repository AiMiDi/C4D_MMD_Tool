...
template <typename ATOMICTYPE> struct BaseLock
{
	BaseLock()
	{
	...
	~BaseLock()
	{
		DebugAssert(_state.LoadRelaxed() == 0, "The CoreLock is still set while being destructed.");
	}
...
/// THREADSAFE.
//----------------------------------------------------------------------------------------
template <typename ATOMICTYPE> struct SpinlockT : protected maxon::details::BaseLock<ATOMICTYPE>
{
	//----------------------------------------------------------------------------------------
	...
	inline void Lock()
	{
		...
		if (MAXON_UNLIKELY(this->AttemptLock() == false))
			this->SpinAndRetry();
	}
	...
	{
		return this->_state.LoadAcquire() != 0;
	}
...
protected:
	...
	{
		return MAXON_LIKELY(this->_state.TryCompareAndSwap(1, 0));
	}
...
/// THREADSAFE.
//----------------------------------------------------------------------------------------
template <typename ATOMICTYPE> struct RWSpinlockT : protected maxon::details::BaseLock<ATOMICTYPE>
{
	using RWState = typename ATOMICTYPE::Type;
	static constexpr RWState WRITER = 1;
	static constexpr RWState WRITER_PENDING = 2;
	static constexpr RWState READERS = ~(WRITER | WRITER_PENDING);
	static constexpr RWState ONE_READER = 4;
	static constexpr RWState BUSY = WRITER | READERS;
...
using Spinlock = SpinlockT<AtomicInt32>;
using RWSpinlock = RWSpinlockT<AtomicInt32>;
