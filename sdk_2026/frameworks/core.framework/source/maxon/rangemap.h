...
public:
		...
		if (it)
		{
			...
			else
			{
				...
				if (it != _map.Begin())
				{
					begin = it;
					--begin;
					if ((begin.GetValue().first + 1 == rMinValue) && (begin.GetValue().GetSecond() == value))
					{
						it = begin;
						goto extendRange;
					}
		...
		extendRange:
			...
			if (last)
			{
				...
				if (last->first <= rMaxValue)
				{
					...
					if (it && (it.GetKey() == rMaxValue + 1) && (it.GetValue().GetSecond() == value))
					{
	...
	addRange:
		{
			...
			if (last)
			{
				...
				if (last->first <= rMaxValue)
				{
					...
					begin.GetValue().GetSecond() = value;
				}
				else if (last->GetSecond() == value)
				{
					// last overlapping range goes beyond r, but has same value
	...
	{
		return Insert(range.minValue, range.maxValue, value);
	}
	...
	const V* FindValue(K key) const
	{
		...
		return (key <= it.GetValue().first) ? &it.GetValue().GetSecond() : nullptr;
	}
	...
	{
		return Erase(range.minValue, range.maxValue);
	}
	...
	public:
		...
		{
			return Super::GetValue().GetSecond();
		}

		Type operator *() const
		{
			const auto& v = Super::GetValue();
			return Type(Range(Super::GetKey(), v.first), v.GetSecond());
		}
	};
	...
	static Result<void> DescribeIO(const DataSerializeInterface& stream)
	{
		...
		// For a RangeSet (where V is UnitType) we treat _map as a map K->K (which is layout-compatible) to simplify the serialization.
		using FakeMapType = typename std::conditional<std::is_same<V, UnitType>::value, typename MAP::template Type<K, K>, MapType>::type;

		// We can't use the Describe macro because that doesn't allow to use a fake type, so we have to manually expand it.
		...
		descriptionStream.DescribeElement<DESCRIBECLASS, Tuple<typename FakeMapType::KeyType, typename FakeMapType::ValueType>, DESCRIBEFLAGS::TYPE_MAP>("_map", (FakeMapType*) &(static_cast<DESCRIBECLASS*>(nullptr)->_map), maxon::DESCRIBEFLAGS::NONE) iferr_return;
		return OK;
	}
...
public:
	...
	// Intel compiler workaround, "using" isn't sufficient for member detection like decltype(set.GetHashCode())
	HashInt GetHashCode() const { return Super::GetHashCode(); }
	UniqueHash GetUniqueHashCode() const { return Super::GetUniqueHashCode(); }
	...
	{
		return Super::Insert(range.minValue, range.maxValue, UnitType());
	}
	...
	{
		return Super::Erase(range.minValue, range.maxValue);
	}
	...
	//----------------------------------------------------------------------------------------
	Bool Contains(T minValue, T maxValue) const
	{
		typename Super::Range r;
		return Super::FindRange(minValue, r) && (maxValue <= r.maxValue);
	}
	...
	//----------------------------------------------------------------------------------------
	Bool Contains(const typename Super::Range& range) const
	{
		typename Super::Range r;
		return Super::FindRange(range.minValue, r) && (range.maxValue <= r.maxValue);
	}
...
{
	Int totalCount = 0;
	for (const auto& itRange : data)
	{
		// Add 1, ranges are inclusive
		totalCount += itRange.GetMax() - itRange.GetMin() + decltype(itRange.GetMax()){1};
	}
	return totalCount;
}
