...
{
public:
	GlobalBase(const Char* name, Result<void> (*init) (GlobalBase* global), void (*free) (GlobalBase* global), TranslationUnit* main);
...
public:
	...
	//----------------------------------------------------------------------------------------
	explicit Global(TranslationUnit& main = g_mainTranslationUnit)
		: _privateSuper(
#ifdef __BASE_FILE__
			__BASE_FILE__,
#else
			"global",
#endif
			nullptr, &PrivateGlobalFree, &main)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	explicit Global(T&& init, TranslationUnit& main = g_mainTranslationUnit)
	...
	//----------------------------------------------------------------------------------------
	template <typename INIT> explicit Global(INIT&& init, TranslationUnit& main = g_mainTranslationUnit)
		: _privateSuper(
#ifdef __BASE_FILE__
			__BASE_FILE__,
#else
			"global",
#endif
			&PrivateGlobalInit<INIT>, &PrivateGlobalFree, &main)
	{
		MAXON_WARNING_PUSH
...
{
public:
	explicit GlobalWithInit(TranslationUnit& main = g_mainTranslationUnit) : Global<T>(&Init, &main)
	{
	}
