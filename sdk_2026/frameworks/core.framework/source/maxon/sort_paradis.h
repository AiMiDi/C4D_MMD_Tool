	...
	if (kth_byte > 0)
	{
		...
		for (Int32 i = 0; i < RADIXBIN; i++)
		{
			...
			if (histogram[i] > 64)
			{
				if (!jobGroup4)
				{
					jobGroup4 = JobGroupRef::Create(JOBGROUPFLAGS::FINALIZE_ON_WAIT) iferr_return;
...
inline Result<void> Paradis(const Block<T>& data, UInt32 threadNum, GETINT&& getInt)
{
	...
	if (data.GetCount() < 1024)
	{
		InsertSort(data, 0, (Int32)data.GetCount(), getInt);
		return OK;
	}
