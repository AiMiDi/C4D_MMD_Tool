...
//----------------------------------------------------------------------------------------
template <typename ATOMICTYPE> void BaseLock<ATOMICTYPE>::PerformanceMonitorLockAcquired() const
{
...
//----------------------------------------------------------------------------------------
template <typename ATOMICTYPE> void BaseLock<ATOMICTYPE>::PerformanceMonitorLockReleased() const
{
	// g_CpuYieldDelegate is guaranteed to be valid.
	g_CpuYieldDelegate((CpuYieldState*) CpuYieldState::LOCK_RELEASE, this);
}

template struct BaseLock<AtomicInt16>;
template struct BaseLock<AtomicInt32>;
...
//----------------------------------------------------------------------------------------
template <typename ATOMICTYPE> void SpinlockT<ATOMICTYPE>::SpinAndRetry()
{
	CoreSpinAndRetry();

	if (MAXON_UNLIKELY(g_trackSpinlocks))
		this->PerformanceMonitorLockAcquired();
}
...
//----------------------------------------------------------------------------------------
template <typename ATOMICTYPE> void SpinlockT<ATOMICTYPE>::CoreSpinAndRetry()
{
	CpuYield wait(this);
	...
	do
	{
		...
		while ((currentState = this->_state.LoadConsume()) != 0)
		{
	...
	} while (this->_state.TryCompareAndSwap(-1, 0) == false);
}
...
template <typename ATOMICTYPE> Bool RWSpinlockT<ATOMICTYPE>::IsReadLocked() const
{
	return (this->_state.LoadConsume() & READERS) != 0;
}

template <typename ATOMICTYPE> Bool RWSpinlockT<ATOMICTYPE>::IsWriteLocked() const
{
	return (this->_state.LoadConsume() & WRITER) != 0;
}

template <typename ATOMICTYPE> void RWSpinlockT<ATOMICTYPE>::ReadLock()
{
	CpuYield wait(this);

	while (true)
	{
		RWState s = this->_state.LoadConsume();
		// no writer or write requests?
		if ((s & (WRITER | WRITER_PENDING)) == 0)
		{
			// successfully stored increased number of readers?
			if (this->_state.TryCompareAndSwap(s + (typename ATOMICTYPE::Type)ONE_READER, s))
				break;
...
template <typename ATOMICTYPE> void RWSpinlockT<ATOMICTYPE>::ReadUnlock()
{
	DebugAssert(this->_state.LoadConsume() & READERS, "RWSpinlock was not read locked");
	this->_state.SwapAdd(-((Int32) ONE_READER));

	if (MAXON_UNLIKELY(g_trackSpinlocks))
		this->PerformanceMonitorLockReleased();
}

template <typename ATOMICTYPE> void RWSpinlockT<ATOMICTYPE>::WriteLock()
{
	CpuYield wait(this);
	...
	while (true)
	{
		...
		if ((s & BUSY) == 0)
		{
			...
			if (this->_state.TryCompareAndSwap(WRITER, s))
				break;
		...
		{
			this->_state.SwapOr(WRITER_PENDING);
		}
		wait.Pause();
	}
...
template <typename ATOMICTYPE> Bool RWSpinlockT<ATOMICTYPE>::AttemptWriteLock()
{
	RWState s = this->_state.LoadConsume();
	...
	Bool res = this->_state.TryCompareAndSwap(WRITER, s) != 0;
...
template <typename ATOMICTYPE> void RWSpinlockT<ATOMICTYPE>::WriteUnlock()
{
	DebugAssert(this->_state.LoadConsume() & WRITER, "RWSpinlock was not write locked");

	// create a memory barrier and unlock
	this->_state.StoreRelease(0);

	if (MAXON_UNLIKELY(g_trackSpinlocks))
		this->PerformanceMonitorLockReleased();
}

template struct SpinlockT<AtomicInt8>;
template struct SpinlockT<AtomicInt16>;
template struct SpinlockT<AtomicInt32>;

template struct RWSpinlockT<AtomicInt16>;
template struct RWSpinlockT<AtomicInt32>;
