...
{
	if constexpr (order != SYSTEM_BYTEORDER)
	{
...
public:
	...
	/// @param[in] valueAtAddr				Pointer to the data.
	/// @param[in] order							Byte order of the data.
	/// @return												Value in system byte order.	
	//----------------------------------------------------------------------------------------
	template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE static T ReadValue(const T& valueAtAddr, BYTEORDER order)
	{
		return (order != SYSTEM_BYTEORDER) ? SwapByteOrder(valueAtAddr) : valueAtAddr;
	}
};
