...
/// @cond INTERNAL
#define DISABLE_IF_JOB_OR_GROUP(TYPE, RET) typename std::enable_if<!(STD_IS_REPLACEMENT(convertible, typename std::remove_reference<TYPE>::type, maxon::JobInterface*) || STD_IS_REPLACEMENT(convertible, typename std::remove_reference<TYPE>::type, maxon::JobGroupInterface*)), RET>::type
/// @endcond
...
} MAXON_ENUM_FLAGS(JOBGROUPADDFLAGS);

//----------------------------------------------------------------------------------------
/// @brief Heterogenous job group: The jobs can be of different type and you can add them to the group
...
public:
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues all jobs of the group including subgroups.
	/// This will add a reference and remove it when the object is no longer needed. Please note that 
	/// a group (like a job) can only be enqueued once.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues all jobs of the group including subgroups and waits for them. 
	/// This implicitely indicates to the system that the current job cannot continue until the group has finished.
	/// THREADSAFE.
	/// @param[in] queue							The queue, use JOBQUEUE_CURRENT for the current queue.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Waits until all jobs of the group have been executed.
	///
	...
	//----------------------------------------------------------------------------------------
	/// @brief Waits until the group has been executed and returns OK on success or any errors returned
	...
	Result<void> Add(JobInterface* job, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT)
	{
		...
		return ThreadServices::AddJob(GetCoreJobGroup(), *job, UInt(flags));
	}
	...
	Result<void> Add(JobRef& job, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT)
	{
		...
		return ThreadServices::AddJob(GetCoreJobGroup(), *unreferenced, UInt(flags));
	}
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <typename T> Result<void> Add(ResultMemT<T*> job, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT)
	{
		JobInterface* ptr = job.GetPointer();
		if (ptr == nullptr)
			return OutOfMemoryError(MAXON_SOURCE_LOCATION);

		return ThreadServices::AddJob(GetCoreJobGroup(), *ptr, UInt(flags));
	}
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <typename JOBREF> Result<void> Add(ResultMemT<JOBREF>&& job, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT)
	{
	...
	//----------------------------------------------------------------------------------------
	/// @brief Encapsulates a lambda or object with operator () in a JobInterface and adds this job to the group.
	/// As long as the group is not enqueued you can add jobs from any thread. As soon as it
	/// is enqueued only jobs belonging to the group are allowed to add further jobs.
	/// THREADSAFE.
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <JOBCANCELLATION B = JOBCANCELLATION::ISOK, typename FN> DISABLE_IF_JOB_OR_GROUP(FN, Result<void>) Add(FN&& src, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT)
	{
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds indivually allocated jobs to the group.
	/// THREADSAFE.
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <typename JOBTYPE> Result<void> Add(BaseArray<JOBTYPE*>&& jobs, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT)
	{
		Block<JOBTYPE*> disconnected = jobs.Disconnect();
		JOBTYPE** memory = &disconnected[0];
		const JobInterface* first = *memory;
		const JobInterfacePOD* pod = first;
		
		return ThreadServices::AddJobs(GetCoreJobGroup(), reinterpret_cast<void**>(memory), disconnected.GetCount(), UInt(pod) - UInt(first), UInt(flags));
	}

	//----------------------------------------------------------------------------------------
	/// @brief Adds an array with multiple jobs of the same type to the group (faster than single Add()s).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a subgroup to the group.
	...
	//----------------------------------------------------------------------------------------
	/// @brief ObservableFinished is an observable that is triggered after this group has been executed.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the group to cancel execution of all jobs that are enqueued. Currently running jobs
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the group to cancel execution and waits until it has finished.
	/// THREADSAFE.
...
/// @cond INTERNAL
	//----------------------------------------------------------------------------------------
	/// @brief Returns the internal CoreJobGroup pointer.
	/// @return												CoreJobGroup pointer or nullptr if allocation failed.
	//----------------------------------------------------------------------------------------
	CoreJobGroup& GetCoreJobGroup() const
	{
		return *const_cast<CoreJobGroup*>(reinterpret_cast<const CoreJobGroup*>(this));
	}

	//----------------------------------------------------------------------------------------
	/// @brief Allocates a JobGroup.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Notifies the group that an operation has finished early (some of its jobs might not
...
/// @cond INTERNAL
	...
	//----------------------------------------------------------------------------------------
	/// @brief Registers an observer job that will be called after all jobs of a group (including sub
	...
	//----------------------------------------------------------------------------------------
	/// @brief Registers an observer job that will be called after all jobs of a group have been executed.
	/// You can specify an optional queue that should be used to run the observer which can be
	/// handly if you for example want to update the UI from the main thread after some threaded
	/// work has been done. By default the observer is not queued and runs in the same context
	/// as the last job.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues all jobs of the group including subgroups.
	/// This will add a reference and remove it when the object is no longer needed).
	/// Please note that a group (like a job) can only be enqueued once.
	/// THREADSAFE.
	/// @param[in] queue							The queue, use JOBQUEUE_CURRENT for the current queue.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a job, a lambda, a BaseArray of jobs or a whole subgroup.
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <typename... ARGS> Result<void> Add(ARGS&&... args) const
	{
		JobGroupInterface* group = GetPointer();

		if (MAXON_LIKELY(group != nullptr))
			return group->Add(std::forward<ARGS>(args)...);

		return NullptrError(MAXON_SOURCE_LOCATION);
	}

	/// The same as above but with the ability to specify the behaviour on early cancellation.
	template <JOBCANCELLATION B, typename T> Result<void> Add(T&& obj, JOBGROUPADDFLAGS flags = JOBGROUPADDFLAGS::DEFAULT) const
	{
		JobGroupInterface* group = GetPointer();

		if (MAXON_LIKELY(group != nullptr))
			return group->Add<B>(std::forward<T>(obj), flags);
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the group to cancel execution of all jobs that are enqueued.
	/// Currently running jobs are not affected unless they call IsCancelled(). If this is a subgroup the parent group
	/// will be cancelled too. The call will not wait for the group to cancel and it can be called from any thread or job.
	/// THREADSAFE.
	//----------------------------------------------------------------------------------------
	void Cancel() const
	{
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a default value of the JobGroupRef (see defaultvalue.h for more details).
...
//----------------------------------------------------------------------------------------
/// @brief This is a template for homogenous static job group: All job of the same type and the
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Allocates a StaticJobGroup.
	...
	{
		return (StaticJobGroupInterface*) ThreadServices::CreateGroup(jobCnt, SIZEOF(T), UInt((extraBufferSize + 63) & ~Int(63)) | UInt(flags));
	}
	...
	{
		return (StaticJobGroupInterface*) ThreadServices::CreateGroupForQueue(queue, threadCnt, SIZEOF(T), UInt((extraBufferSize + 63) & ~Int(63)) | UInt(flags));
	}
...
public:
	...
	static ResultMemT<StaticJobGroupRef> Create(JobQueueInterface* queue, Int& threadCnt, STATICJOBGROUPFLAGS flags, Int extraBufferSize = 0)
	{
		return StaticJobGroupRef(StaticJobGroupInterface<T>::Alloc(queue, threadCnt, flags, extraBufferSize), 0);
	}

	//----------------------------------------------------------------------------------------
	/// @brief Creates a StaticJobGroup.
	/// @param[in] jobCnt							The maximum number of jobs for this group.
	/// @param[in] flags							STATICJOBGROUPFLAGS::DEFAULT by default, the other custom modes are for ParallelFor only.
	/// @param[in] extraBufferSize		By default 0. Size of a private buffer that can be allocated together with the group.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues all jobs of the group including subgroups (will add a reference and remove
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues all jobs of the group including subgroups and waits for them. This implicitely
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a more generic JobGroupRef.
	/// THREADSAFE.
	/// @return												Cast to JobGroupRef.
	//----------------------------------------------------------------------------------------
	operator const JobGroupRef&() const
	{
		return *reinterpret_cast<const JobGroupRef*>(this);
	}

	//----------------------------------------------------------------------------------------
	/// @brief Returns a default value of the StaticJobGroupRef (see defaultvalue.h for more details).
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Creates a wrapper array around the static buffer of a StaticJobGroup. By default the
	...
	explicit StaticJobArray(Int initialJobCnt, StaticJobGroupInterface<T>* group, void* buffer)
	{
		if (group)
		{
			this->_ptr = (T*) ThreadServices::GetStaticJobBuffer(group->GetCoreJobGroup(), &this->_capacity);
		...
		this->_cnt = this->_capacity < initialJobCnt ? this->_capacity : initialJobCnt;
	}
