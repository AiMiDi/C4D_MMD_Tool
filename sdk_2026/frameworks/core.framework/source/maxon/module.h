...
private:
	...
	InternedIdInitializer* _nextOfBinary; ///< Next in singly linked list of initializers of the current binary.
...
#define MAXON_INTERNED_ID(IID, NAME) \
	struct NAME##_PrivateHelper { static constexpr maxon::LiteralId GetId() { return maxon::LiteralId(IID); } }; /* to support ID_CASE */ \
	extern maxon::InternedId NAME
...
#define MAXON_INTERNED_ID_LOCAL(IID, NAME) \
	struct NAME##_PrivateHelper { static constexpr maxon::LiteralId GetId() { return maxon::LiteralId(IID); } }; /* to support ID_CASE */ \
	maxon::InternedId NAME; \
	static maxon::InternedIdInitializer g_iid_##NAME(&NAME, IID)
...
public:
	...
	InternedIdInitializer*& GetInternedIdInitializer()
	{
		return _internedIds;
	}
...
private:
	...
	// _abiVersion has to be the first member of Binary, and Binary must have standard-layout.
	Int _abiVersion;

	// The version of the core framework used for this binary.
	Int _coreFrameworkVersion;

	// Points to a singly linked list of ModuleInfo objects of this binary.
	// We must not initialize this with nullptr: This will be done anyway as Binary is used as a global object,
  ...
  Initializer _initializer; ///< Initializer function. This will be set by the object system.
	...
	InternedIdInitializer* _internedIds; ///< Head of singly linked list of interned id initializers of this binary.
...
public:
	...
	{
		return (GetBase().GetFlags() != EntityBase::FLAGS::NONE) && !(GetBase().GetFlags() & EntityBase::FLAGS::SYNTHETIC) && !IsFramework();
	}
	...
	/// APIS list in projectdefinition.txt of the module plus the indirectly used frameworks.
	/// @return												Identifiers of directly or indirectly used frameworks of this module.
	//----------------------------------------------------------------------------------------
	Block<const Id> GetUsedFrameworks() const;

	//----------------------------------------------------------------------------------------
	/// DEPENDENCY is used for GetExpectedModuleDependencies() to determine the dependency mode given in projectdefinition.txt.
	//----------------------------------------------------------------------------------------
	enum class DEPENDENCY;

#ifndef OPTIONAL // Windows defines an OPTIONAL macro which we can't #undef.
	enum class DEPENDENCY
	{
	...
	} MAXON_ENUM_LIST_CLASS(DEPENDENCY);
#endif

	//----------------------------------------------------------------------------------------
	/// Returns the identifiers and dependency modes of all modules on which this module depends.
	/// This is set up by the Modules, ModulesWeak and ModulesOptional lists in projectdefinition.txt of the module (no recursion).
	/// @return												Directly used modules of this module, sorted by their identifier.
	//----------------------------------------------------------------------------------------
	Block<const Tuple<const Char*, DEPENDENCY>> GetExpectedModuleDependencies() const;

	//----------------------------------------------------------------------------------------
	/// Returns first element of EntityReference.
	//----------------------------------------------------------------------------------------
...
private:
  ...
  InternalData* _internalData; ///< Internal data for this module.
	...
	const Tuple<const Char*, DEPENDENCY>* _expectedModuleDependencies; ///< List of all modules which are expected to be used by this module according to projectdefinition.txt
	Int _expectedModuleDependenciesCount; ///< Number of elements in _expectedModuleDependencies.
	...
	friend class SetExpectedModuleDependencies;
...
template <typename D, TranslationUnit& UNIT, Bool WEAK, typename CHECK = void> class DeclarationUse : public EntityUse
{
public:
	DeclarationUse() : EntityUse(D::GetInstance().GetReference(), &UNIT, D::HEADER, WEAK)
	{
	}

	static DeclarationUse _instance;

	const typename D::Type& DefaultValueImpl() const
	{
		return maxon::DefaultValue<const typename D::Type&>();
	}

	static MAXON_ATTRIBUTE_FORCE_INLINE const typename D::Type& DefaultValue()
	{
		return _instance.DefaultValueImpl();
	}
};

template <typename D, TranslationUnit& UNIT, Bool WEAK>
class DeclarationUse<D, UNIT, WEAK, typename std::enable_if<maxon::CStringCompare::GetHashCode(D::PrivateImplementationModule) != details::ModuleIdHash>::type>
{
public:
	const typename D::Type& DefaultValueImpl() const
	{
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const typename D::Type& DefaultValue()
	{
		return reinterpret_cast<const DeclarationUse&>(g_PrivateDependencyOnModule<UNIT, WEAK, typename PrivateFixedSizeArrayBuilder<D, std::make_index_sequence<SIZEOF(D::PrivateImplementationModule)>>::type>).DefaultValueImpl();
	}
};


template <typename D, TranslationUnit& UNIT, Bool WEAK, typename CHECK> DeclarationUse<D, UNIT, WEAK, CHECK> DeclarationUse<D, UNIT, WEAK, CHECK>::_instance;
...
struct TranslationUnitAndModule
{
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Returns the singleton instance representing the published object of MAXON_DECLARATION.
	/// @return												The Declaration instance of the MAXON_DECLARATION.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
#ifdef MAXON_TARGET_WINDOWS // Windows is able to merge functions with identical code, then NO_INLINE is counterproductive
#else
	MAXON_ATTRIBUTE_NO_INLINE
#endif
	Declaration() : Declaration(C::GetId(), C::GetTranslationUnitAndModule())
	{
	}
	...
	/// The usage of Get() creates a strong dependency of the calling source file on the definition of the object.
	/// @return												The referenced object (or a reference to the default value if the initialization failed).
	/// @see WeakGet
	//----------------------------------------------------------------------------------------
	template <TranslationUnit& UNIT> const T& Get() const
	{
		...
		{
			return Initialize(std::conditional<C::DEPENDENCIES_ENABLED, DeclarationUse<C, UNIT, false>, Declaration>::type::DefaultValue());
		}
	}

	// TODO: (Ole) Documentation
	MAXON_ATTRIBUTE_FORCE_INLINE const T& GetWithoutDependency() const
	{
		return _object ? *_object : DefaultValue();
	}

	//----------------------------------------------------------------------------------------
	/// Returns the object referred to by this reference, or a default value if no definition exists
	/// for this declaration. If the reference hasn't been initialized yet and a definition exists,
	/// the initialization of the corresponding definition and its references is triggered.
	///
	/// Unlike Get(), the usage of WeakGet() only creates a weak dependency of the calling source file on the definition of the object.
	/// This means that a definition need not exist. In this case, a default value is returned.
	/// @return												The referenced object (or a reference to the default value if no matching definition exists or if the initialization failed).
	/// @see Get
	//----------------------------------------------------------------------------------------
	template <TranslationUnit& UNIT> Result<const T&> WeakGet() const
	{
#ifdef PRIVATE_MAXON_UNITY_BUILD_ID
		static_assert(AlwaysFalse<C>::value, "WeakGet() can't be used in unity builds.");
#endif
		...
		else
		{
			...
			DebugStop();
			return DeclarationUse<C, UNIT, true>::DefaultValue();
		}
	}
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Bool IsInitialized() const
	{
		return _super.IsInitialized();
	}

	MAXON_ATTRIBUTE_FORCE_INLINE const EntityReference& GetReference() const
	{
		return _super;
	}

	MAXON_ATTRIBUTE_FORCE_INLINE operator const EntityReference&() const
	{
		return _super;
	}
...
private:
	...
	/// @param[in] unitAndModule			The translation unit and module containing the object reference.
	//----------------------------------------------------------------------------------------
	Declaration(LiteralId&& iid, maxon::details::TranslationUnitAndModule unitAndModule) : _super(C::TYPE, std::move(iid), unitAndModule.unit, unitAndModule.module), _object(nullptr)
	{
	}

	EntityReference _super; ///< EntityReference base object.
	mutable const T* _object; ///< Pointer to the referenced object. This is set by the object system when the reference is initialized.

	static MAXON_ATTRIBUTE_FORCE_INLINE const T& DefaultValue()
	{
		return maxon::DefaultValue<const T&>();
	}

	const T& Initialize(const T& def) const
	{
		return *reinterpret_cast<const T*>(_super.InitializeDeclaration(&def));
	}
...
namespace details
{
template <typename C> const Declaration<typename C::Type, C> g_declaration{};
}

template <typename T, typename C> class DeclarationWrapper
{
public:
	using DeclarationType = maxon::Declaration<T, C>;
	using DeclarationHelper = C;
	using Type = T;

	// for MAXON_DATATYPE this is overridden by EntityBase::TYPE::DATATYPE
	static constexpr EntityBase::TYPE TYPE = maxon::details::EntityType<T>::value;

	constexpr DeclarationWrapper()
	{
	}

	MAXON_ATTRIBUTE_FORCE_INLINE const T& GetWithoutDependency() const
	{
		return GetInstance().GetWithoutDependency();
	}
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const DeclarationType& GetInstance()
	{
		return maxon::details::g_declaration<C>;
	}
};
...
public:
	...
	/// @param[in] flags							Additional flags for the definition.
	/// @param[in] type								The EntityBase::TYPE for the definition.
	/// @tparam T											Type of the published object.
	//----------------------------------------------------------------------------------------
	template <typename T> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), ::MaxonRegistry* dummy, TranslationUnit* unit, const Char* file,
																				 EntityBase::FLAGS flags, EntityBase::TYPE type = maxon::details::EntityType<T>::value)
		: _super(type, std::move(iid), unit, file, flags),
			_init(reinterpret_cast<void*>(init)),
			_initDefinition(&InitObject<T>)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename T, typename REG> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), REG* dummy, TranslationUnit* unit, const Char* file,
																											 EntityBase::FLAGS flags, EntityBase::TYPE type = maxon::details::EntityType<T>::value)
		: _super(type, std::move(iid), unit, file, flags),
			_init(reinterpret_cast<void*>(init)),
			_initDefinition(&InitRegistryEntry<T, REG>),
			_registry(REG::GetId())
	{
	}
	...
	const void* PrivateGet()
	{
		...
		Result<void> r = _super.GetBase().Initialize(Id::DefaultValue());
...
private:
	...
	template <typename T> static Result<void> InitObject(ObjectDefinition* this_)
	{
		...
		{
			r2 = this_->AddClassExtension(Dereference<T>::GetPointer(*(T*) this_->_object.PrivateGetPtr()));
			if (r2 == FAILED)
				return r2.GetErrorStorage();
		}
		return OK;
	...
	Result<void> AddClassExtension(const void* ptr);
...
/// @endcode
/// @note
/// The executed code may run on an arbitrary thread. For the rare case of requiring something to initialize on the main thread before the Cinema API is
...
#define PRIVATE_MAXON_INITIALIZATION_C(ID) \
	namespace \
	{ \
		struct MAXON_CONCAT(PrivateInitialization_, ID) : public maxon::details::InitializationFunctions \
		{ \
			MAXON_CONCAT(PrivateInitialization_, ID)(maxon::Result<void> (*init) (), void (*free) ()) : maxon::details::InitializationFunctions(MAXON_FILE, init, free, &PRIVATE_MAXON_TRANSLATION_SUBUNIT) { } \
		}; \
	} \
	static MAXON_CONCAT(PrivateInitialization_, ID) MAXON_CONCAT(s_initialization_, ID)
...
/// @param[in] ...								Optional code to put into the declaration's helper class, e.g. MAXON_IMPLEMENTATION_MODULE.
///
/// @see @ref object_declaration
//----------------------------------------------------------------------------------------
#define MAXON_DECLARATION(T, Name, id, ...) \
	PRIVATE_MAXON_DECLARATION(T, Name, id, static constexpr, __VA_ARGS__)
...
#define PRIVATE_MAXON_DECLARATION(T, Name, id, storage, ...) \
	struct Name##_PrivateHelper : public maxon::DeclarationWrapper<T, Name##_PrivateHelper> \
	{ \
		constexpr Name##_PrivateHelper() = default; \
		using Registry = MaxonRegistry; \
		static maxon::details::TranslationUnitAndModule GetTranslationUnitAndModule(); \
		static constexpr maxon::LiteralId GetId() { return maxon::LiteralId(false, id); } \
		static constexpr const maxon::Char* const HEADER = MAXON_FILE; \
		static const maxon::Bool DEPENDENCIES_ENABLED = maxon::Bool(MAXON_CONCAT(MAXON_DEPENDENCY_ENABLE, 1)); \
		template <maxon::TranslationUnit& UNIT = PRIVATE_MAXON_TRANSLATION_SUBUNIT> static MAXON_ATTRIBUTE_FORCE_INLINE const T& Get() { return GetInstance().template Get<UNIT>(); } \
		template <maxon::TranslationUnit& UNIT = PRIVATE_MAXON_TRANSLATION_SUBUNIT> MAXON_ATTRIBUTE_FORCE_INLINE const T& operator ()() const { return GetInstance().template Get<UNIT>(); } \
		template <maxon::TranslationUnit& UNIT = PRIVATE_MAXON_TRANSLATION_SUBUNIT> MAXON_ATTRIBUTE_FORCE_INLINE auto WeakGet() const { return GetInstance().template WeakGet<UNIT>(); } \
		__VA_ARGS__; \
	}; \
	storage Name##_PrivateHelper Name PRIVATE_MAXON_DECLARATION_END

#define PRIVATE_MAXON_DECLARATION_REGISTER_1(cnt, Name) \
	PRIVATE_MAXON_REGISTER_CHECK("MAXON_DECLARATION_REGISTER"); \
	PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(decltype(Name)); \
	static maxon::Result<decltype(Name)::Type> Private_InitObj##cnt (MAXON_SOURCE_LOCATION_DECLARATION, maxon::Id&); \
	static maxon::ObjectDefinition g_obj##cnt(decltype(Name)::GetId(), &Private_InitObj##cnt, (decltype(Name)::Registry*) nullptr, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, MAXON_FILE, maxon::EntityBase::FLAGS::NONE); \
...
#define PRIVATE_MAXON_DECLARATION_REGISTER_2(cnt, REGISTRY, Name) \
	...
	static maxon::ObjectDefinition g_obj##cnt(maxon::ObjectDefinition::MakeId(Name), &Private_InitObj##cnt, (REGISTRY::Registry*) nullptr, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, MAXON_FILE, maxon::EntityBase::FLAGS::HIDDEN); \
...
#define PRIVATE_MAXON_DECLARATION_REGISTER_DECLARATION(Name, prefix, id) \
	maxon::details::TranslationUnitAndModule prefix Name##_PrivateHelper::GetTranslationUnitAndModule() { return {&PRIVATE_MAXON_TRANSLATION_SUBUNIT, &PRIVATE_MAXON_MODULE}; }
...
#define MAXON_DEPENDENCY_REGISTER(Name) \
	maxon::EntityDefinition g_dependency_##Name(maxon::EntityBase::TYPE::DEPENDENCY, maxon::LiteralId(#Name), &PRIVATE_MAXON_TRANSLATION_SUBUNIT, MAXON_FILE, maxon::EntityBase::FLAGS::HIDDEN);
#endif
...
#define MAXON_DEPENDENCY(Name) \
	...
	static maxon::EntityUse MAXON_CONCAT(s_ud_##Name, __LINE__)("." #Name, &PRIVATE_MAXON_TRANSLATION_SUBUNIT, &g_dependency_##Name, false);
#else
#define MAXON_DEPENDENCY(Name)
#endif
...
template <UInt64 MODULE_ID_HASH = details::ModuleIdHash> extern EntityDefinition g_privateReflectionRegistrationDependency;

//----------------------------------------------------------------------------------------
/// MAXON_DEPENDENCY_ON_REFLECTION has to be added to a source file whenever the initialization of
/// entities in that source file depends on the successful registration of reflection of the same module.
/// @code
/// MAXON_DEPENDENCY_ON_REFLECTION;
/// @endcode
///
/// @see @ref manual_dep
//----------------------------------------------------------------------------------------
#define MAXON_DEPENDENCY_ON_REFLECTION \
	static maxon::EntityUse s_dep_reflection(".reflection", &PRIVATE_MAXON_TRANSLATION_SUBUNIT, &maxon::g_privateReflectionRegistrationDependency<>, false)
