...
/// to a relaxed value.LoadAcquire() followed by a MemoryFenceAcquire().
	...
	#define MemoryFenceAcquire()							__atomic_thread_fence(__ATOMIC_ACQUIRE)
...
#ifdef MAXON_USE_ATOMIC_INTRINSICS
	#define MemoryFenceRelease()							__atomic_thread_fence(__ATOMIC_RELEASE)
#elif defined(MAXON_TARGET_CPU_X86)
	// On x86 it is sufficient if the compiler doesn't reorder the code.
...
#ifdef MAXON_USE_ATOMIC_INTRINSICS
	#define MemoryFenceSequential()						__atomic_thread_fence(__ATOMIC_SEQ_CST)
#elif defined(MAXON_TARGET_CPU_X86)
...
{
	static_assert(sizeof(T) == sizeof(Int64), "sizeof(T) != sizeof(Int64)");
...
public:
	...
	inline T SwapOr(T orValue)
	{
		return (T)atomic_swap_or16((Int16 volatile*)&_value, (Int16)orValue);
	}
...
template <typename T> class AtomicFloatType
{
	...
	static_assert(STD_IS_REPLACEMENT(floating_point, T), "Only floating point types allowed.");
...
public:
	...
	Bool Dec()
	{
		...
		DebugAssert(Int(oldValue) >= Int(16), "Reference count is invalid.");
	...
	Bool ConditionalInc()
	{
		...
		do
		{
			...
			DebugAssert(Int(oldValue) >= 0);
			DebugAssert(oldValue != UInt(0x6d6d6d6d6d6d6d6d) && oldValue != UInt(-16), "Warning: You might access invalid memory.");
	...
	//----------------------------------------------------------------------------------------
	/// Returns whether an object has more references then the given #count.
	...
	{
		UInt v = _value.LoadRelaxed();
		return (Int(v >> 4) > count) || (v & 8);
	}
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const StrongReferenceCounter& DefaultValue()
	{
