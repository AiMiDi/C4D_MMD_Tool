#ifndef SSE_VECTOR_8F32_H__
#define SSE_VECTOR_8F32_H__
...
class Vector8f32
{
	...
	friend Vector8i32 CastToVector8i32(const Vector8f32& a);
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit Vector8f32(__m256 f)
	{
		_val = f;
	}
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit Vector8f32(Float32 f)
	{
		_val = (f == 0.0f) ? _mm256_setzero_ps() : _mm256_set1_ps(f);
	}
...
protected:
	/// The internal AVX vector
	union { __m256 _val; Float32 v[8]; };
...
private:
	...
	alignas(32) Float32 _dummy[8]; // dummy to keep the structure size constant

#endif // C4D_HAS_AVX_CAPABILITY
};

#ifdef C4D_HAS_AVX_CAPABILITY
static_assert(sizeof(Vector8f32) == sizeof(__m256), "size mismatch");
#else
static_assert(sizeof(Vector8f32) == 8 * sizeof(Float32), "size mismatch");
#endif // C4D_HAS_AVX_CAPABILITY

} // namespace maxon

#endif // SSE_VECTOR_8F32_H__
