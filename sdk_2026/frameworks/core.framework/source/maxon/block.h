...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Int GetCount() const { return _size; }
...
{
public:
	MAXON_ATTRIBUTE_FORCE_INLINE BlockBase(T* ptr, Int size, Int stride = 0) : _ptr(ptr), _size(size) { }
...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE explicit BaseIterator(CollectionType& a, Int start = 0) : _data(a.GetFirst() + start) { }

	MAXON_ATTRIBUTE_FORCE_INLINE explicit BaseIterator(ValueType* pos = nullptr, Int index = 0, Int stride = 0)
		: _data(pos)
	{
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Bool operator ==(const BaseIterator& b) const
	{
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Int operator -(const BaseIterator& b) const
	{
		return _data - b._data;
	}
	...
	public:
		...
		{
			return this->_error.GetReferencedError() || this->GetValue() != other.GetValue();
		}
		...
		{
			return {*this->GetValue()._data, reinterpret_cast<const Result<void>&>(this->_error)};
		}
	};
...
public:
	...
	public:
		using SUPER::SUPER;
		using Iterator = BaseIterator<typename SUPER::ValueType, STRIDED>;
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Block() : Super(nullptr, 0, 0) { }
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE constexpr Block(T* ptr, Int size, Int stride = (STRIDED && GENERIC) ? -1 : SIZEOF(StrideType)) : Super(ptr, size, stride)
	{
		// TODO: (Ole) find a way to make the assert work with constexpr (xcode complains, win not)
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Block(Iterator start, Int size, Int stride = (STRIDED && GENERIC) ? -1 : SIZEOF(StrideType)) : Super(&*start, size, stride)
	{
		DebugAssert(STRIDED ? (stride >= 0) : (stride == SIZEOF(StrideType))); // in the strided Generic case, this ensures that an argument for stride has been specified
	}
	...
	/// of the array.
	//----------------------------------------------------------------------------------------
	template <Int N> MAXON_ATTRIBUTE_FORCE_INLINE MAXON_IMPLICIT Block(T (&array)[N]) : Super(static_cast<T*>(array), N, SIZEOF(StrideType)) { }
	...
	{
		static_assert(CONSTBLOCK, "Only a Block with const type can be initialized from an initializer list.");
	}
	...
	//----------------------------------------------------------------------------------------
	/// Constructs a Byte block of raw bytes using the given values.
	/// This constructor can only be used for Byte blocks, and (for safety reasons) only when ANY1 is void or has size 1.
	/// @param[in] ptr								Pointer to the first object of type ANY1.
	/// @param[in] size								Number of elements of type ANY1.
	//----------------------------------------------------------------------------------------
	template <typename ANY1, typename = typename std::enable_if<BYTE_BLOCK && (SIZEOF(std::conditional_t<std::is_void<ANY1>::value, Char, ANY1>) == 1), ANY1>::type> MAXON_ATTRIBUTE_FORCE_INLINE Block(ANY1* ptr, Int size) : Super(reinterpret_cast<T*>(ptr), size, 1)
	{
	}

	//----------------------------------------------------------------------------------------
	/// Constructs a Byte block of raw bytes from the given block of type ANY.
	/// This constructor can only be used for Byte blocks.
	/// @param[in] src								A block of type ANY.
	//----------------------------------------------------------------------------------------
	template <typename ANY, typename = typename std::enable_if<BYTE_BLOCK, ANY>::type> MAXON_ATTRIBUTE_FORCE_INLINE MAXON_IMPLICIT Block(const Block<ANY>& src) : Super(reinterpret_cast<T*>(src.GetFirst()), src.GetCount() * SIZEOF(ANY), 1)
	{
	}

	//----------------------------------------------------------------------------------------
	/// Constructs a Byte block of raw bytes from the given array of type ANY.
	/// This constructor can only be used for Byte blocks.
	/// @param[in] array							An array of type ANY.
	//----------------------------------------------------------------------------------------
	template <typename ANY, Int N, typename = typename std::enable_if<BYTE_BLOCK, ANY>::type> MAXON_ATTRIBUTE_FORCE_INLINE MAXON_IMPLICIT Block(ANY (&array)[N]) : Super(reinterpret_cast<T*>(array), N * SIZEOF(ANY), 1) { }
	...
	static constexpr Bool BYTE_OR_CHAR = std::is_same<const Byte, const T>::value || std::is_same<const Char, const T>::value;
	...
	{
		return *reinterpret_cast<const Block<typename InheritConst<SwapByteChar, T>::type, STRIDED>*>(this);
	}
	...
	{
		return *reinterpret_cast<const Block<const SwapByteChar, STRIDED>*>(this);
	}
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE T* GetFirst() const { return this->_ptr; }

	//----------------------------------------------------------------------------------------
	/// Sets the first element of this block, may be @c nullptr.
	/// @param[in] value							Pointer to first element of this block.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE void SetFirst(T* value) { this->_ptr = value; }
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE T* GetLast() const { return GetPtr(this->_size-1); }
	...
	//----------------------------------------------------------------------------------------
	/// Returns a slice of this block with contains the elements from the given @p start index up to the end of the block.
	/// @param[in] start							Start index of the block.
	/// @return												Block starting at @p start.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Block Slice(Int start) const
	{
		DebugAssert((UInt) start <= (UInt) this->_size);
		return Block(GetPtr(start), this->_size - start, this->GetStride());
	}

	//----------------------------------------------------------------------------------------
	/// Returns a slice of this block with contains the elements from the given @p start index up to but not including the given @p end index.
	/// @param[in] start							Start index of the block.
	/// @param[in] end								End index of the block. The end index is exclusive.
	/// @return												Block with elements from @p start up to @p end.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Block Slice(Int start, Int end) const
	{
		...
		return Block(GetPtr(start), end - start, this->GetStride());
	}
	...
	{
		const Int s = this->GetCount() - suffix.GetCount();
		return s >= 0 && this->Slice(s, this->GetCount()) == suffix;
	}
	...
	template <typename T2, Bool S2, Bool M2> Result<void> CopyValuesFrom(const Block<T2, S2, M2>& other)
	{
		Int n = this->GetCount();
		if (MAXON_UNLIKELY(n != other.GetCount()))
		...
		const Int otherStride = other.GetStride();
		while (--n >= 0)
		{
			if (M2 && !std::is_const<T2>::value)
			{
				*dest = std::move(const_cast<typename std::remove_const<T2>::type&>(*src));
			}
			else
			{
				auto res = AssignCopy<T>(*dest, *src);
	...
	template <typename T2, Bool S2, Bool M2> Result<void> ConstructValuesFrom(const Block<T2, S2, M2>& other)
	{
		...
		while (--n >= 0)
		{
			if (M2 && !std::is_const<T2>::value)
			{
				new (dest) T(std::move(const_cast<typename std::remove_const<T2>::type&>(*src)));
			}
			else
			{
				auto res = NewCopy<T>(*dest, *src);
				if (res == FAILED)
					return res;
			}
			AdvanceByByteOffset(dest, destStride);
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Int GetBlock(Int index, typename std::conditional<CONSTBLOCK, DeleteReturnType01, Block<T, false>&>::type block) const { return static_cast<const Block*>(this)->GetBlock(index, reinterpret_cast<Block<const T, false>&>(block)); }

	MAXON_ATTRIBUTE_FORCE_INLINE Int GetBlock(Int index, typename std::conditional<CONSTBLOCK, DeleteReturnType02, StridedBlock<T>&>::type block) const { block.Set(GetFirst(), this->GetCount(), GetStride()); return 0; }
	MAXON_ATTRIBUTE_FORCE_INLINE Int GetBlock(Int index, StridedBlock<const T>& block) const { block.Set(GetFirst(), this->GetCount(), GetStride()); return 0; }
	...
	{
		if (IsTriviallyHashable<T>::value && !IsStrided())
		{
			// We can hash the raw bytes because T is trivially hashable and there's no stride.
			return UniqueHash::GetUniqueHashCode(reinterpret_cast<const Byte*>(GetFirst()), SIZEOF(T) * this->GetCount());
		}
		else
		{
			return Super::GetUniqueHashCode();
		}
	}

	//----------------------------------------------------------------------------------------
	/// Mixes the content of this block into hash. If this block has @ref IsTriviallyHashable "trivially hashable"
	/// elements and no stride, the raw bytes are taken. Otherwise each element @c e is mixed into
	/// hash using @c{hash.HashAndCombine<HASH>(e)}.
	...
	template <typename HASH = DefaultCompare> void CombineUniqueHash(UniqueHasher& hash) const
	{
		...
		if (IsTriviallyHashable<T>::value && stride == SIZEOF(T))
		{
			hash.Combine(reinterpret_cast<const Byte*>(GetFirst()), SIZEOF(T) * this->GetCount());
		}
		else
		{
			const T* p = this->_ptr;
	...
	template <typename BLOCK, typename COMPARE> typename std::enable_if<!STRIDED && std::is_same<BLOCK, Block>::value, Bool>::type IsEqualImpl(const BLOCK& other, COMPARE&& cmp, OverloadRank1) const
	{
		const Int cnt = this->GetCount();
		if (cnt != other.GetCount())
			return false;
		const T* const t = this->_ptr;
		const T* const o = other._ptr;
		if (t == o)
			return true;
		for (Int i = 0; i < cnt; ++i)
		{
			if (!cmp.IsEqual(t[i], o[i]))
				return false;
		}
		return true;
	}
...
{
	return StridedBlock<typename maxon::details::BlockType<T>::type>(ptr, size, stride);
}
...
{
	return {str, str ? (Int)strlen(reinterpret_cast<const Char*>(str)) : 0};
}
...
static_assert(STD_IS_REPLACEMENT(trivially_copyable, Block<Char>), "Block not trivially copyable.");
#endif
...
template <typename T, Int N> struct CArray
{
	...
	operator Block<typename std::conditional<STD_IS_REPLACEMENT(const, T), DeleteReturnType01, const T>::type>() const { return Block<const T>(values, N); };
	operator StridedBlock<T>() const { return StridedBlock<T>(values, N, SIZEOF(T)); };
	operator Block<typename std::conditional<STD_IS_REPLACEMENT(const, T), DeleteReturnType01, const T>::type, true>() const { return StridedBlock<const T>(values, N, SIZEOF(T)); };
