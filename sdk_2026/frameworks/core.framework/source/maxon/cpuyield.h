...
public:
	//----------------------------------------------------------------------------------------
	/// @param[in] res								The contended resource because of which Pause might have to be called. Might be a nullptr (but shouldn't).
	//----------------------------------------------------------------------------------------
	explicit CpuYield(const void* res = nullptr)
	{
		_count = 0;
	...
	~CpuYield()
	{
		// Log the end of a spin loop if g_trackCpuYield is enabled.
		if (MAXON_UNLIKELY(_count != 0 && g_trackCpuYield))
		{
			_count = LIMIT<Int32>::MAX;
	...
	void Reset()
	{
		// Only reset values if there already was spinning (otherwise the initial LOCK_BEGIN_SPIN is wrong).
		if (_count > 0)
		{
			// Set _count to 1 so that the destructor can invoke the diagnostic delegate.
			_count = 1;
			_loopSize = LOOPS_START_COUNT;
		}
	}
...
private:
	...
	{
		for (Int j = 0; j < 16; j++)
#if defined MAXON_COMPILER_CLANG || (defined MAXON_COMPILER_GCC && defined MAXON_TARGET_LINUX)
