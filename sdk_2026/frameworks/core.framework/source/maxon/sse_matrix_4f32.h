...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE explicit Matrix4f32(const Matrix& m)
	{
		...
		v3 = Vector4f32(m.off.x, m.off.y, m.off.z, 1.0f);
#else
		Vector2f64 a, b;

		a.LoadUnaligned(&m.sqmat.v1.x);
		b.LoadSingle(&m.sqmat.v1.z);
		v0 = ConvertToVector4f32(a, b);

		a.LoadUnaligned(&m.sqmat.v2.x);
		b.LoadSingle(&m.sqmat.v2.z);
		v1 = ConvertToVector4f32(a, b);

		a.LoadUnaligned(&m.sqmat.v3.x);
		b.LoadSingle(&m.sqmat.v3.z);
		v2 = ConvertToVector4f32(a, b);

		a.LoadUnaligned(&m.off.x);
		b = Vector2f64(m.off.z, 1.0);
		v3 = ConvertToVector4f32(a, b);
#endif
	}

	MAXON_ATTRIBUTE_FORCE_INLINE explicit Matrix4f32(const Matrix32& m)
	{
		v0 = Vector4f32(m.sqmat.v1.x, m.sqmat.v1.y, m.sqmat.v1.z, 0.0f);
		v1 = Vector4f32(m.sqmat.v2.x, m.sqmat.v2.y, m.sqmat.v2.z, 0.0f);
		v2 = Vector4f32(m.sqmat.v3.x, m.sqmat.v3.y, m.sqmat.v3.z, 0.0f);
	...
	template<Int32 iw, Int32 iz, Int32 iy, Int32 ix> MAXON_ATTRIBUTE_FORCE_INLINE Vector4f32 Multiply(
		const Vector4f32& x, const Vector4f32& y, const Vector4f32& z, const Vector4f32& w) const
	{
		Vector4f32 ret;

		Vector4f32 vx, vy, vz, vw;

#define ASSIGN_VECTOR(dst, src, index) \
		if constexpr (index >= 0 && index < 4) \
			dst = Vector4f32::Shuffle<index, index, index, index>(src); \
		else \
			dst = src;

		ASSIGN_VECTOR(vx, x, ix);
		ASSIGN_VECTOR(vy, y, iy);
		ASSIGN_VECTOR(vz, z, iz);
		ASSIGN_VECTOR(vw, w, iw);

#undef ASSIGN_VECTOR

		ret = v0 * vx + v1 * vy + v2 * vz + v3 * vw;
