...
namespace timezone
{
constexpr Hours GMT	 = Hours(0);	// Greenwich Mean Time
constexpr Hours CET	 = Hours(+1);	// Central European Time
constexpr Hours EET	 = Hours(+2);	// Eastern European Time
constexpr Hours PST	 = Hours(-8);	// Pacific Standard Time
constexpr Hours PNT	 = Hours(-7);	// Phoenix Standard Time
constexpr Hours CAT	 = Hours(-1);	// Central African Time
constexpr Hours AKST = Hours(-9);	// Alaska Standard Time
}
...
//----------------------------------------------------------------------------------------
/// Predefined datetime to format an ISO 8601 date string 
/// e.g. UniversalDateTime t; t.FormatTime(DATETIME_FORMAT_ISO8601) => "2019-11-28T16:12:13.000Z"
//----------------------------------------------------------------------------------------
#define DATETIME_FORMAT_ISO8601 "%Y-%m-%dT%H:%M:%S.000Z"
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Checks if the set year is a leap year.
	/// @return												True for a leap year, otherwise false.
	//----------------------------------------------------------------------------------------
	Bool IsLeapYear() const;

	//----------------------------------------------------------------------------------------
	/// Returns the day of the set time.
	...
	//----------------------------------------------------------------------------------------
	/// Returns the current timezone offset to the universal time coordinate (UTC).
	...
	//----------------------------------------------------------------------------------------
	/// Get the number of days in a given month of a given year. This will automatically produce correct results for leap years.
	/// @param[in] year								Year value.
	/// @param[in] month							Month value [1..12].
	/// @return												The number of days for the given month in the given year or an error object when month is out of range.
	//----------------------------------------------------------------------------------------
	static Result<UChar> GetDaysInMonth(Int year, Int month);

	//----------------------------------------------------------------------------------------
	/// Returns the day of the passed date.
	...
	//----------------------------------------------------------------------------------------
	/// Returns the first day of the week as specified by the operating system's locale settings.
	/// @return												The first day of the week.
	//----------------------------------------------------------------------------------------
	static DAYOFWEEK GetFirstWeekday();
	...
	/// The passed values (except for the year) are checked for their validity!
	...
	//----------------------------------------------------------------------------------------
	/// Creates a local date-time object by passing a Unix time stamp.
...
public:
	...
	{
		_timestamp += (UInt64)t.GetSeconds();
		return *this;
	}
	...
	{
		return Seconds((Float64)_timestamp - (Float64)x._timestamp);
	}

	//----------------------------------------------------------------------------------------
	/// Checks if the UniversalDateTime has a timestamp greater than 0. Even 0 is a valid timestamp this function can be used
	...
	/// The passed parameters will be checked for validity. That includes the year.
	...
	//----------------------------------------------------------------------------------------
	/// Create a universal date-time object by passing an ISO 8601 conformant string. This implementation is based on a subset of the ISO 8601
	/// specification as described e.g. here: https://en.wikipedia.org/wiki/ISO_8601.
	/// When only year and month are given, this will result in a value that represents the beginning (00:00) of the first day of that month in UTC.
	/// All dates are assumed to be gregorian. Unnecessary whitespace at the beginning and end of the string will be trimmed.
	/// Every time information more granular than seconds will be accepted but quietly discarded (not rounded).
	/// When no time zone information is given, the time is assumed to be in the current local time zone of the machine the code is run on.
	/// \warning Only the core standard is supported, extensions that are
	/// only allowed by agreement between sender and receiver (such as negative years) are not supported. Week numbering and ordinal dates are also not yet supported.
	///
	/// @param[in] str					The ISO 8601 conformant string
	/// @return 				The universal date-time object.
	//----------------------------------------------------------------------------------------
	...
	/// @return												A default value of the UniversalDateTime.
	//----------------------------------------------------------------------------------------
	static MAXON_ATTRIBUTE_FORCE_INLINE const UniversalDateTime& DefaultValue() { return g_defaultValue; }

private:
	UInt64 _timestamp = 0;
};

MAXON_DATATYPE(UniversalDateTime, "net.maxon.datatype.universaldatetime", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
MAXON_DATATYPE(LocalDateTime, "net.maxon.datatype.localdatetime", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));

#include "datetime1.hxx"
#include "datetime2.hxx"

//----------------------------------------------------------------------------------------
/// Helper function to use ScanString ScanParameter description.
