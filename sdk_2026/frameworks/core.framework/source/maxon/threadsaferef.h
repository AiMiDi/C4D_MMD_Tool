...
public:
	...
	POINTER AcquirePtr() const
	{
		...
		if (MAXON_UNLIKELY((value & 1) || (_ptr.TryCompareAndSwap(value | 1, value) == false)))
		{
			CpuYield wait(this);
	...
	POINTER ChangePtr(POINTER ptr) const
	{
		...
		if (MAXON_UNLIKELY((value & 1) || (_ptr.TryCompareAndSwap(UInt(ptr), value) == false)))
		{
			CpuYield wait(this);
	...
	Bool TryCompareAndSwapPtr(POINTER ptr, POINTER compare) const
	{
		...
		if (MAXON_UNLIKELY((value & 1) || (_ptr.TryCompareAndSwap(UInt(ptr), UInt(compare)) == false)))
		{
			CpuYield wait(this);
