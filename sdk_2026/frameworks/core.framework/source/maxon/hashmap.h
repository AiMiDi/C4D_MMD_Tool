...
//----------------------------------------------------------------------------------------
template <typename Entry, typename GENERAL_ALLOCATOR, typename BLOCK_ALLOCATOR> class HashMapAllocator
{
...
public:
	...
	{
		return _bucketAllocator.Alloc(size, MAXON_SOURCE_LOCATION_FORWARD);
	}
	void FreeBuckets(void* buckets)
	{
		_bucketAllocator.Free(buckets);
	}
	Entry* AllocEntry(Int size = SIZEOF(Entry))
	{
		return (Entry*)_entryAllocator.Alloc(size, MAXON_SOURCE_LOCATION_NAME(Entry));
	}
...
//----------------------------------------------------------------------------------------
template <typename Entry, typename ONE_ALLOCATOR> class HashMapAllocator<Entry, ONE_ALLOCATOR, ONE_ALLOCATOR> : public ONE_ALLOCATOR
{
public:
	void* AllocBuckets(Int size, MAXON_SOURCE_LOCATION_DECLARATION)
	{
		return this->Alloc(size, MAXON_SOURCE_LOCATION_FORWARD);
	}
	void FreeBuckets(void* buckets)
	{
		this->Free(buckets);
	...
	Entry* AllocEntry(Int size = SIZEOF(Entry))
	{
		return (Entry*)this->Alloc(size, MAXON_SOURCE_LOCATION_NAME(Entry));
	}
	void FreeEntry(const Entry* entry)
	{
		this->Free(entry);
...
class MAXON_ATTRIBUTE_EMPTY_BASES HashMap
	: public MapBase<HashMap<K, V, HASH, ENTRY_HANDLER, ALLOCATOR, MODE, INITIAL_CAPACITY, LOAD_FACTOR, ENTRY_ALLOCATOR>, K, V, EmptyClass, HASH>,
		protected HashMapAllocator<HashMapEntry<K, V, ENTRY_HANDLER, HASH>, ALLOCATOR, ENTRY_ALLOCATOR>
{
public:
	...
	//----------------------------------------------------------------------------------------
	explicit HashMap(const ALLOCATOR& alloc) : HashMapAllocator<HashMapEntry<K, V, ENTRY_HANDLER, HASH>, ALLOCATOR, ENTRY_ALLOCATOR>(alloc), _table(nullptr), _tableLengthM1(-1), _nonemptyBuckets(nullptr), _nonemptyBucketCount(0),
	...
	HashMap(HashMap&& src) : 
		HashMapAllocator<HashMapEntry<K, V, ENTRY_HANDLER, HASH>, ALLOCATOR, ENTRY_ALLOCATOR>(std::move(src)), 
		MAXON_MOVE_MEMBERS(_table, _tableLengthM1, _nonemptyBuckets, _nonemptyBucketCount, _size, _resizeThreshold)
	{
		src._table = nullptr;
...
protected:
	...
	Bool ResizeTableImpl(Int length)
	{
		...
		Bucket* t = (Bucket*)this->AllocBuckets(SIZEOF(Bucket) * length + (NONEMPTYBUCKETS ? SIZEOF(Bucket*) * length : 0), MAXON_SOURCE_LOCATION_NAME(Bucket));
		if (!t)
		{
			return false;
		}
