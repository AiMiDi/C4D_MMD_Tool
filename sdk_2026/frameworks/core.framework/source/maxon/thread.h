...
public:
	...
	{
		return ThreadServices::GetCurrentThreadType(threadIdentifier);
	}
...
template <typename IMPLEMENTATION, Bool USE_VIRTUAL_DESTRUCTOR = false> class ThreadInterfaceTemplate : public ThreadInterface
{
public:
	ThreadInterfaceTemplate() : ThreadInterface(GetJumpTable<IMPLEMENTATION, USE_VIRTUAL_DESTRUCTOR>()) {}
...
public:
	...
	{
		return ObservableCancelledBase<INTERFACE>::GetObservable((INTERFACE*) Ptr::GetPointer());
	}
	...
	static THREADTYPE GetCurrentThreadType(Int& threadIdentifier)
	{
		return INTERFACE::GetCurrentThreadType(threadIdentifier);
	}
...
template <typename FN> static auto ExecuteOnMainThreadFallback(FN&& fn) -> decltype(fn())
{
	...
	struct alignas(MAXON_FALSE_SHARING_SIZE) StaticJob
	{
		...
		ClosureJob<FN, JOBCANCELLATION::AUTOMATIC, typename std::invoke_result<FN>::type> _job;
	...
	staticJob._job.Enqueue(JobQueueRef::GetMainThreadQueue());
	staticJob._job.Wait();
	...
	return ExecuteOnMainThreadResult<typename std::invoke_result<FN>::type>::GetResult(staticJob._job.GetResult(TIMEVALUE_INFINITE, WAITMODE::DEFAULT));
}
}
...
//----------------------------------------------------------------------------------------
template <typename FN> inline auto ExecuteOnMainThread(FN&& fn, WAITMODE waitMode = WAITMODE::DEFAULT, TimeValue timeout = TIMEVALUE_INFINITE) -> decltype(fn())
{
	...
	{
		// On allocation error use the fallback to guarantee execution.
		iferr (auto job = JobRef::Create(std::forward<FN&&>(fn)))
			return maxon::details::ExecuteOnMainThreadFallback(std::forward<FN&&>(fn));

		job.Enqueue(JobQueueRef::GetMainThreadQueue());
		
		if (waitMode != WAITMODE::DONT_WAIT)
		{
			if (waitMode == WAITMODE::RETURN_ON_CANCEL)
			{
				// Special handling to cancel the job when it's still in the main thread queue and hasn't been executed yet ...
				if (job.Wait(timeout, WAITMODE::RETURN_ON_CANCEL) == false)
				{
					job.CancelAndWait();
					
					// Now just call GetResult() without waiting to return OperationCancelledError.
					timeout = Seconds(0);
				}
			}
			return maxon::details::ExecuteOnMainThreadResult<typename std::invoke_result<FN>::type>::GetResult(job.GetResult(timeout, waitMode));
		}
	}

	// Return void if the lambda returns no value and waitMode is WAITMODE::DONT_WAIT.
	return decltype(fn())();
}
