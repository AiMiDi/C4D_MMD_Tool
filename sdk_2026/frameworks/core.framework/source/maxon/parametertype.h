...
public:
	...
	explicit ParameterType(const DataType& t = DataType::DefaultValue(), FLAGS f = FLAGS::NONE) : _type(t), _typeForData(DataTypeLib::GetDataTypeForData(t) iferr_ignore("TODO")), _flags(f)
	{
	}
	...
	{
		const DataType& dt = maxon::GetDataType<T, GET_DATATYPE_POLICY::ERROR_FOR_UNKNOWN>();
		FLAGS flags = FLAGS::INPUT;
		using X = typename std::conditional<STD_IS_REPLACEMENT(reference, T), typename std::remove_reference<T>::type*, T>::type;
		if (STD_IS_REPLACEMENT(pointer, X) && !STD_IS_REPLACEMENT(const, typename std::remove_pointer<X>::type) && !STD_IS_REPLACEMENT(rvalue_reference, T))
		{
			flags |= FLAGS::OUTPUT;
		}
	...
	//----------------------------------------------------------------------------------------
	/// Returns true if a value of this type is passed to a reflection function invocation via a by-reference Argument.
	/// This holds for lvalue and rvalue reference types, non-scalar types as well as scalar types which don't fit into an Argument (i.e., into an Int64).
	/// @return												True if this type is passed via a by-reference Argument, false otherwise.
	//----------------------------------------------------------------------------------------
	Bool UsesByReferenceArgument() const
	{
#ifdef MAXON_TARGET_64BIT
		// true if REFERENCE_MASK is set or SCALAR is unset or INT_SIZE_OR_LESS is unset
		// has to match !Argument::ByValueTest
		return Bool((_type->GetValueKind() ^ (VALUEKIND::SCALAR | VALUEKIND::INT_SIZE_OR_LESS)) & (VALUEKIND::REFERENCE_MASK | VALUEKIND::SCALAR | VALUEKIND::INT_SIZE_OR_LESS));
#else
		return Bool((_type->GetValueKind() ^ VALUEKIND::SCALAR) & (VALUEKIND::REFERENCE_MASK | VALUEKIND::SCALAR)) || (_type->GetSize() >= SIZEOF(Int64));
#endif
	}
