...
// Disable exceptions for vcruntime.h before including anything
	...
	#define DOXYGEN_SWITCH_REVERSED(DOX, ...) __VA_ARGS__
#endif
...
#if defined(MAXON_DEPENDENCY_ENABLE) && defined(PRIVATE_MAXON_UNITY_BUILD_ID)
	static int g_dummy = PRIVATE_MAXON_UNITY_BUILD_ID();
...
//----------------------------------------------------------------------------------------
#define MAXON_DEPENDENCY_ENABLE 1234
#define PRIVATE_MAXON_DEPENDENCY_ENABLE_ORIGINAL 1234

#endif

// this is so that MAXON_CONCAT(MAXON_DEPENDENCY_ENABLE, 1) in PRIVATE_MAXON_DECLARATION expands to false if MAXON_DEPENDENCY_ENABLE is undefined
#define MAXON_DEPENDENCY_ENABLE1 false
...
/// @see MAXON_ATTRIBUTE_FORCE_INLINE. Will do nothing in debug builds, allowing to set breakpoints painlessly.
#define MAXON_ATTRIBUTE_FORCE_RELEASE_INLINE MAXON_IF_TARGET_DEBUG(, MAXON_ATTRIBUTE_FORCE_INLINE)
...
/// use MAXON_ATTRIBUTE_USED to tell the linker it must not dead-strip a function or static variable (even if it determines that the rest of the program does not refer to the object)
	...
	#define MAXON_HAS_ATTRIBUTE_ALWAYS_CONST 1
	#define MAXON_ATTRIBUTE_ALWAYS_CONST __attribute__((const))
	#define MAXON_ATTRIBUTE_ALWAYS_PURE	 __attribute__((pure))
#else
	#define MAXON_HAS_ATTRIBUTE_ALWAYS_CONST 0
	...
	#define PRIVATE_MAXON_DEPRECATED_ENUMVALUE(name, value, reason) name [[deprecated(reason)]] = value
#else
...
//----------------------------------------------------------------------------------------
template <typename T, Bool INTEGRAL = STD_IS_REPLACEMENT(integral, T)> struct StandardType
{
	using type = T;
};
...
template <typename T> struct StandardType<T, true>
{
	...
	static constexpr Bool KEEP = STD_IS_REPLACEMENT(same, DT, Bool) || STD_IS_REPLACEMENT(same, DT, Utf16Char) || STD_IS_REPLACEMENT(same, DT, Utf32Char);
...
static constexpr Int InvalidArrayIndex = -1;
...
static_assert(sizeof(GenericComponent) == 1);
...
//----------------------------------------------------------------------------------------
/// HashInt is used as return type of GetHashCode functions. It has the same bit width as Int.
...
//----------------------------------------------------------------------------------------
template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE const T& GetZeroRef()
{
	return *reinterpret_cast<const T*>(g_zeroMemory);
}
...
//----------------------------------------------------------------------------------------
static constexpr const void* const g_zeroPtr = nullptr;

//----------------------------------------------------------------------------------------
/// Returns a C++ reference to an object of type const T whose memory consists of immutable zero bytes
/// The size of T must not exceed the size of a pointer. This is not checked by the compiler to allow for incomplete types.
//----------------------------------------------------------------------------------------
template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE const T& GetPtrSizedZeroRef()
{
...
#ifdef _HAS_CHAR8_T
inline const String& StringInterface_GetAndCacheConstString(const char8_t* str, Int len, Int isUtf8)
{
	return StringInterface_GetAndCacheConstString((const Char*) str, len, isUtf8);
}

constexpr inline Bool PrivateIsUtf8(const char8_t* str, Int len)
{
	return (len == 0) ? false : ((len == 1) ? ((*str & 0x80) != 0) : (PrivateIsUtf8(str, len / 2) || PrivateIsUtf8(str + len / 2, len - len / 2)));
}
#endif
...
//----------------------------------------------------------------------------------------
/// DeleteReturnType01 to 09 is an incomplete type which is used for return types
/// of functions which should never be invoked. This is typically needed for the
/// disabling of function templates.
//----------------------------------------------------------------------------------------
class DeleteReturnType01;
class DeleteReturnType02;
class DeleteReturnType03;
class DeleteReturnType04;
class DeleteReturnType05;
class DeleteReturnType06;
class DeleteReturnType07;
class DeleteReturnType08;
...
static constexpr ENUM_DONT_INITIALIZE DONT_INITIALIZE = ENUM_DONT_INITIALIZE::VALUE;
...
static constexpr IN_PLACE_TYPE IN_PLACE = IN_PLACE_TYPE::VALUE;
...
template <typename... T> struct ParameterPack
{
	...
	{
		static constexpr Int value = LIMIT<Int>::MIN;
...
template <typename T, typename... REST> struct ParameterPack<T, REST...>
{
	...
	template <typename X> struct IndexOf
	{
		...
		static constexpr Int found = Rest::found + STD_IS_REPLACEMENT(same, T, X);
		static constexpr Int value = !STD_IS_REPLACEMENT(same, T, X) ? Rest::value + 1 : Rest::found ? LIMIT<Int>::MIN : 0;
	};
...
// Specialisation for PTMF to deduce the result type
...
// Specialisation for PTMF const to deduce the result type
...
public:
	...
	static constexpr Bool BOTH_DERIVED = STD_IS_REPLACEMENT(base_of, Base, typename std::decay<A>::type) && STD_IS_REPLACEMENT(base_of, Base, typename std::decay<B>::type);
	static constexpr Bool BOTH_LVALUES = STD_IS_REPLACEMENT(lvalue_reference, A) && STD_IS_REPLACEMENT(lvalue_reference, B);
	static constexpr Bool CONST_LVALUE = STD_IS_REPLACEMENT(const, typename std::remove_reference<A>::type) || STD_IS_REPLACEMENT(const, typename std::remove_reference<B>::type);
...
#define MAXON_MEMBERFUNCTION_DETECTOR(Name) \
	...
	{ \
		...
		struct Derived : public std::conditional<STD_IS_REPLACEMENT(class, DT), DT, maxon::EmptyClass>::type, public Default { Derived() = delete; }; \
...
template <typename T, Int COUNTER> class IsComplete : public IsCompleteBase<T, COUNTER, decltype(maxon::details::IsCompleteHelper(static_cast<T*>(nullptr)))::value> { };
...
#define MAXON_MEMBERTYPE_DETECTOR(Name, Member, Default) \
	...
	{ \
		...
		static const maxon::Bool value = !STD_IS_REPLACEMENT(same, type, std::false_type); \
	};
...
/// type alias for the @c ErasedType of T. Otherwise @c type is a type alias for T itself.
/// @tparam T											Type to check.
//----------------------------------------------------------------------------------------
...
template <typename REF> struct GetDirectlyReferencedTypeCheckConstAndPtr
{
	...
	static const Bool value = Base::value || STD_IS_REPLACEMENT(pointer, RefType);
...
//----------------------------------------------------------------------------------------
/// This type trait gets the @c HasBaseDetector member type of @p T. If this exists,
/// <tt>CheckHasBase<T>::value</tt> will be @c true and <tt>CheckHasBase<T>::type</tt> will be a type alias to this member type.
/// @tparam T											Type to check.
//----------------------------------------------------------------------------------------
MAXON_MEMBERTYPE_DETECTOR(CheckHasBase, HasBaseDetector, std::false_type);
...
{
	static const Bool value = STD_IS_REPLACEMENT(scalar, T) && !STD_IS_REPLACEMENT(floating_point, T);
};


//----------------------------------------------------------------------------------------
/// This type trait determines if the UniqueHash of values of type @p T can be computed "trivially"
...
/// Alternatively you can add a static member function "static constexpr Bool TriviallyHashable() { return true; }".
/// @tparam T											Type to check.
//----------------------------------------------------------------------------------------
template <typename T, typename = Bool> struct IsTriviallyHashable
{
	static const Bool value = STD_IS_REPLACEMENT(scalar, T);
};

// Allow to override IsTriviallyHashable by static member function TriviallyHashable.
// We use a member function instead of a constant or type because only a function allows to make use of T
// in its definition (for constant or type we'd get "incomplete T" errors when using things like sizeof(T)).
template <typename T> struct IsTriviallyHashable<T, decltype(T::TriviallyHashable())>
{
	static const Bool value = T::TriviallyHashable();
};
...
/// @tparam T											Type to check.
//----------------------------------------------------------------------------------------
template <typename T> struct IsZeroInitialized : public std::integral_constant<Bool, STD_IS_REPLACEMENT(scalar, T)>::type
{
};
...
// HAS_BASE_DETECTOR_OR_SAME is 2 when D and B are the same, 1 if the class has a HasBaseDetector member type, 0 otherwise.
template <typename D, typename B, Int HAS_BASE_DETECTOR_OR_SAME> class HasBaseHelper : public std::is_base_of<B, D>
{
public:
	static B* Cast(D* ptr)
	{
		// We don't use static_cast because that doesn't work when D has private inheritance from B.
		static_assert(std::is_base_of<B, D>::value, "Illegal cast.");
		return reinterpret_cast<B*>(ptr);
	}
};

template <typename D, typename B> class HasBaseHelper<D, B, 1> : public D::HasBaseDetector::template Check<B>
{
};
...
//----------------------------------------------------------------------------------------
template <typename D, typename B> struct HasBaseDetector
{
...
//----------------------------------------------------------------------------------------
/// IsDerived can be used to bind BASE to the second parameter of HasBaseDetector:
...
{
	template <typename DERIVED> struct Check : public HasBaseDetector<DERIVED, BASE>
	{
	};
};
...
{
	static constexpr Bool value = STD_IS_REPLACEMENT(same, TO, FROM);
};
...
{
	static constexpr Bool value = STD_IS_REPLACEMENT(base_of, TO, FROM) || (!SAFE && STD_IS_REPLACEMENT(base_of, FROM, TO)) || GenericCastMemberTrait<TO, typename std::remove_const<FROM>::type, SAFE>::value;
};

template <typename TO, typename FROM, Bool SAFE> struct GenericCastMemberTrait<TO&, FROM&, SAFE>
{
	static constexpr Bool value = !SAFE && (STD_IS_REPLACEMENT(base_of, TO, FROM) || STD_IS_REPLACEMENT(base_of, FROM, TO) || GenericCastMemberTrait<TO, typename std::remove_const<FROM>::type, SAFE>::value);
};
...
{
public:
	static const Bool value = STD_IS_REPLACEMENT(scalar, T) || STD_IS_REPLACEMENT(reference, T) || STD_IS_REPLACEMENT(void, T);
...
public:
	...
	static type* GetPointer(typename std::conditional<STD_IS_REPLACEMENT(const, T), DummyParamType&, T*>::type ptr)
	{
		return ptr;
	}
};
...
//----------------------------------------------------------------------------------------
template <typename T, Bool ALSO_REFS = true> class Dereference : public DereferenceHelper<T, ALSO_REFS && GetReferencedType<T>::value, STD_IS_REPLACEMENT(function, typename std::remove_pointer<T>::type)>
{
};
...
template <typename T, Bool ALLOW_NONCONST = false> inline T DefaultValue();

template <typename T> [[deprecated("use DefaultValue (since 2024.0)")]] MAXON_ATTRIBUTE_FORCE_INLINE T NullValue()
{
...
{
public:
	template <typename... ARGS> constexpr explicit ForEach(ARGS&&...)
	{
	}
};
...
#include "maxon/utilities/hash.h"
...
namespace details
{
static constexpr UInt64 ModuleIdHash = CStringCompare::GetHashCode(MAXON_MODULE_ID);

inline constexpr Bool CheckImplementationModule(const void*, maxon::Char)
{
	return true;
}

template <typename T> inline constexpr Bool CheckImplementationModule(T*, decltype(T::PrivateImplementationModule[0]))
{
	// return maxon::CStringCompare::GetHashCode(T::PrivateImplementationModule) == details::ModuleIdHash;
	// Copied from CStringCompare::GetHashCode, for some unknown reasons GCC doesn't compile the line above.
	HashInt hash = 0;
	for (Int i = 0; i < SIZEOF(T::PrivateImplementationModule) - 1; ++i)
	{
		hash = hash * 31 + UChar(T::PrivateImplementationModule[i]);
	}
	return hash == details::ModuleIdHash;
}
...
#define PRIVATE_MAXON_CHECK_IMPLEMENTATION_MODULE(DECLARATION) \
	static_assert(maxon::details::CheckImplementationModule(static_cast<DECLARATION*>(nullptr), maxon::Char(0)), "The implementation module \"" MAXON_MODULE_ID "\" doesn't match the one given in MAXON_IMPLEMENTATION_MODULE.")
...
/// This would break binary compatibility and prevent register usage (on Windows).
...
public:
	...
	// Do not add a constructor to this class and do not use default member initialization!
	// It is intentionally without constructor to be able to pass the value in a register on all operating systems.
	// Adding constructors would furthermore break binary compatibility.
	// Do not add a constructor to this class and do not use default member initialization!
	...
	MAXON_ATTRIBUTE_FORCE_INLINE void operator =(const ErrorPtr& error)
	{
	...
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE const Error* GetReferencedError() const
	{
		return _error;
	}

	MAXON_ATTRIBUTE_FORCE_INLINE operator ErrorPtr&()
	{
		return *reinterpret_cast<ErrorPtr*>(this);
	}

	MAXON_ATTRIBUTE_FORCE_INLINE operator NoErrorPtr&()
	{
...
public:
	...
	enum class FLAGS : UInt32
	{
		...
		/// This flag is set for sourceprocessor-generated translation units.
		GENERATED = 0x0800,
		...
		/// The object definition is for a Class.
		CLASS_DEFINITION = 0x1000,	// (we can re-use FRAMEWORK because the former is only used for modules and this is for objects)
	...
	void PrivateAddFlags(FLAGS flags)
	{
		_flags |= flags;
	}
...
template <UInt64 MODULE_ID_HASH = ModuleIdHash> struct ModuleInfoWrapper
{
...
public:
	...
	TranslationUnit(TranslationUnit* main, const Char* unityId, EntityBase::FLAGS flags);
	...
	TranslationUnit* PrivateGetCurrentUnit()
	{
		return _currentSubUnit ? _currentSubUnit : this;
	}
...
private:
	...
	inline void AddWeakUse(WeakUse* use);
	...
	const WeakUse* _weakUses; ///< Head of singly linked list of weak uses contained in this unit.
	...
	TranslationUnit* _currentSubUnit;	///< Points to the current subunit of a unity build, only used for the main translation unit.
	TranslationUnit* _previousSubUnit; ///< Points to the previous subunit of a unity build, nullptr for the main translation unit.

	friend class ObjectModel;
	friend class EntityDefinition;
	friend class EntityUse;
	friend class WeakUse;
...
/// @endcond
		...
		#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS maxon::EntityBase::FLAGS::FRAMEWORK_REGISTRATION
	#else
		#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS maxon::EntityBase::FLAGS::NONE
	#endif
#endif

#ifdef MAXON_INITIALIZATION_IGNORE_WARNINGS
	#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_2 | maxon::EntityBase::FLAGS::IGNORE_WARNINGS
#else
	#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_2
#endif

#ifdef PRIVATE_MAXON_GENERATED_UNIT
	#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_3 | maxon::EntityBase::FLAGS::GENERATED
#else
	#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_3
#endif

// We put g_mainTranslationUnit in an anonymous namespace: Otherwise we had to use static, but then some compilers
// don't allow to use g_mainTranslationUnit as template argument.
namespace
{

#define PRIVATE_MAXON_PLACEHOLDER_FILENAME "??.cpp"

#define PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_ALL PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_2 PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_3

#ifdef PRIVATE_MAXON_REGISTRATION_UNIT
TranslationUnit g_mainTranslationUnit{&PRIVATE_MAXON_MODULE, "register.cpp", EntityBase::FLAGS::REGISTRATION_UNIT};
#elif defined(MAXON_TARGET_DEBUG) && defined(PRIVATE_MAXON_UNITY_BUILD_ID) && !defined(MAXON_DISABLE_SOURCE_LOCATION)
TranslationUnit g_mainTranslationUnit{&PRIVATE_MAXON_MODULE, MAXON_STRINGIFY(PRIVATE_MAXON_UNITY_BUILD_ID), PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_ALL};
#elif defined(MAXON_TARGET_DEBUG) && defined(__BASE_FILE__) && !defined(MAXON_DISABLE_SOURCE_LOCATION)
TranslationUnit g_mainTranslationUnit{&PRIVATE_MAXON_MODULE, __BASE_FILE__, PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_ALL};
#else
// For release builds the dependency error output on startup might not contain all the relevant information.
// If there are problems with the dependency you might have to use a debug build to examine this.
TranslationUnit g_mainTranslationUnit{&PRIVATE_MAXON_MODULE, PRIVATE_MAXON_PLACEHOLDER_FILENAME, PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_ALL};
#endif

#define PRIVATE_MAXON_TRANSLATION_SUBUNIT maxon::g_mainTranslationUnit
...
enum class SYSTEMBEHAVIORFLAGS : Int
{
	...
	IGNORE_UNKNOWN_CONFIGURATIONS =  (1 << 2),	///< Do not warn about unknown configurations (python).
	CREATE_PREFERENCES = 				 (1 << 3)	///< Create preference directory.
} MAXON_ENUM_FLAGS(SYSTEMBEHAVIORFLAGS);
...
// The DefaultValue template is a useful helper to return a default value of a generic type
...
} MAXON_ENUM_ORDERED_FLAGS(VALUEKIND, "net.maxon.datatype.enum.valuekind", EARLY);
...
{
	static constexpr VALUEKIND KIND = ConditionalFlag(STD_IS_REPLACEMENT(trivially_copyable, T), VALUEKIND::TRIVIALLY_COPYABLE)
#ifndef MAXON_COMPILER_GCC
		| ConditionalFlag(STD_IS_REPLACEMENT(trivially_constructible, T), VALUEKIND::TRIVIALLY_CONSTRUCTIBLE)
#endif
		| ConditionalFlag(STD_IS_REPLACEMENT(trivially_destructible, T), VALUEKIND::TRIVIALLY_DESTRUCTIBLE)
		| ConditionalFlag(STD_IS_REPLACEMENT(scalar, T), VALUEKIND::SCALAR)
		| ConditionalFlag(STD_IS_REPLACEMENT(integral, T), VALUEKIND::INTEGRAL | VALUEKIND::VALUE)
		| ConditionalFlag(STD_IS_REPLACEMENT(signed, T), VALUEKIND::SIGNED)
		| ConditionalFlag(STD_IS_REPLACEMENT(floating_point, T), VALUEKIND::FLOATING_POINT | VALUEKIND::VALUE)
		| ConditionalFlag(IsTriviallyEquatable<T>::value, VALUEKIND::TRIVIALLY_EQUATABLE)
		| ConditionalFlag(IsTriviallyHashable<T>::value, VALUEKIND::TRIVIALLY_HASHABLE)
...
{
	static constexpr VALUEKIND KIND = T::Handler::KIND;
};

template <> struct ValueKindHelper<ObjectInterface, false>
{
	static constexpr VALUEKIND KIND = VALUEKIND::VIRTUAL_INTERFACE | VALUEKIND::ABSTRACT;
};
...
// need to be defined before newobj.h include
...
template <typename... INPUTTYPE> inline Result<Id> Id::Create(INPUTTYPE&&... inputId)
{
	iferr_scope;
	Id id;
	id.Init(std::forward<INPUTTYPE>(inputId)...) iferr_return;
	return id;
}
...
public:
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <typename SRC> static MAXON_ATTRIBUTE_FORCE_INLINE ResultOk<void> Copy(T& dst, SRC&& src)
	{
		new (std::addressof(dst)) T(std::forward<SRC>(src));
		return OK;
	}
...
//----------------------------------------------------------------------------------------
/// @brief Default placement new call which does not null the memory (unless it's a scalar type). 
...
//----------------------------------------------------------------------------------------
template <typename T, Bool ZEROINITIALIZE = false> typename std::enable_if<!std::is_scalar<T>::value, T*>::type PlacementNew(void* ptr)
{
	if constexpr (ZEROINITIALIZE)
		return new (ptr) T();
	else
		return new (ptr) T;
}
...
//----------------------------------------------------------------------------------------
/// @brief Placement new call with forwarded values.
//----------------------------------------------------------------------------------------
template <typename T, Bool ZEROINITIALIZE, typename... ARGS> T* PlacementNew(void* ptr, ARGS&&... args)
{
...
//----------------------------------------------------------------------------------------
template <typename T, typename ARG> MAXON_ATTRIBUTE_FORCE_INLINE auto NewCopy(T& dst, const ARG& src)
{
...
{
	return T(std::forward<T>(src));
}

//----------------------------------------------------------------------------------------
/// Copy assignment
//----------------------------------------------------------------------------------------
template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE auto AssignCopy(T& dst, const T& src)
{
...
//----------------------------------------------------------------------------------------
template <typename T, typename U> MAXON_ATTRIBUTE_FORCE_INLINE auto AssignCopyConvert(T& dst, const U& src)
{
	return ObjectConstructor<T, TestForCopyFromMember<T>::isSupported>::AssignCopy(dst, src);
}
...
/// @tparam T											Type of object.
//----------------------------------------------------------------------------------------
template <typename T> inline DestructorPointer GetDestructorPointer()
{
	// To avoid creation of unnecessary function code we use Char for any trivially destructible type.
	return (DestructorPointer) &Destruct<typename std::conditional<std::is_trivially_destructible<T>::value, Char, T>::type>;
}
...
namespace details
{
template <typename T, typename... ARGS> static maxon::ResultOk<T> CreateHelper(OverloadRank0, ARGS&&... args)
{
	return T(std::forward<ARGS>(args)...);
}

template <typename T, typename... ARGS> static typename std::enable_if<maxon::HasInit<T>::value, maxon::Result<T>>::type CreateHelper(OverloadRank1, ARGS&&... args)
{
	iferr_scope;
	T obj;
	obj.Init(std::forward<ARGS>(args)...) iferr_return;
	return obj;
}
}
...
{
	return maxon::details::CreateHelper<T>(OVERLOAD_MAX_RANK, std::forward<ARGS>(args)...);
}
...
//----------------------------------------------------------------------------------------
/// Use this class when you need a default argument for parameters of type VALUETYPE&.
...
public:
	...
	explicit DefaultLValue(const VALUETYPE& value) : _value(value)
	{}
...
/// @}
	...
	#define MAXON_MODULE_SUFFIX "xdl64"
#elif defined MAXON_TARGET_LINUX
	#ifdef MAXON_TARGET_64BIT
		#define MAXON_MODULE_SUFFIX "xso64"
	#else
		#define MAXON_MODULE_SUFFIX "xso32"
	#endif
#elif defined(MAXON_TARGET_MACOS) || defined(MAXON_TARGET_IOS)
	#define MAXON_MODULE_SUFFIX "xlib"
#else
