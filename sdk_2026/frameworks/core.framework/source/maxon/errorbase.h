...
{
	MAXON_INTERFACE(OutOfMemoryErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.outofmemoryerror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(FunctionNotImplementedErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.functionnotimplementederror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(IllegalArgumentErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.illegalargumenterror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(NullptrErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.nullptrerror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(IllegalStateErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.illegalstateerror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(UnexpectedErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.unexpectederror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(OperationCancelledErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.OperationCancelledError", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
};
...
{
	MAXON_INTERFACE(AggregatedErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.aggregatederror", MAXON_IMPLEMENTATION_MODULE("net.maxon.kernel"));
...
public:
	...
	explicit AggregatedError(MAXON_SOURCE_LOCATION_DECLARATION);
...
public:
	...
	template <typename TO> using HasBaseDetector = maxon::HasBaseDetector<const ErrorInterface, TO>;

	ErrorPtr& PrivateGetRefMember();

	const ErrorPtr& PrivateGetRefMember() const;

	void PrivateSetPointer(const ErrorInterface* const* ptr) { _ptr = ptr; }
...
//----------------------------------------------------------------------------------------
class ErrorPtr : public ErrorInterface::ReferenceFunctions<RefBaseFn<ErrorIndirectRef>>::type
{
public:
	//----------------------------------------------------------------------------------------
...
#define PRIVATE_MAXON_iferr_(ErrType, BUILTINLIKELY, LINE, ...) \
	maxon::ThreadReferencedError MAXON_CONCAT(tmperr_, LINE); \
	__VA_ARGS__ % MAXON_CONCAT(tmperr_, LINE); \
	MAXON_WARNING_PUSH \
	MAXON_WARNING_DISABLE_VARIABLE_SHADOWING \
	if (maxon::ErrType err = MAXON_CONCAT(tmperr_, LINE); BUILTINLIKELY(err)) \
	MAXON_WARNING_POP


#define PRIVATE_MAXON_iferr(...)		PRIVATE_MAXON_iferr_(ErrorPtr, MAXON_UNLIKELY, __COUNTER__, __VA_ARGS__)
#define PRIVATE_MAXON_ifnoerr(...)	PRIVATE_MAXON_iferr_(NoErrorPtr, MAXON_LIKELY, __COUNTER__, __VA_ARGS__)
...
//----------------------------------------------------------------------------------------
/// This macro checks if the given object is nullptr, if thats the case an
/// nullptr is returned from the current function. The condition will be part
/// of the error message (taken over verbatim). The macro can only be used in a function
/// which returns a Result<T>.
///
/// Optionally, you can specify a message which will be used instead of the verbatim condition.
...
///
/// For examples, see #CheckArgument.
/// @param[in] pointer					The pointer to check for If it evaluates to false, an NullptrError will be returned.
/// @param[in] ...								Optional message and parameters, will be formatted by @ref FormatString.
/// @see CheckArgument
/// @see CheckAssert
/// @see CheckStateDebugStop
//----------------------------------------------------------------------------------------
#define CheckPointer(pointer, ...) do { if (MAXON_UNLIKELY(pointer == nullptr)) iferr_throw(maxon::NullptrError(PRIVATE_MAXON_CHECKERROR_A("Nullptr Error", condition, MAXON_VA_ARGS_POPULATED(__VA_ARGS__), __VA_ARGS__))); } while (false)

//----------------------------------------------------------------------------------------
/// Like CheckPointer, but an additional DebugStop is executed on error.
//----------------------------------------------------------------------------------------
#define CheckPointerDebugStop(pointer, ...) do { if (MAXON_UNLIKELY(pointer == nullptr)) iferr_throw(maxon::NullptrError(PRIVATE_MAXON_CHECKERROR_A("Nullptr Error", condition, MAXON_VA_ARGS_POPULATED(__VA_ARGS__), __VA_ARGS__)).DbgStop()); } while (false)
