...
// from https://stackoverflow.com/questions/36167517/m256d-transpose4-equivalent
...
class Vector4f64
{
	...
	friend Vector4f64 ConvertToVector4f64(const Vector2f64& a, const Vector2f64& b);

	friend void ConvertToVectors2f64(const Vector4f64& a, Vector2f64& result1, Vector2f64& result2);
...
public:
	...
	{
		val = (d == 0.0) ? _mm256_setzero_pd() : _mm256_set1_pd(d);
	}
	...
	{
		val = _mm256_set_pd(d3, d2, d1, d0);
	}

	//----------------------------------------------------------------------------------------
	/// Constructs the object from an input vector.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit Vector4f64(const Vector& v, Bool aligned)
	{
...
#error TODO
		...
		val = _mm256_castps_pd(_mm256_set_m128(_mm_castpd_ps(yy), _mm_castpd_ps(xx)));
#endif
	}
	...
	/// Constructs the object from an input vector.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE explicit Vector4f64(const Vector& v, Float w, Bool aligned)
	{
#ifdef MAXON_TARGET_SINGLEPRECISION
		if (aligned)
		{
...
#error TODO
	...
	//----------------------------------------------------------------------------------------
	/// Loads a single value from f and broadcasts it to all elements of the returned vector.
	//----------------------------------------------------------------------------------------
	static Vector4f64 Broadcast(const Float64* v)
	{
		return Vector4f64(_mm256_broadcast_sd(v));
	}
	...
	{
		return Vector4f64(_mm256_add_pd(a.val, b.val));
	}
	...
	{
		return Vector4f64(_mm256_add_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	{
		return Vector4f64(_mm256_add_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator -(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_sub_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator -(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_sub_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator -(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_sub_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator *(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_mul_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator *(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_mul_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator *(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_mul_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator /(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_div_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator /(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_div_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator /(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_div_pd(_mm256_set1_pd(a), b.val));
	}
	...
	{
		val = _mm256_add_pd(val, _mm256_set1_pd(a));
		return *this;
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Vector4f64& operator -=(const Float64 a)
	{
		val = _mm256_sub_pd(val, _mm256_set1_pd(a));
		return *this;
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Vector4f64& operator *=(const Float64 a)
	{
		val = _mm256_mul_pd(val, _mm256_set1_pd(a));
		return *this;
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Vector4f64& operator /=(const Float64 a)
	{
		val = _mm256_div_pd(val, _mm256_set1_pd(a));
		return *this;
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 HorizontalAdd(const Vector4f64 &a)
	{
		Vector4f64 ret;
		__m256d x, y;
		x = _mm256_permute2f128_pd(a.val, a.val, 1); // 2 3 0 1
		x = _mm256_add_pd(x, a.val); // (2 + 0) (3 + 1) (0 + 2) (1 + 3)
		y = _mm256_unpacklo_pd(x, x); // (2 + 0) (2 + 0) (0 + 2) (0 + 2)
		x = _mm256_unpackhi_pd(x, x); // (3 + 1) (3 + 1) (1 + 3) (1 + 3)
		ret.val = _mm256_add_pd(x, y); // (2 + 0 + 3 + 1) (2 + 0 + 3 + 1) (0 + 2 + 1 + 3) (0 + 2 + 1 + 3)*/
		return ret;
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 MultiplyAdd(const Vector4f64 &a, const Vector4f64 &b, const Vector4f64 &c)
	{
		...
		return Vector4f64(_mm256_fmadd_pd(a.val, b.val, c.val));
#else
		return Vector4f64(_mm256_add_pd(_mm256_mul_ps(a.val, b.val), c.val));
#endif
	}
	...
	{
		Vector4f64 ret;
		ret.val = _mm256_and_pd(a.val, b.val);
		return ret;
	}
	...
	{
		Vector4f64 ret;
		ret.val = _mm256_or_pd(a.val, b.val);
		return ret;
	}
	...
	{
		Vector4f64 ret;
		ret.val = _mm256_xor_pd(a.val, b.val);
		return ret;
	}
	...
	{
		return Vector4f64(_mm256_blendv_pd(val, b.val, mask.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator <=(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmple_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator <=(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_cmple_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator <=(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmple_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator ==(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpeq_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator ==(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_cmpeq_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator ==(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpeq_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator >=(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpge_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator >=(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_cmpge_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator >=(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpge_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator >(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpgt_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator >(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_cmpgt_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator >(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpgt_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator <(const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmplt_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator <(const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_cmplt_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator <(const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmplt_pd(_mm256_set1_pd(a), b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator != (const Vector4f64 &a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpneq_pd(a.val, b.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator != (const Vector4f64 &a, const Float64 b)
	{
		return Vector4f64(_mm256_cmpneq_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 operator != (const Float64 a, const Vector4f64 &b)
	{
		return Vector4f64(_mm256_cmpneq_pd(_mm256_set1_pd(a), b.val));
	}
	...
	{
		return _mm256_movemask_pd(_mm256_cmpneq_pd(a.val, b.val)) == 0;
	}
	...
	{
		return _mm256_movemask_pd(_mm256_cmpneq_pd(a.val, _mm256_set1_pd(b))) == 0;
	}
	...
	{
		return _mm256_movemask_pd(_mm256_cmpneq_pd(_mm256_set1_pd(a), b.val)) == 0;
	}
	...
	{
		return Vector4f64(_mm256_min_pd(a.val, b.val));
	}
	...
	{
		return Vector4f64(_mm256_min_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	{
		return Vector4f64(_mm256_min_pd(_mm256_set1_pd(a), b.val));
	}
	...
	{
		return Vector4f64(_mm256_max_pd(a.val, b.val));
	}
	...
	{
		return Vector4f64(_mm256_max_pd(a.val, _mm256_set1_pd(b)));
	}
	...
	{
		return Vector4f64(_mm256_max_pd(_mm256_set1_pd(a), b.val));
	}
	...
	{
		return Vector4f64(_mm256_permute2f128_pd(v.val, v.val, ((int)(((B) << 4) | (A)))));
	}

	template<int D, int C, int B, int A> static MAXON_ATTRIBUTE_FORCE_INLINE Vector4f64 Shuffle(const Vector4f64& v)
	{
		return Vector4f64(_mm256_permute4x64_pd(v.val, A + (B << 2) + (C << 4) + (D << 6)));
	}


	// Sets all elements of the vector to the element at the given index.
	template<int A> static MAXON_ATTRIBUTE_FORCE_INLINE Vector4f64 SetAll(const Vector4f64& x)
	{
		return Vector4f64(_mm256_permute4x64_pd(x.val, A + (A << 2) + (A << 4) + (A << 6)));
	}
	...
	{
		return Vector4f64(_mm256_shuffle_pd(a.val, b.val, (mb23 << 3) | (ma23 << 2) | (mb01 << 1) | (ma01)));
	}
	...
	{
		return Vector4f64(_mm256_blendv_pd(b.val, a.val, mask.val));
	}
	...
	{
		return Vector4f64(_mm256_sqrt_pd(a.val));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 Abs(const Vector4f64& a)
	{
		...
		return Vector4f64(_mm256_and_pd(_mm256_castsi256_pd(mask), a.val));
	}
	...
	{
		return Vector4f64(_mm256_blendv_pd(_mm256_set1_pd(1.0), _mm256_set1_pd(-1.0), _mm256_cmp_pd(a.val, _mm256_setzero_pd(), _CMP_NGE_UQ)));
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Vector4f64 SignMask(const Vector4f64& a)
	{
		...
		return Vector4f64(_mm256_and_pd(a.val, _mm256_castsi256_pd(mask)));
	}
	...
	{
		return _mm256_movemask_pd(a.val) != 0;
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE friend Int SelectMin(const Vector4f64& mask, const Vector4f64& a)
	{
		...
		const Int32 value					= _mm256_movemask_pd(Any(validMin) ? validMin.val : mask.val);
	...
	static void Transpose(Vector4f64* p)
	{
		_MM_TRANSPOSE4_PD(p[0].val, p[1].val, p[2].val, p[3].val);
	}
...
#ifdef C4D_HAS_AVX_CAPABILITY
static_assert(sizeof(Vector4f64) == sizeof(__m256d), "size mismatch");
#else
static_assert(sizeof(Vector4f64) == 4 * sizeof(Float64), "size mismatch");
#endif // C4D_HAS_AVX_CAPABILITY
