...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief ObservableFinished is an observable that is triggered after this job has been executed.
...
//----------------------------------------------------------------------------------------
/// @brief A job is a threaded entity that can be enqueued or added to a group.
/// A JobRef is similar to a ThreadRef but all jobs on the same queue (JobQueueRef) share a set of
...
public:
	//----------------------------------------------------------------------------------------
	/// @brief Initializes the JobInterface including the jump table to the implementation methods.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Waits until this job has been executed.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Waits until this job has been executed and returns the result.
	/// THREADSAFE.
	/// @param[in] timeout						Maximum wait interval (or TIMEVALUE_INFINITE for no time-out).
	/// @param[in] mode								WAITMODE::DEFAULT by default. WAITMODE::RETURN_ON_CANCEL means that Wait() will return if the caller has been cancelled even if the condition has not been set yet.
	/// @return												OK on success.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Waits until this job has been executed and returns the result via std::move.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the job to cancel execution.
	/// If the job is a part of a group the whole group will be cancelled. Cancellation of a job does not affect its finish observers.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if the job should stop.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the job to cancel execution and waits until it has finished.
	/// If the job is a part of a group the whole group will be cancelled. Cancellation of a job does not affect its finish observers.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues this job (will add a reference and remove it when the object is no longer needed).
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a job to this job's group (see JobGroup). The caller must belong to a group.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a subgroup to this job's group. The caller must belong to a group.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the group a job belongs to.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief ObservableCancelled is an observable that is triggered when this job is cancelled.
	/// THREADSAFE.
	/// @return												Custom observable.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Appends a job which is enqueued after this job has been executed.
	/// THREADSAFE.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the index of the internal worker thread which is running this job.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the number of worker threads for the current job context. 
	/// This might be different from the actual CPU core or thread count.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if the currently running job (or thread) should stop.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a pointer to the currently running job which can be used to check if it's cancelled.
	/// Works for jobs and threads.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a readable string of the content.
...
/// @cond INTERNAL
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the internal jump table for this job.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Registers an observer job that will be called after this job has been executed.
	/// You can specify an optional queue that should be used to run the observer which can be
	/// handly if you for example want to update the UI from the main thread after some threaded
	/// work has been done. By default the observer is not queued and runs in the same context
	/// as the job.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Registers an observer job that will be called when this job has been cancelled.
...
private:
	...
	template <JOBCANCELLATION B, typename FN, typename DEPENDENCY, typename... ARGS> static ResultPtr<JobInterface> CreateJobWithDependency(DEPENDENCY&& dependency, FN&& src, ARGS&&... args);
	template <JOBCANCELLATION B, typename FN, typename... ARGS> static ResultPtr<JobInterface> CreateJobWithDependency(std::nullptr_t, FN&& src, ARGS&&... args);
...
//----------------------------------------------------------------------------------------
/// @brief Restricted job interface with the only purpose of being able to check for cancellation.
...
//----------------------------------------------------------------------------------------
/// @brief Reference to a job for checking cancellation status only (JobStatusInterface).
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a default value of the JobRef (see defaultvalue.h for more details).
...
/// @brief Template for jobs which return more than just a void result.
...
//----------------------------------------------------------------------------------------
/// @brief Template for jobs which inherit from JobInterface (and are not created using a lambda).
...
template <JOBCANCELLATION B, typename FN, typename DEPENDENCY, typename... ARGS> ResultPtr<JobInterface> JobInterface::CreateJobWithDependency(DEPENDENCY&& dependency, FN&& src, ARGS&&... args)
{
	using Job = ClosureJobWithDependency<FN, B, decltype(src(std::forward<ARGS>(args)...)), ARGS...>;
	return NewObj(Job, std::forward<FN>(src), reinterpret_cast<const JobDependencyGroupInterface*>(dependency), std::forward<ARGS>(args)...);
}

template <JOBCANCELLATION B, typename FN, typename... ARGS> ResultPtr<JobInterface> JobInterface::CreateJobWithDependency(std::nullptr_t, FN&& src, ARGS&&... args)
{
	using Job = ClosureJobWithDependency<FN, B, decltype(src(std::forward<ARGS>(args)...)), ARGS...>;
	return NewObj(Job, std::forward<FN>(src), nullptr, std::forward<ARGS>(args)...);
}
...
template <typename RESULT_TYPE> struct ResultMemJob : public ResultMemT<JobResultRef<RESULT_TYPE>>
{
	...
	//----------------------------------------------------------------------------------------
	/// @brief Chains a job to a predecessor.
	...
	//----------------------------------------------------------------------------------------
	template <typename FN> MAXON_WARN_UNUSED auto Then(FN&& src, JobQueueInterface* queue = JOBQUEUE_CURRENT) -> ResultMemJob<typename GetSignature<decltype(&FN::operator ())>::ReturnType>
	{
		return this->GetValue().Then(std::forward<FN>(src), queue);
	}
...
//	template <typename SUPER> using ResultFunctions = maxon::details::ResultJobFunctions<SUPER>;
	...
	//----------------------------------------------------------------------------------------
	/// @brief Creates a reference to a job with n arguments.
	/// @param[in] src								Lambda or object with operator ().
	...
	//----------------------------------------------------------------------------------------
	/// @brief Creates a reference to a job with n arguments which has a dependency.
	/// @param[in] dependency					Pointer to a dependency (can be any object, memory block, etc.). Passing a nullptr indicates an independent job (which might even be executed recursively).
	/// @param[in] src								Lambda or object with operator ().
	/// @param[in] args								Arguments for the function.
	/// @tparam B											Behaviour for early job cancellation.
	/// @tparam FN										Type of function/lambda, deduced by the compiler.
	/// @tparam DEPENDENCY						Type of dependency, deduced by the compiler.
	/// @tparam ARGS									Parameter types.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	template <JOBCANCELLATION B = JOBCANCELLATION::AUTOMATIC, typename FN, typename DEPENDENCY, typename... ARGS> static auto CreateWithDependency(DEPENDENCY&& dependency, FN&& src, ARGS&&... args) -> ResultMemT<JobResultRef<decltype(src(std::forward<ARGS>(args)...))>>
	{
		return JobResultRef<decltype(src(std::forward<ARGS>(args)...))>(CreateJobWithDependency<B>(std::forward<DEPENDENCY>(dependency), std::forward<FN>(src), std::forward<ARGS>(args)...), 0);
	};

	//----------------------------------------------------------------------------------------
	/// @brief Waits until the referenced job has been executed.
	/// As long as a job hasn't been started it is considered not to have finished yet. Once
	/// it has run this will return immediately.
	///
	/// <B>Wait() might execute other jobs in the current queue until the one you are waiting
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the job to cancel execution. If the job is a part of a group the whole
	...
	//----------------------------------------------------------------------------------------
	/// @brief Asks the job to cancel execution and waits until it has finished. If the job is a part
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues the job the ref points to.
	/// THREADSAFE.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues a lambda or object with operator ().
	/// THREADSAFE.
	/// @param[in] src								Lambda or object with operator ().
	/// @param[in] queue							The queue the object should run on (JOBQUEUE_CURRENT for current queue).
	/// @tparam B											Behaviour for early job cancellation.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues a lambda or object with operator () and n arguments.
	/// THREADSAFE.
	/// @param[in] src								Lambda or object with operator ().
	/// @param[in] queue							The queue the object should run on (JOBQUEUE_CURRENT for current queue).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues a lambda or object with operator () and waits until the job has finished.
	/// THREADSAFE.
	/// @param[in] src								Lambda or object with operator ().
	/// @param[in] queue							The queue the object should run on (JOBQUEUE_CURRENT for current queue).
	/// @tparam B											Behaviour for early job cancellation.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Enqueues a lambda or object with operator () and n arguments and waits until the job has finished.
	/// THREADSAFE.
	/// @param[in] src								Lambda or object with operator ().
	/// @param[in] queue							The queue the object should run on (JOBQUEUE_CURRENT for current queue).
	/// @param[in] args								Argument(s) for the function.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the number of worker threads for the current job context. This might be different
...
public:
	...
	/// @return												Job reference or error.
	//----------------------------------------------------------------------------------------
	template <typename FN> MAXON_WARN_UNUSED auto Then(FN&& src, JobQueueInterface* queue = JOBQUEUE_CURRENT) const -> ResultMemJob<typename GetSignature<decltype(&FN::operator ())>::ReturnType>
	{
		...
		JobResultInterface<RESULTVALUETYPE>* prev = this->GetPointer();
		if (prev == nullptr)
			return FAILED;
...
private:
	...
	{
		using Job = ContinuationJob<RESULTVALUETYPE, FN, RET, PACK>;
...
public:
	...
	template <typename FN> MAXON_WARN_UNUSED auto Then(FN&& src, JobQueueInterface* queue = JOBQUEUE_CURRENT) const -> ResultMemJob<typename GetSignature<decltype(&FN::operator ())>::ReturnType>
	{
		...
		// Return if the allocation of the job failed.
		JobResultInterface<Result<RESULTVALUETYPE>>* prev = this->GetPointer();
		if (prev == nullptr)
			return FAILED;
