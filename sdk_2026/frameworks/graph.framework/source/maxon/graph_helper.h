#ifndef GRAPH_HELPER_H__
#define GRAPH_HELPER_H__
...
{
	MAXON_INTERFACE_NONVIRTUAL(GraphModelHelper, MAXON_REFERENCE_STATIC, "net.maxon.graph.interface.graphmodelhelper", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns all nodes in a graph which match the given collection of attribute values.
	/// @details Only nodes that match all attribute values in #matchingData will be yielded. The mask must contain at least one attribute value to match, otherwise no nodes are yielded.
	/// @code
