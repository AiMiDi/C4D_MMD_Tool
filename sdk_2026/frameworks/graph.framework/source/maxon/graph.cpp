...
{
	return GetPath().GetNodeDepth() > GetGraph().GetViewRootPath().GetCount();
}

Bool GraphNode::IsRoot() const
{
	// False by default if graph is nullptr.
	return GetGraph().IsRoot(*this);
}
