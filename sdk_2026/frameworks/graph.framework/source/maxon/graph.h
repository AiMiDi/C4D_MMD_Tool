...
#if defined(MAXON_TARGET_CPU_X86) && defined(MAXON_ASSEMBLY_GCC) && !defined(MAXON_TARGET_LINUX)
...
/// 2. The /dependency/ wire is only needed for internal connections within evaluation nodes such as core node wrappers.
...
/// 4. The /hidden/ wire is used mark a connection as hidden. What this means exactly is model-specific, for example
///    NodesGraphModelInterface doesn't show those connections.
/// 5. The /inhibit/ wire is used to temporarily disable the other wires. When this wire is present, the other wires
///    should be considered as non-existing.
///	6. The /slink/ wire is a marker which is used for node-specific purposes, where the source port of the connection
...
///	7. The /tlink/ wire is a marker which is used for node-specific purposes, where the target port of the connection
///	   determines the meaning.
///
/// In most cases you only need a combination of a value and dependency wire.
///
///	The Wires class has the members #value, #dependency, #context, #hidden, #inhibit, #slink, #tlink to store information about these wires for a connection.
...
///
/// The wires of a connection can also be accessed by index, see VALUE, DEPENDENCY, CONTEXT, HIDDEN, INHIBIT, SLINK, TLINK and #operator[]. For a loop over the wires
...
struct Wires
{
	...
	//----------------------------------------------------------------------------------------
	/// Constructs a normal wire for values and dependencies. The #value
	/// and #dependency members will be set to the given #mode, all other members to WIRE_MODE::NONE.
	/// @param[in] mode								The mode to use for the value and dependency wires.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT constexpr Wires(WIRE_MODE mode) : value(mode), dependency(mode) { }
	...
	/// @param[in] index							The index of the wire, one of VALUE, DEPENDENCY, CONTEXT, HIDDEN, INHIBIT, SLINK.
	...
	//----------------------------------------------------------------------------------------
	constexpr Wires(WIRE_MODE val, WIRE_MODE dep) : value(val), dependency(dep) { }
	...
	//----------------------------------------------------------------------------------------
	constexpr Wires(WIRE_MODE val, WIRE_MODE dep, WIRE_MODE ctx, WIRE_MODE hid, WIRE_MODE inh, WIRE_MODE sln, WIRE_MODE tln) : value(val), dependency(dep), context(ctx), hidden(hid), inhibit(inh), slink(sln), tlink(tln) { }

	WIRE_MODE value = WIRE_MODE::NONE;					///< The mode for the value part of the connection.
	WIRE_MODE unused = WIRE_MODE::NONE;					///< Unused.
	...
	static const Int UNUSED = 1;								///< The index to address the unused part, used in #operator[] and Wires(Int,WIRE_MODE).
	...
	/// @param[in] index							The index of the wire, one of VALUE, DEPENDENCY, CONTEXT, HIDDEN, INHIBIT, SLINK, TLINK.
	...
	{
		return {mode, mode, mode, mode, mode, mode, mode};
	}
...
#else
		return { Min(value, other.value), Min(unused, other.unused), Min(dependency, other.dependency), Min(context, other.context), Min(hidden, other.hidden), Min(inhibit, other.inhibit), Min(slink, other.slink), Min(tlink, other.tlink) };
#endif
	}
	...
	Wires Combine(const Wires& other) const
	{
		...
		return Wires{a, false};
#else
		return { Max(value, other.value), Max(unused, other.unused), Max(dependency, other.dependency), Max(context, other.context), Max(hidden, other.hidden), Max(inhibit, other.inhibit), Max(slink, other.slink), Max(tlink, other.tlink) };
#endif
	}
	...
	/// for a wire with the same value in this and other, or WIRE_MODE::NONE if the values differ.
	/// @param[in] other							Another Wires object.
	...
	Wires EqualWires(const Wires& other) const
	{
		...
		static_assert(Wires::COUNT == 8, "When there's padding in Wires we have to mask out the padding from a.");
		return Wires{a, false};
#else
		Wires result;
		for (Int m = 0; m < Wires::COUNT; ++m)
	...
	{
		return FormatString("(@,@,@,@,@,@,@)", value, dependency, context, hidden, inhibit, slink, tlink);
	}
...
private:
	...
	/// Constructs the wires using the given modes.
	/// @param[in] val								The mode for the value wire.
	/// @param[in] un									The mode for the unused wire.
	/// @param[in] dep								The mode for the dependency wire.
	/// @param[in] ctx								The mode for the context wire.
	/// @param[in] hid								The mode for the hidden wire.
	/// @param[in] inh								The mode for the inherit wire.
	...
	/// @param[in] tln								The mode for the tlink wire.
	//----------------------------------------------------------------------------------------
	constexpr Wires(WIRE_MODE val, WIRE_MODE un, WIRE_MODE dep, WIRE_MODE ctx, WIRE_MODE hid, WIRE_MODE inh, WIRE_MODE sln, WIRE_MODE tln) : value(val), unused(un), dependency(dep), context(ctx), hidden(hid), inhibit(inh), slink(sln), tlink(tln) { }
};
...
//----------------------------------------------------------------------------------------
/// NodeConnection represents the start and end points of a connection.
...
/// a single root node (see GetViewRoot()). The root node and any other node may have an arbitrary
...
{
	MAXON_INTERFACE(GraphModelInterface, MAXON_REFERENCE_NORMAL, "net.maxon.graph.interface.graphmodel", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
...
public:
	...
	enum class GET_CONNECTIONS_MODE
	{
		...
		SKIP_REROUTER = 256,				///< Hop over rerouter nodes.
	 	MAXON_DEPRECATED_ENUMVALUE(WITH_IMPLICIT_FLAG, 32, "Deprecated. Use Wires::All(WIRE_MODE::ALL_INCLUDING_IMPLICIT).")
	} MAXON_ENUM_FLAGS_CLASS(GET_CONNECTIONS_MODE);

	MAXON_METHOD Bool IsRoot(const GraphNode& node);

	//----------------------------------------------------------------------------------------
	/// Returns the current view path of the model.
	/// This can be an empty path, meaning the model is currently editing or viewing the root of the graph.
	/// Or it can be a path to a node in the graph which an editor currently uses to show the content of the node at this path.
	/// @return												The view root path of this model.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD const NodePath& GetViewRootPath();

	//----------------------------------------------------------------------------------------
	/// Returns the node at GetViewRootPath.
	/// @see GetViewRootPath.
	/// @markDeprecated Use GetViewRoot() instead.
	/// @return												The view root node of this model.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD GraphNode GetViewRoot();
	...
	//----------------------------------------------------------------------------------------
	/// Returns the number of active transactions. Transactions can be nested,
	/// so the transaction count is the current nesting depth.
	/// @return												Number of active transactions, 0 if there's no active transaction at all.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Duplicates the subgraph given by the selection within this graph. The nodes and ports
	/// of the subgraph are added into this graph using new unique identifiers.
...
protected:
	...
	MAXON_METHOD Result<Bool> GetConnectionValues(const GraphNode& source, const GraphNode& target, const ValueReceiver<CONNECTION_POSITION, const InternedId&, const ConstDataPtr&>& receiver);
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Finds a child with the given id. This searches all direct children
	/// (whether nodes, port lists or ports) of this node for a child
	...
	//----------------------------------------------------------------------------------------
	/// Finds a child port with the given id. This searches all direct children of this node for a child
	/// which is a port and has the identifier #name. The first found child is returned.
	/// @param[in] name								The identifier to look for.
	/// @return												The first direct child port whose id is #name, or a null node.
	//----------------------------------------------------------------------------------------
	Result<typename SFINAEHelper<GraphNode, BASE>::type> FindPort(const InternedId& name) const
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		return this->PrivateGetGraph()->FindChild(this->GetGraphNode(), name, NODE_KIND::PORT_MASK);
	}
	...
	/// port to the #target port. But you can also add dependency wires by using an
	...
	/// nodes and call #valueReceiver each time the attribute is found on a base.
	/// @param[in] attr								The attribute whose value shall be obtained.
	/// @param[in] expectedType				The expected type of the attribute value, may be nullptr if no check shall be done.
	...
	/// @code
	/// String name = port.GetValue<decltype(Name)>().GetOrDefault() iferr_return;
	/// @endcode
	...
	/// for the attribute of the correct type, an empty Opt is returned.
	/// @code
	/// String name = port.GetValue(Name).GetOrDefault() iferr_return;
	/// @endcode
	/// @param[in] attr								The attribute whose value shall be obtained.
	/// @return												The attribute value of this node, may be an empty Opt.
	//----------------------------------------------------------------------------------------
	...
	/// for the attribute of the correct type, an empty Opt is returned.
	/// @code
	/// String name = port.GetStoredValue<decltype(Name)>().GetOrDefault() iferr_return;
	/// @endcode
	/// @tparam ATTR									The attribute whose value shall be obtained.
	/// @return												The attribute value of this node, may be an empty Opt.
	//----------------------------------------------------------------------------------------
	...
	/// for the attribute of the correct type, an empty Opt is returned.
	/// @code
	/// String name = port.GetStoredValue(Name).GetOrDefault() iferr_return;
	/// @endcode
	/// @param[in] attr								The attribute whose value shall be obtained.
	/// @return												The attribute value of this node, may be an empty Opt.
	//----------------------------------------------------------------------------------------
	...
	{
		PRIVATE_MAXON_GRAPHFN_PROLOGUE;
		static_assert(!ATTR::READONLY, "Can't set value of read-only attribute.");
	...
	{
		iferr_scope;
		const ConstDataPtr ptr = GetValue(PortValue, GetDataType<T>()) iferr_return;
		return Opt<const T&>(reinterpret_cast<const T*>(ptr.PrivateGetPtr()));
	}
	...
	{
		iferr_scope;
		const ConstDataPtr ptr = GetValue(EffectivePortValue, GetDataType<T>()) iferr_return;
		return Opt<const T&>(reinterpret_cast<const T*>(ptr.PrivateGetPtr()));
	}
	...
	{
		return SetPortValue<T>(std::forward<T>(value));
	}
	...
	/// the attribute value has been set last for this node, or a default value if it hasn't been set
	/// at all.
...
/// 1. It may be a default value, then it doesn't represent any node. IsNull() returns true in this state.
...
public:
	...
	{
		return _graph ? _graph.GetPointer()->GetPath(*this) : NodePath::DefaultValue();
	}
	...
	UniqueHash GetUniqueHashCode() const
	{
		return GetPath().GetUniqueHashCode();
	}
	...
	//----------------------------------------------------------------------------------------
	/// Checks if this GraphNode is the root node of the graph.
	/// GraphNode{} is not considered a root node, because it doesn't belong to any graph.
	/// @return												True if this GraphNode is a valid root node.
	//----------------------------------------------------------------------------------------
	Bool IsRoot() const;
...
/// GraphModelInterface::GetViewRootPath(), this is not necessarily the absolute top level because
