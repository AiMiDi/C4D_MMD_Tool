...
MAXON_ATTRIBUTE(DataDictionary, GraphNodeSelectionOld, "net.maxon.mvp.graphnodeselection");
MAXON_ATTRIBUTE(DataDictionary, GraphWireSelectionOld, "net.maxon.mvp.graphwireselection");
MAXON_ATTRIBUTE(DataDictionary, GraphPortSelectionOld, "net.maxon.mvp.graphportselection");

MAXON_ATTRIBUTE(DataDictionary, GraphNodeSelection, "net.maxon.mvp.graphnodeselection2");
MAXON_ATTRIBUTE(DataDictionary, GraphWireSelection, "net.maxon.mvp.graphwireselection2");
MAXON_ATTRIBUTE(DataDictionary, GraphPortSelection, "net.maxon.mvp.graphportselection2");


namespace details
{

template <typename> inline Opt<NodePath> GetPath(const NodePath& p)
{
	return p;
}
...
template <typename ATTR, typename PRES, typename SEL> static Result<void> SetGraphSelectionImpl(const GraphNode& root, const SEL& selection, SELECTION_MODE mode)
{
	...
	{
		dict = root.GetValue<ATTR>().GetOrDefault() iferr_return;
	}

	for (const auto& s : selection)
	{
		Opt<NodePath> key = maxon::details::GetPath<PRES>(s);
		if (key)
		{
			*key = key->GetPart(rootPathLen, StringEnd()) iferr_return;
