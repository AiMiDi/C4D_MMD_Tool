...
{
	static const PIXELDEPTH DEFAULTALPHA_VALUE = static_cast<PIXELDEPTH>(255); ///< returns the default alpha value for 100% transparency.
	...
	static const PIXELDEPTH PIXELTOLERANCE_ONE = static_cast<PIXELDEPTH>(1);		///< returns the value that represents a 8 bit digit tolerance of this pixel type.
...
/// to signed
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix8s& dst, const Pix8u src)   { dst = Pix8s(src + LIMIT<Pix8s>::MIN); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix8s& dst, const Pix8s src)   { dst = src; }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix8s& dst, const Pix16u src)  { dst = Pix8s((src >> 8) + LIMIT<Pix8s>::MIN); }
...
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix8s& dst, const Pix32f src)  { dst = Pix8s(Pix8u(ClampValue(Pix32f(src * 255.0f + 0.5f), Pix32f(0.0f), Pix32f(255.0f))) + LIMIT<Pix8s>::MIN); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix8s& dst, const Pix64f src)  { dst = Pix8s(Pix8u(ClampValue(Pix64f(src * 255.0 + 0.5), Pix64f(0.0), Pix64f(255.0))) + LIMIT<Pix8s>::MIN); }

MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16s& dst, const Pix8u src)  { dst = Pix16s((Pix16u(src << 8) + src) + LIMIT<Pix16s>::MIN); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16s& dst, const Pix8s src)  { dst = Pix16s(Pix16u(src << 8) + src); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16s& dst, const Pix16u src) { dst = Pix16s(src + LIMIT<Pix16s>::MIN); }
...
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16s& dst, const Pix32f src) { dst = Pix16s(Pix16u(ClampValue(Pix32f(src * 65535.0f + 0.5f), Pix32f(0.0f), Pix32f(65535.0f))) + LIMIT<Pix16s>::MIN); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16s& dst, const Pix64f src) { dst = Pix16s(Pix16u(ClampValue(Pix64f(src * 65535.0 + 0.5), Pix64f(0.0), Pix64f(65535.0))) + LIMIT<Pix16s>::MIN); }
...
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16f& dst, const Pix32u src)  { Pix32f d; ConvertPixel(d, src); dst = ClampValue(d, LIMIT<Pix16f>::MIN, LIMIT<Pix16f>::MAX); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16f& dst, const Pix32s src)  { Pix32f d; ConvertPixel(d, src); dst = ClampValue(d, LIMIT<Pix16f>::MIN, LIMIT<Pix16f>::MAX); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16f& dst, const Pix16f src)  { dst = src; }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16f& dst, const Pix32f src)  { Pix32f d; ConvertPixel(d, src); dst = ClampValue(d, LIMIT<Pix16f>::MIN, LIMIT<Pix16f>::MAX); }
MAXON_ATTRIBUTE_FORCE_INLINE void ConvertPixel(Pix16f& dst, const Pix64f src)  { Pix32f d; ConvertPixel(d, src); dst = ClampValue(d, LIMIT<Pix16f>::MIN, LIMIT<Pix16f>::MAX); }
...
//----------------------------------------------------------------------------------------
template <typename PIXELDEPTH> MAXON_ATTRIBUTE_FORCE_INLINE Bool IsPixelIdentical(PIXELDEPTH a, PIXELDEPTH b, PIXELDEPTH oneTolerance)
{
	PIXELDEPTH tolerance{0};
	
	if (oneTolerance > PIXELDEPTH(0))
		tolerance = oneTolerance * PixelDepthInfo<PIXELDEPTH>::PIXELTOLERANCE_ONE;
	else
		tolerance = PixelDepthInfo<PIXELDEPTH>::PIXELTOLERANCE_ZERO;
...
public:
	...
	MAXON_METHOD Bool IsIdenticalPixel(const ImageConstBuffer& a, const ImageConstBuffer& b, Int oneTolerance) const
	{
		...
		for (Int c = 0; c < _channelCount; c++)
		{
			...
			if (!IsPixelIdentical(*(PixelDepth*)pixA, *(PixelDepth*)pixB, PixelDepth(oneTolerance)))
				return false;
		}
		return true;
	}
