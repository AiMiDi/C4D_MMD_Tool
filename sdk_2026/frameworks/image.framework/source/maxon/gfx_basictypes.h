...
public:
	...
	Vec2<T, 1> GetBottomRightPoint() const
	{
	...
	DrawRect GetIntersectionWith(const DrawRect& rect) const
	{
		const T interLeft   = Max(GetLeftEdge(), rect.GetLeftEdge());
		const T interRight  = Min(GetRightEdge(), rect.GetRightEdge());
		const T interTop    = Max(GetTopEdge(), rect.GetTopEdge());
		const T interBottom = Min(GetBottomEdge(), rect.GetBottomEdge());
		return DrawRect(interLeft, interTop, Max(T(0), interRight - interLeft), Max(T(0), interBottom - interTop));
	}
