...
// forward declaration of IsZeroInitialized to get register.cpp to compile.
class PixelFormatGroup;
template <> struct IsZeroInitialized<PixelFormatGroup> : public std::true_type { };
...
inline Result<CONVERTPIXELFUNC> LambdaWrapper(CONVERTPIXELFUNCSTATIC func, const ChannelOffsets& dstChannelOffsets, const ChannelOffsets& srcChannelOffsets)
{
	...
	f.CopyFrom(
		...
		{
			// it's important to move the variable into the lambda to avoid crashing.
			UseVariable(helper);
...
{
	MAXON_INTERFACE(PixelFormatInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.image.interface.pixelformat", MAXON_IMPLEMENTATION_MODULE("net.maxon.image"));
...
{
	ImageChannel channelBlock[] = { CHANNELS::Get().template GetChannel<PIXELTYPE>()... };
