...
Result<MediaSessionRef> MediaSessionImport(const Url& fileName, const TimeValue& targetTime, MEDIASESSIONFLAGS flags,
	...
	const ValueReceiver<const FileFormatHandler&>& fileFormatsRes)
{
	iferr_scope;
	...
	checkHandler.CopyFrom([&session, &selectionHandler, &outputConverter, &targetTime, flags, &secondRun, &progressCallback, preferredHandler]
		(const Url& url, const FileFormatHandler& handler) -> Result<Bool>
		{
			...
			if (progressCallback)
			{
				ProgressRef progress = session.GetProgress();
				progress.InitProgress(Milliseconds(250.0), Milliseconds(100.0)) iferr_return;
				progress.ObservableProgressNotification(true).AddObserver(progressCallback, GetZeroRef<JobQueueRef>()) iferr_return;
			}
	...
	if (!res)
	{
		fileFormats = FileFormatDetectionInterface::DetectAll<MediaInputRef>(fileName, FILEFORMATDETECTIONFLAGS::NONE, checkHandler) iferr_return;
	}
