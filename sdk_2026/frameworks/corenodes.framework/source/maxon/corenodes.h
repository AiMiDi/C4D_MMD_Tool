...
/// of all input ports as arguments, where an empty TrivialDataPtr indicates an unknown
...
/// |-------------------------------------------------------|    |------------------------------|  |  |-----------------------------------------|
/// |-------------------------------------------------------|    |------------------------------|  |  |-----------------------------------------|
/// |+AddChild(id:Id, node:CoreNode): CoreNodeInstancePtr   |    |                              |  |  |+AddChild<MN>(id:Id): MicroNodePtr<MN>   |
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Sets the identifier of this core node. The identifier is used to derive identifiers of
	/// instances of this node when they have no explicit identifier (such as "mynode.1" if the
	/// node has identifier "mynode").
	/// @param[in] identifier					The identifier of the core node.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Finds the child of this group with the given identifier.
	/// @param[in] identifier					A identifier.
	/// @return												Child node instance having the given identifier, or a nullptr if no such child exists.
	/// @MAXON_ANNOTATION{interfaceParameter=Interface}
	//----------------------------------------------------------------------------------------
	MAXON_METHOD const typename Interface::Instance* FindChild(const Id& identifier) const;
	...
	/// @param[in] port								Identifier of the port. The port has to exist.
	/// @return												PortInfo of the identified port.
	//----------------------------------------------------------------------------------------
	...
	/// @code
	/// Bool pure = node.GetValue(corenodes::CoreNodeInterface::Pure).GetOrDefault() iferr_return;
	/// @endcode
	/// @param[in] attr								The attribute whose value shall be obtained.
	/// @return												The attribute value of this node, may be an empty Opt.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> MAXON_FUNCTION Result<Opt<typename ATTR::ValueType>> GetValue(const ATTR& attr) const
	{
		iferr_scope;
		Data v = GetValue(attr.Get()) iferr_return;
		typename ATTR::ValueType* ptr = v.GetPtr<typename ATTR::ValueType>();
		if (!ptr)
			return {};
		return Opt<typename ATTR::ValueType>(std::move(*ptr));
	}
	...
	/// @param[in] value							The value for the attribute.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> MAXON_FUNCTION Result<void> SetValue(const ATTR& attr, const typename ATTR::ValueType& value)
	{
		return SetValue(attr.Get(), ConstDataPtr(value));
	}
	...
	/// @param[in] value							The value for the attribute.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> MAXON_FUNCTION Result<void> SetValue(const ATTR& attr, typename ATTR::ValueType&& value)
	{
		return SetValue(attr.Get(), MoveDataPtr(std::move(value)));
	}
...
public:
	...
	/// @return												True if Invoke() is supported, false otherwise.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Bool IsInvocable() const;

	//----------------------------------------------------------------------------------------
	/// Invokes the core node directly (as opposed to the invocation from within a compiled
	/// core node graph). This is only supported for a limited set of core nodes, see IsInvocable().
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> Invoke(const Context& ctx, const Block<const Tuple<TrivialDataPtr, CORENODE_PORT_FLAGS>>& args) const;

	//----------------------------------------------------------------------------------------
	/// Invokes the core node directly (see Invoke). Prior to invoke all #args elements
	/// which to not exactly match the expected DataType will be converted. Use Invoke to use the given #args as is.
	/// This is only supported for a limited set of core nodes, see IsInvocable().
	/// The main purpose is to assist the nodes compiler in constant propagation at compile-time.
	/// @param[in] ctx								The execution context. Will contain messages if invoked node has warnings to report.
	/// @param[in,out] args						The arguments: At first the values for input ports,
	/// 															afterwards storage locations where output ports write their results.
	...
	//----------------------------------------------------------------------------------------
	/// Writes this core node to a GML file (graph modeling language). This can be used to
...
//! 		MAXON_ATTRIBUTE_FORCE_RELEASE_INLINE Result<void> Process(const Ports<in1, in2, out>& ports) const
//! 		{
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Adds a micro node of class MN to this group using the given identifier. The created micro node
	/// is owned by this group.
	/// @tparam MN										The micro node class to instantiate.
	/// @param[in] identifier					The identifier for the child. Identifiers have to be unique within a group.
	/// 															If an empty identifier is passed a unique identifier will be chosen.
	...
	/// 															by this group.
	//----------------------------------------------------------------------------------------
	template <typename MN, typename... ARGS> MAXON_FUNCTION Result<MicroNodePtr<MN>> AddChild(const Id& identifier, ARGS&&... args)
	{
		iferr_scope;
		MN* microNode = NewObj(MN, std::forward<ARGS>(args)...) iferr_return;
		microNode->template PrivateInit<MN>();
		return GenericReinterpretCast<Result<MicroNodePtr<MN>>>(AddMicroNode(identifier, microNode));
	}

	//----------------------------------------------------------------------------------------
	/// Adds a micro node of class MN to this group. The created micro node
	/// is owned by this group. A unique identifier is chosen automatically.
	/// @tparam MN										The micro node class to instantiate.
	/// @return												A pointer to the micro node instance. The instance is owned
	/// 															by this group.
	//----------------------------------------------------------------------------------------
	template <typename MN> MAXON_FUNCTION Result<MicroNodePtr<MN>> AddChild()
	{
		...
		microNode->template PrivateInit<MN>();
		return GenericReinterpretCast<Result<MicroNodePtr<MN>>>(AddMicroNode(Id::DefaultValue(), microNode));
	}
	...
	//! };
	/// @endcode
	/// @param[in] port								One of the ports declared by a MAXON_PORT macro (an empty parentheses pair is needed after the identifier, see example).
	...
	{
		return PrivateGetPort<typename PORT::PortIdType>(PORT::GetId(), GetDataType<typename PORT::PortDeclarationType>(), WIRETYPE::NONE);
	}
	...
	/// 															A parentheses pair, possibly including indices, is needed after the identifier, see GetPort().
	...
	{
		return GenericReinterpretCast<Result<MicroNodePtr<typename OUTPORT::PortClass::MicroNodeClass>>>(PrivateGetMicroNode(OUTPORT::GetId(), GetDataType<Generic>(), OUTPORT::MicroNodeClass::MICRONODE_TYPE, OUTPORT::MICRONODE_FLAGS));
	}
	...
	{
		return PrivateGetPort<typename PORT::PortIdType>(PORT::GetId(), type, WIRETYPE::NONE);
	}

	template <typename PORT> MAXON_FUNCTION Result<void> AddWires(const PORT& port, WIRETYPE wires)
	{
		return PrivateGetPort<typename PORT::PortIdType>(PORT::GetId(), GetDataType<Generic>(), wires);
	}
	...
	template <typename PORTID> Result<PORTID> PrivateGetPort(const Id& identifier, const DataType& vtype, WIRETYPE wtypes)
	{
		iferr_scope;
		Int i = PrivateGetPort(PORTID(identifier), vtype, wtypes) iferr_return;
		return PORTID(i);
	}
	...
	struct PrivateAutoCreatePortInfo
	{
		...
		const Generic* arg;
		Result<DataType> (*getPortType) (const Id&, const Generic*);
		Result<void> result;

		void CreatePort(const Id& identifier, MicroNode::TYPE type, MicroNode::FLAGS flags, const DataType& vtype);
	};

	template <typename NODECLASS> Result<void> PrivateAutoCreatePorts(const Generic* arg, Result<DataType> (*getPortType)(const Id&, const Generic*))
	{
		const Bool EMPTY = STD_IS_REPLACEMENT(empty, NODECLASS);
		if (EMPTY)
		{
			return OK;
		}
		...
		{
			typename std::aligned_union<0, NODECLASS>::type mem;
			...
			result.group = this;
			result.arg = arg;
			result.getPortType = getPortType;
	...
	MAXON_FUNCTION Result<MicroNodePtr<>> CreatePort(const Id& identifier, MicroNode::TYPE type, MicroNode::FLAGS flags, const DataType& vtype)
	{
		return PrivateCreatePort(identifier, type, flags, vtype, false);
	}

	MAXON_METHOD Result<MicroNodePtr<>> PrivateCreatePort(const Id& id, MicroNode::TYPE type, MicroNode::FLAGS flags, const DataType& vtype, Bool allowExisting);

	MAXON_METHOD Result<Int> PrivateGetPort(const PortId& pid, const DataType& vtype, WIRETYPE wtypes);

	MAXON_METHOD Result<OutPortId> PrivateAddParameter(MicroNodeInterface* paramRoot, const DataType& type, const Id& identifier, MicroNode::TYPE portType, MicroNode::FLAGS flags, const ConstDataPtr& constant, MicroNodeInterface*& portOut);

	static MAXON_METHOD void PrivateDescribePortAccess(PortAccessDescription* desc, const Id& identifier, ACCESS_INDEX index, PORT_MODE mode);

	static MAXON_METHOD void PrivateBeginLazy(PortAccessDescription* desc);

	static MAXON_METHOD void PrivateEndLazy(PortAccessDescription* desc);

private:
	MAXON_METHOD Result<MicroNodePtr<>> AddMicroNode(const Id& identifier, MicroNode* child);

	MAXON_METHOD Result<MicroNodePtr<>> PrivateGetMicroNode(const Id& identifier, const DataType& vtype, MicroNode::TYPE ptype, MicroNode::FLAGS flags);
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Adds a core node as an instance to this group using the given identifier.
	///
	...
	/// 															by this group.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<CoreNodeInstancePtr> AddChild(const Id& identifier, const CoreNode& node);

	/// @copydoc AddChild
	MAXON_FUNCTION Result<CoreNodeInstancePtr> AddChild(const Id& identifier, Result<CoreNode>&& node);

	//----------------------------------------------------------------------------------------
	/// Adds a core node as an instance to this group using the given identifier. nodeId specifies
	...
	/// 															by this group.
	//----------------------------------------------------------------------------------------
	MAXON_FUNCTION Result<CoreNodeInstancePtr> AddChild(const Id& identifier, IdOrBuilder&& nodeId, const DataDictionary& args = GetZeroRef<DataDictionary>());

	/// @copydoc AddChild(Id, IdOrBuilder, DataDictionary)
	MAXON_FUNCTION Result<CoreNodeInstancePtr> AddChild(const Id& identifier, IdOrBuilder&& nodeId, Result<DataDictionary>&& args);
	...
	/// 															by this group.
	//----------------------------------------------------------------------------------------
	template <typename TYPE> MAXON_FUNCTION Result<CoreNodeInstancePtr> AddChild(const Id& identifier, IdOrBuilder&& nodeId, DataDictionary args = DataDictionary())
	{
		return AddChild(identifier, std::move(nodeId), args.Set(CoreNodeBaseInterface::Type, GetDataType<TYPE>()));
	}
...
template <typename N, typename... ARGS> inline Result<typename std::decay<typename GetSignature<decltype(&N::Init)>::Parameters::template At<0>::type>::type> CreateNode(OverloadRank0, const Id& nodeId, ARGS&&... args)
{
	...
	if (nodeId.IsPopulated())
		n.SetId(nodeId);
	n.GetPointer()->template PrivateAutoCreatePorts<N>(nullptr, nullptr) iferr_return;
...
{
	iferr_scope;
	decltype(auto) p = N::ProcessArgs(std::forward<ARGS>(args)...) iferr_return;
	using ArgType = typename std::decay<decltype(p)>::type;
	using Ref = typename std::decay<typename GetSignature<decltype(&N::Init)>::Parameters::template At<0>::type>::type;
	Ref n = NewObj(typename Ref::ReferencedType) iferr_return;
	if (nodeId.IsPopulated())
		n.SetId(nodeId);
	n.GetPointer()->template PrivateAutoCreatePorts<N>(reinterpret_cast<const Generic*>(&p), [] (const Id& port, const Generic* arg) -> Result<DataType> { return N::GetPortType(port, *reinterpret_cast<const ArgType*>(arg)); }) iferr_return;
	N::Init(n, std::move(p)) iferr_return;
	return n;
}
...
{
	iferr_scope;
	auto n = maxon::corenodes::details::CreateNode<N>(OVERLOAD_MAX_RANK, nodeId, std::forward<ARGS>(args)...) iferr_return;
	return n.Finalize();
}
...
{
	return CreateNode<N>(Id::DefaultValue());
}
...
template <Bool SINGLE_PARAM> struct CoreNodeFactoryFunctionHelper
{
	...
	{
		iferr_scope;
		Id identifier = factory.Get(CoreNodeInterface::NodeId, Id());
		auto n = maxon::corenodes::details::CreateNode<N>(OVERLOAD_MAX_RANK, identifier, args) iferr_return;
...
template <> struct CoreNodeFactoryFunctionHelper<true>
{
	template <typename N> static Result<CoreNode> Create(FactoryInterface::ConstPtr factory, const DataDictionary&)
	{
		iferr_scope;
		Id identifier = factory.Get(CoreNodeInterface::NodeId, Id());
		auto n = maxon::corenodes::details::CreateNode<N>(OVERLOAD_MAX_RANK, identifier) iferr_return;
...
template <typename CORENODE> struct SetParameters<CORENODE, typename SFINAEHelper<void, typename CORENODE::Parameters>::type>
{
	...
	template <typename... P> static Result<void> SetImpl(DataDictionary& dict, ParameterPack<P...>*)
	{
		...
		ForEach{(MAXON_LIKELY(err.GetReferencedError() == nullptr) && (params.Set(P::Get(), GetDataType<typename P::ValueType>()) % err, false))...};
...
template <typename CORENODE> inline Result<CoreNodeFactory> CreateCoreNodeFactory(DataDictionary&& meta = DataDictionary())
{
	iferr_scope;
	maxon::corenodes::details::SetParameters<CORENODE>::Set(meta) iferr_return;
	using ValueType = typename GetValueType<CORENODE>::type;
	if (!STD_IS_REPLACEMENT(same, ValueType, void))
	{
		meta.Set(CoreNodeInterface::Type, GetDataType<ValueType>()) iferr_return;
	}
	maxon::corenodes::details::AddOptimizer<CORENODE>(meta) iferr_return;
	return FactoryFactory::CreateCached<CoreNode>(&maxon::corenodes::details::CoreNodeFactoryFunctionHelper<GetSignature<decltype(&CORENODE::Init)>::Parameters::COUNT == 1>::template Create<CORENODE>, 100, meta);
}
...
#define PRIVATE_MAXON_REGISTER_CORE_NODE_A(name, cls, initMetadata, line, ...) \
	...
	{ \
		...
		initMetadata \
		return maxon::corenodes::CreateCoreNodeFactory<NodeCls>(std::move(meta)); \
	}
