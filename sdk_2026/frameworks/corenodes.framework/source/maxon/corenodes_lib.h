...
class CoreNodesLib
{
	MAXON_INTERFACE_NONVIRTUAL(CoreNodesLib, MAXON_REFERENCE_STATIC, "net.maxon.corenode.interface.corenodeslib");
public:
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Allows to fetch a zero or non zero constant value for a specific DataType.
	/// For instance "GetConstant(GetDataType<Matrix>(), 0)" will return a Matrix with a null translation,
	/// rotation and scale. "GetConstant(GetDataType<Matrix>(), 1)" would return an identity matrix.
	/// "GetConstant(GetDataType<ColorA>(), 1)" will return a ColorA's default value, hence ColorA{0.0_f, 0.0_f, 0.0_f, 1.0_f}.
	/// Mostly used to node optimizers.
	/// @param[in] type								The requested DataType.
	/// @param[in] value							0 to get a zero initialized version of the data, 1 to get non-zero (default value) for #type.
	/// @return												The zero or non zero default value for this DataType.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD ConstDataPtr GetConstant(const DataType& type, Int value);

	//----------------------------------------------------------------------------------------
	/// Applies the required conversion sequence on the TrivialDataPtr's content to transform it into the
	/// desired DataType. This supports VALUEKIND::FUNCTION value types. The owner of the resulting TrivialDataPtr
	/// is in #resultReferences, i.e. keep it valid as long as #arg can be used.
	/// @private
	/// @param[in,out] arg						A value to be converted using CoreNodesLib::GetConversion (and more for VALUEKIND::FUNCTION).
	/// @param[in] expectedType				The desired type for #arg.
	/// @param[out] resultReferences	A Data array to maintain until the content of #arg is not needed anymore.
	///																#arg becomes invalid when resultReferences is out of scope or reset.
	/// @return												True if the conversion succeeded, then #arg will be updated to the new value.
	//----------------------------------------------------------------------------------------
...
/// Separator character used to delimit the CoreNode instance identifier from the instruction identifier suffix.
/// For instance in an instruction identifier like "cube@RjgMOC8QOpTpIJWyTRxK5m/primitive/generategeometry#!BASIC_1@60",
/// the CoreNode instance identifier is "cube@RjgMOC8QOpTpIJWyTRxK5m/primitive/generategeometry#".
