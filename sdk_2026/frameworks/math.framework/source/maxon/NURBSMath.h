...
public:
	...
	NURBSCurve(const NURBSCurve<D>& other) : NURBSCurveInterface(other._clsMTable), _curveDegree(other._curveDegree), _knotMode(other._knotMode), _memInit(other._memInit)
	{
		_points.CopyFrom(other._points) iferr_ignore("Cannot throw in copy constructor."_s);
		_knots.CopyFrom(other._knots) iferr_ignore("Cannot throw in copy constructor."_s);
	}
	...
	Result<void> SetKnots(Block<const Float>&& knots)
	{
		...
		_knots.Flush();
		_knots.AppendAll(std::move(knots)) iferr_return;
...
protected:
	...
	/// This would change the curve shape though and the goal of this function is to add additional points into both the knot vector and the
	...
	Result<Pair<Block<Vector4d>, Int>> InsertKnot(Float u, Int span, Int nTimes)
	{
		...
		if (MAXON_UNLIKELY(nTimes < 1))
			return IllegalArgumentError(MAXON_SOURCE_LOCATION, "Control points must be inserted at least one time."_s);

		KnotInfo info = GetKnotInfo(span) iferr_return;

		if (info.multiplicity + nTimes > _curveDegree && _knots[span] == u)
			return IllegalArgumentError(MAXON_SOURCE_LOCATION, "Multiplicity of final knot higher than degree of curve."_s);

		const Int multiplicity = (_knots[span] == u) ? ((info.multiplicity - 1) + nTimes) : nTimes - 1;
		...
		InitInternal(_points.GetCount() + nTimes, _curveDegree, _knotMode) iferr_return;
		...
		for (i = span - multiplicity; i < oldPoints.GetCount(); ++i)
			_points[i + nTimes] = oldPoints[i];

		// Save the original control points that will be replaced by new ones.
		// We also copy one leading point, that will not be replaced because it is needed to calculate the next one.
		// Since newly inserted points with a multiplicity bigger than 0 will reduce the number of original control points 
		// that affect the span where the points are inserted we reduce the termination point by the multiplicity.
		for (i = 0; i <= _curveDegree - multiplicity; ++i)
		...
		for (j = 1; j <= nTimes; ++j)
		{
			...
			for (i = 0; i <= _curveDegree - j - multiplicity; ++i)
			{
			...
			_points[span + nTimes - j - multiplicity] = detail::HomogeneousToNormal(rtemp[_curveDegree - j - multiplicity]);
		}
		...
		Int firstPoint = span - _curveDegree + 1;
		Block<Vector4d> returnBlock{&_points[firstPoint], _curveDegree - multiplicity};
		return Pair<Block<Vector4d>, Int>{returnBlock, firstPoint};
	}
...
template<Int D> MAXON_IMPLEMENTATION_SIMPLE_REGISTER(NURBSCurve<D>);
