...
{
	_root = nullptr;
	_nodelist.Resize(0) iferr_cannot_fail("why not use reset or flush?");
	_stack.Resize(0) iferr_cannot_fail("why not use reset or flush?");
}
...
Int	KDTree::FindNearest(Int threadIndex, const Vector& point, Float maxRange, BaseArray<KDTreeNearest>& list, Int maxElements, Bool sortResults)
{
	...
	{
		localStack.Append(_root->child[smaller])	iferr_cannot_fail("no check is necessary as capacity is guaranteed");
	}

	// always walk through other side
	if (_root->child[1 - smaller])
	{
		localStack.Append(_root->child[1 - smaller]) iferr_cannot_fail("no check is necessary as capacity is guaranteed");
	}
...
Int	KDTree::FindRange(Int threadIndex, const Vector& point, Float maxRange, BaseArray<KDTreeNearest>& list, Bool sortResults)
{
	...
	list.Resize(0) iferr_ignore("why not use reset or flush??");
