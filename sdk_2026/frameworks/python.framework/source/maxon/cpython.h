...
#if !defined(PRIVATE_MODULE_net_maxon_pythonvm) && !defined(PRIVATE_MODULE_net_maxon_c4d_python)
...
#define PY_VERSION_MINOR 11
#define PY_VERSION_MICRO 3
...
enum class PyStringEncodings
{
  ...
  Utf32,
  PyDecode
} MAXON_ENUM_LIST(PyStringEncodings);
...
class NativePyFrameObject;
class NativePyCodeObject;
...
enum class CPyMemAllocatorDomain
{
	...
	CPYMEM_DOMAIN_MEM,
	CPYMEM_DOMAIN_OBJ
} MAXON_ENUM_LIST(CPyMemAllocatorDomain);
...
public:
	...
	MAXON_METHOD NativePyFrameObject* CPyFrame_GetBack(NativePyFrameObject* frame) const;
	MAXON_METHOD NativePyCodeObject*  CPyFrame_GetCode(NativePyFrameObject* frame) const;
	MAXON_METHOD Int32							  CPyFrame_GetLasti(NativePyFrameObject* frame) const;
  ...
  MAXON_METHOD Result<void> CPy_SetPath(const BaseArray<Url>& paths);
	...
	//----------------------------------------------------------------------------------------
	/// Converts an number object to a python int value. GIL must be held.
	///
	/// @param[in] o									Int, float or complex value.
	/// @return												Python object representing the number as an int.
	//----------------------------------------------------------------------------------------
	...
	///
	/// @param[in] o									Int, float or complex value.
	/// @return												Python object representing the number as a float.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD CPyRef CPyNumber_Float(const CPyRef& o);
	...
	MAXON_FUNCTION MPyHashMapRef MPyHashMap_New(HashMap<KEY, VALUE>& map)
	{
		iferr_scope_handler{
				DebugStop();
				return MPyHashMapRef::DefaultValue();
		};

		HashMap<Data, Data> dst;
		for (const auto& src_entry : map)
		{
			Data k, v;
			k.Set(src_entry.GetKey()) iferr_return;
			v.Set(src_entry.GetValue())  iferr_return;

			maxon::HashMap<maxon::Id, maxon::Data>::Entry& entry = map.InsertMultiEntry(std::move(k)) iferr_return;
			entry.SetValue(std::move(v));
		}

		return MPyHashMap_New(std::move(dst));
	}
	...
	MAXON_FUNCTION Result<void> RegisterBuiltinConverter(const DelegatePyObject_AsData& f1, const DelegatePyObject_FromData<T>& f2)
	{
		...
		static_assert(!STD_IS_REPLACEMENT(same, T, Data), "Data must not be registered");
	...
	MAXON_METHOD MPyDataRef MPyData_New(const DataType& dt, Generic* data, Bool owner, const DataType& mapDt = DataType::DefaultValue());
	...
	template <typename T>
	MAXON_FUNCTION typename std::enable_if<STD_IS_REPLACEMENT(same, T, DataType), ResultPtr<DataType>>::type ParseArgument(const CPyRef& args, Int32 argIndex, Bool isOptional = false);

	template <typename T>
	MAXON_FUNCTION Result<T> ParseRef(const CPyRef& args, Int32 argIndex, Bool isOptional = false);

	template <typename T>
	MAXON_FUNCTION typename std::enable_if<STD_IS_REPLACEMENT(reference, T) || STD_IS_REPLACEMENT(pointer, T), Result<T>>::type ParseArgument(const CPyRef& args, BlockArray<Data>& tempStack, Int32 argIndex, Bool isOptional = false);
...
MAXON_DATATYPE(Array, "net.maxon.python.datatype.array");
...
MAXON_FUNCTION CPyRef CPythonLibraryInterface::Enum_ToMapperInstance(T enumm)
{
	...
	auto PyLongOrLong = CPyObject_FromGeneric(dt->GetUnderlyingType()->GetUnderlyingType(), (Generic*)&enumm, OWNERSHIP::NORMAL);

	iferr (GetAssociatedDataType(dt.GetUnqualifiedType()))
	{
		return PyLongOrLong;
	}

	return CPyObject_ToMapperInstance(dt, PyLongOrLong);
}

template <typename T>
MAXON_FUNCTION T CPythonLibraryInterface::CPyFloat_As(const CPyRef& o)
{
	static_assert(STD_IS_REPLACEMENT(floating_point, T), "T must be a floating point type");
...
template <typename T>
MAXON_FUNCTION CPyRef CPythonLibraryInterface::CPyFloat_From(std::enable_if_t<STD_IS_REPLACEMENT(same, T, Float32), Float32> o)
{
	return CPyFloat_FromFloat32(o);
}

template <typename T>
MAXON_FUNCTION CPyRef CPythonLibraryInterface::CPyFloat_From(std::enable_if_t<STD_IS_REPLACEMENT(same, T, Float64), Float64> o)
{
...
{
	static_assert(STD_IS_REPLACEMENT(integral, T), "T must be an integral type");
	auto res = CPyLong_AsInt64(o);
	if (res > LIMIT<T>::MAX)
	{
		CPyErr_SetString(CPyExc_OverflowError(), "value is too large");
		return -1;
...
MAXON_FUNCTION Result<T> CPythonLibraryInterface::ParseGeneric(const CPythonLibraryRef& pylib, const CPyRef& args, Int32 argIndex, DataType& dt, Bool unpackData)
{
	...
	{
		ptr = (T*)reinterpret_cast<ObjectRef*>(ptr)->GetPointer();
	}
...
template <typename T>
MAXON_FUNCTION typename std::enable_if<!STD_IS_REPLACEMENT(reference, T) && !STD_IS_REPLACEMENT(pointer, T), Result<T>>::type CPythonLibraryInterface::ParseArgument(const CPyRef& args, BlockArray<Data>& tempStack, Int32 argIndex, Bool isOptional)
{
  iferr_scope;
...
template <typename T>
MAXON_FUNCTION typename std::enable_if<STD_IS_REPLACEMENT(same, T, DataType), ResultPtr<DataType>>::type CPythonLibraryInterface::ParseArgument(const CPyRef& args, Int32 argIndex, Bool isOptional)
{
  iferr_scope;
...
template <typename T>
MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<STD_IS_REPLACEMENT(pointer, T), T>::type PrivateGetPtr(Data* data)
{
	return (typename std::remove_pointer<T>::type*)(*(Generic**)data->PrivateGetPtr());
}

template <typename T>
MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<STD_IS_REPLACEMENT(reference, T), T>::type PrivateGetPtr(Data* data)
{
...
MAXON_FUNCTION typename std::enable_if<STD_IS_REPLACEMENT(reference, T) || STD_IS_REPLACEMENT(pointer, T), Result<T>>::type CPythonLibraryInterface::ParseArgument(const CPyRef& args, BlockArray<Data>& tempStack, Int32 argIndex, Bool isOptional)
{
	...
	if (MAXON_LIKELY(item != nullptr))
	{
		const DataType& expected = GetDataType<typename std::decay<T>::type>();
		Data* res = CPyObject_AsData(0, item, expected, tempStack) iferr_return;
		DebugAssert(res);

		// replace with if constexpr in C++17
		// if (STD_IS_REPLACEMENT(same, T, Data&) || STD_IS_REPLACEMENT(same, T, Data*))
	...
	{
		CPyErr_Clear();
		return (T)DefaultValue<const std::remove_reference_t<T>&>();
	}
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::CPyFrameRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::CPyTracebackRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::CPyCodeRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::MPyClassRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::MPyMemberRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::MPyFunctionRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
{
	auto* lib = o.GetLibrary();
	iferr (auto res = lib->GetMPyHashMapRefClass().Create())
		return {};
	res.Init(lib, REFTYPE::BORROWED, o.Get<py::NativePyObject>());
	return res;
}
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::MPyDataTypeRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
//----------------------------------------------------------------------------------------
template <typename T>
typename std::enable_if<STD_IS_REPLACEMENT(same, T, py::MPyDataRef), T>::type CPyCast(const CPyRef& o)
{
	auto* lib = o.GetLibrary();
...
// Internal functions for Python3Output
...
/// Python3Output outputs text into the console under the Python 3 category.
...
/// THREADSAFE.
#define Python3Output(formatString, ...) do { FORMATTEMPLATECHECK(formatString, ##__VA_ARGS__); _Python3Output(maxon::TARGETAUDIENCE::ALL, maxon::FormatTemplateX<MAXON_VA_NARGS(__VA_ARGS__)>(formatString, ##__VA_ARGS__), __LINE__, MAXON_FILE); } while(false)
...
#define PRIVATE_MAXON_CPYTHON_FUNCTION_REGISTER(funcname, SUPERTYPE) static maxon::py::NativePyObject* MAXON_CONCAT(CPythonFunction3_, funcname) (maxon::py::NativePyObject* selfLib, maxon::py::NativePyObject* args, maxon::py::NativePyObject*) \
	{ \
		auto lib = PrivateExtractCPythonLibrary3(selfLib); \
		return MAXON_CONCAT(CPythonFunction_, funcname)(lib, maxon::py::CPyInterface::CreateTempRef(lib, args)).template GetAndIncref<>(); \
	} \
	maxon::Error MAXON_CONCAT(CPythonStoreResult3_, funcname) = maxon::PrivateRegisterCPythonFunc(MAXON_STRINGIFY(funcname), MAXON_CONCAT(CPythonFunction3_, funcname), SUPERTYPE _functions3);
...
#define PRIVATE_MAXON_COMPONENT_CPYTHON_MODULE(NO_CONSTANTS) \
	protected: \
		...
		BaseArray<NativePyCFunctionTable> _functions3; \
	public: \
		MAXON_METHOD Result<void> GetFunctions3(BaseArray<NativePyCFunctionTable>& functions)\
		{\
			return functions.CopyFrom(_functions3); \
		} \
		NO_CONSTANTS
