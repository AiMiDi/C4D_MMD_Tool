...
public:
	...
	ValueType type;
	UChar padding[8 - sizeof(ValueType)];
};
...
public:
	using ValueType = TYPE;
	using ValueTypePadded = std::conditional_t<sizeof(TYPE) < 8, PaddedType<TYPE>, TYPE>;

	PrivateVertexWrapper()
	{
		ClearMemType(&_data[0], sizeof(ValueTypePadded), 0);
		_multiple = false;
	}
	...
	Result<void> CopyFrom(const PrivateVertexWrapper<TYPE>& src)
	{
		...
		{
			MemCopyType((ValueTypePadded*)GetDataPointer(), (ValueTypePadded*)src.GetDataPointer(), 1);
			_multiple = false;
		}
	...
	HashInt GetHashCode() const
	{
		...
		if constexpr (IsTriviallyHashable<ValueType>::value)
		{
			if (IsMultipleValue())
			{
				const UInt32 count = *static_cast<UInt32*>(dataPointer);
				return UniqueHash::GetUniqueHashCode(reinterpret_cast<const Byte*>(static_cast<UInt32*>(dataPointer) + 1), count * sizeof(UInt32) + count * sizeof(ValueType)).GetHashCode();
			}
			return UniqueHash::GetUniqueHashCode(reinterpret_cast<const Byte*>(static_cast<ValueType*>(dataPointer)), sizeof(ValueType)).GetHashCode();
		}
		else
		{
			if (IsMultipleValue())
			{
				...
				return polyIndices.GetUniqueHashCode().GetHashCode() + 31 * data.GetHashCode();
			}

			return static_cast<ValueType*>(dataPointer)->GetHashCode();
		}
	}
...
protected:
	...
	void MoveContent(PrivateVertexWrapper<TYPE>&& src)
	{
		...
		MemCopy(&_data[0], &src._data[0], sizeof(ValueTypePadded));
		_multiple = src._multiple;

		ClearMemType(&src._data[0], sizeof(ValueTypePadded), 0);
		src._multiple = false;
	}
...
private:
	...
	UChar _data[sizeof(ValueTypePadded)];
