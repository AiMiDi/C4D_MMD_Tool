...
{
	DISPLAYMODE displaymode = DISPLAYMODE::UNKNOWN;		///< The display mode: @enumerateEnum{DISPLAYMODE}
	Bool backfaceCulling = false;											///< Backface culling.
	Bool editMode = false;														///< Edit mode.
	const maxon::Color32* vertexColor = nullptr;			///< An array of vertex colors for each point.
	Bool dontFreeVertexColor = false;									///< If @formatConstant{false}, the caller frees @ref vertex_color. If @formatConstant{true}, the called function owns the memory, the caller does not free @ref vertex_color.
	Bool vertexColorShading = false;									///< Vertex color shading.
	DISPLAYCONTROL_MODE colorMode = DISPLAYCONTROL_MODE::PER_VERTEX;	///< Color mode.
	VERTEXCOLOR_DISPLAYMODE vertexColorDisplayMode = VERTEXCOLOR_DISPLAYMODE::NONE;	///< Is set to ALPHA only if paint tool is active and mode is vertexColor(Alpha), is set to COLOR if the tool is active and the mode is vertexColor(RGB) or vertexColor(RGB + Alpha), and set to NONE if the tool is inactive.
};
...
#define PLUGINFLAG_TOOL_IS_SELECTION				(1 << 14)									///< The tool is a selection tool, will be used to toggle the tool accordingly. @since R22v
#define PLUGINFLAG_TOOL_NO_TOPOLOGY_EDIT	  (1 << 15)									///< The tool do not affect the polygon object topology (for example is a transform tool), it is used internally to allow advance UV display in the 3d viewport. @since R22v
/// @}
...
/// @param[in] create							If @formatConstant{true}, the container is created if it does not exist.
...
/// @copydoc GetToolData
inline const BaseContainer* GetToolData(const BaseDocument* doc, Int32 pluginid, Bool create = true)
{
	return GetToolData(MAXON_REMOVE_CONST(doc), pluginid, create);
}
