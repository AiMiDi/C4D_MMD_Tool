...
enum
{
	...
	FONT_BIG				= 4,			///< Big font (+3 points).
	FONT_BIG_BOLD		= 5,			///< Big Bold font (+3 points).
	FONT_ITALIC			= 6,			///< Italics font.
	FONT_SMALL_BOLD	= 7,			///< 1 point smaller bold font.
	FONT_BIGGER			= 8,			///< 1 point bigger font.
	FONT_HUGE				= 9,			///< Huge font (+7 points).
	FONT_SMALL			= 10			///< 1 point smaller font.
};
/// @}
...
enum
{
	...
	KEY_F31				= 0xF1BE,
	KEY_F32				= 0xF1BF,
	TAP_TWOFINGER	= 0xF1C0,
	TAP_THREEFINGER	= 0xF1C1,
	TAP_FOURFINGER	= 0xF1C2
};
/// @}
...
enum
{
	...
	BFH_FIXED												= 128,				///< Fixed horizontal size
	BFV_FIXED												= 256,				///< Fixed vertical size
...
enum
{
	...
	MENURESOURCE_SEPARATOR				= 2,			///< @formatConstant{true} if it is a separator.
...
enum
{
	...
	DR_MULTILINE_NO_DARK_BACKGROUND 	= 2048,			///< Uses not dark but normal background color. @since R22
	DR_MULTILINE_NO_BORDER						= 4096,			///< No border. @since R22
	DR_MULTILINE_NO_SCROLLBARS				= 8192,			///< Shows all the text without scrollbars. @since R22
	DR_MULTILINE_BOLD									= 16384,		///< Bold font. @since R22
	DR_MULTILINE_DROP_TEXT_QUOTED			= 32768,		///< Drag & drop BaseList2d name with quotes. @since R25
	DR_MULTILINE_RESERVED_MONOSPACED	= 65536,						///< Internal use. @since R26
	DR_MULTILINE_RESERVED_SYNTAXCOLOR	= 131072,						///< Internal use. @since R26
	DR_MULTILINE_RESERVED_STATUSBAR		= 262144,						///< Internal use. @since R26
	DR_MULTILINE_RESERVED_HIGHLIGHTLINE	= 524288,					///< Internal use. @since R26
	DR_MULTILINE_LINECOUNT							= 1048576,				///< Default multiline line count. @since R26
	DR_MULTILINE_SUBMITONENTER						= 2097152,				///< Submits the text when pressing enter.
	...
	DR_BUTTON_STYLE_NORMAL			= 0,	///< Normal button style.
	DR_BUTTON_STYLE_EMPHASIZED	= 1,	///< Accentuated button style.
	DR_BUTTON_STYLE_DIMMED			= 2,	///< Attenuated button style.

	GADGET_FLAG_HANDLE_SCROLL_GROUP				= 1 << 0,				///< @markprivate CoreUI user area handles scroll group on its own.
...
enum
{
	...
	CMD_VALUE				= 1,							///< Checked.
	CMD_VALUE_MASK	= 0x0FFFFFFF,			///< Valid mask for value.
...
enum
{
	...
	BFM_INPUT_AFTER_SCROLL						= 'bIAS',			///< Input message ID sent after scroll area processed input messages.
	...
	BFM_INPUT_IDLE 				= 0x8000,		///< BUGFIX #20600/9383: idle if no events are pending (otherwise mouse polling would takes 100 % cpu)
	...
	BFM_INPUT_WHEELSCROLL							= 'iwsc',			///< ::Bool True for scroll events on a classic wheel mouse. False for everything else (touchpad, magic mouse).
	...
	BFM_ACTION_KEYUP  = 'keyu',			///< ::Bool Key Up pressed while editfield is active.
	BFM_ACTION_KEYDOWN = 'keyd',		///< ::Bool Key Down pressed while editfield is active.
	...
	BFM_SETVIEWPORTORIGIN							= 'cORG',			///< @markInternal Set the scroll area origin.
	BFM_SCROLLGROUP_OFFSETCHANGED			= 'cOFF',			///< @markInternal internal message to notify inner elements about scroll change
