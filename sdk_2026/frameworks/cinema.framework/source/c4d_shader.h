...
//----------------------------------------------------------------------------------------
/// @brief Transforms a color along one of the basic transformation paths independent of the OCIO settings of a document.
/// @details Supported are `LINEAR_TO_VIEW`, `SRGB_TO_VIEW`, `LINEAR_TO_SRGB`, and `SRGB_TO_LINEAR` of `COLORSPACETRANSFORMATION`.
...
//----------------------------------------------------------------------------------------
/// @brief Transforms an array of colors along one of the basic transformation paths independent of the OCIO settings of a document.
/// @details Supported are `LINEAR_TO_VIEW`, `SRGB_TO_VIEW`, `LINEAR_TO_SRGB`, and `SRGB_TO_LINEAR` of `COLORSPACETRANSFORMATION`.
/// To convert along any other paths, one must retrieve an `OcioConverter` using `BaseDocument::GetBasicColorConverter` or
/// `BaseDocument::GetColorConverterForActiveDocument`.
...
public:
	...
	~OcioConverter()
	{
		C4DOS_Sh->DestructOcioConverter(this);
	}
	...
	{
		return C4DOS_Sh->TransformColorOcio64(this, v, colorSpaceTransformation);
	}
	...
	/// @return												The transformed color.
	//----------------------------------------------------------------------------------------
	maxon::Color64 TransformColor(const maxon::Color64 &v, COLORSPACETRANSFORMATION colorSpaceTransformation) const
	{
		return C4DOS_Sh->TransformColorOcio64(this, v.GetVector(), colorSpaceTransformation).GetColor();
	}
	...
	{
		return C4DOS_Sh->TransformColorOcio32(this, v, colorSpaceTransformation);
	}
	...
	/// @return												The transformed color.
	//----------------------------------------------------------------------------------------
	maxon::Color32 TransformColor(const maxon::Color32&v, COLORSPACETRANSFORMATION colorSpaceTransformation) const
	{
		return C4DOS_Sh->TransformColorOcio32(this, v.GetVector(), colorSpaceTransformation).GetColor();
	}
	...
	{
		return C4DOS_Sh->TransformColorsOcio64(this, v, count, colorSpaceTransformation);
	}
	...
	/// @param[in] colorSpaceTransformation	Specifies source and destination color space.
	//----------------------------------------------------------------------------------------
	void TransformColors(maxon::Color64* v, Int count, COLORSPACETRANSFORMATION colorSpaceTransformation) const
	{
		return C4DOS_Sh->TransformColorsOcio64(this, reinterpret_cast<Vector64*>(v), count, colorSpaceTransformation);
	}
	...
	{
		return C4DOS_Sh->TransformColorsOcio32(this, v, count, colorSpaceTransformation);
	}
	...
	/// @param[in] colorSpaceTransformation	Specifies source and destination color space.
	//----------------------------------------------------------------------------------------
	void TransformColors(maxon::Color32* v, Int count, COLORSPACETRANSFORMATION colorSpaceTransformation) const
	{
		return C4DOS_Sh->TransformColorsOcio32(this, reinterpret_cast<Vector32*>(v), count, colorSpaceTransformation);
	}

	//----------------------------------------------------------------------------------------
	/// Transforms multiple colors. Input and output can be the same to overwrite the input colors.
	/// IF the size of the blocks mismatch, fewer elements will be transformed.
	/// @param[in] colorIn						Array of colors to convert.
	/// @param[in] colorSpaceTransformation	Specifies source and destination color space.
	/// @param[out] colorOut					Array in whcih the converted colors should be written.
	//----------------------------------------------------------------------------------------
	void TransformColors(const maxon::StridedBlock<const maxon::Color64>& colorIn, COLORSPACETRANSFORMATION colorSpaceTransformation, const maxon::StridedBlock<maxon::Color64>& colorOut) const
	{
		return C4DOS_Sh->TransformColorsOcio64B(this, colorIn, colorSpaceTransformation, colorOut);
	}
	...
	/// @param[out] colorOut					Array in whcih the converted colors should be written.
	//----------------------------------------------------------------------------------------
	void TransformColors(const maxon::StridedBlock<const maxon::Color32>& colorIn, COLORSPACETRANSFORMATION colorSpaceTransformation, const maxon::StridedBlock<maxon::Color32>& colorOut) const
	{
		return C4DOS_Sh->TransformColorsOcio32B(this, colorIn, colorSpaceTransformation, colorOut);
	}
	...
	{
		return C4DOS_Sh->GetOcioConverter(this, colorSpaceTransformation, converter);
	}
	...
	{
		return C4DOS_Sh->IsOcioConverter(this);
	}
};
...
public:
	...
	/// @param[in] overrideViewTransform	Index within the viewtransform array to override the view transform. Ignored, if the value is not a valid index.
	//----------------------------------------------------------------------------------------
	explicit InitRenderStruct(const BaseDocument* t_doc, Int32 overrideViewTransform = -1) { Init(t_doc, overrideViewTransform); }
...
private:
	static constexpr UInt32 CURRENT_VERSION = 1u;		///< Current version number of this structure.
	UInt32							version;										///< The version of this structure in the upper 8 bits, version of @C4D in the lower 24 bits.
public:
	...
	const BaseDocument*	doc;												///< The document to render. Can be @formatConstant{nullptr}, always check.
	...
	OcioConverterRef		ocioConverter;							///< OCIO converter. This is always a valid pointer (see BaseDocument::GetColorConverter()).

	//----------------------------------------------------------------------------------------
	/// Gets the current @C4D version.
	/// @return												The C4D version.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Int32 GetC4DVersion() const { return (Int32)(version & 0x00ffffff); }

	//----------------------------------------------------------------------------------------
	/// Gets the version of this structure.
	/// @return												The version of this structure.
	//----------------------------------------------------------------------------------------
	MAXON_ATTRIBUTE_FORCE_INLINE Int32 GetStructureVersion() const { return (Int32)((version >> 24) & 0xff); }
	...
	{
		if (GetStructureVersion() >= 1 && ocioConverter->IsOcioConverter())
		{
			// Input colors are in rendering space when OCIO is active.
			// Then no conversion is necessary and we simply return the input color. There are a few other places
			// that need this treatment, e.g. material thumbnail, Color Chooser, ...
			return input;
		}
		else
		{
			if (document_colorprofile == DOCUMENT_COLORPROFILE_SRGB && linear_workflow)
				return BasicTransformColor(input, COLORSPACETRANSFORMATION::SRGB_TO_LINEAR);
			else if (document_colorprofile == DOCUMENT_COLORPROFILE_LINEAR && !linear_workflow)
				return BasicTransformColor(input, COLORSPACETRANSFORMATION::LINEAR_TO_SRGB);
		}
		return input;
	}
	maxon::Color TransformColor(const maxon::Color& input) const
	{
		...
		{
			if (document_colorprofile == DOCUMENT_COLORPROFILE_SRGB && linear_workflow)
				return maxon::Color(BasicTransformColor(Vector(input), COLORSPACETRANSFORMATION::SRGB_TO_LINEAR));
			else if (document_colorprofile == DOCUMENT_COLORPROFILE_LINEAR && !linear_workflow)
				return maxon::Color(BasicTransformColor(Vector(input), COLORSPACETRANSFORMATION::LINEAR_TO_SRGB));
		}
		return input;
	}
};
...
public:
	...
	/// @copydoc Alloc
	static TexData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
public:
	...
	{
		rayobject = C4DOS_Sh->Obj_to_Num(nullptr, t_rayobject) + 1;
		polygon = t_polygon;
		if (second)
	...
	{
		return C4DOS_Sh->ID_to_Obj(vd, *this);
	}
#endif
...
struct Multipass
{
	...
	Vector*			vp_magicBulletLooks;				///< Channel without Magic Bullet Looks effect. @since R23
...
struct ChannelData
{
	...
	//----------------------------------------------------------------------------------------
	Int32 GetCurrentCPU() { return C4DOS_Sh->GetCurrentCPU(vd); }
};
