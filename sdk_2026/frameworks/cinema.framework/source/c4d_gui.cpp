...
static Int32 CDialogCallBack(CDialog* cd, CUserArea* cu, BaseContainer* msg)
{
	...
	{
		// UserAreaMessage
		GeUserArea* usr = (GeUserArea*)C4DOS_Cu->GetUserData(cu);
		if (!usr)
			return false;
	...
	{
		// DialogMessage
		GeDialog* dlg = (GeDialog*)C4DOS_Cd->GetUserData(cd);
		if (!dlg)
			return false;
	...
	if (result.GetId() != NOTOK)
		C4DOS_Cd->SetMessageResult(cd, &result);

	return res;
}

Bool GeDialog::GetFolding()
{
...
{
	Bool zeroSize = true;
	C4DOS_Cd->AddGadget(Get(), DIALOG_HIDEOREXPAND, C4DGUIWINDOWID, nullptr, (Int32)FOLDMODE::HIDEMYSELF, zeroSize, allowClose, 0, nullptr, nullptr);
}

Int32 GeDialog::IsDockedOrTabbed()
{
	Int32 result = 0;
	C4DOS_Cd->AddGadget(Get(), DIALOG_ISDOCKEDORTABBED, 0, nullptr, 0, 0, 0, 0, nullptr, (void**)&result);
	return result;
}
...
GeDialog::GeDialog()
{
	cd						= C4DOS_Cd->Alloc(CDialogCallBack, this);
	t_lastcoremsg	= 0;
	createlayout	= false;
...
{
	return C4DOS_Cd->CBF_GetWindowHandle(reinterpret_cast<CBaseFrame*>(cd));
}

void* GeDialog::GetWindowInterface()
{
	return C4DOS_Cd->CBF_GetWindowInterface(reinterpret_cast<CBaseFrame*>(cd));
}

void* GeDialog::GetGuiInterface()
{
	return C4DOS_Cd->CBF_GetGuiInterface();
}
...
Bool GeDialog::IsMinimized() const
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ISMINIMIZED, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}

void* GeDialog::FindCustomGui(Int32 id, Int32 pluginid)
{
	return C4DOS_Cd->FindCustomGui(Get(), id);
}
...
void* GeDialog::AddCustomGui(Int32 id, Int32 pluginid, const maxon::String& name, Int32 flags, Int32 minw, Int32 minh, const BaseContainer& t_customdata)
{
	...
	void* r = nullptr;
	C4DOS_Cd->AddGadget(Get(), DIALOG_SDK, id, &name, flags, minw, minh, 0, &customdata, &r);
	return r;
}

Bool GeDialog::ReleaseLink()
{
	return C4DOS_Cd->ReleaseLink(Get());
}
...
Bool GeDialog::Open(DLG_TYPE dlgtype, Int32 pluginid, Int32 xpos, Int32 ypos, Int32 defaultw, Int32 defaulth, Int32 subid, OPENDIALOGFLAGS flags)
{
	if (!cd)
		return false;
	C4DOS_Cd->AddGadget(cd, DIALOG_SETIDS, pluginid, nullptr, subid, 0, 0, 0, nullptr, nullptr);
	return C4DOS_Cd->Open(cd, dlgtype, nullptr, xpos, ypos, defaultw, defaulth, flags);
}
...
GeData GeDialog::SendMessage(const GadgetPtr& id, const BaseContainer& msg)
{
	if (!cd)
		return false;
	return C4DOS_Cd->SendUserAreaMessage(cd, id.Id(), (BaseContainer*)&msg, id.Ptr());
}

void GeDialog::SetTimer(Int32 timer)
{
	C4DOS_Cd->SetTimer(cd, timer);
}
...
void GeDialog::SetTitle(const maxon::String& title)
{
	if (!cd)
		return;
	C4DOS_Cd->AddGadget(cd, DIALOG_SETTITLE, 0, (String*)&title, 0, 0, 0, 0, nullptr, nullptr);
}
...
Bool GeDialog::Enable(const GadgetPtr& id, Bool enabled)
{
	if (!cd)
		return false;
	return C4DOS_Cd->Enable(cd, id.Id(), enabled, id.Ptr());
}
...
Bool GeDialog::IsEnabled(const GadgetPtr& id)
{
	if (!cd)
		return false;
	return C4DOS_Cd->IsEnabled(cd, id.Id(), id.Ptr());
}
...
Bool GeDialog::SetPopup(const GadgetPtr& id, const BaseContainer& bc)
{
	if (!cd)
		return false;
	return C4DOS_Cd->SetPopup(cd, id.Id(), (BaseContainer*)&bc, id.Ptr());
}
...
Bool GeDialog::Local2Global(Int32* x, Int32* y)
{
	if (!cd)
		return false;
	return C4DOS_Cu->Local2Global((CBaseFrame*)cd, x, y);
}
...
Bool GeDialog::Global2Local(Int32* x, Int32* y)
{
	if (!cd)
		return false;
	return C4DOS_Cu->Global2Local((CBaseFrame*)cd, x, y);
}
...
Bool GeDialog::Screen2Local(Int32* x, Int32* y)
{
	if (!cd)
		return false;
	return C4DOS_Cu->Screen2Local((CBaseFrame*)cd, x, y);
}
...
Bool GeDialog::Local2Screen(Int32* x, Int32* y)
{
	if (!cd)
		return false;
	return C4DOS_Cu->Local2Screen((CBaseFrame*)cd, x, y);
}
...
Bool GeDialog::SetColorField(const GadgetPtr& id, const Vector& color, Float brightness, Float maxbrightness, Int32 flags)
{
	if (!cd)
		return false;
	return C4DOS_Cd->SetColorField(cd, id.Id(), color, brightness, maxbrightness, flags, id.Ptr());
}
...
Bool GeDialog::GetBool(const GadgetPtr& id, Bool& value) const
{
	if (!cd)
		return false;
	Int32 val;
	Bool	ok = C4DOS_Cd->GetInt32(cd, id.Id(), val, id.Ptr());
	value = val != 0;
	return ok;
}
...
Bool GeDialog::GetInt32(const GadgetPtr& id, Int32& value) const
{
	if (!cd)
		return false;
	return C4DOS_Cd->GetInt32(cd, id.Id(), value, id.Ptr());
}
...
Bool GeDialog::GetFloat(const GadgetPtr& id, Float& value) const
{
	if (!cd)
		return false;
	return C4DOS_Cd->GetFloat(cd, id.Id(), value, id.Ptr());
}
...
Bool GeDialog::GetString(const GadgetPtr& id, maxon::String& text) const
{
	if (!cd)
		return false;
	Bool		res = C4DOS_Cd->GetString(cd, id.Id(), text, id.Ptr());
	return res;
}
...
Bool GeDialog::GetColorField(const GadgetPtr& id, Vector& color, Float& brightness) const
{
	if (!cd)
		return false;
	return C4DOS_Cd->GetColorField(cd, id.Id(), color, brightness, id.Ptr());
}
...
{
	if (!doc)
		doc = C4DOS_Ge->GetActiveDocument();
	if (!doc)
		return false;
#ifndef __API_INTERN__
...
Bool GeDialog::CheckTristateChange(const GadgetPtr& id)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_GETTRISTATE, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}
...
C4DGadget* GeDialog::AddCheckbox(Int32 id, Int32 flags, Int32 initw, Int32 inith, const maxon::String& name)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_CHECKBOX, id, (String*)&name, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
Bool GeDialog::MenuSetResource(Int32 id)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_MENURESOURCE, id, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}
...
C4DGadget* GeDialog::AddSeparatorH(Int32 initw, Int32 flags)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_SEPARATOR, 0, nullptr, flags, initw, 0, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddSeparatorV(Int32 initw, Int32 flags)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_SEPARATOR, 0, nullptr, flags, 0, initw, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
Bool GeDialog::AddSubDialog(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_SUBDIALOG, id, nullptr, flags, initw, inith, 0, nullptr, nullptr);
}
...
C4DGadget* GeDialog::AddRadioButton(Int32 id, Int32 flags, Int32 initw, Int32 inith, const maxon::String& name)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_RADIOBUTTON, id, (String*)&name, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddRadioText(Int32 id, Int32 flags, Int32 initw, Int32 inith, const maxon::String& name)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_RADIOTEXT, id, (String*)&name, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}

C4DGadget* GeDialog::AddButton(Int32 id, Int32 flags, Int32 initw, Int32 inith, const maxon::String& name, Int32 buttonStyle)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_BUTTON, id, (String*)&name, flags, initw, inith, buttonStyle, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddStaticText(Int32 id, Int32 flags, Int32 initw, Int32 inith, const maxon::String& name, Int32 borderstyle)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_STATICTEXT, id, (String*)&name, flags, initw, inith, borderstyle, nullptr, &r);
	return (C4DGadget*)r;
}

Bool GeDialog::AddListView(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_LISTVIEW, id, nullptr, flags, initw, inith, 0, nullptr, nullptr);
}
...
C4DGadget* GeDialog::AddArrowButton(Int32 id, Int32 flags, Int32 initw, Int32 inith, Int32 arrowtype)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_ARROWBUTTON, id, nullptr, flags, initw, inith, arrowtype, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddEditShortcut(Int32 id, Int32 flags, Int32 initw, Int32 inith, Int32 shortcutflags)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_EDITSHORTCUT, id, nullptr, flags, initw, inith, shortcutflags, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddEditText(Int32 id, Int32 flags, Int32 initw, Int32 inith, Int32 editflags)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_EDITTEXT, id, nullptr, flags, initw, inith, editflags, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddMultiLineEditText(Int32 id, Int32 flags, Int32 initw, Int32 inith, Int32 style)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_MULTILINEEDITTEXT, id, nullptr, flags, initw, inith, style, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddEditNumber(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_EDITNUMBER, id, nullptr, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddEditNumberArrows(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_EDITNUMBERUD, id, nullptr, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddEditSlider(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_EDITSLIDER, id, nullptr, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddSlider(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_SLIDER, id, nullptr, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddColorField(Int32 id, Int32 flags, Int32 initw, Int32 inith, Int32 colorflags)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_COLORFIELD, id, nullptr, flags, initw, inith, colorflags, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddColorChooser(Int32 id, Int32 flags, Int32 initw, Int32 inith, Int32 layoutflags, const BaseContainer& settings)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_COLORCHOOSER, id, nullptr, flags, initw, inith, layoutflags, &settings, &r);
	return (C4DGadget*)r;
}
...
Bool GeDialog::AddRadioGroup(Int32 id, Int32 flags, Int32 columns, Int32 rows)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_RADIOGROUP, id, nullptr, flags, columns, rows, 0, nullptr, nullptr);
}
...
C4DGadget* GeDialog::AddComboBox(Int32 id, Int32 flags, Int32 initw, Int32 inith, Bool specialalign, Bool allowFiltering)
{
	...
	if (allowFiltering)
		xflags |= 2;
	C4DOS_Cd->AddGadget(cd, DIALOG_COMBOBOX, id, nullptr, flags, initw, inith, xflags, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddComboButton(Int32 id, Int32 flags, Int32 initw, Int32 inith, Bool specialalign, Bool allowFiltering)
{
	...
	if (allowFiltering)
		xflags |= 2;
	C4DOS_Cd->AddGadget(cd, DIALOG_COMBOBUTTON, id, nullptr, flags, initw, inith, xflags, nullptr, &r);
	return (C4DGadget*)r;
}
...
C4DGadget* GeDialog::AddPopupButton(Int32 id, Int32 flags, Int32 initw, Int32 inith, Bool allowFiltering)
{
	...
	if (allowFiltering)
		xflags |= 1;
	C4DOS_Cd->AddGadget(cd, DIALOG_POPUPBUTTON, id, nullptr, flags, initw, inith, xflags, nullptr, &r);
	return (C4DGadget*)r;
}
...
Bool GeDialog::AddChild(const GadgetPtr& id, Int32 subid, const maxon::String& child)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_CHILD, id.Id(), (String*)&child, subid, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::AddChildren(const GadgetPtr& id, const BaseContainer& bc)
{
	BrowseConstContainer br(&bc);
	Int32								 sid;
	const GeData*				 dat;
...
Bool GeDialog::FreeChildren(const GadgetPtr& id)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_FREECHILDREN, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}
...
C4DGadget* GeDialog::AddUserArea(Int32 id, Int32 flags, Int32 initw, Int32 inith)
{
	if (!cd)
		return nullptr;
	void* r = nullptr;
	C4DOS_Cd->AddGadget(cd, DIALOG_USERAREA, id, nullptr, flags, initw, inith, 0, nullptr, &r);
	return (C4DGadget*)r;
}
...
Bool GeDialog::TabGroupBegin(Int32 id, Int32 flags, Int32 tabtype)
{
	if (!cd)
		return false;
	return C4DOS_Cd->TabGroupBegin(cd, id, flags, tabtype);
}
...
Bool GeDialog::GroupBegin(Int32 id, Int32 flags, Int32 cols, Int32 rows, const maxon::String& title, Int32 groupflags, Int32 initw, Int32 inith)
{
	if (!cd)
		return false;
	return C4DOS_Cd->GroupBegin(cd, id, flags, cols, rows, (String*)&title, groupflags, initw, inith);
}

Bool GeDialog::GroupEnd()
{
	if (!cd)
		return false;
	return C4DOS_Cd->GroupEnd(cd);
}
...
Bool GeDialog::GroupSpace(Int32 spacex, Int32 spacey)
{
	if (!cd)
		return false;
	return C4DOS_Cd->GroupSpace(cd, spacex, spacey);
}
...
Bool GeDialog::GroupBorder(UInt32 borderstyle)
{
	if (!cd)
		return false;
	return C4DOS_Cd->GroupBorder(cd, borderstyle | BORDER_WITH_TITLE);
}
...
Bool GeDialog::GroupBorderNoTitle(UInt32 borderstyle)
{
	if (!cd)
		return false;
	return C4DOS_Cd->GroupBorder(cd, borderstyle);
}
...
Bool GeDialog::GroupBorderSpace(Int32 left, Int32 top, Int32 right, Int32 bottom)
{
	if (!cd)
		return false;
	return C4DOS_Cd->GroupBorderSize(cd, left, top, right, bottom);
}
...
Bool GeDialog::AttachUserArea(GeUserArea& ua, const GadgetPtr& id, USERAREAFLAGS userareaflags)
{
	...
	ua.cu	 = nullptr;
	ua.dlg = this;
	ua.cu	 = C4DOS_Cd->AttachUserArea(cd, id.Id(), &ua, userareaflags, id.Ptr());
...
Bool GeDialog::AddDlgGroup(Int32 type)
{
	if (!cd)
		return false;
	return C4DOS_Cd->AddGadget(cd, DIALOG_DLGGROUP, 0, nullptr, type, 0, 0, 0, nullptr, nullptr);
}
...
Bool GeDialog::AttachSubDialog(SubDialog* userdlg, Int32 id)
{
	...
	return C4DOS_Cd->AttachSubDialog(Get(), id, userdlg->Get());
}

Bool GeDialog::CheckClose()
{
	return C4DOS_Cd->AddGadget(Get(), DIALOG_CHECKCLOSE, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}
...
Bool GeDialog::LoadDialogResource(Int32 id, GeResource* lr, Int32 flags)
{
	...
	if (!lr)
		lr = &g_resource;
	return C4DOS_Cd->LoadDialogResource(cd, id, lr->Get(), flags);
#else
	return C4DOS_Cd->LoadDialogResource(cd, id, nullptr, flags);
#endif
}

Bool GeDialog::SetVisibleArea(Int32 scrollgroupid, Int32 x1, Int32 y1, Int32 x2, Int32 y2)
{
	return C4DOS_Cd->SetVisibleArea(cd, scrollgroupid, x1, y1, x2, y2);
}

Bool GeDialog::GetVisibleArea(Int32 scrollgroupid, Int32* x1, Int32* y1, Int32* x2, Int32* y2)
{
	return C4DOS_Cd->GetVisibleArea(cd, scrollgroupid, x1, y1, x2, y2);
}

Bool GeDialog::GetItemDim(const GadgetPtr& id, Int32* x, Int32* y, Int32* w, Int32* h) const
{
	return C4DOS_Cd->GetItemDim(cd, id.Id(), x, y, w, h, id.Ptr());
}

Bool GeDialog::LayoutChanged(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_LAYOUTCHANGED, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::LayoutChangedNoRedraw(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_LAYOUTCHANGED, id.Id(), nullptr, true, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::Activate(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ACTIVATE, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::IsActive(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ISACTIVE, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}
...
Bool GeDialog::GroupWeightsSave(const GadgetPtr& id, BaseContainer& weights)
{
	...
	Bool ret = C4DOS_Cd->AddGadget(cd, DIALOG_SAVEWEIGHTS, id.Id(), nullptr, 0, 0, 0, 0, &storehere, (void**)id.Ptr());
	if (ret)
		weights = storehere;
	return ret;
}
...
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_LOADWEIGHTS, id.Id(), nullptr, 0, 0, 0, 0, &weights, (void**)id.Ptr());
}

Bool GeDialog::LayoutFlushGroup(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_FLUSHGROUP, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::LayoutFlushDisableRedraw(const GadgetPtr& id, Bool disable)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_DISABLEREDRAW, id.Id(), nullptr, disable, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::RemoveElement(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_REMOVEGADGET, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::HideElement(const GadgetPtr& id, Bool hide)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_HIDEELEMENT, id.Id(), nullptr, hide, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::IsElementHidden(const GadgetPtr& id)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ISELEMENTHIDDEN, id.Id(), nullptr, 0, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::ScrollGroupBegin(Int32 id, Int32 flags, Int32 scrollflags, Int32 initw, Int32 inith)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_SCROLLGROUP, id, nullptr, flags, initw, inith, scrollflags, nullptr, nullptr);
}

Bool GeDialog::RestoreLayout(Int32 pluginid, Int32 subid, void* secret)
{
	C4DOS_Cd->AddGadget(cd, DIALOG_SETIDS, pluginid, nullptr, subid, 0, 0, 0, nullptr, nullptr);
	return C4DOS_Cd->RestoreLayout(cd, secret);
}

void GeDialog::ForceAllowMouseWheel(const GadgetPtr& id, Bool force)
{
	C4DOS_Cd->AddGadget(cd, DIALOG_FORCEALLOWMOUSEWHEEL, id.Id(), nullptr, force, 0, 0, 0, nullptr, (void**)id.Ptr());
}

Bool GeDialog::MenuSubBegin(const maxon::String& string)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ADDSUBMENU, 0, (String*)&string, 0, 0, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuSubEnd()
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ENDSUBMENU, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuAddCommand(Int32 cmdid)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ADDMENUCMD, cmdid, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuAddSeparator(const maxon::String& title)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ADDMENUSEP, 0, (String*)&title, 0, 0, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuAddString(Int32 id, const maxon::String& string)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_ADDMENUSTR, id, (String*)&string, 0, 0, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuInitString(Int32 id, Bool enabled, Bool checked)
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_INITMENUSTR, id, nullptr, enabled, checked, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuFlushAll()
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_FLUSHMENU, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}

Bool GeDialog::MenuFinished()
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_SETMENU, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}
...
Bool GeDialog::GetDragPosition(const BaseContainer& msg, Int32* x, Int32* y)
{
	...
	return C4DOS_Cd->Screen2Local(cd, x, y);
}

Bool GeDialog::GetDragObject(const BaseContainer& msg, Int32* type, void** object)
{
	return C4DOS_Cd->GetDragObject(cd, (BaseContainer*)&msg, type, object);
}


Bool GeDialog::CheckValueRanges()
{
	return C4DOS_Cd->AddGadget(cd, DIALOG_CHECKNUMBERS, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}


// to override
Bool GeDialog::CreateLayout()
{
	return true;
}

Bool GeDialog::InitValues()
{
	return true;
}

void GeDialog::DestroyWindow()
{
}
...
Bool GeDialog::OpenPopUpMenu(Int32 menuid, Int32 localx, Int32 localy, Int32 watchhotkey)
{
	if (localx != -1 && localy != -1)
		C4DOS_Cu->Local2Screen((CBaseFrame*)cd, &localx, &localy);

	return C4DOS_Cd->OpenPopUpMenu(cd, menuid, localx, localy, watchhotkey);
}
...
Bool GeUserArea::OpenPopUpMenu(Int32 menuid, Int32 localx, Int32 localy, Int32 watchhotkey)
{
	...
	if (localx != -1 && localy != -1)
		C4DOS_Cu->Local2Screen(reinterpret_cast<CBaseFrame*>(cu), &localx, &localy);

	return C4DOS_Cd->OpenPopUpMenu(dlg->Get(), menuid, localx, localy, watchhotkey);
}
...
Bool GeModalDialog::Open(Int32 xpos, Int32 ypos, Int32 defaultw, Int32 defaulth, Bool resizable, OPENDIALOGFLAGS flags)
{
	dlg_result = false;
	return GeDialog::Open(resizable ? DLG_TYPE::MODAL_RESIZEABLE : DLG_TYPE::MODAL, 0, xpos, ypos, defaultw, defaulth, 0, flags) && dlg_result;
}
...
{
	return C4DOS_Cu->SetDragDestination(cu, cursor);
}
...
Bool GeDialog::SetDragDestination(Int32 cursor, Int32 gadgetid)
{
	if (gadgetid)
	{
		CBaseFrame* cbf = C4DOS_Cd->CBF_FindBaseFrame(Get(), gadgetid);
		return C4DOS_Cd->CBF_SetDragDestination(cbf, cursor);
	}
	return C4DOS_Cd->SetDragDestination(cd, cursor);
}
...
{
	return C4DOS_Cu->GetWidth(cu);
}

Int32 GeUserArea::GetHeight() const
{
	return C4DOS_Cu->GetHeight(cu);
}

Int32 GeUserArea::GetId() const
{
	return C4DOS_Cu->GetID(cu);
}

Bool GeUserArea::IsEnabled() const
{
	return C4DOS_Cu->IsEnabled(cu);
}

Bool GeUserArea::HasFocus() const
{
	return C4DOS_Cu->HasFocus(cu);
}
...
{
	C4DOS_Cu->DrawBezierLine(cu, startPoint, bezierPoints, closed, lineWidth, lineStyle);
}
...
{
	C4DOS_Cu->DrawSetPenV(cu, color);
}

void GeUserArea::DrawSetPen(Int32 id)
{
	C4DOS_Cu->DrawSetPenI(cu, id);
}

Vector GeUserArea::DrawGetPenColor(Int32 id) const
{
	return C4DOS_Cu->DrawGetPenColor(reinterpret_cast<CBaseFrame*>(cu), id);
}
...
{
	C4DOS_Cu->DrawSetOpacity(cu, opacity);
}
...
{
	C4DOS_Cd->CBF_ActivateFading(reinterpret_cast<CBaseFrame*>(cu), milliseconds);
}

void GeUserArea::AdjustColor(Int32 colorid, Int32 highlightid, Float percent)
{
	C4DOS_Cd->CBF_AdjustColor(reinterpret_cast<CBaseFrame*>(cu), colorid, highlightid, percent);
}

HOTKEYFLAGS GeUserArea::IsHotkeyDown(Int32 id) const
{
	return C4DOS_Cu->IsHotkeyDown(cu, id);
}

void GeUserArea::GetBorderSize(Int32 type, Int32* l, Int32* t, Int32* r, Int32* b) const
{
	C4DOS_Cu->GetBorderSize(cu, type, l, t, r, b);
}

void GeUserArea::DrawBorder(Int32 type, Int32 x1, Int32 y1, Int32 x2, Int32 y2)
{
	C4DOS_Cu->DrawBorder(cu, type, x1, y1, x2, y2);
}

void GeUserArea::SetTimer(Int32 timer)
{
	C4DOS_Cu->SetTimer(cu, timer);
}

Bool GeUserArea::GetInputState(Int32 askdevice, Int32 askchannel, BaseContainer& res) const
{
	return C4DOS_Cu->GetInputState(reinterpret_cast<CBaseFrame const*>(cu), askdevice, askchannel, &res);
}

Bool GeUserArea::GetInputEvent(Int32 askdevice, BaseContainer& res) const
{
	return C4DOS_Cu->GetInputEvent(reinterpret_cast<CBaseFrame const*>(cu), askdevice, &res);
}

void GeUserArea::KillEvents()
{
	C4DOS_Cu->KillEvents(reinterpret_cast<CBaseFrame*>(cu));
}
...
{
	C4DOS_Cu->DrawSetFont(cu, fontid);
}

Int32 GeUserArea::DrawGetTextWidth(const maxon::String& text) const
{
	return C4DOS_Cu->DrawGetTextWidth(cu, (String*)&text);
}

Int32 GeUserArea::DrawGetTextWidth_ListNodeName(BaseList2D* node, Int32 fontid) const
{
	return C4DOS_Cu->DrawGetTextWidth_ListNodeName(cu, node, fontid);
}

Int32 GeUserArea::DrawGetTextIndexFromPixel(const maxon::String& text, Float pixelPosition) const
{
	return C4DOS_Cu->DrawGetTextIndexFromPixel(cu, text, pixelPosition);
}

void GeUserArea::DrawSetTextRotation(Float textrotation)
{
	C4DOS_Cu->DrawSetTextRotation(cu, textrotation);
}

Int32 GeUserArea::DrawGetFontHeight() const
{
	return C4DOS_Cu->DrawGetFontHeight(cu);
}

Int32 GeUserArea::DrawGetFontBaseLine() const
{
	return C4DOS_Cu->DrawGetFontBaseLine(cu);
}

const maxon::OSFontObjectRef& GeUserArea::DrawGetFontObject() const
{
	return C4DOS_Cu->DrawGetFontObject(cu);
}

void GeUserArea::DrawSetTextCol(Int32 fg, Int32 bg)
{
	C4DOS_Cu->DrawSetTextColII(cu, fg, bg);
}

void GeUserArea::DrawSetTextCol(const Vector& fg, Int32 bg)
{
	C4DOS_Cu->DrawSetTextColVI(cu, fg, bg);
}

void GeUserArea::DrawSetTextCol(Int32 fg, const Vector& bg)
{
	C4DOS_Cu->DrawSetTextColIV(cu, fg, bg);
}

void GeUserArea::DrawSetTextCol(const Vector& fg, const Vector& bg)
{
	C4DOS_Cu->DrawSetTextColVV(cu, fg, bg);
}
...
void GeUserArea::DrawSetTextCol(const GeData& fg, const GeData& bg)
{
	if (fg.GetType() == DA_VECTOR)
	{
		if (bg.GetType() == DA_VECTOR)
			C4DOS_Cu->DrawSetTextColVV(cu, fg.GetVector(), bg.GetVector());
		else if (bg.GetType() == DA_LONG)
			C4DOS_Cu->DrawSetTextColVI(cu, fg.GetVector(), bg.GetInt32());
		else
			CriticalStop();
	}
	...
	{
		if (bg.GetType() == DA_VECTOR)
			C4DOS_Cu->DrawSetTextColIV(cu, fg.GetInt32(), bg.GetVector());
		else if (bg.GetType() == DA_LONG)
			C4DOS_Cu->DrawSetTextColII(cu, fg.GetInt32(), bg.GetInt32());
		else
			CriticalStop();
	}
	else
...
{
	C4DOS_Cu->DrawText(cu, txt, x, y, flags);
}
...
{
	C4DOS_Cu->DrawImageRef(cu, imageRef, wx, wy, ww, wh, opacity, mode);
}
...
{
	C4DOS_Cu->DrawCustomButton(cu, x, y, w, h, ids, nofill, focus);
}

void GeUserArea::DrawEditFieldBackground(Int32 x1, Int32 y1, Int32 x2, Int32 y2, Bool noFill, Bool hasFocus)
{
	C4DOS_Cu->DrawEditFieldBackground(cu, x1, y1, x2, y2, noFill, hasFocus);
}

void GeUserArea::DrawComboBox(Int32 x1, Int32 y1, Int32 x2, Int32 y2, IconData* iconData, const String& text, Bool noFill, Bool hasFocus)
{
	C4DOS_Cu->DrawComboBox(cu, x1, y1, x2, y2, iconData, text, noFill, hasFocus);
}

void GeUserArea::SetClippingRegion(Int32 x, Int32 y, Int32 w, Int32 h)
{
	C4DOS_Cu->SetClippingRegion(cu, x, y, w, h);
}

void GeUserArea::ClearClippingRegion()
{
	C4DOS_Cu->ClearClippingRegion(cu);
}

Bool GeUserArea::OffScreenOn()
{
	return C4DOS_Cu->OffScreenOn(cu);
}

Bool GeUserArea::OffScreenOn(Int32 x, Int32 y, Int32 w, Int32 h)
{
	return C4DOS_Cu->OffScreenOnRect(cu, x, y, w, h);
}

void GeUserArea::ScrollArea(Int32 xdiff, Int32 ydiff, Int32 x, Int32 y, Int32 w, Int32 h)
{
	C4DOS_Cu->ScrollArea(cu, xdiff, ydiff, x, y, w, h);
}
...
Float GeUserArea::GetPixelRatio() const
{
	if (!cu)
		return 1.0;
	return C4DOS_Cu->GetPixelRatio(reinterpret_cast<CBaseFrame const*>(cu));
}

Bool GeUserArea::Global2Local(Int32* x, Int32* y) const
{
	return C4DOS_Cu->Global2Local(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Local2Global(Int32* x, Int32* y) const
{
	return C4DOS_Cu->Local2Global(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Local2Screen(Int32* x, Int32* y) const
{
	return C4DOS_Cu->Local2Screen(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Screen2Local(Int32* x, Int32* y) const
{
	return C4DOS_Cu->Screen2Local(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Global2Local(Float& x, Float& y) const
{
	return C4DOS_Cu->Global2LocalF(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Local2Global(Float& x, Float& y) const
{
	return C4DOS_Cu->Local2GlobalF(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Local2Screen(Float& x, Float& y) const
{
	return C4DOS_Cu->Local2ScreenF(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}

Bool GeUserArea::Screen2Local(Float& x, Float& y) const
{
	return C4DOS_Cu->Screen2LocalF(reinterpret_cast<CBaseFrame const*>(cu), x, y);
}
...
void GeUserArea::Redraw(Bool threaded)
{
	if (dlg)
	{
		...
		if (threaded)
		{
			C4DOS_Cd->SendRedrawThread(dlg->Get(), GetId());
		}
		else
		{
...
{
	C4DOS_Cu->SendParentMessage(cu, &msg);
	return true;
}

GeData GeUserArea::SendParentMessageResult(const BaseContainer& msg)
{
	return C4DOS_Cu->SendParentMessageResult(cu, &msg);
}

void GeUserArea::MouseDragStart(Int32 button, Float mx, Float my, MOUSEDRAGFLAGS flag)
{
	C4DOS_Cu->MouseDragStart(cu, button, mx, my, flag);
}

MOUSEDRAGRESULT GeUserArea::MouseDrag(Float* mx, Float* my, BaseContainer* channels)
{
	return C4DOS_Cu->MouseDrag(cu, mx, my, channels);
}

MOUSEDRAGRESULT GeUserArea::MouseDragEnd()
{
	return C4DOS_Cu->MouseDragEnd(cu);
}

Bool GeUserArea::IsR2L() const
{
	return C4DOS_Cu->IsR2L(cu);
}

Bool GeUserArea::Init()
{
	return true;
}

Bool GeUserArea::InitValues()
{
	return true;
}
...
{
	return C4DOS_Cu->HandleMouseDrag(cu, &msg, type, data, dragflags);
}
...
{
	return C4DOS_Ge->ShowPopupMenu(parent, screenx, screeny, (BaseContainer*)&bc, flags, res_mainid);
}

Bool RemoveLastCursorInfo(LASTCURSORINFOFUNC func)
{
	return C4DOS_Cd->RemoveLastCursorInfo(func);
}

String Shortcut2String(Int32 shortqual, Int32 shortkey)
{
	return C4DOS_Cd->Shortcut2String(shortqual, shortkey);
}
...
{
	return C4DOS_Cd->GetIconCoordInfo(id, ident);
}

Bool GetInterfaceIcon(Int32 type, Int32 id_x, Int32 id_y, Int32 id_w, Int32 id_h, IconData& d)
{
	return C4DOS_Cd->GetInterfaceIcon(type, id_x, id_y, id_w, id_h, d);
}
...
{
	return C4DOS_Ge->EwBfGetInputEvent(nullptr, askdevice, &res);
}

Bool GetInputState(Int32 askdevice, Int32 askchannel, BaseContainer& res)
{
	return C4DOS_Ge->EwBfGetInputState(nullptr, askdevice, askchannel, &res);
}
#endif
...
{
	return C4DOS_Cu->GetInputState((CBaseFrame*)cd, askdevice, askchannel, &res);
}

Bool GeDialog::GetInputEvent(Int32 askdevice, BaseContainer& res)
{
	return C4DOS_Cu->GetInputEvent((CBaseFrame*)cd, askdevice, &res);
}

void GeDialog::KillEvents()
{
	C4DOS_Cu->KillEvents(reinterpret_cast<CBaseFrame*>(cd));
}
...
{
	C4DOS_Cd->AddGadget(cd, DIALOG_SETDEFAULTCOLOR, id.Id(), nullptr, colorid, mapid, 0, 0, nullptr, (void**)id.Ptr());
}

void GeDialog::SetDefaultColor(const GadgetPtr& id, Int32 colorid, const Vector& color)
{
	C4DOS_Cd->AddGadget(cd, DIALOG_SETDEFAULTCOLOR, id.Id(), nullptr, colorid, NOTOK, 0, 0, (BaseContainer*)&color, (void**)id.Ptr());
}


Bool GeDialog::GroupBeginInMenuLine()
{
	return C4DOS_Cd->AddGadget(Get(), DIALOG_MENUGROUPBEGIN, 0, nullptr, 0, 0, 0, 0, nullptr, nullptr);
}
...
Bool iCustomGui::SupportLayoutSwitch()
{
	if (!plugin)
		return false;
	return (plugin->infoBits & CUSTOMGUI_SUPPORT_LAYOUTSWITCH) != 0;
}
...
const Char* ProgressThread::GetThreadName() { return "Progress Thread"; }
...
void ProgressThread::MarkAsInClose(ProgressDialog* pdlg)
{
	...
	dlg.StoreRelease(reinterpret_cast<ProgressDialog*>(value));
}
...
void ProgressThread::Main()
{
	iferr_scope_handler
	{
		DiagnosticOutput("In this worst case the user has to close the progress dialog via the closer");
		DiagnosticVarOutput(err);
	};
...
Bool ProgressDialog::CreateLayout()
{
	SetTitle(GetTitle());
...
Bool ProgressDialog::InitValues()
{
	thread.End();
...
C4DThread* ProgressDialog::GetThread()
{
	return &thread;
}
...
UpdateDialogHelper UpdateDialogHelper::BeginLayoutChange(GeDialog* dialog, const GadgetPtr& gadget, Bool disableRedraws, Bool doNotFlush)
{
	...
	if (C4DOS_Cd->AddGadget(dialog->Get(), DIALOG_FLUSHGROUP, obj._gadget.Id(), nullptr, doNotFlush, 0, 0, 0, nullptr, (void**)obj._gadget.Ptr()))
	{
		if (disableRedraws)
		{
...
{
	return C4DOS_Cd->GetDnDFilename(msg, dragType, dragObj, texturesOnly, updateUsage, loadUrl);
}
