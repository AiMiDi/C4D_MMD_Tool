...
template <> struct ExtractData<CustomDataType>
{
	template <typename RES> static MAXON_ATTRIBUTE_FORCE_INLINE void Get(const GeData* data, RES& res)
	{
		if (auto customDataType = data->GetCustomDataType<typename std::remove_const<typename std::remove_pointer<RES>::type>::type>())
		{
			res = customDataType;
		}
	}
...
#ifdef __API_INTERN__
		if (auto link = data->GetAliasLink())
#else
		if (auto link = data->GetBaseLink())
#endif
		{
			res = link;
		}
