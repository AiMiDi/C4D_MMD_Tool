...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE CKey : public GeListNode
{
private:
	CKey();
	~CKey();
...
public:
	...
	/// @copydoc Alloc
	static CKey* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	/// @return												The time.
	//----------------------------------------------------------------------------------------
	const BaseTime& GetTime() const { return CaCall(GetTimeRef) (); }
	...
	//----------------------------------------------------------------------------------------
	const BaseTime& GetTimeLeft() const { return CaCall(GetTimeLeftRef) (); }
	...
	//----------------------------------------------------------------------------------------
	const BaseTime& GetTimeRight() const { return CaCall(GetTimeRightRef) (); }
...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE CCurve : public BaseList2D
{
private:
	CCurve();
	~CCurve();
...
/// @addAllocFreeAutoAllocNote
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE CTrack : public BaseList2D
{
private:
	CTrack();
	~CTrack();
...
public:
	...
	/// @copydoc Alloc
	static CTrack* Alloc(const maxon::SourceLocation&, BaseList2D* bl, const DescID& id) { return Alloc(bl, id); }
	...
	//----------------------------------------------------------------------------------------
	Bool FillKey(const BaseDocument* doc, const BaseList2D* bl, CKey* key) const { return CaCall(FillKey) (doc, bl, key); }
	...
	//----------------------------------------------------------------------------------------
	const BaseList2D* GetObject() const { return static_cast<const BaseList2D*>(AtCall(GetMain) ()); }

	//----------------------------------------------------------------------------------------
	/// Gets the host object of the track.
	/// @return												The host object, or @formatConstant{nullptr}.
	//----------------------------------------------------------------------------------------
	BaseList2D* GetObject() { return MAXON_NONCONST_COUNTERPART(GetObject()); }
	...
	/// @markInternal
	//----------------------------------------------------------------------------------------
	void SetLambda(maxon::Delegate<Float(const BaseDocument* doc, BaseList2D* op, const BaseTime& tt, ANIMATEFLAGS flags, Bool* chg, void* data)> lambdaFn) { return CaCall(SetLambda) (std::move(lambdaFn)); }

	//----------------------------------------------------------------------------------------
	/// @markInternal
	//----------------------------------------------------------------------------------------
	Bool HasLambda() const { return CaCall(HasLambda) (); }

	//----------------------------------------------------------------------------------------
	/// @markInternal
	//----------------------------------------------------------------------------------------
	void ClearLambda() { return CaCall(ClearLambda) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the data of the track at @formatParam{time}.
	/// @param[in] doc								The document for the track.
	/// @param[in] op									The op yielding the track.
	/// @param[in] time								The time.
	/// @return												The track data.
	//----------------------------------------------------------------------------------------
	GeData GetAnimatedGeData(const BaseDocument* doc, BaseList2D* op, const BaseTime& time) const { return CaCall(GetAnimatedGeData)(doc, op, time); }
