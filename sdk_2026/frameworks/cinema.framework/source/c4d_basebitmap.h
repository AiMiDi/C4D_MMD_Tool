...
#define PIX_UCHAR(p) ((cinema::PIX_C*)(p))

/// Number of color info bit for UWORD depth mode
#define PIX_UWORD(p) ((cinema::PIX_W*)(p))

/// Number of color info bit for FLOAT depth mode
#define PIX_FLOAT(p) ((cinema::PIX_F*)(p))
/// @}
...
#define B3D_BITDEPTH(mode) (((cinema::Int32)mode >> BITDEPTH_SHIFT) & (BITDEPTH_UCHAR | BITDEPTH_UWORD | BITDEPTH_FLOAT))

/// Returns the COLORMODE for the @formatParam{mode}.
#define B3D_COLOR_MODE(mode) (cinema::COLORMODE(cinema::Int32(mode) & ~((BITDEPTH_UCHAR | BITDEPTH_UWORD | BITDEPTH_FLOAT) << BITDEPTH_SHIFT)))

/// Sets the depth color mode.
#define B3D_SETDEPTH(depth) (cinema::COLORMODE(depth << BITDEPTH_SHIFT))
...
#define COLORBYTES_GRAYw		(COLORBYTES_GRAY * sizeof(cinema::PIX_W))				///< 16-bit grayscale.
#define COLORBYTES_AGRAYw		(COLORBYTES_AGRAY * sizeof(cinema::PIX_W))			///< 16-bit grayscale with 16-bit alpha.
#define COLORBYTES_RGBw			(COLORBYTES_RGB * sizeof(cinema::PIX_W))				///< 16-bit RGB.
#define COLORBYTES_ARGBw		(COLORBYTES_ARGB * sizeof(cinema::PIX_W))				///< 16-bit RGB with 16-bit alpha.
#define COLORBYTES_GRAYf		(COLORBYTES_GRAY * sizeof(cinema::PIX_F))				///< Floating point grayscale.
#define COLORBYTES_AGRAYf		(COLORBYTES_AGRAY * sizeof(cinema::PIX_F))			///< Floating point grayscale with floating point alpha.
#define COLORBYTES_RGBf			(COLORBYTES_RGB * sizeof(cinema::PIX_F))				///< Floating point RGB.
#define COLORBYTES_ARGBf		(COLORBYTES_ARGB * sizeof(cinema::PIX_F))				///< Floating point RGB with floating point alpha.
...
#define BASEBITMAP_DATA_IMAGEREFWRAPPER		1020			///< @markPrivate maxon::ImageRef
#define BASEBITMAP_DATA_PROGRESSHOOK			1021			///< ::void* @markPrivate hack to pass ProgressHook into RenderMaterialPreview()
#define BASEBITMAP_DATA_PROGRESSHOOK_CTX	1022			///< ::void* @markPrivate hack to pass ProgressHook Context into RenderMaterialPreview()
/// @}
...
//------------------------------------------------------------------------------------------------
class ColorProfile : public CustomDataTypeT<CUSTOMDATATYPE_COLORPROFILE>
{
private:
	ColorProfile();
	~ColorProfile();
...
public:
	...
	enum COLORPROFILEINFO
	{
		...
		COLORPROFILEINFO_NAME						= 4				///< Name of the color profile.
	};
	/// @}
...
public:
	...
	/// @copydoc Alloc
	static ColorProfile* Alloc(const maxon::SourceLocation&) { return Alloc(); }

	//----------------------------------------------------------------------------------------
	/// @allocatesA{color profile data}
	/// @param[in] profile						The internal profile used to initialize the new color profile.
	/// @return												@allocReturn{color profile data}
	//----------------------------------------------------------------------------------------
	static ColorProfile* Alloc(const maxon::ColorProfile&  profile);

	/// @copydoc Alloc
	static ColorProfile* Alloc(const maxon::SourceLocation&, const maxon::ColorProfile&  profile) { return Alloc(profile); }
	...
	//----------------------------------------------------------------------------------------
	/// Gets the hash code of the color profile.
	/// @return												The hash code of the color profile.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Sets the internal color profile. @markPrivate
	/// @since R2023
...
public:
	...
	/// @copydoc Alloc
	static ColorProfileConvert* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
public:
	...
	/// @copydoc Alloc
	static BaseBitmap* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetBw() const { return C4DOS_Bm->GetBw(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the height of the bitmap in pixels.
	/// @return												The bitmap height in pixels.
	//----------------------------------------------------------------------------------------
	Int32 GetBh() const { return C4DOS_Bm->GetBh(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the number of bits per pixel.
	/// @return												The number of bits per pixel.
	//----------------------------------------------------------------------------------------
	Int32 GetBt() const { return C4DOS_Bm->GetBt(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the number of bytes per line.
	/// @return												The number of bytes per line.
	//----------------------------------------------------------------------------------------
	Int32 GetBpz() const { return C4DOS_Bm->GetBpz(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetPen(Int32 r, Int32 g, Int32 b) { C4DOS_Bm->SetPen(this, r, g, b); }
	...
	//----------------------------------------------------------------------------------------
	void Line(Int32 x1, Int32 y1, Int32 x2, Int32 y2) { C4DOS_Bm->Line(this, x1, y1, x2, y2); }
	...
	//----------------------------------------------------------------------------------------
	void Arc(Int32 x, Int32 y, Float radius, Float angle_start, Float angle_end, Int32 subdiv = 32) { C4DOS_Bm->Arc(this, x, y, radius, angle_start, angle_end, subdiv); }
	...
	//----------------------------------------------------------------------------------------
	void GetPixel(Int32 x, Int32 y, UInt16* r, UInt16* g, UInt16* b) const { C4DOS_Bm->GetPixel(this, x, y, r, g, b); }

	//----------------------------------------------------------------------------------------
	/// Gets the color at [@formatParam{x},@formatParam{y}].\n
	/// The range of the assigned parameters are @em 0.0 to @em 255.0, regardless of the bit depth of the image, where (@em 255.0,@em 255.0,@em 255.0) is white.
	/// The range returns Float value, so 8, 16 and 32 bit precision is supported.
	/// @param[in] x									The X coordinate.
	/// @param[in] y									The Y coordinate.
	/// @return												Color as form of a 3D Vector.
	//----------------------------------------------------------------------------------------
	Vector32 GetPixelDirect(Int32 x, Int32 y) const { return C4DOS_Bm->GetPixelDirect(this, x, y); }
	...
	//----------------------------------------------------------------------------------------
	Bool SetPixel(Int32 x, Int32 y, Int32 r, Int32 g, Int32 b) { return C4DOS_Bm->SetPixel(this, x, y, r, g, b); }
	...
	//----------------------------------------------------------------------------------------
	void GetAlphaPixel(BaseBitmap* channel, Int32 x, Int32 y, UInt16* val) const { C4DOS_Bm->GetAlphaPixel(this, channel, x, y, val); }
	...
	//----------------------------------------------------------------------------------------
	Bool SetAlphaPixel(BaseBitmap* channel, Int32 x, Int32 y, Int32 val) { return C4DOS_Bm->SetAlphaPixel(this, channel, x, y, val); }
	...
	//----------------------------------------------------------------------------------------
	void GetPixelCnt(Int32 x, Int32 y, Int32 cnt, UChar* buffer, Int32 inc, COLORMODE dstmode, PIXELCNT flags, ColorProfileConvert* conversion = nullptr) const { C4DOS_Bm->GetPixelCnt(this, x, y, cnt, buffer, inc, dstmode, flags, conversion); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool SetPixelCnt(Int32 x, Int32 y, Int32 cnt, UChar* buffer, Int32 inc, COLORMODE srcmode, PIXELCNT flags) { return C4DOS_Bm->SetPixelCnt(this, x, y, cnt, buffer, inc, srcmode, flags); }
	...
	//----------------------------------------------------------------------------------------
	UInt32 GetDirty() const { return C4DOS_Bm->GetDirty(this); }

	//----------------------------------------------------------------------------------------
	/// Makes the bitmap dirty. @markPrivate
	//----------------------------------------------------------------------------------------
	void SetDirty() { C4DOS_Bm->SetDirty(this); }
	...
	//----------------------------------------------------------------------------------------
	const BaseBitmap* GetUpdateRegionBitmap() const { return C4DOS_Bm->GetUpdateRegionBitmap(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the updated region of a bitmap.
	/// @return												The updated region.
	//----------------------------------------------------------------------------------------
	BaseBitmap* GetUpdateRegionBitmap() { return C4DOS_Bm->GetUpdateRegionBitmap(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the update region for the bitmap and resets it if requested.
	/// @param[out] xMin							Left side of the update region.
	/// @param[out] xMax							Right side of the update region.
	/// @param[out] yMin							Top side of the update region.
	/// @param[out] yMax							Bottom side of the update region.
	/// @param[in] reset							True, if the update region should be reset.
	//----------------------------------------------------------------------------------------
	void GetUpdateRegion(Int32& xMin, Int32& xMax, Int32& yMin, Int32& yMax, Bool reset) { C4DOS_Bm->GetUpdateRegion(this, xMin, xMax, yMin, yMax, reset); }

	void UpdateRegionAddPoint(Int32 x, Int32 y, Bool updateImageRefRegions = false) { C4DOS_Bm->UpdateRegionAddPoint(this, x, y, updateImageRefRegions); }

	void UpdateRegionAddLine(Int32 x1, Int32 x2, Int32 y, Bool updateImageRefRegions = false) { C4DOS_Bm->UpdateRegionAddLine(this, x1, x2, y, updateImageRefRegions); }

	void UpdateRegionAddRegion(Int32 x1, Int32 y1, Int32 x2, Int32 y2, Bool updateImageRefRegions = false) { C4DOS_Bm->UpdateRegionAddRegion(this, x1, y1, x2, y2, updateImageRefRegions); }
	...
	//----------------------------------------------------------------------------------------
	Bool SetColorProfile(const ColorProfile* profile, Int32 index = COLORPROFILE_INDEX_IMAGE) { return C4DOS_Bm->SetColorProfile(this, profile, index); }
	...
	//----------------------------------------------------------------------------------------
	const ColorProfile* GetColorProfile(Int32 index = COLORPROFILE_INDEX_IMAGE) const { return C4DOS_Bm->GetColorProfile(this, index); }
	...
	//----------------------------------------------------------------------------------------
	Int GetMemoryInfo() const { return C4DOS_Bm->GetMemoryInfo(this); }
	...
	//----------------------------------------------------------------------------------------
	/// Gets the ImageRef for a BaseBitmap. The reference is stored internally. For the same saveBit / alpha / getFlags tuple you will get the same reference.
	/// @param[in] saveBit						A combination of the following flags: @enumerateEnum{SAVEBIT}
	/// @param[in] isAlphaChannel			True if the BaseBitmap is part a alpha channel within another basebitmap.
	/// @param[in] getFlags						Flags that are passed to GetPixelCnt.
	/// @param[out] imageRef					A reference that gets the imageRef assigned.
	/// @return												@trueIfOtherwiseFalse{the function succeeded}
	//----------------------------------------------------------------------------------------
	Bool GetImageRef(SAVEBIT saveBit, Bool isAlphaChannel, PIXELCNT getFlags, maxon::ImageRef& imageRef) const { return C4DOS_Bm->GetImageRefEx(this, saveBit, isAlphaChannel, getFlags, imageRef); }

	Bool GetImageRef(SAVEBIT saveBit, Bool isAlphaChannel, maxon::ImageRef& imageRef) const { return GetImageRef(saveBit, isAlphaChannel, PIXELCNT::NONE, imageRef); }
...
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE MultipassBitmap : public BaseBitmap
{
private:
	MultipassBitmap();
	~MultipassBitmap();
...
public:
	...
	//----------------------------------------------------------------------------------------
	static MultipassBitmap* Alloc(Int32 bx, Int32 by, COLORMODE mode) { return C4DOS_Bm->MPB_AllocWrapperPB(bx, by, mode); }

	/// @copydoc Alloc
	static MultipassBitmap* Alloc(const maxon::SourceLocation&, Int32 bx, Int32 by, COLORMODE mode) { return Alloc(bx, by, mode); }
	...
	//----------------------------------------------------------------------------------------
	static MultipassBitmap* AllocWrapper(BaseBitmap* bmp) { return C4DOS_Bm->MPB_AllocWrapper(bmp); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetLayerCount() const { return C4DOS_Bm->MPB_GetLayerCount(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the number of alpha layers in the multipass bitmap.
	/// @return												The alpha layer count.
	//----------------------------------------------------------------------------------------
	Int32 GetAlphaLayerCount() const { return C4DOS_Bm->MPB_GetAlphaLayerCount(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the number of hidden layers in the multipass bitmap.
	/// @return												The hidden layer count.
	//----------------------------------------------------------------------------------------
	Int32 GetHiddenLayerCount() const { return C4DOS_Bm->MPB_GetHiddenLayerCount(this); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* GetSelectedLayer() const { return C4DOS_Bm->MPB_GetSelectedLayer(this); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* GetLayerNum(Int32 num) { return C4DOS_Bm->MPB_GetLayerNum(this, num); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* GetAlphaLayerNum(Int32 num) { return C4DOS_Bm->MPB_GetAlphaLayerNum(this, num); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* GetHiddenLayerNum(Int32 num) { return C4DOS_Bm->MPB_GetHiddenLayerNum(this, num); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* AddLayer(MultipassBitmap* insertafter, COLORMODE colormode, Bool hidden = false) { return C4DOS_Bm->MPB_AddLayer(this, insertafter, colormode, hidden); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* AddFolder(MultipassBitmap* insertafter, Bool hidden = false) { return C4DOS_Bm->MPB_AddFolder(this, insertafter, hidden); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* AddAlpha(MultipassBitmap* insertafter, COLORMODE colormode) { return C4DOS_Bm->MPB_AddAlpha(this, insertafter, colormode); }
	...
	//----------------------------------------------------------------------------------------
	Bool DeleteLayer(MultipassBitmap*& layer) { Bool ret = C4DOS_Bm->MPB_DeleteLayer(this, layer); layer = nullptr; return ret; }
	...
	//----------------------------------------------------------------------------------------
	void SetMasterAlpha(BaseBitmap* master) { C4DOS_Bm->MPB_SetMasterAlpha(this, master); }

	//-------------------------------------------------------------------------------------------------
	/// Free the hidden layers.
	//-------------------------------------------------------------------------------------------------
	void FreeHiddenLayers() { C4DOS_Bm->MPB_FreeHiddenLayers(this); }
	...
	//----------------------------------------------------------------------------------------
	GeData GetParameter(MPBTYPE id) const { return C4DOS_Bm->MPB_GetParameter(this, id); }
	...
	//----------------------------------------------------------------------------------------
	Bool SetParameter(MPBTYPE id, const GeData& par) { return C4DOS_Bm->MPB_SetParameter(this, id, par); }
	...
	//-------------------------------------------------------------------------------------------------
	void ClearImageData() { C4DOS_Bm->MPB_ClearImageData(this); }
	...
	//----------------------------------------------------------------------------------------
	MultipassBitmap* FindUserID(Int32 id, Int32 subid = 0) { return C4DOS_Bm->MPB_FindUserID(this, id, subid); }

	//----------------------------------------------------------------------------------------
	/// Gets a @BP3D paint bitmap for the multipass bitmap.
	/// @return												The paint bitmap.
	//----------------------------------------------------------------------------------------
	PaintBitmap* GetPaintBitmap() { return C4DOS_Bm->MPB_GetPaintBitmap(this); }
	const PaintBitmap* GetPaintBitmap() const { return C4DOS_Bm->MPB_GetPaintBitmap(MAXON_REMOVE_CONST(this)); }
...
#define BMP_INSERTLAST ((cinema::MultipassBitmap*)-1)
...
public:
	...
	/// @copydoc Alloc
	static BaseBitmapLink* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
public:
	...
	/// @copydoc Alloc
	static MovieLoader* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
public:
	...
	/// @copydoc Alloc
	static MovieSaver* Alloc(const maxon::SourceLocation&) { return Alloc(); }
