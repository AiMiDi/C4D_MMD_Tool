...
{
	#define BcCall(fnc) (this->*C4DOS_Bc->fnc)
...
public:
	...
	BaseContainer()
	{
		if (MAXON_LIKELY(C4DOS_Bc != nullptr))
	...
	//----------------------------------------------------------------------------------------
	void FlushAll() { return BcCall(FlushAll) (); }
	...
	//----------------------------------------------------------------------------------------
	Int32 FindIndex(Int32 id, const GeData** ppData = nullptr) const
	{
		return BcCall(FindIndex)(id, ppData);
	}

	Int32 FindIndex(Int32 id, GeData** ppData)
	{
		return BcCall(FindIndex)(id, const_cast<const GeData**>(ppData));
	}
	...
	//----------------------------------------------------------------------------------------
	const GeData* GetIndexData(Int32 index) const { return BcCall(GetIndexData) (index); }

	Bool SetIndexData(Int32 index, const GeData& data) { return BcCall(SetIndexData1)(index, data); }
	Bool SetIndexData(Int32 index, GeData&& data) { return BcCall(SetIndexData2)(index, std::move(data)); }
	...
	/// @return												The inserted data. @theOwnsPointed{container,data}
	//----------------------------------------------------------------------------------------
	GeData* InsDataAfter(Int32 id, const GeData& n, const GeData* last) { return BcCall(InsDataAfter) (id, n, last); }
	...
	//----------------------------------------------------------------------------------------
	Bool SetData(Int32 id, const GeData& n) { return BcCall(SetData1) (id, n); }
	Bool SetData(Int32 id, GeData&& n) { return BcCall(SetData2)(id, std::move(n)); }

	//----------------------------------------------------------------------------------------
	/// Sets an arbitrary data at the specified @formatParam{id}. If a value exists under the same ID, its content will be changed.
	/// This function will not set the container dirty and can be used to store data without affecting the object system evaluation.
	/// @param[in] id									The ID of the element to set.
	/// @param[in] n									The data to set.
	//----------------------------------------------------------------------------------------
	Bool SetDataNoDirty(Int32 id, const GeData& n) { return BcCall(SetDataNoDirty) (id, n); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsPopulated() const { return BcCall(GetIndexId) (0) != NOTOK; }

	//----------------------------------------------------------------------------------------
	/// Checks if the container is empty.
	/// @return												@trueIfOtherwiseFalse{the container is empty}
	//----------------------------------------------------------------------------------------
	Bool IsEmpty() const { return BcCall(GetIndexId) (0) == NOTOK; }
	...
	//----------------------------------------------------------------------------------------
	BaseContainer* GetContainerInstanceWritable(Int32 id) { return BcCall(GetContainerInstanceWritable) (id); }
	...
	/// @return												A pointer to the sub-container, or @formatConstant{nullptr} if it does not exist. @theOwnsPointed{container,sub-container}
	//----------------------------------------------------------------------------------------
	const BaseContainer* GetContainerInstance(Int32 id) const { return BcCall(GetContainerInstance) (id); }
	...
	//----------------------------------------------------------------------------------------
	const BaseList2D* GetLink(Int32 id, const BaseDocument* doc, Int32 instanceof = 0) const { return BcCall(GetLink) (id, doc, instanceof); }

	/// @copydoc GetLink
	BaseList2D* GetLink(Int32 id, BaseDocument* doc, Int32 instanceof = 0) const { return const_cast<BaseList2D*>(BcCall(GetLink) (id, doc, instanceof)); }

	BaseList2D* GetLink(Int32 id, std::nullptr_t doc, Int32 instanceof = 0) const { return const_cast<BaseList2D*>(BcCall(GetLink) (id, nullptr, instanceof)); }
	...
	/// @copydoc GetObjectLink
	BaseObject* GetObjectLink(Int32 id, BaseDocument* doc) const { return const_cast<BaseObject*>(GetObjectLink(id, static_cast<const BaseDocument*>(doc))); }

	BaseObject* GetObjectLink(Int32 id, std::nullptr_t doc) const { return const_cast<BaseObject*>(GetObjectLink(id, static_cast<const BaseDocument*>(nullptr))); }
	...
	/// @copydoc GetMaterialLink
	BaseMaterial* GetMaterialLink(Int32 id, BaseDocument* doc) const { return const_cast<BaseMaterial*>(GetMaterialLink(id, static_cast<const BaseDocument*>(doc))); }

	BaseMaterial* GetMaterialLink(Int32 id, std::nullptr_t doc) const { return const_cast<BaseMaterial*>(GetMaterialLink(id, static_cast<const BaseDocument*>(nullptr))); }
	...
	//----------------------------------------------------------------------------------------
	template<typename DATATYPE> const DATATYPE* GetCustomDataType(Int32 id) const
	{
	...
	/// @return												The custom datatype, or @formatConstant{nullptr} if it does not exist or the wrong @formatParam{datatype} was specified. @theOwnsPointed{container,custom datatype}
	//----------------------------------------------------------------------------------------
	const CustomDataType* GetCustomDataTypeI(Int32 id, Int32 datatype) const
	{
		const GeData& dat = GetData(id);
		return dat.GetCustomDataTypeI(datatype);
	}
	...
	//----------------------------------------------------------------------------------------
	void SetLink(Int32 id, const C4DAtomGoal* link) { return BcCall(SetLink) (id, link); }

	void SetMaxonData(Int32 id, const maxon::Data& data) { return BcCall(SetMaxonData)(id, data); }

	void SetMaxonData(Int32 id, maxon::Data&& data) { return BcCall(SetMaxonDataMove)(id, std::move(data)); }

	void SetMaxonData(Int32 id, const maxon::ConstDataPtr& data) { return BcCall(SetMaxonDataPtr)(id, data); }
