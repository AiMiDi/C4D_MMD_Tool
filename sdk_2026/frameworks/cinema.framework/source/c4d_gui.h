...
#define MENU_ALLOWSUBMENUCOMMAND (LIMIT<cinema::Int32>::MAX - 1)
...
#define COREMSG_CINEMA_EXECUTEMANAGER				300001038			///< Optionally pass the object manager dialog pointer for @ref COREMSG_CINEMA_EXECUTEEDITORCOMMAND.
...
struct BezierPoint
{
	...
	BezierPoint(Vector2d&& ic1, Vector2d&& ic2, Vector2d&& ip2)
		: c1(std::move(ic1)), c2(std::move(ic2)), p(std::move(ip2))
	{
	}
...
/// @}
	...
	//----------------------------------------------------------------------------------------
	/// Sets the draw color.
	/// @note To draw the default background color please use @ref GeUserArea::DrawSetPenToParentBackgroundColor() instead of the Cinema API @c DrawSetPen(COLOR_BG).
	...
	//----------------------------------------------------------------------------------------
	Vector DrawGetPenColor(Int32 id) const;
	...
	/// @note For proper rendering in right-to-left (R2L) layouts (e.g., Arabic), you must negate
	///       the x-coordinates of all path points. Additionally, for arcs, the clockwise direction
	///       should be reversed.
	...
	void DrawEditFieldBackground(Int32 x1, Int32 y1, Int32 x2, Int32 y2, Bool noFill, Bool hasFocus);

	void DrawComboBox(Int32 x1, Int32 y1, Int32 x2, Int32 y2, IconData* iconData, const String& text, Bool noFill, Bool hasFocus);
	...
	/// @param[in] pixelPosition			Pixel to examine.
	/// @return												Index of the pixel within the text.
	//----------------------------------------------------------------------------------------
	Int32 DrawGetTextIndexFromPixel(const maxon::String& text, Float pixelPosition) const;
	...
	//----------------------------------------------------------------------------------------
	/// @markPrivate Retrieves the font object of the current font.
	/// @return												The font object of the current font.
	//----------------------------------------------------------------------------------------
...
/// @}
	...
	/// @param[in] buttonStyle				The style of the button: @link DR_BUTTON_STYLE_NORMAL DR_BUTTON_STYLE@endlink
	/// @return												The added gadget, or @formatConstant{nullptr} if an error occurred. @theOwnsPointed{dialog,gadget}
	//----------------------------------------------------------------------------------------
	C4DGadget* AddButton(Int32 id, Int32 flags, Int32 initw, Int32 inith, const maxon::String& name, Int32 buttonStyle = DR_BUTTON_STYLE_NORMAL);
	...
	/// @param[in] id									The control ID.
	/// @return												@trueIfOtherwiseFalse{the element is hidden}
	//----------------------------------------------------------------------------------------
	Bool IsElementHidden(const GadgetPtr& id);

	//----------------------------------------------------------------------------------------
	/// By default elements must be on focus in order to receive mouse wheel events. This is to avoid to interfere with page scrolling.
	/// This function allows to circumvent this constraint in order to receive mouse wheel events always, even if element is not on focus.
	/// @note Use with caution and only when really needed (e.g. document and layout tabs above of Cinema 4D layout).
	/// @param[in] id									The control ID.
	/// @param[in] force							@formatConstant{true} to force receive mouse wheel events always. @formatConstant{false} to use the default behavior, i.e. mouse wheel events are only received when element is focused.
	//----------------------------------------------------------------------------------------
...
public:
	...
	//----------------------------------------------------------------------------------------
	Bool GetResult() { return dlg_result; }
...
//----------------------------------------------------------------------------------------
inline Vector GetGuiWorldColor(Int32 cid) { return C4DOS_Ge->GetGuiWorldColor(cid); }
