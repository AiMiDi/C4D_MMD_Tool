...
/// @endcode
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE GeColliderCache
{
	private:
		GeColliderCache();
		~GeColliderCache();
	...
	/// @copydoc Alloc
	static GeColliderCache* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
/// @endcode
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE GeColliderEngine
{
	private:
		GeColliderEngine();
		~GeColliderEngine();
	...
	/// @copydoc Alloc
	static GeColliderEngine* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
/// @addAllocFreeAutoAllocNote
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE GeRayCollider
{
	private:
		GeRayCollider();
		~GeRayCollider();
	...
	/// @copydoc Alloc
	static GeRayCollider* Alloc(const maxon::SourceLocation&) { return Alloc(); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool Init(const BaseObject* goal, Bool force = false);
...
struct RayColliderLib : public C4DLibrary
{
		...
		Bool						(GeRayCollider::*Init										)(const BaseObject* goal, Bool force);
