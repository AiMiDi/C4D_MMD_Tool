...
{
	return CheckLib<NgonBaseLib>(LIBRARY_NGON, offset, lib_ngonbase);
}

#define NgonBaseLibCall(b) 		cinema::NgonBaseLib *lib = cinema::CheckNgonBaseLib(LIBOFFSET(cinema::NgonBaseLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iNgonBase*>(this)->*lib->b)

#define NgonBaseLibCallC(b) 		cinema::NgonBaseLib *lib = cinema::CheckNgonBaseLib(LIBOFFSET(cinema::NgonBaseLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const cinema::iNgonBase*>(this)->*lib->b)

#define NgonBaseLibCallR(a, b)  cinema::NgonBaseLib *lib = cinema::CheckNgonBaseLib(LIBOFFSET(cinema::NgonBaseLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iNgonBase*>(this)->*lib->b)

#define NgonBaseLibCallRC(a, b)  cinema::NgonBaseLib *lib = cinema::CheckNgonBaseLib(LIBOFFSET(cinema::NgonBaseLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const cinema::iNgonBase*>(this)->*lib->b)
...
{
	NgonBaseLibCallR(false, CopyTo)(reinterpret_cast<iNgonBase*>(dst));
}

Bool NgonBase::Write(HyperFile *hf) const
{
	NgonBaseLibCallRC(false, Write)(hf);
}
...
Bool NgonBase::CheckHash(PolygonObject* op, maxon::HashInt& hash) const
{
	NgonBaseLibCallRC(false, CheckHash)(op, hash);
}

UChar NgonBase::GetEdgeStates(Int32 id) const
{
	NgonBaseLibCallRC(false, GetEdgeStates)(id);
}

Int32 NgonBase::GetCount() const
{
	NgonBaseLibCallRC(0, GetCount)();
}
...
{
	NgonBaseLibCallRC(NOTOK, FindPolygon)(id, l);
}
...
{
	NgonBaseLibCallRC(false, GetFlags)();
}

Int32 NgonBase::GetDirtyCount() const
{
	NgonBaseLibCallRC(false, GetDirtyCount)();
}
...
Bool NgonBase::ToSelect(BaseSelect *pSelect, Int32 mode) const
{
	NgonBaseLibCallRC(false, ToSelect)(pSelect, mode);
}

void NgonBase::GetCenter(Int32 id, const CPolygon *vadr, const Vector *padr, Vector *ip, Vector *in) const
{
	NgonBaseLibCallC(GetCenter)(id, vadr, padr, ip, in);
}
...
NgonBase* NgonBase::Alloc(const maxon::SourceLocation&, const NgonBase& src)
{
	NgonBase* dst = Alloc();
	MAXON_REMOVE_CONST(src).CopyTo(dst); // CopyTo not const ?!?!
	return dst;
}

const Pgon* NgonBase::GetNgonsR() const
{
	NgonBaseLibCallRC(nullptr, GetNgonsR)();
}

Bool NgonBase::IsValidNgon(const Pgon* pgon, const CPolygon* vadr) const
{
	NgonBaseLibCallRC(false, IsValidNgonR)(pgon, vadr);
}

maxon::HashInt NgonBase::GetHashCode() const
{
	NgonBaseLibCallRC(false, GetHashCode)();
}
...
void PgonEdge::GetEdge(const CPolygon *vadr, Int32 &p1, Int32 &p2) const
{
