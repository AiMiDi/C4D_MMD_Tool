...
public:
	...
	/// @copydoc Alloc
	static CustomDataTag* Alloc(const maxon::SourceLocation&, Int32 tagId) { return Alloc(tagId); }
	...
	//----------------------------------------------------------------------------------------
	template <typename TYPE> maxon::PolyData<TYPE> GetPolygonData(Int32 polygonIndex) const
	{
		const maxon::PolyData<const void*> data = GetConstPolygonDataPtr(polygonIndex);
		return maxon::PolyData<TYPE>(*(TYPE*)(data.a), *(TYPE*)(data.b), *(TYPE*)(data.c), *(TYPE*)(data.d));
	}
	...
	{
		ifnoerr (maxon::PolyData<TYPE&> r = GetPolygonDataRef<TYPE>(polygonIndex))
		{
			r = std::move(value);
		}
	}
...
class CINEWARE_SINGLEINHERITANCE iCustomDataTag;
...
struct CustomaDataTagLibrary : public C4DLibrary
{
	...
	maxon::Result<void> 											(iCustomDataTag::*DisplayControl)						(BaseDocument* doc, BaseObject* op, BaseDraw* bd, BaseDrawHelp* bh, ControlDisplayStruct& cds) const;
