...
public:
	...
	/// @copydoc Alloc
	static InstanceObject* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	/// Set matrices for the instance(s). The array size determines the instance count.
	/// This function sets the matrices in local position relative to the Instance objects position.
	/// @param[in] matrices						Instance matrix array.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> SetLocalInstanceMatrices(const maxon::BaseArray<Matrix>& matrices);
	maxon::Result<void> SetLocalInstanceMatrices(maxon::BaseArray<Matrix>&& matrices);
	/// @}
};
...
struct InstanceObjectLibrary : public C4DLibrary
{
	...
	maxon::Result<void> (iInstanceObject::*SetLocalInstanceMatrices)(const maxon::BaseArray<Matrix>& matrices);
	maxon::Result<void> (iInstanceObject::*SetLocalInstanceMatricesMove)(maxon::BaseArray<Matrix>&& matrices);
};
