...
{
	return CheckLib<CloudObjectLib>(LIB_CLOUD, offset, lib_CloudObject);
}

#define CloudCall(b)       cinema::CloudObjectLib *lib = cinema::CheckCloudObjectLib(LIBOFFSET(cinema::CloudObjectLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iCloudObject*>(this)->*lib->b)

#define CloudCallR(a, b)     cinema::CloudObjectLib *lib = cinema::CheckCloudObjectLib(LIBOFFSET(cinema::CloudObjectLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iCloudObject*>(this)->*lib->b)
