...
public:
	...
	/// @copydoc Alloc
	static SelectionChanger* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool InitFromSelection(const BaseSelect *selection, Int32 selection_mode, PolygonObject *op = nullptr);
...
class CINEWARE_SINGLEINHERITANCE iSelectionChanger;

struct SelectionChangerLibrary : public C4DLibrary
{
	Bool							(iSelectionChanger::*InitFromTag			)(BaseTag *targtag, PolygonObject *op);
	Bool							(iSelectionChanger::*InitFromSelection)(const BaseSelect *selection, Int32 selection_mode, PolygonObject *op);
