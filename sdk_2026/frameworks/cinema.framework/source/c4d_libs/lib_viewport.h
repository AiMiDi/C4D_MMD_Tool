...
class CINEWARE_SINGLEINHERITANCE iViewportHUD;
class CINEWARE_SINGLEINHERITANCE iViewportHighlight;
class ViewHUD;
class HUDGroupList;
...
public:
	...
	/// @copydoc Alloc
	static ViewportHUD* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
public:
	...
	/// @copydoc Alloc
	static ViewportHighlight* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	Bool IsHighlightActive(const BaseDocument* doc, const BaseDraw* bd);
};
...
struct ViewportHighlightLib : public C4DLibrary
{
	...
	Bool (iViewportHighlight::*IsHighlightActive)(const BaseDocument* doc, const BaseDraw* bd);
};
