...
maxon::Result<void> SceneColorConverter::Init(const maxon::OcioConfig& config, const maxon::CString& inputColorSpaceLowName, const maxon::CString& inputColorSpaceHighName,
	const maxon::CString& renderColorSpaceName, CONVERSION_FLAGS flags)
{
	SceneColorConverterLibCallR(maxon::IllegalStateError(MAXON_SOURCE_LOCATION), Init3)(config, inputColorSpaceLowName, inputColorSpaceHighName, renderColorSpaceName, flags);
}
...
maxon::Result<Bool> SceneColorConverter::ConvertObjectsGeneric(maxon::Block<BaseList2D*> objects, const maxon::Delegate<void(BaseList2D*)>& addUndo)
{
	SceneColorConverterLibCallR(maxon::IllegalStateError(MAXON_SOURCE_LOCATION), ConvertObjects2)(objects, addUndo);
}
} // namespace cinema
