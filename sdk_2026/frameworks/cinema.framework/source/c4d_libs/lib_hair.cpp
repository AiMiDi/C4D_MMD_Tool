...
{
	return CheckLib<HairObjectLib>(LIBRARY_HAIR, offset, lib_hair);
}

static HairColliderLib* CheckHairColliderLib(Int32 offset)
{
	return CheckLib<HairColliderLib>(LIBRARY_HAIR_COLLIDER, offset, lib_hair_collider);
}

#define HairObjectLibCall(b) cinema::HairObjectLib * lib = cinema::CheckHairObjectLib(LIBOFFSET(cinema::HairObjectLib, b));	\
	if (!lib || !lib->b) return; \
	(lib->b)

#define HairObjectLibCallR(a, b) cinema::HairObjectLib * lib = cinema::CheckHairObjectLib(LIBOFFSET(cinema::HairObjectLib, b));	\
	if (!lib || !lib->b) return a; \
	return (lib->b)

#define HairColliderLibCall(b) cinema::HairColliderLib * lib = cinema::CheckHairColliderLib(LIBOFFSET(cinema::HairColliderLib, b));	\
	if (!lib || !lib->b) return; \
	(reinterpret_cast<cinema::iHairCollider*>(this)->*lib->b)

#define HairColliderLibCallR(a, b) cinema::HairColliderLib * lib = cinema::CheckHairColliderLib(LIBOFFSET(cinema::HairColliderLib, b));	\
	if (!lib || !lib->b) return a; \
	return (reinterpret_cast<cinema::iHairCollider*>(this)->*lib->b)
...
Bool HairLibrary::GetHairGlobal(const BaseDocument* doc) { HairObjectLibCallR(0, hairGetHairGlobal) (doc); }
