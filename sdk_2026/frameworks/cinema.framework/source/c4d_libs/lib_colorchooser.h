...
public:
	...
	/// @copydoc Alloc
	static ColorSwatchGroup* Alloc(const maxon::SourceLocation&, const String& name = String(), Bool selected = false, const ColorAlphaArray& colors = ColorAlphaArray()) { return Alloc(name, selected, colors); }
...
public:
	...
	/// @copydoc Alloc
	static ColorSwatchData* Alloc(const maxon::SourceLocation&, BaseDocument* doc = nullptr, Bool global = false) { return Alloc(doc, global); }
...
class CINEWARE_SINGLEINHERITANCE iColorSwatchGroup;
...
class CINEWARE_SINGLEINHERITANCE iColorSwatchData;
