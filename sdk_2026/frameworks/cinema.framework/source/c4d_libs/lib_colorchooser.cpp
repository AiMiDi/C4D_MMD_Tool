...
{
	return CheckLib<ColorUtilsLibrary>(LIB_COLORCHOOSER, offset, lib_colorchooser);
}

#define ColorUtilsCall(b)         cinema::ColorUtilsLibrary *lib = cinema::CheckColorUtilsLib(LIBOFFSET(cinema::ColorUtilsLibrary, b)); \
																															if (!lib || !lib->b) return; \
																															(lib->b)

#define ColorUtilsCallR(a, b)			cinema::ColorUtilsLibrary *lib = cinema::CheckColorUtilsLib(LIBOFFSET(cinema::ColorUtilsLibrary, b)); \
...
{
	return CheckLib<ColorSwatchDataLibrary>(LIB_COLORSWATCHDATA, offset, lib_colorswatchdata);
}

#define ColorSwatchDataCall(b)					cinema::ColorSwatchDataLibrary *lib = cinema::CheckColorSwatchDataLibrary(LIBOFFSET(cinema::ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return; \
																																					(reinterpret_cast<cinema::iColorSwatchData*>(this)->*lib->b)

#define ColorSwatchDataCallR(a, b)			cinema::ColorSwatchDataLibrary *lib = cinema::CheckColorSwatchDataLibrary(LIBOFFSET(cinema::ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return a; \
																																					return (reinterpret_cast<cinema::iColorSwatchData*>(this)->*lib->b)

#define ColorSwatchDataCallRC(a, b)			cinema::ColorSwatchDataLibrary *lib = cinema::CheckColorSwatchDataLibrary(LIBOFFSET(cinema::ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return a; \
																																					return (reinterpret_cast<const cinema::iColorSwatchData*>(this)->*lib->b)

#define ColorSwatchDataCallS(b)					cinema::ColorSwatchDataLibrary* lib = cinema::CheckColorSwatchDataLibrary(LIBOFFSET(cinema::ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return; \
																																					(lib->b)

#define ColorSwatchDataCallSR(a, b)			cinema::ColorSwatchDataLibrary* lib = cinema::CheckColorSwatchDataLibrary(LIBOFFSET(cinema::ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return a; \
																																					return (lib->b)
...
{
	ColorSwatchDataCallRC(0, GetGroupCount)((Int)category);
}
...
{
	return CheckLib<ColorSwatchGroupLibrary>(LIB_COLORSWATCHGROUP, offset, lib_colorswatchgroup);
}

#define ColorSwatchGroupCall(b)         cinema::ColorSwatchGroupLibrary *lib = cinema::CheckColorSwatchGroupLibrary(LIBOFFSET(cinema::ColorSwatchGroupLibrary, b)); \
																																						if (!lib || !lib->b) return; \
																																						(reinterpret_cast<cinema::iColorSwatchGroup*>(this)->*lib->b)

#define ColorSwatchGroupCallR(a, b)			cinema::ColorSwatchGroupLibrary *lib = cinema::CheckColorSwatchGroupLibrary(LIBOFFSET(cinema::ColorSwatchGroupLibrary, b)); \
																																						if (!lib || !lib->b) return a; \
																																						return (reinterpret_cast<cinema::iColorSwatchGroup*>(this)->*lib->b)

#define ColorSwatchGroupCallRC(a, b)			cinema::ColorSwatchGroupLibrary *lib = cinema::CheckColorSwatchGroupLibrary(LIBOFFSET(cinema::ColorSwatchGroupLibrary, b)); \
																																						if (!lib || !lib->b) return a; \
																																						return (reinterpret_cast<const cinema::iColorSwatchGroup*>(this)->*lib->b)
...
{
	ColorSwatchGroupCallRC(nullptr, GetName)();
}
...
{
	ColorSwatchGroupCallRC(0, GetColorCount)();
}

Bool ColorSwatchGroup::GetColor(Int index, maxon::ColorA& color, Bool* selected) const
{
	ColorSwatchGroupCallRC(false, GetColor)(index, color, selected);
}
...
{
	ColorSwatchGroupCallRC(false, GetColors)(colors, selectedOnly);
}
...
{
	ColorSwatchGroupCallRC(false, HasDuplicatedColors)();
}
...
{
	ColorSwatchGroupCallRC(false, IsColorSelected)(index);
}
...
{
	ColorSwatchGroupCallRC(false, IsGroupSelected)();
}
