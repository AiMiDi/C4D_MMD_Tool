...
#define iGeClipMap	cinema::GeClipMap
...

{
	...
	/// @copydoc Alloc
	static GeClipMap* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		/// Enumerates all fonts and returns them in a container. The data is cached for faster access. @see GeClipMap::ClearFontListCache()\n
		...
		//----------------------------------------------------------------------------------------
		static const BaseContainer& GetFontListHierarchical();
		...
		//----------------------------------------------------------------------------------------
		static const BaseContainer& GetFontListFlat();

		//----------------------------------------------------------------------------------------
		/// Clears the cached font lists, which will be rebuilt when calling @ref GeClipMap::GetFontListHierarchical() or @ref GeClipMap::GetFontListFlat(). Suitable to include new/removed fonts.
		//----------------------------------------------------------------------------------------
		static void ClearFontListCache();
		...
		//----------------------------------------------------------------------------------------
		/// Returns a valid (installed) font description that closely matches the input @formatParam{fontDescription}. @see GeClipMap::IsFontInstalled()
		...
		//----------------------------------------------------------------------------------------
		static BaseContainer GetInstalledFont(const BaseContainer& fontDescription, Bool* isFontInstalled = nullptr);

		//----------------------------------------------------------------------------------------
		/// Gets the font name stored in the input font description, without checking agaisnt installed fonts.
		/// @note	Empty result means is a default Cinema 4D system font description.
		/// @param[in] fontDescription		The font description.
		/// @return												The raw font name.
		//----------------------------------------------------------------------------------------
		static String GetRawFontName(const BaseContainer& fontDescription);
...
struct GeClipMapLib : public C4DLibrary
{
		...
		const BaseContainer& (*GetFontListHierarchical)();
		void								 (*ClearFontListCache)();
		const BaseContainer& (*GetFontListFlat)();
		Bool 								 (*IsFontInstalled)(const BaseContainer& fontDescription, BaseContainer* installedFontDescription);
		BaseContainer 			 (*GetInstalledFont)(const BaseContainer& fontDescription, Bool* isFontInstalled);
		String 							 (*GetRawFontName)(const BaseContainer& fontDescription);
};
