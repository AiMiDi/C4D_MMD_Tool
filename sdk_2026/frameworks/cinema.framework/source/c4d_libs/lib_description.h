...
/// Root description ID.
#define DESCID_ROOT						ConstDescID(DescLevel(1000491, 0, 0))
...
enum DESC
{
	...
	DESC_UNIMPORTANTFORDEFAULTS = 85,		///< Ignore this parameter when comparing defaults

	DESC_BUTTON_STYLE = 86,							///< Button style for @link DTYPE_BUTTON@endlink. @since 2024.000
		DESC_BUTTON_STYLE_NORMAL			= 0,	///< Normal button style.
...
#define CUSTOMGUI_STRINGLINK			1060867								///< String Field with d&d filter and icons
#define CUSTOMGUI_PATH						300002000							///< ::String System file path list.
#define CUSTOMGUI_RADIOBUTTONS		200000281							///< ::Int32 Radio buttons.
/// @}
...
struct DescLevel
{
	Int32 id = 0;			 ///< ID of the level.
	Int32 dtype = 0;	 ///< Data type. Either a custom ID or one of these: @link DTYPE_NONE DTYPE@endlink.
	Int32 creator = 0; ///< Creator ID.
	...
	{
		return FormatString("@/@/@", id, dtype, creator);
	}
};
...
//----------------------------------------------------------------------------------------
class DescID : public iCustomDataType<DescID, CUSTOMDATATYPE_DESCID> // iCustomDataType is OK here since the size is the same
{
public:
	...
	constexpr explicit DescID(maxon::IN_PLACE_TYPE v, const DescLevel (&ids)[elementCount])
		: _constData(ids, elementCount)
	{
		static_assert(elementCount > 0, "elementCount must be > 0");
	}
	...
	//----------------------------------------------------------------------------------------
	/// Pushes a new level onto the stack.
	/// @param[in] subids							Levels to push.
	//----------------------------------------------------------------------------------------
...
private:
	// #DescIdSizeCheck must fit to size of internal DescID structure
	union
	{
...
{
public:
	static constexpr Int32 _ids[] = {T...};

	using Array = DescLevel[sizeof...(T) / 3];

	static const Array& ToArray()
	{
		return reinterpret_cast<const Array&>(_ids);
	}
};

#define YDescLevel(A, B, C, ...) A, B, C

#define XDescLevel(...) MAXON_EXPAND(YDescLevel(__VA_ARGS__, 0, 0))

#define AddDescX(t, i, a) X##t
...
class Description
{
	...
	/// @copydoc Alloc
	static Description* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
#define HandleDescGetVector(tid, vector, t_data, flags) \
	switch (tid[1].id) \
	{ \
		case 0:			t_data = cinema::GeData(vector);   flags |= cinema::DESCFLAGS_GET::PARAM_GET; break; \
		case 1000:	t_data = cinema::GeData(vector.x); flags |= cinema::DESCFLAGS_GET::PARAM_GET; break; \
		case 1001:	t_data = cinema::GeData(vector.y); flags |= cinema::DESCFLAGS_GET::PARAM_GET; break; \
		case 1002:	t_data = cinema::GeData(vector.z); flags |= cinema::DESCFLAGS_GET::PARAM_GET; break; \
	} \
...
#define HandleDescSetVector(v, tid, vector, t_data, flags) \
	switch (tid[1].id) \
	{ \
		case 0:			v = t_data.GetVector(); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
		case 1000:	v = cinema::Vector(t_data.GetFloat(), vector.y, vector.z); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
		case 1001:	v = cinema::Vector(vector.x, t_data.GetFloat(), vector.z); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
		case 1002:	v = cinema::Vector(vector.x, vector.y, t_data.GetFloat()); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
	} \

#define HandleDescSetVector32(v, tid, vector, t_data, flags) \
	switch (tid[1].id) \
	{ \
		case 0:			v = Vector32(t_data.GetVector()); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
		case 1000:	v = cinema::Vector32((Float32)t_data.GetFloat(), vector.y, vector.z); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
		case 1001:	v = cinema::Vector32(vector.x, (Float32)t_data.GetFloat(), vector.z); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
		case 1002:	v = cinema::Vector32(vector.x, vector.y, (Float32)t_data.GetFloat()); flags |= cinema::DESCFLAGS_SET::PARAM_SET; break; \
	} \
...
struct DescriptionCommand : public DescriptionBaseMessage
{
	...
	BaseContainer _data;															///< Auxiliary data container
};
...
//----------------------------------------------------------------------------------------
/// Message struct for the @ref MSG_DESCRIPTION_IMEXPORT_VERIFY_SUFFIX message.
/// @since R26
//----------------------------------------------------------------------------------------
struct DescriptionImExportVerifySuffix : public DescriptionBaseMessage
{
	String* _suffix = nullptr;			 ///< The suffix which needs to be verified. Can be @formatConstant{nullptr}.
	Bool		_addToRecentList = true; ///< This can be set to @formatConstant{false} to prevent the exported filename to be added to the recent document list.
};

//----------------------------------------------------------------------------------------
/// Message struct for the @ref MSG_DESCRIPTION_IMPEXPORT_VERIFY_FILENAME message.
/// @since R2023.100
//----------------------------------------------------------------------------------------
struct DescriptionImExportVerifyFilename : public DescriptionBaseMessage
{
	Filename* _filename = nullptr;			 ///< The filename which needs to be verified. Can be @formatConstant{nullptr}.
};
...
struct DescriptionLib : public C4DLibrary
{
	...
	void *								(*BrowseInit)(const Description *desc);
	Bool									(*GetNext)(const Description *desc, void *handle, const BaseContainer **bc, DescID &id, DescID &groupid);
	void									(*BrowseFree)(const Description *desc, void *&handle);
	...
	void									(*DescID_InitC)(DescID* id, const maxon::Block<const DescLevel>& ids);
	...
	void									(*DescID_PushId2)(DescID* id, const std::initializer_list<DescLevel>& subids);
	...
	void									(*DescID_CopyTo)(const DescID& src, DescID& dest);
	void									(*DescID_MoveTo)(DescID&& src, DescID& dest);
	Bool									(*DescID_Compare)(DescID *d1, DescID *d2);
	Bool									(*DescID_IsPartOf)(const DescID& thisPtr, const DescID& cmp, Int32* pos);
	Int32									(*DescID_GetDepth)(const DescID& thisPtr);
	...
	Bool									(*DescID_Write)(const DescID *id, HyperFile *hf);
...
inline DescID::DescID(maxon::IN_PLACE_TYPE v, const maxon::Block<const DescLevel>& ids)
{
	DebugAssert(_constData.GetCount() > 0, "elementCount must be > 0");

	DescriptionLib* lib = CheckDescriptionLib(LIBOFFSET(DescriptionLib, DescID_Init));
	if (!lib || !lib->DescID_InitC)
		return;
	lib->DescID_InitC(this, ids);
}
