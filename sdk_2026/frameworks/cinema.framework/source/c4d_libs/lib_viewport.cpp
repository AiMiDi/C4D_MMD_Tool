...
static ViewportHUDLib *CheckViewportHUDLib(Int32 offset) { return CheckLib<ViewportHUDLib>(LIBRARY_HUD, offset, lib_viewhud); }
static ViewportHighlightLib *CheckViewportHighlightLib(Int32 offset) { return CheckLib<ViewportHighlightLib>(LIBRARY_HIGHLIGHT, offset, lib_viewhlight); }

static ViewportHUDControlLib *CheckViewportHUDControlLib(Int32 offset)
{
	return CheckLib<ViewportHUDControlLib>(LIBRARY_HUD_CONTROL, offset, lib_viewhudctrl);
}

#define ViewportHUDLibCall(b) 		cinema::ViewportHUDLib *lib = cinema::CheckViewportHUDLib(LIBOFFSET(cinema::ViewportHUDLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iViewportHUD*>(this)->*lib->b)

#define ViewportHUDLibCallR(a, b)  cinema::ViewportHUDLib *lib = cinema::CheckViewportHUDLib(LIBOFFSET(cinema::ViewportHUDLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iViewportHUD*>(this)->*lib->b)

#define ViewportHighlightLibCall(b) 		cinema::ViewportHighlightLib *lib = cinema::CheckViewportHighlightLib(LIBOFFSET(cinema::ViewportHighlightLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iViewportHighlight*>(this)->*lib->b)

#define ViewportHighlightLibCallR(a, b)  cinema::ViewportHighlightLib *lib = cinema::CheckViewportHighlightLib(LIBOFFSET(cinema::ViewportHighlightLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iViewportHighlight*>(this)->*lib->b)

#define ViewportHUDControlLibCall(b)  cinema::ViewportHUDControlLib *lib = cinema::CheckViewportHUDControlLib(LIBOFFSET(cinema::ViewportHUDControlLib, b)); \
															if (!lib || !lib->b) return; \
															(lib->b)

#define ViewportHUDControlLibCallR(a, b)  cinema::ViewportHUDControlLib *lib = cinema::CheckViewportHUDControlLib(LIBOFFSET(cinema::ViewportHUDControlLib, b)); \
															if (!lib || !lib->b) return a; \
															return (lib->b)
...
Bool ViewportHighlight::IsHighlightActive(const BaseDocument* doc, const BaseDraw* bd)
{
	ViewportHighlightLibCallR(0, IsHighlightActive)(doc, bd);
}
} // namespace cinema
