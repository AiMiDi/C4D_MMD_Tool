...
#ifndef NOTINDEX
#define NOTINDEX				(cinema::LIMIT<cinema::Int32>::MAX)			///< Invalid index.
#endif
...
public:
	...
	//----------------------------------------------------------------------------------------
	inline Bool IsSegmentEnd() const
	{
	...
	//----------------------------------------------------------------------------------------
	inline Int32 Edge()  const
	{
		if (edge_index&PGONEDGE_NOEDGES)
			return NOTOK;
		return (edge_index&PGONEDGE_EDGEINDEX)%4;
	}
...
public:
	...
	/// @return												The edge.
	//----------------------------------------------------------------------------------------
	inline PgonEdge* GetEdge(Int32 i) { if (m_Edge.IsPopulated()) return &m_Edge[i]; else return nullptr; }

	//----------------------------------------------------------------------------------------
	/// Gets access directly to a const PgonEdge. Normally the helper functions below should be used.
	/// @param[in] i									An edge index.
	/// @return												The edge.
	//----------------------------------------------------------------------------------------
	inline const PgonEdge* GetEdge(Int32 i) const { if (m_Edge.IsPopulated()) return &m_Edge[i]; else return nullptr; }
	...
	/// @param[out] p2								Assigned the second point index.
	//----------------------------------------------------------------------------------------
	void GetEdge(Int32 i, const CPolygon* vadr, Int32& p1, Int32& p2) const { m_Edge[i].GetEdge(vadr, p1, p2); }

private:

	maxon::BaseArray<PgonEdge> m_Edge;
	Int32			m_Count;
	UInt32		m_CRC;
};
#endif
...
public:
	...
	/// @copydoc Alloc
	static NgonBase* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool ToSelect(BaseSelect *pSelect, Int32 mode) const;
	...
	//----------------------------------------------------------------------------------------
	void GetCenter(Int32 id, const CPolygon *vadr, const Vector *padr, Vector *ip, Vector *in) const;
	...
	/// @copydoc Alloc
	static NgonBase* Alloc(const maxon::SourceLocation&, const NgonBase& src);

	//----------------------------------------------------------------------------------------
	/// Retrieves the read only array of Pgon data.
	/// @return												The N-gon array.
	//----------------------------------------------------------------------------------------
	const Pgon* GetNgonsR() const;

	/// @copydoc IsValidNgon
	Bool IsValidNgon(const Pgon* pgon, const CPolygon* vadr) const;

	//----------------------------------------------------------------------------------------
	/// Gets the hash code of the Ngon data
	/// @return												The hash code of the Ngon data.
	//----------------------------------------------------------------------------------------
	...
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Bool CheckHash(PolygonObject* op, maxon::HashInt& hash) const;
};
...
struct NgonBaseLib : public C4DLibrary
{
	...
	Bool (iNgonBase::*CopyTo)(iNgonBase *dst) const;
	Bool (iNgonBase::*Write)(HyperFile *hf) const;
	...
	Bool (iNgonBase::*ToSelect)(BaseSelect *pSelect, Int32 mode) const;
	void (iNgonBase::*GetCenter)(Int32 id, const CPolygon *vadr, const Vector *padr, Vector *ip, Vector *in) const;
	...
	Bool (iNgonBase::*Copy)(const Pgon *src, Pgon *dst);
	...
	const Pgon* (iNgonBase::*GetNgonsR)() const;
	Bool (iNgonBase::*IsValidNgonR)(const Pgon* pgon, const CPolygon* vadr) const;
	maxon::HashInt (iNgonBase::*GetHashCode)() const;
	Bool (iNgonBase::* CheckHash)(PolygonObject* op, maxon::HashInt& hash) const;
};
