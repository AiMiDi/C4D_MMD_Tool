...
#define VPBUFFER_MAGICBULLETLOOKSRAW		206				///< RGBA beauty image before Magic Bullet Looks effect @since R23.
/// @}
...
#define VPbloom										1040365 			///< Bloom Videopost Effect. @since R21
#define VPMagicBulletLooks				1054755				///< Magic Bullet Looks Effect. @since R23
...
// this file.
#define VPcolormapping		 1001194 ///< Videopost plugin 'Color Mapping'. @PublicExposure
#define VPcylindricallens	 1001186 ///< Videopost plugin 'Cylindrical Lens'. @PublicExposure
#define VPnormalpass			 1028398 ///< Videopost plugin 'Normal Pass'. @PublicExposure
#define VPpositionpass		 1027117 ///< Videopost plugin 'Position Pass'. @PublicExposure
#define VPrsrenderer			 1036219 ///< Videopost plugin 'Redshift'. @PublicExposure
#define VPrsposteffects		 1040189 ///< Videopost plugin 'Redshift Post-Effects'. @PublicExposure
#define VPtoons						 1011015 ///< Videopost plugin 'Sketch and Toon'. @PublicExposure
#define VPvectormotionblur 1002008 ///< Videopost plugin 'Vector Motion Blur'. @PublicExposure
#define VPwatermark				 1025462 ///< Videopost plugin 'Watermark'. @PublicExposure
/// @}
...
/// @{
/// Video post priority.
#define VPPRIORITY_OBJECTGLOW			600				///< Priority for object glow.				(scattering in scene)
#define VPPRIORITY_OBJECTMOTIONBLUR 	500				///< Priority for object motion blur.		(aperture/shutter)
#define VPPRIORITY_DEPTHOFFIELD			400				///< Priority for depth of field 			(lens)
#define VPPRIORITY_LENSEFFECTS			300				///< Priority for the lens effect.			(lens)
#define VPPRIORITY_BLOOM				200				///< Priority for bloom, after lens effects	(lens)
#define VPPRIORITY_FILTERSOFT			100				///< Priority for filter soft.				(post effect)
#define VPPRIORITY_FILTERSHARPEN		100				///< Priority for sharpen filter.			(post effect)
#define VPPRIORITY_FILTERMEDIAN			100				///< Priority for median filter.			(post effect)
#define VPPRIORITY_MAGICBULLETLOOKS -100			///< Priority for magic bullet looks effect. Should be applied as the last post effect. (post effects)
...
#define PLUGINFLAG_VIDEOPOST_STATIC						(1 << 8)																																					///< Static video post effect (automatically created, cannot be dragged). @since R23
#define PLUGINFLAG_VIDEOPOST_OFFBYDEFAULT			(1 << 9)																																					///< Make a video post effect disabled by default when it gets created. Only supported for static effects for now. @since R23
#define PLUGINFLAG_VIDEOPOST_SETTINGMODE			(1 << 10)																																					///< @markPrivate
#define PLUGINFLAG_VIDEOPOST_HIDE							(1 << 11)																																					///< @markPrivate
#define PLUGINFLAG_VIDEOPOST_RENDERER_SUPPORTS_DIRTY_REGIONS	(1 << 12)																													///< Renderer supports dirty regions to partially update the rendering in the PV.
/// @}
...
#define MSG_VIDEOPOST_SETPREVIEW_VIEWPORT	1055286				///< Called to request Magic Bullet Looks Video Post to set the preview image from the current image in Viewport. @since R23
#define MSG_VIDEOPOST_SETPREVIEW					1055287				///< Called to request Magic Bullet Looks Video Post to set the preview image with the image provided in the message data. @since R23
#define MSG_VIDEOPOST_OPENEDITOR					1056194				///< Called to request Magic Bullet Looks Video Post to open the Looks Editor. Return true in the message data if the Look was edited. @since R24
#define MSG_VIDEOPOST_APPLY								1056195				///< Called to request Magic Bullet Looks Video Post to apply the Look on the image provided in the message data. @since R24
...
#define MSG_VIDEOPOST_SETTINGMODE_QUERY		1056687				///< @markInternal
#define MSG_VIDEOPOST_SETTINGMODE_CHANGED	1056686				///< @markInternal

//----------------------------------------------------------------------------------------
/// Message struct for the @ref MSG_VIDEOPOST_SETTINGMODE_QUERY message.
/// @markInternal
//----------------------------------------------------------------------------------------
struct VideoPostSettingModeQuery
{
	maxon::BaseArray<String>	_modes;											///< @markInternal
	Int												_active = 0;								///< @markInternal
};
...
struct GlobalIlluminationInformation
{
	...
	Float refractive_cutoff;					///< Set the refractive cutoff threshold.
...
{
	const ColorProfile* renderColorProfile;		///< Color profile of renderer - can be either linear or sRGB.
	const ColorProfile* imageColorProfile;		///< Color profile of output image.
	ColorProfile* renderingColorSpace;				///< Render color space.
	ColorProfile* viewColorSpace;							///< View color space.
	ColorProfile* displayColorSpace;					///< Display color space.
};
...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE VideoPostData : public NodeData
{
public:
	...
	// static NodeData *Alloc() { return NewObjClear(MyNodeData); }
	// Inherited from NodeData
	...
	/// For example to disable the video post effect for the Hardware renderer:
	/// @code
	/// Bool MyEffect::RenderEngineCheck(const BaseVideoPost* node, Int32 id) const
	/// {
	/// 	// The following render engines are not supported by this effect
	/// 	if (id == RDATA_RENDERENGINE_HARDWARE)
	/// 		return false;
	///
	/// 	return true;
	/// }
	...
	/// @param[in] node								The BaseVideoPost connected with the VideoPostData instance. Equal to <tt>static_cast</tt><@ref BaseVideoPost*>Get(). Provided for speed and convenience. @cinemaOwnsPointed{video post}
	/// @param[in] vps								The video post information. @callerOwnsPointed{video post struct}. Can be nullptr.
	/// @param[in] info								The color profile information.
	//----------------------------------------------------------------------------------------
