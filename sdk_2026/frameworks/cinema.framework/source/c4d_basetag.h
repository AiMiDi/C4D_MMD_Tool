...
#define TAG_IMPLEMENTS_DRAW_FUNCTION	(1 << 8)			///< The tag implements the ::Draw function.
#define TAG_VARIABLEFIELD							(1 << 9)			///< The tag is a variable tag with field
#define TAG_ICONCHOOSER_PARENT_IGNORE (1 << 10)			///< ex ID_ICONCHOOSER_SETTINGS_PARENT_IGNORE in settings container
#define TAG_MODIFYMATRIX							(1 << 11)			///< The tag creates a copy of the local matrix and modifies it.
/// @}

#define MSG_CHECKDIRTY_MODIFYMATRIX_TAG 1061665
...
public:
	...
	/// @copydoc Alloc
	static BaseTag* Alloc(const maxon::SourceLocation&, Int32 type) { return Alloc(type); }
	...
	//----------------------------------------------------------------------------------------
	BaseTag* GetNext() { return (BaseTag*)AtCall(GetNext) (); }
	const BaseTag* GetNext() const { return (BaseTag*)AtCall(GetNext) (); }
	...
	//----------------------------------------------------------------------------------------
	BaseTag* GetPred() { return (BaseTag*)AtCall(GetPred) (); }
	const BaseTag* GetPred() const { return (BaseTag*)AtCall(GetPred) (); }
	...
	//----------------------------------------------------------------------------------------
	const BaseObject* GetObject() const { return static_cast<const BaseObject*>(AtCall(GetMain) ()); }

	//----------------------------------------------------------------------------------------
	/// Gets the object that the tag is attached to.
	/// @return												The host object, or @formatConstant{nullptr} if the tag is not associated with an object. @cinemaOwnsPointed{object}
	//----------------------------------------------------------------------------------------
	BaseObject* GetObject() { return MAXON_NONCONST_COUNTERPART(GetObject()); }
	...
	//----------------------------------------------------------------------------------------
	BaseTag* GetOrigin() { return C4DOS_Tg->GetOrigin(this); }
	const BaseTag* GetOrigin() const { return C4DOS_Tg->GetOrigin(MAXON_REMOVE_CONST(this)); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	void SetOrigin(BaseTag* origin) { C4DOS_Tg->SetOrigin(this, origin); }
	...
	/// Draws a tag.
	//----------------------------------------------------------------------------------------
	Bool Draw(BaseObject *op, BaseDraw *bd, BaseDrawHelp *bh) { return C4DOS_Tg->Draw(this, op, bd, bh); }

	//----------------------------------------------------------------------------------------
	/// Finds all tags on this tag's object and its caches which match this tag and belong to a point object.
	/// If this tag's object is an undeformed point object itself, the function will return just this tag.
	/// Otherwise it recursively checks the cache (and also the deform cache if useDeformed is true) for matching tags,
	/// where "matching" means that either the cache tag has this tag as origin, or it has the same name.
...
public:
	...
	/// @copydoc Alloc
	static VariableTag* Alloc(const maxon::SourceLocation&, Int32 type, Int32 count) { return Alloc(type, count); }
	...
	//----------------------------------------------------------------------------------------
	/// Returns a UniqueHash of the data.
	/// @return												A unique hash value for all data elements.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Gets the copy-on-write reference to the data array of the variable tag.
	/// @return												A reference to the data array.
	//----------------------------------------------------------------------------------------
	const VariableTagDataRef<>& GetData() const;
...
public:
	...
	/// @copydoc Alloc
	static VertexMapTag* Alloc(const maxon::SourceLocation&, Int32 count) { return Alloc(count); }
	...
	//----------------------------------------------------------------------------------------
	const Float32* GetDataAddressR() const;
...
public:
	...
	/// @copydoc Alloc
	static SelectionTag* Alloc(const maxon::SourceLocation&, Int32 type) { return Alloc(type); }
...
public:
	...
	/// @copydoc Alloc
	static UVWTag* Alloc(const maxon::SourceLocation&, Int32 count) { return Alloc(count); }
	...
	//----------------------------------------------------------------------------------------
	UVWStruct GetSlow(Int32 i) { UVWStruct us; C4DOS_Tg->UvGet(this, i, &us); return us; }
	...
	//----------------------------------------------------------------------------------------
	void SetSlow(Int32 i, const UVWStruct& s) { C4DOS_Tg->UvSet(this, i, (UVWStruct*) &s); }
	...
	//----------------------------------------------------------------------------------------
	void CpySlow(Int32 dst, UVWTag* srctag, Int32 src) { C4DOS_Tg->UvCpy(this, dst, srctag, src); }

	//----------------------------------------------------------------------------------------
	/// Gets a handle to the read-only UVW data.
	/// @return												The read-only UVW data. @theOwnsPointed{tag,UVW data}
	//----------------------------------------------------------------------------------------
	ConstUVWHandle GetDataAddressR() const;
	...
	//----------------------------------------------------------------------------------------
	/// Calculates the bounding box of the UVW data.
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	static void Get(ConstUVWHandle dataptr, Int32 i, UVWStruct& res) { C4DOS_Tg->UvGet2(dataptr, i, &res); }
	...
	/// @param[out] res								Assigned the UVW coordinates for the polygon.
	//----------------------------------------------------------------------------------------
	static void Get(ConstUVWHandle dataptr, Int32 i, SUVWStruct& res) { C4DOS_Tg->UvGet2S(dataptr, i, &res); }
	...
	//----------------------------------------------------------------------------------------
	static void Set(UVWHandle dataptr, Int32 i, const UVWStruct& s) { C4DOS_Tg->UvSet2(dataptr, i, s); }
	...
	//----------------------------------------------------------------------------------------
	static void Copy(UVWHandle destptr, Int32 dst, ConstUVWHandle srcptr, Int32 src) { C4DOS_Tg->UvCpy2(srcptr, src, destptr, dst); }

	/// @}

	/// @name Operations on the Pin Point Selection
	/// @{

	//----------------------------------------------------------------------------------------
	/// Clears the pin point selection for this tag.
	//----------------------------------------------------------------------------------------
	void ClearPinSelection() { C4DOS_Tg->ClearPinSelection(this); }
	...
	/// @name Operations on the UVW Topology and UVW Object.
	/// @{

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the start of the read-only array of UVW polygons.
	/// @return												The first element in the read-only polygon array. @theOwnsPointed{UVWTag, array}
	//----------------------------------------------------------------------------------------
	const CPolygon* GetUVWPolygonR() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the start of the writable array of UVW polygons.
	/// @return												The first element in the read-only polygon array. @theOwnsPointed{UVWTag, array}
	//----------------------------------------------------------------------------------------
	CPolygon* GetUVWPolygonW();

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the number of UVW polygons.
	/// @return												The polygon count.
	//----------------------------------------------------------------------------------------
	Int32 GetUVWPolygonCount() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the start of the read-only array of UVW points.
	/// @return												The first element in the read-only point array. @theOwnsPointed{UVWTag, array}
	//----------------------------------------------------------------------------------------
	const Vector* GetUVWPointR() const;
	...
	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the number of UVW points.
	/// @return												The polygon count.
	//----------------------------------------------------------------------------------------
	Int32 GetUVWPointCount() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Changes the number of points and polygons in the UVW Topology.
	/// @param[in] pointCount					The new number of UVW points.
	/// @param[in] polygonCount				The new number of UVW polygons. Should be in sync with the PolygonObject.
	/// @return												@trueIfOtherwiseFalse{the number of points and polygons was changed successfully}
	//----------------------------------------------------------------------------------------
	Bool ResizeUVWTopology(Int32 pointCount, Int32 polygonCount);

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Checks the validity of the UVW Topology.
	/// If the topology is invalid, the returned error will contain more details about why the
	/// topology is not valid, such as the relevant point/polygon index.
	/// @return												OK if the topology is valid, otherwise error.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> IsUVWTopologyValid() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Optimizes the UVW topology while respecting the topology of the owning Polygon Object. 
	/// If two uv points are connected to the same 3D point, and share the same value, they 
	/// will be merged into a single uv point.
	/// @return												OK on success, otherwise error.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> OptimizeUVWTopology();

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the UV data represented as a PolygonObject. It allows reading various UV data 
	/// information, including proper topology and polygon connectivity information. 
	/// This PolygonObject can accessed as a regular polygon object, including reading the 
	/// polygons via GetPolygonR() and points via GetPointR(). The polygon count and the 
	/// ngon structure will always be in sync with the PolygonObject that owns this UVWTag, 
	/// but the point count will not correspond to either 4 * GetPolygonCount() nor the point
	/// count of the owning object. Rather, the point count corresponds to the number of unique
	/// UV points. 
	/// @return												Read-Only UVW Object.
	//----------------------------------------------------------------------------------------
	const PolygonObject* GetUVWObjectR() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the UV data represented as a PolygonObject. It allows modifying various UV data
	/// information, including proper topology and polygon connectivity information. 
	/// When this polygon object is modified, topology and connectivity modification is 
	/// allowed.
	/// It is also allowed to increase or decrease the point count, to facilitate welding uv 
	/// points and new seam creation. It is not possible to change the polygon count of this 
	/// object because the polygon count must be in sync with the PolygonObject that owns 
	/// this tag. 
	/// If polygon count of this object is to be changed, then MSG_POLYGONS_CHANGED should be 
	/// sent to the tag. Direct calls to ResizeObject() or MSG_POLYGONS_CHANGED will fail. 
	/// If the uv object polygon count, and PolygonObject polygon count are out of sync, this
	/// might lead to instability and crashes. Changing the point count via ResizeObject() or
	/// MSG_POINTS_CHANGED is allowed and valid, and sometimes necessary to  create the 
	/// desired connectivity information or UV seams. 
	/// The connectivity information of this object must also follow the connectivity 
	/// information of the object owning the tag. That means that if two points are not 
	/// connected on the owning PolygonObject (in 3D) they  cannot be connected in UV space. 
	/// To check whether the modifications done to the UV Object are valid, 
	/// @ref IsUvtopologyValid() can be used. 
	/// @return												Writable UVW Object.
	//----------------------------------------------------------------------------------------
	PolygonObject* GetUVWObjectW();

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the selected UV points.
	/// @return												The selection of visible points. @theOwnsPointed{UVW Tag,base select}
	//----------------------------------------------------------------------------------------
	BaseSelect* GetWritableUVWPointS();
	...
	/// @return												The selection of visible points. @theOwnsPointed{UVW Tag,base select}
	//----------------------------------------------------------------------------------------
	const BaseSelect* GetUVWPointS() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the selected UV edges.
	/// @return												The selection of visible edges. @theOwnsPointed{UVW Tag,base select}
	//----------------------------------------------------------------------------------------
	EdgeBaseSelect* GetWritableUVWEdgeS();
	...
	/// @return												The selection of visible edges. @theOwnsPointed{UVW Tag,base select}
	//----------------------------------------------------------------------------------------
	const EdgeBaseSelect* GetUVWEdgeS() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the selected UV polygons.
	/// @return												The selection of visible polygons. @theOwnsPointed{UVW Tag,base select}
	//----------------------------------------------------------------------------------------
	BaseSelect* GetWritableUVWPolygonS();
	...
	/// @return												The selection of visible polygons. @theOwnsPointed{UVW Tag,base select}
	//----------------------------------------------------------------------------------------
	const BaseSelect* GetUVWPolygonS() const;
...
public:
	...
	/// @copydoc Alloc
	static NormalTag* Alloc(const maxon::SourceLocation&, Int32 count) { return Alloc(count); }
	...
	//----------------------------------------------------------------------------------------
	ConstNormalHandle GetDataAddressR() const;
	...
	//----------------------------------------------------------------------------------------
	static void Get(ConstNormalHandle dataptr, Int32 i, NormalStruct& res) { C4DOS_Tg->NrmGet(dataptr, i, &res); }
	...
	//----------------------------------------------------------------------------------------
	static void Set(NormalHandle dataptr, Int32 i, const NormalStruct& s) { C4DOS_Tg->NrmSet(dataptr, i, s); }
	...
	//----------------------------------------------------------------------------------------
	static void Copy(NormalHandle destptr, Int32 dst, ConstNormalHandle srcptr, Int32 src) { C4DOS_Tg->NrmCpy(srcptr, src, destptr, dst); }
...
public:
	...
	/// @copydoc Alloc
	static TextureTag* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
public:
	...
	/// @copydoc Alloc
	static StickTextureTag* Alloc(const maxon::SourceLocation&, Int32 type) { return Alloc(type); }
...
public:
	...
	/// Example:
	/// @code
	/// ConstVertexColorHandle dataptr = static_cast<VertexColorTag*>(tag)->GetDataAddressR()
	...
	//----------------------------------------------------------------------------------------
	static void Get(ConstVertexColorHandle dataptr, Int32 i, VertexColorStruct& res) { C4DOS_Tg->VcolGet(dataptr, i, &res); }

	//----------------------------------------------------------------------------------------
	/// Sets the Vertex Colors Unclamped for the polygon @formatParam{i} in per polygon vertex mode.\n
	/// Example:
	/// @code
	/// VertexColorHandle dataptr = static_cast<VertexColorTag*>(tag)->GetDataAddressW()
	/// VertexColorStruct vcs;
	/// for (Int32 i = 0; i < faceCount; ++i)
	/// {
	/// 	VertexColorTag::Get(dataptr, faces[i], vcs);
	...
	//----------------------------------------------------------------------------------------
	static void Set(VertexColorHandle dataptr, Int32 i, const VertexColorStruct& s) { C4DOS_Tg->VcolSet(dataptr, i, s); }
	...
	/// @copydoc Alloc
	static VertexColorTag* Alloc(const maxon::SourceLocation&, Int32 count) { return Alloc(count); }
...
public:
	...
	/// @copydoc Alloc
	static HNWeightTag* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	Bool GetTagData(HNData* data) const;
...
public:
	...
	/// @param[in] original						If true, the original display mode is returned.
...
//----------------------------------------------------------------------------------------
/// Backtracks the origin of a tag to where it most likely came from.
/// This includes cloning, cache copies and alembic proxies.
/// @param[in] startTag						The tag to start the search from.
/// @return												An origin tag if one was found different to the startTag.
//----------------------------------------------------------------------------------------
