...
#define MAXELEMENTS 500000000			///< Maximum number of points.
...
//----------------------------------------------------------------------------------------
inline Matrix MatrixMove(const Vector& t)
{
	Matrix erg;
	erg.off = t;
	return erg;
...
//----------------------------------------------------------------------------------------
inline Matrix MatrixScale(const Vector& s)
{
	Matrix erg;
...
//----------------------------------------------------------------------------------------
inline Matrix MatrixRotX(Float w)
{
	Matrix erg;
...
/// @return												The rotation matrix.
//----------------------------------------------------------------------------------------
inline Matrix MatrixRotY(Float w)
{
	Matrix erg;
	Float	 cs = Cos(w), sn = Sin(w);
...
/// @return												The rotation matrix.
//----------------------------------------------------------------------------------------
inline Matrix MatrixRotZ(Float w)
{
	Matrix erg;
	Float	 cs = Cos(w), sn = Sin(w);
...
public:
	...
	//-------------------------------------------------------------------------------------------------
	inline void Init() { min = Vector64(MAXVALUE_FLOAT64); max = Vector64(MINVALUE_FLOAT64); used = false; }
	...
	{
		used = true;
		// Min/Max() instead of if() allows the compiler to use SSE/AVC vminpd, vmaxpd
		min.x = Min(p.x, min.x);
		max.x = Max(p.x, max.x);
		min.y = Min(p.y, min.y);
		max.y = Max(p.y, max.y);
	...
	//----------------------------------------------------------------------------------------
	inline Bool IsPopulated() const { return used; }
...
public:
	...
	/// Resets the minimum and maximum to the largest values possible.
	//-------------------------------------------------------------------------------------------------
	inline void Init() { min = Vector32(MAXVALUE_FLOAT32); max = Vector32(MINVALUE_FLOAT32); used = false; }
...
public:
	...
	{
		constexpr Float inv = 1.0_f / (2147483648.0_f + 1.0_f); // +1.0 in case of imprecision
...
public:
	...
	//----------------------------------------------------------------------------------------
	static Stratified2DRandom* Alloc() { return C4DOS_Sh->SNAlloc(); }

	/// @copydoc Alloc
	static Stratified2DRandom* Alloc(const maxon::SourceLocation&) { return Alloc(); }

	//----------------------------------------------------------------------------------------
	/// @destructsAlloc{generators}
	/// @param[in,out] rnd						@theToDestruct{generator}
	//----------------------------------------------------------------------------------------
	static void Free(Stratified2DRandom*& rnd) { C4DOS_Sh->SNFree(rnd); rnd = nullptr; }
	...
	//----------------------------------------------------------------------------------------
	Bool Init(Int32 initial_value, Bool reset) { return (this->*C4DOS_Sh->SNInit)(initial_value, reset); }
	...
	//----------------------------------------------------------------------------------------
	void GetNext(Float* xx, Float* yy) { (this->*C4DOS_Sh->SNGetNext)(xx, yy); }
};
...
//----------------------------------------------------------------------------------------
inline Float SNoise(const Vector& p) { return C4DOS_Sh->SNoise(p); }
...
/// @return												The signed noise value, between @em -1.0 and @em 1.0.
//----------------------------------------------------------------------------------------
inline Float SNoise(const Vector& p, Float t) { return C4DOS_Sh->SNoiseT(p, t); }
...
//----------------------------------------------------------------------------------------
inline Float Noise(const Vector& p) { return C4DOS_Sh->Noise(p); }
...
/// @return												The noise value, between @em 0.0 and @em 1.0.
//----------------------------------------------------------------------------------------
inline Float Noise(const Vector& p, Float t) { return C4DOS_Sh->NoiseT(p, t); }
...
//----------------------------------------------------------------------------------------
inline Float PNoise(const Vector& p, const Vector& d) { return C4DOS_Sh->PNoise(p, d); }
...
/// @return												The periodical noise value.
//----------------------------------------------------------------------------------------
inline Float PNoise(const Vector& p, Float t, const Vector& d, Float dt) { return C4DOS_Sh->PNoiseT(p, t, d, dt); }
...
//----------------------------------------------------------------------------------------
inline Float Turbulence(const Vector& p, Float oct, Bool abs) { return C4DOS_Sh->Turbulence(p, oct, abs); }
...
/// @return												The turbulence value, between @em -1.0 and @em 1.0 unless @formatParam{abs} is @formatConstant{true}, in which case it will be between @em 0.0 to @em 1.0.
//----------------------------------------------------------------------------------------
inline Float Turbulence(const Vector& p, Float t, Float oct, Bool abs) { return C4DOS_Sh->TurbulenceT(p, t, oct, abs); }
...
//----------------------------------------------------------------------------------------
inline Float WavyTurbulence(const Vector& p, Float t, Float oct, Float start) { return C4DOS_Sh->WavyTurbulence(p, t, oct, start); }
...
//----------------------------------------------------------------------------------------
inline void InitFbm(Float* table, Int32 max_octaves, Float lacunarity, Float h) { C4DOS_Sh->InitFbm(table, max_octaves, lacunarity, h); }
...
//----------------------------------------------------------------------------------------
inline Float Fbm(Float* table, const Vector& p, Float oct) { return C4DOS_Sh->Fbm(table, p, oct); }
...
/// @return												The fBm value.
//----------------------------------------------------------------------------------------
inline Float Fbm(Float* table, const Vector& p, Float t, Float oct) { return C4DOS_Sh->FbmT(table, p, t, oct); }
...
//----------------------------------------------------------------------------------------
inline Float RidgedMultifractal(Float* table, const Vector& p, Float oct, Float offset, Float gain) { return C4DOS_Sh->RidgedMultifractal(table, p, oct, offset, gain); }
...
//----------------------------------------------------------------------------------------
inline Float CalcSpline(Float x, const Float* knot, Int32 nknots) { return C4DOS_Sh->CalcSpline(x, knot, nknots); }
...
//----------------------------------------------------------------------------------------
inline Vector CalcSpline(Float x, const Vector* knot, Int32 nknots) { return C4DOS_Sh->CalcSplineV(x, knot, nknots); }
...
/// @return												The signed noise value, between @em -1.0 and @em 1.0.
//----------------------------------------------------------------------------------------
inline Float SNoiseP(Vector p, Float t, Int32 t_repeat) { return C4DOS_Sh->SNoiseP(p, t, t_repeat); }
...
/// @return												The turbulence value, between @em -1.0 and @em 1.0 unless @formatParam{abs} is @formatConstant{true}, in which case it will be between @em 0.0 to @em 1.0.
//----------------------------------------------------------------------------------------
inline Float TurbulenceP(Vector p, Float t, Float oct, Bool abs, Int32 t_repeat) { return C4DOS_Sh->TurbulenceP(p, t, oct, abs, t_repeat); }
...
/// @return												The fBm value.
//----------------------------------------------------------------------------------------
inline Float FbmP(Float* table, Vector p, Float t, Float oct, Int32 t_repeat) { return C4DOS_Sh->FbmP(table, p, t, oct, t_repeat); }
...
/// @return												The fractal value.
//----------------------------------------------------------------------------------------
inline Float RidgedMultifractalP(Float* table, Vector p, Float t, Float oct, Float offset, Float gain, Int32 t_repeat) { return C4DOS_Sh->RidgedMultifractalP(table, p, t, oct, offset, gain, t_repeat); }
...
public:
	...
	/// @copydoc Alloc
	static VolumeData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	Vector CalcShadow(const RayLight* l, const Vector64& p, const Vector64& bumpn, const Vector64& phongn, const Vector64& orign, const Vector64& rayv, Bool transparency, const RayHitID& hitid, RAYBIT raybits) { return C4DOS_Sh->CalcShadow(this, l, p, bumpn, phongn, orign, rayv, transparency, hitid, raybits); }
	...
	//----------------------------------------------------------------------------------------
	Bool IlluminateAnyPoint(const RayLight* rl, Vector* col, Vector64* light_vector, const Vector64& p, ILLUMINATEFLAGS flags, RAYBIT raybits) { return C4DOS_Sh->Illuminate(this, rl, col, light_vector, p, Vector64(0.0), Vector64(0.0), Vector64(0.0), Vector64(0.0), flags, RayHitID(), raybits, false, nullptr); }
	...
	//----------------------------------------------------------------------------------------
	void IlluminanceSurfacePoint(IlluminanceSurfacePointData* f, Vector* diffuse, Vector* specular) { C4DOS_Sh->IlluminanceSurfacePoint(this, f, diffuse, specular); }
	...
	//----------------------------------------------------------------------------------------
	Vector IlluminanceAnyPoint(const Vector64& p, ILLUMINATEFLAGS flags, RAYBIT raybits) { return C4DOS_Sh->IlluminanceAnyPoint(this, p, flags, raybits); }
	...
	{
		if (data)
			C4DOS_Sh->Illuminance(this, diffuse, specular, model, data);
		else
			C4DOS_Sh->Illuminance1(this, diffuse, specular, exponent);
	}
	...
	//----------------------------------------------------------------------------------------
	Int32 GetCurrentCPU() const { return C4DOS_Sh->GetCurrentCPU(this); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetCPUCount() const { return C4DOS_Sh->GetCPUCount(this); }
	...
	//----------------------------------------------------------------------------------------
	Int32 Obj_to_Num(const RayObject* obj) const { return C4DOS_Sh->Obj_to_Num(this, obj); }
	...
	//----------------------------------------------------------------------------------------
	const RayObject* GetObj(Int32 index) const { return C4DOS_Sh->GetObj(this, index); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the overall number of objects.
	/// @return												The total number of objects.
	//----------------------------------------------------------------------------------------
	Int32 GetObjCount() const { return C4DOS_Sh->GetObjCount(this); }
	...
	//----------------------------------------------------------------------------------------
	void GetUVW(const RayObject* op, Int32 uvwind, Int32 local_id, PolyVector* uvw) const { C4DOS_Sh->GetUVW(this, op, uvwind, local_id, uvw); }
	...
	//----------------------------------------------------------------------------------------
	void GetNormals(const RayObject* op, Int32 local_id, PolyVector* norm) const { C4DOS_Sh->GetNormals(this, op, local_id, norm); }
	...
	//----------------------------------------------------------------------------------------
	TexData* GetTexData(const RayObject* op, Int32 index) const { return C4DOS_Sh->GetTexData(this, op, index); }
	...
	//----------------------------------------------------------------------------------------
	Vector64 GetNormal(const RayObject* op, Int32 polygon, Bool second) const { return C4DOS_Sh->GetNormal(this, op, polygon, second); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetRayPolyState(const RayObject* op, Int32 local_id) const { return C4DOS_Sh->GetRayPolyState(this, op, local_id); }
	...
	//----------------------------------------------------------------------------------------
	Int32* GetUniqueID(const RayObject* op, Int32& length) const { return C4DOS_Sh->GetUniqueID(this, op, length); }
	...
	//----------------------------------------------------------------------------------------
	Vector64 GetSmoothedNormal(const RayHitID& hitid, const Vector64& p) const { return C4DOS_Sh->GetSmoothedNormal(this, hitid, p); }
	...
	//----------------------------------------------------------------------------------------
	Bool GetRS(const RayHitID& hitid, const Vector64& p, Float* r, Float* s) const { return C4DOS_Sh->GetRS(this, hitid, p, r, s); }
	...
	//----------------------------------------------------------------------------------------
	void GetWeights(const RayHitID& hitid, const Vector64& p, RayPolyWeight* wgt) const { C4DOS_Sh->GetWeights(this, hitid, p, wgt); }
	...
	//----------------------------------------------------------------------------------------
	const RayLight* GetLight(Int32 index) const { return C4DOS_Sh->GetLight(this, index); }

	//----------------------------------------------------------------------------------------
	/// Gets the overall number of lights.
	/// @return												The number of lights.
	//----------------------------------------------------------------------------------------
	Int32 GetLightCount() const { return C4DOS_Sh->GetLightCount(this); }
	...
	//----------------------------------------------------------------------------------------
	Int32 Light_to_Num(const RayLight* light) const { return C4DOS_Sh->Light_to_Num(this, light); }
	...
	//----------------------------------------------------------------------------------------
	const void* GetRayData(Int32 i) const { return C4DOS_Sh->GetRayData(this, i); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the raytracer camera.
	/// @return												The camera. @theOwnsPointed{volume data,camera}
	//----------------------------------------------------------------------------------------
	const RayCamera* GetRayCamera() const { return (RayCamera*)C4DOS_Sh->GetRayData(this, RAY_CAMERA); }
	...
	//----------------------------------------------------------------------------------------
	const RayParameter* GetRayParameter() const { return (RayParameter*)C4DOS_Sh->GetRayData(this, RAY_PARAMETER); }
	...
	//----------------------------------------------------------------------------------------
	const RayEnvironment* GetRayEnvironment() const { return (RayEnvironment*)C4DOS_Sh->GetRayData(this, RAY_ENVIRONMENT); }
	...
	//----------------------------------------------------------------------------------------
	const RayObject* GetRaySky() const { return (RayObject*)C4DOS_Sh->GetRayData(this, RAY_SKY); }
	...
	//----------------------------------------------------------------------------------------
	const RayObject* GetRayForeground() const { return (RayObject*)C4DOS_Sh->GetRayData(this, RAY_FOREGROUND); }
	...
	//----------------------------------------------------------------------------------------
	const RayObject* GetRayBackground() const { return (RayObject*)C4DOS_Sh->GetRayData(this, RAY_BACKGROUND); }
	...
	//----------------------------------------------------------------------------------------
	void GetRay(Float x, Float y, Ray* ray) const { C4DOS_Sh->GetRay(this, x, y, ray); }
	...
	//----------------------------------------------------------------------------------------
	Vector64 ScreenToCamera(const Vector64& p) const { return C4DOS_Sh->ScreenToCamera(this, p); }
	...
	//----------------------------------------------------------------------------------------
	Vector64 CameraToScreen(const Vector64& p) const { return C4DOS_Sh->CameraToScreen(this, p); }
	...
	//----------------------------------------------------------------------------------------
	Vector CalcHardShadow(const Vector64& p1, const Vector64& p2, CALCHARDSHADOW flags, const RayHitID& last_hit, Int32 recursion_id = 0, void* recursion_data = nullptr) { return C4DOS_Sh->CalcHardShadow(this, p1, p2, flags, last_hit, recursion_id, recursion_data); }
	...
	//----------------------------------------------------------------------------------------
	Vector GetPointUVW(const TexData* tdp, const RayHitID& hit, const Vector64& p) const { return C4DOS_Sh->GetPointUVW(this, tdp, hit, p); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool ProjectPoint(const TexData* tdp, const RayHitID& hit, const Vector64& p, const Vector64& n, Vector* uv) const { return C4DOS_Sh->ProjectPoint(this, tdp, hit, p, n, uv); }
	...
	//----------------------------------------------------------------------------------------
	void StatusSetText(const maxon::String& str) const { C4DOS_Sh->StatusSetText(this, str); }
	...
	//----------------------------------------------------------------------------------------
	void StatusSetBar(Float percentage) const { C4DOS_Sh->StatusSetBar(this, percentage); }
	...
	//----------------------------------------------------------------------------------------
	void StatusSetSpinMode(Bool on) const { C4DOS_Sh->StatusSetSpinMode(this, on); }
	...
	//----------------------------------------------------------------------------------------
	Vector CalcVisibleLight(const Ray* ray, Float maxdist, Vector* trans) const { return C4DOS_Sh->CalcVisibleLight(this, ray, maxdist, trans); }
	...
	//----------------------------------------------------------------------------------------
	void GetXY(Int32* x, Int32* y, Int32* scale) const { C4DOS_Sh->GetXY(this, x, y, scale); }
	...
	//----------------------------------------------------------------------------------------
	void SetXY(Float x, Float y) { C4DOS_Sh->SetXY(this, x, y); }
	...
	//----------------------------------------------------------------------------------------
	MinMax GetSceneBoundaries() const { Vector64 min, max; MinMax MM; C4DOS_Sh->GetSceneBoundaries(this, min, max); MM.Set(min, max); return MM; }

	//----------------------------------------------------------------------------------------
	/// Copies the volume data to another VolumeData.
	/// @param[out] dst								The destination VolumeData. @callerOwnsPointed{volume data}
	//----------------------------------------------------------------------------------------
	void CopyTo(VolumeData* dst) const { C4DOS_Sh->CopyVolumeData(this, dst); }
	...
	//----------------------------------------------------------------------------------------
	void Init(VolumeData* from) { C4DOS_Sh->InitVolumeData(from, this); }
	...
	//----------------------------------------------------------------------------------------
	void OutOfMemory() { C4DOS_Sh->OutOfMemory(this); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool CalcFgBg(Bool foreground, Int32 x, Int32 y, Int32 subx, Int32 suby, Vector* color, Float* alpha) const { return C4DOS_Sh->CalcFgBg(this, foreground, x, y, subx, suby, color, alpha); }
	...
	//----------------------------------------------------------------------------------------
	Float GetLightFalloff(Int32 falloff, Float inner, Float outer, Float dist) const { return C4DOS_Sh->GetLightFalloff(falloff, inner, outer, dist); }
	...
	//----------------------------------------------------------------------------------------
	Bool TestBreak() { return C4DOS_Sh->TestBreak(this); }
	...
	//----------------------------------------------------------------------------------------
	BaseVideoPost* GetVideoPost(Int32 nth) const { return C4DOS_Sh->FindVideoPost(this, nth, true); }
	...
	/// @return												The video post, or @formatConstant{nullptr} if the index is too large.
	//----------------------------------------------------------------------------------------
	BaseVideoPost* FindVideoPost(Int32 id) const { return C4DOS_Sh->FindVideoPost(this, id, false); }
	...
	//----------------------------------------------------------------------------------------
	const VPFragment** GetFragments(Int32 x, Int32 y, Int32 cnt, VPGETFRAGMENTS flags) const { return C4DOS_Sh->VPGetFragments(this, x, y, cnt, flags); }
	...
	//----------------------------------------------------------------------------------------
	Bool AddLensGlow(const LensGlowStruct* lgs, Vector* lgs_pos, Int32 lgs_cnt, Float intensity, Bool linear_workflow) const { return C4DOS_Sh->AddLensGlow(this, lgs, lgs_pos, lgs_cnt, intensity, linear_workflow); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool SampleLensFX(const VPBuffer* rgba, const VPBuffer* fx, BaseThread* bt) const { return C4DOS_Sh->SampleLensFX(this, rgba, fx, bt); }
	...
	//----------------------------------------------------------------------------------------
	Int32 TranslateObjIndex(Int32 index, Bool old_to_new) const { return C4DOS_Sh->TranslateObjIndex(this, index, old_to_new); }
	...
	//----------------------------------------------------------------------------------------
	Bool TranslatePolygon(const RayObject* op, Int32 local_index, Vector* previous_points) const { return C4DOS_Sh->TranslatePolygon(this, op, local_index, previous_points); }
	...
	//----------------------------------------------------------------------------------------
	Vector TraceColor(Ray* ray, Float maxdist, const RayHitID& last_hit, SurfaceIntersection* si) { return C4DOS_Sh->TraceColor(this, ray, maxdist, last_hit, si); }
	...
	/// @return												The color.
	//----------------------------------------------------------------------------------------
	Vector TraceColorDirect(Ray* ray, Float maxdist, Int32 raydepth, RAYBIT raybits, const RayHitID& last_hit, Vector64* oldray, SurfaceIntersection* si) { return C4DOS_Sh->TraceColorDirect(this, ray, maxdist, raydepth, raybits, last_hit, oldray, si); }
	...
	//----------------------------------------------------------------------------------------
	Bool TraceGeometry(const Ray* ray, Float maxdist, const RayHitID& last_hit, SurfaceIntersection* si) const { return C4DOS_Sh->TraceGeometry(this, ray, maxdist, last_hit, si); }
	...
	/// @return												@trueIfOtherwiseFalse{an intersection was found}
	//----------------------------------------------------------------------------------------
	Bool TraceGeometryEnhanced(const Ray* ray, Float maxdist, const RayHitID& last_hit, Int32 raydepth, RAYBIT raybits, Vector64* oldray, SurfaceIntersection* si) const { return C4DOS_Sh->TraceGeometryEnhanced(this, ray, maxdist, last_hit, raydepth, raybits, oldray, si); }
	...
	//----------------------------------------------------------------------------------------
	void GetSurfaceData(SurfaceData* cd, Bool calc_illum, Bool calc_shadow, Bool calc_refl, Bool calc_trans, Ray* ray, const SurfaceIntersection& si) { C4DOS_Sh->GetSurfaceData(this, cd, calc_illum, calc_shadow, calc_refl, calc_trans, ray, si); }
	...
	//----------------------------------------------------------------------------------------
	Vector CentralDisplacePointUV(const RayObject* op, Int32 local_id, Int32 uu, Int32 vv) const { return C4DOS_Sh->CentralDisplacePointUV(this, op, local_id, uu, vv); }
	...
	/// @return												The displacement vector.
	//----------------------------------------------------------------------------------------
	Vector CentralDisplacePointFromUVPoint(const RayObject *op, Int32 local_id, Bool bFirst, Float l1, Float l2, const Vector& uv, const UVWStruct& uvw) const { return C4DOS_Sh->CentralDisplacePointFromUVPoint(this, op, local_id, bFirst, l1, l2, uv, uvw); }
	...
	//----------------------------------------------------------------------------------------
	Vector CalcDisplacementNormals(Float par_u, Float par_v, Int32 u0, Int32 v0, Int32 u1, Int32 v1, Int32 u2, Int32 v2, const Vector& a, const Vector& b, const Vector& c, const RayObject* op, Int32 polynum) const { return C4DOS_Sh->CalcDisplacementNormals(this, par_u, par_v, u0, v0, u1, v1, u2, v2, a, b, c, op, polynum); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Vector CentralDisplaceUVGetNormal(const RayObject* op, Int32 local_id, Int32 uu, Int32 vv) const { return C4DOS_Sh->CentralDisplaceUVGetNormal(this, op, local_id, uu, vv); }
	...
	//----------------------------------------------------------------------------------------
	Vector CentralDisplaceGetSurfaceNormalFromUVPoint(const RayObject *op, Int32 local_id, Bool bFirst, Float par_u, Float par_v, const Vector& uv, const UVWStruct& uvw) const { return C4DOS_Sh->CentralDisplaceGetSurfaceNormalFromUVPoint(this, op, local_id, bFirst, par_u, par_v, uv, uvw); }
	...
	/// 															UV point at @formatParam{uv}.
	//----------------------------------------------------------------------------------------
	Vector CentralDisplaceGetShadingNormalFromUVPoint(const RayObject *op, Int32 local_id, Bool bFirst, Float par_u, Float par_v, const Vector& uv, const UVWStruct& uvw) const { return C4DOS_Sh->CentralDisplaceGetShadingNormalFromUVPoint(this, op, local_id, bFirst, par_u, par_v, uv, uvw); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Bool CentralDisplacePointUVGetBarycentric(const RayObject *op, Int32& local_id, Int32& uu, Int32& vv, RayPolyWeight& bary, Bool& displaceQuad) const { return C4DOS_Sh->CentralDisplacePointUVGetBarycentric(this, op, local_id, uu, vv, bary, displaceQuad); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Bool GetDisplaceInfo(const RayObject* op, DisplaceInfo& info) const { return C4DOS_Sh->GetDisplaceInfo(this, op, info); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Int32 GetDisplaceNeighbor(const RayObject* op, Int32 local_id, Int32 side) const { return C4DOS_Sh->GetDisplaceNeighbor(this, op, local_id, side); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	void CorrectLocalID(const RayObject* op, Int32& local_id) const { C4DOS_Sh->CorrectLocalID(this, op, local_id); }
	...
	//----------------------------------------------------------------------------------------
	Vector CalcBackTransformPoint() const { return C4DOS_Sh->CalcBackTransformPoint(this); }
	...
	//----------------------------------------------------------------------------------------
	Matrix64 CalcMotionObjectMatrixInv(const RayObject *op) const { return C4DOS_Sh->CalcMotionObjectMatrixInv(this, op); }

	//----------------------------------------------------------------------------------------
	/// Skips the render process.
	/// @note Useful for custom renderers.
	//----------------------------------------------------------------------------------------
	void SkipRenderProcess() { C4DOS_Sh->SkipRenderProcess(this); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the current Render* instance.
	/// @return												The render instance.
	//----------------------------------------------------------------------------------------
	const Render* GetRenderInstance() const { return C4DOS_Sh->GetRenderInstance(this); }
	...
	//----------------------------------------------------------------------------------------
	void GetDUDV(const TexData* tex, const Vector64& p, const Vector64& phongn, const Vector64& orign, const RayHitID& hit, Bool forceuvw, Vector* ddu, Vector* ddv, Bool usetangents, const RayPolyWeight* weight) const { C4DOS_Sh->GetDUDV(this, tex, p, phongn, orign, hit, forceuvw, ddu, ddv, usetangents, weight); }
	...
	/// @param[in] usetangents				@formatConstant{true} to use UV tangents to compute DU/DV components.
	//----------------------------------------------------------------------------------------
	void GetDUDVFromUVPoint(const RayObject* op, const TexData* tex, Float par_u, Float par_v, const Vector64& uv, const Vector64& p, const Vector64& phongn, const Vector64& orign, const UVWStruct& uvw, RayHitID& hit, Bool forceuvw, Vector* ddu, Vector* ddv, Bool usetangents) { C4DOS_Sh->GetDUDVFromUVPoint(this, op, tex, par_u, par_v, uv, p, phongn, orign, uvw, hit, forceuvw, ddu, ddv, usetangents); }
	...
	//----------------------------------------------------------------------------------------
	void InitSurfacePointProperties(TexData* td) { C4DOS_Sh->InitSurfacePointProperties(this, td); }
	...
	{
		maxon::Tuple<Int32, Int32> t{fakeCurrentThreadIdx, fakeThreadCount};
		return C4DOS_Sh->AttachVolumeDataFake(this, camera, renderdata, &t);
	}
	...
	//----------------------------------------------------------------------------------------
	Bool HasVolumeDataFake() const { return C4DOS_Sh->HasVolumeDataFake(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Bool SaveShaderStack(RayShaderStackElement*& stack, Int32& stack_cnt) { return C4DOS_Sh->SaveShaderStack(this, stack, stack_cnt); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Bool RestoreShaderStack(RayShaderStackElement* stack, Int32 stack_cnt) { return C4DOS_Sh->RestoreShaderStack(this, stack, stack_cnt); }
	...
	//----------------------------------------------------------------------------------------
	const RayObject* GetRaySky(Int32 index) const { return C4DOS_Sh->GetSky(this, index); }

	//----------------------------------------------------------------------------------------
	/// Gets the number of sky objects.
	/// @return												The number of sky objects.
	//----------------------------------------------------------------------------------------
	Int32 GetRaySkyCount() const { return C4DOS_Sh->GetSkyCount(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	RaySampler* GetRaySampler(void* guid, Int32 depth) { return C4DOS_Sh->GetSampler(this, guid, depth); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Float GetRayWeight() const { return C4DOS_Sh->GetRayWeight(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Float CalcAmbientOcclusion(SurfaceIntersection* si, RaySampler* sampler, Float minlength, Float maxlength, Float index, Bool self, Bool trans, Float weight, Bool reverse) { return C4DOS_Sh->CalcAmbientOcclusion(this, si, sampler, minlength, maxlength, index, self, trans, weight, reverse); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Vector CalcSubsurfaceScattering(Vector scattering, Vector absorption, Vector diffuse, Float ior) { return C4DOS_Sh->CalcSubsurfaceScattering(this, scattering, absorption, diffuse, ior); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the number of stereoscopic streams that are rendered.
	/// @return												The number of stereoscopic streams.
	//----------------------------------------------------------------------------------------
	Int32 GetStreamCount() const { return C4DOS_Sh->GetStreamCount(this); }
	...
	//----------------------------------------------------------------------------------------
	void GetStreamRay(Int32 stream, Float x, Float y, Ray* ray) const { C4DOS_Sh->GetStreamRay(this, stream, x, y, ray); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool CalcIndirectIllumination(RayHemisphere* hs, Float weight) { return C4DOS_Sh->CalcIndirectIllumination(this, hs, weight); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool CalcIndirectPDF(const Vector& point, const Vector& normal, const Vector& ray_in, const Vector& ray_out, Float& pdf_qmc, Float& pdf_area, Float& pdf_portal, Float& pdf_sky) { return C4DOS_Sh->CalcIndirectPDF(this, point, normal, ray_in, ray_out, pdf_qmc, pdf_area, pdf_portal, pdf_sky); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	Bool CalcIndirectPath(VolumeData* sd, const Vector& dir, void* source, void* target) { return C4DOS_Sh->CalcIndirectPath(this, dir, source, target); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	RayRadianceObject* CreateRadianceMaps() { return C4DOS_Sh->CreateRadianceMaps(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	RayRadianceObject* GetRadianceMaps() { return C4DOS_Sh->GetRadianceMaps(this); }
	...
	//----------------------------------------------------------------------------------------
	Vector CalcRadianceValue(Ray* ray, const SurfaceIntersection& si) { return C4DOS_Sh->CalcRadianceValue(this, ray, si); }
	...
	/// @return												The radiance value.
	//----------------------------------------------------------------------------------------
	Vector CalcRadiancePoly(RayRadianceObject* obj, SurfaceIntersection* si, Int32 poly, Int32 u, Int32 v, Bool back) { return C4DOS_Sh->CalcRadiancePoly(this, obj, si, poly, u, v, back); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsPhysicalRender() const { return C4DOS_Sh->IsPhysicalRender(this); }
	...
	//----------------------------------------------------------------------------------------
	Float GetPhysicalCameraIntensity() const { return C4DOS_Sh->GetPhysicalCameraIntensity(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetPhysicalRayTime(Float time) { return C4DOS_Sh->SetPhysicalRayTime(this, time); }
	...
	//----------------------------------------------------------------------------------------
	const RayMotionObject* GetMotionObject(RayObject *op) { return C4DOS_Sh->GetMotionObject(this, op); }
	...
	/// @return												The motion data.
	//----------------------------------------------------------------------------------------
	const RayMotionLight* GetMotionLight(RayLight *light) { return C4DOS_Sh->GetMotionLight(this, light); }
	...
	/// @return												The motion data.
	//----------------------------------------------------------------------------------------
	const RayMotionCamera* GetMotionCamera(Int32 stream) { return C4DOS_Sh->GetMotionCamera(this, stream); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool GetTile(Int32& xMin, Int32& yMin, Int32& xMax, Int32& yMax, Int32& renderThreadIndex) const { return C4DOS_Sh->GetTile(this, xMin, yMin, xMax, yMax, renderThreadIndex); }
};
...
public:
	...
	/// @return												The width.
	//----------------------------------------------------------------------------------------
	Int32 GetBw() const { return C4DOS_Sh->VPGetInfo(this, VPGETINFO::XRESOLUTION); }

	//----------------------------------------------------------------------------------------
	/// Gets the height of the videopost buffer.
	/// @return												The height.
	//----------------------------------------------------------------------------------------
	Int32 GetBh() const { return C4DOS_Sh->VPGetInfo(this, VPGETINFO::YRESOLUTION); }
	...
	/// 															- 32 (::Float)
	//----------------------------------------------------------------------------------------
	Int32 GetBt() const { return C4DOS_Sh->VPGetInfo(this, VPGETINFO::BITDEPTH); }

	//----------------------------------------------------------------------------------------
	/// Gets the bytes per pixel for the videopost buffer.
	/// @return												The number of bytes per pixel.
	//----------------------------------------------------------------------------------------
	Int32 GetCpp() const { return C4DOS_Sh->VPGetInfo(this, VPGETINFO::CPP); }
...
/// @note Buffers that were requested by plugins can be existent, but invisible (not listed in the Picture Viewer %Multipass menu).
	...
	//----------------------------------------------------------------------------------------
	Bool GetVisibleBit() const { return C4DOS_Sh->VPGetInfo(this, VPGETINFO::VISIBLE) != 0; }
};
...
enum BakeTexEnums
{
	...
	BAKE_TEX_OPTIMAL_MAPPING_PACKED									= 3,			///< Packed.
...
//----------------------------------------------------------------------------------------
/// @markPrivate
/// Usually we solve for an angle in a 360 deg (-PI to PI) range but there can be the scenario that a different range is required
//----------------------------------------------------------------------------------------
