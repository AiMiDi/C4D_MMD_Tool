...
#define MoccaCallR(a, b) cinema::MoccaLibrary* lib = cinema::CheckMoccaLib(LIBOFFSET(cinema::MoccaLibrary, b)); \
...
MTCharacterBodyPart* MTCharacterBodyPart::Alloc()
{
	// MTCharacterBodyPart is an AllocType, hence NewObj would lead to a recursion.
	MTCharacterBodyPart* m = static_cast<MTCharacterBodyPart*>(maxon::DefaultAllocator::Alloc(SIZEOF(MTCharacterBodyPart), MAXON_SOURCE_LOCATION));
	if (m)
		new (m) MTCharacterBodyPart();
	return m;
}

void MTCharacterBodyPart::Free(MTCharacterBodyPart*& ptr)
{
	// MTCharacterBodyPart is an AllocType, hence DeleteObj would lead to a recursion.
	if (ptr)
	{
		ptr->~MTCharacterBodyPart();
