...
public:
	...
	/// @copydoc Alloc
	static ParserCache* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
//----------------------------------------------------------------------------------------
inline Int32 GeGetTimer() { return C4DOS_Ge->GetTimer(); }

//----------------------------------------------------------------------------------------
/// Get the current timer count in milliseconds.
/// @return												The current timer count in milliseconds.
//----------------------------------------------------------------------------------------
inline Float64 GeGetMilliSeconds() { return C4DOS_Ge->GeGetMilliSeconds(); }
...
//----------------------------------------------------------------------------------------
inline Int32 GetC4DVersion() { return C4DOS.version; }
...
//----------------------------------------------------------------------------------------
/// @brief Executes a builtin modelling operation as for example extruding a polygon selection or selecting all points of an object.
...
//----------------------------------------------------------------------------------------
/// Clears the output of the Python console window in Cinema 4D.
/// This function removes all text currently displayed in the Python console,
/// providing a clean slate for new output.
/// @note This affects only the Python console and does not impact other consoles or logs.
//------------------------------------------------------
...
public:
	...
	//----------------------------------------------------------------------------------------
	Registry* GetNext() { return (Registry*)AtCall(GetNext) (); }
	...
	//----------------------------------------------------------------------------------------
	Registry* GetPred() { return (Registry*)AtCall(GetPred) (); }
...
//----------------------------------------------------------------------------------------
/// @brief Waits while a task is running.
/// @note Main thread only.
/// @param[in] type								The task: For now only CHECKISRUNNING::VIEWDRAWING is supported.
/// @return												@trueIfOtherwiseFalse{The task has finished}
//----------------------------------------------------------------------------------------
...
/// @markPrivate
//----------------------------------------------------------------------------------------
inline void _GeDebugBreak(Int32 line, const Char* file) { C4DOS_Ge->GeDebugBreak(line, file); }
...
public:
	...
	/// @copydoc Alloc
	static LassoSelection* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
#define COREMSG_CINEMA_GETMACHINEFEATURES			'gOGL'			///< Retrieves machine features:\n
	...
	/// @{
	#define DRAWPORT_TYPE_AVAILABLE													1000	///< @since R18
	#define DRAWPORT_EXTENSION_STRING												1003
	#define DRAWPORT_SUPPORT_ENHANCED												1005
	#define DRAWPORT_RENDERER_NAME													1006
	#define DRAWPORT_VERSION_STRING													1007
	#define DRAWPORT_VENDOR_NUM															1008
		#define DRAWPORT_VENDOR_UNKNOWN												0
		#define DRAWPORT_VENDOR_NVIDIA												1
		#define DRAWPORT_VENDOR_AMD														2
		#define DRAWPORT_VENDOR_INTEL													3
		#define DRAWPORT_VENDOR_APPLE													4
	#define DRAWPORT_VENDOR_NAME														1009
	#define DRAWPORT_SHADING_LANGUAGE_VERSION_STRING				1010
	#define DRAWPORT_MULTITEXTURE														1013
	#define DRAWPORT_MAX_2DTEXTURE_SIZE											1014
	#define DRAWPORT_MAX_3DTEXTURE_SIZE											1015
	#define DRAWPORT_MAX_VERTEX_ATTRIBS											1016
	#define DRAWPORT_MAX_TEX_IMAGE_UNITS_VERTEX							1017
	#define DRAWPORT_MAX_TEX_IMAGE_UNITS_FRAGMENT						1018
	#define DRAWPORT_DRIVER_VERSION_STRING									1030	// Only supported on Windows
	#define DRAWPORT_FBO_Z_DEPTH														1033
	#define DRAWPORT_FRAMEBUFFER_OBJECT_MULTISAMPLE					1034	// Int32 Maximum samples.
	#define DRAWPORT_MAX_TEX_IMAGE_UNITS_GEOMETRY						1037
	#define DRAWPORT_API_VERSION_INT												1040
	#define DRAWPORT_SHADING_LANGUAGE_VERSION_INT						1041
	#define DRAWPORT_RENDERBUFFER_MASK											1043	// UInt64
	#define DRAWPORT_RENDER_TO_FP16_TEXTURE									1044
	#define DRAWPORT_RENDER_TO_FP32_TEXTURE									1045
	#define DRAWPORT_STEREO_BUFFER													1046
	#define DRAWPORT_DRIVER_OUTDATED												1047	// only supported on Windows
	#define DRAWPORT_SUPPORT_PRIMITIVERESTARTINDEX					1048
	#define DRAWPORT_SUPPORT_GEOMETRYSHADER									1049
	#define DRAWPORT_SUPPORT_OSX_10_7												1050
	#define DRAWPORT_MAX_TEX_IMAGE_UNITS_TESS_CONTROL				1051
	#define DRAWPORT_MAX_TEX_IMAGE_UNITS_TESS_EVAL					1052
	#define DRAWPORT_MAX_TESSELLATION_LEVEL									1053
	#define DRAWPORT_API_TYPE								                1054 // String
	#define DRAWPORT_VRAM_SIZE							                1055 // UInt64, VRAM size in bytes
	#define DRAWPORT_REQUIRED_VERSION_STRING								1056 // String
	/// @}
...
#define C4D_MEMORY_STAT_OPENGL_ALLOCATED					10			///< ::Int32: Allocated OpenGL memory in KiB
#define C4D_MEMORY_STAT_OPENGL_USED								11			///< ::Int32: Used OpenGL memory in KiB
/// @}
...
//----------------------------------------------------------------------------------------
/// Check if the current context is UV, if UV mode is selected or the UV Texture Editor
...
inline Bool DEBUGMAINTHREADCHECK()
{
	if (GeIsMainThread())
		return true;
	CriticalStop();
...
#else
	#define DEBUGMAINTHREADCHECK() (!GeIsMainThread() ? false : true)
#endif
}
