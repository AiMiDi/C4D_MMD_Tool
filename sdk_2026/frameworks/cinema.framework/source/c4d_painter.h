...
public:
	...
	/// The edges have to be indexed as follows: <tt>4 * polygon + edge</tt> where @c polygon is the polygon index and @c edge is the edge index between @em 0 and @em 3.
	/// @param[in] uvEdgeSelection		The pointer to the new UV Edge selection.
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
...
#define GETACTIVEUVSET_EDGESELECTION				(1 << 9)			///< Get the selected edges.
...
#define GETACTIVEUVSET_ALL									(0xFFFF & ~GETACTIVEUVSET_CHECKENABLING)	///< Combine all flags except the enable status and ignore UV view private option.
/// @}
...
#define UVCOMMAND_RELAX												10003			///< Relax UV.
	...
	#define RELAXUV_SAFE												1028	///< Bool Try to handle errors. @since R22
	#define RELAXUV_ERROR_MESSAGE								1029	///< Bool Return an error message in case of error. @since R22
	#define RELAXUV_ERROR_POLY_SELECTION				1030	///< Bool In case of error select the polygons at the origin of this error or not. @since R22 SP1
...
#define UVCOMMAND_PACK_UV											10016			///< UV Packing
#define UVCOMMAND_AUTOMATICUV									10017			///< Fully automatic packed UVs.
#define UVCOMMAND_PACK_RASTERIZED							10018			///< Rasterized UV Packing Algorithm
#define UVCOMMAND_PACK_GEOMETRIC							10019			///< Geometric UV Packing Algorithm
#define UVCOMMAND_PACK_BOUNDINGBOX						10020			///< Bounding Box UV Packing Algorithm (named Realign Before r22)
/// @}
...
public:
	...
	//----------------------------------------------------------------------------------------
	Int32 GetBw() const { return C4DOS_Pa->PB_GetBw(this); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the height of the paint bitmap.
	/// @return												The height of the paint bitmap.
	//----------------------------------------------------------------------------------------
	Int32 GetBh() const { return C4DOS_Pa->PB_GetBh(this); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the paint texture if possible.
	/// @return												The paint texture, otherwise @formatConstant{nullptr}.
	//----------------------------------------------------------------------------------------
	PaintTexture* GetPaintTexture() { return C4DOS_Pa->PB_GetPaintTexture(this); }
	const PaintTexture* GetPaintTexture() const { return C4DOS_Pa->PB_GetPaintTexture(MAXON_REMOVE_CONST(this)); }
	...
	//----------------------------------------------------------------------------------------
	PaintBitmap* GetParent() { return C4DOS_Pa->PB_GetParent(this); }
	const PaintBitmap* GetParent() const { return C4DOS_Pa->PB_GetParent(MAXON_REMOVE_CONST(this)); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the first child layer.
	/// @return												The first child layer, or @formatConstant{nullptr} if there is none.
	//----------------------------------------------------------------------------------------
	PaintLayer* GetLayerDownFirst() { return C4DOS_Pa->PB_GetLayerDownFirst(this); }
	const PaintLayer* GetLayerDownFirst() const { return C4DOS_Pa->PB_GetLayerDownFirst(MAXON_REMOVE_CONST(this)); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the last child layer.
	/// @return												The last child layer, or @formatConstant{nullptr} if there is none.
	//----------------------------------------------------------------------------------------
	PaintLayer* GetLayerDownLast() { return C4DOS_Pa->PB_GetLayerDownLast(this); }
	const PaintLayer* GetLayerDownLast() const { return C4DOS_Pa->PB_GetLayerDownLast(MAXON_REMOVE_CONST(this)); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the first alpha channel layer.
	/// @return												The first alpha channel layer, or @formatConstant{nullptr} if there is none.
	//----------------------------------------------------------------------------------------
	PaintLayer* GetAlphaFirst() { return C4DOS_Pa->PB_GetAlphaFirst(this); }
	const PaintLayer* GetAlphaFirst() const { return C4DOS_Pa->PB_GetAlphaFirst(MAXON_REMOVE_CONST(this)); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the last alpha channel layer.
	/// @return												The last alpha channel layer, or @formatConstant{nullptr} if there is none.
	//----------------------------------------------------------------------------------------
	PaintLayer* GetAlphaLast() { return C4DOS_Pa->PB_GetAlphaLast(this); }
	const PaintLayer* GetAlphaLast() const { return C4DOS_Pa->PB_GetAlphaLast(MAXON_REMOVE_CONST(this)); }
	...
	//----------------------------------------------------------------------------------------
	PaintLayerBmp* AddAlphaChannel(Int32 bitdepth, PaintLayer* prev = nullptr, Bool undo = true, Bool activate = true) { return C4DOS_Pa->PB_AddAlphaChannel(this, bitdepth, prev, undo, activate); }

	//----------------------------------------------------------------------------------------
	/// Checks if the alpha mask can be applied.
	/// @return												@trueIfOtherwiseFalse{the alpha mask can be applied}
	//----------------------------------------------------------------------------------------
	Bool AskApplyAlphaMask() const { return C4DOS_Pa->PB_AskApplyAlphaMask(this); }
	...
	//----------------------------------------------------------------------------------------
	void ApplyAlphaMask(Int32 x, Int32 y, Int32 cnt, PIX* bits, COLORMODE mode, Bool inverted, Int32 flags) const { C4DOS_Pa->PB_ApplyAlphaMask(this, x, y, cnt, bits, mode, inverted, flags); }
	...
	//----------------------------------------------------------------------------------------
	PaintLayerMask* FindSelectionMask(PaintBitmap** toplevel, Int32* bitdepth) { return C4DOS_Pa->PB_FindSelectionMask(this, toplevel, bitdepth); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the color mode of the paint bitmap.
	/// @return												The color mode: @enumerateEnum{COLORMODE}
	//----------------------------------------------------------------------------------------
	COLORMODE GetColorMode() const { return C4DOS_Pa->PB_GetColorMode(this); };
	...
	//----------------------------------------------------------------------------------------
	UInt32 GetDirty(DIRTYFLAGS flags) const { return C4DOS_Pa->PB_GetDirty(this, flags); }
	...
	//----------------------------------------------------------------------------------------
	void UpdateRefresh(Int32 xmin, Int32 ymin, Int32 xmax, Int32 ymax, UInt32 flags) { C4DOS_Pa->PB_UpdateRefresh(this, xmin, ymin, xmax, ymax, flags); }
	...
	//----------------------------------------------------------------------------------------
	void UpdateRefreshAll(UInt32 flags, Bool reallyall) { C4DOS_Pa->PB_UpdateRefreshAll(this, flags, reallyall); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool ReCalc(BaseThread* thread, Int32 x1, Int32 y1, Int32 x2, Int32 y2, BaseBitmap* bmp, Int32 flags, UInt32 showbit) { return C4DOS_Pa->PB_ReCalc(this, thread, x1, y1, x2, y2, bmp, flags, showbit); }
	/// @addtogroup RECALC
	/// @ingroup group_enumeration
	/// @{
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	static Bool ConvertBits(Int32 cnt, const PIX* src, Int32 srcinc, COLORMODE srcmode/* MODE_xxx */, PIX* dst, Int32 dstinc, COLORMODE dstmode/* MODE_xxx */, Int32 dithery, Int32 ditherx) { return C4DOS_Pa->PB_ConvertBits(cnt, src, srcinc, srcmode, dst, dstinc, dstmode, dithery, ditherx); }
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	Bool GetShowBit(Bool hierarchy, UInt32 bit) { return C4DOS_Pa->PL_GetShowBit(this, hierarchy, bit); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool SetShowBit(Bool onoff, UInt32 bit) { return C4DOS_Pa->PL_SetShowBit(this, onoff, bit); }
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	Bool GetEditMaskMode() { return C4DOS_Pa->PLM_GetEditMaskMode(this); }
};
...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE PaintLayerBmp : public PaintLayer
{
private:
	PaintLayerBmp();
	~PaintLayerBmp();
...
public:
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool ImportFromBaseBitmap(BaseBitmap* bmp, Bool usealpha) { return C4DOS_Pa->PLB_ImportFromBaseBitmap(this, bmp, usealpha); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool ImportFromBaseBitmapAlpha(BaseBitmap* bmp, BaseBitmap* channel) { return C4DOS_Pa->PLB_ImportFromBaseBitmapAlpha(this, bmp, channel); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool GetPixelCnt(Int32 x, Int32 y, Int32 cnt, PIX* dst, COLORMODE dstmode, PIXELCNT flags) { return C4DOS_Pa->PLB_GetPixelCnt(this, x, y, cnt, dst, dstmode, flags); }
	...
	/// @param[in] flags							The flags: @enumerateEnum{PIXELCNT}
	//----------------------------------------------------------------------------------------
	void SetPixelCnt(Int32 x, Int32 y, Int32 cnt, const PIX* src, Int32 incsrc, COLORMODE srcmode, PIXELCNT flags) { C4DOS_Pa->PLB_SetPixelCnt(this, x, y, cnt, src, incsrc, srcmode, flags); }
	...
	//----------------------------------------------------------------------------------------
	void GetBoundingBox(Int32& x1, Int32& y1, Int32& x2, Int32& y2, Bool hasselectionpixels = false) const { C4DOS_Pa->PLB_GetBoundingBox(this, x1, y1, x2, y2, hasselectionpixels); }
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	PaintLayer* GetFirstLayer() { return C4DOS_Pa->PB_GetLayerDownFirst(this); }
	const PaintLayer* GetFirstLayer() const { return C4DOS_Pa->PB_GetLayerDownFirst(MAXON_REMOVE_CONST(this)); }
	...
	//----------------------------------------------------------------------------------------
	PaintLayer* GetLastLayer() { return C4DOS_Pa->PB_GetLayerDownLast(this); }
	const PaintLayer* GetLastLayer() const { return C4DOS_Pa->PB_GetLayerDownLast(MAXON_REMOVE_CONST(this)); }
	...
	//----------------------------------------------------------------------------------------
	PaintLayerBmp* AddLayerBmp(PaintLayer* insertafter, PaintLayer* layerset = nullptr, COLORMODE mode = COLORMODE::ARGB, Bool useundo = true, Bool activate = true) { return C4DOS_Pa->PT_AddLayerBmp(this, insertafter, layerset, mode, useundo, activate); }
	...
	//----------------------------------------------------------------------------------------
	PaintLayerFolder* AddLayerFolder(PaintLayer* insertafter, PaintLayer* insertunder, Bool useundo = true, Bool activate = true) { return C4DOS_Pa->PT_AddLayerFolder(this, insertafter, insertunder, useundo, activate); }
	...
	//----------------------------------------------------------------------------------------
	void SetActiveLayer(PaintLayer* layer, Bool activatetexture, Bool show = true) { C4DOS_Pa->PT_SetActiveLayer(this, layer, activatetexture, show); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the selected layer.
	/// @return												The selected layer, or @formatConstant{nullptr} if there is none.
	//----------------------------------------------------------------------------------------
	PaintLayer* GetActive() { return C4DOS_Pa->PT_GetActive(this); }
	...
	//----------------------------------------------------------------------------------------
	void GetLinkLayers(AtomArray& layers, Bool addfolders) { C4DOS_Pa->PT_GetLinkLayers(this, layers, addfolders); }
	...
	//----------------------------------------------------------------------------------------
	void SetColorMode(COLORMODE newcolormode, Bool doundo) { C4DOS_Pa->PT_SetColorMode(this, newcolormode, doundo); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the filename of the paint texture.
	/// @return												The filename.
	//----------------------------------------------------------------------------------------
	const Filename GetFilename() const { return C4DOS_Pa->PT_GetFilename(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	static PaintTexture* GetTexture(const String &name, const Filename &filename, Bool create) { return C4DOS_Pa->PT_GetTexture(name, filename, create); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	BaseBitmap* GetVisibleBitmap(Bool recalc = true) { return C4DOS_Pa->PT_GetVisibleBitmap(this, recalc); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	BaseBitmap* GetSelectionMaskTexture(UInt64 width, UInt64 height, Int32& dirtyCount) { return C4DOS_Pa->PT_GetSelectionMaskTexture(this, width, height, dirtyCount); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	static Bool GetProjectionPaintTextures(const ViewWindow* viewWindow, PaintTexture*& paintTexture) { return C4DOS_Pa->PT_GetProjectionPaintTextures(viewWindow, paintTexture); }
	...
	//----------------------------------------------------------------------------------------
	static GeListHead* GetPaintTextureHead() { return C4DOS_Pa->PT_GetPaintTextureHead(); }
	...
	//----------------------------------------------------------------------------------------
	static inline PaintTexture* GetPaintTextureOfBaseChannel(BaseDocument* doc, BaseChannel* bc) { return C4DOS_Pa->GetPaintTextureOfBaseChannel(doc, bc); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	static Bool SetSelected_Texture(PaintBitmap* bmp, PaintMaterial* preferred) { return C4DOS_Pa->PT_SetSelected_Texture(bmp, preferred); }

	//----------------------------------------------------------------------------------------
	/// Retrieves the selected paint texture.
	/// @return												The selected paint texture, or @formatConstant{nullptr} if no paint is selected.
	//----------------------------------------------------------------------------------------
	static PaintTexture* GetSelectedTexture() { return C4DOS_Pa->PT_GetSelectedTexture(); }
	...
	//----------------------------------------------------------------------------------------
	static PaintTexture* GetSelectedTexturePP(PaintMaterial** ppmat = nullptr) { return C4DOS_Pa->PT_GetSelectedTexturePP(ppmat); }

	/// Index of the image color profile.
	static constexpr Int32 COLORPROFILE_INDEX_IMAGE						= 0;
	/// Index of the renderspace color profile.
	static constexpr Int32 COLORPROFILE_INDEX_RENDERSPACE			= 1;
	/// Index of the view transform.
	static constexpr Int32 COLORPROFILE_INDEX_VIEW_TRANSFORM	= 2;
	/// Index of the displayspace color profile.
	static constexpr Int32 COLORPROFILE_INDEX_DISPLAYSPACE		= 3;

	//----------------------------------------------------------------------------------------
	/// Sets the color profile for the paint texture.
	/// @since R17.032
	/// @param[in] profile						The color profile to set.
	/// @param[in] index							Index of the colorprofile to set (COLORPROFILE_INDEX_*).
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	Bool SetColorProfile(const ColorProfile& profile, Int32 index = COLORPROFILE_INDEX_IMAGE) { return C4DOS_Pa->PT_SetColorProfile(this, profile, index); }

	//----------------------------------------------------------------------------------------
	/// Gets the color profile for the paint texture.
	/// @since R17.032
	/// @param[in] index							Index of the colorprofile to retrieve (COLORPROFILE_INDEX_*).
	/// @return												The color profile.
	//----------------------------------------------------------------------------------------
	const ColorProfile& GetColorProfile(Int32 index = COLORPROFILE_INDEX_IMAGE) const { return C4DOS_Pa->PT_GetColorProfile(this, index); }
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	void EnableMaterial(BaseDocument* doc, Bool on, Bool suppressevent = false, Bool domaterialundo = true) { C4DOS_Pa->PM_EnableMaterial(this, doc, on, suppressevent, domaterialundo); }
	...
	//----------------------------------------------------------------------------------------
	static PaintMaterial* GetActivePaintMaterial(BaseDocument* doc = nullptr, BaseMaterial** mat = nullptr) { return C4DOS_Pa->PM_GetActivePaintMaterial(doc, mat); }
	...
	//----------------------------------------------------------------------------------------
	static PaintMaterial* GetPaintMaterialFromTexture(PaintTexture* tex, Bool onlyeditable) { return C4DOS_Pa->PM_GetPaintMaterialFromTexture(tex, onlyeditable); }
	...
	/// @return												@trueIfOtherwiseFalse{successful}
	//----------------------------------------------------------------------------------------
	static Bool UnloadPaintMaterial(BaseDocument* doc, BaseMaterial* material, Bool forcesave) { return C4DOS_Pa->PM_UnloadPaintMaterial(doc, material, forcesave); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	static Bool SaveDocument(BaseDocument* doc, Bool& cancel) { return C4DOS_Pa->PM_SaveDocument(doc, cancel); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	static Bool AskNeedToSave(BaseDocument* doc) { return C4DOS_Pa->PM_AskNeedToSave(doc); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	BaseMaterial* GetBaseMaterial(BaseDocument* doc) { return C4DOS_Pa->PM_GetBaseMaterial(this, doc); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	static void GetSelectionMaskState(Bool& hasSelectionMask, Bool& isEditMaskActive) { return C4DOS_Pa->PM_GetSelectionMaskState(hasSelectionMask, isEditMaskActive); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	//----------------------------------------------------------------------------------------
	static void GetMaterialChannelDisplay(BaseDocument* doc, Bool& isSingleDisplayActive, Int32& materialChannel) { return C4DOS_Pa->PM_GetMaterialChannelDisplay(doc, isSingleDisplayActive, materialChannel); }
};
...
{
	PaintBitmap* hoveredBitmap = nullptr;
	Bool projectionPaint = false;
	Int32 bitmapWidth = 0, bitmapHeight = 0;
	Int32 mousePosX = 0, mousePosY = 0;
	Int32 brushWidth = 0, brushHeight = 0;
	Float projectionPaintFreezeFactor = 1.0;
};
