...
const BaseVideoPost* RenderData::GetFirstVideoPost() const
{
	return C4DOS_Bd->RdGetFirstVideoPostC(this);
}
...
void RenderData::GetResolution(Float& width, Float& height, Float& pixelAspect, Float& filmAspect)
{
	C4DOS_Bd->RdGetResolution(this, width, height, pixelAspect, filmAspect);
}

void RenderData::SetResolution(Float width, Float height, Float pixelAspect)
{
	C4DOS_Bd->RdSetResolution(this, width, height, pixelAspect);
}
...
BaseObject* BaseDocument::GetSceneCamera() const
{
	return C4DOS_Bd->GetSceneCamera(this);
}

Bool BaseDocument::SetSceneCamera(BaseObject* camera)
{
	return C4DOS_Bd->SetSceneCamera(this, camera);
}
...
BaseDocument* LoadDocument(const Filename& name, SCENEFILTER loadflags, BaseThread* thread, maxon::String* errorString, const ProgressDelegate& progressDelegate)
{
	...
	{
		url.SetData(maxon::ConstDataPtr(maxon::Id("progress")), maxon::Data(progressDelegate)) iferr_ignore("progress information missing");
	}
...
BaseDraw* BaseDocument::GetTexBaseDraw(Int32 num) const
{
	return C4DOS_Bd->GetTexBaseDraw(this, num);
}

Int32 BaseDocument::GetTexBaseDrawCount() const
{
	return C4DOS_Bd->GetTexBaseDrawCount(this);
}

BaseDraw* BaseDocument::GetActiveTexBaseDraw() const
{
	return C4DOS_Bd->GetActiveTexBaseDraw(this);
}
...
maxon::Result<const maxon::UpdatableAssetRepositoryRef&> BaseDocument::GetSceneRepository() const
{
	return MAXON_REMOVE_CONST(this)->GetSceneRepository(false);
}
...
{
	return BdCall(GetThumbnailColorConverter)();
}
...
{
	return BdCall(GetActiveUVWObjects)(selection, mode);
}
