...
#ifdef __API_INTERN__
	Int32	hitid = C4DOS_Bo->GetHighlightHandle(op, bd);
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (i = 0; i < cnt; i++)
		{
			...
			if (info.type == HANDLECONSTRAINTTYPE::INVALID)
				continue;
			bd->DrawHandle(info.position, DRAWHANDLE::VERYBIG, 0);
...
Int32 ObjectData::GetHandleCount(const BaseObject* op) const
{
	return 0;
}
																															 ...
																															 ACCESSED_OBJECTS_MASK modifyWrite, ACCESSED_OBJECTS_MASK dirtyRead, ACCESSED_OBJECTS_MASK modifyRead)
{
	...
	switch (method)
	{
		case METHOD_ID::CHECK_DIRTY:
			if (falloff)
			{
				// The falloff's data is part of deformer's BaseContainer, so make sure the container is readable.
				access.EnsureReadable(deformer, ACCESSED_OBJECTS_MASK::DATA | ACCESSED_OBJECTS_MASK::FALLOFF) iferr_return;
		...
		case METHOD_ID::MODIFY_OBJECT:
			if (falloff)
			{
				...
				if (falloff->HasContent())
				{
					falloff->GetAccessedObjects(deformer, METHOD_ID::SAMPLE, access) yield_return;
				}
			}
...
{
	if (!RegisterObjectPlugin(id, str, info, g, description, icon, disklevel))
		return false;

	if (!C4DOS_Bo->RegisterObjectTypeToCategory(id, category))
		return false;
