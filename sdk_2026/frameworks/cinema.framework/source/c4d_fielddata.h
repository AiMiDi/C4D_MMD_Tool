...
#endif
#include "maxon/algorithms_base.h"
...
static const Int32 FLtrack = 1062168;						///< Track FieldLayer
...
struct FieldOutput
{
	...
	//----------------------------------------------------------------------------------------
	maxon::Result<FieldOutputBlock> GetSubBlock(Int offset, Int blockSize = FIELD_EXECUTION_BLOCK_SIZE);

	maxon::Result<ConstFieldOutputBlock> GetSubBlock(Int offset, Int blockSize = FIELD_EXECUTION_BLOCK_SIZE) const;
...
template <Bool CONSTBLOCK> struct FieldOutputBlockTemplate
{
	...
	/// @return												The sub block.
	//----------------------------------------------------------------------------------------
	maxon::Result<FieldOutputBlockTemplate> GetSubBlock(Int offset, Int blockSize = FIELD_EXECUTION_BLOCK_SIZE);
	...
	{
		return *reinterpret_cast<const typename std::conditional<CONSTBLOCK, maxon::DeleteReturnType01, ConstFieldOutputBlock>::type*>(this);
	}

private:
	friend FieldOutputBlock FieldOutput::GetBlock();
	friend ConstFieldOutputBlock FieldOutput::GetBlock() const;
	friend maxon::Result<FieldOutputBlock> FieldOutput::GetSubBlock(Int offset, Int blockSize);
	friend maxon::Result<ConstFieldOutputBlock> FieldOutput::GetSubBlock(Int offset, Int blockSize) const;
...
public:
	...
	maxon::Block<BFloat>	_value;					/// The weight value at this point in space
	maxon::Block<BFloat>	_alpha;					/// The alpha value for color and direction at this point in space, only available when color or direction are allocated.
	maxon::Block<BVector>	_color;					/// The color at this point in space
	maxon::Block<BVector>	_direction;			/// The slope at this point in space
	maxon::Block<BVector>	_rotation;			/// The rotational velocity (axle + magnitude)
	maxon::Block<BVector>	_pivot;					/// The rotational pivot point in space
	maxon::Block<BBool>		_deactivated;		/// The deactivated state for this point (i.e. cut from interior shape will be true) this skips remapping
...
public:
	...
	FieldInput(const Vector* pos, const Vector* dir, const Vector* uvw, Int allocatedCount, Matrix transform = Matrix(), Int fullCount = 0)
		: _position(pos, allocatedCount), _direction(dir, dir ? allocatedCount : 0), _uvw(uvw, uvw ? allocatedCount : 0),
		_blockCount(allocatedCount), _fullArraySize(Max(allocatedCount, fullCount)), _transform(transform), _allocatedArraySize(allocatedCount)
	{
		DebugAssert(IsValid());
	}
...
public:
	...
	maxon::Block<const UInt32> _uniqueId;		///< Just a PLACEHOLDER for now - The (optional) uniqueIds used for id look ups for e.g. the formula field or for a memory id for the decay/delay field, if this is a sub-block then element 0 is at _blockOffset within the larger array, caller owns this array, size must match or be greater than count value.
...
{
	FieldLayerLink() { }
	FieldLayerLink(const BaseList2D* op) : _object(op) { }
	FieldLayerLink(const BaseList2D* op, const GeData& data) : _object(op), _secondaryData(data) { }
	FieldLayerLink(const BaseList2D* op, GeData&& data) : _object(op), _secondaryData(std::move(data)) { }

	const BaseList2D* _object = nullptr;	///< The linked object
...
public:
	...
	//----------------------------------------------------------------------------------------
	static maxon::Result<FieldInfo> Create(const BaseList2D* caller, const FieldInput& inputs, FIELDSAMPLE_FLAG callingFlags = FIELDSAMPLE_FLAG::VALUE, Int threadIndex = 0, Int threadCount = 0);
	...
	/// @return												A maxon::Result containing the resulting FieldInfo.
	//----------------------------------------------------------------------------------------
	static maxon::Result<FieldInfo> Create(const BaseList2D* caller, const BaseDocument* doc, FIELDSAMPLE_FLAG callingFlags, const FieldInput& inputs);
	...
	/// @return												A maxon::Result containing the resulting FieldInfo.
	//----------------------------------------------------------------------------------------
	static maxon::Result<FieldInfo> Create(const FieldCallerStack& caller, const BaseDocument* doc, FIELDSAMPLE_FLAG callingFlags, const FieldInput& inputs);
	...
	//----------------------------------------------------------------------------------------
	static FieldInfo Create(FIELDSAMPLE_FLAG callingFlags, BaseThread* thread, const BaseDocument* document, Int currentThreadIndex, Int threadCount, const FieldInput& inputs = FieldInput())
	{
		FieldInfo newInfo;
	...
	/// @return												A maxon::Result containing the resulting FieldInfo.
	//----------------------------------------------------------------------------------------
	static maxon::Result<FieldInfo> Create(FIELDSAMPLE_FLAG callingFlags, BaseThread* thread, const BaseDocument* doc, Int currentThreadIndex, Int threadCount, const FieldInput& inputs, const FieldCallerStack& caller)
	{
		iferr_scope;
...
/// @return												A maxon::Result containing the resulting FieldInfo.
//----------------------------------------------------------------------------------------
	static maxon::Result<FieldInfo> Create(FIELDSAMPLE_FLAG callingFlags, BaseThread* thread, const BaseDocument* doc, Int currentThreadIndex, Int threadCount, const FieldInput& inputs, const std::initializer_list<const BaseList2D*>& callers)
	{
		iferr_scope;
...
public:
	...
	const BaseDocument*		 _doc = nullptr;									///< The calling document @required
...
class CINEWARE_SINGLEINHERITANCE FieldObject : public BaseObject
{
...
public:
	...
	{
		return static_cast<FieldObject*>(C4DOS_Bo->Alloc(type));
	}
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<maxon::GenericData> InitSampling(const FieldInfo& info) const { return BoCall (FiInitSampling)(info); }
	...
	//----------------------------------------------------------------------------------------
	void FreeSampling(const FieldInfo& info, maxon::GenericData& extraData) const { BoCall (FiFreeSampling)(info, extraData); }
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> Sample(const FieldInput& inputs, FieldOutputBlock& outputs, const FieldInfo& info, const maxon::GenericData& extraData, const FIELDOBJECTSAMPLE_FLAG& objectSampleFlags = FIELDOBJECTSAMPLE_FLAG::NONE) const { return BoCall (FiSample)(inputs, outputs, info, extraData, objectSampleFlags); }
	...
	//----------------------------------------------------------------------------------------
	FIELDOBJECT_FLAG GetFieldFlags(const BaseDocument* doc) const { return BoCall (FiGetFlags)(doc); }

	//----------------------------------------------------------------------------------------
	/// Calls the dirtycheck function of this field object.
	/// @param[in] doc								If non-null, the document.
	//----------------------------------------------------------------------------------------
	void CheckDirty(const BaseDocument* doc) { return BoCall(CheckDirty)(doc); }
};

class CINEWARE_SINGLEINHERITANCE FieldLayer : public BaseList2D
{
...
public:
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<maxon::GenericData> InitSampling(const FieldInfo& info) const { return FLCall (InitSampling)(info); }
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> Sample(const FieldInput& inputs, FieldOutputBlock& outputs, const FieldInfo& info, const maxon::GenericData& extraData) const { return FLCall (Sample)(inputs, outputs, info, extraData); }
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> Aggregate(const FieldInput& inputs, const FieldOutputBlock& outputs, const FieldInfo& info, const maxon::GenericData& extraData) const { return FLCall(Aggregate)(inputs, outputs, info, extraData); }

	//----------------------------------------------------------------------------------------
	/// Free's the Field Sampling. Releases any memory allocated during InitSampling.
	/// @param[in] info								A fully populated FieldInfo structure.
	//----------------------------------------------------------------------------------------
	void FreeSampling(const FieldInfo& info, maxon::GenericData& extraData) const { FLCall (FreeSampling)(info, extraData); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsEqual(const FieldLayer* comp) const { return FLCall (IsEqual)(comp); }
	...
	//----------------------------------------------------------------------------------------
	FieldLayerLink GetLinkedObject(const BaseDocument* doc) const { return FLCall (GetLinkedObject)(doc); }
	...
	{
		return C4DOS_FL->Alloc(type);
	}

	/// @copydoc Alloc
	static FieldLayer* Alloc(const maxon::SourceLocation&, Int32 type) { return Alloc(type); }
	...
	maxon::Result<Bool> GetAccessedObjectsForSampling(const BaseList2D* caller, AccessedObjectsCallback& access) const { return FLCall (GetAccessedObjectsForSampling)(caller, access); }
};
