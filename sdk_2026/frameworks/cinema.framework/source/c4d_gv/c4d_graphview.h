...
class GvPortList
{
	...
	//----------------------------------------------------------------------------------------
		static GvPortList* Alloc() { return (GvPortList*)C4DOS_Gv->portlist->Alloc(); }

	/// @copydoc Alloc
	static GvPortList* Alloc(const maxon::SourceLocation&) { return Alloc(); }

	//----------------------------------------------------------------------------------------
	/// @destructsAlloc{port lists}
	/// @param[in,out] list						@theToDestruct{port lists}
	//----------------------------------------------------------------------------------------
		static void Free(GvPortList* &list) { C4DOS_Gv->portlist->Free(reinterpret_cast<iGvPortList*&>(list)); }
	...
	/// @{
		...
		//----------------------------------------------------------------------------------------
		Int32 GetCount() const { return (reinterpret_cast<const iGvPortList*>(this)->*C4DOS_Gv->portlist->GetCount)(); }
		...
		//----------------------------------------------------------------------------------------
		Bool Append(GvPortListEntry *e) { return (reinterpret_cast<iGvPortList*>(this)->*C4DOS_Gv->portlist->Append)(e); }
		...
		//----------------------------------------------------------------------------------------
		Bool Remove(GvPortListEntry *e) { return (reinterpret_cast<iGvPortList*>(this)->*C4DOS_Gv->portlist->Remove)(e); }
		...
		//----------------------------------------------------------------------------------------
		GvPortListEntry* GetIndex(Int32 i) const { return (reinterpret_cast<const iGvPortList*>(this)->*C4DOS_Gv->portlist->GetIndex)(i); }

		//----------------------------------------------------------------------------------------
		/// Clears the list.
		//----------------------------------------------------------------------------------------
		void FlushAll() { (reinterpret_cast<iGvPortList*>(this)->*C4DOS_Gv->portlist->FlushAll)(); }
...
class GvInit
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		void SetError(GvCalcError error) { C4DOS_Gv->init->SetError(this, error); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the error flag.
		/// @return												The error flag: @enumerateEnum{GvError}
		//----------------------------------------------------------------------------------------
		GvCalcError GetError() { return C4DOS_Gv->init->GetError(this); }
};
...
class GvValue
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		Bool Calculate(GvNode *node, GvPortIO io, GvRun *r, GvCalc *c, Int32 index = 0, BaseTime *time = nullptr) { return C4DOS_Gv->value->Calculate(this, node, io, r, c, index, time); }
		...
		//----------------------------------------------------------------------------------------
		Bool IsConnected(Int32 index) { return C4DOS_Gv->value->IsConnected(this, index); }

		//----------------------------------------------------------------------------------------
		/// Checks if there are any ports in the value.
		/// @return												@trueIfOtherwiseFalse{if NrOfPorts() > 0}
		//----------------------------------------------------------------------------------------
		Bool IsPort() { return C4DOS_Gv->value->IsPort(this); }

		//----------------------------------------------------------------------------------------
		/// The number of ports in this value.
		/// @return												The port count.
		//----------------------------------------------------------------------------------------
		Int32 NrOfPorts() { return C4DOS_Gv->value->NrOfPorts(this); }
		...
		/// @return												The value ID.
		//----------------------------------------------------------------------------------------
		GvValueID GetValueID() { return C4DOS_Gv->value->GetValueID(this); }
		...
		/// @return												The main ID.
		//----------------------------------------------------------------------------------------
		Int32 GetMainID() { return C4DOS_Gv->value->GetMainID(this); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* GetPort(Int32 index = 0) { return C4DOS_Gv->value->GetPort(this, index); }
};
...
class GvQuery
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		GvCalcFlags GetFlags() { return C4DOS_Gv->query->GetFlags(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the calculation flags.
		/// @param[in] flags							The calculation flags: @enumerateEnum{GvCalcFlags}
		//----------------------------------------------------------------------------------------
		void SetFlags(GvCalcFlags flags) { C4DOS_Gv->query->SetFlags(this, flags); }

		//----------------------------------------------------------------------------------------
		/// Gets the error flag.
		/// @return												The error flag: @enumerateEnum{GvError}
		//----------------------------------------------------------------------------------------
		GvCalcError GetError() { return C4DOS_Gv->query->GetError(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the error flag.
		/// @param[in] error							The error flag: @enumerateEnum{GvError}
		//----------------------------------------------------------------------------------------
		void SetError(GvCalcError error) { C4DOS_Gv->query->SetError(this, error); }
};
...
class GvRun
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		void SetCalculationTable(GvCalcTable *t) { C4DOS_Gv->run->SetCalculationTable(this, t); }
		...
		//----------------------------------------------------------------------------------------
		Bool AddNodeToCalculationTable(GvNode *node, Bool force_add = false) { return C4DOS_Gv->run->AddNodeToCalculationTable(this, node, force_add); }
		...
		//----------------------------------------------------------------------------------------
		Bool CalculateTable(GvCalc *calc) { return C4DOS_Gv->run->CalculateTable(this, calc); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the ID of the calculated CPU.
		/// @return												The CPU ID. Normally @em 0.
		//----------------------------------------------------------------------------------------
		Int32 GetCpuID() { return C4DOS_Gv->run->GetCpuID(this); }

		//----------------------------------------------------------------------------------------
		/// Increments the run ID. Used to check if a port is already calculated.
		//----------------------------------------------------------------------------------------
		void IncrementID() { C4DOS_Gv->run->IncrementID(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the error flag.
		/// @param[in] error							The error flag: @enumerateEnum{GvError}
		//----------------------------------------------------------------------------------------
		void SetError(GvCalcError error) { C4DOS_Gv->run->SetError(this, error); }

		//----------------------------------------------------------------------------------------
		/// Gets the error flag.
		/// @return												The error flag: @enumerateEnum{GvError}
		//----------------------------------------------------------------------------------------
		GvCalcError GetError() { return C4DOS_Gv->run->GetError(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the failure state.
		/// @param[in] state							The failure state: @enumerateEnum{GvCalcState}
		//----------------------------------------------------------------------------------------
		void SetState(GvCalcState state) { C4DOS_Gv->run->SetState(this, state); }

		//----------------------------------------------------------------------------------------
		/// Gets the failure state.
		/// @return												The failure state: @enumerateEnum{GvCalcState}
		//----------------------------------------------------------------------------------------
		GvCalcState GetState() { return C4DOS_Gv->run->GetState(this); }
		...
		//----------------------------------------------------------------------------------------
		Bool IsIterationPath() { return C4DOS_Gv->run->IsIterationPath(this); }
};
...
class GvCalcTable
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		Bool AddNodeToTable(GvRun *run, GvNode *node) { return C4DOS_Gv->table->AddNodeToTable(this, run, node); }

		//----------------------------------------------------------------------------------------
		/// Clears the table without destroying it.
		/// @param[in] run								The run helper. @callerOwnsPointed{run helper}
		//----------------------------------------------------------------------------------------
		void ResetTable(GvRun *run) { C4DOS_Gv->table->ResetTable(this, run); }
		...
		//----------------------------------------------------------------------------------------
		Int32 GetTableCount(GvRun *run) { return C4DOS_Gv->table->GetTableCount(this, run); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetTableNode(GvRun *run, Int32 id) { return C4DOS_Gv->table->GetTableNode(this, run, id); }
};
...
class GvNode : public BaseList2D
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetNext() { return (GvNode*)AtCall(GetNext)(); }
		const GvNode* GetNext() const { return MAXON_REMOVE_CONST(this)->GetNext(); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetPred() { return (GvNode*)AtCall(GetPred)(); }
		const GvNode* GetPred() const { return MAXON_REMOVE_CONST(this)->GetPred(); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetUp() { return (GvNode*)AtCall(GetUp)(); }
		const GvNode* GetUp() const { return MAXON_REMOVE_CONST(this)->GetUp(); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetDown() { return (GvNode*)AtCall(GetDown)(); }
		const GvNode* GetDown() const { return MAXON_REMOVE_CONST(this)->GetDown(); }
		...
		/// Redraws the GV node.
		//----------------------------------------------------------------------------------------
		void Redraw() { C4DOS_Gv->node->Redraw(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the title of the GV node.
		/// @return												The title.
		//----------------------------------------------------------------------------------------
		const String GetTitle() const { return C4DOS_Gv->node->GetTitle(this, 0); }

		//----------------------------------------------------------------------------------------
		/// Sets the title of the GV node.
		/// @param[in] title							The title to set.
		//----------------------------------------------------------------------------------------
		void SetTitle(const maxon::String &title) { C4DOS_Gv->node->SetTitle(this, title, 0); }
		...
		//----------------------------------------------------------------------------------------
		Bool CreateOperator() { return C4DOS_Gv->node->CreateOperator(this, 0); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the node master that the GV node belongs to.
		/// @return												The node master of this node. @cinemaOwnsPointed{node master}
		//----------------------------------------------------------------------------------------
		GvNodeMaster* GetNodeMaster() { return C4DOS_Gv->node->GetNodeMaster(this); }
		const GvNodeMaster* GetNodeMaster() const { return C4DOS_Gv->node->GetNodeMaster(MAXON_REMOVE_CONST(this)); }
		...
		//----------------------------------------------------------------------------------------
		GvOperatorData* GetOperatorData() { return C4DOS_Gv->node->GetOperatorData(this); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the operator ID for the GV node.
		/// @return												The operator ID.
		//----------------------------------------------------------------------------------------
		Int32 GetOperatorID() const { return C4DOS_Gv->node->GetOperatorID(this); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the owner ID of the GV node.
		/// @return												The owner ID. Here are the predefined IDs: @enumerateEnum{ID_GV_OWNER}
		//----------------------------------------------------------------------------------------
		Int32 GetOwnerID() const { return C4DOS_Gv->node->GetOwnerID(this); }
		...
		//----------------------------------------------------------------------------------------
		Bool IsGroupNode() const { return C4DOS_Gv->node->IsGroupNode(this); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* AddPort(GvPortIO io, Int32 id, GvPortFlags flags = GV_PORT_FLAG_IS_VISIBLE, Bool message = false) { return C4DOS_Gv->node->AddPort(this, io, id, flags, message); }
		...
		//----------------------------------------------------------------------------------------
		Bool AddPortIsOK(GvPortIO io, Int32 id) { return C4DOS_Gv->node->AddPortIsOK(this, io, id); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetPortType(GvPort *port, Int32 id) { return C4DOS_Gv->node->SetPortType(this, port, id); }
		...
		//----------------------------------------------------------------------------------------
		Bool ResetPortType(Int32 id) { return C4DOS_Gv->node->ResetPortType(this, id); }
		...
		//----------------------------------------------------------------------------------------
		void RemovePort(GvPort *port, Bool message = true) { C4DOS_Gv->node->RemovePort(this, port, message); }
		...
		//----------------------------------------------------------------------------------------
		Bool RemovePortIsOK(GvPort *port) { return C4DOS_Gv->node->RemovePortIsOK(this, port); }
		...
		//----------------------------------------------------------------------------------------
		void RemoveUnusedPorts(Bool message = true) { C4DOS_Gv->node->RemoveUnusedPorts(this, message); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* AddConnection(GvNode *source_node, GvPort *source_port, GvNode *dest_node, GvPort *dest_port) { return C4DOS_Gv->node->AddConnection(this, source_node, source_port, dest_node, dest_port); }

		//----------------------------------------------------------------------------------------
		/// Removes all connections from all ports of the GV node.
		//----------------------------------------------------------------------------------------
		void RemoveConnections() {	C4DOS_Gv->node->RemoveNodeConnections(this); }
		...
		//----------------------------------------------------------------------------------------
		void GetPortList(GvPortIO port, GvPortList &portlist) const { C4DOS_Gv->node->GetPortList(this, port, portlist, 0); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetPortDescription(GvPortIO port, Int32 id, GvPortDescription *pd) const { return C4DOS_Gv->node->GetPortDescription(this, port, id, pd, 0); }

		//----------------------------------------------------------------------------------------
		/// Gets the number of input ports of the GV node.
		/// @return												The number of input ports.
		//----------------------------------------------------------------------------------------
		Int32 GetInPortCount() const { return C4DOS_Gv->node->GetInPortCount(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the number of output ports of the GV node.
		/// @return												The number of output ports.
		//----------------------------------------------------------------------------------------
		Int32 GetOutPortCount() const { return C4DOS_Gv->node->GetOutPortCount(this); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* GetInPort(Int32 index) { return C4DOS_Gv->node->GetInPort(this, index); }
		const GvPort* GetInPort(Int32 index) const { return C4DOS_Gv->node->GetInPort(MAXON_REMOVE_CONST(this), index); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* GetOutPort(Int32 index) { return C4DOS_Gv->node->GetOutPort(this, index); }
		const GvPort* GetOutPort(Int32 index) const { return C4DOS_Gv->node->GetOutPort(MAXON_REMOVE_CONST(this), index); }
		...
		/// @return												The retrieved input port, or @formatConstant{nullptr} if failed. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* GetInPortFirstMainID(Int32 id) { return C4DOS_Gv->node->GetInPortFirstMainID(this, id); }
		const GvPort* GetInPortFirstMainID(Int32 id) const { return C4DOS_Gv->node->GetInPortFirstMainID(MAXON_REMOVE_CONST(this), id); }
		...
		/// @return												The retrieved output port, or @formatConstant{nullptr} if failed. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* GetOutPortFirstMainID(Int32 id) { return C4DOS_Gv->node->GetOutPortFirstMainID(this, id); }
		const GvPort* GetOutPortFirstMainID(Int32 id) const { return C4DOS_Gv->node->GetOutPortFirstMainID(MAXON_REMOVE_CONST(this), id); }
		...
		/// @return												The retrieved input port, or @formatConstant{nullptr} if failed. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* GetInPortMainID(Int32 id, Int32 &start) { return C4DOS_Gv->node->GetInPortMainID(this, id, start); }
		const GvPort* GetInPortMainID(Int32 id, Int32& start) const { return C4DOS_Gv->node->GetInPortMainID(MAXON_REMOVE_CONST(this), id, start); }
		...
		/// @return												The retrieved output port, or @formatConstant{nullptr} if failed. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* GetOutPortMainID(Int32 id, Int32 &start) { return C4DOS_Gv->node->GetOutPortMainID(this, id, start); }
		const GvPort* GetOutPortMainID(Int32 id, Int32& start) const { return C4DOS_Gv->node->GetOutPortMainID(MAXON_REMOVE_CONST(this), id, start); }
		...
		/// @return												The retrieved input port, or @formatConstant{nullptr} if failed. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* GetInPortSubID(Int32 id) { return C4DOS_Gv->node->GetInPortSubID(this, id); }
		const GvPort* GetInPortSubID(Int32 id) const { return C4DOS_Gv->node->GetInPortSubID(MAXON_REMOVE_CONST(this), id); }
		...
		/// @return												The retrieved output port, or @formatConstant{nullptr} if failed. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* GetOutPortSubID(Int32 id) { return C4DOS_Gv->node->GetOutPortSubID(this, id); }
		const GvPort* GetOutPortSubID(Int32 id) const { return C4DOS_Gv->node->GetOutPortSubID(MAXON_REMOVE_CONST(this), id); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* GetPort(Int32 sub_id) { return C4DOS_Gv->node->GetPort(this, sub_id); }
		const GvPort* GetPort(Int32 sub_id) const { return C4DOS_Gv->node->GetPort(MAXON_REMOVE_CONST(this), sub_id); }
		...
		//----------------------------------------------------------------------------------------
		Int32 GetPortIndex(Int32 sub_id) const { return C4DOS_Gv->node->GetPortIndex(this, sub_id); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* CalculateInPortIndex(Int32 port_index, GvRun *run, GvCalc *calc) { return C4DOS_Gv->node->NodeCalculateInPortIndex(this, port_index, run, calc); }
		...
		/// @return												The calculated port, or @formatConstant{nullptr}. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* CalculateOutPortIndex(Int32 port_index, GvRun *run, GvCalc *calc) { return C4DOS_Gv->node->NodeCalculateOutPortIndex(this, port_index, run, calc); }
		...
		/// @return												The calculated port, or @formatConstant{nullptr}. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* CalculateInPort(GvPort *port, GvRun *run, GvCalc *calc) { return C4DOS_Gv->node->NodeCalculateInPort(this, port, run, calc); }
		...
		/// @return												The calculated port, or @formatConstant{nullptr}. @theOwnsPointed{GV node,port}
		//----------------------------------------------------------------------------------------
		GvPort* CalculateOutPort(GvPort *port, GvRun *run, GvCalc *calc) { return C4DOS_Gv->node->NodeCalculateOutPort(this, port, run, calc); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool SetRecalculate(GvRun *r, Bool force_set = false) { return C4DOS_Gv->node->SetRecalculate(this, r, force_set); }
		...
		//----------------------------------------------------------------------------------------
		BaseContainer* GetOpContainerInstance() { return C4DOS_Gv->node->GetOpContainerInstance(this); }
		const BaseContainer* GetOpContainerInstance() const { return C4DOS_Gv->node->GetOpContainerInstance(MAXON_REMOVE_CONST(this)); }

		//----------------------------------------------------------------------------------------
		/// Retrieves a copy of the internal operator's container.
		/// @return												A copy of the operator's container.
		//----------------------------------------------------------------------------------------
		BaseContainer GetOperatorContainer() const { return C4DOS_Gv->node->GetOperatorContainer(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the internal operator's container.
		/// @param[in] bc									The container to set for the operator.
		//----------------------------------------------------------------------------------------
		void SetOperatorContainer(const BaseContainer &bc) { C4DOS_Gv->node->SetOperatorContainer(this, bc); }
		...
		//----------------------------------------------------------------------------------------
		const String OperatorGetDetailedText() { return C4DOS_Gv->node->OperatorGetDetailedText(this); }
		...
		//----------------------------------------------------------------------------------------
		const String OperatorGetErrorString(Int32 error) { return C4DOS_Gv->node->OperatorGetErrorString(this, error); }
		...
		//----------------------------------------------------------------------------------------
		Bool OperatorSetData(GvDataType type, void *data, GvOpSetDataMode mode) { return C4DOS_Gv->node->OperatorSetData(this, type, data, mode); }
		...
		//----------------------------------------------------------------------------------------
		Bool OperatorIsSetDataAllowed(GvDataType type, void *data, GvOpSetDataMode mode) { return C4DOS_Gv->node->OperatorIsSetDataAllowed(this, type, data, mode); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the operator icon.
		/// @param[in] dat								Filled with the operator icon data.
		//----------------------------------------------------------------------------------------
		void OperatorGetIcon(IconData &dat) { C4DOS_Gv->node->OperatorGetIcon(this, dat); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetSelectState() { return C4DOS_Gv->node->GetSelectState(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the failure state of the GV node.
		/// @return												The failure state.
		//----------------------------------------------------------------------------------------
		Bool GetFailureState() { return C4DOS_Gv->node->GetFailureState(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the disabled state of the GV node.
		/// @return												The disabled state.
		//----------------------------------------------------------------------------------------
		Bool GetDisabledState() { return C4DOS_Gv->node->GetDisabledState(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the open state of the GV node.
		/// @return												The open state.
		//----------------------------------------------------------------------------------------
		Bool GetOpenState() { return C4DOS_Gv->node->GetOpenState(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the open state of the GV node.
		/// @param[in] state							The open state.
		//----------------------------------------------------------------------------------------
		void SetOpenState(Bool state) { C4DOS_Gv->node->SetOpenState(this, state); }

		//----------------------------------------------------------------------------------------
		/// Gets the lock state of the GV node.
		/// @return												The lock state.
		//----------------------------------------------------------------------------------------
		Bool GetLockState() { return C4DOS_Gv->node->GetLockState(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the lock state of the GV node.
		/// @param[in] state							The lock state.
		//----------------------------------------------------------------------------------------
		void SetLockState(Bool state) { C4DOS_Gv->node->SetLockState(this, state); }

		//----------------------------------------------------------------------------------------
		/// Gets the show port name state of the GV node.
		/// @return												The show port name state.
		//----------------------------------------------------------------------------------------
		Bool GetShowPortNamesState() { return C4DOS_Gv->node->GetShowPortNamesState(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the show port name state of the GV node.
		/// @param[in] state							The show port name state.
		//----------------------------------------------------------------------------------------
		void SetShowPortNamesState(Bool state) { C4DOS_Gv->node->SetShowPortNamesState(this, state); }
		...
		//----------------------------------------------------------------------------------------
		GvValue* AllocCalculationHandler(Int32 main_id, GvCalc *calc, GvRun *run, Int32 singleport) { return C4DOS_Gv->node->AllocCalculationHandler(this, main_id, calc, run, singleport); }
		...
		//----------------------------------------------------------------------------------------
		void FreeCalculationHandler(GvValue *&value) { C4DOS_Gv->node->FreeCalculationHandler(this, value); }
		...
		//----------------------------------------------------------------------------------------
		Bool CalculateRawData(GvValueID value_id, const void* const data1, const void* const data2, void *dest, GvValueFlags calculation, Float parm1) { return C4DOS_Gv->node->CalculateRawData(this, value_id, data1, data2, dest, calculation, parm1); }
...
class GvPort
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		void RemoveConnection() { C4DOS_Gv->port->RemoveConnection(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the number of connections, including both the incoming and outgoing connections.
		/// @return												The number of connections.
		//----------------------------------------------------------------------------------------
		Int32 GetNrOfConnections() { return C4DOS_Gv->port->GetNrOfConnections(this); }

		//----------------------------------------------------------------------------------------
		/// Removes all outgoing connections.
		//----------------------------------------------------------------------------------------
		void RemovePortConnections() { C4DOS_Gv->port->RemovePortConnections(this); }

		//----------------------------------------------------------------------------------------
		/// Checks if there is an incoming connection.
		/// @return												@trueIfOtherwiseFalse{there is an incoming connection to the port}
		//----------------------------------------------------------------------------------------
		Bool IsIncomingConnected() const { return C4DOS_Gv->port->IsIncomingConnected(this); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetIncomingDestination(GvNode *&node, GvPort *&port) { return C4DOS_Gv->port->GetIncomingDestination(this, node, port); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetIncomingSource(GvNode *&node, GvPort *&port) { return C4DOS_Gv->port->GetIncomingSource(this, node, port); }
		...
		//----------------------------------------------------------------------------------------
		GvPortIO GetIO() { return C4DOS_Gv->port->GetIO(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the main ID of the port.
		/// @param[in] id									The main ID to set.
		//----------------------------------------------------------------------------------------
		void SetMainID(Int32 id) { C4DOS_Gv->port->SetMainID(this, id); }

		//----------------------------------------------------------------------------------------
		/// Gets the main ID of the port.
		/// @return												The main ID.
		//----------------------------------------------------------------------------------------
		Int32 GetMainID() const { return C4DOS_Gv->port->GetMainID(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the sub ID of the port.
		/// @return												The sub ID.
		//----------------------------------------------------------------------------------------
		Int32 GetSubID() const { return C4DOS_Gv->port->GetSubID(this); }

		//----------------------------------------------------------------------------------------
		/// Sets the user ID of the port.
		/// @param[in] id									The user ID to set.
		//----------------------------------------------------------------------------------------
		void SetUserID(Int32 id) { C4DOS_Gv->port->SetUserID(this, id); }

		//----------------------------------------------------------------------------------------
		/// Gets the user ID of the port.
		/// @return												The user ID.
		//----------------------------------------------------------------------------------------
		Int32 GetUserID() { return C4DOS_Gv->port->GetUserID(this); }
		...
		//----------------------------------------------------------------------------------------
		GvValueID GetValueType() { return C4DOS_Gv->port->GetValueType(this); }
		...
		//----------------------------------------------------------------------------------------
		void SetVisible(Bool v) { C4DOS_Gv->port->SetVisible(this, v); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetVisible() { return C4DOS_Gv->port->GetVisible(this); }
		...
		//----------------------------------------------------------------------------------------
		void SetValid(GvRun *r, Bool v) { C4DOS_Gv->port->SetValid(this, r, v); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetValid(GvRun *r) { return C4DOS_Gv->port->GetValid(this, r); }
		...
		/// @param[in] r									The run helper. @callerOwnsPointed{run helper}
		//----------------------------------------------------------------------------------------
		void SetCalculated(GvRun *r) { C4DOS_Gv->port->SetCalculated(this, r); }
		...
		//----------------------------------------------------------------------------------------
		GvPort* Calculate(GvNode *bn, GvRun *r, GvCalc *c) { return C4DOS_Gv->port->Calculate(this, bn, r, c); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool SetRecalculate(GvRun *r, Bool force_set = false) { return C4DOS_Gv->port->SetRecalculate(this, r, force_set); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetBool(Bool *b, GvRun *r) { return C4DOS_Gv->port->GetBool(this, b, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetInteger(Int32 *i, GvRun *r) { return C4DOS_Gv->port->GetInteger(this, i, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetFloat(Float *f, GvRun *r) { return C4DOS_Gv->port->GetFloat(this, f, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetVector(Vector *v, GvRun *r) { return C4DOS_Gv->port->GetVector(this, v, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetNormal(Vector *n, GvRun *r) { return C4DOS_Gv->port->GetNormal(this, n, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetMatrix(Matrix *m, GvRun *r) { return C4DOS_Gv->port->GetMatrix(this, m, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetTime(BaseTime *t, GvRun *r) { return C4DOS_Gv->port->GetTime(this, t, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetString(String *s, GvRun *r) { return C4DOS_Gv->port->GetString(this, s, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetObject(BaseList2D *&o, GvRun *r) { return C4DOS_Gv->port->GetObject(this, o, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetData(void *d, GvValueID type, GvRun *r) { return C4DOS_Gv->port->GetData(this, d, type, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was retrieved}
		//----------------------------------------------------------------------------------------
		Bool GetDataInstance(const void *&d, GvValueID type, GvRun *r) { return C4DOS_Gv->port->GetDataInstance(this, d, type, r); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetBool(Bool b, GvRun *r) { return C4DOS_Gv->port->SetBool(this, b, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetInteger(Int32 i, GvRun *r) { return C4DOS_Gv->port->SetInteger(this, i, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetFloat(Float f, GvRun *r) { return C4DOS_Gv->port->SetFloat(this, f, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetVector(const Vector &v, GvRun *r) { return C4DOS_Gv->port->SetVector(this, v, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetMatrix(const Matrix &m, GvRun *r) { return C4DOS_Gv->port->SetMatrix(this, m, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetNormal(const Vector &n, GvRun *r) { return C4DOS_Gv->port->SetNormal(this, n, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetTime(const BaseTime &t, GvRun *r) { return C4DOS_Gv->port->SetTime(this, t, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetString(const maxon::String &s, GvRun *r) { return C4DOS_Gv->port->SetString(this, s, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetObject(const BaseList2D* o, GvRun *r) { return C4DOS_Gv->port->SetObject(this, o, r); }
		...
		/// @return												@trueIfOtherwiseFalse{the value was set}
		//----------------------------------------------------------------------------------------
		Bool SetData(const void* d, GvValueID type, GvRun *r) { return C4DOS_Gv->port->SetData(this, d, type, r); }
		...
		//----------------------------------------------------------------------------------------
		const String GetName(const GvNode *node) const { return C4DOS_Gv->port->GetName(this, node); }

		//----------------------------------------------------------------------------------------
		/// Sets the name of the port.
		/// @param[in] name								The new port name.
		//----------------------------------------------------------------------------------------
		void SetName(const maxon::String &name) { C4DOS_Gv->port->SetName(this, name); }
		...
		//----------------------------------------------------------------------------------------
		Bool CopyPortData(GvPort *source, GvRun *r) { return C4DOS_Gv->port->CopyPortData(this, source, r); }
		...
		//----------------------------------------------------------------------------------------
		Bool CopyRawData(void *source, GvRun *r) { return C4DOS_Gv->port->CopyRawData(this, source, r); }
		...
		//----------------------------------------------------------------------------------------
		Bool CalculateRawData(void *data, void *dest, GvRun *r, GvValueFlags calculation, Float parm1 = 0.0_f) { return C4DOS_Gv->port->CalculateRawData(this, data, dest, r, calculation, parm1); }
		...
		/// @return												@trueIfOtherwiseFalse{the result is @ref GV_CALC_ERR_NONE} If there is an error GvRun::SetError() is called.
		//----------------------------------------------------------------------------------------
		Bool CalculateRawRawData(void *data1, void *data2, void *dest, GvRun *r, GvValueFlags calculation, Float parm1 = 0.0_f) { return C4DOS_Gv->port->CalculateRawRawData(this, data1, data2, dest, r, calculation, parm1); }
		...
		/// @return												@trueIfOtherwiseFalse{the result is @ref GV_CALC_ERR_NONE} If there is an error GvRun::SetError() is called.
		//----------------------------------------------------------------------------------------
		Bool CalculatePortData(GvPort *data, void *dest, GvRun *r, GvValueFlags calculation, Float parm1 = 0.0_f) { return C4DOS_Gv->port->CalculatePortData(this, data, dest, r, calculation, parm1); }
		...
		/// @return												@trueIfOtherwiseFalse{the result is @ref GV_CALC_ERR_NONE} If there is an error GvRun::SetError() is called.
		//----------------------------------------------------------------------------------------
		Bool CalculateRawDataRev(void *data, void *dest, GvRun *r, GvValueFlags calculation, Float parm1 = 0.0_f) { return C4DOS_Gv->port->CalculateRawDataRev(this, data, dest, r, calculation, parm1); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetTag(BaseList2D *&t, GvRun *r, Int32 *index = nullptr) { return C4DOS_Gv->port->GetTag(this, t, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetTag(const BaseList2D* const t, GvRun *r, Int32 index = 0) { return C4DOS_Gv->port->SetTag(this, t, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetMaterial(BaseList2D *&m, GvRun *r, Int32 *index = nullptr) { return C4DOS_Gv->port->GetMaterial(this, m, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetMaterial(const BaseList2D* const m, GvRun *r, Int32 index = 0) { return C4DOS_Gv->port->SetMaterial(this, m, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetInstance(BaseList2D *&i, GvRun *r, Int32 *index = nullptr) { return C4DOS_Gv->port->GetInstance(this, i, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetInstance(const BaseList2D* const i, GvRun *r, Int32 index = 0) { return C4DOS_Gv->port->SetInstance(this, i, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetObjectWithIndex(BaseList2D *&o, GvRun *r, Int32 *index = nullptr) { return C4DOS_Gv->port->GetObjectWithIndex(this, o, r, index); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetObjectWithIndex(const BaseList2D* const o, GvRun *r, Int32 index = 0) { return C4DOS_Gv->port->SetObjectWithIndex(this, o, r, index); }
		...
		//----------------------------------------------------------------------------------------
		GvDestination* GetOutgoing(Int32 index) { return C4DOS_Gv->port->GetOutgoing(this, index); }
		...
		/// @since R17.032
		//----------------------------------------------------------------------------------------
		GeData* GetUserData() { return C4DOS_Gv->port->GetUserData(this); }

		//----------------------------------------------------------------------------------------
		/// @markInternal
		/// @since R17.032
		//----------------------------------------------------------------------------------------
		void SetUserData(const GeData& data) { C4DOS_Gv->port->SetUserData(this, data); }
...
class GvNodeMaster : public BaseList2D
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		GvNode* AllocNode(Int32 id) { return C4DOS_Gv->master->AllocNode(this, id); }

		//----------------------------------------------------------------------------------------
		/// Frees GV nodes allocated with AllocNode().
		/// @param[in,out] node						@theToDestruct{GV node}
		//----------------------------------------------------------------------------------------
		void FreeNode(GvNode *&node) { C4DOS_Gv->master->FreeNode(this, node); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* CreateNode(GvNode *parent, Int32 id, GvNode *insert = nullptr, Int32 x = -1, Int32 y = -1) { return C4DOS_Gv->master->CreateNode(this, parent, id, insert, x, y); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetRoot() { return C4DOS_Gv->master->GetRoot(this); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the owner of the node master.
		/// @return												The owner. @cinemaOwnsPointed{owner}
		//----------------------------------------------------------------------------------------
		BaseList2D* GetOwner() { return C4DOS_Gv->master->GetOwner(this); }
		...
		//----------------------------------------------------------------------------------------
		Bool InsertFirst(GvNode* parent, GvNode* node) { return C4DOS_Gv->master->InsertFirst(this, parent, node); }
		...
		/// @return												@trueIfOtherwiseFalse{the node was inserted}
		//----------------------------------------------------------------------------------------
		Bool InsertLast(GvNode* parent, GvNode* node) { return C4DOS_Gv->master->InsertLast(this, parent, node); }
		...
		//----------------------------------------------------------------------------------------
		Bool SetHierarchy(GvNode *insert, GvNode *node, GvInsertMode mode = GV_INSERT_AFTER) { return C4DOS_Gv->master->SetHierarchy(this, insert, node, mode); }
		...
		//----------------------------------------------------------------------------------------
		GvCalcError QueryCalculation(GvQuery *query, BaseThread *thread) { return C4DOS_Gv->master->QueryCalculation(this, query, thread); }
		...
		//----------------------------------------------------------------------------------------
		GvCalcError InitCalculation(GvInit *init, BaseThread *thread) { return C4DOS_Gv->master->InitCalculation(this, init, thread); }
		...
		//----------------------------------------------------------------------------------------
		GvCalcError Calculate(Int32 cpu_id) { return C4DOS_Gv->master->Calculate(this, cpu_id); }
		...
		//----------------------------------------------------------------------------------------
		GvCalcError Recalculate(GvNodeMaster *master, GvNode *node, Int32 cpu_id) { return C4DOS_Gv->master->Recalculate(this, node, cpu_id); }

		//----------------------------------------------------------------------------------------
		/// Frees resources allocated by the GV nodes for calculation.
		//----------------------------------------------------------------------------------------
		void FreeCalculation() { C4DOS_Gv->master->FreeCalculation(this); }
		...
		//----------------------------------------------------------------------------------------
		GvCalcError Execute(BaseThread *thread, GvCalcFlags flags = GV_CALC_FLAG_NONE) { return C4DOS_Gv->master->Execute(this, thread, flags); }
		...
		//----------------------------------------------------------------------------------------
		maxon::Result<Bool> GetBranchInfo(const maxon::ValueReceiver<const BranchInfo&>& info, GETBRANCHINFO flags) const { return C4DOS_Gv->master->GetBranchInfo(this, info); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the current run helper.
		/// @return												The run helper. @callerOwnsPointed{GvRun}
		//----------------------------------------------------------------------------------------
		GvRun* GetRun() { return C4DOS_Gv->master->GetRun(this); }

		//----------------------------------------------------------------------------------------
		/// Retrieves the current calculation helper.
		/// @return												The calculation helper. @callerOwnsPointed{GvRun}
		//----------------------------------------------------------------------------------------
		GvCalc* GetCalc() { return C4DOS_Gv->master->GetCalc(this); }
		...
		//----------------------------------------------------------------------------------------
		Bool IsEnabled() { return C4DOS_Gv->master->IsEnabled(this); }
		...
		//----------------------------------------------------------------------------------------
		GvQuery* AllocQuery() { return C4DOS_Gv->master->AllocQuery(this); }

		//----------------------------------------------------------------------------------------
		/// Frees GV query objects allocated with AllocQuery().
		/// @param[in,out] query					@theToDestruct{query object}
		//----------------------------------------------------------------------------------------
		void FreeQuery(GvQuery *&query) { C4DOS_Gv->master->FreeQuery(this, query); }
		...
		//----------------------------------------------------------------------------------------
		GvInit* AllocInit() { return C4DOS_Gv->master->AllocInit(this); }

		//----------------------------------------------------------------------------------------
		/// Frees GV init objects allocated with AllocInit().
		/// @param[in,out] init						@theToDestruct{GV init object}
		//----------------------------------------------------------------------------------------
		void FreeInit(GvInit *&init) { C4DOS_Gv->master->FreeInit(this, init); }
		...
		//----------------------------------------------------------------------------------------
		GvUserDataID RegisterUserData(void *data = nullptr) { return C4DOS_Gv->master->RegisterUserData(this, data); }
		...
		//----------------------------------------------------------------------------------------
		void SetUserData(GvUserDataID id, void *data) { C4DOS_Gv->master->SetUserData(this, id, data); }
		...
		//----------------------------------------------------------------------------------------
		void* GetUserData(GvUserDataID id) { return C4DOS_Gv->master->GetUserData(this, id); }
		...
		//----------------------------------------------------------------------------------------
		GvCalcTable* AllocCalculationTable(Int32 cpu_count, Bool sort = true, Int32 nr_of_preallocated_entries = 16, Bool iteration = false) { return C4DOS_Gv->master->AllocCalculationTable(this, cpu_count, sort, nr_of_preallocated_entries, iteration); }

		//----------------------------------------------------------------------------------------
		/// Frees calculation tables allocated with AllocCalculationTable().
		/// @param[in] table							@theToDestruct{table}
		//----------------------------------------------------------------------------------------
		void FreeCalculationTable(GvCalcTable *&table) { C4DOS_Gv->master->FreeCalculationTable(this, table); }
		...
		//----------------------------------------------------------------------------------------
		Bool AddToDrawList(GvNode *bn, void **data = nullptr, Int32 data_size = 0) { return C4DOS_Gv->master->AddToDrawList(this, bn, data, data_size); }
		...
		//----------------------------------------------------------------------------------------
		GvCopyBuffer* GetCopyBuffer(GvNode *first = nullptr, Bool copy_selected = true) { return C4DOS_Gv->master->GetCopyBuffer(this, first, copy_selected); }

		//----------------------------------------------------------------------------------------
		/// Frees copy buffers allocated with GetCopyBuffer().
		/// @param[in] buffer							@theToDestruct{buffer}
		//----------------------------------------------------------------------------------------
		void FreeCopyBuffer(GvCopyBuffer *&buffer) { C4DOS_Gv->master->FreeCopyBuffer(this, buffer); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool PasteFromBuffer(GvCopyBuffer &buffer, GvInsertMode mode = GV_INSERT_UNDER, GvNode *dest = nullptr, Int32 x = GV_INVALID_POS_VALUE, Int32 y = GV_INVALID_POS_VALUE, Bool center = false, void *info = nullptr) { return C4DOS_Gv->master->PasteFromBuffer(this, buffer, mode, dest, x, y, center, info); }
		...
		//----------------------------------------------------------------------------------------
		void SetPrefs(const BaseContainer &bc) { C4DOS_Gv->master->MasterSetPrefs(this, bc); }
		...
		//----------------------------------------------------------------------------------------
		void GetPrefs(BaseContainer &bc) { C4DOS_Gv->master->MasterGetPrefs(this, bc); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool AddUndo() { return C4DOS_Gv->master->AddUndo(this); }
...
class GvNodeGUI
{
	...
	public:
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool Attach(GeDialog *dialog, GvNodeMaster *master) { return C4DOS_Gv->gui->GuiAttach(this, dialog, master); }

		//----------------------------------------------------------------------------------------
		/// Detaches the node GUI.
		/// @note Normally you do not need to call this function.
		//----------------------------------------------------------------------------------------
		void Detach() { C4DOS_Gv->gui->GuiDetach(this); }
		...
		//----------------------------------------------------------------------------------------
		void MouseDown(Int32 x, Int32 y, Int32 chn, Int32 qa, const BaseContainer &msg) { C4DOS_Gv->gui->GuiMouseDown(this, x, y, chn, qa, msg); }
		...
		//----------------------------------------------------------------------------------------
		Int32 Message(const BaseContainer &msg, BaseContainer &result) { return C4DOS_Gv->gui->GuiMessage(this, msg, result); }
		...
		//----------------------------------------------------------------------------------------
		Int32 Command(Int32 id) { return C4DOS_Gv->gui->GuiCommand(this, id); }
		...
		//----------------------------------------------------------------------------------------
		GeUserArea* GetUserArea() { return C4DOS_Gv->gui->GuiGetUserArea(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the node master for the node GUI.
		/// @return												The node master. @theOwnsPointed{node GUI,node master}
		//----------------------------------------------------------------------------------------
		GvNodeMaster* GetMaster() { return C4DOS_Gv->gui->GuiGetMaster(this); }

		//----------------------------------------------------------------------------------------
		/// Gets the dialog for the node GUI.
		/// @return												The dialog. @theOwnsPointed{node GUI,dialog}
		//----------------------------------------------------------------------------------------
		GeDialog* GetDialog() { return C4DOS_Gv->gui->GuiGetDialog(this); }
		...
		//----------------------------------------------------------------------------------------
		void SelectAllNodes(GvNode *node, Bool select_state, Bool add_to_selection = false) { C4DOS_Gv->gui->GuiSelectAllNodes(this, node, select_state, add_to_selection); }

		//----------------------------------------------------------------------------------------
		/// Removes all selected nodes.
		/// @param[in] node								The node. @callerOwnsPointed{node}
		//----------------------------------------------------------------------------------------
		void RemoveAllSelectedNodes(GvNode *node) { C4DOS_Gv->gui->GuiRemoveAllSelectedNodes(this, node); }
		...
		//----------------------------------------------------------------------------------------
		void SelectNode(GvNode *node, Bool select_state, Bool add_to_selection = false, Bool send_message = true) { C4DOS_Gv->gui->GuiSelectNode(this, node, select_state, add_to_selection, send_message); }
		...
		//----------------------------------------------------------------------------------------
		void DisableSelected(GvNode *node, Bool disable_state) { C4DOS_Gv->gui->GuiDisableSelected(this, node, disable_state); }
		...
		//----------------------------------------------------------------------------------------
		void SetFocus(GvNode *node, Bool activate) { C4DOS_Gv->gui->GuiSetFocus(this, node, activate); }
		...
		//----------------------------------------------------------------------------------------
		GvNode* GetNodeGlobal(Int32 x, Int32 y) { return C4DOS_Gv->gui->GuiGetNodeGlobal(this, x, y); }
		...
		/// @return												The found node, or @formatConstant{nullptr} if there was no match. @theOwnsPointed{node GUI,node}
		//----------------------------------------------------------------------------------------
		GvNode* GetNodeLocal(GvNode *node, Int32 x, Int32 y) { return C4DOS_Gv->gui->GuiGetNodeLocal(this, node, x, y); }
		...
		//----------------------------------------------------------------------------------------
		Bool IsInNodeBody(GvNode *node, Int32 x, Int32 y) { return C4DOS_Gv->gui->GuiIsInNodeBody(this, node, x, y); }
		...
		//----------------------------------------------------------------------------------------
		Bool IsInNodeHead(GvNode *node, Int32 x, Int32 y) { return C4DOS_Gv->gui->GuiIsInNodeHead(this, node, x, y); }
		...
		//----------------------------------------------------------------------------------------
		void SetPrefs(const BaseContainer &bc) { C4DOS_Gv->gui->GuiSetPrefs(this, bc); }
		...
		//----------------------------------------------------------------------------------------
		void GetPrefs(BaseContainer &bc) { C4DOS_Gv->gui->GuiGetPrefs(this, bc); }
		...
		//----------------------------------------------------------------------------------------
		void SetNodePosGlobal(GvNode *node, Int32 x, Int32 y, Bool center = false) { C4DOS_Gv->gui->GuiSetNodePosGlobal(this, node, x, y, center); }
		...
		/// @param[in] y									The Y coordinate.
		//----------------------------------------------------------------------------------------
		void SetNodePos(GvNode *node, Int32 x, Int32 y) { C4DOS_Gv->gui->GuiSetNodePos(this, node, x, y); }
		...
		//----------------------------------------------------------------------------------------
		void SetNodeSize(GvNode *node, Int32 width, Int32 height) { C4DOS_Gv->gui->GuiSetNodeSize(this, node, width, height); }

		//----------------------------------------------------------------------------------------
		/// Optimizes the size of @formatParam{node}.
		/// @param[in] node								The node. @callerOwnsPointed{node}
		//----------------------------------------------------------------------------------------
		void OptimizeNode(GvNode *node) { C4DOS_Gv->gui->GuiOptimizeNode(this, node); }
		...
		/// @param[in] node								The node. @callerOwnsPointed{node}
		//----------------------------------------------------------------------------------------
		void ShowAllNodes(GvNode *node) { C4DOS_Gv->gui->GuiShowAllNodes(this, node); }

		//----------------------------------------------------------------------------------------
		/// Centers nodes.
		/// @param[in] node								The node. @callerOwnsPointed{node}
		//----------------------------------------------------------------------------------------
		void CenterNodes(GvNode *node) { C4DOS_Gv->gui->GuiCenterNodes(this, node); }

		//----------------------------------------------------------------------------------------
		/// Aligns all nodes to the upper left.
		/// @param[in] node								The node. @callerOwnsPointed{node}
		//----------------------------------------------------------------------------------------
		void AlignNodesToUpperLeft(GvNode *node) { C4DOS_Gv->gui->GuiAlignNodesToUpperLeft(this, node); }
		...
		//----------------------------------------------------------------------------------------
		void GetZoom(GvNode *node, Float &zoom) { C4DOS_Gv->gui->GuiGetZoom(this, node, zoom); }

		//----------------------------------------------------------------------------------------
		/// Initializes the node GUI shapes.
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool InitShapes() { return C4DOS_Gv->gui->GuiInitShapes(this); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool NodeContextMenu(GvNode *node) { return C4DOS_Gv->gui->GuiNodeContextMenu(this, node); }
...
class GvWorld
{
	...
	public:
		...
		//----------------------------------------------------------------------------------------
		GvNodeMaster* AllocNodeMaster(BaseList2D *object, Bool add_to_list = true, Bool send_messages = true) { return C4DOS_Gv->world->AllocNodeMaster(this, object, add_to_list, send_messages); }

		//----------------------------------------------------------------------------------------
		/// Frees node masters allocated with AllocNodeMaster().
		/// @param[in,out] master					@theToDestruct{node master}
		//----------------------------------------------------------------------------------------
		void FreeNodeMaster(GvNodeMaster *&master) { C4DOS_Gv->world->FreeNodeMaster(this, master); }
		...
		//----------------------------------------------------------------------------------------
		GvNodeGUI* AllocNodeGUI(GvShape *shape, GvShape *group, Int32 user_area_id) { return C4DOS_Gv->world->AllocNodeGUI(this, shape, group, user_area_id); }

		//----------------------------------------------------------------------------------------
		/// Frees node GUIs allocated with AllocNodeGUI().
		/// @param[in,out] gui						@theToDestruct{node GUI}
		//----------------------------------------------------------------------------------------
		void FreeNodeGUI(GvNodeGUI *&gui) { C4DOS_Gv->world->FreeNodeGUI(this, gui); }
		...
		//----------------------------------------------------------------------------------------
		GvShape* AllocShape() { return C4DOS_Gv->world->AllocShape(this); }
		...
		//----------------------------------------------------------------------------------------
		GvShape* AllocGroupShape() { return C4DOS_Gv->world->AllocGroupShape(this); }

		//----------------------------------------------------------------------------------------
		/// Frees shapes allocated with AllocShape() or AllocGroupShape().
		/// @param[in,out] shape					@theToDestruct{shape}
		//----------------------------------------------------------------------------------------
		void FreeShape(GvShape *&shape) { C4DOS_Gv->world->FreeShape(this, shape); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool RegisterHook(const GvHook &hook, void *user) { return C4DOS_Gv->world->RegisterHook(this, hook, user); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool AttachHook(Int32 hook_id, GvHookCallback callback) { return C4DOS_Gv->world->AttachHook(this, hook_id, callback); }
		...
		/// @param[in] hook_id						The hook ID.
		//----------------------------------------------------------------------------------------
		void DetachHook(Int32 hook_id) { C4DOS_Gv->world->DetachHook(this, hook_id); }
		...
		//----------------------------------------------------------------------------------------
		BaseList2D* GetHookInstance(BaseDocument *doc, Int32 hook_id) { return C4DOS_Gv->world->GetHookInstance(this, doc, hook_id); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool SendHookMessage(BaseDocument *doc, GvNodeMaster *master, GvMessHook &data, Int32 owner_id) { return C4DOS_Gv->world->SendHookMessage(this, doc, master, data, owner_id); }
		...
		/// @return												@trueIfOtherwiseFalse{successful}
		//----------------------------------------------------------------------------------------
		Bool SendOperatorMessage(BaseDocument *doc, Int32 message_id, void *data) { return C4DOS_Gv->world->SendOperatorMessage(this, doc, message_id, data); }
		...
		//----------------------------------------------------------------------------------------
		Bool OpenDialog(Int32 id, GvNodeMaster *master) { return C4DOS_Gv->world->OpenDialog(this, id, master); }

		//----------------------------------------------------------------------------------------
		/// Closes a dialog opened with OpenDialog().
		/// @param[in] id									The plugin ID that the dialog was opened with.
		//----------------------------------------------------------------------------------------
		void CloseDialog(Int32 id) { C4DOS_Gv->world->CloseDialog(this, id); }
		...
		//----------------------------------------------------------------------------------------
		void RedrawAll() { C4DOS_Gv->world->RedrawAll(this); }

		//----------------------------------------------------------------------------------------
		/// Redraws a node master.
		/// @param[in] master							The node master to redraw. @callerOwnsPointed{node master}
		//----------------------------------------------------------------------------------------
		void RedrawMaster(GvNodeMaster *master) { C4DOS_Gv->world->RedrawMaster(this, master); }
		...
		//----------------------------------------------------------------------------------------
		Bool AttachNode(GvNodeMaster *master, GvNode *node, Int32 x, Int32 y) { return C4DOS_Gv->world->AttachNode(this, master, node, x, y); }
		...
		//----------------------------------------------------------------------------------------
		const String GetString(const maxon::String &title, const maxon::String &default_value) { return C4DOS_Gv->world->GetString(this, title, default_value); }
		...
		//----------------------------------------------------------------------------------------
		Float GetFloat(const maxon::String &title, Float default_value) { return C4DOS_Gv->world->GetFloat(this, title, default_value); }
		...
		//----------------------------------------------------------------------------------------
		Int32 GetInteger(const maxon::String &title, Int32 default_value) { return C4DOS_Gv->world->GetInteger(this, title, default_value); }
		...
		//----------------------------------------------------------------------------------------
		Int32 GetDataTypesMenu(BaseContainer &bc, BaseContainer &index, Int32 first_menu_id, Int32 first_sub_id, Bool show_undefined_type = false, GvValueFlags flags = GV_CALC_NOP) { return C4DOS_Gv->world->GetDataTypesMenu(this, bc, index, first_menu_id, first_sub_id, show_undefined_type, flags); }
		...
		/// @return												The number of data types found.
		//----------------------------------------------------------------------------------------
		Int32 GetDataTypes(BaseContainer &bc, GvDataOptions options = GV_DATA_OPTIONS_NONE, GvValueFlags flags = GV_CALC_NOP) { return C4DOS_Gv->world->GetDataTypes(this, bc, options, flags); }
		...
		//----------------------------------------------------------------------------------------
		Bool GetDataTypesTable(GvDataInfo *&info, Int32 &count) { return C4DOS_Gv->world->GetDataTypesTable(this, info, count); }
		...
		//----------------------------------------------------------------------------------------
		Int32 GetDataTypeIndex(GvDataID id) { return C4DOS_Gv->world->GetDataTypeIndex(this, id); }
		...
		//----------------------------------------------------------------------------------------
		GvDataInfo* GetDataTypeInfo(GvDataID id) { return C4DOS_Gv->world->GetDataTypeInfo(this, id); }
		...
		/// @return												The number of data types found.
		//----------------------------------------------------------------------------------------
		Int32 GetDataTypeNames(BaseContainer &bc, GvDataID *ids) { return C4DOS_Gv->world->GetDataTypeNames(this, bc, ids); }
		...
		//----------------------------------------------------------------------------------------
		BaseBitmap* GetDefaultOperatorIcon(GvOperatorType type) { return C4DOS_Gv->world->GetDefaultOperatorIcon(this, type); }
		...
		//----------------------------------------------------------------------------------------
		GvNodeMaster* GetMaster(Int32 id) { return C4DOS_Gv->world->GetMaster(this, id); }
		...
		//----------------------------------------------------------------------------------------
		GvNodeGUI* GetMasterGUI(GvNodeMaster *master, UInt32 nr = 0) { return C4DOS_Gv->world->GetMasterGUI(this, master, nr); }
		...
		//----------------------------------------------------------------------------------------
		UInt32 GetUniqueID() { return C4DOS_Gv->world->GetUniqueID(this); }
		...
		/// @since R16.050
		//----------------------------------------------------------------------------------------
		void SetForceUpdate() { return C4DOS_Gv->world->SetForceUpdate(this); }
		...
		//----------------------------------------------------------------------------------------
		void SetPrefs(const BaseContainer &bc) { C4DOS_Gv->world->WorldSetPrefs(this, bc); }
		...
		//----------------------------------------------------------------------------------------
		void GetPrefs(BaseContainer &bc) { C4DOS_Gv->world->WorldGetPrefs(this, bc); }
...
//----------------------------------------------------------------------------------------
inline const GvDataInfo* GvGetDataInfo(const GvNode *bn, Int32 id)
{
	const BaseContainer *bc = bn->GetOpContainerInstance(); if (!bc) return nullptr;
