...
void GvFreeValuesTable(GvNode *bn, GvValuesInfo &info)
{
	...
	info.in_values.Flush();
	info.out_ports.Flush();
}
...
Bool GvBuildInValuesTable(GvNode *bn, maxon::BaseArray<GvValue*>& ports, GvCalc *c, GvRun *r, GvIdTablePtr ids)
{
	...
	else
	{
		...
		for (i = 0; i < entries; ++i)
		{
			...
			{
				if (!bn->GetPortDescription(GV_PORT_INPUT, ids[i], &pd))
					goto error;
				if (pd.ports_min > 0)
					goto error;
				valid = pd.flags & GV_PORTDESCRIPTION_MULTIPLE ? false : true;
			}
...
GvWorld* GvGetWorld()
{
	return C4DOS_Gv->globals->GetWorld();
}

const	String GvGetEmptyString()
{
	return C4DOS_Gv->globals->GetEmptyString();
}

const String GvGetErrorString(const maxon::String &command, Int32 err)
{
	return C4DOS_Gv->globals->GetErrorString(command, err);
}

void GvGetPortList(GvPortsDescInfo *info, GvPortIO port, GvPortList &portlist)
{
	C4DOS_Gv->globals->GetPortList(info, port, portlist);
}

Bool GvGetAllDataTypes(GvPortDescInfo *info, UInt32 default_flag, Int32 first_id)
{
	return C4DOS_Gv->globals->GetAllDataTypes(info, default_flag, first_id);
}

void GvFreePortDescInfo(GvPortDescInfo *info)
{
	if (C4DOS_Gv->globals->FreePortDescInfo) C4DOS_Gv->globals->FreePortDescInfo(info);
}

Bool GvRegisterValueType(GV_VALUE_HANDLER *data, Int32 struct_size)
{
	if (!C4DOS_Gv->globals || !C4DOS_Gv->globals->RegisterValueType) return false;
	return C4DOS_Gv->globals->RegisterValueType(data, struct_size);
}

Bool GvRegisterDataType(GV_DATA_HANDLER *data, Int32 struct_size, const char *symbol)
{
	if (!C4DOS_Gv->globals || !C4DOS_Gv->globals->RegisterDataType) return false;
	return C4DOS_Gv->globals->RegisterDataType(data, struct_size, symbol);
}

Bool GvRegisterValGroupType(GV_VALGROUP_HANDLER *data, Int32 struct_size)
{
	if (!C4DOS_Gv->globals || !C4DOS_Gv->globals->RegisterValGroupType) return false;
	return C4DOS_Gv->globals->RegisterValGroupType(data, struct_size);
}

Bool GvRegisterOpClassType(GV_OPCLASS_HANDLER *data, Int32 struct_size)
{
	if (!C4DOS_Gv->globals || !C4DOS_Gv->globals->RegisterOpClassType) return false;
	return C4DOS_Gv->globals->RegisterOpClassType(data, struct_size);
}

Bool GvRegisterOpGroupType(GV_OPGROUP_HANDLER *data, Int32 struct_size)
{
	if (!C4DOS_Gv->globals || !C4DOS_Gv->globals->RegisterOpGroupType) return false;
	return C4DOS_Gv->globals->RegisterOpGroupType(data, struct_size);
}

GV_OPCLASS_HANDLER* GvFindRegisteredOpClass(GvOpClassID id)
{
	return C4DOS_Gv->globals->FindRegisteredOpClass(id);
}

GV_OPGROUP_HANDLER* GvFindRegisteredOpGroup(GvOpGroupID id)
{
	return C4DOS_Gv->globals->FindRegisteredOpGroup(id);
}
