...
//----------------------------------------------------------------------------------------
using NodeMaterialLinkResolveFunc = Delegate<Result<cinema::BaseList2D*>(cinema::BaseLink* link, cinema::BaseDocument* baseDocument)>;
...
/// @C4D API version
#define API_VERSION	2026100

class CINEWARE_SINGLEINHERITANCE C4DAtom;
class CINEWARE_SINGLEINHERITANCE C4DAtomGoal;
...
struct C4D_Shader
{
	...
	Vector64				(*TransformColorOcio64					)(const OcioConverter* ocioConverter, const Vector64& v, COLORSPACETRANSFORMATION colorSpaceTransformation);
	Vector32				(*TransformColorOcio32					)(const OcioConverter* ocioConverter, const Vector32& v, COLORSPACETRANSFORMATION colorSpaceTransformation);
	void						(*GetOcioConverter							)(const OcioConverter* ocioConverter, COLORSPACETRANSFORMATION colorSpaceTransformation, maxon::ColorProfileConvert& converter);
	...
	void						(*TransformColorsOcio64B				)(const OcioConverter* ocioConverter, const maxon::StridedBlock<const maxon::Color64>& colorIn, COLORSPACETRANSFORMATION colorSpaceTransformation, const maxon::StridedBlock<maxon::Color64>& colorOut);
	void						(*TransformColorsOcio32B				)(const OcioConverter* ocioConverter, const maxon::StridedBlock<const maxon::Color32>& colorIn, COLORSPACETRANSFORMATION colorSpaceTransformation, const maxon::StridedBlock<maxon::Color32>& colorOut);
};
...
struct C4D_BaseContainer
{
	...
	Bool						(BaseContainer::*SetDataNoDirty			)(Int32 id, const GeData& n);
};
...
struct C4D_Bitmap
{
	...
	Bool						(*GetImageRef					)(const BaseBitmap *bm, SAVEBIT saveBit, Bool isAlphaChannel, maxon::ImageRef& imageRef);
	...
	Bool						(*GetImageRefEx				)(const BaseBitmap *bm, SAVEBIT saveBit, Bool isAlphaChannel, PIXELCNT getFlags, maxon::ImageRef& imageRef);
	UInt32					(MultipassBitmap::*GetLayerDirty)() const;

	void						(*UpdateRegionAddPoint)(BaseBitmap *bm, Int32 x, Int32 y, Bool updateImageRefRegions);
	void						(*UpdateRegionAddLine	)(BaseBitmap *bm, Int32 x1, Int32 x2, Int32 y, Bool updateImageRefRegions);
	void						(*UpdateRegionAddRegion)(BaseBitmap *bm, Int32 x1, Int32 y1, Int32 x2, Int32 y2, Bool updateImageRefRegions);
	Bool						(*ProfileGetHashCode	 )(const ColorProfile *profile);
};
...
struct C4D_UserArea
{
	...
	Bool						(*GetInputState				)(const CBaseFrame *cu, Int32 askdevice, Int32 askchannel, BaseContainer *res);
	Bool						(*GetInputEvent				)(const CBaseFrame *cu, Int32 askdevice, BaseContainer *res);
	void						(*KillEvents					)(CBaseFrame *cu);
	void						(*DrawSetFont					)(CUserArea *cu, Int32 fontid);
	Int32						(*DrawGetTextWidth		)(const CUserArea *cu, const maxon::String *text);
	Int32						(*DrawGetFontHeight		)(const CUserArea *cu);
	...
	Bool						(*Global2Local				)(const CBaseFrame *cu, Int32 *x, Int32 *y);
	...
	Int32						(*DrawGetTextWidth_ListNodeName)(const CUserArea *cu, BaseList2D *node, Int32 fontid);
	...
	Int32						(*DrawGetFontBaseLine					)(const CUserArea *cu);
	Bool						(*Local2Global								)(const CBaseFrame *cu, Int32 *x, Int32 *y);
	Bool						(*Local2Screen								)(const CBaseFrame *cu, Int32 *x, Int32 *y);
	...
	Bool						(*Global2LocalF								)(const CBaseFrame *cu, Float& x, Float& y);
	Bool						(*Local2GlobalF								)(const CBaseFrame *cu, Float& x, Float& y);
	Bool						(*Screen2LocalF								)(const CBaseFrame *cu, Float& x, Float& y);
	Bool						(*Local2ScreenF								)(const CBaseFrame *cu, Float& x, Float& y);
	...
	void						(*DrawEditFieldBackground			)(CUserArea *cu, Int32 x1, Int32 y1, Int32 x2, Int32 y2, Bool noFill, Bool hasFocus); // @since 2026.0
	void						(*DrawComboBox								)(CUserArea *cu, Int32 x1, Int32 y1, Int32 x2, Int32 y2, IconData* iconData, const String& text, Bool noFill, Bool hasFocus); // @since 2026.0
};
...
struct C4D_Atom
{
	...
	const BaseList2D*			(BaseList2D::*GetMain					)() const;
	...
	// 2026.1
	const UniqueIDMap& (BaseList2D::*GetUniqueIDMap)() const;
};
...
struct C4D_BaseList
{
	...
	const CTrack*						(BaseList2D::*GetFirstCTrack	)() const;
	const CTrack*						(BaseList2D::*FindCTrack			)(const DescID &id) const;
...
class VariableTagData;

//----------------------------------------------------------------------------------------
/// TypedVariableTagData<T> represents the data held by a VariableTag with elements of type T.
/// It is a subclass of the generic VariableTagData.
//----------------------------------------------------------------------------------------
template <typename T> class TypedVariableTagData : public maxon::SFINAEHelper<VariableTagData, T>::type
{
public:
	~TypedVariableTagData() = delete;

	//----------------------------------------------------------------------------------------
	/// Returns the data array as a block of elements of type T.
	/// @return												The elements as a block.
	//----------------------------------------------------------------------------------------
	maxon::Block<const T> GetBlock() const
	{
		const maxon::StridedBlock<const maxon::Generic> block = maxon::SFINAEHelper<VariableTagData, T>::type::GetBlock();
		DebugAssert(block.GetStride() == SIZEOF(T) || (block.IsEmpty() && block.GetStride() == 0));
		return {reinterpret_cast<const T*>(block.GetFirst()), block.GetCount()};
	}

	static const TypedVariableTagData& DefaultValue()
	{
		return maxon::GetZeroRef<TypedVariableTagData>();
	}
};

//----------------------------------------------------------------------------------------
/// A type alias for a copy-on-write reference of VariableTagData or TypedVariableTagData<T>.
/// A VariableTag holds its data array via such a reference.
/// @tparam T											The element type of the VariableTag's elements.
//----------------------------------------------------------------------------------------
template <typename T = maxon::Generic> using VariableTagDataRef = maxon::StrongCOWRef<typename std::conditional<std::is_same<T, maxon::Generic>::value, VariableTagData, TypedVariableTagData<T>>::type>;

enum class VARIABLETAGACCESS
{
	READACCESS = 0,
	WRITEACCESS = 1,
	COUNT = 2,
	DATARAW = 3,
	DATAHASH = 4
} MAXON_ENUM_LIST(VARIABLETAGACCESS);

using ValidationDelegate = maxon::Delegate<maxon::Result<Bool>(VariableTag*, VARIABLETAGACCESS, VariableTagDataRef<>&)>;
...
struct C4D_Tag
{
	...
	const VariableTagDataRef<>& (*GetVariableTagData)(const VariableTag* tag);

	void						(*FreeVariableTagData)(const VariableTagData* data);
	maxon::StridedBlock<const maxon::Generic> (*GetVariableTagDataBlock)(const VariableTagData* data);
	const maxon::UniqueHash& (*GetVariableTagDataHashCode)(const VariableTagData* data);
	void (*SetValidationDelegate)(VariableTag* bt, ValidationDelegate&& validationDelegate);
	maxon::Result<Bool> (*FindMatchingTagsOnPointObject)(const BaseTag* tag, Bool useDeformed, Bool getRealTag, const maxon::ValueReceiver<const BaseTag*>& receiver);
	
	const CPolygon* (*GetUVWPolygonR)(const UVWTag* uvwTag);
	CPolygon* (*GetUVWPolygonW)(UVWTag* uvwTag);
	Int32 (*GetUVWPolygonCount)(const UVWTag* uvwTag);
	const Vector* (*GetUVWPointR)(const UVWTag* uvwTag);
	Vector* (*GetUVWPointW)(UVWTag* uvwTag);
	Int32 (*GetUVWPointCount)(const UVWTag* uvwTag);
	Bool (*ResizeUVWTopology)(UVWTag* uvwTag, Int32 pointCount, Int32 polygonCount);
	maxon::Result<void> (*IsUVWTopologyValid)(const UVWTag* tag);
	maxon::Result<void> (*OptimizeUVWTopology)(UVWTag* tag);
	const PolygonObject* (*GetUVWObjectR)(const UVWTag* tag);
	PolygonObject* (*GetUVWObjectW)(UVWTag* tag);
	BaseSelect* (*GetWritableUVWPointS)(UVWTag* tag);
	const BaseSelect* (*GetUVWPointS)(const UVWTag* tag);
	EdgeBaseSelect* (*GetWritableUVWEdgeS)(UVWTag* tag);
	const EdgeBaseSelect* (*GetUVWEdgeS)(const UVWTag* tag);
	BaseSelect* (*GetWritableUVWPolygonS)(UVWTag* tag);
	const BaseSelect* (*GetUVWPolygonS)(const UVWTag* tag);
};
...
struct C4D_Object
{
	...
	UInt64					(*GetGUID							)(const BaseObject *op);
	...
	Bool						(BaseObject::*IsQuaternionRotationMode)() const;
	...
	void (BaseObject::*GetDrawStateData)(DrawStateData& state) const;
	...
	Bool (BaseObject::*HasDeformMatrix)() const;
	...
	Bool (*ReplaceElement)(AssetData* priv, BaseList2D* oldOwner, BaseList2D* newOwner, Int32 oldId, Int32 newId, GeListNode* startNodeSearch);
	maxon::Opt<maxon::Color32>(BaseObject::*GetDisplayColor32)() const;

	Bool (*PyBuildNgon)(PolygonObject* op);
	void						(*InsertTagCheckName)(BaseObject* op, BaseTag* tp, BaseTag* pred);
};
...
struct C4D_Document
{
	...
	Bool						(*SetSceneCamera				)(BaseDocument* doc, BaseObject* camera);
	...
	void						(*RdGetResolution				)(RenderData* rd, Float& width, Float& height, Float& pixelAspect, Float& filmAspect);
	void						(*RdSetResolution				)(RenderData* rd, Float width, Float height, Float pixelAspect);
	...
	Bool (BaseDocument::*HasUncachedDynamicMoGraph)() const;
	...
	const BaseVideoPost*	(*RdGetFirstVideoPostC)(const RenderData *rd);

	void (BaseDocument::*GetActiveUVWObjects)(AtomArray& selection, GETACTIVEUVWOBJECTSMODE mode) const;
	Int32 (*GetTexBaseDrawCount)(const BaseDocument* doc);
	BaseDraw* (*GetTexBaseDraw)(const BaseDocument* doc, Int32 num);
	BaseDraw* (*GetActiveTexBaseDraw)(const BaseDocument* doc);
};
...
struct C4D_Thread
{
	...
	Bool						(*ViewSchedulerMessage)(BaseThread* bt, Int32 type, void* data);
};
...
struct C4D_Material
{
	...
	BaseShader*			(*MakeShader					)(BaseMaterial* bm, Int32 type, Int32 channelId, BaseShader* pred);
...
struct C4D_CAnimation
{
	...
	Bool						(CTrack::*FillKey							)(const BaseDocument *doc, const BaseList2D *bl, CKey *key) const;
	Bool						(CTrack::*TrackInformation		)(const BaseDocument *doc, CKey *key, maxon::String *str, Bool set) const;
	...
	// 2025_3
	const BaseTime& (CKey::*GetTimeRef)() const;
	const BaseTime& (CKey::*GetTimeLeftRef)() const;
	const BaseTime& (CKey::*GetTimeRightRef)() const;
};
...
struct C4D_BaseDraw
{
	...
	void						(*SetMatrix_Matrix		)(BaseDraw *bd, const BaseObject *op, const Matrix &mg);
	...
	void						(*SetMatrix_MatrixO			)(BaseDraw *bd, const BaseObject *op, const Matrix &mg, Int32 zoffset);
	...
	void						(*BbSetVertexColor		)(BaseDrawHelp *bb, maxon::Color32* vertexcolor, DISPLAYCONTROL_MODE colorMode);
	...
	Bool						(*IsUvViewport)(const BaseDraw* bd);
};
...
struct C4D_BaseView
{
	...
	Int32						(*GetFrameScreen			)(const BaseDraw *bv, Int32 *cl, Int32 *ct, Int32 *cr, Int32 *cb);
...
struct C4D_General
{
	...
	// R2024.2
	void (*SendSceneCreationEvent)(maxon::DataDictionary eventData);
...
struct OperatingSystem
{
	...
	//----------------------------------------------------------------------------------------
	/// VariableTagData holds the data array of a VariableTag.
	//----------------------------------------------------------------------------------------
	class VariableTagData
	{
	public:
		~VariableTagData() = delete;

		//----------------------------------------------------------------------------------------
		/// Returns the data array as a strided block of generic elements. The subclass TypedVariableTagData<T>
		/// overloads this function to return a block of elements of type T.
		/// @return												The elements as a block.
		//----------------------------------------------------------------------------------------
		maxon::StridedBlock<const maxon::Generic> GetBlock() const
		{
			return C4DOS_Tg->GetVariableTagDataBlock(this);
		}
		...
		//----------------------------------------------------------------------------------------
		const maxon::UniqueHash& GetUniqueHashCode() const
		{
			return C4DOS_Tg->GetVariableTagDataHashCode(this);
		}

	private:
		void RemoveReference() const
		{
			if (MAXON_UNLIKELY(maxon::details::PrivateGetReferenceCounter(this).Dec()))
			{
				C4DOS_Tg->FreeVariableTagData(this);
			}
		}
