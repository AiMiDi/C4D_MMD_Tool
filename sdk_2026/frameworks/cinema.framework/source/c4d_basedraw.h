...
public:
	...
	/// @copydoc Alloc
	static BaseDrawHelp* Alloc(const maxon::SourceLocation&, BaseDraw* bd, BaseDocument* doc) { return Alloc(bd, doc); }
	...
	//----------------------------------------------------------------------------------------
	BaseDocument* GetDocument() { return C4DOS_Br->BbGetDocument(this); }
	...
	//----------------------------------------------------------------------------------------
	BaseTag* GetActiveTag() { return C4DOS_Br->BbGetActiveTag(this); }
	...
	//----------------------------------------------------------------------------------------
	const Matrix& GetMg() { return C4DOS_Br->BbGetMg(this); }
	...
	//----------------------------------------------------------------------------------------
	maxon::Block<const Matrix> GetMatrices() { return C4DOS_Br->BbGetMatrices(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the matrix returned by GetMg().
	/// @param[in] mg									The new global matrix of the object to be drawn.
	//----------------------------------------------------------------------------------------
	void SetMg(const Matrix& mg) { C4DOS_Br->BbSetMg(this, mg); }
	...
	//----------------------------------------------------------------------------------------
	BaseContainer GetDisplay() { BaseContainer bc; C4DOS_Br->BbGetDisplay(this, &bc); return bc; }

	//----------------------------------------------------------------------------------------
	/// Sets the display mode for the object to be drawn. See @em Tdisplay.h for values.
	/// @param[in] bc									The new display mode container.
	//----------------------------------------------------------------------------------------
	void SetDisplay(BaseContainer* bc) { C4DOS_Br->BbSetDisplay(this, bc); }

	//----------------------------------------------------------------------------------------
	/// Gets a pointer to an array of vertex colors or nullptr if no vertex colors are set for the object.
	/// @return												Vertex colors.
	//----------------------------------------------------------------------------------------
	const maxon::Color32* GetVertexColor() { return C4DOS_Br->BbGetVertexColor(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the vertex color for the object.
	/// @param[in] vertexColor				Vertex colors.
	/// @param[in] colorMode					Color mode (colors can be set either for the object, vertices or per polygon vertex).
	//----------------------------------------------------------------------------------------
	void SetVertexColor(maxon::Color32* vertexColor, DISPLAYCONTROL_MODE colorMode) { C4DOS_Br->BbSetVertexColor(this, vertexColor, colorMode); }

	//----------------------------------------------------------------------------------------
	/// Checks if vertex color shading is active.
	/// @return												True, if vertex color shading is active, false otherwise.
	//----------------------------------------------------------------------------------------
	Bool GetVertexColorShading() { return C4DOS_Br->BbGetVertexColorShading(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the vertex color shading mode.
	/// @param[in] vertexcolorShading	True, if vertex color shading should hapen, false otherwise.
	//----------------------------------------------------------------------------------------
	void SetVertexColorShading(Bool vertexcolorShading) { C4DOS_Br->BbSetVertexColorShading(this, vertexcolorShading); }
	...
	//----------------------------------------------------------------------------------------
	DISPLAYMODE GetDisplayMode(Bool original) const { return C4DOS_Br->BbGetDisplayMode(this, original); }
	...
	//----------------------------------------------------------------------------------------
	void SetDisplayMode(DISPLAYMODE mode, Bool original) { C4DOS_Br->BbSetDisplayMode(this, mode, original); }

	//----------------------------------------------------------------------------------------
	/// Gets the current level of detail.
	/// @return												The current level of detail.
	//----------------------------------------------------------------------------------------
	Float32 GetLod() const { return C4DOS_Br->BbGetLod(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the current level of detail.
	/// @param[in] lod								The new level of detail.
	//----------------------------------------------------------------------------------------
	void SetLod(Float32 lod) { C4DOS_Br->BbSetLod(this, lod); }

	//----------------------------------------------------------------------------------------
	/// Gets the vector display mode.
	/// @return												The vector display mode.
	//----------------------------------------------------------------------------------------
	Int32 GetVectorDisplayMode() const { return C4DOS_Br->BbGetVectorDisplayMode(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the vector display mode.
	/// @param[in] vectorDisplayMode	The new vector display mode.
	//----------------------------------------------------------------------------------------
	void SetVectorDisplayMode(Int32 vectorDisplayMode) { C4DOS_Br->BbSetVectorDisplayMode(this, vectorDisplayMode); }
	...
	//----------------------------------------------------------------------------------------
	void SetActiveFlags(Bool active, Bool hierarchyActive, Bool highlighted, Bool hierarchyHighlighted) { C4DOS_Br->BbSetActiveFlags(this, active, hierarchyActive, highlighted, hierarchyHighlighted); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsHighlight() const { return C4DOS_Br->BbIsHighlight(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the color object.
	/// @return												The color object.
	//----------------------------------------------------------------------------------------
	BaseObject* GetColorObject() const { return C4DOS_Br->GetColorObject(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the color object.
	/// @param[in] colorObject				The color object.
	//----------------------------------------------------------------------------------------
	void SetColorObject(BaseObject* colorObject) { C4DOS_Br->SetColorObject(this, colorObject); }
...
{
public:
	explicit HUDTextEntry(const String& txt = String(), const Vector& position = Vector(), const Int32& monoFontColorSchemeFlags = 0) : _txt(txt), _position(position), _monoFontColorSchemeFlags(monoFontColorSchemeFlags) { }

	String _txt; 			///< The text to draw to the HUD
	Vector _position;	///< The screen space position for the text
	Int32 _monoFontColorSchemeFlags;		///< If true it will cache each character and render quicker (skipping system functions)
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	const Matrix& GetBaseMatrix() const { return C4DOS_Bv->GetBaseMatrix(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetBaseMatrix(const Matrix& m) { C4DOS_Bv->SetBaseMatrix(this, m); }

	//----------------------------------------------------------------------------------------
	/// Gets the rotation of the planar views.
	/// @return												The planar rotation.
	//----------------------------------------------------------------------------------------
	Float GetPlanarRotation() const { return C4DOS_Bv->GetPlanarRotation(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the rotation of the planar views.
	/// @param[in] r									The new planar rotation.
	//----------------------------------------------------------------------------------------
	void SetPlanarRotation(Float r) { C4DOS_Bv->SetPlanarRotation(this, r); }
	...
	//----------------------------------------------------------------------------------------
	Bool TestPoint(Float x, Float y) const { return C4DOS_Bv->TestPoint(this, x, y); }
	...
	//----------------------------------------------------------------------------------------
	Bool TestPointZ(const Vector& p) const { return C4DOS_Bv->TestPointZ(this, p); }
	...
	//----------------------------------------------------------------------------------------
	Bool ClipLine2D(Vector* p1, Vector* p2) const { return C4DOS_Bv->ClipLine2D(this, p1, p2); }
	...
	//----------------------------------------------------------------------------------------
	Bool ClipLineZ(Vector* p1, Vector* p2) const { return C4DOS_Bv->ClipLineZ(this, p1, p2); }
	...
	//----------------------------------------------------------------------------------------
	Vector WS(const Vector& p) const { return C4DOS_Bv->WS(this, p); }
	...
	//----------------------------------------------------------------------------------------
	Vector SW(const Vector& p) const { return C4DOS_Bv->SW(this, p); }
	...
	/// @return												The point in world space.
	//----------------------------------------------------------------------------------------
	Vector SW_Reference(Float x, Float y, const Vector& wp) const { return C4DOS_Bv->SW_R(this, x, y, wp); }
	...
	//----------------------------------------------------------------------------------------
	Vector WC(const Vector& p) const { return C4DOS_Bv->WC(this, p); }
	...
	/// @return												The point in world space.
	//----------------------------------------------------------------------------------------
	Vector CW(const Vector& p) const { return C4DOS_Bv->CW(this, p); }
	...
	/// @return												The point in camera space.
	//----------------------------------------------------------------------------------------
	Vector SC(const Vector& p) const { return C4DOS_Bv->SC(this, p); }
	...
	/// @return												The point in screen space.
	//----------------------------------------------------------------------------------------
	Vector CS(const Vector& p, Bool z_inverse) const { return C4DOS_Bv->CS(this, p, z_inverse); }
	...
	//----------------------------------------------------------------------------------------
	Vector WC_V(const Vector& v) const { return C4DOS_Bv->WC_V(this, v); }
	...
	//----------------------------------------------------------------------------------------
	Vector CW_V(const Vector& v) const { return C4DOS_Bv->CW_V(this, v); }
	...
	//----------------------------------------------------------------------------------------
	Vector WS_V(const Vector &v, const Vector &p) const { return C4DOS_Bv->WS_V(this, v, p); }
	...
	/// @return												The vector in world space.
	//----------------------------------------------------------------------------------------
	Vector SW_V(const Vector &v, const Vector &p) const { return C4DOS_Bv->SW_V(this, v, p); }
	...
	//----------------------------------------------------------------------------------------
	Bool ZSensitiveNear() const { return C4DOS_Bv->ZSensitiveNear(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the near-clipping of @em Z sensitive view.
	/// @return												The Z near-clipping distance.
	//----------------------------------------------------------------------------------------
	Float ZSensitiveNearClipping() const { return C4DOS_Bv->ZSensitiveNearClipping(this); }

	//----------------------------------------------------------------------------------------
	/// Indicates if the view is @em Z far-clipping sensitive.
	/// @return												@trueIfOtherwiseFalse{the view has Z far-clipping}
	//----------------------------------------------------------------------------------------
	Bool ZSensitiveFar() const { return C4DOS_Bv->ZSensitiveFar(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the far-clipping of @em Z sensitive view.
	/// @return												The Z far-clipping distance.
	//----------------------------------------------------------------------------------------
	Float ZSensitiveFarClipping() const { return C4DOS_Bv->ZSensitiveFarClipping(this); }
	...
	//----------------------------------------------------------------------------------------
	Bool BackfaceCulling(const Vector& n, const Vector& p) { return C4DOS_Bv->BackfaceCulling(this, n, p); }

	//----------------------------------------------------------------------------------------
	/// Gets the projection used by the view. See @em Ocamera.h for projection types values.
	/// @return												The projection type.
	//----------------------------------------------------------------------------------------
	Int32 GetProjection() const { return C4DOS_Bv->GetProjection(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the stereoscopic camera information data.
	/// @return												The stereoscopic camera data. @cinemaOwnsPointed{StereoCameraInfo}
	//----------------------------------------------------------------------------------------
	StereoCameraInfo* GetStereoInfo() const { return C4DOS_Bv->GetStereoInfo(this); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetColorSpace() const { return C4DOS_Bv->GetColorSpace(this); }
...
#define DRAW_GET_VIEWMATRIX_PROJECTION_LARGE_Z			1		///< Deprecated, returns the same value as DRAW_GET_VIEWMATRIX_PROJECTION.
#define DRAW_GET_VIEWMATRIX_INV_CAMERA							2		///< Inverse camera matrix.
#define DRAW_GET_VIEWMATRIX_MODELVIEW_PROJECTION_EX	3		///< Deprecated, returns the same value as DRAW_GET_VIEWMATRIX_PROJECTION.
/// @}


/// Plugin ID for BaseDraw Display Filter preset type.\n
/// Can be used to filter a list of presets for the Content Browser.
static const Int32 ID_DISPLAYFILTER_BROWSERPRESET = 1054164; ///< BaseDraw Display Filter preset ID.
...
public:
	...
	//----------------------------------------------------------------------------------------
	static BaseDraw* Alloc() { return C4DOS_Br->AllocBaseDraw(); }

	/// @copydoc Alloc
	static BaseDraw* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	static void Free(BaseDraw*& bd) { C4DOS_Br->FreeBaseDraw(bd); }
	...
	//----------------------------------------------------------------------------------------
	Bool HasCameraLink() { return C4DOS_Br->HasCameraLink(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetSceneCamera(BaseObject* op, Bool animate = false) { C4DOS_Br->SetSceneCamera(this, op, animate); }
	...
	//----------------------------------------------------------------------------------------
	BaseObject* GetEnvironmentObject() const { return C4DOS_Br->GetEnvironmentObject(this); }
	...
	//----------------------------------------------------------------------------------------
	BaseObject* GetSkyObject() const { return C4DOS_Br->GetSkyObject(this); }
	...
	//----------------------------------------------------------------------------------------
	BaseObject* GetBackgroundObject() const { return C4DOS_Br->GetBackgroundObject(this); }
	...
	//----------------------------------------------------------------------------------------
	BaseObject* GetForegroundObject() const { return C4DOS_Br->GetForegroundObject(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the editor camera.
	/// @return												The editor camera. Guaranteed to never be @formatConstant{nullptr}.
	//----------------------------------------------------------------------------------------
	BaseObject* GetEditorCamera() { return C4DOS_Br->GetEditorCamera(this); }

	//----------------------------------------------------------------------------------------
	/// Gets all X-Ref objects.
	/// @return												An array of X-Ref objects.
	//----------------------------------------------------------------------------------------
	const AtomArray& GetXRefObjects() const { return C4DOS_Br->GetXRefObjects(this); }
...
#define INIT_CLIPBOX_ADJUSTGLSIZE 1
	...
	//----------------------------------------------------------------------------------------
	void InitClipbox(Int32 left, Int32 top, Int32 right, Int32 bottom, Int32 flags) { C4DOS_Br->InitClipbox(this, left, top, right, bottom, flags); }
	...
	//----------------------------------------------------------------------------------------
	void InitView(BaseContainer* camera, const Matrix& op_m, Float sv, Float pix_x, Float pix_y, Bool fitview) { C4DOS_Br->InitView(this, camera, op_m, sv, pix_x, pix_y, fitview); }
	...
	//----------------------------------------------------------------------------------------
	void InitializeView(BaseDocument* doc, BaseObject* cam, Bool editorsv) { C4DOS_Br->InitializeView(this, doc, cam, editorsv); }
	...
	/// @since R19
	//----------------------------------------------------------------------------------------
	void DrawEnd() { C4DOS_Br->DrawEnd(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Prepares the selected objects for rendering (i.e. sets the BIT_ACTIVESELECTIONDRAW bit).
	//----------------------------------------------------------------------------------------
	static void PrepareActionSelections(const BaseDocument* doc, Bool withChildren) { C4DOS_Br->PrepareActionSelections(doc, withChildren); }
	...
	//----------------------------------------------------------------------------------------
	Vector GetObjectColor(const BaseDrawHelp* bh, const BaseObject* op, Bool lines = false, Bool usedInBaseDraw = true, Int instanceIndex = 0) const { return C4DOS_Br->GetObjectColor(this, op, bh, lines, usedInBaseDraw, instanceIndex); }
	...
	//----------------------------------------------------------------------------------------
	Vector CheckColor(const Vector& col) { return C4DOS_Br->CheckColor(this, col); }
	...
	//----------------------------------------------------------------------------------------
	void SetTransparency(Int32 trans) { C4DOS_Br->SetTransparency(this, trans); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetTransparency() const { return C4DOS_Br->GetTransparency(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetPen(const Vector& col, Int32 flags = 0) { C4DOS_Br->SetPen(this, col, flags); }

	//----------------------------------------------------------------------------------------
	/// Sets the drawn point size.
	/// @param[in] pointsize					The new point size.
	//----------------------------------------------------------------------------------------
	void SetPointSize(Float pointsize) { C4DOS_Br->SetPointSize(this, pointsize); }
	...
	//----------------------------------------------------------------------------------------
	Float GetHandleSize(DRAWHANDLE type) const { return C4DOS_Br->GetHandleSize(this, type); }
	...
	//----------------------------------------------------------------------------------------
	void SetLightList(Int32 mode) { C4DOS_Br->SetLightList(this, mode); }
...
/// @}
	...
	//----------------------------------------------------------------------------------------
	Vector ConvertColor(const Vector& c) { return C4DOS_Br->ConvertColor(this, c); }
	...
	//----------------------------------------------------------------------------------------
	Vector ConvertColorReverse(const Vector& c) { return C4DOS_Br->ConvertColorReverse(this, c); }
	...
	//----------------------------------------------------------------------------------------
	void LineZOffset(Int32 offset) { C4DOS_Br->LineZOffset(this, offset); }
	...
	//----------------------------------------------------------------------------------------
	void SetDepth(Bool enable) { C4DOS_Br->SetDepth(this, enable); }
	...
	/// @markInternal
	//-------------------------------------------------------------------------------------------------
	void SetMatrix_Projection() { C4DOS_Br->SetMatrix_Projection(this); }

	//-------------------------------------------------------------------------------------------------
	/// Sets the transformation matrix to screen coordinates, i.e. from <i>(0, 0)</i> to <i>(width, height)</i>.
	/// @note This method only affects the new 3D drawing methods below it, i.e. DrawLine(), LineStrip(), DrawHandle(), DrawPoly(), DrawTexture(), DrawCircle(), DrawBox(), DrawPolygon() and DrawSphere().
	//-------------------------------------------------------------------------------------------------
	void SetMatrix_Screen() { C4DOS_Br->SetMatrix_Screen(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetMatrix_Screen(Int32 zoffset) { C4DOS_Br->SetMatrix_ScreenO(this, zoffset); }

	//-------------------------------------------------------------------------------------------------
	/// @markPrivate
	//-------------------------------------------------------------------------------------------------
	void SetMatrix_Screen(Int32 zoffset, const Matrix4d* m) { C4DOS_Br->SetMatrix_ScreenOM(this, zoffset, m); }

	//-------------------------------------------------------------------------------------------------
	/// Sets the transformation matrix to the camera system.
	/// @note This method only affects the new 3D drawing methods below it, i.e. DrawLine(), LineStrip(), DrawHandle(), DrawPoly(), DrawTexture(), DrawCircle(), DrawBox(), DrawPolygon() and DrawSphere().
	//-------------------------------------------------------------------------------------------------
	void SetMatrix_Camera() { C4DOS_Br->SetMatrix_Camera(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetMatrix_Matrix(const BaseObject* op, const Matrix& mg) { C4DOS_Br->SetMatrix_Matrix(this, op, mg); }
	...
	/// 															- selected edges (level @em 4)
	//----------------------------------------------------------------------------------------
	void SetMatrix_Matrix(const BaseObject* op, const Matrix& mg, Int32 zoffset) { C4DOS_Br->SetMatrix_MatrixO(this, op, mg, zoffset); }
...
/// @ingroup group_enumeration
/// @{
#define LEGACY_DRAW_FLAGS_DEPTH_TEST_ALWAYS					(1 << 10)				///< Depth test always passes.
#define LEGACY_DRAW_FLAGS_CLIPPING_DISABLE_BORDER		(1 << 11)				///< Clip against the view border.
#define LEGACY_DRAW_FLAGS_CLIPPING_DISABLE_DEPTH		(1 << 12)				///< Z clipping.

#define NOCLIP_D	LEGACY_DRAW_FLAGS_CLIPPING_DISABLE_BORDER			///< Clip against the view border.
#define NOCLIP_Z	LEGACY_DRAW_FLAGS_CLIPPING_DISABLE_DEPTH			///< Z clipping.
...
/// @}
	...
	//----------------------------------------------------------------------------------------
	void DrawPoint2D(const Vector& p) { C4DOS_Br->DrawPoint2D(this, p); }
	...
	//----------------------------------------------------------------------------------------
	void DrawLine2D(const Vector& p1, const Vector& p2) { C4DOS_Br->DrawLine2D(this, p1, p2); }
	...
	//----------------------------------------------------------------------------------------
	void DrawHandle2D(const Vector& p, DRAWHANDLE type = DRAWHANDLE::SMALL) { C4DOS_Br->DrawHandle2D(this, p, type); }
	...
	//----------------------------------------------------------------------------------------
	void DrawCircle2D(Int32 mx, Int32 my, Float rad) { C4DOS_Br->DrawCircle2D(this, mx, my, rad); }
	...
	//----------------------------------------------------------------------------------------
	void DrawHandle(const Vector& vp, DRAWHANDLE type, Int32 flags) { C4DOS_Br->DrawHandle(this, vp, type, flags); }
	...
	/// @param[in] type								The handle type: @enumerateEnum{DRAWHANDLE}
	//----------------------------------------------------------------------------------------
	void DrawHandleWorld(const Vector& vp, const Vector &col, DRAWHANDLE type) { C4DOS_Br->DrawHandleWorld(this, vp, col, type); }
	...
	//----------------------------------------------------------------------------------------
	void DrawPointArray(Int32 cnt, const Vector32* vp, const Float32* vc = nullptr, Int32 colcnt = 0, const Vector32* vn = nullptr) { C4DOS_Br->DrawPointArray(this, cnt, vp, vc, colcnt, vn); }
	...
	/// @param[in] flags							The legacy drawing flags: @ref LEGACY_DRAW_FLAGS, @ref NOCLIP_D or @ref NOCLIP_Z.
	//----------------------------------------------------------------------------------------
	void DrawLine(const Vector& p1, const Vector& p2, Int32 flags) { C4DOS_Br->DrawLine(this, p1, p2, flags); }
	...
	/// @param[in] flags							The legacy drawing flags: @ref LEGACY_DRAW_FLAGS, @ref NOCLIP_D or @ref NOCLIP_Z.
	//----------------------------------------------------------------------------------------
	void DrawArc(const Vector& pos, Float radius, Float angle_start, Float angle_end, Int32 subdiv = 32, Int32 flags = 0) { C4DOS_Br->DrawArc(this, pos, radius, angle_start, angle_end, subdiv, flags); }
	...
	/// @param[in] flags							The legacy drawing flags: @ref LEGACY_DRAW_FLAGS, @ref NOCLIP_D or @ref NOCLIP_Z.
	//----------------------------------------------------------------------------------------
	void DrawPoly(const Vector* vp, const Vector* vf, const Vector* vn, Int32 anz, Int32 flags) { C4DOS_Br->DrawPoly(this, vp, vf, vn, anz, flags); }
	...
	//----------------------------------------------------------------------------------------
	void DrawTexture(const BaseBitmap* bmp, const Vector* padr4, const Vector* cadr, const Vector* vnadr, const Vector* uvadr, Int32 pntcnt, DRAW_ALPHA alphamode, DRAW_TEXTUREFLAGS flags) { C4DOS_Br->DrawTexture(this, bmp, padr4, cadr, vnadr, uvadr, pntcnt, alphamode, flags); }
	...
	//----------------------------------------------------------------------------------------
	void DrawCircle(const Matrix& m) { C4DOS_Br->DrawCircle(this, m); }
	...
	//----------------------------------------------------------------------------------------
	void DrawBox(const Matrix& m, Float size, const Vector& col, Bool wire) { C4DOS_Br->DrawBox(this, m, size, col, wire); }
	...
	//----------------------------------------------------------------------------------------
	void DrawPolygon(const Vector* p, const Vector* f, Bool quad) { C4DOS_Br->DrawPolygon(this, p, f, quad); }
	...
	//----------------------------------------------------------------------------------------
	void DrawSphere(const Vector& off, const Vector& size, const Vector& col, Int32 flags) { C4DOS_Br->DrawSphere(this, off, size, col, flags); }
#define BDRAW_DRAW_SPHERE_FLAGS_NO_SHADING 1 ///< If set for DrawSphere(), the sphere will not be shaded, it will be drawn with the specified color.
	...
	//-------------------------------------------------------------------------------------------------
	void DrawArrayEnd() { C4DOS_Br->DrawArrayEnd(this); }
	...
	//----------------------------------------------------------------------------------------
	DRAWRESULT DrawPolygonObject(BaseDrawHelp* bh, BaseObject* op, DRAWOBJECT flags, BaseObject* parent = nullptr, const Vector& col = Vector(.5)) { return C4DOS_Br->DrawPObject(this, bh, op, flags, DRAWPASS::OBJECT, parent, col); }
	...
	/// @since R19
	//----------------------------------------------------------------------------------------
	Bool DrawHighlight(BASEDRAW_HIGHLIGHT flags) { return C4DOS_Br->DrawHighlight(this, flags); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// @since R19
	//----------------------------------------------------------------------------------------
	Bool DrawPostPass() { return C4DOS_Br->DrawPostPass(this); }

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// @since R21
	//----------------------------------------------------------------------------------------
	Bool DrawSceneHook(BaseDocument *doc, BaseDrawHelp *bh, BaseThread *bt, SCENEHOOKDRAW flags, Int32 start = LIMIT<Int32>::MAX, Int32 end = LIMIT<Int32>::MIN) { return C4DOS_Br->DrawSceneHook(doc, this, bh, bt, flags, start, end); }
	...
	//-------------------------------------------------------------------------------------------------
	void LineStripBegin() { C4DOS_Br->LineStripBegin(this); }

	//-------------------------------------------------------------------------------------------------
	/// Finishes line strips started with LineStripBegin().
	//-------------------------------------------------------------------------------------------------
	void LineStripEnd() { C4DOS_Br->LineStripEnd(this); }
	...
	//----------------------------------------------------------------------------------------
	void LineStrip(const Vector& vp, const Vector& vc, Int32 flags) { C4DOS_Br->LineStrip(this, vp, vc, flags); }
	...
	//----------------------------------------------------------------------------------------
	Bool InitDrawXORPolyLine() { return C4DOS_Br->InitDrawXORLine(this); }
	...
	// @see Example in DrawXORPolyLine()
	//-------------------------------------------------------------------------------------------------
	void FreeDrawXORPolyLine() { C4DOS_Br->FreeDrawXORLine(this); }
	...
	//----------------------------------------------------------------------------------------
	void DrawXORPolyLine(const Float32* p, Int32 cnt) { C4DOS_Br->DrawXORPolyLine(this, p, cnt); }
	...
	/// @see Example in DrawXORPolyLine().
	//-------------------------------------------------------------------------------------------------
	void BeginDrawXORPolyLine() { C4DOS_Br->BeginDrawXORPolyLine(this); }
	...
	//----------------------------------------------------------------------------------------
	void EndDrawXORPolyLine(Bool blit) { C4DOS_Br->EndDrawXORPolyLine(this, blit); }
	...
	//----------------------------------------------------------------------------------------
	Bool GetHighlightPassColor(const BaseDrawHelp& bh, Bool lineObject, Vector& col) const { return C4DOS_Br->GetHighlightPassColor(this, bh, lineObject, col); }
	...
	/// @param[in] doc								The view's document. @callerOwnsPointed{document}
	//----------------------------------------------------------------------------------------
	void InitUndo(BaseDocument* doc) { C4DOS_Br->InitUndo(this, doc); }

	//----------------------------------------------------------------------------------------
	/// Performs an undo operation in the view. This is the same as the user selecting <i>Undo View</i> from within @C4D.
	/// @param[in] doc								The view's document. @callerOwnsPointed{document}
	//----------------------------------------------------------------------------------------
	void DoUndo(BaseDocument* doc) { C4DOS_Br->DoUndo(this, doc); }
	...
	//----------------------------------------------------------------------------------------
	void SetDrawParam(Int32 id, const GeData& data) { C4DOS_Br->SetDrawParam(this, id, data); }
	...
	//----------------------------------------------------------------------------------------
	GeData GetDrawParam(Int32 id) const { return C4DOS_Br->GetDrawParam(this, id); }
...
#define DRAW_PARAMETER_BGIMAGE_RATIO							1003			///< ::Float Ratio of background image, must be set by the viewport renderer.
...
#define DRAW_PARAMETER_OGL_SEMI										16
/// @}
...
#define DRAWPORT_TYPE_OGL_HQ		2			///< Viewport Effects.
/// @}
	...
	//----------------------------------------------------------------------------------------
	void AddMessageHook(BaseDrawMessageHook fn) { C4DOS_Br->AddMessageHook(this, fn); }
	...
	//----------------------------------------------------------------------------------------
	Bool AddToPostPass(BaseObject* op, BaseDrawHelp* bh) { return C4DOS_Br->AddToPostPass(this, op, bh, 0); }
	...
	//----------------------------------------------------------------------------------------
	DISPLAYFILTER GetDisplayFilter() const { return C4DOS_Br->GetDisplayFilter(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetDisplayFilter(DISPLAYFILTER displayFilter) { return C4DOS_Br->SetDisplayFilter(this, displayFilter); }

	//----------------------------------------------------------------------------------------
	/// Gets the reduction mode of the base draw.
	/// @return												The reduction mode: @enumerateEnum{DISPLAYMODE}
	//----------------------------------------------------------------------------------------
	DISPLAYMODE GetReductionMode() const { return C4DOS_Br->GetReductionMode(this); }

	//----------------------------------------------------------------------------------------
	/// Sets the current drawpass.
	/// @param[in] drawPass						The new drawpass: @enumerateEnum{DRAWPASS}
	//----------------------------------------------------------------------------------------
	void SetDrawPass(DRAWPASS drawPass) { return C4DOS_Br->SetDrawPass(this, drawPass); }

	//----------------------------------------------------------------------------------------
	/// Gets the current drawpass.
	/// @return												The current drawpass: @enumerateEnum{DRAWPASS}
	//----------------------------------------------------------------------------------------
	DRAWPASS GetDrawPass() const { return C4DOS_Br->GetDrawPass(this); }
	...
	//----------------------------------------------------------------------------------------
	Bool GetDrawStatistics(maxon::DataDictionary& statistics) const { return C4DOS_Br->GetDrawStatistics(this, statistics); }

	//----------------------------------------------------------------------------------------
	/// Gets the draw flags.
	/// @return												The draw flags.
	//----------------------------------------------------------------------------------------
	DRAWFLAGS GetDrawFlags() const { return C4DOS_Br->GetDrawFlags(this); }
	...
	//----------------------------------------------------------------------------------------
	EditorWindow* GetEditorWindow() { return C4DOS_Br->GetEditorWindow(this); }
	...
	//----------------------------------------------------------------------------------------
	void GetGridStep(Float& step, Float& fade) { C4DOS_Br->GetGridStep(this, step, fade); }
	...
	//----------------------------------------------------------------------------------------
	Vector4d ScreenToClipSpace(const Vector& screenCoordinates, Int32 sampleLocation) const { return C4DOS_Br->ScreenToClipSpace(this, screenCoordinates, sampleLocation); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsViewOpen(BaseDocument* doc) { return C4DOS_Br->IsViewOpen(this, doc); }
	...
	//----------------------------------------------------------------------------------------
	void OverrideCamera(StereoCameraInfo* si) { C4DOS_Bv->OverrideCamera(this, si); }
	...
	//----------------------------------------------------------------------------------------
	Bool PointInRange(const Vector& p, Int32 x, Int32 y, Float range = 10.0_f) const { return C4DOS_Br->PointInRange(this, p, x, y, range); }

	//----------------------------------------------------------------------------------------
	/// Offsets the OpenGL clipping planes by this offset.
	/// @param[in] o									The offset.
	//----------------------------------------------------------------------------------------
	void SetClipPlaneOffset(Float o) { C4DOS_Br->SetClipPlaneOffset(this, o); }
	...
	//----------------------------------------------------------------------------------------
	void SetTexture(BaseBitmap* bm, Bool tile, DRAW_ALPHA alphamode, DRAW_TEXTUREFLAGS flags) { return C4DOS_Br->SetTexture(this, bm, tile, alphamode, flags); }

	//----------------------------------------------------------------------------------------
	/// Gets the preferred texture flags for HUD drawing.
	/// @return												The preferred texture flags for HUD drawing.
	//----------------------------------------------------------------------------------------
	DRAW_TEXTUREFLAGS GetPreferredHUDTextureFlags() const { return C4DOS_Br->GetPreferredHUDTextureFlags(this); }
	...
	//----------------------------------------------------------------------------------------
	Float SimpleShade(const Vector& p, const Vector& n) { return C4DOS_Br->SimpleShade(this, p, n); }

	//----------------------------------------------------------------------------------------
	/// Checks for thread breaks in the draw.
	/// @return												@trueIfOtherwiseFalse{a stopping condition has occurred}
	//----------------------------------------------------------------------------------------
	Bool TestBreak() { return C4DOS_Br->TestBreak(this); }
	...
	/// @extOpenGlMode
	//----------------------------------------------------------------------------------------
	Bool IsOpenGL() const { return C4DOS_Br->IsOpenGL(this); }

	//----------------------------------------------------------------------------------------
	/// @extOpenGlMode
	//----------------------------------------------------------------------------------------
	Bool IsEnhancedOpenGL() const { return C4DOS_Br->IsEnhancedOpenGL(this); }

	//----------------------------------------------------------------------------------------
	/// @extOpenGlMode
	/// @return												@em -2: no viewport window, @em -1: off-screen view, @em 0: system error, @em 1: success.
	//----------------------------------------------------------------------------------------
	[[deprecated("use the other overload (since 2024.4)")]] Int32 GetFrameScreen(Int32* cl, Int32* ct, Int32* cr, Int32* cb) const;
	...
	//----------------------------------------------------------------------------------------
	void DrawHUDText(Int32 x, Int32 y, const maxon::String &txt) { return C4DOS_Br->DrawHUDText(this, x, y, txt); }
	...
	//----------------------------------------------------------------------------------------
	void DrawMultipleHUDText(const maxon::BaseArray<HUDTextEntry>& texts) { return C4DOS_Br->DrawMultipleHUDText(this, texts); }

	//----------------------------------------------------------------------------------------
	/// Sets the redraw helper that is used for all draw operations.
	/// @param[in] redrawHelper				The redraw helper.
	//----------------------------------------------------------------------------------------
	void SetRedrawHelper(const maxon::DrawportRedrawHelperRef& redrawHelper) { C4DOS_Br->SetRedrawHelper(this, redrawHelper); }

	//----------------------------------------------------------------------------------------
	/// Gets the redraw helper that is used for all draw operations.
	/// @return												The redraw helper.
	//----------------------------------------------------------------------------------------
	const maxon::DrawportRedrawHelperRef& GetRedrawHelper() const { return C4DOS_Br->GetRedrawHelper(this); }

	//----------------------------------------------------------------------------------------
	/// Gets the editor lights.
	/// @return												An array of editor lights.
	//----------------------------------------------------------------------------------------
	const maxon::BaseArray<EditorLight>& GetEditorLights() const { return C4DOS_Br->GetEditorLights(this); }
	...
	/// @since R19
	//----------------------------------------------------------------------------------------
	void SetViewportRender(const maxon::Id& draw, const maxon::Id& pick) { return C4DOS_Br->SetViewportRender(this, draw, pick); }
	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// @since R19
	//----------------------------------------------------------------------------------------
	void GetViewportRender(maxon::ViewportRenderRef& viewportRender, Bool draw) const { return C4DOS_Br->GetViewportRender(this, viewportRender, draw); }
	//----------------------------------------------------------------------------------------
	/// Gets the underlying drawport.
	/// @param[out] drawport					The drawport.
	//----------------------------------------------------------------------------------------
	void GetDrawport(maxon::DrawportRef& drawport) const { C4DOS_Br->GetDrawport(this, drawport); }
	...
	//----------------------------------------------------------------------------------------
	void GetViewportImage(maxon::ImageRef& viewportImage) const { C4DOS_Br->GetViewportImage(this, viewportImage); }
	...
	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Check if the BaseDraw belongs to a UV viewport.
	/// @return												@trueIfOtherwiseFalse{belongs to UV viewport }
	//----------------------------------------------------------------------------------------
	Bool IsUvViewport() const { return C4DOS_Br->IsUvViewport(this); }
};
...
{
	const ViewportPixel* next = nullptr;	///< The pointer to the next element, or @formatConstant{nullptr} if this is the last element for the pixel.
	BaseObject* op = nullptr;							///< The object.
	Float32 z = 0.0f;											///< The Z coordinate of the current element.
	Int32 i = NOTOK;											///< The element index (may be point, polygon, edge (4*polygon+edge) or spline point (segment offset + point)).
	Bool visible = true;									///< is the pixel visible with the current view settings.
};
...
public:
	...
	/// @copydoc Alloc
	static ViewportSelect* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	const ViewportPixel* GetNearestPoint(BaseObject* op, Int32& x, Int32& y, Int32 maxrad = LIMIT<Int32>::MAX, Bool onlyselected = false, Int32* ignorelist = nullptr, Int32 ignorecnt = 0, Bool onlyVisible = false) const;
	...
	//----------------------------------------------------------------------------------------
	const ViewportPixel* GetNearestEdge(BaseObject* op, Int32& x, Int32& y, Int32 maxrad = LIMIT<Int32>::MAX, Bool onlyselected = false, Int32* ignorelist = nullptr, Int32 ignorecnt = 0, Bool onlyVisible = false) const;
