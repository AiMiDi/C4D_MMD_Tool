...
#define GradientCall(b) 			cinema::CustomGuiGradientLib *lib = cinema::CheckGradientLib(LIBOFFSET(cinema::CustomGuiGradientLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iGradient*>(this)->*lib->b)

#define GradientCallC(b) 			cinema::CustomGuiGradientLib *lib = cinema::CheckGradientLib(LIBOFFSET(cinema::CustomGuiGradientLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const cinema::iGradient*>(this)->*lib->b)

#define GradientCallR(a, b) 		cinema::CustomGuiGradientLib *lib = cinema::CheckGradientLib(LIBOFFSET(cinema::CustomGuiGradientLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iGradient*>(this)->*lib->b)

#define GradientCallRC(a, b) 		cinema::CustomGuiGradientLib *lib = cinema::CheckGradientLib(LIBOFFSET(cinema::CustomGuiGradientLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const cinema::iGradient*>(this)->*lib->b)

#define GradientGuiCall(b) 		cinema::CustomGuiGradientLib *lib = cinema::CheckGradientLib(LIBOFFSET(cinema::CustomGuiGradientLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<cinema::iGradientCustomGui*>(this)->*lib->b)

#define GradientGuiCallR(a, b) cinema::CustomGuiGradientLib *lib = cinema::CheckGradientLib(LIBOFFSET(cinema::CustomGuiGradientLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<cinema::iGradientCustomGui*>(this)->*lib->b)
...
maxon::Result<maxon::GradientRenderData> Gradient::PrepareRenderData() const
{
	GradientCallRC(maxon::UnexpectedError(MAXON_SOURCE_LOCATION, "Gradient::PrepareRenderData() not found."_s), PrepareRenderData2)();
}

maxon::Result<GradientRenderDataTuple> Gradient::PrepareRenderDataWithAlpha() const
{
	GradientCallRC(maxon::UnexpectedError(MAXON_SOURCE_LOCATION, "Gradient::PrepareRenderDataWithAlpha() not found."_s), PrepareRenderDataWithAlpha2)();
}
