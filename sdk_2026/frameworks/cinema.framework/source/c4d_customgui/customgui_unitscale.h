...
//----------------------------------------------------------------------------------------
class UnitScaleData : public CustomDataTypeT<CUSTOMDATATYPE_UNITSCALE>
{
private:
	UnitScaleData();
	~UnitScaleData();
public:
	...
	/// @copydoc Alloc
	static UnitScaleData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
// INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF
	...
	class CINEWARE_SINGLEINHERITANCE iUnitScaleCustomGui : public iBaseCustomGui
	{
		iUnitScaleCustomGui(const BaseContainer &settings, CUSTOMGUIPLUGIN *plugin) : iBaseCustomGui(settings, plugin) { }
	};
	class CINEWARE_SINGLEINHERITANCE iUnitScaleData {};
#else
...
struct UnitScaleCustomGuiLib : public BaseCustomGuiLib
{
	...
	String					(iUnitScaleData::*ToUnitString)() const;
};
