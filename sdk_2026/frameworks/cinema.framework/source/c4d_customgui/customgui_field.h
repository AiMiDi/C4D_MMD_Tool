...
enum class FIELDLIST_FLAGS
{
	...
	SIMPLEGUI									= (1 << 7),		///< Use a simplified GUI, this is only possible if there is only a single standard field layer in the datatype
...
//----------------------------------------------------------------------------------------
class FieldList : public CustomDataTypeT<CUSTOMDATATYPE_FIELDLIST>
{
	/// Default constructor.
	FieldList() = default;
	/// Destructor.
...
public:
	...
	/// @copydoc Alloc
	static FieldList* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	/// Use this method to avoid to avoid having to pre create FieldInfo and FieldOutput structures.
	...
	maxon::Result<Bool> GetAccessedObjects(const BaseList2D* caller, METHOD_ID method, AccessedObjectsCallback& access) const;
};
...
class CINEWARE_SINGLEINHERITANCE iFieldCustomDatatype
{
};
#else
...
{
	maxon::Result<FieldOutput>(iFieldCustomDatatype::*SampleListSimple		)(const BaseList2D& caller, const FieldInput& inputs, FIELDSAMPLE_FLAG flags) const;
	maxon::Result<void>				(iFieldCustomDatatype::*SampleList					)(const FieldInfo& info, const FieldInput& inputs, FieldOutput& outputs) const;
	maxon::Result<void>				(iFieldCustomDatatype::*SampleListWithLambda)(const FieldInfo& info, const FieldLambdaInput&& inputFunc, const FieldLambdaOutput&& outputFunc) const;

	maxon::Result<maxon::GenericData>				(iFieldCustomDatatype::*DirectInitSampling	)(const FieldInfo& info) const;
	maxon::Result<void>				(iFieldCustomDatatype::*DirectSample				)(const FieldInput& inputs, FieldOutputBlock& outputs, const FieldInfo& info, const maxon::GenericData& extraData) const;
	void											(iFieldCustomDatatype::*DirectFreeSampling	)(const FieldInfo& info, maxon::GenericData& extraData) const;
	maxon::Result<void>				(iFieldCustomDatatype::*ForEachLayer				)(const maxon::Delegate<maxon::Result<void>(const FieldLayer* layer, const maxon::GenericData& extra)>& callback, const maxon::GenericData& listData) const;
	UInt32										(iFieldCustomDatatype::*GetDirty						)(const BaseDocument* doc) const;
	...
	GeListHead*								(iFieldCustomDatatype::*GetLayersRootWritable)(Bool create);
	const GeListHead*					(iFieldCustomDatatype::*GetLayersRootConst  )() const;
	...
	maxon::Result<Bool>				(iFieldCustomDatatype::*GetAccessedObjects	)(const BaseList2D* caller, METHOD_ID method, AccessedObjectsCallback& access) const;
};
