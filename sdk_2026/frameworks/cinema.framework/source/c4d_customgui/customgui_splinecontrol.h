...
enum
{
	...
	SPLINECONTROL_CUSTOMCOLOR_COL,						///< The custom color. RGB range: [0.0, 1.0].
...
//-------------------------------------------------------------------------------------------------
class SplineData : public CustomDataTypeT<CUSTOMDATATYPE_SPLINE>
{
private:
	SplineData();
	~SplineData();
public:
	...
	/// @copydoc Alloc
	static SplineData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	Bool CopyTo(SplineData* pDest) const;
	...
	//----------------------------------------------------------------------------------------
	/// Get the SplineMapper object for this spline.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> GetSplineMapper(maxon::SplineMapper& splineMapper) const;
...
class CINEWARE_SINGLEINHERITANCE SplineControlMessagesF
{
public:
...
struct SplineControlFunctionTable
{
	...
	class CINEWARE_SINGLEINHERITANCE iSplineCustomGui
	{
	};
	class CINEWARE_SINGLEINHERITANCE iSpline
	{
	};
#else
...
struct CustomGuiSplineLib : public BaseCustomGuiLib
{
	...
	SplineData*					(iSplineCustomGui::*GetSplineData)();
...
//Bool								(iSpline::*SetBackgoundImage)(BaseBitmap* pBmp);
	...
	Bool								(iSpline::*CopyTo)(SplineData* pDest) const;
	...
	maxon::Result<void>	(iSpline::*GetSplineMapper)(maxon::SplineMapper& splineMapper) const;
};
