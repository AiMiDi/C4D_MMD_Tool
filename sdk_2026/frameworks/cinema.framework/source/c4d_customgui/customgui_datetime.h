...
//----------------------------------------------------------------------------------------
class DateTimeData : public CustomDataTypeT<DATETIME_DATA>
{
private:
	DateTimeData();
	~DateTimeData();
...
public:
	...
	/// @copydoc Alloc
	static DateTimeData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
// INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF
	...
	class CINEWARE_SINGLEINHERITANCE iDateTimeControl : public iBaseCustomGui
	{
		iDateTimeControl(const BaseContainer &settings, CUSTOMGUIPLUGIN *plugin) : iBaseCustomGui(settings, plugin) { }
	};
	class CINEWARE_SINGLEINHERITANCE iDateTimeData
	{
	};
#else
