...
/// @{
#define TREEVIEW_BORDER								'brdr'			///< ::Bool Set to @formatConstant{true} to show an outer tree border.
#define TREEVIEW_OUTSIDE_DROP					'tvod'			///< ::Bool Set to @formatConstant{true} if an object may be dropped under all the objects in the tree view.
#define TREEVIEW_HIDE_LINES						'tvhl'			///< ::Bool Set to @formatConstant{true} if no lines should be drawn (flat list, no hierarchy indents used).
#define TREEVIEW_CTRL_DRAG						'ctdr'			///< ::Bool Set to @formatConstant{true} if item may be duplicated by Ctrl + Drag.
#define TREEVIEW_NO_MULTISELECT				'tvnm'			///< ::Bool Set to @formatConstant{true} if no multiple selection is allowed.
#define TREEVIEW_HAS_HEADER						'tvhh'			///< ::Bool Set to @formatConstant{true} if the tree view may have a header line.
#define TREEVIEW_RESIZE_HEADER				'tvrh'			///< ::Bool Set to @formatConstant{true} if the column width can be changed by the user.
#define TREEVIEW_MOVE_COLUMN					'tvmc'			///< ::Bool Set to @formatConstant{true} if the user can move the columns.
#define TREEVIEW_FIXED_LAYOUT					'tvfl'			///< ::Bool Set to @formatConstant{true} if all lines have the same height.
#define TREEVIEW_NOAUTOCOLUMNS				'noau'			///< ::Bool Set to @formatConstant{true} if only the first line is asked for the columns width, resulting in a huge speedup.
#define TREEVIEW_NO_OPEN_CTRLCLK			'tvcd'			///< ::Bool Set to @formatConstant{true} if it is not allowed to open the complete tree with Ctrl + Click.
#define TREEVIEW_ALT_DRAG							'tvad'			///< ::Bool Set to @formatConstant{true} if Alt should be used instead of Ctrl for drag and drop; implies item may be duplicated by Alt + Drag.
#define TREEVIEW_NO_BACK_DELETE				'tvnb'			///< ::Bool Set to @formatConstant{true} to disable "delete pressed" messages if backspace was hit.
#define TREEVIEW_NO_DELETE						'tvnd'			///< ::Bool Set to @formatConstant{true} to disable Delete Message Callback completely for backspace and delete.
#define TREEVIEW_ALTERNATE_BG					'tvab'			///< ::Bool Set to @formatConstant{true} to alternate background color per line.
#define TREEVIEW_CURSORKEYS						'tvck'			///< ::Bool Set to @formatConstant{true} if cursor keys should be processed. @b Note: The focus item has to be set to @formatConstant{nullptr} if it is deleted and this flag is set.
#define TREEVIEW_NOENTERRENAME				'tver'			///< ::Bool Set to @formatConstant{true} to suppress the rename popup when the user presses enter.
#define TREEVIEW_NO_VERTICALSCROLL		'novs'			///< ::Bool Set to @formatConstant{true} to disable vertical scrolling and show the full list.
#define TREEVIEW_ADDROW								'adrw'			///< ::Bool Set to @formatConstant{true} to show an add new column row at the bottom of the list.
#define TREEVIEW_RESIZABLE						'rsiz'			///< ::Bool Set to @formatConstant{true} to make treeview resizable by dragging the bottom edge.
#define TREEVIEW_CUSTOM_BG_COLOR			'cubg'			///< Custom background color. Can be an Int32 color ID (@see c4d_colors.h) or a Vector color in range [0.0, 1.0]. If unset, default application background color will be used. @b Notes: Only is applied if TREEVIEW_ALTERNATE_BG is @formatConstant{false}; TreeViewFunctions::GetBackgroundColor() overrides it. @since R23
#define TREEVIEW_VERTICAL_SPACE     	'spcy'			///< ::Int32 Vertical space between lines in pixels.
...
#define TREEVIEW_POPUP_EDIT_NOTIFY		'tvpn'			///< ::Bool Set to @formatConstant{true} to get notified when an edit popup is opened and closed in the @ref GeDialog::Message() function. Example of use:
																									...
																									/// @endcode
																									///< @since 2025.300
#define TREEVIEW_EMPTY_TEXT_GREYED_OUT	'mptg'		///< ::Bool Set to @formatConstant{true} to draw greyed out empty text.
/// @}
...
#define LV_USERTREE			'ustr'			///< %User tree. If set, the tree view calls TreeViewFunctions::DrawCell() for the left cell of each line (otherwise it would display the object's name in the cell). TreeViewFunctions::GetLineHeight() needs to be defined as well.
...
class TreeViewFunctions
{
	public:
		...
		/// @return												The bubble help of the object. Mouse cursor change is supported by adding this suffix: "&mXX&" where XX is a mouse cursor ID: @enumerateEnum{MOUSE}. Example:
		/// @code
...
/// @}
		...
		/// @param[in] obj								An object in the tree.
		///
		/// @return												@trueIfOtherwiseFalse{The object is selectable}
		//----------------------------------------------------------------------------------------
		...
		//----------------------------------------------------------------------------------------
		/// StartInput is called before input loop starts.
		...
		/// @param[in] root								The tree root.
		/// @param[in] userdata						The userdata.
		/// @param[in] data								Data generated in StartInput()
		//----------------------------------------------------------------------------------------
...
class CINEWARE_SINGLEINHERITANCE TreeViewF
{
public:
	...
	String	GetBubbleHelp(TreeViewFunctions* pFuncs, void* root, void* userdata, void* obj, Int32 col, MouseInfo* info);
	...
	maxon::Result<maxon::Data> StartInput(TreeViewFunctions* pFuncs, void* root, void* userdata);
	void FinishInput(TreeViewFunctions* pFuncs, void* root, void* userdata, maxon::Data& obj);
};
...
struct InternalFunctionTable
{
	...
	/*54*/ String		(TreeViewF::*GetBubbleHelp)(TreeViewFunctions* pFuncs, void* root, void* userdata, void* obj, Int32 col, MouseInfo* info);
	...
	/*58*/ maxon::Result<maxon::Data> (TreeViewF::*StartInput)(TreeViewFunctions* pFuncs, void* root, void* userdata);
	/*59*/ void (TreeViewF::*FinishInput)(TreeViewFunctions* pFuncs, void* root, void* userdata, maxon::Data& obj);
};
...
class TreeViewCustomGui : public BaseCustomGui<CUSTOMGUI_TREEVIEW>
{
	...
	public:
		...
		/// @param[in] show								Set to @formatConstant{true} to show the lines, to @formatConstant{false} to hide them.
		//----------------------------------------------------------------------------------------
		void SetShowLines(Bool show);

		//----------------------------------------------------------------------------------------
		/// Shows/hides the tree hierarchical lines.
		/// @return												@formatConstant{true} if lines are visible, @formatConstant{false} if are hidden.
		//----------------------------------------------------------------------------------------
		Bool GetShowLines() const;

		//----------------------------------------------------------------------------------------
		/// Gets the line and column hit at coordinates. Input coordinates must be on screen space.
		/// @param[in] screenX						The x coordinate in screen space.
		/// @param[in] screenY						The y coordinate in screen space.
		/// @param[in] lineHit						The line hit.
		/// @param[in] columnHit					The column hit.
		/// @return												@formatConstant{true} if a valid element is hit, otherwise @formatConstant{false}.
		//----------------------------------------------------------------------------------------
...
struct TreeViewLib : public BaseCustomGuiLib
{
	...
	void					(iTreeViewCustomGui::*SetShowLines)(Bool show);
	Bool 					(iTreeViewCustomGui::*GetShowLines)() const;
	Bool					(iTreeViewCustomGui::*GetLineAndColumnHit)(Int32 screenX, Int32 screenY, Int32& lineHit, Int32& columnHit);
};
