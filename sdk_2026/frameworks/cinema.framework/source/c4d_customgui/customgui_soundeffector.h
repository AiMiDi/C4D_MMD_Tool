...
/// @since R19
//-------------------------------------------------------------------------------------------------
class SoundEffectorData : public CustomDataTypeT<CUSTOMDATATYPE_SOUNDEFFECTOR>
{
private:
 ...
 public:
	...
	/// @copydoc Alloc
	static SoundEffectorData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
class CINEWARE_SINGLEINHERITANCE iSoundData;
class CINEWARE_SINGLEINHERITANCE iSoundDataCustomGui;
class CINEWARE_SINGLEINHERITANCE iProbe;
...
struct CustomGuiSoundDataLib : public BaseCustomGuiLib
{
	...
	Bool								(iSoundData::*CopyTo									)(SoundEffectorData* pDest) const;
	...
	const Gradient*			(iSoundData::*GetGradient							)() const;
	...
	Bool								(iSoundData::*IsFFTSamplingCacheDirty)(const BaseDocument& doc) const; // R20
};
