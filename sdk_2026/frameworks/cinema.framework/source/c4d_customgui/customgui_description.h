...
#define DESCRIPTION_ALLOW_DROPDOWN_FILTER	'aldf'		///< ::Bool Enables drop down widgets in a dialog to perform filtering based on typed string.
...
// INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF -- INTERNAL STUFF
	...
	class CINEWARE_SINGLEINHERITANCE iDescriptionCustomGui
	{
	};
#else
...
struct DescriptionGuiLib : public BaseCustomGuiLib
{
	...
	void			(iDescriptionCustomGui::*SetTranslation)(const maxon::Delegate<Bool(C4DAtom* node, const DescID& id, DescID& resultId, C4DAtom*& resultAtom)>& func, Bool objectControl);
