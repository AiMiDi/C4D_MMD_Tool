...
{
	// C4DOS_Ge could be nullptr if futurama part is loaded and but not the Cinema API
	if (C4DOS_Ge == nullptr)
		return nullptr;
...
{
	return C4DOS_Ge->ClearPythonConsole();
}
...
{
	DebugAssert((int(type) & int(CHECKISRUNNING::WAITFOR)) == 0, "Call WaitWhileRunning");
	return C4DOS_Ge->CheckIsRunning(type);
}

Bool WaitWhileRunning(CHECKISRUNNING type)
{
	type = CHECKISRUNNING(int(type) | int(CHECKISRUNNING::WAITFOR));
	return C4DOS_Ge->CheckIsRunning(type);
}
