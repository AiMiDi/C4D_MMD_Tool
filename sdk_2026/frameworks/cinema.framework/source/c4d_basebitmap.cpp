...
{
	// MovieLoader is an AllocType, hence NewObj would lead to a recursion.
	MovieLoader* m = static_cast<MovieLoader*>(maxon::DefaultAllocator::Alloc(SIZEOF(MovieLoader), MAXON_SOURCE_LOCATION));
	if (m)
		new (m) MovieLoader();
	return m;
}
...
{
	// MovieLoader is an AllocType, hence DeleteObj would lead to a recursion.
	if (ml)
	{
		ml->~MovieLoader();
		maxon::DefaultAllocator::Free(ml);
	}
}
...
#define CallBmLoader(fnc) (static_cast<cinema::BitmapLoaderData*>((static_cast<cinema::BITMAPLOADERPLUGIN*>(GetPluginStructure())->adr))->*static_cast<cinema::BITMAPLOADERPLUGIN*>(GetPluginStructure())->fnc)
#define CallBmSaver(fnc)	(static_cast<cinema::BitmapSaverData*>((static_cast<cinema::BITMAPSAVERPLUGIN*>(GetPluginStructure())->adr))->*static_cast<cinema::BITMAPSAVERPLUGIN*>(GetPluginStructure())->fnc)
...
maxon::HashInt ColorProfile::GetHashCode() const
{
	return C4DOS_Bm->ProfileGetHashCode(this);
}
