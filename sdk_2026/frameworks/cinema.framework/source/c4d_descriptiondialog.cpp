...
{
	firstaction = false;
	return cinema::InteractiveModeling_Restart(doc);
}
...
Bool DescriptionToolData::Message(BaseDocument* doc, BaseContainer& data, Int32 type, void* t_data)
{
	...
	if (type == MSG_DESCRIPTION_CHECKUPDATE)
	{
		...
		if (dirty != last_dirty && !firstaction && interactive)
		{
			...
			if (doc->GetActiveTexBaseDraw()) // will return nullptr if new uv editor is disabled.
			{
				doc->GetActiveUVWObjects(*pActive, GETACTIVEUVWOBJECTSMODE::EDITABLE);
			}
			else
			{
			...
			md.arr = pActive;
			md.doc = doc;
			md.flags = MODELINGCOMMANDFLAGS::CREATEUNDO | MODELINGCOMMANDFLAGS::SETESCTESTTHREAD;
