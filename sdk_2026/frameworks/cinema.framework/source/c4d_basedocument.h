...
/// @see SceneHookData
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE BaseSceneHook : public BaseList2D
{
private:
	BaseSceneHook();
	~BaseSceneHook();
...
public:
	...
	/// @copydoc Alloc
	static RenderData* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	const BaseVideoPost* GetFirstVideoPost() const;
	...
	/// @param[out] width							The image output width.
	/// @param[out] height						The image output height.
	/// @param[out] pixelAspect				The ratio of a pixel's on-screen width to it's on-screen height.
	/// @param[out] filmAspect				The X-to-Y aspect ratio of the rendered image.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] width							The image output width.
	/// @param[in] height							The image output height.
...
public:
	...
	/// @copydoc Alloc
	static LayerObject* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
/// @addAllocFreeAutoAllocNote
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE BaseDocument : public BaseList2D
{
private:
	BaseDocument();
	~BaseDocument();
...
public:
	...
	/// @copydoc Alloc
	static BaseDocument* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	const BaseDocument* GetNext() const { return static_cast<const BaseDocument*>(AtCall(GetNext)()); }

	//----------------------------------------------------------------------------------------
	/// Gets the next document in the list. Convenience version of GeListNode::GetNext() returning a pointer to a BaseDocument.
	/// @return												The next document, or @formatConstant{nullptr} if there is none. @cinemaOwnsPointed{document}
	//----------------------------------------------------------------------------------------
	BaseDocument* GetNext() { return const_cast<BaseDocument*>(static_cast<const BaseDocument*>(AtCall(GetNext)())); }
	...
	//----------------------------------------------------------------------------------------
	const BaseDocument* GetPred() const { return static_cast<const BaseDocument*>(AtCall(GetPred)()); }

	//----------------------------------------------------------------------------------------
	/// Gets the previous document in the list. Convenience version of GeListNode::GetNext() returning a pointer to a BaseDocument.
	/// @return												The previous document, or @formatConstant{nullptr} if there is none. @cinemaOwnsPointed{document}
	//----------------------------------------------------------------------------------------
	BaseDocument* GetPred() { return const_cast<BaseDocument*>(static_cast<const BaseDocument*>(AtCall(GetPred)())); }
	...
	/// @param[in] checkNames					If @formatConstant{true}, it avoids duplicated names by prepending an incremental number to the name when needed.
	...
	/// @return												The scene camera of the active BaseDraw or @formatConstant{nullptr} if there is no active view.
	//----------------------------------------------------------------------------------------
	BaseObject* GetSceneCamera() const;

	//----------------------------------------------------------------------------------------
	/// Sets the scene camera of the active BaseDraw.
	/// @return												@trueIfOtherwiseFalse{the camera was set successfully}
	//----------------------------------------------------------------------------------------
	...
	/// @return												Reference to a repository on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<const maxon::UpdatableAssetRepositoryRef&> GetSceneRepository() const;
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a converter to convert colors along the OCIO conversion paths defined by the settings of this document.
	/// @details The returned value is never nullptr. If OCIO is not active or if there was an error to create the OCIO converter a dummy is returned.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a converter to convert colors along the OCIO conversion paths defined by the currently active document.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a converter to convert thumbnail colors along the OCIO conversion paths defined by the settings of this document.
	/// @details The returned value is never nullptr. If OCIO is not active or if there was an error to create the OCIO converter a dummy is returned.
	/// @return												The color converter that belongs to the document for thumbnails.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the active UVW tags data represented as PolygonObjects
	/// @param[out] selection					Filled with the selected UVW objects.
	/// @param[in] mode								Mode: @enumerateEnum{GETACTIVEUVWOBJECTSMODE}
	//----------------------------------------------------------------------------------------
	void GetActiveUVWObjects(AtomArray& selection, GETACTIVEUVWOBJECTSMODE mode) const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the texture view BaseDraw from one of the editor views.
	/// @param[in] num								If there are multiple views this selects the view. @em 0 <= @formatParam{num} < GetTexBaseDrawCount()
	/// @return												The requested Texture view in the editor, or @formatConstant{nullptr} if it is not available. @theOwnsPointed{document,view}
	//----------------------------------------------------------------------------------------
	BaseDraw* GetTexBaseDraw(Int32 num) const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the number of texture views in the editor.
	/// @note While this function may sound trivial and cheap, internally it is not. For performance reasons, it is not recommended to use this function inside a loop or inside a loop condition.
	/// @return												The BaseDraw count in the editor.
	//----------------------------------------------------------------------------------------
	Int32 GetTexBaseDrawCount() const;

	//----------------------------------------------------------------------------------------
	/// @markPrivate
	/// Gets the active texture view BaseDraw from one of the editor views.
	/// @return												The active texture view in the editor or @formatConstant{nullptr} if there is no active texture view. @theOwnsPointed{document,view}
	//----------------------------------------------------------------------------------------
	BaseDraw* GetActiveTexBaseDraw() const;
};
...
// flag that can be passed to SetActiveTag to prevent an update of the Attribute Manager
const Int32 SELECTION_NOMANAGERUPDATE = (1 << 31);
