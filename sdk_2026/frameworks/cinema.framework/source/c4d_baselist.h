...
namespace cinema
{
	#define AtCall(fnc) (this->*cinema::C4DOS_At->fnc)
...
namespace cinema
{
	...
	#define INSTANCEOF(X, Y)								\
		public:																\
			typedef Y SUPER;										\
			template <typename... ARGS> static X* Alloc(const maxon::SourceLocation&, ARGS&&... args) { return static_cast<X*>(Alloc(std::forward<ARGS>(args)...)); }  /* Add support for UniqueRef */  \
		private:
	// macros for instanceof

	#define AtCall(fnc) (this->*cinema::C4DOS_At->fnc)
	#define BoCall(fnc) (this->*cinema::C4DOS_Bo->fnc)
	#define BlCall(fnc) (this->*cinema::C4DOS_Bl->fnc)
...
enum class MODIFIER_FLAG : Int64
{
	...
	NONE = 0                           ///< No flags.
} MAXON_ENUM_FLAGS(MODIFIER_FLAG);
/// @}

/// Message structure for the @ref MSG_GETMODIFIERFLAGS message. @markPrivate
struct ModifierFlagsMsgData
{
	MODIFIER_FLAG flags = MODIFIER_FLAG::NONE;
};
...
#define MSG_DESCRIPTION_CHECKUPDATE_SYNC_BLOCKING	(1 << 25)			///< when MSG_DESCRIPTION_CHECKUPDATE_SYNC is set, this forces the refresh to be synchronous to the value edit.
/// @}
...
#define MSG_DESCRIPTION_IMPEXPORT_VERIFY_FILENAME					1060032				///< Same as MSG_DESCRIPTION_IMPEXPORT_VERIFY_SUFFIX for the default Filename offered in the file dialog. @correspondingData{DescriptionImExportVerifyFilename}
...
#define MSG_GETCUSTOM_NAME_ADDITION								1060371			///< Sent to a BaseList2D to retrieve a custom name addition. The atoms needs to be registered with OBJECT_CUSTOM_NAME_ADDITION. @correspondingData{String}
...
#define MSG_CHECKINSIDEUNDO												1063385			///< Check if the document is currently processing an undo. This might be important to skip adding undos for certain events in case the events where triggered by DoUndo itself. @correspondingData{Bool} @since 2024.5
...
#define MSG_REQUESTISGLOBALSPACEMODIFIER					1066076			///< @since 2026.1 - sent to a deformer in case the object info flag is registered with OBJECT_POTENTIALLY_GLOBALSPACEMODIFIER. If returned true, the deformer will regenerate with global matrix change of the object. @correspondingData{Bool}
...
struct PreSaveValidationStruct
{
	...
	explicit PreSaveValidationStruct(SAVEDOCUMENTFLAGS flags) : _flags(flags) {}

	/// Save flags.
	const SAVEDOCUMENTFLAGS _flags;
	using DenyEntry = maxon::Tuple<BaseList2D*, maxon::Data /* Asset type or reason */, maxon::Id /* Asset identifier */, maxon::Url /* Asset URL */>;
	...
	maxon::BaseArray<DenyEntry> _denyList;
...
#define MSG_GETMODIFIERFLAGS											1060370				///< @markPrivate @since 2023.200
#define MSG_ADDFIELDDIRTY													1059923				///< @markPrivate
#define MSG_CLEARVARIABLETAG											1059935				///< @markPrivate
...
#define MSG_MODIFYTAG_CHECKDIRTY									1066407				///< This message is send to modifier tags before dirty checks for it to evaluate if it needs to set itself dirty.
...
#define MSG_MULTI_CLEARSUGGESTEDFOLDER	200000040						///< Objects have to remove absolute paths (only if they support @ref MSG_GETALLASSETS).\n
																														...
																														///< @see MSG_GETALLASSETS MSG_RENAMETEXTURES. Struct ModifyFilenameDataStruct will be sent with the message. Call HandleModifyFilenameMessage() to process the data correctly by the c4d api
#define MSG_MULTI_SCRIPTINFO 						1061310							///< Sent to nodes to gather information about the scripting content they host in their parameters. See the correspoding message data `ScriptInfoData` for more information on how to handle this message.
...
#define MSG_INVALIDATEHASH						1062551				///< Triggers a data hash recalculation in variable tags. Can be used to refresh the hash without triggering a COW make writable. Use with caution.
...
#define MSG_DOCUMENT_CONVERT_COLORS		1059361				///< Sent when converting the document colors from sRGB to rendering space
...
/// }
/// @endcode
/// If the material link is stored in a BaseContainer, you can also use the convenience function HandleMaterialInContainer
/// which handles both translation and marking.
//----------------------------------------------------------------------------------------
struct MarkMaterials
{
	...
	BaseMaterial* omat;														///< Original material. If this is nullptr, all materials shall be collected in the materials set @cinemaOwnsPointed{material}
	BaseMaterial* nmat;														///< New material. @cinemaOwnsPointed{material}
	maxon::HashSet<BaseMaterial*> materials;			///< All found materials, only used when omat is nullptr.

	static Bool IsTranslation(const void* msgData) { return msgData && static_cast<const MarkMaterials*>(msgData)->omat != nullptr; }
...
{
	const DescID* descid = nullptr;						///< The description ID of the value set.
	DESCFLAGS_SET flags = DESCFLAGS_SET::NONE; ///< The flags of the value set.
};
...
/// 	BaseContainer& data = abc->GetDataInstanceRef();
///
...
/// @brief Describes the script contexts that can be conveyed by `ScriptInfoData` events.
...
} MAXON_ENUM_LIST(SCRIPT_CONTEXT);

/// @brief Describes scripting languages that can be conveyed by `ScriptInfoData` events.
...
} MAXON_ENUM_LIST(SCRIPT_LANGUAGE);

/// @brief Broadcasted alongside the message `MSG_MULTI_SCRIPTINFO` to query scene elements for scripts they contain and might execute.
/// @details The message is usually broadcasted at a document level with `C4DAtom::MultiMessage` to collect information for a whole document:
/// ```
/// BaseDocument* doc;
/// ScriptInfoData infoData;
/// doc->MultiMessage(MULTIMSG_ROUTE::BROADCAST, MSG_MULTI_SCRIPTINFO, &infoData);
...
///       item.language = SCRIPTINFO_LANGUAGE::PYTHON;
///       item.context = SCRIPTINFO_CONTEXT::UNKNOWN;
...
///       item.isExecuted = bc.GetBool(ID_BASEOBJECT_GENERATOR_FLAG);
///
...
{
	/// @brief Describes a singular script parameter held by a node, e.g., the `OPYTHON_CODE` parameter of `Opython`.
	/// @details Nodes that hold more then one script, e.g., the Sketch & Toon material which holds scripts for its color, opacity, and
	/// thickness channels, will yield multiple  `ScriptInfoItem` upon being queried.
	/// @note Due to the inner makeup of `Tcavisualselector`, it is for users of the public API only possible to access the Python code of
	/// the *active* hot spot. But when `MSG_MULTI_SCRIPTINFO` is being broadcasted to a `Tcavisualselector` tag, it will collect
	/// information for all hot spots, not only the active one. This makes it impossible for users of the public API to retrieve the associated
	/// `Tcavisualselector` code for `ScriptInfoItem` event data in this case.
	struct ScriptInfoItem
	{
		ScriptInfoItem(): node(nullptr), pid(nullptr), language(SCRIPT_LANGUAGE::UNKOWN),
											context(SCRIPT_CONTEXT::UNKNOWN), isDefault(false), isExecuted(true) {}

		BaseList2D* node;         ///< The node owning the code, only meaningful when broadcasted as a multi-message to multiple nodes.
		DescID* pid;              ///< The identifier of the parameter which holds the script code. @note Unreliable for `Tcavisualselector`, see class description for details.
		SCRIPT_LANGUAGE language; ///< The scripting language the code is written in, as for example CPython.
		SCRIPT_CONTEXT context;   ///< The context in which the script is used, as for example a Python Generator object.
		Bool isDefault;           ///< If the code is the default code of #node.
		Bool isExecuted;          ///< If #node is in a state where the code at #pid will be executed on node evaluation.
	};
...
{
	Int32 type = 0;								///< The message type: @enumerateEnum{MSG_DOCUMENTINFO_TYPE}
	Int32 fileformat = 0;					///< The file format: (Set for load, merge and before/after save only. Otherwise @ref NOTOK.) @enumerateEnum{FORMAT_FILES}
	BaseDocument* doc = nullptr;	///< The current document. @cinemaOwnsPointed{document}
	...
	BaseList2D* bl = nullptr;			///< The object/tag/material that got inserted. It is used if @ref type is of one of the following values:
																///< - @ref MSG_DOCUMENTINFO_TYPE_OBJECT_INSERT
																///< - @ref MSG_DOCUMENTINFO_TYPE_TAG_INSERT
																///< - @ref MSG_DOCUMENTINFO_TYPE_MATERIAL_INSERT
	Bool gui_allowed = false;			///< It is set to @formatConstant{true} if a GUI (dialog) is allowed.\n
	...
	void* data = nullptr;					///< A pointer to message data structure. Depends on @ref type.
	Bool logUsage = false;				///< If set to @formatConstant{true} the import format of loaded files is logged into the analytics. @since 2024.300
	Bool autoSave = false;				///< It is set to @formatConstant{true} on autosave. @since 2025.000
};
...
public:
	...
	{
		return _filename == e._filename &&
					 _assetname == e._assetname &&
					 _channelId == e._channelId &&
					 _netRequestOnDemand == e._netRequestOnDemand &&
					 _owner == e._owner &&
					 _exists == e._exists &&
					 _paramId == e._paramId &&
					 _nodePath == e._nodePath &&
					 _nodeSpace == e._nodeSpace &&
...
struct AssetData
{
	...
	//----------------------------------------------------------------------------------------
	AssetData(const BaseDocument* doc, RootTextureString* rs, ASSETDATA_FLAG flags) : _doc(doc), _rs(rs), _flags(flags), _override(nullptr), _channelId(NOTOK) { }

	const BaseDocument*	_doc = nullptr;									///< The document.
	...
	//----------------------------------------------------------------------------------------
	/// Replaces all elements with matching oldElement owner and oldId with the respective new parameters.
	/// @param[in] oldOwner										Previous Asset Owner
	/// @param[in] newOwner										New Asset Owner		
	/// @param[in] oldId											Previous ParamID
	/// @param[in] newId											New ParamID
	/// @param[in] startNodeSearch						Start the replacing operation from this node (this skips earlier GeListNodes).
	/// @return																True if the element was replaced.
	//----------------------------------------------------------------------------------------
	Bool ReplaceElement(BaseList2D* oldOwner, BaseList2D* newOwner, Int32 oldId, Int32 newId, GeListNode* startNodeSearch = nullptr);
};
...
/// Message struct for the @ref MSG_DOCUMENT_CONVERT_COLORS message.
struct MessageDocumentConvertColors
{
	...
	ColorProfileConvertInterface* _convertCpu;			///< The already setup OCIO Color Converter.
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	static GeMarker* Alloc() { return (*C4DOS_At->GeMarkerAlloc)(); }

	/// @copydoc Alloc
	static GeMarker* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	static void Free(GeMarker*& obj) { (*C4DOS_At->GeMarkerFree)(obj); }
...
public:
	...
	/// @return												The type, for example @em Ocube.
	//----------------------------------------------------------------------------------------
	Int32 GetDiskType() const { return C4DOS_Bl->GetDiskType(this); }
	...
	//----------------------------------------------------------------------------------------
	C4DAtom* GetClone(COPYFLAGS flags, AliasTrans* trn) const { return AtCall(GetClone) (flags, trn); }
	...
	//----------------------------------------------------------------------------------------
	Bool CopyTo(C4DAtom* dst, COPYFLAGS flags, AliasTrans* trn) const { return AtCall(CopyTo) (dst, flags, trn); }
	...
	//----------------------------------------------------------------------------------------
	UInt32 GetDirty(DIRTYFLAGS flags) const { return C4DOS_Bo->GetDirty(this, flags); }
	...
	/// @param[in] flags							Flags: @enumerateEnum{DIRTYFLAGS}
	//----------------------------------------------------------------------------------------
	void SetDirty(DIRTYFLAGS flags) { C4DOS_Bo->SetDirty(this, flags); }
...
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE C4DAtomGoal : public C4DAtom
{
private:
	C4DAtomGoal();
	~C4DAtomGoal();
...
public:
	//----------------------------------------------------------------------------------------
	/// Checks if this object has any BaseLinks pointing to it.
	/// @return												True if there are links pointing to this object, false otherwise.
	//----------------------------------------------------------------------------------------
	Bool HasLinks() const { return C4DOS_Bo->HasLinks(this); }
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	static AtomArray* Alloc() { return (*C4DOS_At->AtomArrayAlloc)(); }

	/// @copydoc Alloc
	static AtomArray* Alloc(const maxon::SourceLocation&) { return Alloc(); }

	//----------------------------------------------------------------------------------------
	/// @destructsAlloc{atom arrays}
	/// @param[in,out] obj						@theToDestruct{atom array}
	//----------------------------------------------------------------------------------------
	static void Free(AtomArray*& obj) { (*C4DOS_At->AtomArrayFree)(obj); }
	...
	//----------------------------------------------------------------------------------------
	Int32 Find(const C4DAtom* obj) { return AtCall(AAFind) (obj); }
...
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE GeListNode : public C4DAtomGoal
{
private:
	GeListNode();
	~GeListNode();
...
public:
	...
	//----------------------------------------------------------------------------------------
	GeListNode* GetNext() { return const_cast<GeListNode*>(AtCall(GetNext) ()); }
	const GeListNode* GetNext() const { return AtCall(GetNext) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the previous node in the list.
	/// @return												The previous node, or @formatConstant{nullptr} if there is none. @cinemaOwnsPointed{node}
	//----------------------------------------------------------------------------------------
	GeListNode* GetPred() { return const_cast<GeListNode*>(AtCall(GetPred) ()); }
	const GeListNode* GetPred() const { return AtCall(GetPred) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the first child node.
	/// @return												The first child node, or @formatConstant{nullptr} if there is none. @cinemaOwnsPointed{node}
	//----------------------------------------------------------------------------------------
	GeListNode* GetDown() { return const_cast<GeListNode*>(AtCall(GetDown) ()); }
	const GeListNode* GetDown() const { return AtCall(GetDown) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the parent node.
	/// @return												The parent node, or @formatConstant{nullptr} if there is none. @cinemaOwnsPointed{node}
	//----------------------------------------------------------------------------------------
	GeListNode* GetUp() { return const_cast<GeListNode*>(AtCall(GetUp) ()); }
	const GeListNode* GetUp() const { return AtCall(GetUp) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the last child node.
	/// @return												The last child node, or @formatConstant{nullptr} if there is none. @cinemaOwnsPointed{node}
	//----------------------------------------------------------------------------------------
	GeListNode* GetDownLast() { return const_cast<GeListNode*>(AtCall(GetDownLast) ()); }
	const GeListNode* GetDownLast() const { return AtCall(GetDownLast) (); }
	...
	//-------------------------------------------------------------------------------------------------
	void FlushChilds() { AtCall(FlushChilds)(); }

	//-------------------------------------------------------------------------------------------------
	/// Moves all children from this to dest
	/// No notifications are send around, this is for internal use if you know what you do.
	/// dest will call FlushChilds() before moving the objects.
	//-------------------------------------------------------------------------------------------------
	void MoveChildrenTo(GeListNode* dest) { AtCall(MoveChildrenTo)(dest); }
	/// @}
	...
	const GeListHead* GetListHead() const { return MAXON_REMOVE_CONST(this)->GetListHead(); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetNodeID(Int32 index = 0) const { return C4DOS_Bl->GetNodeID((GeListNode*)this, index); }
	...
	//----------------------------------------------------------------------------------------
	template <typename CAST> const CAST* GetNodeData(Int32 index = 0) const { return static_cast<const CAST*>(C4DOS_Bl->GetNodeData(const_cast<GeListNode*>(this), index)); }
	...
	/// @return												The node data, or @formatConstant{nullptr}. @theOwnsPointed{node, NodeData}
	//----------------------------------------------------------------------------------------
	template <typename CAST> CAST* GetNodeData(Int32 index = 0) { return static_cast<CAST*>(C4DOS_Bl->GetNodeData(this, index)); }
	...
	//----------------------------------------------------------------------------------------
	const BaseDocument* GetDocument() const { return AtCall(GetDocument) (); }

	/// @copydoc GetDocument
	BaseDocument* GetDocument() { return MAXON_NONCONST_COUNTERPART(GetDocument()); }
	...
	/// @code
	/// maxon::BufferedBaseArray<BranchInfo, MAX> branch;
	/// Int32 count = node->GetBranchInfo(branch, MAX, GETBRANCHINFO::NONE);
	/// @endcode
	/// @param[in] info								ValueReceiver will be called for all found BranchInfo structures.
	/// @param[in] flags							Flags: @enumerateEnum{GETBRANCHINFO}
	/// @return												The number of BranchInfo elements filled in.
	//----------------------------------------------------------------------------------------
	maxon::Result<Bool> GetBranchInfo(const maxon::ValueReceiver<const BranchInfo&>& info, GETBRANCHINFO flags) const { return AtCall(GetBranchInfo) (info, flags); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsDocumentRelated() const { return AtCall(IsDocumentRelated) (); }
	...
	/// @markPrivate
	//-------------------------------------------------------------------------------------------------
	UInt32 GetNBitMask(Int32 index) const { return AtCall(GetNBitMask)(index); }
	...
	/// @markPrivate
	//-------------------------------------------------------------------------------------------------
	Int32 GetInfo() const { return C4DOS_Bo->GetInfo(this); }
...
/// @addAllocFreeAutoAllocNote
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE GeListHead : public GeListNode
{
private:
	GeListHead();
	~GeListHead();
...
public:
	...
	/// @copydoc Alloc
	static GeListHead* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	GeListNode* GetParent() { return const_cast<GeListNode*>(AtCall(GetParent) ()); }
	const GeListNode* GetParent() const { return AtCall(GetParent) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the first node in the list.
	/// @return												The first node in the list. @theOwnsPointed{list, node}
	//----------------------------------------------------------------------------------------
	GeListNode* GetFirst() { return const_cast<GeListNode*>(AtCall(GetFirst) ()); }
	const GeListNode* GetFirst() const { return AtCall(GetFirst) (); }

	//----------------------------------------------------------------------------------------
	/// Gets the last node in the list.
	/// @return												The last node in the list. @theOwnsPointed{list, node}
	//----------------------------------------------------------------------------------------
	GeListNode* GetLast() { return const_cast<GeListNode*>(AtCall(GetLast) ()); }
	const GeListNode* GetLast() const { return AtCall(GetLast) (); }
...
/// @addAllocFreeAutoAllocNote
//-------------------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE GeListHeadCB : public GeListHead
{
private:
	GeListHeadCB();
	~GeListHeadCB();
...
public:
	...
	/// @copydoc Alloc
	static GeListHeadCB* Alloc(const maxon::SourceLocation&) { return Alloc(); }
...
/// @addAllocFreeAutoAllocNote
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE BaseList2D : public GeListNode
{
private:
	BaseList2D();
	~BaseList2D();
...
public:
	...
	/// @copydoc Alloc
	static BaseList2D* Alloc(const maxon::SourceLocation&, Int32 type) { return Alloc(type); }
	...
	const BaseList2D* GetNext() const { return (BaseList2D*)AtCall(GetNext) (); }
	...
	const BaseList2D* GetPred() const { return (BaseList2D*)AtCall(GetPred) (); }
	...
	//----------------------------------------------------------------------------------------
	void SetBit(Int32 mask) { C4DOS_Bl->SetAllBits(this, C4DOS_Bl->GetAllBits(this) | mask); }
	...
	//----------------------------------------------------------------------------------------
	Bool GetBit(Int32 mask) const { return (C4DOS_Bl->GetAllBits(this) & mask) == mask; }
	...
	//----------------------------------------------------------------------------------------
	void DelBit(Int32 mask) { C4DOS_Bl->SetAllBits(this, C4DOS_Bl->GetAllBits(this) & ~mask); }
	...
	//----------------------------------------------------------------------------------------
	Int32 GetAllBits() const { return C4DOS_Bl->GetAllBits(this); }

	//----------------------------------------------------------------------------------------
	/// Sets all the object's bits at once.
	/// @param[in] bits								The new object's bits.
	//----------------------------------------------------------------------------------------
	void SetAllBits(Int32 bits) { C4DOS_Bl->SetAllBits(this, bits); }
	...
	void SetBitEx(Int32 mask, Int32 flags) { C4DOS_Bl->SetBitEx(this, mask, flags); }
	...
	/// It is recommended to use GetDataInstanceRef() instead because GetData makes a copy of the BaseContainer which can be costly.
	...
	/// @param[in] add								If @formatConstant{true} the values are merged, otherwise the old values are discarded.
	//----------------------------------------------------------------------------------------
	void SetData(const BaseContainer& bc, Bool add = true) { C4DOS_Bl->SetData(this, &bc, add); }
	...
	//----------------------------------------------------------------------------------------
	const BaseContainer& GetDataInstanceRef() const { return *C4DOS_Bl->GetDataInstance(this); }

	/// @copydoc GetDataInstanceRef
	BaseContainer& GetDataInstanceRef() { return *C4DOS_Bl->GetDataInstanceWritable(this); }

	//----------------------------------------------------------------------------------------
	/// Retrieves a pointer to the read-only object container.
	/// It is recommended to use GetDataInstanceRef() instead.
	/// @see GetDataInstanceRef()
	...
	//----------------------------------------------------------------------------------------
	const BaseContainer* GetDataInstance() const { return C4DOS_Bl->GetDataInstance(this); }

	/// @copydoc GetDataInstance
	BaseContainer* GetDataInstance() { return C4DOS_Bl->GetDataInstanceWritable(this); }
	...
	//----------------------------------------------------------------------------------------
	void SetName(const maxon::String& name, Bool setDirty = true) { AtCall(SetName) (name, setDirty); }
	...
	//----------------------------------------------------------------------------------------
	String GetBubbleHelp() { return C4DOS_Bl->GetBubbleHelp(this); }
	...
	/// @markDeprecated
	//-------------------------------------------------------------------------------------------------
	void GetMarkerStampEx(UInt32* l1, UInt32* l2) { return C4DOS_Bl->GetMarker(this, l1, l2); }
	...
	//----------------------------------------------------------------------------------------
	/// Returns all application IDs as a UniqueIDMap.
	/// @return												The map of all application IDs.
	//----------------------------------------------------------------------------------------
	const UniqueIDMap& GetUniqueIDMap() const { return AtCall(GetUniqueIDMap) (); }
	...
	//----------------------------------------------------------------------------------------
	LayerObject* GetLayerObject(const BaseDocument* doc) { return BlCall(GetLayerObject)(doc); }
	const LayerObject* GetLayerObject(const BaseDocument* doc) const { return MAXON_REMOVE_CONST(this)->GetLayerObject(doc); }
	...
	//----------------------------------------------------------------------------------------
	const LayerData* GetLayerData(const BaseDocument* doc, Bool rawdata = false) const { return BlCall(GetLayerData)(doc, rawdata); }
	...
	//----------------------------------------------------------------------------------------
	GeListHead* GetCTrackRoot(Bool create) { return BlCall(GetCTrackRoot)(create); }

	//----------------------------------------------------------------------------------------
	/// Gets the track root of the object.
	/// @return												The track root.
	//----------------------------------------------------------------------------------------
	const GeListHead* GetCTrackRoot() const { return MAXON_REMOVE_CONST(this)->GetCTrackRoot(false); }

	//----------------------------------------------------------------------------------------
	/// Gets the first track of the object.
	/// @return												The first track, or @formatConstant{nullptr} if there are no tracks.
	//----------------------------------------------------------------------------------------
	const CTrack* GetFirstCTrack() const { return BlCall(GetFirstCTrack)(); }

	//----------------------------------------------------------------------------------------
	/// Gets the first track of the object.
	/// @return												The first track, or @formatConstant{nullptr} if there are no tracks.
	//----------------------------------------------------------------------------------------
	CTrack* GetFirstCTrack() { return MAXON_NONCONST_COUNTERPART(GetFirstCTrack()); }
	...
	//----------------------------------------------------------------------------------------
	const CTrack* FindCTrack(const DescID& id) const { return BlCall(FindCTrack)(id); }
	...
	/// @return												The found track, or @formatConstant{nullptr} if there was no match.
	//----------------------------------------------------------------------------------------
	CTrack* FindCTrack(const DescID& id) { return MAXON_NONCONST_COUNTERPART(FindCTrack(id)); }
...
/// @{
	...
	/// @markPrivate
	//-------------------------------------------------------------------------------------------------
	GeListHead* GetNLARoot(Bool create) { return BlCall(GetNLARoot)(create); }
	...
	//----------------------------------------------------------------------------------------
	const BaseList2D* GetMain() const { return AtCall(GetMain) (); }

	//----------------------------------------------------------------------------------------
	/// Goes up the next level, e.g. from tag to object or XPresso node to tag, or object to document, etc.
	/// @return												The main object of the object.
	//----------------------------------------------------------------------------------------
	BaseList2D* GetMain() { return MAXON_NONCONST_COUNTERPART(GetMain()); }
...
#define FreeListNode(v) { if (v) cinema::C4DOS_Bl->Free(MAXON_REMOVE_CONST(v)); v = nullptr; }

/// Destructs objects derived from BaseList2D. It is basically the same as BaseList2D::Free().
#define blDelete(v) FreeListNode(v)
...
public:
	...
	/// @copydoc Alloc
	static BaseLink* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	const BaseList2D* GetLink(const BaseDocument* doc, Int32 instanceof = 0) const { return C4DOS_Ln->GetLink(this, doc, instanceof); }

	/// @copydoc GetLink
	BaseList2D* GetLink(BaseDocument* doc, Int32 instanceof = 0) const { return const_cast<BaseList2D*>(C4DOS_Ln->GetLink(this, doc, instanceof)); }

	BaseList2D* GetLink(std::nullptr_t doc, Int32 instanceof = 0) const { return const_cast<BaseList2D*>(C4DOS_Ln->GetLink(this, nullptr, instanceof)); }
	...
	//----------------------------------------------------------------------------------------
	const C4DAtomGoal* GetLinkAtom(const BaseDocument* doc, Int32 instanceof = 0) const { return C4DOS_Ln->GetLinkAtom(this, doc, instanceof); }

	/// @copydoc GetLinkAtom
	C4DAtomGoal* GetLinkAtom(BaseDocument* doc, Int32 instanceof = 0) const { return const_cast<C4DAtomGoal*>(C4DOS_Ln->GetLinkAtom(this, doc, instanceof)); }

	C4DAtomGoal* GetLinkAtom(std::nullptr_t doc, Int32 instanceof = 0) const { return const_cast<C4DAtomGoal*>(C4DOS_Ln->GetLinkAtom(this, nullptr, instanceof)); }
	...
	//----------------------------------------------------------------------------------------
	BaseList2D* ForceGetLink() const { return C4DOS_Ln->ForceGetLink(this); }
	...
	//----------------------------------------------------------------------------------------
	C4DAtomGoal* ForceGetLinkAtom() const { return const_cast<C4DAtomGoal*>(C4DOS_Ln->ForceGetLinkAtom(this)); }
	...
	//----------------------------------------------------------------------------------------
	void SetLink(const C4DAtomGoal* list) { C4DOS_Ln->SetLink(this, list); }
	...
	//----------------------------------------------------------------------------------------
	Bool Read(HyperFile* hf) { return C4DOS_Ln->Read(this, hf); }
	...
	//----------------------------------------------------------------------------------------
	Bool Write(HyperFile* hf) const { return C4DOS_Ln->Write(this, hf); }
	...
	//----------------------------------------------------------------------------------------
	BaseLink* GetClone(COPYFLAGS flags, AliasTrans* trn) const { return C4DOS_Ln->GetClone(this, flags, trn); }
	...
	//----------------------------------------------------------------------------------------
	Bool CopyTo(BaseLink* dst, COPYFLAGS flags, AliasTrans* trn) const { return C4DOS_Ln->CopyTo(this, dst, flags, trn); }
	...
	//----------------------------------------------------------------------------------------
	Bool IsCacheLink() const { return C4DOS_Ln->IsCacheLink(this); }

	//-------------------------------------------------------------------------------------------------
	/// @markPrivate
	//-------------------------------------------------------------------------------------------------
	void SetUpPointer(C4DAtom* t_up_pointer) { return C4DOS_Ln->SetUpPointer(this, t_up_pointer); }
...
public:
	...
	/// @copydoc Alloc
	static AliasTrans* Alloc(const maxon::SourceLocation&) { return Alloc(); }
	...
	//----------------------------------------------------------------------------------------
	Bool Init(const BaseDocument* doc) { return C4DOS_Ln->TrnInit(this, doc); }
	...
	//----------------------------------------------------------------------------------------
	void Translate(Bool connect_oldgoals) { C4DOS_Ln->TrnTranslate(this, connect_oldgoals); }
