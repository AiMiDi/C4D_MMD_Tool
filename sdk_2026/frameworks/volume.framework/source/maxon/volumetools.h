...
struct FilterSettings
{
	...
	cinema::SplineData* splineData = nullptr;
};
...
struct FieldSampleData
{
	...
	Bool error = false;															// True when there's an error in the initalization of field sampling.
	const cinema::FieldList* fieldList = nullptr;								// The field list object to use. Can be nullptr if fieldOwner is a field object for direct field sampling.
	const cinema::BaseList2D* fieldOwner = nullptr;							// Can be either a field object for direct sampling or needs to be the owner of this field list.
	const cinema::BaseDocument* doc = nullptr;									// The document to evaluate the field list for (for field object links).
};
...
struct VolumeConversionPolygon
{
	...
	{
		return reinterpret_cast<const UInt32*>(this)[index & 3];
	}
	UInt32& operator[](UInt index)
	{
		return reinterpret_cast<UInt32*>(this)[index & 3];
	}
...
public:
	...
	/// @see MeshToVolume, same thing, just converts from new GeometryObject type.
	/// @param[in] geometry						The geometry object wrapper.
	/// @param[in] polygonObjectMatrix	The global position, scale and rotation matrix of the polygon object.
	/// @param[in] gridSize						The desired gridsize of the created volume.
	/// @param[in] bandWidthInterior	The amount of exterior voxels that distance information is calculated for.
	/// @param[in] bandWidthExterior	The amount of interior voxels that distance information is calculated for.
	...
	/// @param[in] volumeTwo					The second SDF volume for the boolean operation.
	/// @param[in] type								The bool type ::BOOLTYPE.
	/// @param[in] radius							The radius of the smoothing area around the overlapping parts.
	/// @return												The booled volume.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Converts the internal data to the nanovdb format and returns it in nanoVDBData.
	///	If the underyling GRIDTYPE is FLOAT or DOUBLE - returns a converted volume of type nanovdb::Fp16 by default.	
	/// If the underyling GRIDTYPE is VECTOR32 or VECTOR64 - returns a converted volume of type nanovdb::Vec3<float> by default.
	/// Note that if the GRIDTYPE is VECTOR32 the resulting velocity vector has to be mirrored along the xy plane (.z *= -1.0f).
	/// @param[in] precision					The precision returned - only FLOAT available for vector grids.
	/// @return												The output BaseArray of binary data.
	//----------------------------------------------------------------------------------------
	...
	/// @brief Clip the volume based on a given bounding box. The boudning box is interger / voxel index bounding box. see openvdb::tools::clip
	/// Note: openvdb::tools::clip input bounding box is	a world-space bounding box  (so the input is float vector)
	/// @Since 2026.0
	/// @param volume input volume
	/// @param boundingbox bounding box of voxel indices
	/// @param keepInterior if true, discard voxels that lie outside the bounding box; if false, discard voxels that lie inside the bounding box 
	/// @return  return a new volume containing the result. 
	static MAXON_METHOD Result<Volume> Clip(const Volume& volume, Range<IntVector32> boundingbox, Bool keepInterior);
