...
template <typename INTERFACE> class ThreadRefTemplate;
...
} MAXON_ENUM_LIST(INTERACTIONTYPE, "net.maxon.command.interationtype");
...
static inline Tuple<InternedId, Data> Param(KEY&& key, T&& data)
{
	...
	Data tmp;
	tmp.SetImpl<TT>(std::forward<T>(data), OVERLOAD_MAX_RANK) iferr_ignore("errors are not handled here", debug);
	return Tuple<InternedId, Data>{key, std::move(tmp)};
}
...
{
	MAXON_INTERFACE(CommandDataInterface, MAXON_REFERENCE_NORMAL, "net.maxon.command.interface.commanddata", MAXON_IMPLEMENTATION_MODULE("net.maxon.command"));
public:
	...
	public:
		...
		template <typename T> void operator ()(T&& arg)
		{
			if (_dd)
			{
				iferr (_dd->SetData(ForwardingDataPtr(MoveDataPtr(std::move(arg.first))), std::move(arg.second)))
					DebugStop();
			}
		}
	};
	...
	template <typename ...ARGS> MAXON_FUNCTION Result<COMMANDRESULT> Invoke(const CommandClass& command, Bool interactive, ARGS&& ...args)
	{
		...
		DataDictionaryObjectInterface* dd = reinterpret_cast<DataDictionaryObjectInterface*>(this);
...
{
	MAXON_INTERFACE(CommandInteractionClassInterface, MAXON_REFERENCE_CONST, "net.maxon.command.interface.commandinteraction", MAXON_IMPLEMENTATION_MODULE("net.maxon.command"));
...
{
	MAXON_INTERFACE(CommandDescriptionClassInterface, MAXON_REFERENCE_CONST, "net.maxon.command.interface.commanddescriptioninterface", MAXON_IMPLEMENTATION_MODULE("net.maxon.command"));
};
...
{
	MAXON_INTERFACE(MigratedCommandClassInterface, MAXON_REFERENCE_CONST, "net.maxon.command.interface.migratedcommand", MAXON_IMPLEMENTATION_MODULE("net.maxon.command"));
...
//----------------------------------------------------------------------------------------
/// Define if the the output conversion was performed or not.
//----------------------------------------------------------------------------------------
enum class MAXON_WARN_UNUSED_CLASS COMMAND_ACTION_STATE
{
	NOT_EXECUTED = 0, ///< Specified action is not implemented or not executed.
	EXECUTED = 1  		///<  Specified action is  implemented or  executed.
} MAXON_ENUM_LIST(COMMAND_ACTION_STATE);
...
{
	MAXON_DISALLOW_COPY_AND_ASSIGN(CommandCacheData)

public:

	CommandCacheData() = default;
	~CommandCacheData();
	CommandCacheData(CommandCacheData&& src) : MAXON_MOVE_MEMBERS(_map, _validateFn) { }

	MAXON_OPERATOR_MOVE_ASSIGNMENT(CommandCacheData)
...
private:
	...
	Delegate<Result<Bool>(const Id& commandId)> _validateFn;	/// validation delegate.
};
...
/// Private.
//----------------------------------------------------------------------------------------
class CommandCacheInterface : MAXON_INTERFACE_BASES(ObjectInterface)
{
	MAXON_INTERFACE(CommandCacheInterface, MAXON_REFERENCE_NORMAL, "net.maxon.command.interface.cache", MAXON_IMPLEMENTATION_MODULE("net.maxon.command"));
...
namespace COMMAND
{
	...
	MAXON_ATTRIBUTE(Bool, USECACHE, "net.maxon.command.usecache", RESOURCE_DEFAULT(false)); 			///< Define if the command have to use the cache.
	MAXON_ATTRIBUTE(CommandCacheDataRef, CACHEDATA, "net.maxon.command.cache"); 									///< The cache to be used.
	using MemoizationType = Tuple<Bool, const void*, Int>;
	MAXON_ATTRIBUTE(MemoizationType, MEMOIZATION, "net.maxon.command.memoization", RESOURCE_DEFAULT(MemoizationType{}));	///< True if the command shall use memoization to remember an reuse results of previous invocations.
