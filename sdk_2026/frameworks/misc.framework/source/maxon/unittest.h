...
{
	MAXON_INTERFACE(UnitTestErrorInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.unittesterror", MAXON_IMPLEMENTATION_MODULE("net.maxon.misc"));
};
...
{
	MAXON_INTERFACE(UnitTestInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.unittest", MAXON_IMPLEMENTATION_MODULE("net.maxon.misc"));

public:
	//----------------------------------------------------------------------------------------
	/// @brief This functions needs to be overridden with the implementation of the test code (see example).
	...
	//----------------------------------------------------------------------------------------
	/// @brief Checks if the Test has been cancelled from outside.
	/// @return												In case of true the test should exit immediately.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Bool IsCancelled() const;

	//----------------------------------------------------------------------------------------
	/// @brief Cancels all UnitTestClasses.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD void Cancel();

	//----------------------------------------------------------------------------------------
	/// @brief Outputs the result to the console in a standard format. This format can be analyzed by tools.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Outputs the result to the console in a standard format. This is similar to AddResult,
	...
	//----------------------------------------------------------------------------------------
	MAXON_METHOD void AddResultBool(const String& testName, Bool state);

	//----------------------------------------------------------------------------------------
	/// @brief Outputs the result with timing to the console in a standard format. Same AddResult() but with timing added.
	/// @param[in] testName						A unique name within your test class. This is the identifier for later comparisons with previous builds.
	/// @param[in] state							The result of the test.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns a path to unit test data (`g_testDataDir`).
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<Url> GetTestDataDirectory();

	/// @brief Timeout for threaded unit tests which is high enough that even an overloaded machine with swapping to VM avoids false positives.
	static constexpr const Minutes THREADTIMEOUT { 5 };
};
...
#define MAXON_TEST_ASSERT_EQ(expected, ...) \
	if (const auto& testAssertValue = __VA_ARGS__; !(testAssertValue == expected)) \
