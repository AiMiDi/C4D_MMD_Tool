...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Converts the formula into simple c-like shader code. This supports 32bit float only.
	/// @param[in] cache							Pre-parsed cache generated by CalculateParserCache. The cache will not be modified during this call.
	/// @param[in] resultVariableName	The variable name where the result should be stored.
	/// @param[out] result						Will be filled with the shader code of the formula.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> GenerateShaderCode(const FormulaParserCacheRef& cache, const String& resultVariableName, String& result);
};
