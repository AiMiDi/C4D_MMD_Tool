...
namespace ASSET
{
	namespace METAPROPERTIES
	{
		namespace BASE
		{
			/// timestamp of the asset when the metadata where UniversalDateTime is copied into this property when meadata/preview is generated ASSETMETADATA::ASSET_TIMESTAMP
...
namespace ASSET
{
	namespace METAPROPERTIES
	{
		...
		namespace C4DFILE
		{
			...
			/// Auto orientation mode used by the placement tool.
			/// - NORMAL: default for most object, align object axis to the surface normal
			/// - UP: for trees or street light
			/// - CEILING: ceiling fan or lamp 
			/// - WALL: frame
			/// - BOUNDINGBOX: ball, pen, objects where the object axis is not a great reference point
			MAXON_ATTRIBUTE(Id, ORIENTATIONMODE, "net.maxon.asset.metaproperties.c4dfile.orientationmode",
				RESOURCE_DEFINE(ENUM_NORMAL, LiteralId("normal"))
				RESOURCE_DEFINE(ENUM_UP, LiteralId("up"))
				RESOURCE_DEFINE(ENUM_CEILING, LiteralId("ceiling"))
				RESOURCE_DEFINE(ENUM_WALL, LiteralId("wall"))
				RESOURCE_DEFINE(ENUM_BOUNDINGBOX, LiteralId("boundingbox")));
		}
