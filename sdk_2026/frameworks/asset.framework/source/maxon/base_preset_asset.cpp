...
{
	Set(0, rawptr) iferr_ignore("continue");
	Set(1, type) iferr_ignore("continue");
}
...
Result<void> AddAssetDepencendyStruct::AddDependency(const AssetDescription& depAsset)
{
	...
	UniversalDateTime creationTimeStamp = depAsset.GetMetaData().Get(ASSETMETADATA::ASSET_TIMESTAMP).GetOrDefault() iferr_return;
