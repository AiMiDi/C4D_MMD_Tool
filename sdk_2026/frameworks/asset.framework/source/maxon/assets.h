...
class BackgroundProgressRef MAXON_FORWARD("maxon/progress.h");
...
/// @brief String prefix applied on language meta data file names ("strings_en-US.meta" for instance).
#define STRINGS_METADATA_PREFIX "strings_"
...
namespace ASSETMETADATA
{
	...
	//----------------------------------------------------------------------------------------
	/// This asset meta data attribute can be set by the user to give more details about the asset's content.
	...
	//----------------------------------------------------------------------------------------
	/// This asset meta data attribute can be set by the user to give more details about the asset's creation context.
	/// The data is optional but should be provided for all assets to help handle version compatibility.
	//----------------------------------------------------------------------------------------
...
{
	MAXON_INTERFACE(AssetMetaDataInterface, MAXON_REFERENCE_CONST, "net.maxon.interface.assetmetadata", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
{
	MAXON_INTERFACE(AssetTypeInterface, MAXON_REFERENCE_CONST, "net.maxon.interface.assettype", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
{
	MAXON_INTERFACE(AssetIdentifierInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.assetidentifier", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
/// The asset's origin data can also be included in the link to allow tracking and debugging of missing assets.
...
public:
	...
	/// @brief Default update policy used in AssetLink.
	static constexpr ASSET_UPDATE_POLICY DEFAULT_LINK_POLICY = ASSET_UPDATE_POLICY::EXPLICIT;

	/// @brief Original name of the asset (default language).
	/// Won't prevent resolving links if an asset was renamed.
	MAXON_ATTRIBUTE_CLASS(String, NAME, "net.maxon.assetlink.origin.name");
	/// @brief Original ASSETMETADATA::ASSET_VERSIONTAG (default language).
	...
	/// @brief Original category of the asset.
	/// Won't prevent resolving links if an asset was changed to a new category.
	MAXON_ATTRIBUTE_CLASS(Id, Category, "net.maxon.assetlink.origin.category");
	/// @brief Original repository name of the asset.
	/// Won't prevent resolving links if an asset was moved to a different repository, this is just to help handling missing assets.
	MAXON_ATTRIBUTE_CLASS(String, REPOSITORYNAME, "net.maxon.assetlink.origin.reponame");
	/// @brief Original repository identifier of the asset.
	/// Won't prevent resolving links if an asset was moved to a different repository, this is just to help handling missing assets.
	MAXON_ATTRIBUTE_CLASS(Id, REPOSITORYID, "net.maxon.assetlink.origin.repoid");
	/// @brief Asset instance data.
	/// Content varies per asset type. All the data required to reinstate a broken asset link should be included.
	/// I.e. any additional data needed to fix a scene that was saved with missing assets.
	MAXON_ATTRIBUTE_CLASS(Data, INSTANCEDATA, "net.maxon.assetlink.origin.instancedata");

	//----------------------------------------------------------------------------------------
	/// @brief Creates a new link to the given asset with requested update policy.
	/// @param[in] asset							The asset to link to. Asset type, identifier and version will be read from it to create the link.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Creates a new link for the given Url.
	/// This is for assets which aren't stored in a repository, but shall be read directly from a Url such as a local image file.
	/// Url links use policy ASSET_UPDATE_POLICY::NEVER.
	/// @param[in] type								The expected type of the asset.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Sets the asset link's url. This will clear the Id and ASSET_UPDATE_POLICY.
	/// @param[in] url								The Url to which this AssetLink points.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Sets the asset link's identifier. This will clear the Url.
	/// One might want to update the asset link's update policy afterwards.
	/// @param[in] aid								The asset identifier and version.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> SetIdAndVersion(const IdAndVersion& aid);

	//----------------------------------------------------------------------------------------
	/// @brief Gets the asset link's origin informations.
	/// @return												Dictionary of of the asset's origin details.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<DataDictionary> GetOriginData() const;

	//----------------------------------------------------------------------------------------
	/// @brief Sets the asset link's origin data informations.
	/// @param[in] originData					The new origin informations (will overwrite existing).
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
...
{
	MAXON_INTERFACE(AssetRepositoryInterface, MAXON_REFERENCE_NORMAL, "net.maxon.interface.assetrepository", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
{
	MAXON_INTERFACE(AssetDescriptionInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.assetdescription", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
{
	MAXON_INTERFACE(AssetBaseWithUpdateInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.assetbasewithupdate", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
public:
	...
	/// @return												The instance's path. Implementation specific.
	//----------------------------------------------------------------------------------------
...
{
	MAXON_INTERFACE(AssetInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.asset", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
public:
	...
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<UpdatableAssetRepositoryRef> CreateRepositoryFromUrl(const Id& rid, const Class<AssetRepositoryRef>& repoType, const Block<const AssetRepositoryRef>& bases, const Url& url, Bool writable, Bool derivedInPrefs, Bool tempRepository, const BackgroundEntryTuple& backgroundJob = GetZeroRef<BackgroundEntryTuple>());
	...
	//----------------------------------------------------------------------------------------
	/// @brief Yields all direct references which this asset instance makes to other assets.
	/// The AssetReference objects are used later for the update mechanism. The method UpdatableAssetInterface::Update gets
	...
	//----------------------------------------------------------------------------------------
	/// Tests if a new asset version can be applied on a given asset.
	/// @param[in] ref								The asset on which to perform the test.
	/// @param[in] update							The new asset version.
	/// @return												True if #update can be applied on an instance of asset #link.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Bool IsApplicableUpdate(const AssetReference& ref, const AssetBase& update);
...
{
	MAXON_INTERFACE(UpdatableAssetInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.updatableasset", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
...
{
	MAXON_INTERFACE(FileAssetInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.interface.fileasset", MAXON_IMPLEMENTATION_MODULE("net.maxon.asset"));
