...
public:
	...
	EdgePairTemplate() = default;
	MAXON_IMPLICIT EdgePairTemplate(T vA, T vB = INITVALUE) : _vA(Min(vA, vB)), _vB(Max(vA, vB)) { }
	EdgePairTemplate(EdgePairTemplate<T, INITVALUE>&& src) : MAXON_MOVE_MEMBERS(_vA, _vB) { }
	MAXON_OPERATOR_COPY_ASSIGNMENT(EdgePairTemplate);
	EdgePairTemplate(const EdgePairTemplate<T, INITVALUE>& src) : _vA(src._vA), _vB(src._vB) { }
	...
	inline Bool operator ==(const EdgePairTemplate<T, INITVALUE>& b) const
	{
...
struct NeighborStorage
{
	MAXON_DISALLOW_COPY_AND_ASSIGN(NeighborStorage);
	NeighborStorage() = default;
	NeighborStorage(NeighborStorage&& src) : MAXON_MOVE_MEMBERS(polygonCountPerPoint, offsetInPolygonListPerPoint, polygonList) { }
	MAXON_OPERATOR_MOVE_ASSIGNMENT(NeighborStorage);

	Result<void> CopyFrom(const NeighborStorage<STORAGEINDEX>& other)
	{
		iferr_scope;
		polygonCountPerPoint.CopyFrom(other.polygonCountPerPoint) iferr_return;
		offsetInPolygonListPerPoint.CopyFrom(other.offsetInPolygonListPerPoint) iferr_return;
		polygonList.CopyFrom(other.polygonList) iferr_return;
		return OK;
	}
	...
	Result<void> Init(const Block<const SimplePolygon>& polygons, STORAGEINDEX pointCount)
	{
		...
		if (polygons.GetCount() > 65535 && std::is_same<STORAGEINDEX, Int32>::value)
		{
			ParallelFor::Dynamic(0, polygons.GetCount(),
				[&polygons, this](Int polygonIndex)
				{
					const SimplePolygon& simplePolygon = polygons[polygonIndex];
					AtomicInt32::SwapIncrement(&polygonCountPerPoint[simplePolygon.a]);
					AtomicInt32::SwapIncrement(&polygonCountPerPoint[simplePolygon.b]);
					AtomicInt32::SwapIncrement(&polygonCountPerPoint[simplePolygon.c]);
					if (simplePolygon.c != simplePolygon.d)
						AtomicInt32::SwapIncrement(&polygonCountPerPoint[simplePolygon.d]);
				}, ParallelFor::Granularity(128));
		}
		else
		{
...
struct EdgeNeigborPolygonsTemplate
{
	...
	static const Block<const STORAGEINDEX> GetEdgePolygons(STORAGEINDEX pointA, STORAGEINDEX pointB, const EdgePolygons::Map& map, const NonManifoldStroage& nonManifold)
	{
		...
		const EdgePolygons* value = (pointA < map.GetCount() && pointA >= 0) ? map[pointA].FindValue(pointB) : nullptr;
		if (value)
		{
