...
void CellData::MarkAllUsedPoints(BaseArray<Char>& pointUseless, Char mark)
{
	for (Int edgeIndex = 0; edgeIndex < _edges.GetCount(); edgeIndex++)
	{
		CellEdge& curEdge = _edges[edgeIndex];
...
// marks every used point in an array with NOTOK or the given mark int
void CellData::MarkAllUsedPoints(BaseArray<Int32>& pointUseless, Int32 mark)
{
	for (Int edgeIndex = 0; edgeIndex < _edges.GetCount(); edgeIndex++)
	{
		CellEdge& curEdge = _edges[edgeIndex];
...
Result<void> CellData::RemoveIsolatedPoints()
{
	...
	BaseArray<Int32> pointUseless;
	pointUseless.Resize(_vertices.GetCount()) iferr_return;

	for (Int32& pointTarg : pointUseless)
		pointTarg = -3;
	...
	Int pointIndex = 0;
	Int usefulCount = 0;
	for (Int32& curShift : pointUseless)
	{
		if (curShift == -3)
		{
			pointShift++;
...
Result<Bool> CellData::CutCellOnPlane(const Pair<Vector, Vector>& plane, Int32 planeNeighborIndex)
{
	...
	for (Int32 faceIndex = 0; faceIndex < _faces.GetCount(); faceIndex++)
	{
		...
		Bool cutRes = CutFaceOnPlane(faceIndex, plane, pointToCutPlaneDistance, edgeIntesection, pointTranslateMap, newEdges, newFaceEdges) iferr_return;
	...
	if (newFaceEdges.GetCount() > 0)
	{
		...
		Bool cutRes2 = ProcessInsideFaceAndAddToEdges(newFaceEdges, newPoints.GetCount(), newEdges) iferr_return;
...
Result<Bool> CellData::ShrinkCellByValue(const BaseArray<Vector>& planeNormals, Float offset)
{
	...
	{
		if (!cutPlanes[planeIndex].second.IsZero())
		{
			Bool cutRes = CutCellOnPlane(cutPlanes[planeIndex], originalPlaneNeighbor[planeIndex]) iferr_return;
...
Bool CellData::CalcAllFaceNormals(BaseArray<Vector>& faceNormals, Float scaleValue)
{
	Float faceSizeCutoff = Min(Abs(scaleValue), 1.0_f);
	...
	for (Int32 faceIndex = 0; faceIndex < _faces.GetCount(); faceIndex++)
	{
		...
		if (normalLength * 0.5 > faceSizeCutoff) // any face smaller than the scale value will be completely lost anyways and the neighbors will cut away enough
		{
			faceNormals[faceIndex] = normal / normalLength;
			wroteNormal = true;
		}
		else
		{
			faceNormals[faceIndex] = Vector(0.0_f);
		}
	}
	return wroteNormal;
}
...
Result<Bool> CellData::ScaleCell(Float scaleValue)
{
	...
	Bool wroteNormal = CalcAllFaceNormals(faceNormals, scaleValue);

	if (!wroteNormal) // full shrinkage
	{
		_adjacentCell.Flush();
		_vertices.Flush();
		_edges.Flush();
	...
	else
	{
		if (scaleValue < 0)
		{
			Bool shrinkRes = ShrinkCellByValue(faceNormals, scaleValue) iferr_return;
			if (!shrinkRes)
			{
				_adjacentCell.Flush();
				_vertices.Flush();
...
Bool CellData::SetupReverseEdges()
{
	...
	BaseArray<Int32> eraseEdge;
	// iterate through all created edges to find the reverse edge
	for (Int32 edgeIndex = 0; edgeIndex < _edges.GetCount(); edgeIndex++)
	{
		CellEdge& curEdge = _edges[edgeIndex];

		if (curEdge._prevEdgeOfFace == curEdge._nextEdgeOfFace)
			continue;

		if (curEdge._start == curEdge._end)
		{
			_edges[curEdge._nextEdgeOfFace]._prevEdgeOfFace = curEdge._prevEdgeOfFace;
			_edges[curEdge._prevEdgeOfFace]._nextEdgeOfFace = curEdge._nextEdgeOfFace;

			iferr (eraseEdge.Append(edgeIndex))
				return false;

			if ((_edges[curEdge._nextEdgeOfFace]._prevEdgeOfFace == _edges[curEdge._nextEdgeOfFace]._nextEdgeOfFace)
				&& (_edges[curEdge._prevEdgeOfFace]._prevEdgeOfFace == _edges[curEdge._prevEdgeOfFace]._nextEdgeOfFace))
			{
				iferr (eraseEdge.Append((Int32)curEdge._nextEdgeOfFace))
					return false;

				iferr (eraseEdge.Append((Int32)curEdge._prevEdgeOfFace))
					return false;

				// erase face
				for (auto it = Iterable::EraseIterator(_faces); it; ++it)
				{
					if (*it == edgeIndex
						|| *it == curEdge._nextEdgeOfFace
						|| *it == curEdge._prevEdgeOfFace)
					{
						it.Erase();
						break;
					}
	...
	if (eraseEdge.GetCount() > 0)
	{
		SimpleSort<> sorter;
		sorter.Sort(eraseEdge);
		Reverse(eraseEdge);

		for (Int32 edgeToRemove : eraseEdge)
		{
			iferr (_edges.Erase(edgeToRemove))
				return false;

			Int32 edgeIndex = 0;
			for (CellEdge& curSearchEdge : _edges)
			{
				if (curSearchEdge._nextEdgeOfFace == edgeToRemove && !eraseEdge.Contains(edgeIndex))
					return false;

				if (curSearchEdge._nextEdgeOfFace > edgeToRemove)
					curSearchEdge._nextEdgeOfFace--;

				if (curSearchEdge._prevEdgeOfFace == edgeToRemove && !eraseEdge.Contains(edgeIndex))
					return false;

				if (curSearchEdge._prevEdgeOfFace > edgeToRemove)
					curSearchEdge._prevEdgeOfFace--;

				if (curSearchEdge._reverseEdge > edgeToRemove)
					curSearchEdge._reverseEdge--;

				edgeIndex++;
			}

			for (Int& curFaceStart : _faces)
			{
				if (curFaceStart > edgeToRemove)
					curFaceStart--;
			}
		}
	}
...
Bool CellData::MarkAllDoublePoints(BaseArray<Int32>& pointUseless, Float epsilonOverride)
{
	iferr_scope;
...
Result<Bool> CellData::MarkAllColinearPoints(BaseArray<Int32>& pointUseless, Float epsilonOverride)
{
	iferr_scope;
		...
		do
		{
			...
			if (pointUseless[currentPoint] != -2)
			{
				...
				if (trianglesize < usedEpsilon)
				{
					...
					{
						pointRemoved = true;
						pointUseless[currentPoint] = (Int32)nextPoint;
					}
				}
			}
...
Result<Bool> CellData::MarkAllUselessPoints(BaseArray<Char>& pointUseless)
{
	iferr_scope;
	...
	for (const Int& faceStart : _faces)
	{
		...
		{
			const CellEdge& curEdge = _edges[curEdgeIndex];
			if (pointUseless[curEdge._end] != (Char)-1)
			{
				...
				{
					pointsRemoved = true;
					pointUseless[curEdge._end] = (Char)-2;
				}
				else
				{
					pointUseless[curEdge._end] = (Char)-1;
				}
			}
			curEdgeIndex = curEdge._nextEdgeOfFace;
		} while (curEdgeIndex != startEdgeIndex);
...
inline Int32 ReturnCorrectNewVertexIndex(maxon::BaseArray<Int32>& shiftArray, const Int32& startIndex)
{
	Int32 returnIndex = startIndex;
	Int32 failCounter = 0;
	while (shiftArray[returnIndex] != -1 && shiftArray[returnIndex] != -2)
	{
		if (shiftArray[shiftArray[returnIndex]] == returnIndex || failCounter > 200)
		{
			shiftArray[returnIndex] = -1;
...
inline Int32 ReturnCorrectNewVertexIndex(maxon::BaseArray<Char>& shiftArray, const Int32& startIndex)
{
	return startIndex;
}

Result<Bool> CellData::ReCreateCellWithOnlyUsefulPoints(BaseArray<Int32>& pointUseless)
{
	iferr_scope;
	...
	if (_vertices.GetCount() == 0 && _verticesPreIndex.GetCount() > 0)
	{
		...
		{
			if (pointUseless[oldPointIndex] == -1)
			{
	...
	for (Int faceIndex = 0; faceIndex < faceCount; faceIndex++)
	{
		...
		do
		{
			...
			{
				endIndex = (Int)pointTranslateMap[ReturnCorrectNewVertexIndex(pointUseless, (Int32)curEdge._end)];
			}

			realStartIndex = (Int)ReturnCorrectNewVertexIndex(pointUseless, (Int32)curEdge._start);
...
Result<Bool> CellData::ReCreateCellWithOnlyUsefulPoints(BaseArray<Char>& pointUseless)
{
	iferr_scope;
	...
	if (_vertices.GetCount() == 0 && _verticesPreIndex.GetCount() > 0)
	{
		for (Int oldPointIndex = 0; oldPointIndex < pointUseless.GetCount(); oldPointIndex++)
		{
			if (pointUseless[oldPointIndex] == (Char)-1)
			{
				pointTranslateMap[oldPointIndex] = newPointsPreIndex.GetCount();
				newPointsPreIndex.Append(_verticesPreIndex[oldPointIndex]) iferr_return;
			}
	...
	for (Int faceIndex = 0; faceIndex < faceCount; faceIndex++)
	{
		...
		do
		{
			...
			if (pointUseless[curEdge._end] == (Char)-2)
			{
				endIndex = NOTOK;
			}
			else
...
Result<Bool> CellData::FixGeometry()
{
	...
	for (Char& pointTarg : pointUseless)
		pointTarg = (Char)-1;
	...
	pointsChanged = MarkAllColinearPoints(pointUseless, pointsChanged) iferr_return;
	if (pointsChanged)
	{
	...
	pointsChanged = MarkAllUselessPoints(pointUseless) iferr_return;
	if (pointsChanged)
	{
		res = ReCreateCellWithOnlyUsefulPoints(pointUseless) iferr_return;
		if (!res)
