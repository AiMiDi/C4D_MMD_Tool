...
public:
	...
	COMPARERESULT Compare(const Poly<T>& other) const
	{
		return ToBlock().Compare(other.ToBlock());
	}
	...
	Int FindDirectedEdge(const T& value1, const T& value2) const
	{
		if (a == value1 && b == value2)
		{
			return 0;
		}
		if (b == value1 && c == value2)
		{
			return 1;
		}
		if (c == d) // triangle
		{
			if (c == value1 && a == value2)
			{
				return 3;
			}
		}
		...
		{
			if (c == value1 && d == value2)
			{
				return 2;
			}
			if (d == value1 && a == value2)
			{
				return 3;
			}
		}
	...
	{
		return Block<const T>(&a, c == d ? 3 : 4);
	}
...
/// @param[in] index							A  vertex index in the loop. Must be in the range [0..vertexCount-1].
/// @param[in] vertexCount				The number of vertices in a closed loop.
/// @return												The decremented index value. If the lower limit is hit vertexCount - 1 is returned.
//----------------------------------------------------------------------------------------
template <typename INDEXTYPE, typename COUNT>
inline INDEXTYPE GetPredecessor(INDEXTYPE index, COUNT vertexCount)
{
	return MAXON_UNLIKELY(index == 0) ? (vertexCount - 1) : (index - 1);
}
...
/// @param[in] index							A  vertex index  in the loop. Must be in the range [0..vertexCount-1].
/// @param[in] vertexCount				The number of vertices in a closed loop.
/// @return												The incremented index value. If the lower limit is hit vertexCount - 1 is returned.
//----------------------------------------------------------------------------------------
template <typename INDEXTYPE, typename COUNT>
inline INDEXTYPE GetSuccessor(INDEXTYPE index, COUNT vertexCount)
{
	return MAXON_UNLIKELY(index + 1 >= (INDEXTYPE)vertexCount) ? 0 : (index + 1);
}
