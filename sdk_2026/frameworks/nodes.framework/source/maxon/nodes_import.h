...
public:
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the complete node path of this node.
	/// @return												Path of this node.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD const NodePath& GetPath() const;

	//----------------------------------------------------------------------------------------
	/// @brief Returns the attributes of this node. An ImportAdapter can use this to modify, add or remove attribute values.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Finds the child of this node with the given identifer.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Removes the child of this node with the given identifier (if it exists).
	/// @param[in] child							Identifier of the child.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Adds a new child node entry to the serialization data, or returns the already existing one.
	...
	MAXON_METHOD Result<SerializationNodeData*> FindOrAddChild(const InternedId& child, CHILD_MODE mode,
																														 const IdAndVersion& base = GetZeroRef<IdAndVersion>(), const DataDictionary& baseArgs = DataDictionary::DefaultValue(), ASSET_UPDATE_POLICY updatePolicy = ASSET_UPDATE_POLICY::IMPLICIT);

	//----------------------------------------------------------------------------------------
	/// @brief Returns the child mode of this entry.
	/// @return												Child mode of entry.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD CHILD_MODE GetChildMode() const;

	//----------------------------------------------------------------------------------------
	/// @brief Sets the asset identifier of the base template for this SerializationNodeData.
	/// An ImportAdapter can modify the identifier to handle renaming of node assets.
	/// The asset identifier is defined only for true nodes, but not if this serialization node
	/// represents a port or port list.
	/// @param[in] newAid							The asset identifier of the new base template to use.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> SetBaseTemplate(const IdAndVersion& newAid);

	//----------------------------------------------------------------------------------------
	/// @brief Returns the asset identifier of the base template for this SerializationNodeData.
	/// An ImportAdapter can modify the identifier to handle renaming of node assets.
	/// The asset identifier is defined only for true nodes, but not if this serialization node
	/// represents a port or port list.
	...
	//----------------------------------------------------------------------------------------
	/// @brief Sets the asset update policy of the base template for this SerializationNodeData.
	/// The update policy is used only for true nodes, not for ports or port lists.
	/// @param[in] newPolicy					The new base template asset update policy.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> SetUpdatePolicy(ASSET_UPDATE_POLICY newPolicy);

	//----------------------------------------------------------------------------------------
	/// @brief Returns the asset update policy of the base template for this SerializationNodeData.
	/// The update policy is used only for true nodes, but not if this serialization node
	/// represents a port or port list.
	/// @return												Asset update policy for the base template.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// @brief Returns the link to the base template's asset for this SerializationNodeData.
	/// @return												Asset identifier of the base template.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD const AssetLink& GetBaseTemplateLink() const;

	//----------------------------------------------------------------------------------------
	/// @brief Overwrites the template arguments for the base template for this SerializationNodeData.
	/// Template arguments are defined only for true nodes, but not if this serialization node
	/// represents a port or port list.
	/// @param[in] args								The new base template arguments.
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<void> SetBaseTemplateArguments(const DataDictionary& args);

	//----------------------------------------------------------------------------------------
	/// @brief Returns the template arguments for the base template for this SerializationNodeData.
	/// Template arguments are defined only for true nodes, but not if this serialization node
	/// represents a port or port list.
...
/// such an adapter can rename identifiers of referenced node templates
