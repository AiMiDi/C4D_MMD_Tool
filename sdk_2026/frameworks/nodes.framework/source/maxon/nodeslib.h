...
class NodesLib
{
	MAXON_INTERFACE_NONVIRTUAL(NodesLib, MAXON_REFERENCE_STATIC, "net.maxon.node.interface.nodeslib", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
public:
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] detachable					True if template instantiations shall be user-convertible to editable nodes (then they are detached from the template).
	/// @return												A NodeSystemBasedNodeTemplate based on #templ.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<NodeSystemBasedNodeTemplate> CreateTemplateFromNodeSystem(const NodeSystem& templ, const AssetRepositoryRef& lookup, Bool detachable);
	...
	/// Function should always be called using customized iferr (...) handling to handle the case where the desired template
	/// is not registered (missing module or plugin) or just missing. In such case, an IllegalArgumentError is returned and should
	/// be handled before the error bubbles up to higher level code.
	...
	/// but restricted to the builtin asset repository. The returned asset is a decorated version of the
	/// corresponding NodeTemplate in the BuiltinNodes registry,
	/// adding support for meta data, resource descriptions and instantiation cache.
	/// @param[in] assetId						The id of the asset to load.
	/// @return												The loaded NodeTemplate asset, or an IllegalArgumentError if no such asset exists.
	//----------------------------------------------------------------------------------------
	...
	///																Pass a default value (the default) to use the wrapped implementation.
	...
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<NodeSystemBasedNodeTemplate> BuildNodeFromDescription(const Id& id, const NodeSystemClass& nodeClass, Bool addDependencies = true,
	...
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD Result<DescriptionProcessor> CreateNodeDescriptionProcessor(Delegate<Result<NodeTemplate>(const Id& descriptionId, const DataDescriptionDefinition& dataDef, const DataDescription& dataDescription)>&& delegate);
	...
	//----------------------------------------------------------------------------------------
	/// Creates a UUID (universally unique identifier) for a port. If prefix is given, the returned identifier
	/// starts with prefix, otherwise it is a UUID only.
	///
	///	The implementation is based on AssetInterface::MakeUuid. In addition to that method it makes sure
	///	that special port ids such as those with component access tags are handled correctly.
	///
	...
	//----------------------------------------------------------------------------------------
	/// Creates a UUID (universally unique identifier) for a port.
	/// The UUID will have a direction-dependent prefix.
	///
	...
	//----------------------------------------------------------------------------------------
	/// Returns the path of the internal GlobalContext input port (i.e., "<globalcontext").
	/// @return												Path of GlobalContext input port.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD const NodePath& GetGlobalContextPath();
};
...
#define PRIVATE_MAXON_DECLARATION_REGISTER_BUILTINNODE(id, enable, CNT, ...) \
	...
	{ \
		...
		return maxon::DisabledError(MAXON_SOURCE_LOCATION); \
	} \
	static maxon::Result<maxon::nodes::NodeTemplate> MAXON_CONCAT(PrivateLazyNode, CNT)()
