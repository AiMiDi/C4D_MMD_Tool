...
#include "maxon/fixedsizearray.h"
...
/// @see NodeSystemManagerInterface::Update
/// @see NodeSystemInterface::Update
//----------------------------------------------------------------------------------------
using UpdateAssetEntry = Tuple<AssetReference /* The current asset instance infos */, AssetBase /* The new asset version */>;

/// LiteralId (constexpr) value type not allowed on ports.
/// @see DataDescriptionInterface::Set
#if __cplusplus >= 202002L
/// Allows to use C++20 concepts with templated arguments and still compile targets with older C++ version.
#define MAXON_CPP20_REQUIRES(conceptDef, valueType) requires conceptDef<valueType>
template <typename T>
concept g_NoLiteralIdValueConcept = std::negation<std::is_same<std::remove_cvref_t<T>, LiteralId>>::value;
#else 
...
struct GNodeDataPtr
{
	...
	explicit operator Bool() const
	{
		...
		// Also compare for system with MSVC compiler. Otherwise we end up in a crash when iterating over a linked list of GNodeDataPtr.
		return ptr != nullptr && system != nullptr;
#else
		return ptr != nullptr;
#endif
	}
...
enum class VALUEMODE : Char
{
	...
	/// The port's value is an unknown constant. This mode happens when the value mode CONSTANT is injected into a template/group
	/// but the corresponding constant value is not injected (because the template/group isn't interested in the value itself).
	UNKNOWN_CONSTANT,	
...
{
	MAXON_INTERFACE(NodeSystemInterface, MAXON_REFERENCE_COPY_ON_WRITE, "net.maxon.node.interface.nodesystem", MAXON_IMPLEMENTATION_MODULE("net.maxon.nodes"));
...
public:
	...
	///
	/// The reference to the node system is moved from the original NodeSystem reference
	/// (on which this function is called) to the returned MutableRoot, so the original reference is cleared.
	/// The returned MutableRoot has the only reference to the mutable node system.
	/// When you are done with all modifications, you have to write the reference to the modified node system
	/// back to the original reference (or some other place), otherwise the modifications will get lost
	...
	///	... // apply some changes to root, system is a null reference along the way
	...
	//----------------------------------------------------------------------------------------
	Result<MutableRoot> BeginModification(const AssetRepositoryRef& repo);
	);
	...
	///
	/// For the topological layering only top-level ports are considered, connections of nested ports are treated as if
	/// they were at their enclosing top-level ports. Therefore the layer of a nested
	...
	//----------------------------------------------------------------------------------------
	///	Converts all attribute values within this node system using the given conversion delegate.
	///	This method can be used e.g. to convert all color values to a different color space.
	///	Each attribute value within this node system (including nested node systems) is passed to the conversion,
	///	and if the conversion returns a value, then this is used as new value. The method handles
	///	- node and port attributes, then path and attribute id are passed to the conversion
	///	- template arguments with a path key, then the path is passed together with an empty attribute id
	///	- template arguments with a non-path key, then both path and attribute id are empty.
	///
	/// @param[in] conversion					The attribute conversion delegate.
	/// @param[in] repo								The repository to use for asset lookup during the conversion.
	/// @return												Node system with converted attributes, will be the original node system if no change has been made.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD Result<NodeSystem> ConvertAttributeValues(const AttributeValueConversion& conversion, const AssetRepositoryRef& repo) const;
...
private:
	...
	MAXON_METHOD Result<void> SetTemplate(const NodeTemplate& templ, const TemplateArguments& args, Bool setValues);
	...
	MAXON_METHOD Result<void> Update(const Block<const UpdateAssetEntry>& updates, const AssetRepositoryRef& lookup, ChangeList* list, const ValueReceiver<const UpdateAssetEntry&>& updatedRecv);
	...
	MAXON_METHOD Result<HomogenousTupleType<3, HashMap<InternedId, InternedId>>> Merge(const NodeSystem& other, const AttributeValueConversion& conversion, Bool validate, ChangeList* list);
	...
	MAXON_METHOD Result<Bool> GetConnectionValues(const GNodeHandle& source, const GNodeHandle& target, const ValueReceiver<CONNECTION_POSITION, const InternedId&, const ConstDataPtr&>& receiver) const;
...
public:
	...
	/// @return												True if the value has been changed, false otherwise.
	//----------------------------------------------------------------------------------------
	template <typename ATTR> MAXON_CPP20_REQUIRES(g_NoLiteralIdValueConcept, typename ATTR::ValueType)
...
public:
	...
	Result<HomogenousTupleType<3, HashMap<InternedId, InternedId>>> Merge(const NodeSystem& other, const AttributeValueConversion& conversion = {}, Bool validate = true, Opt<ChangeList&> list = {}) const
	{
		iferr_scope;
		CheckState(_system);
		return _system->Merge(other, conversion, validate, list);
	}
	...
	//----------------------------------------------------------------------------------------
	Result<void> SetTemplate(const NodeTemplate& templ, const TemplateArguments& args) const;
	...
	/// @param[in] args								The template arguments used for the instantiation.
	//----------------------------------------------------------------------------------------
	[[deprecated("Use the overload with 2 parameters (since 2024.4).")]] Result<void> SetTemplate(const InstantiationTrace& trace, const NodeTemplate& templ, const TemplateArguments& args) const
	{
		return SetTemplate(templ, args);
	}
...
public:
	...
	/// LiteralId data type is not allowed #value type. See DataDictionary::Set.
	/// @param[in] value							New value for the port.
	/// @return												True if the value has been changed, false otherwise.
	//----------------------------------------------------------------------------------------
	template <typename T> MAXON_CPP20_REQUIRES(g_NoLiteralIdValueConcept, T)
	...
	/// LiteralId data type is not allowed. See DataDictionary::Set.
