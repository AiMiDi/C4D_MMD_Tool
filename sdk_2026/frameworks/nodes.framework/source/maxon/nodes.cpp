...
template <typename S> Result<MutableRoot> NodeSystemInterface::Hxx1::COWFn<S>::BeginModification(const AssetRepositoryRef& repo)
{
	...
	iferr (w->BeginModificationImpl(repo))
		return err;
	return MutableRoot(std::move(this->PrivateGetRefMember()));
}

template Result<MutableRoot> NodeSystem::ReferenceBaseSuper::BeginModification(const AssetRepositoryRef& repo);
template Result<MutableRoot> Result<NodeSystem>::Base::BeginModification(const AssetRepositoryRef& repo);
...
Result<void> MutableRoot::SetTemplate(const NodeTemplate& templ, const TemplateArguments& args) const
{
	iferr_scope;
	CheckState(_system);
	return _system->SetTemplate(templ, args, true);
}
...
template <typename BASE> Result<MutablePort> MutablePortListFunctions<BASE>::AddPort(const InternedId& portId) const
{
	return AddPort(portId.Get());
}
...
template <typename BASE> Result<MutablePort> MutablePortListFunctions<BASE>::FindOrAddPort(const InternedId& portId) const
{
	return FindOrAddPort(portId.Get());
}
