        ...
        if method.kind == 'MAXON_METHOD':
            ...
            if cls.interface == Class.VIRTUAL:
                ...
                if freeMethod and cls.derived:
                    ...
                    ptr = 'BaseInterface::MTable::_instance.'
                elif inheritedMethod:
        ...
        if cls.refClass and method.includeInRefClass():
            ...
            for cowMode in range(1 if onlyCow else 0, 2):
                ...
                if method.static:
                    ...
                    if result:
                        rfd << 'if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this);'
                    ...
                    if method.const:
                        rfd << 'const ' + cls.name + '* o_ = (const ' + cls.name + '*) this->GetPointer();' \
                            << 'if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::DEEP_CONSTNESS | maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { o_ = ' + cls.name + '::NullValuePtr(); if (!o_) return ' + defaultReturn.replace('@', 'NULLPTR') + '; }'
                    else:
                        rfd << cls.name + '* o_ = (' + cls.name + '*) this->' + ('MakeWritable(' + cowReset + ').GetPointer();' if cowMode else 'GetPointer();') \
                            << 'if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (' + ('maxon::VALUEKIND::DEEP_CONSTNESS | ' if cowMode else '') + 'maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) {' + ('' if (cowMode or methodRetType != 'void') else ' DebugStop();')  + ' return ' + nullptrReturn.replace('@', 'MAKE_WRITABLE_FAILED' if cowMode else 'NULLPTR') + '; }'
                    oPointer = 'o_'
                    if method.forward:
