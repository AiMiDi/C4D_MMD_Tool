...
class Parser(object):
    ...
    # returns tuple (type as string, bool, param name, forward-bool) where the bool tells if the type is a const-qualified type
    # and forward is True when the type contains one of the types of self.forwards (but not if they are followed by * or &)
        ...
        if t == 'MAXON_REGISTRY' or t == 'MAXON_DECLARATION' or t == 'MAXON_DECLARATION_CLASS':
            ...
            if t == 'namespace':
                ...
                if t == '=':
                    ...
                    if t == 'MAXON_FORWARD':
                        self.forwards.add(name)
                        self.lex.consume('(')
                        self.root.forwardHeaders.add(self.lex.getString())
                        self.lex.consume(')')
                        t = self.lex.get(owner)
                    return decl
                else:
