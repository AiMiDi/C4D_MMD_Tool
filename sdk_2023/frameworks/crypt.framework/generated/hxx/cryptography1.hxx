...
struct CryptographyStreamConversionInterface::Hxx1
{
	...
	public:
		...
		static ConstFn PrivateLookupFn(CryptographyStreamConversionInterface**);
...
/// @return												True, if it is a chaining chiffre.
	...
	public:
		...
		using PrivateBaseClass::PrivateBaseClass;
		using PrivateBaseClass::PrivateLookupFn;
		static Fn PrivateLookupFn(CryptographyStreamConversionInterface**);
	...
	public:
		...
		using PrivateBaseClass::PrivateBaseClass;
		using PrivateBaseClass::PrivateLookupFn;
		static COWFn PrivateLookupFn(CryptographyStreamConversionInterface**);
	...
	typename std::conditional<std::is_const<typename ST::type::PrivateReferencedType>::value, ConstFn<typename StreamConversionInterface::Hxx1::template FnHelper<ST>::type>, Fn<typename StreamConversionInterface::Hxx1::template FnHelper<ST>::type>>::type
	> { };
};
class CryptographyStreamConversionInterface::Hxx1::Reference :
#ifdef DOXYGEN
public FnDoxy<CryptographyStreamConversionRef>
#else
public maxon::RefBase<Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<CryptographyStreamConversionInterface, maxon::StrongRefHandler, CryptographyStreamConversionRef>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::RefBase<Hxx1::FnHelper<maxon::RefBaseFn<maxon::DirectRef<CryptographyStreamConversionInterface, maxon::StrongRefHandler, CryptographyStreamConversionRef>>>>);
	using NonConst = typename Super::ReferenceClass;
};

/// CryptographyStreamConversionRef is the reference class of CryptographyStreamConversionInterface.
///
/// CryptographyStreamConversionInterface class for encrypting and decrypting.
/// To allocate an instance of a CryptographyStreamConversionRef call StreamConversions::<<encoder/decoder>>.Create(cryptoSettings).
/// Encoder and decoder can be of AesEncoder, AesDecoder, BlowfishEncoder, BlowfishDecoder etc.
/// Currently supported cryptography methods are:
///
/// AES
///		Use StreamConversions::AesEncoder.Create(cryptoSettings) as name to allocate a new instance.
...
///
/// Blowfish
///		Use StreamConversions::BlowfishEncoder.Create(cryptoSettings) to allocate a class instance. Blowfish is a keyed, symmetric block cipher. @see https://en.wikipedia.org/wiki/Blowfish_(cipher)
...
///
///		Use BlowfishLegacyEncoder/BlowfishLegacyDecoder as the name to allocate the class which is compatible to CINEMA 4D R5 - R14 blowfish.
///		The Block size must be 64 or 2048 bit. Valid key lengths are 32 to 448 bit (must be divisible by 8).
///		The Init() flag bit 1 enables the streaming mode which causes that identical blocks are encrypted different and so it's harder to find patterns.
