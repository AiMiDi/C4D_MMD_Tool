...
/// Returns a readable string of the content.
/// @return												The converted result.
		inline typename std::conditional<S::HAS_ERROR, maxon::Result<CString>, CString>::type ToCString() const;
...
/// @return                       String on success.
		[[deprecated("use ConvertToUiNameWithRepository instead")]] inline Result<String> ConvertToUiName(CONVERTTOUINAMEFLAGS flags) const;
...
/// @return                       Url on success. Please note that the url might be translated into another scheme (e.g. assetdb:// -> asset://).
		[[deprecated("use ConvertFromUiNameWithRepository instead")]] static inline Result<Url> ConvertFromUiName(const String& str);
...
/// @return                       String on success.
		inline Result<String> ConvertToUiNameWithRepository(CONVERTTOUINAMEFLAGS flags, const AssetRepositoryRef& lookupRepository) const;
/// ConvertFromUiName converts a string into a url.
/// @param[in] str								String to convert.
/// @param[in] lookupRepository		AssetRespoitory to search in.
/// @return                       Url on success. Please note that the url might be translated into another scheme (e.g. assetdb:// -> asset://).
		static inline Result<Url> ConvertFromUiNameWithRepository(const String& str, const AssetRepositoryRef& lookupRepository);
	};
	/// Intermediate helper class for UrlInterface.
...
/// that under Windows all backslashes will be changed into forward slashes.
/// @param[in] path								The new path.
/// @param[in] convertWindowsBackslashes	True to convert backslashes under windows.
/// @return												OK on success.
		inline Result<void> SetPath(const String& path, Bool convertWindowsBackslashes) const;
...
/// @param[in] convertWindowsBackslashes	True to convert backslashes under windows.
/// @return												OK on success.
		inline Result<void> SetPath(const String& path, Bool convertWindowsBackslashes);
/// Stores a system path in a Url.
/// This call needs to be done after the appropriate scheme has been set.
/// If no scheme is set URLSCHEME_FILESYSTEM will be automatically set.
/// The system path may be converted to a different internal representation, e.g. split
