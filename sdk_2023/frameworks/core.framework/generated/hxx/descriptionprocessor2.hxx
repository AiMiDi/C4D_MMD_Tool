...
struct DescriptionProcessorInterface::MTable
{
	...
	PRIVATE_MAXON_MF_POINTER(DescriptionProcessorInterface_Process, Process, true, maxon::GenericComponent, const, (Result<GenericData>), const Id& descriptionId, const DataDescription& dataDescription);
...
struct DescriptionProcessorInterface::Hxx2
{
	...
	public:
		...
		PRIVATE_MAXON_MF_WRAPPER(DescriptionProcessorInterface_Process, maxon::GenericComponent, const, (Result<GenericData>), const Id& descriptionId, const DataDescription& dataDescription) { return ((const typename S::Implementation*) PRIVATE_MAXON_MF_THIS(const maxon::GenericComponent))->Process(descriptionId, dataDescription); }
	};
...
{
	const DescriptionProcessorInterface::MTable& mt_ = PRIVATE_MAXON_VTABLE(DescriptionProcessorInterface, this); return PRIVATE_MAXON_MF_INVOKE(((const maxon::GenericComponent*) this + mt_.DescriptionProcessorInterface_Process_Offset), mt_.DescriptionProcessorInterface_Process, descriptionId, dataDescription);
}
...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, Result<GenericData>>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); const DescriptionProcessorInterface* o_ = (const DescriptionProcessorInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::DEEP_CONSTNESS | maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { o_ = DescriptionProcessorInterface::NullValuePtr(); if (!o_) return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(DescriptionProcessorInterface, o_, DescriptionProcessorInterface_Process) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBaseDetector::template Check<DescriptionProcessorInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<DescriptionProcessorInterface>() : PRIVATE_MAXON_VTABLE(DescriptionProcessorInterface, o_); 
	return (PRIVATE_MAXON_MF_INVOKE((reinterpret_cast<const maxon::GenericComponent*>(o_) + mt_.DescriptionProcessorInterface_Process_Offset), mt_.DescriptionProcessorInterface_Process, descriptionId, dataDescription));
}
