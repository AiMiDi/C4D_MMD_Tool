...
public:
	...
	MAXON_METHOD Result<String> ConvertToUiName(const UrlInterface& url, CONVERTTOUINAMEFLAGS flags) const
	{
		return maxon::Cast<IoHandler>(self.GetDecoratedObject()).ConvertToUiName(url, flags);
	}
	...
	{
		return maxon::Cast<IoHandler>(self.GetDecoratedObject()).ConvertToUiNameWithRepository(url, flags, lookupRepository);
	}
