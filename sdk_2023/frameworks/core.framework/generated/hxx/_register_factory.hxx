...
namespace maxon
{
	...
	maxon::Bool FactoryFactory::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
	{
		...
		if (maxon::details::NullReturnType<Result<GenericFactory::NonConst>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->FactoryFactory_PrivateCreate = PRIVATE_MAXON_MF_CAST(decltype(FactoryFactory_PrivateCreate), &Hxx2::Wrapper<Hxx2::Unresolved>::FactoryFactory_PrivateCreate);
		#else
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if (maxon::details::NullReturnType<Result<GenericFactory::NonConst>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->FactoryFactory_PrivateCreateCached = PRIVATE_MAXON_MF_CAST(decltype(FactoryFactory_PrivateCreateCached), &Hxx2::Wrapper<Hxx2::Unresolved>::FactoryFactory_PrivateCreateCached);
		#else
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if (maxon::details::NullReturnType<Result<const Data&>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->FactoryFactory_CreateDataCached = PRIVATE_MAXON_MF_CAST(decltype(FactoryFactory_CreateDataCached), &Hxx2::Wrapper<Hxx2::Unresolved>::FactoryFactory_CreateDataCached);
		#else
