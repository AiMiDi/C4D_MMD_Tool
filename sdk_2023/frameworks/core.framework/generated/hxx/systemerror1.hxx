...
/// @param[in] osError						CFError to set.
		inline typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type SetCFError(CFErrorRef osError) const;
	};
	/// Intermediate helper class for NSSystemErrorInterface.
	template <typename S> class COWFn : public ConstFn<S>
	{
...
/// Sets the encapsulated CFError.
/// @param[in] osError						CFError to set.
		inline typename std::conditional<maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS), void, maxon::Result<void>>::type SetCFError(CFErrorRef osError);
	};
	template <typename ST> struct FnHelper : public std::conditional<
	maxon::Bool(ST::type::Handler::KIND & (maxon::VALUEKIND::COW_MASK | maxon::VALUEKIND::DEEP_CONSTNESS)),
	COWFn<typename ErrorInterface::Hxx1::template FnHelper<ST>::type>,
