...
struct UrlFileCacheEntryInterface::Hxx2
{
	...
	public:
		...
		PRIVATE_MAXON_MF_WRAPPER(UrlFileCacheEntryInterface_GetSourceUrl, UrlFileCacheEntryInterface, const, (Url)) { return C::Get(PRIVATE_MAXON_MF_THIS(const UrlFileCacheEntryInterface))->GetSourceUrl(); }
		PRIVATE_MAXON_MF_WRAPPER(UrlFileCacheEntryInterface_GetCacheUrl, UrlFileCacheEntryInterface, const, (Url)) { return C::Get(PRIVATE_MAXON_MF_THIS(const UrlFileCacheEntryInterface))->GetCacheUrl(); }
	};
...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<Url>, Url>::type>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); const UrlFileCacheEntryInterface* o_ = (const UrlFileCacheEntryInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::DEEP_CONSTNESS | maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { o_ = UrlFileCacheEntryInterface::NullValuePtr(); if (!o_) return maxon::PrivateIncompleteNullReturnValue<Url>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } 
