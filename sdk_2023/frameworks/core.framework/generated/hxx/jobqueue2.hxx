...
struct JobQueueInterface::Hxx2
{
	...
	public:
		...
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_GetThreadCount, JobQueueInterface, const, (Int)) { return C::Get(PRIVATE_MAXON_MF_THIS(const JobQueueInterface))->GetThreadCount(); }
		...
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_Cancel, JobQueueInterface,, (void), UInt ownerId) { return C::Get(PRIVATE_MAXON_MF_THIS(JobQueueInterface))->Cancel(ownerId); }
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_CancelAndWait, JobQueueInterface,, (void), UInt ownerId) { return C::Get(PRIVATE_MAXON_MF_THIS(JobQueueInterface))->CancelAndWait(ownerId); }
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_EnableThreadAffinity, JobQueueInterface,, (void), UInt ownerId) { return C::Get(PRIVATE_MAXON_MF_THIS(JobQueueInterface))->EnableThreadAffinity(ownerId); }
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_AlwaysUseCurrentThread, JobQueueInterface,, (void), UInt ownerId) { return C::Get(PRIVATE_MAXON_MF_THIS(JobQueueInterface))->AlwaysUseCurrentThread(ownerId); }
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_SetIdleSpinTime, JobQueueInterface,, (void), UInt ownerId, TimeValue spinTime) { return C::Get(PRIVATE_MAXON_MF_THIS(JobQueueInterface))->SetIdleSpinTime(ownerId, std::forward<TimeValue>(spinTime)); }
...
/// @cond IGNORE
		...
		PRIVATE_MAXON_MF_WRAPPER(JobQueueInterface_ExecuteJobs, JobQueueInterface,, (void), TimeValue timeout) { return C::Get(PRIVATE_MAXON_MF_THIS(JobQueueInterface))->ExecuteJobs(std::forward<TimeValue>(timeout)); }
/// @endcond
...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<Int>, Int>::type>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); const JobQueueInterface* o_ = (const JobQueueInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::DEEP_CONSTNESS | maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { o_ = JobQueueInterface::NullValuePtr(); if (!o_) return 0; } 
...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); JobQueueInterface* o_ = (JobQueueInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { DebugStop(); return maxon::PrivateLogNullptrError(); } 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto JobQueueInterface::Hxx1::Fn<S>::AlwaysUseCurrentThread(UInt ownerId) const -> typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); JobQueueInterface* o_ = (JobQueueInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { DebugStop(); return maxon::PrivateLogNullptrError(); } 
	PRIVATE_MAXON_MF_INVOKE((o_), MTable::_instance.JobQueueInterface_AlwaysUseCurrentThread, ownerId);
	return (typename ReturnHelper::ReturnType) maxon::OK;
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto JobQueueInterface::Hxx1::Fn<S>::SetIdleSpinTime(UInt ownerId, TimeValue spinTime) const -> typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); JobQueueInterface* o_ = (JobQueueInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { DebugStop(); return maxon::PrivateLogNullptrError(); } 
