...
namespace maxon
{
	...
	maxon::Bool CrashHandler::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
	{
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if (maxon::details::NullReturnType<Result<void>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->CrashHandler_SetCrashDataDirectory = PRIVATE_MAXON_MF_CAST(decltype(CrashHandler_SetCrashDataDirectory), &Hxx2::Wrapper<Hxx2::Unresolved>::CrashHandler_SetCrashDataDirectory);
		#else
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if (maxon::details::NullReturnType<Result<void>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->CrashHandler_GetProcessModules = PRIVATE_MAXON_MF_CAST(decltype(CrashHandler_GetProcessModules), &Hxx2::Wrapper<Hxx2::Unresolved>::CrashHandler_GetProcessModules);
		#else
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if (maxon::details::NullReturnType<Result<void>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->CrashHandler_ResolveBugReport = PRIVATE_MAXON_MF_CAST(decltype(CrashHandler_ResolveBugReport), &Hxx2::Wrapper<Hxx2::Unresolved>::CrashHandler_ResolveBugReport);
		#else
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if (maxon::details::NullReturnType<maxon::ObservableRef<ObservableCrashDataDirectoryDelegate>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->CrashHandler_ObservableCrashDataDirectory = PRIVATE_MAXON_MF_CAST(decltype(CrashHandler_ObservableCrashDataDirectory), &Hxx2::Wrapper<Hxx2::Unresolved>::CrashHandler_ObservableCrashDataDirectory);
		#else
