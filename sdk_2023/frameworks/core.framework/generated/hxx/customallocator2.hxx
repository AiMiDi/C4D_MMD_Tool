...
struct CustomAllocatorInterface::Hxx2
{
	...
	public:
		...
		PRIVATE_MAXON_MF_WRAPPER(CustomAllocatorInterface_Alloc_1, CustomAllocatorInterface,, (void*), Int s, const maxon::SourceLocation& allocLocation) { return C::Get(PRIVATE_MAXON_MF_THIS(CustomAllocatorInterface))->Alloc(s, allocLocation); }
		PRIVATE_MAXON_MF_WRAPPER(CustomAllocatorInterface_AllocClear, CustomAllocatorInterface,, (void*), Int s, const maxon::SourceLocation& allocLocation) { return C::Get(PRIVATE_MAXON_MF_THIS(CustomAllocatorInterface))->AllocClear(s, allocLocation); }
		PRIVATE_MAXON_MF_WRAPPER(CustomAllocatorInterface_Realloc, CustomAllocatorInterface,, (void*), void* data, Int size, const maxon::SourceLocation& allocLocation) { return C::Get(PRIVATE_MAXON_MF_THIS(CustomAllocatorInterface))->Realloc(data, size, allocLocation); }
		PRIVATE_MAXON_MF_WRAPPER(CustomAllocatorInterface_PrivateAlloc, CustomAllocatorInterface,, (void*), Int size, Bool clear, const maxon::SourceLocation& allocLocation) { return C::Get(PRIVATE_MAXON_MF_THIS(CustomAllocatorInterface))->PrivateAlloc(size, clear, allocLocation); }
		PRIVATE_MAXON_MF_WRAPPER(CustomAllocatorInterface_PrivateFree, CustomAllocatorInterface,, (void), void* data) { return C::Get(PRIVATE_MAXON_MF_THIS(CustomAllocatorInterface))->PrivateFree(data); }
		PRIVATE_MAXON_MF_WRAPPER(CustomAllocatorInterface_PrivateGetDataSize, CustomAllocatorInterface,, (Int), void* data) { return C::Get(PRIVATE_MAXON_MF_THIS(CustomAllocatorInterface))->PrivateGetDataSize(data); }
	};
...
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::NEVER_NULLPTR), false, typename std::conditional<S::HAS_ERROR, maxon::Result<void*>, void*>::type>; if (S::HAS_ERROR && MAXON_UNLIKELY(this->GetErrorStorage().GetReferencedError())) return ReturnHelper::ReturnErrorOf(this); CustomAllocatorInterface* o_ = (CustomAllocatorInterface*) this->GetPointer(); if (!std::integral_constant<maxon::Bool, maxon::Bool(S::Handler::KIND & (maxon::VALUEKIND::NEVER_NULLPTR))>::value && MAXON_UNLIKELY(!o_)) { return nullptr; } 
