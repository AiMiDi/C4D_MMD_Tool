...
public:
	...
	/// @param[in] nullValue					Pointer to a default-constructed object of type T. The lifetime needs to be longer than the lifetime of the constructed DataTypeImpl object.
	...
	//----------------------------------------------------------------------------------------
	template <typename T, typename C> DataTypeImpl(const Id& type, const DataTypeImpl* underlying, const DataTypeImpl* element, const T* nullValue, const C* dummy,
		...
		_underlying(underlying), _elementType(element), _extraType(extra), _parametricType(pt), _implementation(impl), _conversions(), _nullValue(reinterpret_cast<const Generic*>(nullValue)),
	...
	/// @param[in] underlying					The underlying, layout-compatible type, may be nullptr.
	/// @param[in] element						The element type (used e.g. for arrays or pointers), may be nullptr.
	/// @param[in] nullValue					Pointer to a default-constructed object of the type. The lifetime needs to be longer than the lifetime of the constructed DataTypeImpl object.
	...
	//----------------------------------------------------------------------------------------
	template <typename C> DataTypeImpl(const Id& type, const DataTypeImpl* underlying, const DataTypeImpl* element, const void* nullValue, Int size, Int alignment, const C* dummy, VALUEKIND kind, const ParametricTypeInterface* pt = nullptr, const DataTypeImpl* extra = nullptr)
		: _id(type), _size(size), _alignment(alignment),
			_valueKind(((size <= SIZEOF(Int)) ? kind | VALUEKIND::INT_SIZE_OR_LESS : kind)
								 | (((element ? element->GetValueKind() : VALUEKIND::NONE) | (underlying ? underlying->GetValueKind() : VALUEKIND::NONE)) & VALUEKIND::CONTAINS_MASK)),
		_underlying(underlying), _elementType(element), _extraType(extra), _parametricType(pt), _typeArgs(nullptr), _conversions(), _nullValue(reinterpret_cast<const Generic*>(nullValue)),
		_construct(&C::Construct), _destruct(&C::Destruct), _moveConstruct(&C::MoveConstruct), _moveFrom(&C::MoveFrom), _copyFrom(&C::CopyFrom),
		_addReference(&C::AddReference), _removeReference(&C::RemoveReference),
		_toString(&C::ToStringImpl), _isEqual(&C::IsEqual), _compare(&C::Compare), _getHashCode(&C::GetHashCode), _getUniqueHashCode(&C::GetUniqueHashCode),
		_describeIO(&C::DescribeIO), _newInstance(&C::Create),
	...
	DataTypeImpl() : _id("uninitialized"), _size(0), _alignment(0), _valueKind(VALUEKIND::NONE), _underlying(), _elementType(), _parametricType(nullptr), _typeArgs(nullptr), _conversions(), _nullValue(nullptr),
	...
	DataTypeImpl(const DataTypeImpl& src, const Id& typeId, VALUEKIND qualification, const ParametricTypeInterface* pt, VALUEKIND remove = VALUEKIND::NONE, Bool copyTypeArgs = true, const DataTypeImpl* element = nullptr) : _id(typeId),
		...
		MAXON_COPY_MEMBERS(_nullValue, _construct, _destruct, _moveConstruct, _moveFrom, _copyFrom, _addReference, _removeReference, _toString, _isEqual, _compare, _getHashCode, _getUniqueHashCode,
...
protected:
	...
	const Generic* _nullValue; ///< Pointer to the null value of the type.
...
public:
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const ConstDataPtr& NullValue() { return GetZeroRef<ConstDataPtr>(); }
...
public:
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const DataPtr& NullValue() { return GetZeroRef<DataPtr>(); }
...
public:
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const MoveDataPtr& NullValue() { return GetZeroRef<MoveDataPtr>(); }
};
