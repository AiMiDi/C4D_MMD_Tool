...
ConstDataPtr DataType::GetNullValue() const
{
	const Generic* def = _ptr ? _ptr->GetNullValue() : nullptr;
...
Result<void> DataTypeImpl::InitializeNullValue()
{
	iferr_scope;
	CheckState(!_nullValue && !(_valueKind & VALUEKIND::ABSTRACT));
	if (_valueKind & VALUEKIND::ZERO_INITIALIZED && _size <= ZERO_MEMORY_SIZE)
	{
		_nullValue = reinterpret_cast<const Generic*>(g_zeroMemory);
	}
	else
	{
		const void* ptr = Create() iferr_return;
		_nullValue = reinterpret_cast<const Generic*>(ptr);
		_flags |= FLAGS::OWNS_NULL_VALUE;
	}
	return OK;
}
...
void DataTypeImpl::ResetNullValue()
{
	if (_flags & FLAGS::OWNS_NULL_VALUE)
	{
		Destruct(_nullValue);
		System::Free(_nullValue);
		_flags &= ~FLAGS::OWNS_NULL_VALUE;
	}
	_nullValue = nullptr;
}
