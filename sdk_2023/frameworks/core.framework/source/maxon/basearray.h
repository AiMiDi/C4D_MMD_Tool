...
public:
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE void Destruct(T* dst, Int destructCnt)
	{
		...
		if (STD_IS_REPLACEMENT(pod, T) == false)
		{
			for (Int i = destructCnt - 1; i >= 0; i--)
				dst[i].~T();
		}
...
protected:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE BaseArrayData(BaseArrayData&& src) : MAXON_MOVE_MEMBERS(_ptr, _cnt, _capacity)
	{
		...
		{
			using Utilities = BaseArrayUtilities<T, STD_IS_REPLACEMENT(trivially_copyable, T) /*|| MEMFLAGS == BASEARRAYFLAGS::MOVEANDCOPYOBJECTS*/>;
...
template <typename T, Int MINCHUNKSIZE = BASEARRAY_DEFAULT_CHUNK_SIZE, BASEARRAYFLAGS MEMFLAGS = BASEARRAYFLAGS::NONE, typename ALLOCATOR = DefaultAllocator> class BaseArray
	: public ArrayBase<BaseArray<T, MINCHUNKSIZE, MEMFLAGS, ALLOCATOR>, T, BaseArrayData<T, ALLOCATOR, STD_IS_REPLACEMENT(empty, ALLOCATOR)>, DefaultCompare>
{
...
// old code without forwarding
	...
	template <typename... ARGS> MAXON_ATTRIBUTE_FORCE_INLINE typename std::enable_if<sizeof...(ARGS) != 1, ResultRef<T>>::type Append(ARGS&&... args)
	{
		...
		return new (_ptr + _cnt++) T(std::forward<ARGS>(args)...);
	}
#endif
...
private:
	...
	MAXON_WARN_UNUSED T* IncreaseCapacity(Int increment = 1, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::ON_GROW_RESERVE_CAPACITY)
	{
		...
		if (MAXON_UNLIKELY(requestedCapacity > _capacity))
		{
			...
			if (STD_IS_REPLACEMENT(base_of, FixedBufferAllocatorBase, ALLOCATOR))
			{
