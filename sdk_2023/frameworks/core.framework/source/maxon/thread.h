...
template <typename FN> inline auto ExecuteOnMainThread(FN&& fn, WAITMODE waitMode, TimeValue timeout) -> decltype(fn())
{
	...
	{
		using JobWithConfirmedCancellation = ClosureJobWithConfirmedCancellation<FN, JOBCANCELLATION::AUTOMATIC, decltype(fn())>;

		// On allocation error use the fallback to guarantee execution.
		iferr (JobWithConfirmedCancellation* cjob = NewObj(JobWithConfirmedCancellation, std::forward<FN>(fn)))
		...
		if (waitMode != WAITMODE::DONT_WAIT)
		{
			if (waitMode == WAITMODE::RETURN_ON_CANCEL)
			{
				...
				{
					// Invoke the implementation of ClosureJobWithConfirmedCancellation::CancelAndWait() because it'll only wait if the job is already in execution.
					cjob->CancelAndWait();
			...
			return maxon::details::ExecuteOnMainThreadResult<typename std::invoke_result<FN>::type>::GetResult(cjob->GetResult(timeout, waitMode));
		}
	}
