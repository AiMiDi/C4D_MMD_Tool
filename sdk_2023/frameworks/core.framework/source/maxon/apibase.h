...
template <typename T> struct StandardType<T, true>
{
	...
	static const Bool KEEP = STD_IS_REPLACEMENT(same, DT, Bool) || STD_IS_REPLACEMENT(same, DT, Utf16Char) || STD_IS_REPLACEMENT(same, DT, Utf32Char);
...
//----------------------------------------------------------------------------------------
static const Int MAXON_FLEXIBLE_ARRAY_LENGTH = 1234; // Don't use 1 as this would allow the compiler to replace the index access array[index] by array[0]
...
template <typename T, typename... REST> struct ParameterPack<T, REST...>
{
	...
	template <typename X> struct IndexOf
	{
		...
		static const Int found = Rest::found + STD_IS_REPLACEMENT(same, T, X);
		static const Int value = !STD_IS_REPLACEMENT(same, T, X) ? Rest::value + 1 : Rest::found ? LIMIT<Int>::MIN : 0;
	};
...
public:
	...
	static const Bool BOTH_DERIVED = STD_IS_REPLACEMENT(base_of, Base, typename std::decay<A>::type) && STD_IS_REPLACEMENT(base_of, Base, typename std::decay<B>::type);
	static const Bool BOTH_LVALUES = STD_IS_REPLACEMENT(lvalue_reference, A) && STD_IS_REPLACEMENT(lvalue_reference, B);
	static const Bool CONST_LVALUE = STD_IS_REPLACEMENT(const, typename std::remove_reference<A>::type) || STD_IS_REPLACEMENT(const, typename std::remove_reference<B>::type);
...
template <typename T, Int COUNTER> class IsComplete : public IsCompleteBase<T, COUNTER, decltype(maxon::details::IsCompleteHelper((T*) nullptr))::value> { };
...
#elif defined(__BASE_FILE__) && !defined(MAXON_DISABLE_SOURCE_LOCATION)
...
#else
TranslationUnit g_translationUnit{&PRIVATE_MAXON_MODULE, "??.cpp", PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS | PRIVATE_MAXON_TRANSLATIONUNIT_FLAGS_2};
#endif
