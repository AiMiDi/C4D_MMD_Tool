...
public:
	...
	/// @note Not available for T = void.
	typename std::conditional<std::is_void<T>::value, DeleteReturnType01, T>::type& GetOrNull() const
	{
	...
	{
		return PrivateNullValue((typename std::integral_constant<Bool, Bool(HANDLER::KIND & VALUEKIND::UNIQUE_REFERENCE)>::type*) nullptr);
	}
	...
	T* Disconnect()
	{
		...
		if (HANDLER::KIND & VALUEKIND::STRONG_MASK && o)
		{
			...
			{
				DebugStop("Reference count must be 1 before call to BaseRef::Disconnect().");
				o = nullptr;
			}
		}
		_object = nullptr;
...
public:
	...
	/// @param[in] o									Pointer to the referenced object, must not be nullptr.
	//----------------------------------------------------------------------------------------
	template <typename T> static MAXON_ATTRIBUTE_NO_INLINE void RemoveReference(const T* o)
	{
...
#define NewObjCopy(T, SRC) (maxon::details::NewDelete<T>::IS_ALLOC_TYPE ? maxon::details::NewDelete<T>::Cast(maxon::details::NewDelete<T>::AllocType::Alloc(maxon::SourceLocation(__FILE__, __LINE__, maxon::PrivateGetClassName<T>()), SRC)) \
														: ResultPtr<T>((T*) PrivateInitCopy(maxon::details::NewDelete<T>::AllocatorType::Alloc(maxon::details::NewDelete<T>::SIZE, maxon::SourceLocation(__FILE__, __LINE__, maxon::PrivateGetClassName<T>())), reinterpret_cast<const typename maxon::details::NewDelete<T>::ConstructorType&>(SRC))))
