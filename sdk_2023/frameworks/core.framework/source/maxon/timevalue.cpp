...
String TimeValue::ToString(const FormatStatement* formatStatement) const
{
	...
	else
	{
		...
		else if (len >= 3 && UnicodeUtils::LowerCase(UChar(format[0])) == UChar('m') && UnicodeUtils::LowerCase(UChar(format[1])) == UChar('m') && UnicodeUtils::LowerCase(UChar(format[2])) == UChar('s'))
		{
			value = GetMinutes();
			value2 = value;
			value = Floor(value);
...
String TimeValue::TimeToString(TIMEFORMAT format, Float64 frameRate) const
{
	...
	switch (format)
	{
		...
		case TIMEFORMAT::FRAMES:
		{
			...
			if (val * 100.0 == Floor(val * 100.0))	// if frame is not a fraction
		...
		smtpMode:
			...
			Int frame = 0;
			if (frameRate >= 1.0)
				frame = Int((val - (Float64)seconds) * frameRate + 0.000001);
