...
#define PRIVATE_MAXON_DATATYPE_REGISTER(T, ret, flags) \
	...
	namespace \
	{ \
		static maxon::DataTypeImpl::Primary g_dataType_##T(DT_##T##_CppName(), &PRIVATE_MAXON_MODULE, DT_##T.GetId(), nullptr, nullptr, (const T*) nullptr, (maxon::DataTypeFunctions<T>*) nullptr, flags); \
		...
		{	\
			...
			if (!(dt->GetValueKind() & maxon::VALUEKIND::ABSTRACT)) { dt->InitializeNullValue() iferr_return; } \
			return ret; \
		} \
...
template <typename T, Bool complete = decltype(maxon::details::IsCompleteHelper((T*) nullptr))::value, typename = void> struct MemberTypeBase
{
	using OrigType = T;
	using BaseType = T;
...
template <typename T> struct MemberTypeBase<T, true, typename std::enable_if<std::is_base_of<InternedId, T>::value, typename SFINAEHelper<void, typename T::ValueType>::type>::type>
{
	using OrigType = T;
