...
template <typename I, typename METHOD, typename T> inline typename MethodDelegateType<METHOD>::type GetMethodDelegate(T&& object, Int offset)
{
	...
	{
		return typename MethodDelegateType<METHOD>::type(true, std::forward<T>(object), *(METHOD*) ((const Char*) MethodDelegateMTable<I>((const ReferencedType*) object) + offset));
	}
	else if (STD_IS_REPLACEMENT(const, typename MethodDelegateType<METHOD>::object))
	{
		...
		if (nullValue)
		{
			...
			return typename MethodDelegateType<METHOD>::type(true, const_cast<ReferencedType*>(nullValue), *(METHOD*) ((const Char*) MethodDelegateMTable<I>(nullValue) + offset));
		}
	}
