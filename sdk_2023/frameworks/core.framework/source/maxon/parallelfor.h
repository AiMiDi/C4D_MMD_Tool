...
#define DISABLE_IF_LOOP_TYPE_IS_INT(TYPE, ...) typename std::enable_if<!STD_IS_REPLACEMENT(convertible, typename std::remove_reference<TYPE>::type, maxon::Int), __VA_ARGS__>::type
...
public:
	...
	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		...
		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, observeForCancellation, threadCnt, granularity, queue);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Dynamic<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), threadCnt, granularity, queue);
	}
	...
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		...
		Dynamic(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT);
	...
	template <typename CONTEXT, PARALLELFORFLAGS FLAGS, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobStatusInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT);

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobStatusInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, observeForCancellation, threadCnt, granularity, queue);
	}

	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP, typename INIT, typename FINALIZE>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Dynamic(FROMTYPE from, INDEXTYPE to, const INIT& init, const LOOP& obj, const FINALIZE& finalize, JobInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int granularity = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		return Dynamic<CONTEXT, PARALLELFORFLAGS::INITTHREADED_FINALIZESYNC, FROMTYPE, INDEXTYPE, LOOP, INIT, FINALIZE>(from, to, init, obj, finalize, reinterpret_cast<JobStatusInterface*>(observeForCancellation), threadCnt, granularity, queue);
	}
	...
	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, observeForCancellation, threadCnt, minChunkSize, queue);
	}

	template <typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static inline Result<void> Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT)
	{
		// Select either NoContextWithResult or NoBreakContext based on return value type.
		using Context = typename NoContextWithResultSelector<decltype(obj(to))>::Context;

		return Static<Context, FROMTYPE, INDEXTYPE, LOOP>(from, to, obj, reinterpret_cast<JobStatusInterface*>(observeForCancellation), threadCnt, minChunkSize, queue);
	}
	...
	template <typename CONTEXT, typename FROMTYPE, typename INDEXTYPE, typename LOOP>
		static MAXON_ATTRIBUTE_NO_INLINE Result<void>
		Static(FROMTYPE from, INDEXTYPE to, const LOOP& obj, JobStatusInterface* observeForCancellation, Int threadCnt = PARALLELFOR_USEMAXIMUMTHREADS, const Int minChunkSize = PARALLELFOR_DEFAULTGRANULARITY, JobQueueInterface* queue = JOBQUEUE_CURRENT);
