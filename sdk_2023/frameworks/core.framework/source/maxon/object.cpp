...
InternedIdInitializer::InternedIdInitializer(InternedId* iid, const Char* str, ModuleInfo* info) : _id(iid), _str(str)
{
	if (_nextOfModule)
	{
	...
	_nextOfModule = info->_internedIds;
...
InterfaceReference::InterfaceReference(const Char* iidWithoutHash, const Char* sourceName, const InterfaceReference* const* baseInterfaces, const Char* methodIds, const METHOD_FLAGS* methodFlags, TranslationUnit* unit, EntityBase::FLAGS refType)
	: _super(EntityBase::TYPE::INTERFACE, iidWithoutHash, unit, refType | EntityBase::FLAGS::EXTENDED), _infoOffset(LIMIT<Int>::MIN), _type(), _basesPtr(baseInterfaces), _methodIds(methodIds), _idWithoutHash(iidWithoutHash), _sourceName(sourceName), _version(nullptr), _depth(-1), _methodFlags(methodFlags)
{
}

InterfaceReference::InterfaceReference(const Char* iidWithoutHash, const Char* sourceName, const InterfaceReference* baseInterface, const Char* methodIds, const METHOD_FLAGS* methodFlags, TranslationUnit* unit, EntityBase::FLAGS refType)
	: InterfaceReference(iidWithoutHash, sourceName, (const InterfaceReference* const*) baseInterface, methodIds, methodFlags, unit, refType | EntityBase::FLAGS::SINGLE_BASE | EntityBase::FLAGS::EXTENDED)
{
}
...
{
	iferr_scope;
	iferr (ObjectInterface* c = GetClass().GetPointer()->CreatePointer(System::GetAllocSize((void*) this)))
		return err;
