...
public:
		...
		for (Entry* e = LoadRelaxed(_table[h].list); e; e = static_cast<Entry*>(e->_next))
		{
			if (e == entry)
			{
				...
				else
				{
					...
					if (MODE == HASHMAP_MODE::DEFAULT && (e->_next == nullptr))
					{
	...
	{
		static_assert(MODE != HASHMAP_MODE::SYNCHRONIZED, "Intersect not supported for a synchronized HashMap.");
		for (Int i = 0; i < _nonemptyBucketCount; ++i)
		{
			Entry* next = nullptr;
			Entry* prev = nullptr;
	...
	public:
		...
		{
			static_assert(MODE == HASHMAP_MODE::DEFAULT, "GetHashCode only supported for a default HashMap.");
			DefaultHasher hash;
		...
		{
			static_assert(MODE == HASHMAP_MODE::DEFAULT, "GetUniqueHashCode only supported for a default HashMap.");
			UniqueHash hash;
			for (Int i = 0; i < m._nonemptyBucketCount; ++i)
			{
				for (const Entry* e = m._nonemptyBuckets[i]->list; e; e = e->GetNextInBucket())
				{
					hash.CombineUnordered(UniqueHash::CombineHashes(HASH::GetUniqueHashCode(e->GetKey()), VALUEHASH::GetUniqueHashCode(e->GetValue())));
				}
			}
			return hash;
		}
...
protected:
	union SimpleBucket
	{
		...
		Int nonemptyBucketsIndex; // not used
	};
	...
	Bool ResizeTableImpl(Int length)
	{
		...
		for (Int j = 0; j <= _tableLengthM1; j++)
		{
			...
			while (entry)
			{
				...
				if (nonEmptyBuckets && (entry->_next == nullptr))
				{
...
template <typename HASH = DefaultCompare, typename ENTRY_HANDLER = HashMapKeyValuePair, typename ALLOCATOR = DefaultAllocator, HASHMAP_MODE MODE = HASHMAP_MODE::DEFAULT, Int INITIAL_CAPACITY = 16, Int LOAD_FACTOR = (MODE == HASHMAP_MODE::SYNCHRONIZED) ? 0 : 10> class HashMapSelector
{
public:
	template <typename K, typename V> using Type = HashMap<K, V, HASH, ENTRY_HANDLER, ALLOCATOR, MODE, INITIAL_CAPACITY, LOAD_FACTOR>;
};
