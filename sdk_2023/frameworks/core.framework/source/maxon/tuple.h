...
public:
	...
	template <typename A1, typename A2> TupleStorage(std::index_sequence<0, 1>* seq, A1&& a1, A2&& a2) : TupleStorage<T1, UnitType>(seq, std::forward<A1>(a1), std::forward<A2>(a2))
	{
	}
};
...
public:
	...
	{
		return GetHashCodeImpl<HASH>((IndexSequence*) nullptr);
	}
	...
	template <typename HASH = DefaultCompare> UniqueHash GetUniqueHashCode() const
	{
		if constexpr (TriviallyHashable())
			...
			return GetUniqueHashCodeImpl<HASH>((IndexSequence*) nullptr);
	}
