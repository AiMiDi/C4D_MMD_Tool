...
Vector64 TransformColor(const Vector64& v, COLORSPACETRANSFORMATION colorSpaceTransformation)
{
	return C4DOS.Sh->TransformColor64(v, colorSpaceTransformation);
}

Vector32 TransformColor(const Vector32& v, COLORSPACETRANSFORMATION colorSpaceTransformation)
{
	return C4DOS.Sh->TransformColor32(v, colorSpaceTransformation);
}

maxon::Color64 TransformColor(const maxon::Color64& v, COLORSPACETRANSFORMATION colorSpaceTransformation)
{
	return C4DOS.Sh->TransformColor64(v.GetVector(), colorSpaceTransformation).GetColor();
}

maxon::Color32 TransformColor(const maxon::Color32& v, COLORSPACETRANSFORMATION colorSpaceTransformation)
{
	return C4DOS.Sh->TransformColor32(v.GetVector(), colorSpaceTransformation).GetColor();
}

void TransformColors(Vector64* v, Int count, COLORSPACETRANSFORMATION colorSpaceTransformation)
{
	return C4DOS.Sh->TransformColors64(v, count, colorSpaceTransformation);
}

void TransformColors(Vector32* v, Int count, COLORSPACETRANSFORMATION colorSpaceTransformation)
{
	return C4DOS.Sh->TransformColors32(v, count, colorSpaceTransformation);
}
...
void InitRenderStruct::Init(BaseDocument* t_doc, Int32 overrideViewTransform)
{
	version |= ((UInt32)GetC4DVersion()) & 0x00ffffff;
	doc = t_doc;
	errorlist = nullptr;
	matname = nullptr;
	vd = nullptr;
	...
	if (t_doc)
	{
		...
		if (bc)
		{
			...
			ifnoerr (OcioConverter* c = OcioConverter::Init(doc, overrideViewTransform))
				ocioConverter = c;
		}
	}
	else
