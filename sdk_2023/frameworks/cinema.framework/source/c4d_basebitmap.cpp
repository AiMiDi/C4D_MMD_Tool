...
{
	C4DOS.Bm->ProfileSetInternalProfile(this, profile);
}
...
{
	return C4DOS.Pa->CLL_GetLockListBitmap(doc, fn, frame, lsl, lockList, initRenderResult);
}

void Unlock(LockList*& lockList)
{
	return C4DOS.Pa->CLL_Unlock(lockList);
}

Bool IsMarkedForDeletion(const LockList* lockList)
{
	return C4DOS.Pa->CLL_IsMarkedForDeletion(lockList);
}
