...
Bool ValidateFieldOutputBlockWithFlags(const FieldOutputBlock &block, FIELDSAMPLE_FLAG flags)
{
	...
	const Bool hasRotation = block._rotation.IsPopulated();
...
FIELDSAMPLE_FLAG ExtractSampleFlags(const FieldOutputBlock& block)
{
	...
	if (block._rotation.IsPopulated())
...
maxon::Result<void> FieldOutputBlock::CopyArrayContentFrom(const FieldOutputBlock& src)
{
	...
	// Validate that destination arrays support the required flags.
	if (MAXON_UNLIKELY(!ValidateFieldOutputBlockWithFlags(*this, flags)))
		return maxon::IllegalArgumentError(MAXON_SOURCE_LOCATION);
	...
	if (copyCount > 0)
	{
		if (MAXON_UNLIKELY(!src._value.IsPopulated() || !src._deactivated.IsPopulated()))
			return maxon::IllegalArgumentError(MAXON_SOURCE_LOCATION);
...
FieldOutput::FieldOutput(FieldOutput&& src)
	: MAXON_MOVE_MEMBERS(_value, _alpha, _color, _direction, _rotation, _deactivated, _count)
{
	src._count = 0;
}
...
//----------------------------------------------------------------------------------------
static inline UInt AddCallerToCallerStackId(UInt currentValue, BaseList2D* caller)
{
	if (caller)
	{
		void* marker = nullptr;
...
void FieldCallerStack::UpdateValue(BaseList2D* caller)
{
	_value = AddCallerToCallerStackId(_value, caller);
}

maxon::Result<void> FieldCallerStack::Add(BaseList2D* caller)
{
	iferr_scope;
...
FieldCallerStack FieldCallerStack::Add(const FieldCallerStack& src, BaseList2D* newCaller)
{
