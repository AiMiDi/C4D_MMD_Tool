...
public:
	...
	// lobals must be a dictionary; locals can be any object that implements the mapping protocol. The parameter start specifies the start token that should be used to parse the source code.If the function fails,
	/// Checks for CPyErr_Occured and clear the exception indicator of Python accordingly. GIL must be held.
	///
	/// @param fp         File pointer where to read the source from.
	/// @param filename   Path of the file.
	...
	MAXON_FUNCTION Result<void> RegisterBuiltinConverter(const DelegatePyObject_AsData& f1, const DelegatePyObject_FromData<T>& f2)
	{
		...
		/// TODO: (Seb) Delegate<Foo<T>> can be casted to another Delegate<Foo<U>> only through a void pointer?
		const void* cast1 = &f1;
	...
	MAXON_METHOD MPyDataRef MPyData_New(const DataType* dt, Generic* data, Bool owner, const DataType* mapDt = nullptr);
	...
	using TYPE_CONVERSION_TUPLE = Tuple<const DataType*, DelegatePyObject_AsData, DelegatePyObject_FromData<>>;
	MAXON_METHOD Result<BaseArray<TYPE_CONVERSION_TUPLE>> GetRegisteredTypesWithConversions();

	template <typename T>
	MAXON_FUNCTION typename std::enable_if<std::is_same<T, DataType*>::value, ResultPtr<const DataType>>::type ParseArgument(const CPyRef& args, Int32 argIndex, Bool isOptional = false);
	...
	template <typename T>
	MAXON_FUNCTION Result<T> ParseGeneric(const CPythonLibraryRef& pylib, const CPyRef& args, Int32 argIndex, const DataType*& dt, Bool unpackData);
	...
	//-------------------------------------------------
	// Creates a scope dictionary which has all builtin modules preset.
	// 
...
template <typename T>
MAXON_FUNCTION Result<T> CPythonLibraryInterface::ParseGeneric(const CPythonLibraryRef& pylib, const CPyRef& args, Int32 argIndex, const DataType*& dt, Bool unpackData)
{
	iferr_scope;
...
template <typename T>
MAXON_FUNCTION typename std::enable_if<std::is_same<T, DataType*>::value, ResultPtr<const DataType>>::type CPythonLibraryInterface::ParseArgument(const CPyRef& args, Int32 argIndex, Bool isOptional)
{
  iferr_scope;
...
//-------------------------------------------------
// Casts an object to a CPyTypeRef. Reference must be of this type, or the behaviour is undefined.
// 
// @param ref         Object to cast.
// @return            Returns the object casted to CPyTypeRef
//-------------------------------------------------
template <typename T>
...
#define PRIVATE_MAXON_CPYTHON_FUNCTION_REGISTER(funcname, SUPERTYPE) static maxon::py::NativePyObject* MAXON_CONCAT(CPythonFunction27_, funcname) (maxon::py::NativePyObject* selfLib, maxon::py::NativePyObject* args, maxon::py::NativePyObject*) \
	{ \
		...
		return MAXON_CONCAT(CPythonFunction_, funcname)(lib, maxon::py::CPyInterface::CreateTempRef(lib, args)).GetAndIncref<>(); \
	} \
