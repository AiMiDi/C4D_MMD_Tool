...
template <typename T> inline typename std::enable_if<GetCollectionKind<T>::value == COLLECTION_KIND::ARRAY, Result<void>>::type ReadDocument(const InputStreamRef& stream, const Id& id, T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
	auto arr = object.ToArray();
	return IoDocument::ReadDocument(stream, id, DataPtr(maxon::GetDataType<ArrayInterface<typename T::ValueType>>(), reinterpret_cast<Generic*>(&arr)), format, dict);
}

template <typename T> inline typename std::enable_if<GetCollectionKind<T>::value != COLLECTION_KIND::ARRAY && !std::is_same<T, Data>::value, Result<void>>::type ReadDocument(const InputStreamRef& stream, const Id& id, T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
	return IoDocument::ReadDocument(stream, id, DataPtr(object), format, dict);
}

template <typename T> inline typename std::enable_if<std::is_same<T, Data>::value, Result<void>>::type ReadDocument(const InputStreamRef& stream, const Id& id, T& object, IOFORMAT format = IOFORMAT::DEFAULT, const DataDictionary& dict = DataDictionary())
{
	return IoDocument::ReadDocument(stream, id, DataPtr(GetDataType<Data>(), reinterpret_cast<Generic*>(&object)), format, dict);
}
