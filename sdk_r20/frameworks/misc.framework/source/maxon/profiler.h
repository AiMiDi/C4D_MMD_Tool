...
public:
	...
	///
	/// 	profiler.OutputStringReport("MyMethod - Profile trace dump"_s, true);
	/// }
	/// @endcode
	///
	...
	//----------------------------------------------------------------------------------------
	/// Outputs the collected profiling information to the debugger output or C4D console.
	/// @param[in] headerString				Header string to output first.
	/// @param[in] outputToConsole		Output report to the C4D console if true, or the debugger otherwise.
	/// @param[in] outputDebugDump		Output debug dump if enabled (OFF by default).
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Gets the time filter threshold value in microseconds.
	/// @return												Time filter threshold value.
	//----------------------------------------------------------------------------------------
	MAXON_METHOD UInt32 GetTimeFilter();

	//----------------------------------------------------------------------------------------
	/// Sets the time filter threshold value in microseconds.
