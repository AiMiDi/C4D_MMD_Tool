...
def Matrix(valueObject, dictionary):
    ...
    if sbtype.IsPointerType():
        addr = valueObject.GetValueAsUnsigned()
    else:
        addr = valueObject.GetLoadAddress()

    expr = "((maxon::Matrix*)0x%x)->ToString(nullptr)" % (addr)

    frame = valueObject.frame

    description = frame.EvaluateExpression(expr)
    ret = String(description, dictionary)
...
def NodePathImpl(valueObject, dictionary, pathOffset):
    ...
    if ptr != 0:
        pathPtr = valueObject.CreateValueFromAddress("_pathptr", ptr + 0, charPtrType.GetPointerType()) # pointer to Id-array, treated as Char**
        count = valueObject.CreateValueFromAddress("_count", ptr + 8, intType).GetValueAsSigned()
        if count > 0:
            result = '{'
            for i in xrange(0, min(count, 20)):
                if i:
                    result += ','
                pathId = valueObject.CreateValueFromAddress("_id" + str(i), pathPtr.GetValueAsUnsigned(0) + 16*i, charPtrType)
