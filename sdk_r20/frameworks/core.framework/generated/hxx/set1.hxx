...
template <typename TYPE> struct SetInterface<TYPE>::ReferenceClassHelper { using type = Set<TYPE>; };

/// Intermediate helper class for SetInterface.
template <typename TYPE> template <typename S> class SetInterface<TYPE>::ConstReferenceFunctionsImpl DOXYGEN_SWITCH(: public S,)
{
public:
...
#ifdef DOXYGEN
template <typename TYPE> template <typename REF> class SetInterface<TYPE>::ConstReferenceFunctionsImplDoxy : public SetInterface::ConstReferenceFunctionsImpl<REF> { };
#endif

/// Intermediate helper class for SetInterface.
template <typename TYPE> template <typename S> class SetInterface<TYPE>::ReferenceFunctionsImpl : public ConstReferenceFunctionsImpl<S>
{
public:
...
#ifdef DOXYGEN
template <typename TYPE> template <typename REF> class SetInterface<TYPE>::ReferenceFunctionsImplDoxy : public SetInterface::ReferenceFunctionsImpl<REF> { };
#endif

/// Intermediate helper class for SetInterface.
template <typename TYPE> template <typename S> class SetInterface<TYPE>::COWReferenceFunctionsImpl : public ConstReferenceFunctionsImpl<S>
{
public:
...
#ifdef DOXYGEN
template <typename TYPE> template <typename REF> class SetInterface<TYPE>::COWReferenceFunctionsImplDoxy : public SetInterface::COWReferenceFunctionsImpl<REF> { };
#endif

template <typename TYPE> template <typename ST> struct SetInterface<TYPE>::ReferenceFunctions { using type = typename std::conditional<maxon::Bool(ST::type::Handler::KIND & (maxon::VALUEKIND::COW_MASK | maxon::VALUEKIND::DEEP_CONSTNESS)), COWReferenceFunctionsImpl<typename ST::type>, typename std::conditional<std::is_const<typename ST::type::PrivateReferencedType>::value, ConstReferenceFunctionsImpl<typename ST::type>, ReferenceFunctionsImpl<typename ST::type>>::type>::type; };
template <typename TYPE> class SetInterface<TYPE>::Reference :
#ifdef DOXYGEN
public COWReferenceFunctionsImplDoxy<Set<TYPE>>
#else
public maxon::ReferenceBase<ReferenceFunctions<maxon::RefBaseFunctions<maxon::DirectRef<const SetInterface<TYPE>, maxon::StrongCOWRefHandler, Set<TYPE>>>>>
#endif
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Reference, maxon::ReferenceBase<ReferenceFunctions<maxon::RefBaseFunctions<maxon::DirectRef<const SetInterface<TYPE>, maxon::StrongCOWRefHandler, Set<TYPE>>>>>);
...
template <typename TYPE> class Set : public SetInterface<TYPE>::Reference
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Set, typename SetInterface<TYPE>::Reference);
};
