...
struct IoErrorInterface::MTable
{
	...
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoErrorInterface, SetUrl))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoErrorInterface, SetUrl), W, MTable>::type::_IoErrorInterface_SetUrl;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoErrorInterface, GetUrl))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoErrorInterface, GetUrl), W, MTable>::type::_IoErrorInterface_GetUrl;
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<IoErrorInterface, ErrorInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(IoErrorInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class IoErrorInterface::Wrapper : public ErrorInterface::Wrapper<S>
{
public:
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoErrorInterface::ReferenceFunctionsImpl<S>::SetUrl(const Url& url) const -> typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); IoErrorInterface* o_ = (IoErrorInterface*) this->GetPointer(); if (MAXON_UNLIKELY(!o_)) return maxon::PrivateLogNullptrError(); const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(IoErrorInterface, o_, IoErrorInterface_SetUrl) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<IoErrorInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<IoErrorInterface>() : PRIVATE_MAXON_VTABLE(IoErrorInterface, o_); 
	mt_._IoErrorInterface_SetUrl(reinterpret_cast<maxon::GenericComponent*>(o_) + mt_._IoErrorInterface_SetUrl_Offset, url);
	return (typename ReturnHelper::ReturnType) maxon::OK;
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoErrorInterface::COWReferenceFunctionsImpl<S>::SetUrl(const Url& url) -> typename std::conditional<maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS), void, maxon::Result<void>>::type
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoErrorInterface::ConstReferenceFunctionsImpl<S>::GetUrl() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<const Url&>, const Url&>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<const Url&>, const Url&>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const IoErrorInterface* o_ = (const IoErrorInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = IoErrorInterface::NullValuePtr(); if (!o_) return maxon::PrivateIncompleteNullReturnValue<const Url&>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(IoErrorInterface, o_, IoErrorInterface_GetUrl) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<IoErrorInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<IoErrorInterface>() : PRIVATE_MAXON_VTABLE(IoErrorInterface, o_); 
...
struct FileFormatErrorInterface::MTable
{
	...
	template <typename W> static maxon::Bool InitWithBases(maxon::ClassInfo* info, maxon::Int offset)
	{
		if (!W::Implementation::PROXY && (!IoErrorInterface::MTable::InitWithBases<W>(info, offset))) return false;
		MTable* vt = (MTable*) maxon::Object::PrivateGetOrCreateMTable(info, _interface); if (!vt) return false;
		vt->Init<W>(offset); return true;
	}
	template <typename S> using CombinedMTable = maxon::CombinedMTable<FileFormatErrorInterface, IoErrorInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(FileFormatErrorInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class FileFormatErrorInterface::Wrapper : public IoErrorInterface::Wrapper<S>
{
public:
};
...
struct UnknownFileSizeErrorInterface::MTable
{
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<UnknownFileSizeErrorInterface, ErrorInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(UnknownFileSizeErrorInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class UnknownFileSizeErrorInterface::Wrapper : public ErrorInterface::Wrapper<S>
{
public:
};
...
struct BaseStreamInterface::MTable
{
	...
	template <typename W> void Init(maxon::Int offset)
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, GetStreamLength))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, GetStreamLength), W, MTable>::type::_BaseStreamInterface_GetStreamLength;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, GetPosition))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, GetPosition), W, MTable>::type::_BaseStreamInterface_GetPosition;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, Close))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, Close), W, MTable>::type::_BaseStreamInterface_Close;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, SeekSupported))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, SeekSupported), W, MTable>::type::_BaseStreamInterface_SeekSupported;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, Seek))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, Seek), W, MTable>::type::_BaseStreamInterface_Seek;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, DisableBuffering))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, BaseStreamInterface, DisableBuffering), W, MTable>::type::_BaseStreamInterface_DisableBuffering;
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<BaseStreamInterface, DataDictionaryObjectInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(BaseStreamInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class BaseStreamInterface::Wrapper : public DataDictionaryObjectInterface::Wrapper<S>
{
public:
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::ConstReferenceFunctionsImpl<S>::GetStreamLength() const -> Result<Int64>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<Int64>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const BaseStreamInterface* o_ = (const BaseStreamInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = BaseStreamInterface::NullValuePtr(); if (!o_) return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(BaseStreamInterface, o_, BaseStreamInterface_GetStreamLength) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<BaseStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<BaseStreamInterface>() : PRIVATE_MAXON_VTABLE(BaseStreamInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::ConstReferenceFunctionsImpl<S>::GetPosition() const -> Result<Int64>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::ConstReferenceFunctionsImpl<S>::SeekSupported() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<Bool>, Bool>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<Bool>, Bool>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const BaseStreamInterface* o_ = (const BaseStreamInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = BaseStreamInterface::NullValuePtr(); if (!o_) return maxon::PrivateLogNullptrError(false); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(BaseStreamInterface, o_, BaseStreamInterface_SeekSupported) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<BaseStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<BaseStreamInterface>() : PRIVATE_MAXON_VTABLE(BaseStreamInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::ReferenceFunctionsImpl<S>::Seek(Int64 position) const -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::COWReferenceFunctionsImpl<S>::Seek(Int64 position) -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::ReferenceFunctionsImpl<S>::DisableBuffering() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); BaseStreamInterface* o_ = (BaseStreamInterface*) this->GetPointer(); if (MAXON_UNLIKELY(!o_)) return maxon::PrivateLogNullptrError(); const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(BaseStreamInterface, o_, BaseStreamInterface_DisableBuffering) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<BaseStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<BaseStreamInterface>() : PRIVATE_MAXON_VTABLE(BaseStreamInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto BaseStreamInterface::COWReferenceFunctionsImpl<S>::DisableBuffering() -> typename std::conditional<maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS), void, maxon::Result<void>>::type
{
...
struct InputStreamInterface::MTable
{
	...
	template <typename W> void Init(maxon::Int offset)
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, InputStreamInterface, BytesAvailable))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, InputStreamInterface, BytesAvailable), W, MTable>::type::_InputStreamInterface_BytesAvailable;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, InputStreamInterface, ReadEOS))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, InputStreamInterface, ReadEOS), W, MTable>::type::_InputStreamInterface_ReadEOS;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, InputStreamInterface, Skip))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, InputStreamInterface, Skip), W, MTable>::type::_InputStreamInterface_Skip;
	...
	template <typename W> static maxon::Bool InitWithBases(maxon::ClassInfo* info, maxon::Int offset)
	{
		if (!W::Implementation::PROXY && (!BaseStreamInterface::MTable::InitWithBases<W>(info, offset))) return false;
		MTable* vt = (MTable*) maxon::Object::PrivateGetOrCreateMTable(info, _interface); if (!vt) return false;
		vt->Init<W>(offset); return true;
	}
	template <typename S> using CombinedMTable = maxon::CombinedMTable<InputStreamInterface, BaseStreamInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(InputStreamInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class InputStreamInterface::Wrapper : public BaseStreamInterface::Wrapper<S>
{
public:
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto InputStreamInterface::ConstReferenceFunctionsImpl<S>::BytesAvailable() const -> Result<Int64>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<Int64>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const InputStreamInterface* o_ = (const InputStreamInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = InputStreamInterface::NullValuePtr(); if (!o_) return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(InputStreamInterface, o_, InputStreamInterface_BytesAvailable) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<InputStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<InputStreamInterface>() : PRIVATE_MAXON_VTABLE(InputStreamInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto InputStreamInterface::ReferenceFunctionsImpl<S>::Read(const Block<Byte>& data) const -> Result<void>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<void>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); InputStreamInterface* o_ = (InputStreamInterface*) this->GetPointer(); if (MAXON_UNLIKELY(!o_)) return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); 
	return (o_->Read(data));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto InputStreamInterface::COWReferenceFunctionsImpl<S>::Read(const Block<Byte>& data) -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto InputStreamInterface::ReferenceFunctionsImpl<S>::ReadEOS(const Block<Byte>& data) const -> Result<Int>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<Int>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); InputStreamInterface* o_ = (InputStreamInterface*) this->GetPointer(); if (MAXON_UNLIKELY(!o_)) return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(InputStreamInterface, o_, InputStreamInterface_ReadEOS) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<InputStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<InputStreamInterface>() : PRIVATE_MAXON_VTABLE(InputStreamInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto InputStreamInterface::COWReferenceFunctionsImpl<S>::ReadEOS(const Block<Byte>& data) -> Result<Int>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<Int>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); InputStreamInterface* o_ = (InputStreamInterface*) this->MakeWritable(true).GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) return ReturnHelper::Return(this, maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::MAKE_WRITABLE_FAILED)); const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(InputStreamInterface, o_, InputStreamInterface_ReadEOS) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<InputStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<InputStreamInterface>() : PRIVATE_MAXON_VTABLE(InputStreamInterface, o_); 
	return (mt_._InputStreamInterface_ReadEOS(reinterpret_cast<maxon::GenericComponent*>(o_) + mt_._InputStreamInterface_ReadEOS_Offset, data));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto InputStreamInterface::ReferenceFunctionsImpl<S>::Skip(Int64 bytes) const -> Result<void>
{
...
struct OutputStreamInterface::MTable
{
	...
	template <typename W> void Init(maxon::Int offset)
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, OutputStreamInterface, Write))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, OutputStreamInterface, Write), W, MTable>::type::_OutputStreamInterface_Write;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, OutputStreamInterface, Flush))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, OutputStreamInterface, Flush), W, MTable>::type::_OutputStreamInterface_Flush;
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<OutputStreamInterface, BaseStreamInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(OutputStreamInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class OutputStreamInterface::Wrapper : public BaseStreamInterface::Wrapper<S>
{
public:
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto OutputStreamInterface::COWReferenceFunctionsImpl<S>::Write(const Block<const Byte>& data) -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto OutputStreamInterface::ReferenceFunctionsImpl<S>::Flush() const -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto OutputStreamInterface::COWReferenceFunctionsImpl<S>::Flush() -> Result<void>
{
...
struct InOutputStreamInterface::MTable
{
	...
	template <typename W> static maxon::Bool InitWithBases(maxon::ClassInfo* info, maxon::Int offset)
	{
		if (!W::Implementation::PROXY && (!InputStreamInterface::MTable::InitWithBases<W>(info, offset) || !OutputStreamInterface::MTable::InitWithBases<W>(info, offset))) return false;
		MTable* vt = (MTable*) maxon::Object::PrivateGetOrCreateMTable(info, _interface); if (!vt) return false;
		vt->Init<W>(offset); return true;
	}
	template <typename S> using CombinedMTable = maxon::CombinedMTable<InOutputStreamInterface, OutputStreamInterface::MTable::CombinedMTable<InputStreamInterface::MTable::CombinedMTable<S>>>;
	PRIVATE_MAXON_COMPBASE(InOutputStreamInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class InOutputStreamInterface::Wrapper : public OutputStreamInterface::Wrapper<InputStreamInterface::Wrapper<S>>
{
public:
};
