...
struct FileFormatHandlerInterface::MTable
{
	...
	Result<const DataType*> (*_FileFormatHandlerInterface_GetHandlerType) (const maxon::GenericComponent* this_);
	...
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, GetFileFormat))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, GetFileFormat), W, MTable>::type::_FileFormatHandlerInterface_GetFileFormat;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, GetHandlerType))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, GetHandlerType), W, MTable>::type::_FileFormatHandlerInterface_GetHandlerType;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, CreateHandler))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, CreateHandler), W, MTable>::type::_FileFormatHandlerInterface_CreateHandler;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, GetDependends))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, FileFormatHandlerInterface, GetDependends), W, MTable>::type::_FileFormatHandlerInterface_GetDependends;
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<FileFormatHandlerInterface, Object::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(FileFormatHandlerInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class FileFormatHandlerInterface::Wrapper : public Object::Wrapper<S>
{
public:
	...
	static Result<const DataType*> _FileFormatHandlerInterface_GetHandlerType(const maxon::GenericComponent* this_) { return ((const typename S::Implementation*) this_)->GetHandlerType(); }
...
MAXON_ATTRIBUTE_FORCE_INLINE auto FileFormatHandlerInterface::GetHandlerType() const -> Result<const DataType*>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto FileFormatHandlerInterface::ConstReferenceFunctionsImpl<S>::GetFileFormat() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<const FileFormat&>, const FileFormat&>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<const FileFormat&>, const FileFormat&>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const FileFormatHandlerInterface* o_ = (const FileFormatHandlerInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = FileFormatHandlerInterface::NullValuePtr(); if (!o_) return maxon::PrivateIncompleteNullReturnValue<const FileFormat&>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(FileFormatHandlerInterface, o_, FileFormatHandlerInterface_GetFileFormat) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<FileFormatHandlerInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<FileFormatHandlerInterface>() : PRIVATE_MAXON_VTABLE(FileFormatHandlerInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto FileFormatHandlerInterface::ConstReferenceFunctionsImpl<S>::GetHandlerType() const -> Result<const DataType*>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<const DataType*>>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const FileFormatHandlerInterface* o_ = (const FileFormatHandlerInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = FileFormatHandlerInterface::NullValuePtr(); if (!o_) return maxon::PrivateGetNullReturnError(maxon::NULL_RETURN_REASON::NULLPTR); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(FileFormatHandlerInterface, o_, FileFormatHandlerInterface_GetHandlerType) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<FileFormatHandlerInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<FileFormatHandlerInterface>() : PRIVATE_MAXON_VTABLE(FileFormatHandlerInterface, o_); 
...
template <typename S> template <typename HANDLER> MAXON_ATTRIBUTE_FORCE_INLINE auto FileFormatHandlerInterface::ConstReferenceFunctionsImpl<S>::CreateHandler(const Url& url) const -> Result<HANDLER>
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, Result<HANDLER>>; static_assert(!std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value, "Can't invoke MAXON_FUNCTION FileFormatHandlerInterface::CreateHandler on super."); if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const FileFormatHandlerInterface* o_ = (const FileFormatHandlerInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = FileFormatHandlerInterface::NullValuePtr(); if (!o_) return maxon::PrivateNullReturnValue<Result<HANDLER>>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } 
	return (o_->CreateHandler<HANDLER>(url));
}
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto FileFormatHandlerInterface::ConstReferenceFunctionsImpl<S>::GetDependends() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<FILEFORMAT_PRIORITY>, FILEFORMAT_PRIORITY>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<FILEFORMAT_PRIORITY>, FILEFORMAT_PRIORITY>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const FileFormatHandlerInterface* o_ = (const FileFormatHandlerInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = FileFormatHandlerInterface::NullValuePtr(); if (!o_) return maxon::PrivateIncompleteNullReturnValue<FILEFORMAT_PRIORITY>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(FileFormatHandlerInterface, o_, FileFormatHandlerInterface_GetDependends) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<FileFormatHandlerInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<FileFormatHandlerInterface>() : PRIVATE_MAXON_VTABLE(FileFormatHandlerInterface, o_); 
...
#ifndef MAXON_COMPILER_CLANG
#endif
		tbl->_FileFormatDetectionInterface_DetectAll = &IMPL::_FileFormatDetectionInterface_DetectAll;
	}
};

template <typename C> class FileFormatDetectionInterface::Wrapper
{
public:
#ifndef MAXON_COMPILER_CLANG
#endif
