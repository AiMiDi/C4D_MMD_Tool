...
struct IoDualInOutputStreamInterface::MTable
{
	...
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, SetInputStream))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, SetInputStream), W, MTable>::type::_IoDualInOutputStreamInterface_SetInputStream;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, SetOutputStream))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, SetOutputStream), W, MTable>::type::_IoDualInOutputStreamInterface_SetOutputStream;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, GetInputStream))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, GetInputStream), W, MTable>::type::_IoDualInOutputStreamInterface_GetInputStream;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, GetOutputStream))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, IoDualInOutputStreamInterface, GetOutputStream), W, MTable>::type::_IoDualInOutputStreamInterface_GetOutputStream;
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<IoDualInOutputStreamInterface, InOutputStreamInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(IoDualInOutputStreamInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class IoDualInOutputStreamInterface::Wrapper : public InOutputStreamInterface::Wrapper<S>
{
public:
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoDualInOutputStreamInterface::COWReferenceFunctionsImpl<S>::SetInputStream(const InputStreamRef& inputStream) -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoDualInOutputStreamInterface::ReferenceFunctionsImpl<S>::SetOutputStream(const OutputStreamRef& outputStream) const -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoDualInOutputStreamInterface::COWReferenceFunctionsImpl<S>::SetOutputStream(const OutputStreamRef& outputStream) -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto IoDualInOutputStreamInterface::ConstReferenceFunctionsImpl<S>::GetOutputStream() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<OutputStreamRef>, OutputStreamRef>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<OutputStreamRef>, OutputStreamRef>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const IoDualInOutputStreamInterface* o_ = (const IoDualInOutputStreamInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = IoDualInOutputStreamInterface::NullValuePtr(); if (!o_) return maxon::PrivateIncompleteNullReturnValue<OutputStreamRef>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(IoDualInOutputStreamInterface, o_, IoDualInOutputStreamInterface_GetOutputStream) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<IoDualInOutputStreamInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<IoDualInOutputStreamInterface>() : PRIVATE_MAXON_VTABLE(IoDualInOutputStreamInterface, o_); 
