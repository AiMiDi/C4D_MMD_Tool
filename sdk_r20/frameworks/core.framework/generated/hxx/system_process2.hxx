...
struct SystemProcessKilledErrorInterface::MTable
{
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<SystemProcessKilledErrorInterface, ErrorInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(SystemProcessKilledErrorInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class SystemProcessKilledErrorInterface::Wrapper : public ErrorInterface::Wrapper<S>
{
public:
};
...
struct SystemProcessOperationErrorInterface::MTable
{
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<SystemProcessOperationErrorInterface, ErrorInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(SystemProcessOperationErrorInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class SystemProcessOperationErrorInterface::Wrapper : public ErrorInterface::Wrapper<S>
{
public:
};
...
struct SystemProcessObjectInterface::MTable
{
	...
	template <typename W> void Init(maxon::Int offset)
	{
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, StartProcess))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, StartProcess), W, MTable>::type::_SystemProcessObjectInterface_StartProcess;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, KillProcess))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, KillProcess), W, MTable>::type::_SystemProcessObjectInterface_KillProcess;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetExitCode))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetExitCode), W, MTable>::type::_SystemProcessObjectInterface_GetExitCode;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, IsRunning))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, IsRunning), W, MTable>::type::_SystemProcessObjectInterface_IsRunning;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetProcessId))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetProcessId), W, MTable>::type::_SystemProcessObjectInterface_GetProcessId;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetProcessHandle))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetProcessHandle), W, MTable>::type::_SystemProcessObjectInterface_GetProcessHandle;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetProcessUrl))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetProcessUrl), W, MTable>::type::_SystemProcessObjectInterface_GetProcessUrl;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetLogFile))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetLogFile), W, MTable>::type::_SystemProcessObjectInterface_GetLogFile;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetUserData))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetUserData), W, MTable>::type::_SystemProcessObjectInterface_GetUserData;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetLoadedDlls))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, GetLoadedDlls), W, MTable>::type::_SystemProcessObjectInterface_GetLoadedDlls;
		...
		if (PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, ObservableProcessFinished))
		{
			auto ptr = &std::conditional<PRIVATE_MAXON_IMPLEMENTS(W::Implementation, SystemProcessObjectInterface, ObservableProcessFinished), W, MTable>::type::_SystemProcessObjectInterface_ObservableProcessFinished;
	...
	template <typename S> using CombinedMTable = maxon::CombinedMTable<SystemProcessObjectInterface, ObserverObjectInterface::MTable::CombinedMTable<S>>;
	PRIVATE_MAXON_COMPBASE(SystemProcessObjectInterface);
	template <typename I> static const typename I::MTable& GetMTable() { return maxon::GetZeroRef<typename I::MTable>(); }
};

template <typename S> class SystemProcessObjectInterface::Wrapper : public ObserverObjectInterface::Wrapper<S>
{
public:
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::COWReferenceFunctionsImpl<S>::StartProcess() -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ReferenceFunctionsImpl<S>::KillProcess() const -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::COWReferenceFunctionsImpl<S>::KillProcess() -> Result<void>
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::GetExitCode() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<Int>, Int>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<Int>, Int>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const SystemProcessObjectInterface* o_ = (const SystemProcessObjectInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = SystemProcessObjectInterface::NullValuePtr(); if (!o_) return 0; } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(SystemProcessObjectInterface, o_, SystemProcessObjectInterface_GetExitCode) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<SystemProcessObjectInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<SystemProcessObjectInterface>() : PRIVATE_MAXON_VTABLE(SystemProcessObjectInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::IsRunning() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<Bool>, Bool>::type
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::GetProcessId() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<UInt>, UInt>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<UInt>, UInt>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const SystemProcessObjectInterface* o_ = (const SystemProcessObjectInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = SystemProcessObjectInterface::NullValuePtr(); if (!o_) return 0; } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(SystemProcessObjectInterface, o_, SystemProcessObjectInterface_GetProcessId) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<SystemProcessObjectInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<SystemProcessObjectInterface>() : PRIVATE_MAXON_VTABLE(SystemProcessObjectInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::GetProcessHandle() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<void*>, void*>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<void*>, void*>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const SystemProcessObjectInterface* o_ = (const SystemProcessObjectInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = SystemProcessObjectInterface::NullValuePtr(); if (!o_) return nullptr; } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(SystemProcessObjectInterface, o_, SystemProcessObjectInterface_GetProcessHandle) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<SystemProcessObjectInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<SystemProcessObjectInterface>() : PRIVATE_MAXON_VTABLE(SystemProcessObjectInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::GetProcessUrl() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<Url>, Url>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<Url>, Url>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const SystemProcessObjectInterface* o_ = (const SystemProcessObjectInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = SystemProcessObjectInterface::NullValuePtr(); if (!o_) return maxon::PrivateIncompleteNullReturnValue<Url>(maxon::NULL_RETURN_REASON::NULLPTR, OVERLOAD_MAX_RANK); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(SystemProcessObjectInterface, o_, SystemProcessObjectInterface_GetProcessUrl) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<SystemProcessObjectInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<SystemProcessObjectInterface>() : PRIVATE_MAXON_VTABLE(SystemProcessObjectInterface, o_); 
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::GetLogFile() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<Url>, Url>::type
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::GetUserData() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<void*>, void*>::type
{
...
template <typename S> MAXON_ATTRIBUTE_FORCE_INLINE auto SystemProcessObjectInterface::ConstReferenceFunctionsImpl<S>::ObservableProcessFinished() const -> typename std::conditional<S::HAS_ERROR, maxon::Result<maxon::ObservableRef<ObservableProcessFinishedDelegate>>, maxon::ObservableRef<ObservableProcessFinishedDelegate>>::type
{
	using ReturnHelper = maxon::ReferenceFunctionErrorReturnHelper<S::HAS_ERROR, false, typename std::conditional<S::HAS_ERROR, maxon::Result<maxon::ObservableRef<ObservableProcessFinishedDelegate>>, maxon::ObservableRef<ObservableProcessFinishedDelegate>>::type>; if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError()) return ReturnHelper::ReturnErrorOf(this); const SystemProcessObjectInterface* o_ = (const SystemProcessObjectInterface*) this->GetPointer(); if (!(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS) && MAXON_UNLIKELY(!o_)) { o_ = SystemProcessObjectInterface::NullValuePtr(); if (!o_) return maxon::ObservableRef<ObservableProcessFinishedDelegate>::NullValue(); } const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(SystemProcessObjectInterface, o_, SystemProcessObjectInterface_ObservableProcessFinished) : ((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBase::template Check<SystemProcessObjectInterface>::value) ? S::PrivateReferencedType::MTable::template GetMTable<SystemProcessObjectInterface>() : PRIVATE_MAXON_VTABLE(SystemProcessObjectInterface, o_); 
...
struct SystemProcessInterface::MTable
{
	...
	{
		tbl->_SystemProcessInterface_CreateProcess = &IMPL::_SystemProcessInterface_CreateProcess;
		tbl->_SystemProcessInterface_StartProcessAndWait = &IMPL::_SystemProcessInterface_StartProcessAndWait;
		tbl->_SystemProcessInterface_GetSystemProcesses = &IMPL::_SystemProcessInterface_GetSystemProcesses;
		tbl->_SystemProcessInterface_GetCurrentProcessId = &IMPL::_SystemProcessInterface_GetCurrentProcessId;
		tbl->_SystemProcessInterface_GetProcess = &IMPL::_SystemProcessInterface_GetProcess;
		tbl->_SystemProcessInterface_WaitForProcess = &IMPL::_SystemProcessInterface_WaitForProcess;
	}
};

template <typename C> class SystemProcessInterface::Wrapper
{
public:
