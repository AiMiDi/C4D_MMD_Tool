...
public:
	...
	/// @copydoc GetValue
	T& UncheckedGetValue() { return _value; }
...
public:
	...
	ResultMemT(ResultMemT&& src) : _value(std::move(src._value))
	{
	}

	MAXON_OPERATOR_MOVE_ASSIGNMENT(ResultMemT);
	...
	/// @copydoc GetValue
	T& GetValue() { DebugAssert(_value); return _value; }
...
public:
	...
	/// pointers at all.
	/// @return												This ResultRef, casted to a ResultPtr.
	//----------------------------------------------------------------------------------------
...
public:
	...
	/// @copydoc GetValue
	MAXON_ATTRIBUTE_FORCE_INLINE T& GetValue() { DebugAssert(!_error._error); return _value; }
	...
	/// @copydoc UncheckedGetValue
	MAXON_ATTRIBUTE_FORCE_INLINE T& UncheckedGetValue() { return _value; }
	...
	MAXON_ATTRIBUTE_FORCE_INLINE T&& MoveValueGetError(ThreadReferencedError& error) { error = _error; return std::move(_value); }
	...
	/// @copydoc GetValueAndError
	MAXON_ATTRIBUTE_FORCE_INLINE T&& MoveValueGetError(Error& error) { PrivateSetError(error, _error._error); return std::move(_value); }
...
public:
	...
	T& MoveValueGetError(ThreadReferencedError& error) { error = _error; return *_value; }

	T& GetValueAndError(Error& error) const { PrivateSetError(error, _error._error); return *_value; }

	T& MoveValueGetError(Error& error) { PrivateSetError(error, _error._error); return *_value; }
	...
	/// pointers at all.
	/// @return												This Result, casted to a Result<T*>.
	//----------------------------------------------------------------------------------------
...
public:
	...
	void MoveValueGetError(ThreadReferencedError& error) { error = _error; }
...
{
public:
	using type = ResultBase<T>;
};

template <typename T> class ResultSuper<T, typename SFINAEHelper<void, typename T::template ResultFunctions<ResultBase<T>>>::type>
	: public T::template ResultFunctions<ResultBase<T>>
{
public:
	using type = ResultSuper;
	using Base = typename T::template ResultFunctions<ResultBase<T>>;

	ResultSuper() = default;
	explicit ResultSuper(const T& value) : Base(value) { }
	explicit ResultSuper(T&& value) : Base(std::move(value)) { }

	// redeclare GetValue in case we inherit a function with that name from T::ResultSuper
	const T& GetValue() const { DebugAssert(!this->_error.GetReferencedError()); return this->_value; }
...
public:
	...
	explicit ResultSuper(T& value) : Base(value) { }

	T& GetValue() const { DebugAssert(!this->_error.GetReferencedError()); return *this->_value; }
...
{
public:
	static_assert(!std::is_rvalue_reference<T>::value, "Can't instantiate Result template with an r-value reference type.");
	using Super = typename ResultSuper<T, void>::type;
	static const Bool T_IS_ERROR = IsErrorClass<T>::value; // if T itself is an error class, we have to disable some constructors to avoid the ambiguity between an error as a valid result or as an error indication

	//----------------------------------------------------------------------------------------
	/// The type to use for copy construction from a T.
	//----------------------------------------------------------------------------------------
	using typename Super::CopyType;

	//----------------------------------------------------------------------------------------
	/// The type to use for move construction from a T.
	//----------------------------------------------------------------------------------------
	...
	// Intel seems to always generate implicit copy constructor (also if this is not explicitly needed, this causes an error if T has no copy constructor)
...
#define PRIVATE_MAXON_ENABLE_IF_ERROR_DUMMY PRIVATE_MAXON_ENABLE_IF_ERROR(DummyParamType*) = (DummyParamType*) nullptr
	...
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<T>::value, ResultMem, const volatile DummyParamType&>::type value) : Super()
	{
		if (value == FAILED)
	...
	/// @param[in] value							The ResultOk from which the value is taken.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<T>::value, ResultOk<T>, DummyParamType&&>::type value) : Super()
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename T2> MAXON_IMPLICIT Result(const ResultMemT<T2>& value, typename std::enable_if<std::is_void<typename SFINAEHelper<T, T2>::type>::value, DummyParamType*>::type = (DummyParamType*) nullptr) : Super()
	{
	...
	/// @param[in] value							If nullptr, set the error object to an OutOfMemoryError.
	//----------------------------------------------------------------------------------------
	template <typename T2> MAXON_IMPLICIT Result(const ResultRef<T2>& value, typename std::enable_if<std::is_void<typename SFINAEHelper<T, T2>::type>::value, DummyParamType*>::type = (DummyParamType*) nullptr) : Super()
	{
		if (!value.GetPointer())
			this->_error = CreateErrorPtr(MAXON_SOURCE_LOCATION, ERROR_TYPE::OUT_OF_MEMORY);
	}
	...
	/// for a general <tt>Result<T></tt>.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<T>::value, ERROR_OK, volatile DummyParamType&>::type) : Super() { }
	...
	/// @param[in] value							The result value.
	//----------------------------------------------------------------------------------------
	template <typename T2, typename = typename std::enable_if<!maxon::HasBase<typename std::decay<T2>::type::ReferencedType, ErrorInterface>::value, T>::type::ReferencedType, typename = typename Super::template ConstructibleFrom<T2>::type>
	...
	/// for a general <tt>Result<T></tt>.
	//----------------------------------------------------------------------------------------
	template <typename T2> Result(const Result<T2>& src, typename std::enable_if<std::is_void<typename SFINAEHelper<T, T2>::type>::value, DummyParamType*>::type = (DummyParamType*) nullptr) : Super() { this->_error = src.GetErrorStorage(); }
	...
	explicit Result(typename std::conditional<std::is_same<typename std::decay<T>::type, Bool>::value, volatile DummyParamType&&, Bool>::type) = delete;
	...
	// This avoids cases like "Func() = value;" where the writer expected Func() to return a T&, but it returns a Result<T&>.
	void operator =(typename std::conditional<std::is_reference<T>::value, T, const DummyParamType&>::type) = delete;
...
{
	return rv.MoveValueGetError(err);
}
...
public:
	...
	{
	public:
		explicit Wrapper(ResultIteratorFunctions* it) : _it(it) { }
