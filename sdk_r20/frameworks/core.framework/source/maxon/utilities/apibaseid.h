...
protected:
	...
	UInt _hash; ///< Combination of hash code (lower bits), then a bit at #RC_STRING_BIT and finally the string length (highest eight bits). Use #HASH_MASK to mask out the #RC_STRING_BIT.
...
public:
	...
	void Reset()
	{
		...
		// Without this test MSVC throws a write access violation for const Id's that are defined in the global scope.
		if (_hash == 0 && _value == nullptr)
			return;
#endif
