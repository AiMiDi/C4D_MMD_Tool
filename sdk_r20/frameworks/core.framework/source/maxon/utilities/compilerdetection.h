...
#undef _HAS_STATIC_ASSERT
	...
	// as of v13 there is no alignas support yet
	#define alignas(_a_)				__declspec(align(_a_))
	#define alignof(_a_)				__alignof(_a_)
	...
	// in VC 11 there is no alignas support yet
	#define alignas(_a_)				__declspec(align(_a_))
	#define alignof(_a_)				__alignof(_a_)
#else
...
#if defined(MAXON_TARGET_WINDOWS) && defined(MAXON_COMPILER_INTEL) // otherwise an internal compiler error happens
template <typename T, size_t N> struct extent<T[N]> : public integral_constant<size_t, N>
{
};
#endif
