...
{
	CriticalAssert(g_privateBinary.GetInitializer());
	return (*g_privateBinary.GetInitializer())(const_cast<EntityBase*>(this), cause);
}
	...
	: _super(type, std::move(idValue), unit, flags | EntityBase::FLAGS::DEFINITION), _nextOfUnit(nullptr), _references(nullptr), _moduleReference(nullptr)
{
	if (unit)
	{
		...
		if (file)
		{
			unit->_super._id = Id(GetFilename(file));
		}
#endif
	}
}
...
EntityDefinition::EntityDefinition(const EntityReference& reference, TranslationUnit* unit, const Char* file, EntityBase::FLAGS flags)
	: EntityDefinition(reference.GetBase().GetType(), Id(reference.GetBase().GetId()), unit, file, flags | EntityBase::FLAGS::DEFINITION)
{
...
ModuleInfo::ModuleInfo(const Char* idValue, TranslationUnit* unit, EntityBase::FLAGS flags) : _super(EntityBase::TYPE::MODULE, Id(idValue), unit, nullptr, flags), _next(g_privateBinary._modules), _binary(&g_privateBinary)
{
	g_privateBinary._modules = this;
}
...
InterfaceReference::InterfaceReference(const Char* iidWithoutHash, const Char* sourceName, const InterfaceReference* const* baseInterfaces, const Char* methodIds, TranslationUnit* unit, EntityBase::FLAGS refType)
	: _super(EntityBase::TYPE::INTERFACE, iidWithoutHash, unit, refType), _infoOffset(LIMIT<Int>::MIN), _type(nullptr), _basesPtr(baseInterfaces), _methodIds(methodIds), _idWithoutHash(iidWithoutHash), _sourceName(sourceName), _version(nullptr), _depth(-1)
{
}
...
Result<Object*> Object::Clone() const
{
	iferr_scope;
	iferr (Object* c = GetClass().GetPointer()->CreatePointer())
		return err;
...
void Object::DestructInstance(const Object* o)
{
...
Bool Object::PrivateIsInstanceOf(const DataType* type) const
{
	if ((type->GetValueKind() & VALUEKIND::OBJECT_REF))
  	type = type->GetElementType();

	const InterfaceDefinition* const* interfaces = type->GetInterfaces();
...
void ObjectDefinition::AddClassExtension(const void* ptr)
{
	...
	const GenericClass& cls = reinterpret_cast<const GenericClass&>(ptr);
