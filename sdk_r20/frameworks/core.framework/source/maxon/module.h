...
// used internally to initialize interned ids defined with MAXON_INTERNED_ID or MAXON_LOCAL_INTERNED_ID
...
//----------------------------------------------------------------------------------------
#define MAXON_LOCAL_INTERNED_ID(IID, NAME) maxon::InternedId NAME; static maxon::InternedIdInitializer g_iid_##NAME(&NAME, IID, &PRIVATE_MAXON_MODULE)
// note: we don't use static here because then we'd have internal linkage, and this would prevent the usage as argument for a non-type template parameter
...
//----------------------------------------------------------------------------------------
extern "C" MAXON_ATTRIBUTE_DLL_PUBLIC Binary g_privateBinary;
...
protected:
	...
	explicit ModuleInfo(const Char* idValue) : _super(EntityBase::TYPE::REGISTRY_MODULE, Id(idValue), nullptr, nullptr, EntityBase::FLAGS::NONE), _next(nullptr), _binary(&g_privateBinary)
	{
	}
...
private:
	...
	const ModuleInfo* _next; ///< Next in singly linked list of modules of the current library
...
template <> struct EntityType<GenericClass>
{
...
public:
	...
	template <TranslationUnit& UNIT = g_translationUnit, UInt64 UNIT_HASH = g_translationUnitHash> const T& operator ()() const
	{
		...
		{
      DebugAssert(g_privateBinary.GetInitializer());
      (*g_privateBinary.GetInitializer())(const_cast<EntityBase*>(&_super.GetDefinition()->GetBase()), _super.GetBase().GetId()) iferr_ignore("_object checked afterwards.");
			if (MAXON_LIKELY(_object))
			{
				return *_object;
			}
	...
	template <TranslationUnit& UNIT = g_translationUnit, UInt64 UNIT_HASH = g_translationUnitHash> Result<const T&> WeakGet() const
	{
		...
		{
			DebugAssert(g_privateBinary.GetInitializer());
			Result<void> res = (*g_privateBinary.GetInitializer())(const_cast<EntityBase*>(&_super.GetDefinition()->GetBase()), _super.GetBase().GetId());
			if (res == FAILED)
...
public:
	...
	template <typename T> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), ::MaxonRegistry* dummy, TranslationUnit* unit, const Char* file, EntityBase::FLAGS flags)
		...
		_init((void*) init), _free(nullptr), _initDefinition(&InitObject<T>), _freeDefinition(&FreeObject<T>)
	{
	}
	...
	template <typename T, typename REG> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), REG* dummy, TranslationUnit* unit, const Char* file, EntityBase::FLAGS flags)
		: _super(maxon::details::EntityType<T>::value, std::move(iid), unit, file, flags | REG::ENTRY_FLAGS),
			_init((void*) init), _free(nullptr), _initDefinition(&InitRegistryEntry<T, REG>), _freeDefinition(&FreeRegistryEntry<T, REG>), _registry(REG::GetId())
	{
	}
	...
	/// @tparam REG										Type of the registry.
	//----------------------------------------------------------------------------------------
	template <typename T, typename REG> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), void (*free) (T&), REG* dummy, TranslationUnit* unit, const Char* file, EntityBase::FLAGS flags)
		: _super(maxon::details::EntityType<T>::value, std::move(iid), unit, file, flags | REG::ENTRY_FLAGS),
			_init((void*) init), _free((void*) free), _initDefinition(&InitRegistryEntry<T, REG>), _freeDefinition(&FreeRegistryEntry<T, REG>), _registry(REG::GetId())
	{
	}
...
private:
	...
	{
		// TODO: (Ole) After _object.Reset() ptr is invalid.
		T* ptr = this_->_free ? (T*) this_->_object.PrivateGetPtr() : nullptr;
		this_->_object.Reset();
		if (std::is_same<const typename Dereference<T>::type, const ClassInterface>::value)
		{
	...
	void* _free; ///< The free function, may be nullptr.
...
#define PRIVATE_MAXON_INITIALIZATION_C(ID) \
	static struct MAXON_CONCAT(PrivateInitialization_, ID) : public maxon::InitializationFunctions \
	{ \
		MAXON_CONCAT(PrivateInitialization_, ID)(maxon::Result<void> (*init) (), void (*free) ()) : maxon::InitializationFunctions(__FILE__, init, free, &maxon::g_translationUnit) { } \
...
#define PRIVATE_MAXON_DECLARATION(T, Name, id, getInstanceBody, storage) \
	...
	{ \
		...
		static maxon::Id GetId() { return maxon::Id(id); } \
...
#define PRIVATE_MAXON_DECLARATION_REGISTER_DECLARATION(Name, prefix, id) \
	prefix Name##PrivateHelper::DeclarationType prefix Name(maxon::LiteralId(id), &maxon::g_translationUnit);
...
#define MAXON_DEPENDENCY_REGISTER(Name) \
	maxon::EntityDefinition g_dependency_##Name(maxon::EntityBase::TYPE::DEPENDENCY, maxon::Id(#Name), &maxon::g_translationUnit, __FILE__, maxon::EntityBase::FLAGS::HIDDEN);
#endif
