...
ResultMem GenericBaseArray::EnsureCapacity(const DataType* elementType, Int requestedCapacity, COLLECTION_RESIZE_FLAGS resizeFlags)
{
	...
	Char* mem = (Char*) DefaultAllocator::Alloc(requestedCapacity * elementType->GetSize(), MAXON_SOURCE_LOCATION);
	if (!mem)
		return FAILED;
...
ResultMem GenericBaseArray::Resize(const DataType* elementType, Int count, COLLECTION_RESIZE_FLAGS resizeFlags)
{
	...
	else
	{
		...
		Char* mem = (Char*) DefaultAllocator::Alloc(totalCapacity * elementType->GetSize(), MAXON_SOURCE_LOCATION);
		if (!mem)
			return FAILED;
		elementType->MoveConstruct(mem, _ptr, _cnt);
		elementType->Destruct(_ptr, _cnt);
	...
	if (fit)
	{
		Char* mem = count ? (Char*) DefaultAllocator::Alloc(count * elementType->GetSize(), MAXON_SOURCE_LOCATION) : (Char*) this;
		if (!mem)
			return OK;
...
UInt GenericBaseArray::GetHashCode(const DataType* elementType) const
{
	...
	for (Int i = 0; i < _cnt; ++i)
	{
		hash = 31 * hash + DefaultCompare::GetHashCode(ptr);
		ptr += elementType->GetSize();
	}
	return hash;
}
