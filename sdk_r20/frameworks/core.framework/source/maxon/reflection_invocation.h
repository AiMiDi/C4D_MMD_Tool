...
#define MAXON_INVOCATION_BASE(N, ...) \
	...
	{ \
		...
		{ \
			*(T*) result = ((T(*)(MAXON_MAKE_COMMA_LIST(MAXON_INVOCATION_BASE_HELPER1, , , __VA_ARGS__))) fn)(MAXON_MAKE_COMMA_LIST(MAXON_INVOCATION_BASE_HELPER5, , , __VA_ARGS__)); \
		} \
		...
		{ \
			...
			*(T*) result = fn.GetStubPointer<typename Type::StubPtrType>()(fn.GetStaticFunctionPointer<void*>() MAXON_MAKE_LIST(MAXON_INVOCATION_BASE_HELPER6, , , , __VA_ARGS__)); \
		} \
	}; \
...
struct DelegateHandlerData
{
	...
	DelegateHandlerData(const DelegateDataType* t, DelegateHandler&& h) : type(t), handler(std::move(h)) { }
#endif
...
template <typename R, typename... ARGS> struct DelegateHandlerHelper
{
	static R Invoke(void* h, ARGS... args)
	{
		...
		d->handler(d->type, DelegateInvocation(&result, a));
		return result;
	}
};
...
template <typename... ARGS> struct DelegateHandlerHelper<void, ARGS...>
{
	static void Invoke(void* h, ARGS... args)
	{
		reflection::Argument a[] = {reflection::Argument::For<ARGS>(args)...};
		DelegateHandlerData* d = (DelegateHandlerData*) h;
		d->handler(d->type, DelegateInvocation(nullptr, a));
	}
};
...
template <typename R> struct DelegateHandlerHelper<R>
{
	static R Invoke(void* h)
	{
		R result;
		DelegateHandlerData* d = (DelegateHandlerData*) h;
		d->handler(d->type, DelegateInvocation(&result, Block<reflection::Argument>()));
		return result;
	}
};
...
template <> struct DelegateHandlerHelper<void>
{
	...
	{
		DelegateHandlerData* d = (DelegateHandlerData*) h;
		d->handler(d->type, DelegateInvocation(nullptr, Block<reflection::Argument>()));
	}
};
