...
namespace URLFLAGS
{
	...
	MAXON_ATTRIBUTE(DataDictionary, HTTP_ADDITIONAL_REQUEST_HEADER, "net.maxon.url.http_additional_request_header");	///< Each entry in the dictionary will be added to the http request in the form "key: value\r\n".
	MAXON_ATTRIBUTE(DataDictionary*, HTTP_RESPONSE_HEADER, "net.maxon.url.http_response_header");	///< Writes the response header into the datadictionary.
	MAXON_ATTRIBUTE(Bool, HTTP_FOLLOWMOVEDFILES, "net.maxon.url.http_followmovedfiles");						///< Set to true if a http connection should follow "301 permanently moved file" and open the moved file instead.
	MAXON_ATTRIBUTE(Bool, HTTP_POST, "net.maxon.url.http_post");						///< Set to true if a you want to send a POST request, otherwise GET will be assumed.
	MAXON_ATTRIBUTE(CString, HTTP_POSTDATA, "net.maxon.url.http_postdata");					///< If HTTPPOST is true, add additional data. the data needs to be url encoded. the cstring eeds to be in the format: "<<parameter>>=<<data>>&..."
	MAXON_ATTRIBUTE(Bool, HTTP_NO_PROXY_CACHING, "net.maxon.url.http_no_proxy_caching");						///< If a proxy Server is used set the "Cache-Control: no-cache"
	MAXON_ATTRIBUTE(Bool, HTTP_DISALLOW_GZIP, "net.maxon.url.http_disallow_gzip");						///< Set to true, if gzip compression is not allowed.
...
public:
	...
	/// E.g. if the Url is "zip://[file:///c:/images.zip]/image01.jpg" a call to SetEmbeddedUrl("file:///c:/otherimages.zip") will result in "zip://[file:///c:/otherimages.zip]/image01.jpg".
	...
	/// E.g. if the Url is "file:///c:/directory/file.txt" a call of SetName("change.tif") will result in "file:///c:/directory/change.tif".
	...
	/// E.g. if the Url is "file:///c:/directory/file.txt" a call of SetSuffix("jpg") will result in "file:///c:/directory/file.jpg".
...
/// E.g. allowed is
/// Url a = Url("file:///HD") + "test"_s + "image.tif"_s;
/// Url b = Url("file:///HD") + Url("test/image.tif"_s);
/// whereas this will fail:
/// Url a = Url("file:///HD") + "test/image.tif"_s;
/// Url a = Url("file:///HD") + Url("file:///test");
///
///
/// Here an overview over different types of Urls:
///
/// - Url("file:///c:/test/image.tif") - this is a regular Windows file
///   equivalent to Url("c:\test\image.tif") or Url("c:/test/image.tif")
...
///
/// - Url("file:///HD/test/image.tif") - this is a regular OS X or Linux file
///   equivalent to Url("/HD/test/image.tif")
/// 	GetScheme() returns URLSCHEME_FILE
/// 	GetAuthority() returns an empty Url with UrlScheme()
/// 	GetPath() returns "HD/test/image.tif"
/// 	GetSystemPath() returns "/HD/test/image.tif"
///
/// - Url("file://qnap/test/image.tif") - this is a file on a Windows network drive
///   equivalent to Url("\\qnap\test\image.tif") or Url("//qnap/test/image.tif")
/// 	GetScheme() returns URLSCHEME_FILE
...
///
/// - Url("ftp://192.168.0.1/test/image.tif") - this is a regular ftp file
///   equivalent to Url(URLSCHEME_FTP, Url(URLSCHEME_AUTHORITY, Url(), "192.168.0.1"), "test/image.tif")
...
///
/// - Url("zip://[http://192.168.0.1:1234/archive.zip]/test/image.tif") - this is a file embedded in a ZIP archive on a Web server.
///   equivalent to Url(URLSCHEME_ZIP, Url("http://192.168.0.1:1234/archive.zip"), "test/image.tif")
/// 	GetScheme() returns URLSCHEME_FILE
/// 	GetAuthority() returns the embedded zip file - a Url with URLSCHEME_HTTP, Authority 192.168.0.1:1234 and path "archive.zip"
/// 	GetPath() returns "test/image.tif"
/// 	GetSystemPath() returns "test/image.tif"
///
/// - Url("test/image.tif") - this is relative path
///   equivalent to Url("./test/image.tif")
