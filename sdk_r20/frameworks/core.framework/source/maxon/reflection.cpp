...
const ComponentInfo& ClassInfo::PrivateGetClassProxyComponentInfo(const Object* o, const ComponentDescriptor& d, const GenericComponent* comp)
{
...
FunctionInfo::FunctionInfo(const Char* n, Member::KIND k, const ParameterType& ret, const Block<const ParameterType>& params, Int64 inOut, const void* nat, InvocationWrapper wrap) : name(n), kind(k), returnType(ret), native(nat), wrapper(wrap)
{
	Bool hasThis = (Bool)(k & (Member::KIND::INSTANCE_FUNCTION | Member::KIND::CONST_INSTANCE_FUNCTION));
	if (hasThis)
		inOut >>= 2;
	for (const ParameterType& t : params.Slice(hasThis ? 2 : 1))
	{
		iferr (parameterTypes.Append(ParameterType(t.GetType(), GetFlags(t, Int(inOut & 3)))))
		{
			name = nullptr;
			break;
		}
