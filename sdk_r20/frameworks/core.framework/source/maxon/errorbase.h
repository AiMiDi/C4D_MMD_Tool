...
#define PRIVATE_MAXON_iferr(...)		MAXON_EXPAND(PRIVATE_MAXON_iferr_(ErrorPtr, __COUNTER__, __VA_ARGS__))
#define PRIVATE_MAXON_ifnoerr(...)	MAXON_EXPAND(PRIVATE_MAXON_iferr_(NoErrorPtr, __COUNTER__, __VA_ARGS__))
#else
...
///
/// Optionally, you can specify a message which will be used in addition to the verbatim condition.
...
/// @param[in] ...								Additional message and parameters, will be formatted by @ref FormatString.
/// @see CheckState
/// @see CheckAssert
//----------------------------------------------------------------------------------------
#define CheckArgument(condition, ...) do { if (MAXON_UNLIKELY(!(condition))) iferr_throw(maxon::IllegalArgumentError(MAXON_SOURCE_LOCATION, maxon::String(FormatString("Illegal argument: Condition " #condition " not fulfilled. " __VA_ARGS__))).DbgStop()); } while (false)
...
/// @see CheckAssert
//----------------------------------------------------------------------------------------
#define CheckState(condition, ...) do { if (MAXON_UNLIKELY(!(condition))) iferr_throw(maxon::IllegalStateError(MAXON_SOURCE_LOCATION, maxon::String(FormatString("Illegal state: Condition " #condition " not fulfilled. " __VA_ARGS__))).DbgStop()); } while (false)
...
/// @see CheckState
//----------------------------------------------------------------------------------------
#define CheckAssert(condition, ...) do { if (MAXON_UNLIKELY(!(condition))) iferr_throw(maxon::UnexpectedError(MAXON_SOURCE_LOCATION, maxon::String(FormatString("Assertion " #condition " failed. " __VA_ARGS__))).DbgStop()); } while (false)
