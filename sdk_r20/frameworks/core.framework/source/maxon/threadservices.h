...
class ThreadServices
{
	...
	friend class JobBarrierInterface;
	friend class ThreadProfilingServices;
...
private:
	...
	//----------------------------------------------------------------------------------------
	/// Allocates a barrier object.
	/// THREADSAFE.
	/// @return												Barrier pointer or nullptr if allocation failed.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD JobBarrierInterface* CreateBarrier();

	//----------------------------------------------------------------------------------------
	/// Enqueues a barrier (will add a reference and remove it when the object is no longer needed).
	/// THREADSAFE.
	/// @param[in] barrier						The barrier job.
	/// @param[in] queue							The queue, use JOBQUEUE_CURRENT for the current queue.
	//----------------------------------------------------------------------------------------
	static MAXON_METHOD void EnqueueBarrier(JobBarrierInterface& barrier, JobQueueInterface* queue);
	...
	//----------------------------------------------------------------------------------------
	/// Notifies the job system of acquire or releasing an exclusive resource in which threaded code is executed.
	...
	static MAXON_METHOD Result<void> ThreadProfiling(Int mode, Block<ThreadInterface*> threads, Block<JobQueueInterface*> queues, QueueProfileData& data);
};
/// @endcond
