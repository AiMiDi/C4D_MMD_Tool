...
public:
	...
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!std::is_base_of<BaseRef, REF>::value && maxon::HasBase<typename REF::ReferencedType, ReferencedType>::value>::type> MAXON_IMPLICIT BaseRef(const REF& src) : _object(maxon::HasBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()))
	{
	...
	template <typename REF, typename = typename std::enable_if<!std::is_base_of<BaseRef, REF>::value && maxon::HasBase<typename REF::ReferencedType, ReferencedType>::value>::type> MAXON_IMPLICIT BaseRef(REF& src) : _object(maxon::HasBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()))
	{
		static_assert(!(HANDLER::KIND & REF::Handler::KIND & VALUEKIND::UNIQUE_REFERENCE), "Can't copy a BaseRef with unique ownership.");
		static_assert(!(((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::UNIQUE_REFERENCE) && ((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::STRONG_MASK)),
									"Can't copy a BaseRef between unique and strong ownership.");
	...
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!std::is_base_of<BaseRef, REF>::value && maxon::HasBase<typename REF::ReferencedType, ReferencedType>::value>::type> BaseRef& operator =(const REF& src)
	{
		static_assert(!(HANDLER::KIND & REF::Handler::KIND & VALUEKIND::UNIQUE_REFERENCE), "Can't copy a BaseRef with unique ownership.");
		static_assert(!(((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::UNIQUE_REFERENCE) && ((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::STRONG_MASK)),
									"Can't copy a BaseRef between unique and strong ownership.");
		return this->operator =(maxon::HasBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()));
	}

	// Duplication for non-const REF, see above for the constructor.
	template <typename REF, typename = typename std::enable_if<!std::is_base_of<BaseRef, REF>::value && maxon::HasBase<typename REF::ReferencedType, ReferencedType>::value>::type> BaseRef& operator =(REF& src)
	{
		static_assert(!(HANDLER::KIND & REF::Handler::KIND & VALUEKIND::UNIQUE_REFERENCE), "Can't copy a BaseRef with unique ownership.");
		static_assert(!(((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::UNIQUE_REFERENCE) && ((HANDLER::KIND | REF::Handler::KIND) & VALUEKIND::STRONG_MASK)),
									"Can't copy a BaseRef between unique and strong ownership.");
	...
	/// @param[in] src								Another BaseRef for the move.
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!std::is_const<REF>::value && !std::is_base_of<BaseRef, REF>::value && maxon::HasBase<typename REF::ReferencedType, ReferencedType>::value>::type> MAXON_IMPLICIT BaseRef(REF&& src) : _object(maxon::HasBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer()))
	{
	...
	/// @param[in] src								BaseRef whose internal pointer shall be moved to this BaseRef.
	//----------------------------------------------------------------------------------------
	template <typename REF, typename = typename std::enable_if<!std::is_const<REF>::value && !std::is_base_of<BaseRef, REF>::value && maxon::HasBase<typename REF::ReferencedType, ReferencedType>::value>::type> BaseRef& operator =(REF&& src)
	{
		...
		_object = maxon::HasBase<typename REF::ReferencedType, ReferencedType>::Cast(src.GetPointer());
		reinterpret_cast<BaseRef&>(src)._object = nullptr;
	...
	//----------------------------------------------------------------------------------------
	/// Returns the DataType of T's reference type.
	//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
template <typename T> class Pointer : public BaseRef<T, PointerHandler>
{
public:
	MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(Pointer, BaseRef<T, PointerHandler>);

	//----------------------------------------------------------------------------------------
	/// Returns true when this Pointer points to the same object as other.
	/// @param[in] other							Another pointer.
	/// @return												True if both pointers point to the same object, false otherwise.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Returns true when this Pointer doesn't point to the same object as other.
	/// @param[in] other							Another pointer.
	/// @return												True if both pointers don't point to the same object, false otherwise.
	//----------------------------------------------------------------------------------------
	Bool operator !=(Pointer other) const
	{
		return this->_object != other._object;
...
#define NewObjCopy(T, SRC) (maxon::details::NewObjHelper<T>::IS_ALLOC_TYPE ? maxon::details::NewObjHelper<T>::AllocType::Alloc(maxon::SourceLocation(__FILE__, __LINE__, maxon::PrivateGetClassName<T>()), SRC) \
														: ResultPtr<T>((T*) PrivateInitCopy(maxon::details::NewObjHelper<T>::AllocatorType::Alloc(maxon::details::NewObjHelper<T>::SIZE, maxon::SourceLocation(__FILE__, __LINE__, maxon::PrivateGetClassName<T>())), reinterpret_cast<const PRIVATE_MAXON_ALLOC_TYPENAME maxon::details::NewObjHelper<T>::ConstructorType&>(SRC))))
...
{
public:
	static const VALUEKIND KIND = Or<VALUEKIND, VALUEKIND::COW_REFERENCE, VALUEKIND::INT_SIZE_OR_LESS, VALUEKIND::ZERO_INITIALIZED, VALUEKIND::REF_OR_POINTER>::value;

#ifdef MAXON_COMPILER_INTEL // Intel bug: MAXON_IS_COW_KIND macro doesn't work in SFINAE contexts
	using COWMarker = void;
...
{
public:
	static const VALUEKIND KIND = Or<VALUEKIND, VALUEKIND::ACOW_REFERENCE, VALUEKIND::INT_SIZE_OR_LESS, VALUEKIND::ZERO_INITIALIZED, VALUEKIND::REF_OR_POINTER>::value;
