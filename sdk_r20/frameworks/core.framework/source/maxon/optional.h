...
public:
	...
	const T& GetValueRef() const
	{
		return *reinterpret_cast<const T*>(&_valueStorage);
	}
...
private:
	...
	void Destruct()
	{
		reinterpret_cast<const T*>(&_valueStorage)->~T();
	}

	using ValueHolder = AlignedStorage<SIZEOF(T), alignof(T)>;

	Bool				_hasValue;
	ValueHolder _valueStorage;
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Returns the moved out contained value, or a default value if Opt contained no value.
	//----------------------------------------------------------------------------------------
	template <typename U>
	T MoveValueOr(U&& defaultValue)
	{
