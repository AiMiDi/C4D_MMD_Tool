...
// TODO: (Ingo) remove when at least GCC 4.9.4 is used.
	...
	/// Calculates dot product of v1 and v2
	template <typename T2, Int S2> friend typename MultiplicativePromotion<T, T2>::type Dot(const Vec2& v1, const Vec3<T2, S2>& v2)
	{
		return v1.x * v2.x + v1.y * v2.y;
	}
...
template <GET_DATATYPE_POLICY POLICY, typename T> inline Result<const DataType*> PrivateGetDataType(Vec2<T, 1>*, OverloadRank0)
{
	iferr_scope;
