...
protected:
	...
	Generic& GetMember(typename std::integral_constant<UInt32, INDEX>::type*)
	{
		...
		Generic* ptr = (Generic*) ((Char*) this + Align(PRE_OFFSET, UInt32(SZ_ALIGN >> 33)));
		return *((SZ_ALIGN & (UInt64(1) << 32)) ? *(Generic**) ptr : ptr);
	}

	Generic& GetMember(typename std::integral_constant<UInt32, INDEX>::type*, Bool)
	{
		static_assert(SIZEOF(TupleStorageLayout) == TupleStorageLayout::SIZE, "Wrong size.");
		static_assert(alignof(TupleStorageLayout) == TupleStorageLayout::ALIGN, "Wrong alignment.");
		Generic* ptr = (Generic*) ((Char*) this + Align(PRE_OFFSET, UInt32(SZ_ALIGN >> 33)));
...
template <typename T> inline constexpr UInt64 GetSizeAndAlignment()
{
...
#else
	// MSVC 2015 has a strange behaviour with the above code: It uses this constructor even when no argument is given at all
	// (it should take the default constructor), and then it doesn't generate code for the constructor.
	template 
	<
		typename U1,
		typename... U,
		typename = typename std::enable_if<sizeof...(U)+1 == Super::COUNT && std::is_constructible<typename Pack::FirstType, U1&&>::value>::type
	>
	MAXON_IMPLICIT Tuple(U1&& u1, U&&... x) : Super((IndexSequence*) nullptr, std::forward<U1>(u1), std::forward<U>(x)...)
	{
	}
#endif
