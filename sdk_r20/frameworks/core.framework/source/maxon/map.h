...
public:
	...
	MapImpl(MapImpl&& src) : Super(_clsMTable), _map(static_cast<typename std::conditional<REFERENCE, MAP, MAP&&>::type>(src._map))
	{
...
//		DebugAssert(System::GetReferenceCounter(&src) == 0);
	...
	typename std::add_lvalue_reference<MAP>::type GetMap() { return REFERENCE ? *(MapType*) this->_refDummy : _map; }

	const MapType& GetMap() const { return REFERENCE ? *(const MapType*) this->_refDummy : _map; }
#else
