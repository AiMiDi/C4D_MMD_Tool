...
/// Marker class similar to HashMapKeyValuePair, but the key will be the last data member of the HashMap entry. This can be used if keys have a variable size.
...
template <typename T, typename HASH = DefaultCompare, typename ALLOCATOR = DefaultAllocator, Bool SYNCHRONIZED = false> class HashSet
	: public SetBase<HashSet<T, HASH, ALLOCATOR, SYNCHRONIZED>, T, Protected<HashMap<T, UnitType, HASH, HashMapKeyValuePair, ALLOCATOR, SYNCHRONIZED>>, HASH>
{
public:
	using MapType = HashMap<T, UnitType, HASH, HashMapKeyValuePair, ALLOCATOR, SYNCHRONIZED>;
