...
public:
	...
	explicit FId(const KEYTYPE& iid) : _id(iid) { }
...
template <typename T> class FId<T, InternedId>
{
	InternedId _id;
...
public:
	...
	explicit FId(const Char* str) : _initializer(&_id, str, &PRIVATE_MAXON_MODULE) { }

	const InternedId& Get() const { return _id; }

	operator const InternedId&() const { return _id; }

	operator const Id&() const { return _id.Get(); }
};
...
#define MAXON_RESOURCE_DATABASE_SCOPE(id) inline maxon::Id GetId() { return maxon::Id(id); } inline maxon::Id PrivateGetDatabaseScope() { return maxon::Id(id); }
...
#define PRIVATE_RESOURCE_ID_CLASS(UID, IID, TYPE, BODY, ...)                                                                                           \
	...
	public:                                                                                                                                  \
		...
		UID##_PrivateAttribute() : maxon::FId<ValueType, maxon::PrivateGetFIdKeyType<decltype(IID)>::type>(IID)                                                       \
		{                                                                                                                                      \
		}                                                                                                                                      \
...
public:
	...
	operator UInt() const
	{
		return _id ? _id->Get().GetHashCode() : 0;
	}
