...
class System
{
	MAXON_INTERFACE_NONVIRTUAL(System, MAXON_REFERENCE_NONE, "net.maxon.interface.system");
public:
	...
	static MAXON_METHOD void GetBinaries(Block<const ModuleInfo* const>& result); 
...
// Even MSVC 2015 doesn't work with +[] lambdas.
template <typename T> static void PrivateDestruct(void* o)
{
	using X = typename std::remove_const<T>::type;
	((X*)o)->~X();
}
...
template <typename T> void PrivateFreeWithDestructor(const T* obj)
{
	...
	{
		void(*destruct)(void*) = &PrivateDestruct<T>;

		System::FreeWithDestructor((void*)obj, destruct);
	}
}
