...
//----------------------------------------------------------------------------------------
#define MAXON_API_ABI_VERSION 1
...
// Intel compiler has a bug that casts to Generic&& are not allowed if this is incomplete
// MSVC has a bug which sometimes doesn't allow to have Generic& as return type
class Generic
{
};
#endif
...
template <typename... T> struct ParameterPack
{
	...
	//----------------------------------------------------------------------------------------
	template <Int INDEX> using At = void;
#else
	template <Int INDEX> using At = SFINAEHelper<DummyReturnType>;
#endif

	//----------------------------------------------------------------------------------------
	/// The first type of this parameter pack.
	//----------------------------------------------------------------------------------------
#if 0 // Commented out because this alias is only used for documentation. The actual alias is defined below in the specialization.
...
template <typename T, typename... REST> struct ParameterPack<T, REST...>
{
	...
	template <typename X> struct IndexOf
	{
		using Rest = typename ParameterPack<REST...>::template IndexOf<X>;
	...
	template <Int INDEX> using At = typename std::conditional<INDEX == 0, SFINAEHelper<T>, typename ParameterPack<REST...>::template At<INDEX-1>>::type;
};
...
template <typename A, typename B> class CommonType
	...
	: public CommonLValue<decltype(*(true ? (typename std::decay<A>::type*) nullptr : (typename std::decay<B>::type*) nullptr)), A, B>
#else
...
#ifdef MAXON_COMPILER_MSVC
template <typename T, Int> class IsComplete
{
private:
	template <typename U, int = sizeof(U)> static Char Test(int);
...
//----------------------------------------------------------------------------------------
/// This type trait checks if @p T is has a ConstReferencedType type member. If so, its member @c value will be @c true and its member @c type will be a
...
/// @tparam T											Type to check.
//----------------------------------------------------------------------------------------
MAXON_MEMBERTYPE_DETECTOR(IsNonvirtualInterfaceType, IsNonvirtualInterfaceType, std::false_type); // true for non-virtual or non-simple virtual interfaces
...
/// @tparam T											Type to check.
//----------------------------------------------------------------------------------------
MAXON_MEMBERTYPE_DETECTOR(IsVirtualInterfaceType, ProxyComponent, std::false_type);
...
#if defined(MAXON_COMPILER_MSVC) || defined(MAXON_COMPILER_INTEL) // they don't allow GenericArithmetic to be used in a union because it has a constructor, use a pointer type instead
...
//----------------------------------------------------------------------------------------
template <typename D, typename B> class HasBase
{
public:
...
{
	static const VALUEKIND KIND = VALUEKIND((std::is_trivially_copyable<T>::value ? UInt64(VALUEKIND::TRIVIALLY_COPYABLE) : 0)
#ifndef MAXON_COMPILER_GCC
		| (std::is_trivially_constructible<T>::value ? UInt64(VALUEKIND::TRIVIALLY_CONSTRUCTIBLE) : 0)
#endif
		| (std::is_trivially_destructible<T>::value ? UInt64(VALUEKIND::TRIVIALLY_DESTRUCTIBLE) : 0)
		| (std::is_scalar<T>::value ? UInt64(VALUEKIND::SCALAR) : 0)
		| (std::is_integral<T>::value ? UInt64(VALUEKIND::INTEGRAL) : 0)
		| (std::is_floating_point<T>::value ? UInt64(VALUEKIND::FLOATING_POINT) : 0)
		| (IsTriviallyEquatable<T>::value ? UInt64(VALUEKIND::TRIVIALLY_EQUATABLE) : 0)
		| (IsZeroInitialized<T>::value ? UInt64(VALUEKIND::ZERO_INITIALIZED) : 0)
		| ((SIZEOF(T) <= SIZEOF(Int)) ? UInt64(VALUEKIND::INT_SIZE_OR_LESS) : 0));
};
...
template <> struct ValueKindHelper<Object, false>
{
...
public:
	...
	/// @return												OK on success.
	//----------------------------------------------------------------------------------------
	static MAXON_ATTRIBUTE_FORCE_INLINE Result<void> AssignCopy(T& dst, const T& src)
	{
...
public:
	...
	/// @return												Always true.
	//----------------------------------------------------------------------------------------
	static MAXON_ATTRIBUTE_FORCE_INLINE ResultOk<void> AssignCopy(T& dst, const T& src)
	{
		dst = src;
		return OK;
	}
...
/// Copy assignment
//----------------------------------------------------------------------------------------
template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE Result<void> AssignCopy(T& dst, const T& src)
{
