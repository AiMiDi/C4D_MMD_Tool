...
typeKeywords = set(['const', 'volatile', 'signed', 'unsigned', 'bool', 'char', 'short', 'int', 'long', 'float', 'double'])
opening = set(['(', '{', '[', '<'])

def isProbablyIdentifier(t):
    return t.find('_') >= 0 or (len(t) >= 2 and not t[1:].islower())
...
def resolveExpr(scope, lex, sep, closing, createLinks, parseDoc=False, firstToken=None, returnFunctionDecl=False, recursionDetection=None, checkOverloadTypes=False):
    s = ''
    wasTypename = False
    prevToken = None
    res = None
    ...
    while True:
        ...
        firstToken = None
        tIsClosing = t.type == Token.EOF or t.type == Token.HTML or t.type == Token.COMMAND or t == ',' or t == '`' \
        ...
        if t in sep or t == 'template':
            source = True
            s += t
        elif t.type == Token.IDENT and not t in typeKeywords:
            ...
            if wasTypename or nameBegin < 0:
                ...
                overloads = res.decl.findByName(t)
                decl = getFirst(overloads)
    ...
    def startList(self, indent, type):
        ...
        if type != '.':
            if not self.openElementsStack or self.openElementsStack[-1].tag != 'list' or self.openElementsStack[-1].indentation < indent:
                self.html += '\n'
                self.push('ol' if type == '#' else 'ul', 'list', indent)
                self.pushElement('li')
            else:
                self.html += '</li>\n<li>'
        self.startPara = False
    ...
    for line in doc:
        ...
        if stripSlashTriple:
            ...
            if lstr.startswith('///< ') or lstr.startswith('///<\t'):
                line = lstr[5:]
            elif lstr.startswith('///<') or lstr.startswith('/// ') or lstr.startswith('///\t'):
                line = lstr[4:]
...
def parseDox(doc, declref, outdir, checkOnly, errors):
    ...
    try:
        ...
        while True:
            ...
            if t.type == Token.EOF:
                break
            if not ((t.type == Token.IDENT or t in emCommands) or t == '::'):
            ...
            if t == '@code' or t == '@snippet':
                ...
                while True:
                    ...
                    if snippet and t.type == Token.COMMENT and t.startswith('////'):
                        lex2.consume('\n')
                        if code:
                            res, removeIndent = formatPre(code.rstrip(), removeIndent)
                        ...
                        if ws:
                            code += quoteHtml(ws)
                        ws = t
                    elif t.type == Token.NEWLINE and not code:
                        ...
                        if t.type == Token.EOF or t == '@endcode':
                            if code:
                                ...
                                out.appendParagraph(res)
                            break
                        elif not wasSep and t.type == Token.IDENT and codeScope.resolve(t):
                            t, s, decl, dummy = resolveExpr(codeScope, lex2, sepSet, whitespaceStarAmpSet, True, True, t)
                            code += s
                            if not decl:
                                code += quoteHtml(t)
                            else:
                                while t.isWhitespace() or t in whitespaceStarAmpSet:
                                    code += t
                                    t = lex2.get()
                ...
                while True:
                    ...
                    if nextToken.type != Token.WHITESPACE:
                        break
                if nextToken == '{':
                    out.push(t, 'em_brace', 10000)
                    nextToken = None
                else:
                    out.push(t, 'em_singleword', 10000)
                ...
                if dummyRef:
                    ...
                    out << ref
            elif t == '@tableofcontents' or t == '@private':
                pass
            elif t == '@CLASS':
                out << '\rC'
            elif t == '@image':
                ...
                if lex.get() == '{':
                    ...
                    while t == '::':
                        t = lex.getNoEof()
                        if decl:
                            decl = getFirst(decl.findByName(t))
                        t = lex.getNoEof()
                    if decl:
                        if alias:
                            if using == '@using':
                ...
                out << t
                emptySpace = 0
            elif t.type == Token.IDENT and len(t) > 2 and t.startswith('_') and t.endswith('_'):
                out << '<em>' + t[1:-1] + '</em>'
            elif not noAutolinkSave and not wasSep and t.type == Token.IDENT and (forceAutolinkSave or isProbablyIdentifier(t) or declref.findParam(t) or declref.resolve(t)):
                if declref.findParam(t):
                    out << '<tt>' + t + '</tt>'
                else:
                    t, s, decl, source = resolveExpr(declref, lex, defaultSep, closingSet, True, True, t, False)
                    if source or forceAutolinkSave or isProbablyIdentifier(s):
                        out << '<tt>' + s + '</tt>'
                    else:
                        out << s
                    nextToken = t
                ...
                if nextToken.type != Token.IDENT:
                    ...
                    out << '<tt>' + s + '</tt>'
            elif (t == '-' or (t == '*' and lex.next().isWhitespace()) or t == '.' or (t.isdigit() and lex.next() == '.')) and prevEmptySpace >= 0:
                if t != '.':
                    nextToken = lex.get()
                    if t.isdigit():
                        t = Token('#')
...
class DeclRef(object):
    ...
    def findByName(self, name):
        if self.childrenMap == None:
            ...
            for c in self.children + [j for i in self.inherited for j in self.inherited[i][0]]:
                if (isinstance(c.decl, CppDeclaration) and c.decl.kind != 'constructor') or isinstance(c.decl, Attribute):
    ...
    def resolveDecl(self, decl):
        if decl.owner:
            ...
            d = getFirst(p.findByName(decl.name))
            return d or p
        else:
            d = self
            while d.owner:
    ...
    def isInDoc(self):
        if self.decl.kind == 'MAXON_DATATYPE':
            return False
        d = self
        while d:
    ...
    def merge(self, decl, changedSet, framework):
    ...
    def addInheritedMembers(self, tscope, public, usings, members, visited, inherited, distance):
        for base, access in self.decl.bases:
    ...
    def collectSpecializationMembers(self):
        if self.instanceof(Class) and self.decl.name.find('<') >= 0:
            t = getFirst(self.owner.findByName(self.decl.simpleName))
            if t:
    ...
    if path.endswith('.dox'):
        ...
        if decls:
            if path.endswith('.ph'):
                ...
                tree.merge(decls, changedSet, framework)
...
def generateDocDatabase(args):
    ...
    for subpage in tree.parentPage:
