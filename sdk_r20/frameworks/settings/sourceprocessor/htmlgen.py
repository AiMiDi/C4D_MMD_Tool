...
class Node(object):
    ...
    def generateTree(self, cw):
        ...
        if self.isNamespace() or self.kind == '@page':
            haveChildren = False
            for c in sorted(self.children, key=lambda x: str(x.name).upper()):
...
def generate(args):
    ...
    cw << htmlDoctype << '<html>' << '<head>' << '<title>MAXON API Documentation</title>' << linkCss << '</head>' << '<frameset cols="20%,80%">'
    cw.incIndent(1)
    cw << '<frame src="tree.html" name="Tree">'
    cw << '<frameset rows="30%,70%">'
    cw.incIndent(1)
    cw << '<frame src="root.html" name="Overview">'
    cw << '<frame src="root_detail.html" name="Detail">'
    cw << '</frameset>'
