...
def replaceDefines(source):
    for findStr, replaceStr in defines:
        pos = 0
        while True:
            pos = source.find(findStr, pos)
            if pos < 0:
                break
            nextPos = pos + len(findStr)
            if source[nextPos] == '(':
                ...
                if defineFound <= newLineFound and commentFound <= newLineFound:
                    ...
                    while True:
                        ...
                        if c == '(':
                            ...
                            replace = replace.replace('%' + str(argIndex), source[argStart:nextPos].strip())
                            if c == ')':
                                break
                            argIndex += 1
                            argStart = nextPos + 1
...
def isComponentComparison(name, params):
    return name in ['IsEqual', 'Compare'] and params and params[0].type in ['const Object*', 'const maxon::Object*']
...
class Parser(object):
    ...
    # returns tuple (type as string, bool) where the bool tells if the type is a const-qualified type
    def parseTypeCheckConst(self, firstToken=None, parentheses=True, allowTilde=False):
        nest = 0
        type = ''
    ...
    def parseType(self, firstToken=None, parentheses=True):
        type, c = self.parseTypeCheckConst(firstToken, parentheses)
        return type
        ...
        try:
            ...
            while True:
                ...
                if self.args.stylecheck and self.args.stylecheckOptions.get('indentation', True) and indent >= 0 and ((prevPrevToken == '\n' and prevToken.type == Token.WHITESPACE) or prevToken == '\n') \
                    ...
                    elif t == 'break' or t == 'if' or t == 'iferr' or t == 'ifnoerr' or t == 'for' or t == 'while' or t == 'do' or t == 'switch' or t == 'else' or t == 'return' or t == 'goto':
        ...
        try:
            ...
            if self.lex.getNoEof() == ')':
                ...
                while True:
                    ...
                    if not ellipsis:
                        type = self.parseType(t)
                        if not params and type == 'void':
                    ...
                    if ellipsis:
                        ...
                        if t.type == Token.IDENT:
                            ...
                            param.name = 'anonymous_param_' + str(len(params) + 1)
                    if t == '=':
    ...
    def tryParseMethod(self, t, doclist, cls, access):
        ...
        static = False
        while True:
            if not t in varOrFunctionPrefixes:
                self.lex.reject()
                return None
            if t == 'MAXON_METHOD' or t == 'MAXON_FUNCTION':
                break
            if t == 'static':
                static = True
            t = self.lex.get(cls)
        ...
        if cls.interface == Class.VIRTUAL and name == 'GetHashCode' and len(method.params) == 0 and cls.name != 'Object':
    ...
    def tryParseVarOrFunction(self, t, owner, access):
        ...
        checkNaming = True
        method = False
        while t in varOrFunctionPrefixes:
            extern = False
            property = False
            if t == 'static':
                static = True
        ...
        if t.type == Token.IDENT or t == '::': # the type might start with ::
            ...
            if t == '::':
                type = None
            if conversion:
                type, constType = self.parseTypeCheckConst(None, False, False)
            else:
                type, constType = self.parseTypeCheckConst(t, True, True)
            if type:
                ...
                if type.endswith(')') and (name == '{' or name == ':'):
                    ...
                    t = name
                    if t == ':':
                        dummy, t = self.parseUntil(owner, t, False, '{', False, -1)
                    self.parseUntil(owner, t, False, '}', False, -1)
                    return True
                    ...
                    if t == ';' or t == '=' or t == '[' or t == '{':
                        ...
                        if namePrefix:
                            return None
                        if checkNaming and isinstance(owner, Namespace) and not name.startswith('g_') and not constType and not self.isObjectiveC \
    ...
    def tryParseObservable(self, t, cls, access):
        ...
        if not (static or t == 'MAXON_OBSERVABLE'):
            return None
        if static and cls.interface != Class.NONVIRTUAL:
            self.errors.append(t.pos, 'Virtual interfaces must not have static observables.')
        self.lex.consume('(')
        type = self.parseType()
        self.lex.consume(',')
    ...
    def parseTemplateParamDecl(self, template):
        ...
        while True:
            ...
            if t.type == Token.IDENT:
                ...
                t = self.lex.getNoEof()
            if t == '::':
                # special case where a nontype template parameter is like typename std::remove_const<T>::type X
                # skip the rest and ignore the parameter
    ...
    def tryParseEnum(self, t, owner, access):
        ...
        if e.registered:
            ...
            if t != ')':
                if t != ',':
                    self.errors.append(t.pos, 'Expected ) or comma, found ' + t + ' instead.')
                else:
            ...
            if name.type == Token.IDENT and (punct == '{' or punct == ':' or punct == ';'):
                ...
                if punct == ':':
                    ...
                    if t == 'public' or t == 'protected' or t == 'private':
                        ...
                        self.lex.reject()
                    t = self.parseType()
                    if t.startswith('MAXON_INTERFACE_BASES'):
                        p = t.find('(')
                        if p < 0:
                            punct.pos.raiseError('Expected ( after MAXON_INTERFACE_BASES.')
                        q = t.find(')')
                        if q < 0:
                            punct.pos.raiseError('Could not find closing ) of MAXON_INTERFACE_BASES.')
                        t = t[p + 1:q]
                        cls.baseInterfaces = t.split(',')
                        if not cls.baseInterfaces:
                            self.errors.append(punct.pos, 'Must not use an empty base interface list for MAXON_INTERFACE_BASES.')
                    else:
                ...
                if punct != ';':
                        ...
                        if i >= 0:
                            ...
                            if basicComp or comp.endswith('Component'):
                                ...
                                for i in bases.split(','):
                                    ...
                                    if skip > 0:
                                        skip -= 1
                                        if not basicComp and not i.startswith(clsName):
                    ...
                    getHashCode = None
                    addToRefClass = None
                    addToRefClassStart = -1
                    addToRefClassAccess = None
                    refClassMembers = []
                    while True:
                        t = self.lex.getNoEof(cls)
                        resetDoc = True
                        ...
                        if t == '}':
                            ...
                            Directive(t, cls)
                            resetDoc = False
                        elif t == 'MAXON_ADD_TO_REFERENCE_CLASS' or t == 'MAXON_ADD_TO_CONST_REFERENCE_CLASS' or t == 'MAXON_ADD_TO_COPY_ON_WRITE_REFERENCE_CLASS':
                            if addToRefClass:
                                self.errors.append(t.pos, 'Nesting of ' + t + ' not allowed.')
                            else:
                                addToRefClass = t
                                t = self.lex.consume('(')
                                addToRefClassStart = self.lex.cin.index
                                addToRefClassAccess = access
                        elif t == ')' and addToRefClass:
                            d = Declaration(addToRefClass, None, cls, access)
                            d.code = self.lex.cin.source[addToRefClassStart:self.lex.cin.index - 1] + '\n'
                            addToRefClass = None
                            access = addToRefClassAccess
                        elif self.handleBlock(cls, t, False, True, -1):
                            pass
                            ...
                            if cls.template:
                                self.errors.append(name.pos, 'Interface templates are only allowed for simple virtual interfaces.')
                        ...
                        elif t == 'MAXON_INTERFACE_SIMPLE_VIRTUAL' or t == 'MAXON_INTERFACE_DERIVED_SIMPLE_VIRTUAL':
                            ...
                            refKind = self.lex.getIdent()
                            self.lex.consume(')')
                            cls.derived = t == 'MAXON_INTERFACE_DERIVED_SIMPLE_VIRTUAL'
                            cls.setRefKind(refKind)
    ...
    def tryParseNamespaceMember(self, t, namespace):
        ...
        if t.type == Token.HXX1_INCLUDE:
            ...
            self.doclist.append(t)
            resetDoc = False
        elif t == '@end' or self.handleBlock(namespace, t, False, True, -1):
            pass
            ...
            if result:
                ...
                if not isinstance(result, Define) and self.args.stylecheck and self.args.stylecheckOptions.get('indentation', True):
                    ...
                    if lw is None or i < namespace.minIndentation or i > namespace.maxIndentation:
                        self.styleerrors.append(declStart.pos, 'Wrong indentation.')
    ...
    def tryParseNamespace(self, t, owner):
        if t == 'namespace' or t == 'RESOURCE' or t == 'RESOURCEi':
            ...
            if name.type == Token.IDENT and self.lex.get(owner) == '{':
    ...
    def parseFile(relFile, resultQueue, args, stampTime, previousFiles, generators, basedir, summary):
        ...
        if os.stat(file).st_mtime > stampTime or relFile not in previousFiles:
            ...
            try:
                if not args.encoding:
                    ...
                    import codecs
                    f = codecs.open(file, encoding=args.encoding)
                ...
                source = replaceDefines(source)
                lex = Lexer(file, source, not args.encoding)
                parser = Parser()
                parser.lex = lex
                ...
                source = f.read()
                f.close()
                errors.append(SourcePosition(file, e.start, 1 + source[0:e.start].count(10), -1), 'Non-ASCII character found in this line.')
            except IOError as e:
        ...
        for file in files:
            if (file.endswith('.h') or file.endswith('.cpp') or file.endswith('.mm')) and not file.startswith('.'):
                ...
                if all(check.lower().find(excl) < 0 for excl in args.excludedPaths):
                    sources[rel] = True
                    if rel in previousFiles and os.stat(file).st_mtime <= stampTime:
                        oldSources.add(file)
    resultQueue = SingleThreadResultQueue() if args.singlethreaded else Queue()
    errorMsg = ''
    errors = False
    exceptions = False
...
def parseSummary(file, root):
    if os.path.isfile(file):
        ...
        for line in sf:
            try:
                intf, value = line.strip().split('=')
                bases, methods = value.strip().split(':')
                n = intf.split('.')
                name = n.pop()
                ns = root
                for i in n:
...
def process(args):
    ...
    for key in keys:
        ...
        elif key == 'apis' and option:
            args.frameworks = [f.strip() for f in option.split(';')]
        elif key == 'stylecheck':
    ...
    if not args.projectId:
        args.projectId = os.path.basename(args.directory)
        if not args.maxon:
        ...
        args.projectId = 'net.maxon.' + args.projectId
    ...
    stamp = os.path.join(generatedDir, 'sourceprocessor.stamp')
    ...
    if stampExists:
        ...
        if os.stat(__file__).st_mtime > stampTime:
            stampTime = 0
        if os.stat(declarations.__file__).st_mtime > stampTime:
            stampTime = 0
        if os.stat(lexer.__file__).st_mtime > stampTime:
            stampTime = 0
        if os.stat(docdb.__file__).st_mtime > stampTime:
            stampTime = 0
        for g in generators:
            if os.stat(g.__file__).st_mtime > stampTime:
                stampTime = 0
        if os.stat(projDefPath).st_mtime > stampTime:
            stampTime = 0
    ...
    for run in range(0, 2):
        ...
        for fwDir in usedFrameworks(args.directory, True, set()):
            summaryFile = os.path.join(fwDir, 'generated', 'summary', 'framework.summary')
        ...
        if not (run == 0 and err and os.path.isfile(ownSummary) and os.stat(ownSummary).st_mtime > ownSummaryStampTime):
            break

        # The errors might be due to outdated summary information, so redo the whole process once.
        sys.stdout.write('Summary information changed, redoing source file processing.\n')
            
    msgFile = os.path.join(generatedDir, 'sourceprocessor.msg')
    ...
    writeToFile(stamp, '\n'.join([s for s in sources if sources[s]]) + '\n')
    os.utime(stamp, None)
