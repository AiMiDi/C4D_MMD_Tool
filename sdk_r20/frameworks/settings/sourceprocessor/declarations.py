...
def cleanupGeneratedFiles(args, dirname, extensionMap, sources):
    dir = os.path.join(args.directory, 'generated', dirname)
...
def usedFrameworks(projectDir, selfAndRecursive, visited):
    ...
    if frameworks:
        for fw in frameworks.split(';'):
            fw = fw.strip()
            if fw:
                fwDir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(projectDir))), 'frameworks', fw)
...
class Parameter(object):
    ...
    def forward(self):
        ...
        elif t.endswith('*') or t.endswith('&') or t == 'va_list':
            s = self.name
        else:
...
class Template(object):
    ...
    def toStr(self, nodef, onlyNames, mode=Entity.DEFAULT):
        ...
        if not onlyNames:
            s = 'template '
        return s + mode.quote('<') + ', '.join(x.toStr(nodef, onlyNames, mode) for x in self.params if mode.listTemplateParam(x)) + mode.quote('>')
...
class Function(CppDeclaration):
    ...
    def includeInRefClass(self):
        return self.name != 'operator =' and self.parseAnnotation('refclass', True) and self.access == 'public' \
...
#   self.bases contains an array of tuples (base class, access), may be empty
...
#   self.refKind is the reference kind, None for non-interfaces or for interfaces which use MAXON_REFERENCE_NONE
#   self.refClass is the name of the reference class, may be None
#   self.constRefClass is the name of the const reference class, may be None
#   self.baseInterfaces is the list of base interfaces for a virtual interfaces
...
#            self.constRefClass = n + 'ConstRef'
        ...
        if prefix:
            ...
            if self.constRefClass:
                self.constRefClass = prefix + self.constRefClass  
    ...
    def declstr(self, mode=Entity.DEFAULT):
        ...
        if mode != Entity.DEFAULT:
            return s
        if self.bases:
            s += ' : ' + ', '.join([a + ' ' + b for b, a in self.bases])
        if self.interface:
