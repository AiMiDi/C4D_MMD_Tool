...
系统 SHALL 在 VMD 动画导入时，将 `VMDFile::m_iks` 中的 IK 启用关键帧转换为 C4D 参数关键帧（CTrack/CKey），设置到对应的 IK 复选框上，并禁用 libMMD 的 VMD IK 启用控制。仅当某个 IK 存在禁用状态的关键帧时才创建 CTrack（默认全部启用时不创建多余的动画轨道）。
...
#### Scenario: VMD IK 全部启用时不创建 CTrack
- **WHEN** 用户导入一个 VMD 文件，其中某 IK 的所有关键帧均为启用
- **THEN** 该 IK 复选框 SHALL 不创建 CTrack，保持默认启用状态
