...
template <typename HASHVALUE, HASHMAP_ENTRY_LAYOUT L> struct DefaultHashMapEntryHandlerBase
{
	...
	template <typename ENTRY, typename KEY, Bool COPY_KEY = TestForCopyFromMember<typename SFINAEHelper<typename ENTRY::KeyType, KEY>::type>::isSupported && STD_IS_REPLACEMENT(reference, KEY)> struct KeyConstructor
	{
	...
	template <typename ENTRY, typename KEY, typename VALUE, Bool COPY_KEY = TestForCopyFromMember<typename SFINAEHelper<typename ENTRY::KeyType, KEY>::type>::isSupported && STD_IS_REPLACEMENT(reference, KEY), Bool COPY_VALUE = TestForCopyFromMember<typename SFINAEHelper<typename ENTRY::ValueType, VALUE>::type>::isSupported && STD_IS_REPLACEMENT(reference, VALUE)> struct KeyValueConstructor
	{
...
struct HashMapKeyValuePair : public DefaultHashMapEntryHandlerBase<HashInt, HASHMAP_ENTRY_LAYOUT::HASH_KEY_VALUE>
{
};
...
struct HashMapValueKeyPair : public DefaultHashMapEntryHandlerBase<HashInt, HASHMAP_ENTRY_LAYOUT::HASH_VALUE_KEY>
{
};
...
struct HashMapKeyHashValuePair : public DefaultHashMapEntryHandlerBase<HashInt, HASHMAP_ENTRY_LAYOUT::KEY_HASH_VALUE>
{
};

/// Base class for HashMap::Entry.
template <typename K, typename V, typename ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT LAYOUT = ENTRY_HANDLER::LAYOUT> class HashMapEntryBase
{
public:
	...
	explicit HashMapEntryBase(HashValueType hash) : _hashCode(hash), _value()
	{
	}

	template <typename KEY> HashMapEntryBase(HashValueType hash, const KEY&) : _hashCode(hash), _value()
	{
	}

	template <typename KEY, typename A> HashMapEntryBase(HashValueType hash, const KEY&, A&& value) : _hashCode(hash), _value(std::forward<A>(value))
	{
	}

protected:
	MAXON_DISALLOW_COPY_AND_ASSIGN(HashMapEntryBase);
	HashMapEntryBase* _next = nullptr;
	HashValueType _hashCode;
	V _value;
};

template <typename K, typename V, typename ENTRY_HANDLER> class HashMapEntryBase<K, V, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::HASH_KEY_VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	HashMapEntryBase(HashValueType hash, const K& key) : _hashCode(hash), _key(key), _value()
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _hashCode(hash), _key(std::move(key)), _value()
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _hashCode(hash), _key(key), _value(std::forward<A>(value))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _hashCode(hash), _key(std::move(key)), _value(std::forward<A>(value))
	{
	}
...
template <typename K, typename V, typename ENTRY_HANDLER> class HashMapEntryBase<K, V, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::KEY_VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	decltype(ENTRY_HANDLER::GetKeyHashCode(std::declval<K>())) GetKeyHashCode() const
	{
		return ENTRY_HANDLER::GetKeyHashCode(_key);
	}
	...
	explicit HashMapEntryBase(HashValueType hash) : _value()
	{
	}

	HashMapEntryBase(HashValueType hash, const K& key) : _key(key), _value()
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _key(std::move(key)), _value()
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _key(key), _value(std::forward<A>(value))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _key(std::move(key)), _value(std::forward<A>(value))
	{
	}
...
template <typename K, typename V, typename ENTRY_HANDLER> class HashMapEntryBase<K, V, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	{
		return ENTRY_HANDLER::GetKeyHashCode(_value);
	}

	Int GetMemorySize() const
	{
		return SIZEOF(HashMapEntryBase) - SIZEOF(K) - SIZEOF(V) + maxon::GetMemorySizeHelper(_value);
	}
	...
	HashMapEntryBase(HashValueType hash, const K& key) : _value()
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _value()
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _value(std::forward<A>(value))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _value(std::forward<A>(value))
	{
	}
...
template <typename K, typename V, typename ENTRY_HANDLER> class HashMapEntryBase<K, V, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::KEY_HASH_VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	HashMapEntryBase(HashValueType hash, const K& key) : _key(key), _hashCode(hash), _value()
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _key(std::move(key)), _hashCode(hash), _value()
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _key(key), _hashCode(hash), _value(std::forward<A>(value))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _key(std::move(key)), _hashCode(hash), _value(std::forward<A>(value))
	{
	}
...
protected:
	...
	friend class SystemImpl; // for check in PrivateGetInternedId
};

template <typename K, typename V, typename ENTRY_HANDLER> class HashMapEntryBase<K, V, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::HASH_VALUE_KEY>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	HashMapEntryBase(HashValueType hash, const K& key) : _hashCode(hash), _value(), _key(key)
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _hashCode(hash), _value(), _key(std::move(key))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _hashCode(hash), _value(std::forward<A>(value)), _key(key)
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _hashCode(hash), _value(std::forward<A>(value)), _key(std::move(key))
	{
	}
...
template <typename K, typename ENTRY_HANDLER> class HashMapEntryBase<K, UnitType, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::HASH_KEY_VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	explicit HashMapEntryBase(HashValueType hash) : _hashCode(hash)
	{
	}

	HashMapEntryBase(HashValueType hash, const K& key) : _hashCode(hash), _key(key)
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _hashCode(hash), _key(std::move(key))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _hashCode(hash), _key(key)
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _hashCode(hash), _key(std::move(key))
	{
	}
...
template <typename K, typename ENTRY_HANDLER> class HashMapEntryBase<K, UnitType, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::KEY_HASH_VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	HashMapEntryBase(HashValueType hash, const K& key) : _key(key), _hashCode(hash)
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _key(std::move(key)), _hashCode(hash)
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _key(key), _hashCode(hash)
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _key(std::move(key)), _hashCode(hash)
	{
	}
...
template <typename K, typename ENTRY_HANDLER> class HashMapEntryBase<K, UnitType, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::HASH_VALUE_KEY>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
...
template <typename K, typename ENTRY_HANDLER> class HashMapEntryBase<K, UnitType, ENTRY_HANDLER, HASHMAP_ENTRY_LAYOUT::KEY_VALUE>
{
public:
	using EntryHandler = ENTRY_HANDLER;
	using HashValueType = typename ENTRY_HANDLER::HashValueType;
	...
	explicit HashMapEntryBase(HashValueType hash)
	{
	}

	HashMapEntryBase(HashValueType hash, const K& key) : _key(key)
	{
	}

	HashMapEntryBase(HashValueType hash, K&& key) : _key(std::move(key))
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, const K& key, A&& value) : _key(key)
	{
	}

	template <typename A> HashMapEntryBase(HashValueType hash, K&& key, A&& value) : _key(std::move(key))
	{
	}
...
/// @tparam ENTRY_ALLOCATOR				An optional memory allocator for the entries only.
/// @see @$ref maps
//----------------------------------------------------------------------------------------
template <typename K, typename V, typename HASH = DefaultCompare, typename ENTRY_HANDLER = HashMapKeyValuePair, typename ALLOCATOR = DefaultAllocator, HASHMAP_MODE MODE = HASHMAP_MODE::DEFAULT, Int INITIAL_CAPACITY = 16, Int LOAD_FACTOR = (MODE == HASHMAP_MODE::SYNCHRONIZED) ? 0 : 10, typename ENTRY_ALLOCATOR = ALLOCATOR> class HashMap
	: public MapBase<HashMap<K, V, HASH, ENTRY_HANDLER, ALLOCATOR, MODE, INITIAL_CAPACITY, LOAD_FACTOR, ENTRY_ALLOCATOR>, K, V, EmptyClass, HASH>
{
public:
	...
	//----------------------------------------------------------------------------------------
	HashMap() : _table(nullptr), _tableLengthM1(-1), _nonemptyBuckets(nullptr), _nonemptyBucketCount(0), _size(0), _resizeThreshold(0)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	explicit HashMap(const ALLOCATOR& alloc) : _allocator(alloc), _table(nullptr), _tableLengthM1(-1), _nonemptyBuckets(nullptr), _nonemptyBucketCount(0),
		_size(0), _resizeThreshold(0)
	{
	}
	...
	~HashMap()
	{
		if (MODE == HASHMAP_MODE::SYNCHRONIZED && _size > 0)
		{
			if (System::_interface.IsInitialized())
				System::PrivateShowSynchronizedHashMapDiagnostics(_size, _tableLengthM1, GetSignature((typename std::conditional<MODE == HASHMAP_MODE::SYNCHRONIZED, Char, void>::type*) nullptr));
		}
		ResetImpl(true);
	}

	HashMap(HashMap&& src) : MAXON_MOVE_MEMBERS(_allocator, _table, _tableLengthM1, _nonemptyBuckets, _nonemptyBucketCount, _size, _resizeThreshold)
	{
		src._table = nullptr;
	...
	ResultMem SetCapacityHint(Int capacity, COLLECTION_RESIZE_FLAGS resizeFlags = COLLECTION_RESIZE_FLAGS::ON_GROW_RESERVE_CAPACITY)
	{
		if (LOAD_FACTOR > 0)
		{
			capacity = ((capacity + 1) * 16 / Max<Int>(LOAD_FACTOR, 1)) + 2;
		}
		Int c = 1;
		while (c < capacity)
		{
	...
	/// of entries will be added, this will eliminate some intermediate resizing. For a
	...
	Int GetMemorySize() const
	{
		static_assert(MODE == HASHMAP_MODE::DEFAULT, "GetMemorySize is only supported for a default HashMap.");
	...
	{
		DebugAssert(MODE == HASHMAP_MODE::DEFAULT, "GetNonEmptyBucketCount is only supported for a default HashMap.");
	...
	{
		DebugAssert(MODE == HASHMAP_MODE::DEFAULT, "GetNonEmptyBucket is only supported for a default HashMap.");
	...
	//----------------------------------------------------------------------------------------
	template <typename KEYHASH = HASH, typename KEY> Opt<V&> FindValue(const KEY& key)
	{
		Entry* e = FindEntryImpl<KEY, KEYHASH>(KEYHASH::GetHashCode(key), key);
		return e ? std::addressof(e->GetValue()) : nullptr;
	}
	...
	/// @return												Pointer to value for the given key, or nullptr if no entry exists for the key.
	//----------------------------------------------------------------------------------------
	template <typename KEYHASH = HASH, typename KEY> Opt<const V&> FindValue(const KEY& key) const
	{
	...
	//----------------------------------------------------------------------------------------
	/// Finds the entry with the given key and (pre-computed) hash, or creates such an entry if it doesn't exist yet.
	...
	/// the size of a new entry for key and a function <tt>Result<void> C::ConstructHashMapEntry(void* ptr, HashInt hash, const K& key)</tt> which uses
	...
	///
	/// The type KEY of the given key need not be the same as K, but then you have to provide
	/// an additional class KEYHASH
	/// to compare a key of type KEY with a key of type K for equality (function KEYHASH::IsEqual(const KEY&, const K&))
	/// unless the default HASH class is already able to do so.
	/// @tparam KEYHASH								Hash class to compare key with the map's keys. Default is HASH.
	/// @tparam KEY										Type of key.
	...
	/// @param[in] hash								The pre-computed hash code of key. This has to be consistent with the used HASH class.
	...
	//----------------------------------------------------------------------------------------
	template <typename KEYHASH = HASH, typename KEY, typename C> Result<Entry*> InsertCtorWithHash(KEY&& key, C&& constructor, HashValueType hash, Bool& created = BoolLValue())
	{
		DebugAssert(hash == KEYHASH::GetHashCode(key));
		if (_table == nullptr)
		{
			DebugAssert(MODE != HASHMAP_MODE::SYNCHRONIZED, "For a synchronized HashMap, you have to call SetCapacityHint at the beginning.");
			if (SetCapacityHint(INITIAL_CAPACITY) == FAILED)
			{
				created = false;
		...
		{
			if ((hash == e->GetKeyHashCode()) && KEYHASH::IsEqual(key, e->GetKey()))
			{
				created = false;
				return e;
			}
		...
		DebugAssert(STD_IS_REPLACEMENT(standard_layout, Entry) || (sz == SIZEOF(Entry)));
		e = _allocator.AllocEntry(sz);
		if (!e)
		{
			created = false;
			return CreateError(MAXON_SOURCE_LOCATION, ERROR_TYPE::OUT_OF_MEMORY);
	...
	/// @return												Entry for the given key, or nullptr if the entry didn't exist and allocation of a new entry failed.
	//----------------------------------------------------------------------------------------
	template <typename KEYHASH = HASH, typename KEY, typename C> MAXON_ATTRIBUTE_FORCE_INLINE Result<Entry*> InsertCtor(KEY&& key, C&& constructor, Bool& created = BoolLValue())
	{
		return InsertCtorWithHash(std::forward<KEY>(key), std::forward<C>(constructor), KEYHASH::GetHashCode(key), created);
	}


	/// @markInternal
	template <typename KEY, typename LAMBDA, Bool COPY_KEY = TestForCopyFromMember<typename SFINAEHelper<K, KEY>::type>::isSupported && STD_IS_REPLACEMENT(reference, KEY)> struct LambdaEntryConstructor
	{
	...
	/// @return												Entry for the given key, or nullptr if the entry didn't exist and allocation of a new entry failed.
	//----------------------------------------------------------------------------------------
	template <typename KEYHASH = HASH, typename KEY, typename LAMBDA> Result<Entry*> InsertLambda(KEY&& key, LAMBDA&& lambda, Bool& created = BoolLValue())
	{
	...
	template <typename KEYHASH = HASH, typename KEY> ResultRef<Entry> Insert(KEY&& key, const V& value, Bool& created = BoolLValue())
	{
		...
		if (e && !created && MODE != HASHMAP_MODE::SYNCHRONIZED)
		{
	...
	template <typename KEYHASH = HASH, typename KEY> ResultRef<Entry> InsertMultiEntry(KEY&& key)
	{
		...
		HashValueType hash = KEYHASH::GetHashCode(key);
		Entry* e = _allocator.AllocEntry();
		if (!e)
		{
			return nullptr;
		}
	...
	ResultOk<void> Erase(const Entry* entry, Bool deleteEntry = true)
	{
		...
		UInt h = entry->GetKeyHashCode() & (UInt) _tableLengthM1;
		Entry* prev = nullptr;
		for (Entry* e = LoadRelaxed(_table[h].list); e; e = static_cast<Entry*>(e->_next))
		{
			if (e == entry)
			{
				--_size;
				...
				{
					StoreRelaxed(_table[h].list, static_cast<Entry*>(e->_next));
	...
	{
		static_assert(MODE == HASHMAP_MODE::DEFAULT, "Intersect not supported for a synchronized HashMap.");
		if constexpr (MODE == HASHMAP_MODE::DEFAULT)
		{
			for (Int i = 0; i < _nonemptyBucketCount; ++i)
			{
				...
				{
					next = static_cast<Entry*>(e->_next);
	...
	Result<void> CopyFromImpl(HASHMAP&& other, COLLECTION_RESIZE_FLAGS resizeFlags, OverloadRank1)
	{
		...
		{
			DebugAssert(MODE != HASHMAP_MODE::SYNCHRONIZED, "For a synchronized HashMap, you have to call SetCapacityHint at the beginning.");
			SetCapacityHint(INITIAL_CAPACITY) iferr_return;
		}

		for (const auto& i : other)
		{
			HashValueType hash = i.GetKeyHashCode();
			Entry* e = _allocator.AllocEntry();
			if (!e)
			{
				return CreateError(MAXON_SOURCE_LOCATION, ERROR_TYPE::OUT_OF_MEMORY);
			...
			e = new (e) Entry(hash);
			if (ENTRY_HANDLER::LAYOUT != HASHMAP_ENTRY_LAYOUT::HASH_VALUE)
			{
				AssignCopy<K>(const_cast<K&>(e->GetKey()), i.GetKey()) iferr_return;
			}
	...
	{
		DebugAssert(!(MODE == HASHMAP_MODE::SYNCHRONIZED && overwrite));
	...
	{
	public:
		static_assert(MODE != HASHMAP_MODE::SYNCHRONIZED, "MultiEntryIterator not supported for a synchronized HashMap.");
	...
	protected:
		...
		{
			DebugAssert(MODE != HASHMAP_MODE::DEFAULT);
			_entry = nullptr;
		...
		void ValidateBucket()
		{
			if (_bucket < 0)
			{
				if (_entry)
				{
					UInt h = _entry->GetKeyHashCode() & (UInt) _map->_tableLengthM1;
					if constexpr (MODE == HASHMAP_MODE::DEFAULT)
					{
		...
		{
			EntryType* p = nullptr;
			EntryType* e = LoadAcquire((MODE != HASHMAP_MODE::DEFAULT ? _map->_table + _bucket : _map->_nonemptyBuckets[_bucket])->list);
			while (e != _entry)
			{
				DebugAssert(e);
				p = e;
		...
		EntryType* _entry;
		CollectionType* _map;
		Int _bucket; // a negative _bucket means that _bucket isn't known and has to be derived from _entry
		...
		if (e)
		{
			...
			if (prev)
			{
				...
				if (!next)
				{
					...
					{
						while ((bucket <= _tableLengthM1) && ((next = LoadAcquire(_table[bucket].list)) == nullptr))
						{
							++bucket;
						}
					}
			...
			{
				_table[e->GetKeyHashCode() & UInt(_tableLengthM1)].list = next;
				if (!next)
				{
					if constexpr (MODE == HASHMAP_MODE::DEFAULT)
					{
	...
	Iterator GetIterator(const Entry* e)
	{
		...
		return Iterator(*this, MODE != HASHMAP_MODE::DEFAULT ? h : _table[h].nonemptyBucketsIndex, const_cast<Entry*>(e));
	}
	...
	ConstIterator GetIterator(const Entry* e) const
	{
		if (!e)
			return ConstIterator(*this, 0, nullptr);
		UInt h = e->GetKeyHashCode() & (UInt) _tableLengthM1;
		return ConstIterator(*this, MODE != HASHMAP_MODE::DEFAULT ? h : _table[h].nonemptyBucketsIndex, e);
	}
	...
	public:
		static Bool IsEqual(const HashMap& a, const HashMap& b, VALUEHASH&& hash = VALUEHASH())
		{
			...
			Int n = MODE != HASHMAP_MODE::DEFAULT ? a._tableLengthM1 + 1 : a._nonemptyBucketCount;
			for (Int i = 0; i < n; ++i)
			{
				for (const Entry* e = LoadAcquire((MODE != HASHMAP_MODE::DEFAULT ? a._table + i : a._nonemptyBuckets[i])->list); e; e = static_cast<const Entry*>(e->GetNextInBucket()))
				{
					const Entry* eb = b.FindEntryImpl<K, HASH>(e->GetKeyHashCode(), e->GetKey());
		...
		static HashInt GetHashCode(const HashMap& m)
		{
			...
			if constexpr (MODE == HASHMAP_MODE::DEFAULT)
			{
				for (Int i = 0; i < m._nonemptyBucketCount; ++i)
				{
					...
					{
						hash.CombineUnordered(DefaultHasher::CombineHashes(e->GetKeyHashCode(), VALUEHASH::GetHashCode(e->GetValue())));
					}
				}
			}
			else
			{
				for (Int i = 0; i <= m._tableLengthM1; ++i)
				{
					for (const Entry* e = m._table[i].list; e; e = e->GetNextInBucket())
					{
						hash.CombineUnordered(DefaultHasher::CombineHashes(e->GetKeyHashCode(), VALUEHASH::GetHashCode(e->GetValue())));
					}
				}
		...
		static UniqueHash GetUniqueHashCode(const HashMap& m)
		{
			...
			if constexpr (MODE == HASHMAP_MODE::DEFAULT)
			{
				for (Int i = 0; i < m._nonemptyBucketCount; ++i)
				{
					for (const Entry* e = m._nonemptyBuckets[i]->list; e; e = e->GetNextInBucket())
					{
						hash.CombineUnordered(UniqueHasher::CombineHashes(HASH::GetUniqueHashCode(e->GetKey()), VALUEHASH::GetUniqueHashCode(e->GetValue())));
					}
				}
			}
			else
	...
	template <typename KEY, typename KEYHASH> const Entry* FindEntryImpl(HashValueType hash, const KEY& key) const
	{
		if (!_table)
		{
			return nullptr;
		}
		for (const Entry* e = LoadAcquire(_table[hash & (UInt) _tableLengthM1].list); e; e = static_cast<const Entry*>(e->_next))
		{
			if ((hash == e->GetKeyHashCode()) && KEYHASH::IsEqual(key, e->GetKey()))
			{
				return e;
	...
	template <typename KEY, typename KEYHASH> Entry* FindEntryImpl(HashValueType hash, const KEY& key)
	{
...
protected:
	union SimpleBucket
	{
		typename std::conditional<MODE == HASHMAP_MODE::SYNCHRONIZED, AtomicPtr<Entry>, Entry*>::type list;
	};
	...
	{
		constexpr Bool NONEMPTYBUCKETS = MODE == HASHMAP_MODE::DEFAULT;
		Bucket* t = (Bucket*) _allocator.AllocBuckets(SIZEOF(Bucket) * length + (NONEMPTYBUCKETS ? SIZEOF(Bucket*) * length : 0), MAXON_SOURCE_LOCATION_NAME(Bucket));
		if (!t)
		{
			return false;
		}
		if constexpr (NONEMPTYBUCKETS)
		{
			_nonemptyBuckets = (Bucket**) (t + length);
			_nonemptyBucketCount = 0;
		}
		...
		for (Int j = 0; j <= _tableLengthM1; j++)
		{
			...
			while (entry)
			{
				Entry* n = static_cast<Entry*>(entry->_next);
				UInt h = entry->GetKeyHashCode() & (UInt) tlM1;
				...
				if constexpr (NONEMPTYBUCKETS)
				{
					if (entry->_next == nullptr)
					{
						...
						_nonemptyBuckets[_nonemptyBucketCount++] = t + h;
					}
				}
	...
	void ResetImpl(Bool destructor)
	{
		...
		{
			_size = LIMIT<Int>::MAX;
			_nonemptyBucketCount = LIMIT<Int>::MAX;
			_table = (Bucket*) 0xdead;
			_tableLengthM1 = LIMIT<Int>::MAX;
			_nonemptyBuckets = (Bucket**) 0xdead;
		}
#endif
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename GENERAL_ALLOCATOR, typename BLOCK_ALLOCATOR> class HashMapAllocator
	{
		GENERAL_ALLOCATOR _bucketAllocator;
		BLOCK_ALLOCATOR _entryAllocator;

	public:
		void* AllocBuckets(Int size, MAXON_SOURCE_LOCATION_DECLARATION) { return _bucketAllocator.Alloc(size, MAXON_SOURCE_LOCATION_FORWARD); }
		void FreeBuckets(void* buckets) { _bucketAllocator.Free(buckets); }
		Entry* AllocEntry(Int size = SIZEOF(Entry)) { return (Entry*) _entryAllocator.Alloc(size, MAXON_SOURCE_LOCATION_NAME(Entry)); }
		void FreeEntry(const Entry* entry) { _entryAllocator.Free(entry); }
		Bool DeleteAllEntries()
		{
			if constexpr (IsArrayAllocator<BLOCK_ALLOCATOR>::value)
			{
				return _entryAllocator.template DeleteAll<Entry>();
			}
			else
			{
				return false;
	...
	//----------------------------------------------------------------------------------------
	/// Typical case of general and entry allocator being the same (keeps the memory layout compatible).
	//----------------------------------------------------------------------------------------
	template <typename ONE_ALLOCATOR> class HashMapAllocator<ONE_ALLOCATOR, ONE_ALLOCATOR>
	{
		ONE_ALLOCATOR _allocator;

	public:
		void* AllocBuckets(Int size, MAXON_SOURCE_LOCATION_DECLARATION) { return _allocator.Alloc(size, MAXON_SOURCE_LOCATION_FORWARD); }
		void FreeBuckets(void* buckets) { _allocator.Free(buckets); }
		Entry* AllocEntry(Int size = SIZEOF(Entry)) { return (Entry*) _allocator.Alloc(size, MAXON_SOURCE_LOCATION_NAME(Entry)); }
		void FreeEntry(const Entry* entry) { _allocator.Free(entry); }
		Bool DeleteAllEntries() { return false; }
	};

	HashMapAllocator<ALLOCATOR, ENTRY_ALLOCATOR >	_allocator; ///< The allocator to use.
...
template <typename HASH = DefaultCompare, typename ENTRY_HANDLER = HashMapKeyValuePair, typename ALLOCATOR = DefaultAllocator, HASHMAP_MODE MODE = HASHMAP_MODE::DEFAULT, Int INITIAL_CAPACITY = 16, Int LOAD_FACTOR = (MODE == HASHMAP_MODE::SYNCHRONIZED) ? 0 : 10, typename ENTRY_ALLOCATOR = ALLOCATOR> class HashMapSelector
{
public:
	template <typename K, typename V> using Type = HashMap<K, V, HASH, ENTRY_HANDLER, ALLOCATOR, MODE, INITIAL_CAPACITY, LOAD_FACTOR, ENTRY_ALLOCATOR>;
};
...
/// @see @$ref sets
//----------------------------------------------------------------------------------------
template <typename T, typename HASH = DefaultCompare, typename ENTRY_HANDLER = HashMapKeyValuePair, typename ALLOCATOR = DefaultAllocator, HASHMAP_MODE MODE = HASHMAP_MODE::DEFAULT, Int INITIAL_CAPACITY = 16, Int LOAD_FACTOR = (MODE == HASHMAP_MODE::SYNCHRONIZED) ? 0 : 10, typename ENTRY_ALLOCATOR = ALLOCATOR> class HashSet
	: public SetBase<HashSet<T, HASH, ENTRY_HANDLER, ALLOCATOR, MODE, INITIAL_CAPACITY, LOAD_FACTOR, ENTRY_ALLOCATOR>, T, Protected<HashMap<T, UnitType, HASH, ENTRY_HANDLER, ALLOCATOR, MODE, INITIAL_CAPACITY, LOAD_FACTOR, ENTRY_ALLOCATOR>>, HASH>
{
public:
...
public:
	...
	/// @param[in] alloc							Used to initialize the underlying allocator by its copy constructor.
	//----------------------------------------------------------------------------------------
	explicit HashSet(const ALLOCATOR& alloc) : Super(alloc)
	{
	}
	...
	{
		static_assert(MODE == HASHMAP_MODE::DEFAULT, "GetHashCode only supported for a default HashSet.");
		DefaultHasher hash;
		for (Int i = 0; i < _nonemptyBucketCount; ++i)
		{
			for (const typename Super::Entry* e = _nonemptyBuckets[i]->list; e; e = e->GetNextInBucket())
			{
				hash.CombineUnordered(e->GetKeyHashCode());
			}
		}
		return hash;
	}
	...
	{
		static_assert(MODE == HASHMAP_MODE::DEFAULT, "GetUniqueHashCode only supported for a default HashSet.");
		UniqueHash hash;
		for (Int i = 0; i < _nonemptyBucketCount; ++i)
		{
			for (const typename Super::Entry* e = _nonemptyBuckets[i]->list; e; e = e->GetNextInBucket())
			{
				hash.CombineUnordered(HASH::GetUniqueHashCode(e->GetKey()));
			}
		}
		return hash;
	}
