...
struct JobInterfacePOD
{
	...
	mutable CoreJobState* _jobState;													///< Internal job data (can be nullptr when no job is running).
};
...
public:
	...
	explicit JobFunctor(const JobInterfacePOD& self)
	{
		...
		_self = reinterpret_cast<const JobInterface*>(reinterpret_cast<const Char*>(&self) - offset);
	}
	...
	{
		return *const_cast<JobInterfaceBase*>(reinterpret_cast<const JobInterfaceBase*>(_self));
	}
