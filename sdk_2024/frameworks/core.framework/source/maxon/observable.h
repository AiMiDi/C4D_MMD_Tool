...
#define PRIVATE_MAXON_OBSERVABLE_IMPL(NAME, ID, Interface) \
	...
	maxon::ObservableRef<typename Interface::MAXON_CONCAT(NAME, Delegate)> NAME(Bool create) const { return MAXON_CONCAT(_, NAME).Get(this->self, ID, create); }
...
#define OBSERVABLE_IMPL_NONVIRTUAL(NAME, OBJECT) \
	...
	ObservableRef<Interface::MAXON_CONCAT(NAME, Delegate)> NAME(Bool create) const { return MAXON_CONCAT(_, NAME).Get(OBJECT, MAXON_STRINGIFY(NAME), create); }
...
{
	MAXON_INTERFACE_NONVIRTUAL(ObservableStaticInterface, MAXON_REFERENCE_STATIC, "net.maxon.interface.observablestatic");
public:
...
public:
	...
	[[deprecated("not requiried anymore, remove Init()")]] Result<void> Init(ObserverObjectInterface* owner, const Id& observableName)
	{
		return OK;
	}
...
//----------------------------------------------------------------------------------------
	...
	#define MAXON_OBSERVABLE(RETTYPE, NAME, ...) \
		...
		MAXON_METHOD maxon::ObservableRef<MAXON_CONCAT(NAME, Delegate)> NAME(Bool create) const;

	#define MAXON_OBSERVABLE_STATIC(RETTYPE, NAME, ...) \
		class MAXON_CONCAT(NAME, Delegate); \
		static MAXON_METHOD maxon::ObservableRef<MAXON_CONCAT(NAME, Delegate)> NAME(Bool create);
#else
	// special defines for doxygen, this simulates a normal function
	#define MAXON_OBSERVABLE(RETTYPE, NAME, ARGUMENTLIST, COMBINER) ObservableRef<> NAME(Bool create) const -> RETTYPE (*) ARGUMENTLIST
	#define MAXON_OBSERVABLE_STATIC(RETTYPE, NAME, ARGUMENTLIST, COMBINER) ObservableRef<> NAME(Bool create) const -> RETTYPE (*) ARGUMENTLIST
#endif
