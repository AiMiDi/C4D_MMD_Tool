...
public:
	...
	//----------------------------------------------------------------------------------------
	template <typename ANY, typename = typename std::enable_if<BYTE_BLOCK, ANY>::type> Block(ANY* ptr, Int size) : Super(reinterpret_cast<T*>(ptr), size * Int(sizeof(typename std::conditional<std::is_void<ANY>::value, Char, ANY>::type)), 1)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename ANY, typename = typename std::enable_if<BYTE_BLOCK, ANY>::type> Block(const Block<ANY>& src) : Super(reinterpret_cast<T*>(src.GetFirst()), src.GetCount() * SIZEOF(ANY), 1)
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename ANY, Int N, typename = typename std::enable_if<BYTE_BLOCK, ANY>::type> MAXON_IMPLICIT Block(ANY (&array)[N]) : Super(reinterpret_cast<T*>(array), N * SIZEOF(ANY), 1) { }
	...
	Int GetBlock(Int index, typename std::conditional<CONSTBLOCK, DeleteReturnType01, Block<T, false>&>::type block) const { return static_cast<const Block*>(this)->GetBlock(index, reinterpret_cast<Block<const T, false>&>(block)); }

	Int GetBlock(Int index, typename std::conditional<CONSTBLOCK, DeleteReturnType02, StridedBlock<T>&>::type block) const { block.Set(GetFirst(), this->GetCount(), GetStride()); return 0; }
