...
{
	template <typename T> static Bool LessThanOrEqual(const T& a, const T& b)
	{
...
/// @tparam COMPARE								The static LessThan and LessThanOrEqual functions of COMPARE are used for value comparisons.
/// 															When KEEP_DUPLICATES is true, only LessThan needs to be present.
...
private:
	template <typename A> Result<void> AppendImpl(A&& value)
	{
		...
		for (auto it = this->End(); it != this->Begin(); )
		{
			...
			else if (MINIMAL ? Cmp::LessThanOrEqual(*it, value) : Cmp::LessThanOrEqual(value, *it))
			{
				// New value already covered by *it.
				DebugAssert(!max);
				return OK;
			}
		}
