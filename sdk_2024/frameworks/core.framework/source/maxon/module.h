...
private:
	...
	Int _abiVersion = MAXON_API_ABI_VERSION;

	// The version of the core framework used for this binary.
	Int _coreFrameworkVersion = MAXON_CORE_FRAMEWORK_VERSION;
...
public:
	...
	const typename D::Type& DefaultValueImpl() { return maxon::DefaultValue<const typename D::Type&>(); }

	static MAXON_ATTRIBUTE_FORCE_INLINE const typename D::Type& DefaultValue() { return reinterpret_cast<DeclarationUse<D, UNIT, UNIT_HASH, WEAK>&>(_instance).DefaultValueImpl(); }
};
...
public:
	...
	template <TranslationUnit& UNIT = g_translationUnit, UInt64 UNIT_HASH = g_translationUnitHash> const T& operator ()() const
	{
		...
		{
			return Initialize(std::conditional<C::DEPENDENCIES_ENABLED, DeclarationUse<Declaration, UNIT, UNIT_HASH, false>, Declaration>::type::DefaultValue());
		}
	}
	...
	template <TranslationUnit& UNIT = g_translationUnit, UInt64 UNIT_HASH = g_translationUnitHash> Result<const T&> WeakGet() const
	{
		...
		else
		{
			...
			DebugStop();
			return DeclarationUse<Declaration, UNIT, UNIT_HASH, true>::DefaultValue();
		}
	}
...
public:
	...
	template <typename T, typename REG> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), REG* dummy, TranslationUnit* unit, const Char* file,
		...
		: _super(type, std::move(iid), unit, file, flags),
			...
			_freeDefinition(&FreeRegistryEntry<T, REG>),
			_registry(REG::GetId())
	{
	}
...
#define PRIVATE_MAXON_DECLARATION(T, Name, id, storage, ...) \
	...
	storage Name##_PrivateHelper Name {}
...
template <UInt64 MODULE_ID_HASH = CStringCompare::GetHashCode(MAXON_MODULE_ID)> extern EntityDefinition g_privateReflectionRegistrationDependency;
...
#define MAXON_DEPENDENCY_ON_REFLECTION \
	static maxon::EntityUse s_ud_reflection(".reflection", &maxon::g_translationUnit, &maxon::g_privateReflectionRegistrationDependency<>)
