...
{
	// using ValueType = std::remove_reference_t<decltype(*(std::declval<C>().Begin()))>;
	// using type = typename std::conditional<STD_IS_REPLACEMENT(const, ValueType), typename GetConstIteratorType<C>::type, typename GetNonConstIteratorType<C>::type>::type;
...
public:
	...
	{
		using NonConstIteratorType = typename GetNonConstIteratorType<CollectionType>::type;
		_it = collection.Erase(*reinterpret_cast<NonConstIteratorType*>(&_it));
		_end = collection.End();
	}
...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE ~NestedForEachIterator()
	{
		if (_valid)
		{
			static_cast<I2*>(_innerIterator)->~I2();
		}
	}
	...
	MAXON_ATTRIBUTE_FORCE_INLINE typename std::remove_reference<ValueType>::type* operator ->() const
	{
		return static_cast<I2*>(_innerIterator)->operator ->();
	}
...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE typename std::remove_reference<ValueType>::type* operator ->() const
	{
		static_assert(STD_IS_REPLACEMENT(reference, ValueType), "Can't use -> where ValueType it not a reference type.");
		return &_map(*_iterator);
	}
...
public:
	...
	MAXON_ATTRIBUTE_FORCE_INLINE IndexForEachIterator& operator +=(Int n)
	{
		_iterator += n;
		_index += n;
		return *this;
