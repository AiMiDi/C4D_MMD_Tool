...
ResultMem GenericBaseArray::Erase(const DataType& elementType, Int index, Int count)
{
	_cnt -= count;
	Int sz = elementType->GetSize();
	elementType->MoveFrom(GetPointerSlow(elementType, index), sz, GetPointerSlow(elementType, index + count), sz, _cnt - index);
	elementType->Destruct(GetPointerSlow(elementType, _cnt), sz, count);
	return OK;
}
