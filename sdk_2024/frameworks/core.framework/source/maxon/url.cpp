...
HashInt UrlInterface::GetHashCode() const
{
	DefaultHasher hash(GetPath().GetHashCode());
	hash.Combine(GetScheme().GetHashCode());
	const UrlScheme& scheme = GetAuthority().GetScheme();
	
	if (scheme != UrlScheme())
		hash.Combine(GetAuthority().GetHashCode());
	
	// #compareurlproblem
	// TODO: (Tilo/Ole) think about comparing urls and hashcodes. take data into account.
	ifnoerr (Data query = GetData(ConstDataPtr(URLFLAGS::QUERY.Get())))
	{
		ifnoerr (String queryStr = query.Get<String>())
		{
			if (queryStr.IsPopulated())
				hash.Combine(queryStr.GetHashCode());
		}
	}
	return hash;
}

UniqueHash UrlInterface::GetUniqueHashCode() const
{
	UniqueHasher hash;
	hash.HashAndCombine(GetPath());
	hash.HashAndCombine(GetScheme());
	const UrlScheme& scheme = GetAuthority().GetScheme();

	if (scheme != UrlScheme())
	{
		hash.HashAndCombine(GetAuthority());
	}
...
static Result<void> DeleteDirectoryRecursively(const Url& dir, Bool force)
{
	...
	for (const auto& it : dir.GetBrowseIterator(GETBROWSEITERATORFLAGS::NOERRORONOPEN))
	{
		...
		arr.Append(browseiterator.GetCurrentPath(), isDirectory) iferr_return;
	}
...
Result<void> UrlInterface::IoCopyFile(const Url& destName, Bool overwrite, Bool removeReadOnly, const IoProgressDelegate& progress) const
{
	iferr (auto connection = OpenConnection())
		return err;

	return connection.IoCopyFileProgress(destName, overwrite, removeReadOnly, progress);
}
...
Result<void> UrlInterface::IoShowInOS(IOSHOWINOSFLAGS flags) const
{
	...
	for (const UrlIoShowInOSDelegate& callBack : UrlIoShowInOSRegistry::GetEntries())
	{
		Bool handled = callBack(this, flags) iferr_return;
		if (handled)
			return OK;
	}
