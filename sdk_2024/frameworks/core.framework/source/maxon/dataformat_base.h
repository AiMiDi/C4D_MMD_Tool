...
public:
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const WriterRef& DefaultValue()
	{
		return *static_cast<const WriterRef*>(&DataFormatBaseWriterRef::DefaultValue());
	}
};
...
public:
	...
	Result<void> Write(const String& data) const
	{
		iferr_scope;
		Utf32CharBuffer buf;
		data.GetUtf32(buf) iferr_return;
		Write(buf) iferr_return;
		return OK;
	}
...
public:
	...
	static MAXON_ATTRIBUTE_FORCE_INLINE const ReaderRef& DefaultValue()
	{
		return *static_cast<const ReaderRef*>(&DataFormatBaseReaderRef::DefaultValue());
	}
};
