	...
	Delegate<void(CLASSDATA* self)>&& timerWaitFn, Delegate<Result<void>(CLASSDATA* self, Error threadError)>&& finalUpdateFunc)
{
	...
	if (!g_exitObservable)
	{
		g_exitObservable = RunLoop::ObservableExitLoop(true).AddObserver(
			[]() -> Result<void>
			{
				FreeThreadedStuff();
				...
				if (g_exitObservable)
				{
					...
					RunLoop::ObservableExitLoop(false).RemoveObserver(g_exitObservable);
	...
	auto callback =
		[this, timerInterval, startWaitFn = std::move(startWaitFn), timerWaitFn = std::move(timerWaitFn)]() mutable -> Result<void>
		{
			...
			if (!_isDone)
			{
				...
				startWaitFn(static_cast<CLASSDATA*>(this)) iferr_return;
				...
				_spinningTimer = TimerRef::AddPeriodicTimer(timerInterval,
					...
					{
						timerWaitFn(static_cast<CLASSDATA*>(this));
					}, JobQueueRef::GetMainThreadQueue()) iferr_return;
			}
			return OK;
		};
...
template <typename CLASSDATA> Bool ThreadedUIUpdate<CLASSDATA>::PrivateCancel()
{
