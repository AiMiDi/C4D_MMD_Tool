...
public:
	...
	static inline void Free(const T* o)
	{
		...
		ForwardFree<STD_IS_REPLACEMENT(destructible, ConstructorType)>::FreeWithDestructor(reinterpret_cast<ConstructorType*>(MAXON_REMOVE_CONST(o)));
	}
...
#define NewObj(T, ...) (maxon::details::NewDelete<T>::IS_ALLOC_TYPE ? maxon::details::NewDelete<T>::Cast(maxon::details::NewDelete<T>::AllocType::Alloc(MAXON_SOURCE_LOCATION_NAME(T), ##__VA_ARGS__)) \
												:	maxon::ResultPtr<T>(reinterpret_cast<T*>(MAXON_SAFE_PLACEMENT_NEW(maxon::details::NewDelete<T>::AllocatorType::Alloc(maxon::details::NewDelete<T>::SIZE, MAXON_SOURCE_LOCATION_NAME(T))) typename maxon::details::NewDelete<T>::ConstructorType{__VA_ARGS__})))


/// Deprecated.
#define NewObjPtr(T, ...) static_cast<T*>(MAXON_SAFE_PLACEMENT_NEW(maxon::details::NewDelete<T>::AllocatorType::Alloc(maxon::details::NewDelete<T>::SIZE, MAXON_SOURCE_LOCATION_NAME(T))) typename maxon::details::NewDelete<T>::ConstructorType(__VA_ARGS__))
...
#define NewObjClear(T, ...) (maxon::details::NewDelete<T>::IS_ALLOC_TYPE ? maxon::details::NewDelete<T>::Cast(maxon::details::NewDelete<T>::AllocType::Alloc(MAXON_SOURCE_LOCATION_NAME(T), ##__VA_ARGS__)).GetValue() \
												:	static_cast<T*>(MAXON_SAFE_PLACEMENT_NEW(maxon::details::NewDelete<T>::AllocatorType::AllocClear(maxon::details::NewDelete<T>::SIZE, MAXON_SOURCE_LOCATION_NAME(T))) typename maxon::details::NewDelete<T>::ConstructorType(__VA_ARGS__)))
#endif
...
#define DeleteObj(obj) \
	do { \
		...
		if (*objectToDelete) \
		{ \
			using NewDeleteType = maxon::details::NewDelete<typename std::remove_pointer<typename std::remove_pointer<decltype(objectToDelete)>::type>::type>; \
			using CtorType = typename NewDeleteType::ConstructorType; \
			if (STD_IS_REPLACEMENT(destructible, CtorType)) \
			{ \
