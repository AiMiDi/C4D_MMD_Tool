...
static constexpr Float32 MINVALUE_FLOAT32 = -3.402823466e+38f;							///< the minimum value a Float32 can represent
static constexpr Float32 MAXVALUE_FLOAT32 =  3.402823466e+38f;							///< the maximum value a Float32 can represent

static constexpr Float64 MINVALUE_FLOAT64 = -1.7976931348623158e+308;				///< the minimum value a Float64 can represent
static constexpr Float64 MAXVALUE_FLOAT64 =  1.7976931348623158e+308;				///< the maximum value a Float64 can represent

static constexpr Float32 MINVALUE_INT32_FLOAT32 = -2147483520.0f;						///< minimum Float32 value that can be represented by Int32 (-0x7FFFFF80). Lower values will results in an overflow
static constexpr Float32 MAXVALUE_INT32_FLOAT32	=  2147483520.0f;						///< maximum Float32 value that can be represented by Int32 ( 0x7FFFFF80). Higher values will results in an overflow

static constexpr Float64 MINVALUE_INT64_FLOAT64 = -9223372036854775295.0;		///< minimum Float64 value that can be represented by Int64 (-0x7ffffffffffffdff). Lower values will results in an overflow
static constexpr Float64 MAXVALUE_INT64_FLOAT64 =  9223372036854775295.0;		///< maximum Float64 value that can be represented by Int64 ( 0x7ffffffffffffdff). Higher values will results in an overflow

static constexpr Float32 MINRANGE_FLOAT32	= -1.0e10f;												///< 'safe' minimum range for Float32. Guarantees that multiplication of two numbers doesn't produce an overflow
static constexpr Float32 MAXRANGE_FLOAT32	=  1.0e10f;												///< 'safe' maximum range for Float32. Guarantees that multiplication of two numbers doesn't produce an overflow

static constexpr Float64 MINRANGE_FLOAT64  = -1.0e100;											///< 'safe' minimum range for Float. Guarantees that multiplication of two numbers doesn't produce an overflow
static constexpr Float64 MAXRANGE_FLOAT64	=   1.0e100;											///< 'safe' maximum range for Float. Guarantees that multiplication of two numbers doesn't produce an overflow

#ifdef MAXON_TARGET_SINGLEPRECISION
static constexpr Float MINVALUE_FLOAT = MINVALUE_FLOAT32;										///< the minimum value a Float can represent
static constexpr Float MAXVALUE_FLOAT = MAXVALUE_FLOAT32;										///< the maximum value a Float can represent

static constexpr Float MINRANGE_FLOAT = MINRANGE_FLOAT32;										///< 'safe' minimum range for Float64. Guarantees that multiplication of two numbers doesn't produce an overflow
static constexpr Float MAXRANGE_FLOAT = MAXRANGE_FLOAT32;										///< 'safe' maximum range for Float64. Guarantees that multiplication of two numbers doesn't produce an overflow
#else
static constexpr Float MINVALUE_FLOAT = MINVALUE_FLOAT64;										///< the minimum value a Float can represent
static constexpr Float MAXVALUE_FLOAT = MAXVALUE_FLOAT64;										///< the maximum value a Float can represent

static constexpr Float MINRANGE_FLOAT = MINRANGE_FLOAT64;										///< 'safe' minimum range for Float64. Guarantees that multiplication of two numbers doesn't produce an overflow
static constexpr Float MAXRANGE_FLOAT = MAXRANGE_FLOAT64;										///< 'safe' maximum range for Float64. Guarantees that multiplication of two numbers doesn't produce an overflow
#endif
...
{
public:
	static constexpr Int64 MIN = -9223372036854775807LL - 1;
	static constexpr Int64 MAX = 9223372036854775807LL;
};
...
{
public:
	static constexpr Int32 MIN = -2147483647L - 1;
	static constexpr Int32 MAX = 2147483647L;
};
...
public:
	...
	static constexpr UInt32 MAX = 0xffffffffULL;
};

template <> class LIMIT<Int16>
{
public:
	static constexpr Int16 MIN = -32767 - 1;
...
{
public:
	static constexpr Char MIN = -127 - 1;
...
{
public:
	static constexpr UChar MIN = 0;
	static constexpr UChar MAX = 0xff;
};

template <> class LIMIT<Float32>
{
public:
	static constexpr Float32 MIN = -3.402823466e+38f;
	static constexpr Float32 MAX = 3.402823466e+38f;
};

template <> class LIMIT<Float64>
{
public:
	static constexpr Float64 MIN = -1.7976931348623158e+308;
	static constexpr Float64 MAX = 1.7976931348623158e+308;
};

#ifndef NOTOK
	static constexpr Int NOTOK = -1; ///< constant used for special cases
#endif

/// floating point constant: PI
static constexpr Float64 PI			   = 3.1415926535897932384626433832795;

/// floating point constant: 1.0 / PI
static constexpr Float64 PI_INV	   = 0.31830988618379067153776752674508;

/// floating point constant: 2.0 * PI
static constexpr Float64 PI2			 = 6.283185307179586476925286766559;

/// floating point constant: 1.0 / (2.0 * PI)
static constexpr Float64 PI2_INV	 = 0.15915494309189533576888376337251;

/// floating point constant: 0.5 * PI
static constexpr Float64 PI05		   = 1.5707963267948966192313216916398;

/// floating point constant: 1.0 / (0.5 * PI)
static constexpr Float64 PI05_INV	 = 0.6366197723675813430755350534900;

/// floating point constant: Sqrt(2.0)
static constexpr Float64 SQRT2		 = 1.4142135623730950488016887242097;

/// floating point constant: Sqrt(2.0 * PI)
static constexpr Float64 SQRT_PI2	 = 2.506628274631000502415765284811;

/// floating point constant: 1.0 / Sqrt(2.0)
static constexpr Float64 SQRT2_INV = 0.7071067811865475244008443621049;

/// floating point constant: Sqrt(3.0)
static constexpr Float64 SQRT3		 = 1.7320508075688772935274463415059;

/// floating point constant: Log(2.0)
static constexpr Float64 LOG2			 = 0.69314718055994530941723212145818;
...
constexpr MAXON_ATTRIBUTE_FORCE_INLINE Float32 Inverse(Float32 f) { return MAXON_UNLIKELY(f == 0.0f) ? 0.0f : 1.0f / f; }

/// Calculates the reciprocal value (multiplicative inverse). If the input value is zero, zero will be returned for safety to avoid exceptions.
constexpr MAXON_ATTRIBUTE_FORCE_INLINE Float64 Inverse(Float64 f) { return MAXON_UNLIKELY(f == 0.0) ? 0.0 : 1.0 / f; }

#ifndef SIZEOF
	/// Calculates the size of a datatype or element.
	#define SIZEOF(...) (maxon::Int(sizeof(__VA_ARGS__)))
#endif
...
{
	using BlendCastType = typename std::conditional<STD_IS_REPLACEMENT(integral, X), Y, X>::type;
	return value1 + X(BlendCastType(value2 - value1) * blendValue);
}
...
constexpr MAXON_ATTRIBUTE_FORCE_INLINE Float32 DegToRad(Float32 r)   { return r * Float32(PI / 180.0); }

/// Converts float value from degrees to radians
constexpr MAXON_ATTRIBUTE_FORCE_INLINE Float64 DegToRad(Float64 r)   { return r * (PI / 180.0); }

/// Converts float value from radians to degrees
constexpr MAXON_ATTRIBUTE_FORCE_INLINE Float32 RadToDeg(Float32 r)   { return r * Float32(180.0 / PI); }

/// Converts float value from radians to degrees
constexpr MAXON_ATTRIBUTE_FORCE_INLINE Float64 RadToDeg(Float64 r)   { return r * (180.0 / PI); }
...
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE_FORCE_INLINE constexpr Bool IsPowerOfTwo(UInt32 x)
{
	return (x != 0) && !(x & (x - 1));
}
...
/// @return												True if @p x is a power of two.
//----------------------------------------------------------------------------------------
MAXON_ATTRIBUTE_FORCE_INLINE constexpr Bool IsPowerOfTwo(UInt64 x)
{
	return (x != 0) && !(x & (x - 1));
}
...
{
	static constexpr Int value = X + Sum<Y...>::value;
};
...
template <typename T, T X, T... Y> struct Or<T, X, Y...>
{
	...
	static constexpr T value = T(IntType(X) | IntType(Or<T, Y...>::value));
};
