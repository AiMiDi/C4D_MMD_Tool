...
namespace maxon
{
	...
	public:
		...
		static MemoryMetadataProxy GetMetadata(const void* target, const Id* uniqueId) { if (maxon::NonvirtualInterfaceReference::PrivateResolve(&_interface, true)) return WeakRefServices::GetMetadata(target, uniqueId); return HXXRET3(UNRESOLVED, MemoryMetadataProxy);}
		static Result<Bool> BrowseAllWeakReferences(const void* oldTarget, const ValueReceiver<WeakRefBase*>& receiver) { if (maxon::NonvirtualInterfaceReference::PrivateResolve(&_interface, false)) return WeakRefServices::BrowseAllWeakReferences(oldTarget, receiver); return HXXRET1(UNRESOLVED);}
	};
	MAXON_WARNING_POP
#ifdef PRIVATE_MAXON_ASSEMBLE_UNRESOLVED
	const maxon::UNRESOLVED_RETURN_TYPE WeakRefServices::MTable::_returnTypes[] = 
	{
		...
		maxon::details::UnresolvedReturnType<Bool>::value,
		maxon::details::UnresolvedReturnType<Bool>::value,
		maxon::details::UnresolvedReturnType<MemoryMetadataProxy>::value,
	...
	maxon::Bool WeakRefServices::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
	{
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if constexpr (maxon::details::UnresolvedReturnType<MemoryMetadataProxy>::value == maxon::UNRESOLVED_RETURN_TYPE::OTHER) tbl->WeakRefServices_GetMetadata = PRIVATE_MAXON_MF_CAST(decltype(WeakRefServices_GetMetadata), &Hxx2::Wrapper<Hxx2::Unresolved>::WeakRefServices_GetMetadata);
		#else
		...
		#endif
		#if defined(PRIVATE_MAXON_ASSEMBLE_UNRESOLVED) && !defined(MAXON_TARGET_DEBUG)
		if constexpr (maxon::details::UnresolvedReturnType<Result<Bool>>::value == maxon::UNRESOLVED_RETURN_TYPE::OTHER) tbl->WeakRefServices_BrowseAllWeakReferences = PRIVATE_MAXON_MF_CAST(decltype(WeakRefServices_BrowseAllWeakReferences), &Hxx2::Wrapper<Hxx2::Unresolved>::WeakRefServices_BrowseAllWeakReferences);
		#else
	...
	MAXON_INTERFACE_REGISTER(WeakRefServices, "net.maxon.interface.weakrefservices", nullptr, maxon::EntityBase::FLAGS::NONE);
	template <typename DUMMY> maxon::Int WeakRefServices::PrivateInstantiateDefaultValueHelper()
	{
		return 0
		| maxon::details::InstantiateDefaultReturnValue<MemoryMetadataProxy>(OVERLOAD_MAX_RANK)
		;
	}
#ifdef WEAKREF_H__
...
static maxon::details::ForceEvaluation s_forceEval_weakrefservices(0
	| maxon::WeakRefServices::PrivateInstantiateDefaultValueHelper<maxon::Int>()
);
