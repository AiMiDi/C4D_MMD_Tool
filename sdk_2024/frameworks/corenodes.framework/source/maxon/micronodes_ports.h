...
#define MAXON_PORT_INPUT_LAMBDA(T, name) PRIVATE_MAXON_INPUT_PORT(T, name, maxon::corenodes::MicroNode::FLAGS::WRAPPER_LAMBDA, false, -1)
...
//----------------------------------------------------------------------------------------
#define MAXON_PORT_INPUT_INDEXED(T, name, N) PRIVATE_MAXON_INPUT_PORT(T, name, maxon::corenodes::MicroNode::FLAGS::NONE, true, N)


//----------------------------------------------------------------------------------------
/// Defines a named input port with an id that can differ from the name.
/// @see MAXON_INPUT_PORT, split name parameter into name + portId.
/// portId supports any constexpr type, hardcoded strings, attribute, LiteralId.
//----------------------------------------------------------------------------------------
#define MAXON_PORT_INPUT_NAMED(T, name, portId) PRIVATE_MAXON_INPUT_PORT_NAMED(T, name, portId, maxon::corenodes::MicroNode::FLAGS::NONE, false, -1)
...
/// @param[in] N									The number of ports.
//----------------------------------------------------------------------------------------
#define MAXON_PORT_OUTPUT_INDEXED(T, name, N) PRIVATE_MAXON_OUTPUT_PORT(T, name, VariableMicroNode, maxon::corenodes::MicroNode::FLAGS::NONE, true, N)
...
/// portId supports any constexpr type, hardcoded strings, attribute, LiteralId.
//----------------------------------------------------------------------------------------
#define MAXON_PORT_OUTPUT_NAMED(T, name, portId) PRIVATE_MAXON_OUTPUT_PORT_NAMED(T, name, portId, VariableMicroNode, maxon::corenodes::MicroNode::FLAGS::NONE, false, -1)
...
protected:
	...
	DirectPortAccess(const DirectPortAccess& src)
	{
		...
		MicroNodeGroupInterface::PrivateDescribePortAccess(src._description, PORT::PrivateGetIndex(), PORT::PortClass::GetSourceCodePortHash(), STD_IS_REPLACEMENT(reference, typename PORT::Type));
	}
...
public:
	...
	Bool IsNeeded(Int portIndex = 0) const
	{
		return !(CORENODE_PORT_FLAGS(GetOwner(portIndex).GetBaseData().GetMember(ACCESS_INDEX(Int(PORT::PrivateGetIndex()) + portIndex)).simd.GetFlags()) & CORENODE_PORT_FLAGS::OUTPUT_UNUSED);
	}

	Bool HasValue(Int portIndex = 0) const
	{
		return !(CORENODE_PORT_FLAGS(GetOwner(portIndex).GetBaseData().GetMember(ACCESS_INDEX(Int(PORT::PrivateGetIndex()) + portIndex)).simd.GetFlags()) & CORENODE_PORT_FLAGS::INPUT_WITHOUT_VALUE);
	}
...
public:
	...
	Element operator [](Int index) const
	{
		...
		return Element(STD_IS_REPLACEMENT(empty, OP) ? _access : _access + index, index);
	}
...
template <typename C, typename MICRO, Int HASH, MicroNode::FLAGS FLAGS, Int INDEXCOUNT> class PortDefBase
{
public:
	using PortClass = C;
	...
	{
		src._info->CreatePort(GetSourceCodePortHash(), C::GetId(), MICRO::MICRONODE_TYPE, FLAGS, GetPortType<typename C::PortDeclarationType>(), INDEXCOUNT);
	}

	static const MicroNode::FLAGS MICRONODE_FLAGS = FLAGS;
...
template <typename C, typename MICRO, Int HASH, MicroNode::FLAGS FLAGS, Int MEMBERS, typename DIMENSION, Int INDEXCOUNT> class PortDefBaseCtor : public PortDefBase<C, MICRO, HASH, FLAGS, INDEXCOUNT>
{
public:
	using Super = PortDefBaseCtor;
//	static_assert(STD_IS_REPLACEMENT(reference, T) || STD_IS_REPLACEMENT(const, typename maxon::GetReferencedType<T, const maxon::Int>::type), "The type of a port must not be a non-const reference.");
...
template <typename C, typename MICRO, Int HASH, MicroNode::FLAGS FLAGS, Int MEMBERS> class PortDefBaseCtor<C, MICRO, HASH, FLAGS, MEMBERS, void, -1> : public PortDefBase<C, MICRO, HASH, FLAGS, -1>
{
public:
	using Super = PortDefBaseCtor;
//	static_assert(STD_IS_REPLACEMENT(reference, T) || STD_IS_REPLACEMENT(const, typename maxon::GetReferencedType<T, const maxon::Int>::type), "The type of a port must not be a non-const reference.");
...
#define PRIVATE_MAXON_PORT(name, portId, ARRAY, CNT, ...) \
	static maxon::Id GetId() { return maxon::Id(portId); } \
...
template <typename T> using PortValueTypeHelper = typename std::conditional<STD_IS_REPLACEMENT(reference, T), typename std::decay<T>::type, typename std::add_const<T>::type>::type;

#define PRIVATE_MAXON_PORT_GET_NAME_HASHCODE(name) (maxon::Int(maxon::corenodes::GetPortHashCodeHelper(name)) & maxon::LIMIT<maxon::Int>::MAX)

#define PRIVATE_MAXON_INPUT_PORT_NAMED(T, name, portId, MICROFLAGS, ARRAY, CNT, ...) \
	struct name : public MAXON_MACROARG_TYPE(maxon::corenodes::details::PortDefBaseCtor<name, maxon::corenodes::VirtualInputPort, ~(PRIVATE_MAXON_PORT_GET_NAME_HASHCODE(portId)), \
		MICROFLAGS | maxon::ConditionalFlag(STD_IS_REPLACEMENT(reference, T), maxon::corenodes::MicroNode::FLAGS::STATE), MAXON_EXPAND(MAXON_VA_NARGS(__VA_ARGS__)), PRIVATE_MAXON_PORT_DIMENSION_##ARRAY(CNT), CNT>) \
	{ \
		...
		PRIVATE_MAXON_PORT(name, portId, ARRAY, CNT, __VA_ARGS__) \
	}; name PrivateRegister_##name = name(maxon::DONT_INITIALIZE)

#define PRIVATE_MAXON_INPUT_PORT(T, name, MICROFLAGS, ARRAY, CNT, ...) PRIVATE_MAXON_INPUT_PORT_NAMED(T, name, MAXON_STRINGIFY(name), MICROFLAGS, ARRAY, CNT, __VA_ARGS__)

#define PRIVATE_MAXON_STATE_PORT(T, name, MICROFLAGS, ARRAY, CNT, ...) \
	struct name : public MAXON_MACROARG_TYPE(maxon::corenodes::details::PortDefBaseCtor<name, maxon::corenodes::VariableMicroNode, PRIVATE_MAXON_PORT_GET_NAME_HASHCODE(MAXON_STRINGIFY(name)), \
		maxon::corenodes::MicroNode::FLAGS::STATE | MICROFLAGS, MAXON_EXPAND(MAXON_VA_NARGS(__VA_ARGS__)), PRIVATE_MAXON_PORT_DIMENSION_##ARRAY(CNT), CNT>) \
	{ \
		...
		PRIVATE_MAXON_PORT(name, MAXON_STRINGIFY(name), ARRAY, CNT, __VA_ARGS__) \
		struct PrivateInput \
		{ \
...
#define PRIVATE_MAXON_OUTPUT_PORT_NAMED(T, name, portId, mtype, MICROFLAGS, ARRAY, CNT, ...) \
	struct name : public MAXON_MACROARG_TYPE(maxon::corenodes::details::PortDefBaseCtor<name, maxon::corenodes::mtype, (PRIVATE_MAXON_PORT_GET_NAME_HASHCODE(portId)), \
		MICROFLAGS, MAXON_EXPAND(MAXON_VA_NARGS(__VA_ARGS__)), PRIVATE_MAXON_PORT_DIMENSION_##ARRAY(CNT), CNT>) \
	{ \
		using PortDeclarationType = const T; \
		using UnannotatedPortType = typename maxon::corenodes::UnannotatedType<T>::type; \
		using PortAccessType = typename std::add_lvalue_reference<UnannotatedPortType>::type; \
...
#define PRIVATE_MAXON_OUTPUT_PORT(T, name, mtype, MICROFLAGS, ARRAY, CNT, ...) PRIVATE_MAXON_OUTPUT_PORT_NAMED(T, name, MAXON_STRINGIFY(name), mtype, MICROFLAGS, ARRAY, CNT, ...)
...
/// @param[in] inId								MAXON_ATTRIBUTE id.
/// @return												The resulting hash code.
//----------------------------------------------------------------------------------------
template <typename PORTIDTYPE>
inline constexpr UInt GetPortHashCodeHelper(PORTIDTYPE const& inId)
{
	return inId.GetId().GetHashCode();
}

//----------------------------------------------------------------------------------------
/// Helper to extract a constexpr hash value from a variety of port name types.
/// @param[in] inId								Literal id string (usually constexpr).
/// @return												The resulting hash code.
//----------------------------------------------------------------------------------------
inline constexpr UInt GetPortHashCodeHelper(LiteralId const& inId)
{
	return inId.GetHashCode();
}

//----------------------------------------------------------------------------------------
/// Helper to extract a constexpr hash value from a variety of port name types.
/// @tparam LENGTH								String length (deduced).
/// @param[in] inId								Hardcoded id string.
/// @return												The resulting hash code.
//----------------------------------------------------------------------------------------
template <Int LENGTH>
inline constexpr UInt GetPortHashCodeHelper(const char (&inId)[LENGTH])
{
	return CStringCompare::GetHashCode(inId);
}
...
{
public:
	template <typename PORT> using MemberAccess = typename std::conditional<Bool(ACCESS_FLAGS & PORTSACCESS_FLAGS::DIRECT_ACCESS_TEST), corenodes::details::DirectPortAccess<PORT, PortsAccess>, corenodes::details::MappedPortAccess<PORT, PortsAccess>>::type;
	static const Int MEMBER_SIZE = (ACCESS_FLAGS & PORTSACCESS_FLAGS::DIRECT_ACCESS_TEST) ? SIZEOF(ViewIteratorMember) : SIZEOF(void*);
	...
	static const Bool ExpectConst = false;
	...
	public:
		using Ports = PortsAccess;
		static const Bool ExpectConst = true;
		...
		static void DescribePortAccess(PortsDescription* desc)
		{
			...
			PortsDescription* descArray[PTRSIZE];
	...
	public:
		...
		{
			static_assert(STD_IS_REPLACEMENT(reference, decltype(_access)), "Wrong constructor call.");
		}
		...
		void operator ++()
		{
			...
			{
				if (!STD_IS_REPLACEMENT(empty, OutputMembers))
				{
					const Int start = reinterpret_cast<ViewIteratorMember*>(static_cast<typename std::conditional<STD_IS_REPLACEMENT(empty, OutputMembers), Members, OutputMembers>::type*>(static_cast<Members*>(nullptr))) - (ViewIteratorMember*) nullptr;
	...
	static void CheckAssertions()
	{
		...
		static_assert((STD_IS_REPLACEMENT(empty, InputMembers) ? 0 : SIZEOF(InputMembers)) + (STD_IS_REPLACEMENT(empty, OutputMembers) ? 0 : SIZEOF(OutputMembers)) == SIZEOF(Members), "Size mismatch.");
