...
public:
	...
	/// @code
	/// Bool pure = node.GetValue(corenodes::CoreNodeInterface::Pure).GetValueOrDefault() iferr_return;
	/// @endcode
	/// @param[in] attr								The attribute whose value shall be obtained.
...
public:
	...
	{
		return PrivateCreatePort(LIMIT<Int>::MIN, name, type, flags, vtype, -1);
	}
...
template <Bool SINGLE_PARAM> struct CoreNodeFactoryFunctionHelper
{
	template <typename N> static Result<CoreNode> Create(FactoryInterface::ConstPtr factory, const DataDictionary& args)
	{
		...
		auto n = maxon::corenodes::details::CreateNode<N>(OVERLOAD_MAX_RANK, name, args) iferr_return;
...
template <> struct CoreNodeFactoryFunctionHelper<true>
{
	template <typename N> static Result<CoreNode> Create(FactoryInterface::ConstPtr factory, const DataDictionary&)
	{
		iferr_scope;
		Id name = factory.Get(CoreNodeInterface::NodeId, Id());
		auto n = maxon::corenodes::details::CreateNode<N>(OVERLOAD_MAX_RANK, name) iferr_return;
...
template <typename CORENODE> inline Result<CoreNodeFactory> CreateCoreNodeFactory(DataDictionary&& meta = DataDictionary())
{
	...
	{
		meta.Set(CoreNodeInterface::Type, GetPortType<ValueType>()) iferr_return;
	}
