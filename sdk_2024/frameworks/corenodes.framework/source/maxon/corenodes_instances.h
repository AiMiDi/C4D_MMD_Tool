...
{
	static_assert(!STD_IS_REPLACEMENT(reference, T), "Wrong template argument.");
...
public:
	...
	enum class TYPE
	{
		...
		FACTOR_HEAD_MASK = 0x2000,

		BLOCK_HEAD = FACTOR_HEAD_MASK + 1,
		CONDITIONAL_HEAD,
		RECURSION_HEAD,

		DEFINITE_FACTOR_HEAD_MASK = 0x4000,
		CONTAINER_LOOP_HEAD_MASK = 0x8000,
		CONTAINER_OUTPUT_HEAD_MASK = 0x10000,
		...
		DEFINITE_LOOP_HEAD = FACTOR_HEAD_MASK | DEFINITE_FACTOR_HEAD_MASK,

		CONTAINER_LOOP_HEAD = FACTOR_HEAD_MASK | CONTAINER_LOOP_HEAD_MASK,
		ARRAY_LOOP_HEAD = CONTAINER_LOOP_HEAD | DEFINITE_FACTOR_HEAD_MASK,

		CONTAINER_LOOP = FACTOR_MASK | CONTAINER_LOOP_MASK | DEFINITE_FACTOR_MASK,
		ARRAY_LOOP = CONTAINER_LOOP | INDEXABLE_FACTOR_MASK,

		CONTAINER_OUTPUT_HEAD = FACTOR_HEAD_MASK | CONTAINER_OUTPUT_HEAD_MASK | OUTPUT_PORT_MASK | VARIABLE_MASK,
...
//		ARRAY_OUTPUT_HEAD = CONTAINER_OUTPUT_HEAD | DEFINITE_FACTOR_HEAD_MASK,
	...
	enum class FLAGS : Int64
	{
		...
		CURRENT_ITV = 1 << 5,
		NEXT_ITV = 1 << 6,
		CONTAINER_MEMBER = 1 << 7,
		COMMUTATIVE = 1 << 8,
		STATE_MAPPER = 1 << 9,
		PARAMETER = 1 << 10,								///< Set for ports added by ParameterBuilder::AddParameter, ports that don't have input/default values given by the graph (like component access ports).
		INPUT_WITHOUT_VALUE = 1 << 11,
		OUTER_DOMAIN = 1 << 12,
		SUSPEND = 1 << 13,
		INDEPENDENT = 1 << 14,
		INCLUDE_IN_PREDECESSORS = 1 << 15,
		FACTOR_REFERENCE = 1 << 16,
		OPTIONAL_INPUT = 1 << 17,
		CURRENT_ITV_GROUP = 1 << 18,
		NEXT_ITV_GROUP = 1 << 19,
		NEW_UNIQUE_NAME = 1 << 20,
		TIME_LOOP = 1 << 21,
		UNDERSCORE_NUMBER_PREFIX = 1 << 22,
		WRAPPER_DONT_WRAP = 1 << 23,
		WRAPPER_HIDDEN = 1 << 24,
		WRAPPER_KEEP_LETTER_CASE = 1 << 25,
		WRAPPER_LAMBDA = 1 << 26,
		WRAPPER_USE_ATTRIBUTE_RESOURCES = 1 << 27,
		WRAPPER_FIXED_VARIADIC = 1 << 28,
		WRAPPER_COPIED_VARIADIC = 1 << 29,
		WRAPPER_INNER_DOMAIN = 1 << 30,							///< Marker flag for inner-domain ports, used by the core node wrapper.
		WRAPPER_OUTER_DOMAIN = 1LL << 31,						///< Marker flag for outer-domain ports, used by the core node wrapper.
		WRAPPER_ENCLOSING_DOMAIN = 1LL << 32,				///< Marker flag for enclosing-domain ports, used by the core node wrapper.
		WRAPPER_STREAM = 1LL << 33,									///< Marker flag for stream ports, used by the core node wrapper.
		WRAPPER_CONTEXT = 1LL << 34,								///< Marker flag for context ports, used by the core node wrapper.
		NO_LIFETIME_PORT = 1LL << 35,
		NO_FACTOR_NODES = 1LL << 36,
		WRAPPER_FLAGS = WRAPPER_DONT_WRAP | WRAPPER_HIDDEN | WRAPPER_KEEP_LETTER_CASE | WRAPPER_LAMBDA | WRAPPER_USE_ATTRIBUTE_RESOURCES | WRAPPER_FIXED_VARIADIC | WRAPPER_COPIED_VARIADIC | WRAPPER_INNER_DOMAIN | WRAPPER_OUTER_DOMAIN | WRAPPER_ENCLOSING_DOMAIN | WRAPPER_STREAM | WRAPPER_CONTEXT,
		ITV_MASK = CURRENT_ITV | NEXT_ITV,
		ITV_GROUP_MASK = CURRENT_ITV_GROUP | NEXT_ITV_GROUP,
		PORT_MODIFIERS = PERMANENT | CACHED | ITV_MASK | OPTIONAL_INPUT | WRAPPER_FLAGS | NO_LIFETIME_PORT | NO_FACTOR_NODES,
		MODIFIERS = PORT_MODIFIERS | COMMUTATIVE | STATE_MAPPER | STATE_MODIFIER | OUTER_DOMAIN | SUSPEND | INDEPENDENT | FACTOR_REFERENCE | ITV_GROUP_MASK | TIME_LOOP,
...
public:
	...
	/// @markDeprecated Use GetCompilationData.
	[[deprecated]] MAXON_METHOD const Data& GetUserData() const;

	/// @markDeprecated Use SetCompilationData.
	[[deprecated]] MAXON_METHOD void SetUserData(Data&& value);
...
public:
	...
	{
		iferr_scope;
		// MSVC workaround: MSVC crashes if GetPortType<typename VARIADIC_ACCESS::ValueType>() is used as default argument for type, therefore use an empty type as default and replace it by the correct type here.
		typename VARIADIC_ACCESS::VPort ports = VARIADIC_ACCESS::VPort::Create(GetParentGroup(), prefix, arg, type ? type : GetPortType<typename VARIADIC_ACCESS::ValueType>(), flags) iferr_return;
	...
	template <typename PORT> MAXON_FUNCTION Result<typename PORT::PortIdType> Export(const PORT& parentPort, WIRETYPE wires = PORT::PortIdType::DEFAULT_WIRE, ACCESS_INDEX index = ACCESS_INDEX::UNUSED, MicroNode::FLAGS flags = MicroNode::FLAGS::NONE)
	{
		...
		ExportImpl(portId, wires, index, GetPortType<typename PORT::PortDeclarationType>(), flags) iferr_return;
...
public:
	...
	//----------------------------------------------------------------------------------------
	MAXON_FUNCTION Result<InPortId> Export(const InPortId& inport, const InPortId& parentPort = InPortId::DefaultValue(), WIRETYPE wires = WIRETYPE::DEFAULT_INPUT)
	{
	...
	//----------------------------------------------------------------------------------------
	MAXON_FUNCTION Result<OutPortId> Export(const OutPortId& outport, const OutPortId& parentPort = OutPortId::DefaultValue(), WIRETYPE wires = WIRETYPE::DEFAULT_OUTPUT)
	{
