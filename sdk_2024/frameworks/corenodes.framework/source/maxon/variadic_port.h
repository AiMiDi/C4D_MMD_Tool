...
public:
	...
	Result<void> Init(MicroNodeGroupInterface* node, const Block<const Member>& members, MicroNode::FLAGS flags = MicroNode::FLAGS::NONE)
	{
		...
		if (duplicateFound >= 0)
		{
			...
			errors.Append(ToTuple(corenodes::PortId(OUTPUT ? PORT_DIR::OUTPUT : PORT_DIR::INPUT, copyMembers[duplicateFound].name), memberError)) iferr_return;
		...
		{
			DataType portType = copyMembers[i].type;
