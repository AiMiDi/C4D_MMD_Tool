...
MAXON_FUNCTION typename std::enable_if<STD_IS_REPLACEMENT(reference, T) || STD_IS_REPLACEMENT(pointer, T), Result<T>>::type CPythonLibraryInterface::ParseArgument(const CPyRef& args, BlockArray<Data>& tempStack, Int32 argIndex, Bool isOptional)
{
	...
	{
		CPyErr_Clear();
		return (T)DefaultValueTypeHelper<const std::remove_reference_t<T>&>::Get();
	}
...
/// THREADSAFE.
#define Python3Output(formatString, ...) do { FORMATTEMPLATECHECK(formatString, ##__VA_ARGS__); _Python3Output(maxon::TARGETAUDIENCE::ALL, maxon::FormatTemplateX<MAXON_VA_NARGS(__VA_ARGS__)>(formatString, ##__VA_ARGS__), __LINE__, __FILE__); } while(false)
