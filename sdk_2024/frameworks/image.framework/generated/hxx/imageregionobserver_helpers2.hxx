...
struct ImageRegionTilesObserverInterface::MTable
{
	...
	template <typename S> using CombinedMTable = maxon::PrivateCombinedMTable<ImageRegionTilesObserverInterface, typename ImageRegionObserverInterface::MTable::template CombinedMTable<S>>;
...
{
	HXXRETURN0(Result<void>); HXXRES; HXXNONCONST(, , false, HXXRET1(NULLPTR)) HXXTABLE(ImageRegionTilesObserverInterface_Init); 
...
{
	HXXRETURN0(Result<void>); HXXRES; HXXNONCONST(, MakeWritable(true)., true, HXXRET2a(HXXRET1(MAKE_WRITABLE_FAILED))) HXXTABLE(ImageRegionTilesObserverInterface_Init); 
	return (PRIVATE_MAXON_MF_INVOKE((reinterpret_cast<maxon::GenericComponent*>(o_) + mt_.ImageRegionTilesObserverInterface_Init_Offset), mt_.ImageRegionTilesObserverInterface_Init, tileSize));
}
...
{
	HXXRETURN0(Result<void>); HXXRES; HXXNONCONST(, , false, HXXRET1(NULLPTR)) HXXTABLE(ImageRegionTilesObserverInterface_GetAndResetDirtyState); 
...
{
	HXXRETURN0(Result<void>); HXXRES; HXXNONCONST(, MakeWritable(true)., true, HXXRET2a(HXXRET1(MAKE_WRITABLE_FAILED))) HXXTABLE(ImageRegionTilesObserverInterface_GetAndResetDirtyState); 
	return (PRIVATE_MAXON_MF_INVOKE((reinterpret_cast<maxon::GenericComponent*>(o_) + mt_.ImageRegionTilesObserverInterface_GetAndResetDirtyState_Offset), mt_.ImageRegionTilesObserverInterface_GetAndResetDirtyState, dirtyState));
}
