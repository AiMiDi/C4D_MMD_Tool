...
namespace maxon
{
	...
	public:
		...
		Result<ForwardRef<SVGDocument>> GetSVGDocument() const 
		{
			...
			const MTable& mt_ = ((const typename ProxyComponent::PrivateSuperMTable*) proxy_info->_supervtable.GetPointer())->template GetMTable<VectorImageInterface>();
			Result<ForwardRef<SVGDocument>> proxy_result = HXXRET1(NULLIMPL);
			...
			(*handler_)(_fn_VectorImageInterface_GetSVGDocument, o_, maxon::reflection::ProxyInvocation(std::addressof(proxy_result), proxy_args, PRIVATE_MAXON_MF_REFLECTION_INVOCATION((Result<ForwardRef<SVGDocument>>)), reinterpret_cast<const void* const&>(mt_.VectorImageInterface_GetSVGDocument)));
			return proxy_result;
		}
