...
enum class PORT_FLAGS
{
	...
	USED_FOR_INPUTPORTTYPE_TEMPLATE_ARGUMENT = 4,		///< The port's type is used to deduce an argument for a TypeTemplateParameter of a downstream input port of a node template.
	...
	USED_FOR_DYNAMIC_TYPE = 4096,										///< There's a downstream port with dynamic type (see DynamicTypeConstraint) where this port's type may have an influence on.

	///< Any kind of template input flag mask.
	USED_FOR_TEMPLATE_MASK = USED_FOR_TEMPLATE_ARGUMENT | USED_FOR_INPUTPORTTYPE_TEMPLATE_ARGUMENT | USED_FOR_INPUTBUNDLE_TEMPLATE_ARGUMENT,
