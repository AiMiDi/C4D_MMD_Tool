...
#define SplineCall(b)					CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iSpline*>(this)->*lib->b)

#define SplineCallR(a, b)     CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iSpline*>(this)->*lib->b)

#define SplineCallRC(a, b)    CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iSpline*>(this)->*lib->b)

#define SplineGuiCall(b)     CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iSplineCustomGui*>(this)->*lib->b)

#define SplineGuiCallC(b)     CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const iSplineCustomGui*>(this)->*lib->b)

#define SplineGuiCallR(a, b) CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iSplineCustomGui*>(this)->*lib->b)

#define SplineGuiCallRC(a, b) CustomGuiSplineLib *lib = CheckCustomGuiSplineLib(LIBOFFSET(CustomGuiSplineLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iSplineCustomGui*>(this)->*lib->b)
