...
#define TreeViewGuiCall(b) 		TreeViewLib *lib = CheckListview(LIBOFFSET(TreeViewLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iTreeViewCustomGui*>(this)->*lib->b)

#define TreeViewGuiCallR(a, b) TreeViewLib *lib = CheckListview(LIBOFFSET(TreeViewLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iTreeViewCustomGui*>(this)->*lib->b)

#define TreeViewGuiCallRC(a, b) TreeViewLib *lib = CheckListview(LIBOFFSET(TreeViewLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iTreeViewCustomGui*>(this)->*lib->b)
