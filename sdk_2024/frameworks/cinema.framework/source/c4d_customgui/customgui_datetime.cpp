...
#define DateTimeCall(b) 			DateTimechooserLib *lib = CheckDateTimechooserLib(LIBOFFSET(DateTimechooserLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iDateTimeData*>(this)->*lib->b)

#define DateTimeCallR(a, b) 		DateTimechooserLib *lib = CheckDateTimechooserLib(LIBOFFSET(DateTimechooserLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iDateTimeData*>(this)->*lib->b)

#define DateTimeCallRC(a, b) 		DateTimechooserLib *lib = CheckDateTimechooserLib(LIBOFFSET(DateTimechooserLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iDateTimeData*>(this)->*lib->b)

#define DateTimeGuiCall(b) 		DateTimechooserLib *lib = CheckDateTimechooserLib(LIBOFFSET(DateTimechooserLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iDateTimeControl*>(this)->*lib->b)

#define DateTimeGuiCallR(a, b) DateTimechooserLib *lib = CheckDateTimechooserLib(LIBOFFSET(DateTimechooserLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iDateTimeControl*>(this)->*lib->b)

#define DateTimeGuiCallRC(a, b) DateTimechooserLib *lib = CheckDateTimechooserLib(LIBOFFSET(DateTimechooserLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iDateTimeControl*>(this)->*lib->b)
