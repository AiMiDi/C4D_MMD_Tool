...
//----------------------------------------------------------------------------------------
class CINEWARE_SINGLEINHERITANCE NodeData : public BaseData
{
protected:
...
public:
	...
	//----------------------------------------------------------------------------------------
	GeListNode* Get() { return private_link; }
    ...
    /// frequently due to nodes being reallocated in the background by Cinema 4D. One should 
    /// therefore be careful with carrying out computationally complex tasks in this function.
    ///
    /// @code
	...
	/// Modify the passed @formatParam{description} as needed, set the appropriate @formatParam{flags}. Make sure to include a call to the parent at the end. 
	/// @note {The \c SUPER syntax is only available when the #INSTANCEOF macro has been invoked for the NodeData which contains the NodeData::GetDDescription implementation.}
	/// @code
	...
	/// Modify the passed @formatParam{t_data} if the right @formatParam{id} is provided, and set the appropriate @formatParam{flags}. Make sure to include a call to the parent at the end. 
	/// @note {The \c SUPER syntax is only available when the #INSTANCEOF macro has been invoked for the NodeData which contains the NodeData::GetDParameter implementation.}
	/// @code
	...
	//----------------------------------------------------------------------------------------
	/// @brief Called to override the writing of parameters.
	/// @details Read the passed @formatParam{t_data} if the right @formatParam{id} was provided, store the data, and set the appropriate @formatParam{flags}. Make sure to include a call to the parent at the end. 
	/// @note {The \c SUPER syntax is only available when the #INSTANCEOF macro has been invoked for the NodeData which contains the NodeData::SetDParameter implementation.}
	/// @code
	...
	/// Read the passed @formatParam{t_data} if the right @formatParam{id} was provided, and return @formatConstant{true} to enable the parameter or @formatConstant{false} to disable it. Make sure to include a call to the parent at the end. 
	/// @note {The \c SUPER syntax is only available when the #INSTANCEOF macro has been invoked for the NodeData which contains the NodeData::GetDEnabling implementation.}
	/// @code
	...
	/// @warning Should not be overridden by regular plugins and should be used with extra care. If @formatConstant{false} is returned no undo is possible.
	/// @param[in] node								The GeListNode connected with the NodeData instance. Equal to Get(). Provided for speed and convenience. @cinemaOwnsPointed{node}
