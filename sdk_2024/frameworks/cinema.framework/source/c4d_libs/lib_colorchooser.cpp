...
#define ColorSwatchDataCall(b)					ColorSwatchDataLibrary *lib = CheckColorSwatchDataLibrary(LIBOFFSET(ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return; \
																																					(reinterpret_cast<iColorSwatchData*>(this)->*lib->b)
...
#define ColorSwatchDataCallR(a, b)			ColorSwatchDataLibrary *lib = CheckColorSwatchDataLibrary(LIBOFFSET(ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return a; \
																																					return (reinterpret_cast<iColorSwatchData*>(this)->*lib->b)

#define ColorSwatchDataCallRC(a, b)			ColorSwatchDataLibrary *lib = CheckColorSwatchDataLibrary(LIBOFFSET(ColorSwatchDataLibrary, b)); \
																																					if (!lib || !lib->b) return a; \
																																					return (reinterpret_cast<const iColorSwatchData*>(this)->*lib->b)
...
#define ColorSwatchGroupCall(b)         ColorSwatchGroupLibrary *lib = CheckColorSwatchGroupLibrary(LIBOFFSET(ColorSwatchGroupLibrary, b)); \
																																						if (!lib || !lib->b) return; \
																																						(reinterpret_cast<iColorSwatchGroup*>(this)->*lib->b)
...
#define ColorSwatchGroupCallR(a, b)			ColorSwatchGroupLibrary *lib = CheckColorSwatchGroupLibrary(LIBOFFSET(ColorSwatchGroupLibrary, b)); \
																																						if (!lib || !lib->b) return a; \
																																						return (reinterpret_cast<iColorSwatchGroup*>(this)->*lib->b)

#define ColorSwatchGroupCallRC(a, b)			ColorSwatchGroupLibrary *lib = CheckColorSwatchGroupLibrary(LIBOFFSET(ColorSwatchGroupLibrary, b)); \
																																						if (!lib || !lib->b) return a; \
																																						return (reinterpret_cast<const iColorSwatchGroup*>(this)->*lib->b)
