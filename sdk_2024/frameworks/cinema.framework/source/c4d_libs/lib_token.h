...
public:
	...
	const BaseDocument*		_doc;						///< The BaseDocument used for rendering, can be a clone of original document.
	const RenderData*			_rData;					///< The RenderData used for rendering.
	const BaseContainer*	_rBc;						///< The BaseContainer with the render settings (can be different from @c _rData->GetDataInstance() eg. RQ change paths).
	const BaseTake*	_cTake;								///< The BaseTake used for rendering.
