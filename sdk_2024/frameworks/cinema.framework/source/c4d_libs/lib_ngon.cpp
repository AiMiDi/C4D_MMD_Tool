...
#define NgonBaseLibCall(b) 		NgonBaseLib *lib = CheckNgonBaseLib(LIBOFFSET(NgonBaseLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iNgonBase*>(this)->*lib->b)

#define NgonBaseLibCallC(b) 		NgonBaseLib *lib = CheckNgonBaseLib(LIBOFFSET(NgonBaseLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const iNgonBase*>(this)->*lib->b)

#define NgonBaseLibCallR(a, b)  NgonBaseLib *lib = CheckNgonBaseLib(LIBOFFSET(NgonBaseLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iNgonBase*>(this)->*lib->b)

#define NgonBaseLibCallRC(a, b)  NgonBaseLib *lib = CheckNgonBaseLib(LIBOFFSET(NgonBaseLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iNgonBase*>(this)->*lib->b)
