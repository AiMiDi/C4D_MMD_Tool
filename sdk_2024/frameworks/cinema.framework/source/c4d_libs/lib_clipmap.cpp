...
#define GeClipMapCall(b)      GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iGeClipMap*>(this)->*lib->b)

#define GeClipMapCallC(b)      GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<const iGeClipMap*>(this)->*lib->b)

#define GeClipMapCallR(a, b)   GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iGeClipMap*>(this)->*lib->b)

#define GeClipMapCallRC(a, b)   GeClipMapLib *lib = CheckLib(LIBOFFSET(GeClipMapLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iGeClipMap*>(this)->*lib->b)
