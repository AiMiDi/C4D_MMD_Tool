...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Gets the value that is overridded at ID pIdx.
	/// @param[in] dataID							ID of the overridden data.
	/// @return												The data at the given index.
	//----------------------------------------------------------------------------------------
	maxon::Result<GeData> GetOverrideParam(const DescID& dataID) const;

	//----------------------------------------------------------------------------------------
	/// Sets the value that is overridded at index pIdx.
	/// @param[in] dataID							ID of the overridden data.
	/// @param[in] d									New value to set.
	/// return												OK on success.
	//----------------------------------------------------------------------------------------
	maxon::Result<void> SetOverrideParam(const DescID& dataID, const GeData& d);
...
//----------------------------------------------------------------------------------------
class BaseOverrideGroup : public BaseObject
{
	INSTANCEOF(BaseOverrideGroup, BaseObject)
...
class CINEWARE_SINGLEINHERITANCE iBaseTake;
class	CINEWARE_SINGLEINHERITANCE iBaseOverride;
class	CINEWARE_SINGLEINHERITANCE iBaseOverrideGroup;
class	CINEWARE_SINGLEINHERITANCE iTakeData;

struct TakeSystemLibrary : public C4DLibrary
{
	Bool									(iBaseTake::*IsMain)												() const;

	Bool									(iBaseTake::*SearchHierarchy)								(const iBaseTake* op) const;
	...
	Bool									(iBaseOverride::*IsOverriddenParam)					(const DescID& descID) const;
	...
	iBaseTake*						(iTakeData::*GetCurrentTake)								() const;
	...
	Bool									(iBaseTake::*IsChecked)											() const;
	void 									(iBaseTake::*SetChecked)										(Bool status);

	void 									(*StopTakeRender)														();
	maxon::Result<GeData>	(iBaseOverride::*GetOverrideParam)					(const DescID& descID) const;
	maxon::Result<void>		(iBaseOverride::*SetOverrideParam)					(const DescID& descID, const GeData& d);
};
