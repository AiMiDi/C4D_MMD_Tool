...
{
	return CheckLib<PrefsLib>(LIBRARY_PREFS, offset, lib_prefs);
}
...
static void FillPrefsPlugin(PREFSPLUGIN *np, DataAllocator *npalloc, Int32 info, BaseBitmap *icon)
{
	...
	np->GetMappedObject = &PrefsDialogObject::GetMappedObject;
}
...
#else
	o->GetDataInstanceRef().SetInt32(IDPREFS_SORTID, sortid);
	o->GetDataInstanceRef().SetInt32(IDPREFS_PARENTID, parentid);
#endif
	return o;
}
...
void PrefsDialogObject::InitPrefsValue(Int32 id, const GeData &data, Description* desc, const DescID &descid, BaseContainer* pBC)
{
	...
	{
		if (!pBC)
			pBC = GetWorldContainerInstance();
		if (pBC->GetData(id) == GeData() || pBC->GetData(id).GetType() != data.GetType())
			pBC->SetData(id, data);
	}
	else
	{
...
void PrefsDialogObject::GetPrefsValue(Int32 id, GeData &data, DESCFLAGS_GET &flags) const
{
	data = GetWorldContainerInstance()->GetData(id);
	flags |= DESCFLAGS_GET::PARAM_GET;
}

void PrefsDialogObject::GetPrefsValue(Int32 id, GeData &data, DESCFLAGS_GET &flags, const PrefsMapTable *table) const
{
