...
#define ZipLibCall(b) 		ZipFileLib *lib = CheckZipLib(LIBOFFSET(ZipFileLib, b)); \
															if (!lib || !lib->b) return; \
															(reinterpret_cast<iZipFile*>(this)->*lib->b)

#define ZipLibCallR(a, b) ZipFileLib *lib = CheckZipLib(LIBOFFSET(ZipFileLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<iZipFile*>(this)->*lib->b)

#define ZipLibCallRC(a, b) ZipFileLib *lib = CheckZipLib(LIBOFFSET(ZipFileLib, b)); \
															if (!lib || !lib->b) return a; \
															return (reinterpret_cast<const iZipFile*>(this)->*lib->b)
