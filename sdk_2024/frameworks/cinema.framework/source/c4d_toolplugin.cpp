...
const String ToolData::GetResourceSymbol() const
{
	return String();
}
...
CDialog* ToolData::AllocDialog(BaseContainer* bc, CDialog* cd_parent, Int32 dlg_id)
{
	...
	if (!dlg)
		return nullptr;
	C4DOS_Cd->AttachSubDialog(cd_parent, dlg_id, dlg->Get());
	return dlg->Get();
}

void ToolData::FreeDialog(CDialog* cd)
{
	SubDialog* dlg = (SubDialog*)C4DOS_Cd->GetUserData(cd);
	DeleteObj(dlg);
}

Bool ToolData::InitDisplayControl(BaseDocument* doc, const BaseContainer& data, BaseDraw* bd, const AtomArray* active)
{
	return false;
}

void ToolData::FreeDisplayControl()
{
}

Bool ToolData::DisplayControl(BaseDocument* doc, BaseObject* op, BaseObject* chainstart, BaseDraw* bd, BaseDrawHelp* bh, ControlDisplayStruct& cds) const
{
	return true;
}
...
Bool ToolData::GetDDescription(const BaseDocument* doc, const BaseContainer& data, Description* description, DESCFLAGS_DESC& flags) const
{
	return false;
}
...
Bool ToolData::GetDParameter(const BaseDocument* doc, const BaseContainer& data, const DescID& id, GeData& t_data, DESCFLAGS_GET& flags) const
{
...
Bool ToolData::GetDEnabling(const BaseDocument* doc, const BaseContainer& data, const DescID& id, const GeData& t_data, DESCFLAGS_ENABLE flags, const BaseContainer* itemdesc) const
{
	return true;
}
...
{
	return C4DOS_Ge->GetToolData(doc, pluginid, create);
}

Float GetToolScale(BaseDraw* bd, AtomArray* arr, Int32 all, Int32 mode)
{
	return C4DOS_Ge->GetToolScale(bd, arr, all, mode);
}

class ToolNodeData : public NodeData
{
public:
	static NodeData *Alloc() { return NewObjClear(NodeData); }
};
...
{
	C4DOS_Ge->EwDrawXORLine(this, x1, y1, x2, y2);
}

void EditorWindow::MouseDragStart(Int32 button, Float mx, Float my, MOUSEDRAGFLAGS flag)
{
	C4DOS_Ge->EwMouseDragStart(this, button, mx, my, flag);
}

MOUSEDRAGRESULT EditorWindow::MouseDrag(Float* mx, Float* my, BaseContainer* channels)
{
	return C4DOS_Ge->EwMouseDrag(this, mx, my, channels);
}

MOUSEDRAGRESULT EditorWindow::MouseDragEnd()
{
	return C4DOS_Ge->EwMouseDragEnd(this);
}

Bool EditorWindow::BfGetInputState(Int32 askdevice, Int32 askchannel, BaseContainer* res)
{
	return C4DOS_Ge->EwBfGetInputState(this, askdevice, askchannel, res);
}

Bool EditorWindow::BfGetInputEvent(Int32 askdevice, BaseContainer* res)
{
	return C4DOS_Ge->EwBfGetInputEvent(this, askdevice, res);
}

HOTKEYFLAGS EditorWindow::IsHotkeyDown(Int32 id)
{
	return C4DOS_Ge->EwBfIsHotkeyDown(this, id);
}
...
{
	return C4DOS_Ge->EWScreen2Local(this, x, y);
}

Bool EditorWindow::Local2Screen(Int32* x, Int32* y)
{
	return C4DOS_Ge->EWLocal2Screen(this, x, y);
}

Bool EditorWindow::Global2Local(Int32* x, Int32* y)
{
	return C4DOS_Ge->EWGlobal2Local(this, x, y);
}

Bool EditorWindow::Local2Global(Int32* x, Int32* y)
{
	return C4DOS_Ge->EWLocal2Global(this, x, y);
}
