...
String				GvOperatorData::GetDetailedText(const GvNode *bn) const { return String(); }
String				GvOperatorData::GetText(const GvNode *bn) const { return String(); }
String				GvOperatorData::GetTitle(const GvNode *bn) const { return String(); }
BaseBitmap*		GvOperatorData::GetPortIcon(GvNode *bn, Int32 id, GvLayoutType layout_type) { return nullptr; }
Vector				GvOperatorData::GetBodyColor(const GvNode *bn) const { return C4DOS_Gv->globals->GetDefaultBodyColor(bn); }
void					GvOperatorData::GetBodySize(const GvNode *bn, Int32 *width, Int32 *height) const { C4DOS_Gv->globals->GetDefaultBodySize(width, height); }
void					GvOperatorData::EditorDraw(GvNode *bn, GvNodeGUI *gui, GeUserArea *da, Int32 x1, Int32 y1, Int32 x2, Int32 y2) { C4DOS_Gv->globals->OperatorDefaultDraw(bn, gui, da, x1, y1, x2, y2); }
Bool					GvOperatorData::GetOperatorDescription(const GvNode *bn, GvOperatorDescription *od) const { return false; }
...
Bool					GvOperatorData::SetRecalculate(GvNode *bn, GvPort *port, GvRun *r, Bool force_set) { return C4DOS_Gv->globals->DefaultSetRecalculate(bn, port, r, force_set); }
...
Int32					GvOperatorData::FillPortsMenu(GvNode *bn, BaseContainer &names, BaseContainer &ids, GvValueID value_type, GvPortIO port, Int32 first_menu_id) { return C4DOS_Gv->globals->FillPortsMenu(bn, names, ids, value_type, port, first_menu_id); }
...
String			  GvOperatorData::GetErrorString(const GvNode *bn, Int32 error) const { return String::IntToString(error); }
...
DescID				GvOperatorData::GetDescID(GvNode *bn, GvPortIO io, Int32 main_id) { return DescID(); }
...
Bool GvOperatorData::Init(GeListNode* bn, Bool isCloneInit)
{
...
{
	return C4DOS_Gv->node->CreateOperator(bn, 1);
}

void GvOperatorData::iGetPortList(GvNode *bn, GvPortIO port, GvPortList &portlist) const
{
	C4DOS_Gv->node->GetPortList(bn, port, portlist, 1);
}

Bool GvOperatorData::iGetPortDescription(const GvNode *bn, GvPortIO port, Int32 id, GvPortDescription *pd) const
{
	return C4DOS_Gv->node->GetPortDescription(bn, port, id, pd, 1);
}
...
Bool GvOperatorData::GetDDescription(const GeListNode *node, Description *description, DESCFLAGS_DESC &t_flags) const
{
	DESCFLAGS_DESC flags = t_flags|DESCFLAGS_DESC_SUPERCALL;

	const NodeData *data = node->GetNodeData<NodeData>();
	NODEPLUGIN*	tab = (C4DOS_Bl->RetrieveTableX(data, 0));
...
Bool GvOperatorData::GetDEnabling(const GeListNode *node, const DescID &id, const GeData &t_data, DESCFLAGS_ENABLE t_flags, const BaseContainer *itemdesc) const
{
...
Bool GvOperatorData::GetDParameter(const GeListNode *node, const DescID &id, GeData &t_data, DESCFLAGS_GET &t_flags) const
{
...
{
	if (!C4DOS_Gv->globals || !C4DOS_Gv->globals->RegisterOperatorPlugin) return false;
	...
	return C4DOS_Gv->globals->RegisterOperatorPlugin(operator_id, str, info, at, disklevel, op_class, op_group, op_owner, &ap, icon);
}
