...
public:
	...
	{
		return maxon::Cast<RenderNoise>(self.GetDecoratedObject()).Init(seed, permutationTablePower, additionalParameters);
	}

	MAXON_METHOD Float32 Noise(const Vector32& p, Float32 time, Float32 octaves, Bool absolute, Float32 sampleRadius) const
	{
		return maxon::Cast<RenderNoise>(self.GetDecoratedObject()).Noise(p, time, octaves, absolute, sampleRadius);
	}
