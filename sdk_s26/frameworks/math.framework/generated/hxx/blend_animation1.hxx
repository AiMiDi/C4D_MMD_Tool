...
/// @param[in] startProgress			Start value.
		inline typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type SetStartProgress(Float startProgress) const;
...
/// @param[in] reverse						True to animate backwards.
		inline typename std::conditional<S::HAS_ERROR, maxon::Result<void>, void>::type SetDirection(Bool reverse) const;
	};
	/// Intermediate helper class for BlendAnimationInterface.
	template <typename S> class COWFn : public ConstFn<S>
	{
...
/// SetStartProgress allows to set the progress where the animation should start. this can be useful if you try to reverse another animation.
/// @param[in] startProgress			Start value.
		inline typename std::conditional<maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS), void, maxon::Result<void>>::type SetStartProgress(Float startProgress);
/// SetDirection allows to change the animation direction to play the reverse blend functions.
/// @param[in] reverse						True to animate backwards.
		inline typename std::conditional<maxon::Bool(S::Handler::KIND & maxon::VALUEKIND::DEEP_CONSTNESS), void, maxon::Result<void>>::type SetDirection(Bool reverse);
	};
	template <typename ST> struct FnHelper : public std::conditional<
	maxon::Bool(ST::type::Handler::KIND & (maxon::VALUEKIND::COW_MASK | maxon::VALUEKIND::DEEP_CONSTNESS)),
	COWFn<typename ObserverObjectInterface::Hxx1::template FnHelper<ST>::type>,
