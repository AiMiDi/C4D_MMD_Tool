...
template <typename S> Result<MutableRoot> NodeSystemInterface::Hxx1::COWFn<S>::BeginModification(Bool keepCurrent)
{
...
template <typename S> Result<MutableRoot> NodeSystemInterface::Hxx1::COWFn<S>::BeginInstantiationModification(const NodeTemplate& templ)
{
	iferr_scope;
	if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError())
		return this->GetErrorStorage();
...
template <typename S> Result<MutableRoot> NodeSystemInterface::Hxx1::COWFn<S>::BeginSubstitutionModification(const AssetRepositoryRef& lookup, const NodeTemplate& original, const TemplateArguments& args)
{
	iferr_scope;
	if (S::HAS_ERROR && this->GetErrorStorage().GetReferencedError())
		return this->GetErrorStorage();
...
Result<MutableNode> MutableRoot::AddChild(const Id& child, const Result<NodeTemplate>& base, const Result<TemplateArguments>& args, const AssetRepositoryRef& lookup, Opt<ChangeList&> list) const
{
	iferr_scope;
	CheckState(_system);
	const NodeTemplate& templ = base iferr_return;
	const TemplateArguments& ta = args iferr_return;
	NodeSystem b = templ.Instantiate(_system->GetNodeSystemClass(), lookup ? lookup : templ.GetRepository(), ta) iferr_return;
	return maxon::nodes::details::ConstructGNodeResult<MutableNode>(_system, _system->AddChild(child, b, list));
}
