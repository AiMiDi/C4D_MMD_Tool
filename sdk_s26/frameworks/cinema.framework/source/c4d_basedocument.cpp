...
BaseSceneHook* BaseDocument::GetFirstSceneHook(void)
{
	return C4DOS.Bd->GetFirstSceneHook(this);
}
...
BaseTime BaseDocument::GetTime(void) const
{
	GeData dat;
	if (!const_cast<BaseDocument*>(this)->GetParameter(DescLevel(DOCUMENT_TIME), dat, DESCFLAGS_GET::NONE))
		return BaseTime();
	return dat.GetTime();
}

Int32 BaseDocument::GetMode(void) const
{
	GeData dat;
	if (!const_cast<BaseDocument*>(this)->GetParameter(DescLevel(DOCUMENT_MODE), dat, DESCFLAGS_GET::NONE))
		return 0;
	return dat.GetInt32();
}
...
Int32 BaseDocument::GetAction(void) const
{
	GeData dat;
	if (!const_cast<BaseDocument*>(this)->GetParameter(DescLevel(DOCUMENT_ACTION), dat, DESCFLAGS_GET::NONE))
		return 0;
	return dat.GetInt32();
}

Bool BaseDocument::IsEditMode(void) const
{
	Int32 mode = GetMode();
...
{
	return C4DOS.Ge->LoadFile(&name, allowAsync);
}
...
BaseDocument* LoadDocument(const Filename& name, SCENEFILTER loadflags, BaseThread* thread, maxon::String* errorString, const ProgressDelegate& progressDelegate)
{
	...
	if (!C4DOS.Ge->LoadDocument(doc, MaxonConvert(url), loadflags, thread, nullptr, errorString))
		blDelete(doc);
	return doc;
}
...
Bool MergeDocument(BaseDocument* doc, const Filename& name, SCENEFILTER loadflags, BaseThread* thread, maxon::String* errorString, const ProgressDelegate& progressDelegate)
{
	...
	return C4DOS.Ge->LoadDocument(doc, MaxonConvert(url), loadflags | SCENEFILTER::MERGESCENE, thread, nullptr, errorString);
}
...
void BaseDocument::GetOcioConfigAndProfiles(maxon::OcioConfig& ocioConfig, maxon::ColorProfile& inputColorSpaceLow, maxon::ColorProfile& inputColorSpaceHigh, maxon::ColorProfile& renderingColorSpace, maxon::ColorProfile& viewTransform, maxon::ColorProfile& viewTransformThumbnails) const
{
	return BdCall(GetOcioConfigAndProfiles) (ocioConfig, inputColorSpaceLow, inputColorSpaceHigh, renderingColorSpace, viewTransform, viewTransformThumbnails);
}

void BaseDocument::GetActiveOcioColorSpacesNames(maxon::CString& inputColorSpaceLow, maxon::CString& inputColorSpaceHigh, maxon::CString& renderingColorSpace, maxon::CString& viewTransform, maxon::CString& displayColorSpace, maxon::CString& viewTransformThumbnails) const
{
	return BdCall(GetActiveOcioColorSpacesNames) (inputColorSpaceLow, inputColorSpaceHigh, renderingColorSpace, viewTransform, displayColorSpace, viewTransformThumbnails);
}
...
maxon::CString BaseDocument::GetNameFromColorSpaceId(const Int32 category, const Int32 index)
{
...
maxon::Result<void> BaseDocument::PrivateSetBaseRepository(const maxon::UpdatableAssetRepositoryRef& baseRepo)
{
	...
	SetParameter(DescLevel(ID_ADDSCENEREPOSITORYBASE), GeData((void*)&baseRepo, VOIDVALUETYPE::VOIDVALUE), DESCFLAGS_SET::DONTCHECKMINMAX | DESCFLAGS_SET::FORCESET);
