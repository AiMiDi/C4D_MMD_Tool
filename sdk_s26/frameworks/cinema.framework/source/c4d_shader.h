...
public:
	~OcioConverter()
	{ 
		C4DOS.Sh->DestructOcioConverter(this);
	}

	static maxon::Result<OcioConverter*> Init(const BaseDocument* doc, Int32 overrideViewTransform = -1)
	{
		return C4DOS.Sh->InitOcioConverter(doc, overrideViewTransform);
	}

	Vector64 TransformColor(const Vector64 &v, COLORSPACETRANSFORMATION colortransformation) const
	{
		return C4DOS.Sh->TransformColorOcio64(this, v, colortransformation);
	}

	Vector32 TransformColor(const Vector32 &v, COLORSPACETRANSFORMATION colortransformation) const
	{
		return C4DOS.Sh->TransformColorOcio32(this, v, colortransformation);
	}

	void GetConverter(COLORSPACETRANSFORMATION colortransformation, maxon::ColorProfileConvert& converter) const
	{
		return C4DOS.Sh->GetOcioConverter(this, colortransformation, converter);
	}
};
...
public:
	...
	//----------------------------------------------------------------------------------------
	explicit InitRenderStruct(BaseDocument* t_doc, Int32 overrideViewTransform = -1) { Init(t_doc, overrideViewTransform); }
...
public:
	...
	maxon::StrongRef<OcioConverter> ocioConverter;	///< @markPrivate OCIO converter
	...
	{
		if (GetStructureVersion() >= 1 && ocioConverter)
		{
			if (document_colorprofile == DOCUMENT_COLORPROFILE_SRGB && linear_workflow)
				return ocioConverter->TransformColor(input, COLORSPACETRANSFORMATION::OCIO_INPUT_LOW_TO_RENDERING);
			else if (document_colorprofile == DOCUMENT_COLORPROFILE_LINEAR && !linear_workflow)
				return ocioConverter->TransformColor(input, COLORSPACETRANSFORMATION::OCIO_RENDERING_TO_INPUT);
			
			// In the future we will we assume input colors are in rendering space when OCIO is active.
		...
		{
			if (document_colorprofile == DOCUMENT_COLORPROFILE_SRGB && linear_workflow)
				return CINEWARE_NAMESPACE::TransformColor(input, COLORSPACETRANSFORMATION::SRGB_TO_LINEAR);
			else if (document_colorprofile == DOCUMENT_COLORPROFILE_LINEAR && !linear_workflow)
				return CINEWARE_NAMESPACE::TransformColor(input, COLORSPACETRANSFORMATION::LINEAR_TO_SRGB);
		}
		return input;
	}
};
