...
static void HandleToolSystemMessage(BaseDocument* doc, Int32 toolId, BaseContainer& data, BaseThread* bt)
{
	if (!doc)
		return;
	...
	if (system)
	{
		ToolSystemMessageData messageData = { toolId, bt, &data };
...
Bool DescriptionToolData::InitTool(BaseDocument* doc, BaseContainer& data, BaseThread* bt)
{
	firstaction = true;
	last_dirty = NOTOK;
	HandleToolSystemMessage(doc, GetToolPluginId(), data, bt);
	return true;
}
...
Bool DescriptionToolData::Message(BaseDocument* doc, BaseContainer& data, Int32 type, void* t_data)
{
	...
	if (type == MSG_DESCRIPTION_COMMAND)
	{
		...
		{
			HandleToolSystemMessage(doc, GetToolPluginId(), data, nullptr);
