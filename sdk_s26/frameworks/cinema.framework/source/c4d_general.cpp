...
{
	return C4DOS.Ge->GetGlobalTexturePaths(docPath);
}
...
{
	C4DOS.Ge->GetAdditionalWorldContainerData(bc);
}
...
{
	C4DOS.Ge->SetActiveScriptObject(type);
}

BaseList2D* CreateNewPythonScript(String& name, const String& body)
{
	return C4DOS.Ge->CreateNewPythonScript(name, body);
}

BaseList2D* LoadPythonScript(const Filename& fn)
{
	return C4DOS.Ge->LoadPythonScript(fn);
}
...
{
	return C4DOS.Ge->GeGetActiveViewportType();
}
...
{
	return C4DOS.Ge->IsAnimationRunning(document);
}

Bool IsUVToolMode(const BaseDocument* doc)
{
	return C4DOS.Ge->IsUVToolMode(doc);
}

Int32 GetFormatDepth(Int32 format, Int32 depth)
{
	return C4DOS.Ge->GetFormatDepth(format, depth);
}
