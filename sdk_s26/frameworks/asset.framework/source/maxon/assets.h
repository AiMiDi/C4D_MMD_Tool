...
namespace ASSETMETADATA
{
	...
	/// This read-only asset meta data attribute has the size in bytes of the asset data.
...
public:
	...
	{
		iferr_scope;
		Data d = Get(metaId) iferr_return;
		T* const ptr = d.GetPtr<T>();
		if (ptr)
			return Opt<T>{std::move(*ptr)};
		return {};
	}
	...
	template <typename ATTR> MAXON_FUNCTION Result<typename ATTR::ValueType> Get(const ATTR& attr, const typename ATTR::ValueType& defaultValue) const
	{
		...
		return d.Get<typename ATTR::ValueType>(defaultValue);
	}
...
enum class ASSET_FIND_MODE
{
	...
	CHECK_CANCELLATION = 1 << 6,									///< Set this flag when you want to check if the job is cancelled. if cancelled a ThreadCancelledError is returned.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// HasIndex returns true if the database has an index.
	//----------------------------------------------------------------------------------------
