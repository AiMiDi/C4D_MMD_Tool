...
public:
	...
	{
		return (GetKeyDataType() == maxon::GetDataType<typename std::remove_const<KT>::type>()) && (GetValueDataType() == maxon::GetDataType<typename std::remove_const<VT>::type>());
	}
	...
	template <typename KT, typename VT> MAXON_FUNCTION void AssertType() const
	{
		...
		if (!STD_IS_REPLACEMENT(same, Generic, KT))
		{
			GetKeyDataType().template AssertType<typename std::conditional<STD_IS_REPLACEMENT(same, Generic, KT), Int, typename std::remove_const<KT>::type>::type>();
		}
		if (!STD_IS_REPLACEMENT(same, Generic, VT))
		{
			GetValueDataType().template AssertType<typename std::conditional<STD_IS_REPLACEMENT(same, Generic, VT), Int, typename std::remove_const<VT>::type>::type>();
		}
#endif
	}
...
#pragma warning disable 597
	...
	operator const MapInterface<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, const V, const Generic), DeleteReturnType01, const Generic>::type>&() const
	{
		return *(MapInterface<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, const V, const Generic), DeleteReturnType01, const Generic>::type>*) this;
	}

	operator MapInterface<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, V, Generic), DeleteReturnType02, Generic>::type>&()
	{
...
#pragma warning enable 597
	...
	operator const NonConstMap<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, V, Generic), DeleteReturnType03, Generic>::type>&()
	{
		return *reinterpret_cast<const NonConstMap<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, V, Generic), DeleteReturnType03, Generic>::type>*>(this);
	}
...
#endif
		: _vtableCls(src._vtableCls), _refDummy(src._refDummy)
	{
	}
#endif
...
public:
	...
	operator const NonConstMap<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, V, Generic), DeleteReturnType01, Generic>::type>&() const
	{
		return *reinterpret_cast<const NonConstMap<Generic, typename std::conditional<STD_IS_REPLACEMENT(same, V, Generic), DeleteReturnType01, Generic>::type>*>(this);
	}
...
public:
	...
	static const Bool CONSTMAP = STD_IS_REPLACEMENT(const, typename std::remove_reference<MAP>::type);
	static const Bool REFERENCE = STD_IS_REPLACEMENT(reference, MAP);
...
//		DebugAssert(System::GetReferenceCounter(&src) == 0);
	...
	{
		return GetDataType<typename std::remove_const<KeyType>::type, GET_DATATYPE_POLICY::GENERIC_FOR_UNKNOWN>();
	}
