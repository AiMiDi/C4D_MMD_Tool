...
public:
	...
	{
		return _id ? _id->GetHashCode() : DefaultHash::EMPTY_HASH;
	}

	/// @copydoc DefaultDoc::GetHashValue
	const HashValue& GetHashValue() const
	{
		return _id ? *reinterpret_cast<const HashValue*>(reinterpret_cast<const Char*>(_id) + HASH_VALUE_OFFSET) : maxon::NullValue<const HashValue&>();
	}
...
private:
	...
	static constexpr Int HASH_VALUE_OFFSET = SIZEOF(Id) + SIZEOF(void*) + SIZEOF(HashInt);
...
public:
	...
	// #LEGACYCOMPATIBILITY Remove, this is deprecated.
	...
	// #LEGACYCOMPATIBILITY return type can be void
	...
	//----------------------------------------------------------------------------------------
	/// Returns a pointer to a default-constructed object of the given type.
	...
	//----------------------------------------------------------------------------------------
	// #LEGACYCOMPATIBILITY can be removed once DataTypeImpl has a pointer to its default value
