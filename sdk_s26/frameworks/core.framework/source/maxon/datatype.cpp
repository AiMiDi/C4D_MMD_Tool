...
{
	DebugAssert(!(type->_valueKind & VALUEKIND::GLOBALLY_REFERENCED));
	// Use fast path and avoid reference counting.
	const_cast<DataTypeImpl*>(type)->_valueKind |= VALUEKIND::GLOBALLY_REFERENCED;
	// To be compatible with legacy plugins we set the reference counter to a very high value so that it shouldn't reach zero when legacy plugins decrement the counter.
	const_cast<StrongReferenceCounter*>(reinterpret_cast<const StrongReferenceCounter*>(type))[-1].SetRelaxed(LIMIT<UInt>::MAX >> 6);
}
...
Result<void> DataTypeCache::Init(Bool showMsg) const
{
	...
	if (!(_type.PrivateGetPointer()->_valueKind & VALUEKIND::GLOBALLY_REFERENCED))
	{
		// Use fast path and avoid reference counting.
		const_cast<DataTypeImpl*>(_type.PrivateGetPointer())->_valueKind |= VALUEKIND::GLOBALLY_REFERENCED;
		// // #LEGACYCOMPATIBILITY: To be compatible with legacy plugins we set the reference counter to a very high value so that it shouldn't reach zero when legacy plugins decrement the counter.
		const_cast<StrongReferenceCounter*>(reinterpret_cast<const StrongReferenceCounter*>(_type.PrivateGetPointer()))[-1].SetRelaxed(LIMIT<UInt>::MAX >> 6);
	}
...
HashValue AbstractDataTypeFunctions::GetHashValue(const DataTypeImpl* type, const void* a)
{
	return {type->GetHashCode(a), 0};
}
