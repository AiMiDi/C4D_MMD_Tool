...
//----------------------------------------------------------------------------------------
/// A Range represents a closed interval of values min ... max (both bounds are inclusive). If T is a vector type, this is actually a multi-dimensional
...
public:
	...
	//----------------------------------------------------------------------------------------
	constexpr Range() : minValue(LIMIT<ScalarType>::MAX), maxValue(LIMIT<ScalarType>::MIN) { }

	//----------------------------------------------------------------------------------------
	/// Constructs a singleton range containing exactly v.
	/// @param[in] v									Initial value for the range.
	//----------------------------------------------------------------------------------------
	explicit constexpr Range(const T& v) : minValue(v), maxValue(v) { }
	...
	//----------------------------------------------------------------------------------------
	constexpr Range(const T& rangeMin, const T& rangeMax) : minValue(rangeMin), maxValue(rangeMax) { }
	...
	//----------------------------------------------------------------------------------------
	void Reset() { minValue = T(LIMIT<ScalarType>::MAX); maxValue = T(LIMIT<ScalarType>::MIN); }

	//----------------------------------------------------------------------------------------
	/// Sets this range to a singleton range containing exactly v.
	/// @param[in] v									Singleton value for the range.
	//----------------------------------------------------------------------------------------
	void Set(const T& v) { minValue = maxValue = v; }
	...
	//----------------------------------------------------------------------------------------
	void Set(const T& rangeMin, const T& rangeMax) { minValue = rangeMin; maxValue = rangeMax; }
	...
	void Add(const T& rangeMin, const T& rangeMax)
	{
		...
		{
			SetMin(minValue, rangeMin);
			SetMax(maxValue, rangeMax);
		}
	}
	...
	{
		if (DefaultCompare::LessThanOrEqual(range.minValue, range.maxValue))
		{
			SetMin(minValue, range.minValue);
			SetMax(maxValue, range.maxValue);
		}
	}
	...
	{
		SetMax(minValue, rangeMin);
		SetMin(maxValue, rangeMax);
	}
	...
	{
		SetMax(minValue, range.minValue);
		SetMin(maxValue, range.maxValue);
	}
	...
	{
		return Range(minValue + shift, maxValue + shift);
	}
	...
	//----------------------------------------------------------------------------------------
	Bool IsEmpty() const { return !DefaultCompare::LessThanOrEqual(minValue, maxValue); }

	//----------------------------------------------------------------------------------------
	/// Returns true if this range is populated.
	/// @return												True if range is not empty.
	//----------------------------------------------------------------------------------------
	Bool IsPopulated() const { return DefaultCompare::LessThanOrEqual(minValue, maxValue); }
	...
	T GetMin() const
	{
		return DefaultCompare::LessThanOrEqual(minValue, maxValue) ? minValue : T(ScalarType(0));
	}
	...
	T GetMax() const
	{
		return DefaultCompare::LessThanOrEqual(minValue, maxValue) ? maxValue : T(ScalarType(0));
	}
	...
	T GetCenter() const
	{
		return DefaultCompare::LessThanOrEqual(minValue, maxValue) ? (maxValue + minValue) * ScalarType(0.5) : T(ScalarType(0));
	}
	...
	/// Note that for integral datatypes this does not calculate the number of elements (which is maxValue - minValue + 1). To do so you need to add 1 manually.
	/// @return												Dimension of the range, or T(0) for an empty range.
	//----------------------------------------------------------------------------------------
	T GetDimension() const
	{
		return DefaultCompare::LessThanOrEqual(minValue, maxValue) ? maxValue - minValue : T(ScalarType(0));
	}
	...
	{
		return DefaultCompare::LessThanOrEqual(minValue, value) && DefaultCompare::LessThanOrEqual(value, maxValue);
	}
	...
	{
		return DefaultCompare::LessThanOrEqual(minValue, value.GetMin()) && DefaultCompare::LessThanOrEqual(value.GetMax(), maxValue);
	}
	...
	{
		return "["_s + GlobalToString(minValue, format) + ","_s + GlobalToString(maxValue, format) + "]"_s;
	}

	T minValue; ///< The inclusive minimum boundary of this range.
	T maxValue; ///< The inclusive maximum boundary of this range. If the minimum boundary is not less than or equal to this, the range is empty.
};
