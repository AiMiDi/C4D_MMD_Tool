...
struct SecureRandom::MTable
{
	...
	Bool (*SecureRandom_GetRandomNumber) (SecureRandomProvider provider, const Block<Byte>& buffer);
	template <typename IMPL> void Init()
	{
		SecureRandom_GetDefaultProvider = &IMPL::SecureRandom_GetDefaultProvider;
		SecureRandom_GetRandomNumber = &IMPL::SecureRandom_GetRandomNumber;
	}
};
...
struct SecureRandom::Hxx2
{
	...
	{
	public:
		static SecureRandomProvider SecureRandom_GetDefaultProvider() { return C::GetDefaultProvider(); }
		static Bool SecureRandom_GetRandomNumber(SecureRandomProvider provider, const Block<Byte>& buffer) { return C::GetRandomNumber(std::forward<SecureRandomProvider>(provider), buffer); }
	};
