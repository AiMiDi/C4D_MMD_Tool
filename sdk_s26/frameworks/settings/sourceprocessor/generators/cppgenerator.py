...
class Generator(object):
    ...
    def processMethod(self, method, cls, freeMethod):
        ...
        if method.kind == 'MAXON_METHOD':
            ...
            if cls.interface == Class.VIRTUAL:
                ...
                if freeMethod:
                    s += 'if (!object)\n\treturn;\nreturn ((' + cls.name + '::MTable*) (object->_vtableCls + 1))[-1].'
                else:
                    s += 'return ((' + cls.name + '::MTable*) (this->_vtableCls + 1))[-1].'
            s += fname + '('
            if not method.static:
                if cls.interface == Class.VIRTUAL:
            ...
            if cls.interface == Class.VIRTUAL:
                if method.static:
                    self.staticReflectionFunctions << 'maxon::reflection::GetFunctionInfo<std::remove_pointer<decltype(' + self.tclass + '::Hxx2::StaticMTable::_instance.' + fname + ')>::type>(nullptr, maxon::reflection::Member::KIND::FUNCTION, ' + paramNames + ', ' + str(inOut) + 'LL, nullptr),'
                else:
                ...
                self.reflectionFunctions << 'maxon::reflection::GetFunctionInfo<std::remove_pointer<decltype(' + cls.name + '::MTable::_instance.' + fname + ')>::type>(nullptr, maxon::reflection::Member::KIND::' \
        ...
        if not (freeMethod and cls.derived) and (method.kind == 'MAXON_METHOD'):
            if cls.interface == Class.VIRTUAL:
                if method.static:
                    self.staticInit << 'tbl->' + fname + ' = &IMPL::' + fname + ';'
                else:
                    isImplemented = 'PRIVATE_MAXON_IMPLEMENTS(typename W::Implementation, ' + cls.name + ', ' + xname + ')'
                    self.xinit << 'if constexpr (' + isImplemented + ')' << '{'
                    self.xinit << fname + ' = &W::' + fname + ';'
                    self.xinit << fname + '_Offset = offset;' << '}'
            elif freeMethod and cls.interface == Class.SIMPLE_VIRTUAL:
                self.init << fname + ' = GetFreeMethod<IMPL>(OVERLOAD_MAX_RANK);'
                self.pointers << 'template <typename IMPL> auto GetFreeMethod(maxon::OverloadRank1) { return &IMPL::' + fname + '; }'
                self.pointers << 'template <typename IMPL, typename = typename IMPL::Implementation::PrivateComponentWithBase> auto GetFreeMethod(maxon::OverloadRank2) { return reinterpret_cast<decltype(' + fname + ')>(&maxon::ObjectInterface::PrivateFreeInstance); }'
            else:
                self.init << fname + ' = &IMPL::' + fname + ';'
            ...
            if cls.interface == Class.NONVIRTUAL or (cls.interface == Class.VIRTUAL and method.static):
                unresolvedImpl = 'tbl->' + fname + ' = &Hxx2::' + ('CWrapper<maxon::StaticHelper<Hxx2::Unresolved>' if cls.interface == Class.VIRTUAL else 'Wrapper<Hxx2::Unresolved') + '>::' + fname + ';'
            ...
            if cls.interface == Class.VIRTUAL and method.static:
                self.staticPointers << methodRetType + ' (*' + fname + ') (' + xparams + ');'
            else:
                self.pointers << methodRetType + ' (*' + fname + ') (' + xparams + ');'
                if cls.interface == Class.VIRTUAL:
                    self.pointers << 'maxon::Int ' + fname + '_Offset;'

            s = 'static ' + deferredRetType + ' ' + fname + '(' + xparams + ') { return '
            if cls.interface == Class.VIRTUAL:
                if method.static:
                ...
                if not cls.singleImpl:
                    ...
                    if cls.qname() == 'maxon.ObjectInterface' and method.name == 'InitObject':
                        ...
                        self.proxyFunctions << '(*handler_)(_fn_' + cname + ', o_, maxon::reflection::ProxyInvocation(' + ('std::addressof(proxy_result)' if methodRetType != 'void' else 'nullptr') + ', proxy_args, ' \
                            + '&maxon::reflection::Invocation<' + methodRetType + ', ' + const + 'maxon::GenericComponent*' + ''.join([', ' + p.type for p in methodParams]) + '>::Invoke, (const void*) mt_.' + fname + '));'
                        if methodRetType != 'void':
            ...
            for cowMode in range(1 if onlyCow else 0, 2):
                ...
                if method.static:
                    ...
                    if method.kind == 'MAXON_FUNCTION':
                        ...
                        if cls.interface == Class.VIRTUAL:
                            rfd << 'const MTable& mt_ = std::is_same<typename S::Handler, maxon::SuperPointerHandler>::value ? PRIVATE_MAXON_SUPERMTABLE(' + cls.name + ', o_, ' + fname + ') : ' \
                                + '((S::PrivateReferencedType::INTERFACE_FLAGS & maxon::EntityBase::FLAGS::SINGLE_IMPLEMENTATION) && S::PrivateReferencedType::HasBaseDetector::template Check<' + cls.name + '>::value) ? S::PrivateReferencedType::MTable::template GetMTable<' + cls.name + '>() : PRIVATE_MAXON_VTABLE(' + cls.name + ', o_);'
                            s = 'mt_.'
                        elif cls.interface == Class.NONVIRTUAL:
                            s = 'MTable::_instance.'
                        else:
    ...
    def processClass(self, cls):
        ...
        if freeMethod and cls.interface != Class.SIMPLE_VIRTUAL:
        ...
        if freeMethod and cls.interface == Class.SIMPLE_VIRTUAL: # make sure that Free method is last in vtable (so it has the same place for all simple virtual interfaces)
            self.processMethod(freeMethod, cls, True)
            ...
            if cls.interface == Class.VIRTUAL:
                ...
                if cls.baseInterfaces:
                    ...
                    for b, g in cls.baseInterfaces:
                        ...
                        baseIndex += 1
                    self.defs << 'if (!S::Implementation::PROXY && !(' + ' && '.join(['Super' + str(x) + '::InitMTablesWithBases(info, offset)' for x in range(0, len(cls.baseInterfaces))]) + ')) return false;'
                ...
                self.defs << '{' << 'public:'
                if not cls.derived:
                    self.defs << 'using Implementation = C;'
            ...
            if cls.refKind:
                ...
                for fn in ['Const', '', 'COW']:
                    ...
                    if fn == 'COW' and self.hasPopulatedOrEmptyFunction:
                        functions << 'explicit operator maxon::Bool() const MAXON_WORKAROUND_CLANG_DELETED_FUNCTION(S);\n'
                        functions << 'explicit operator maxon::Bool() MAXON_WORKAROUND_CLANG_DELETED_FUNCTION(S);'
                    if fn == 'Const':
                        # Add all usings to the ConstFn class as this is the base reference function class.
                        usings = cls.usings
                    else:
                        usings = []
                        # Check for each using if there's a function/method with the same name in Fn/COWFn.
                        # Then the inherited using from ConstFn is hidden, so we have to repeat it.
                        for uc, un in cls.usings:
                            for f in cls.findAll(Function, un):
                                if not (f.const or f.static):
                                    usings.append((uc, un))
                                    break
                ...
                for ref in ['Reference', 'NonConstRef']:
                    ...
                    if cls.refKind == Class.REF_UNIQUE:
                        ...
                        self.decls << '{' << 'public:'
                        # Workaround for Intel 18 bug for the ArrayInterface<TYPE>::NonConstRef case. Intel says: "error #3466: inheriting constructors must be inherited from a direct base class".
                        # We don't use MAXON_DEFAULT_REFERENCE_CONSTRUCTORS for templated interfaces.
                        if not (ref == 'NonConstRef' and templatePrefix):
                            self.decls << 'MAXON_DEFAULT_REFERENCE_CONSTRUCTORS(' + ref + ', ' + superRef + ');'
