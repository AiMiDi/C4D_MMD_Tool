...
namespace YAML {
	...
	struct convert<String> {
		static Node encode(const String& rhs) {
			Node node;
			node.push_back(std::string(rhs.GetCStringCopy(STRINGENCODING::UTF8)));
			return node;
		}
	...
	Emitter& operator << (YAML::Emitter& out, const String& str) {
		out << str.GetCStringCopy(STRINGENCODING::UTF8);
		return out;
	}
}
...
NameConversion::UpdataNameConversionDialog::UpdataNameConversionDialog(NameConversion* name_conversion_) : name_conversion(name_conversion_) {
	...
	DefaultNameMap = NameMapArray.FindIndex("Name_conversion.yml"_s);
...
Bool NameConversion::UpdataNameConversionDialog::Command(Int32 id, const BaseContainer& msg)
{
	...
	case  DLG_NAME_CONVER_OK:
	{
		...
		for (Int i = 0; i < conversion_count; i++) {
			...
			if (str2==""_s) {
				...
				yaml_out << YAML::BeginMap << YAML::Key << str1 << YAML::Value << str2 << YAML::EndMap;
				fout << '\n' << yaml_out.c_str();
			}
		}
		fout.close();
	...
	case(DLG_NAME_CONVER_NEW_ADD): 
	{
		...
		Filename fn = GeGetPluginResourcePath() + Filename("Name_conversion\\") + Filename(fn_s + ".yml");
		if (GeFExist(fn)) 
		{
			if (!QuestionDialog(IDS_MES_INQUIRY_OVERWRITING_FILE)) 
			{
				const Int32 add_fn_index = NameMapArray.FindIndex(fn_s + ".yml");
		...
		if (!file->Open(fn, FILEOPEN::WRITE))return false;
		file->Close();
		NameMapArray.Reset();
		AutoAlloc<BrowseFiles> bf;
		bf->Init(GeGetPluginResourcePath() + Filename("Name_conversion"), BROWSEFILES_CALCSIZE);
	...
	case(DLG_NAME_CONVER_NEW_INCREMENT_ADD): 
	{
		...
		{
			if (!QuestionDialog(IDS_MES_INQUIRY_OVERWRITING_FILE)) {
				const Int32 increment_fn_index = NameMapArray.FindIndex(fn_s + ".yml");
