...
#define PRIVATE_MAXON_INPUT_PORT(T, name, LOCAL, ARRAY, CNT, ...) \
	struct name : public MAXON_MACROARG_TYPE(maxon::corenodes::details::PortDefBaseCtor<name, maxon::corenodes::VirtualInputPort, ~(PRIVATE_MAXON_PORT_GET_NAME_HASHCODE(name)), LOCAL, STD_IS_REPLACEMENT(reference, T), MAXON_EXPAND(MAXON_VA_NARGS(__VA_ARGS__)), PRIVATE_MAXON_PORT_DIMENSION_##ARRAY(CNT), CNT>) \
	{ \
