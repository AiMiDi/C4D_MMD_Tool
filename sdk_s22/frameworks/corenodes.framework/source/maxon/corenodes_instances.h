...
//		ARRAY_OUTPUT_HEAD = CONTAINER_OUTPUT_HEAD | DEFINITE_FACTOR_HEAD_MASK,
	...
	enum class FLAGS
	{
		...
		CURRENT_ITV = 1 << 6,
		NEXT_ITV = 1 << 7,
		PORT_MODIFIERS = PERMANENT | CACHED | HIDDEN | CURRENT_ITV | NEXT_ITV,
		...
		MODIFIERS = PORT_MODIFIERS | COMMUTATIVE | STATE_MAPPER | STATE_MODIFIER | OUTER_DOMAIN,
		ALL = 0xfffffff
...
public:
	...
	template <typename VARIADIC_ACCESS, typename MN> MAXON_FUNCTION Result<BaseArray<typename VARIADIC_ACCESS::VPort::PortId>> AddVariadicPort(VARIADIC_ACCESS MN::* acc, const CString& prefix, const Data& arg, WIRETYPE wires = WIRETYPE::ALL, const DataType& type = GetZeroRef<DataType>())
	{
		...
		typename VARIADIC_ACCESS::VPort ports = VARIADIC_ACCESS::VPort::Create(GetParentGroup(), prefix, arg, type ? type : GetPortType<typename VARIADIC_ACCESS::ValueType>()) iferr_return;
