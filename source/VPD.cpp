...
namespace tool {
	maxon::Result<void> VPD_pose::LoadFromFile(Filename& fn) {
		iferr_scope;
		...
		/* 将读取的字符转为UTF8 */
		utility::SJIStoUTF8(all_char_mem, all_char, all_char_length);
		...
		{
			mmd::VPDBoneData* bone_post = NewObj(mmd::VPDBoneData)iferr_return;
	...
	maxon::Result<void> VPD_pose::SaveToFile(Filename& fn) {
		return maxon::OK;
	}
	maxon::Result<void> VPD_pose::FromFileImportPose(VPD_pose_import_settings setting) {
		iferr_scope;
		...
		nodes.Reset();
		for (mmd::VPDBoneData& bone_data : this->bones) {
			...
			if (bone_tag_list_ptr != nullptr) {
				...
				for (BaseObject* bone : *bone_tag_list) {
					...
					bone->SetRelRot(utility::QuaternionToEuler(bone_data.rotation));
				}
			}
		}
		EventAdd(EVENT::NONE);
	...
	maxon::Result<void> VPD_pose::FromDocumentExportPose(VPD_post_export_settings setting) {
		return maxon::OK;
	}
}