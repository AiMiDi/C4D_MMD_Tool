...
void MMDCoronaMaterialAdapter::ReadFrom(const BaseMaterial* material, MMDMaterialData& data)
{
	...
	GeData ge_data;
	if (GetAtomParameter(material, ConstDescID(DescLevel(CORONA_DIFFUSE_COLOR)), ge_data, DESCFLAGS_GET::NONE))
		data.diffuse_rgb = ge_data.GetVector();
	if (GetAtomParameter(material, ConstDescID(DescLevel(CORONA_ALPHA_COLOR_LEVEL)), ge_data, DESCFLAGS_GET::NONE))
