...
{
	// Result::GetValue() 返回左值引用；UniqueRef 禁止拷贝赋值，必须 std::move（R20 等旧 SDK 会触发 static_assert）
	maxon::Result<FilenameRef> res = FilenameRef::Create(GeGetPluginResourcePath() + Filename("name_conversion\\"));
	name_mapping_config_dirname_ = std::move(res.GetValue());
}
