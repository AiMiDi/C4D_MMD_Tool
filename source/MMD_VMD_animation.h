...
namespace mmd{
	...
	private:
		...
		{
		public:
			static Bool LessThan(const VMD_Shadow& a, const VMD_Shadow& b) { return a.frame_no < b.frame_no; }
			static Bool IsEqual(const VMD_Shadow& a, const VMD_Shadow& b) { return a.frame_no == b.frame_no; }
		};
		class VMDModelSortedArray : public maxon::SortedArray<VMDModelSortedArray, maxon::PointerArray<VMD_Model>>
		{
		public:
			static Bool LessThan(const VMD_Model& a, const VMD_Model& b) { return a.frame_no < b.frame_no; }
			static Bool IsEqual(const VMD_Model& a, const VMD_Model& b) { return a.frame_no == b.frame_no; }
		};
		//最初使用动作模型名称
		String ModelName;
		//是否为摄像机动作
	...
	public:		
		...
		//生成函数
		static NodeData* Alloc() { return NewObjClear(OMMDCamera); }
	};
