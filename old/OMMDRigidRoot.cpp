...
namespace tool {
	Bool OMMDRigidRoot::Init(GeListNode* node)
	{
	...
	void OMMDRigidRoot::Free(GeListNode* node)
	{

	}
	Bool OMMDRigidRoot::Read(GeListNode* node, HyperFile* hf, Int32 level)
	{
		if (level == 1) {
	...
	Bool OMMDRigidRoot::Write(GeListNode* node, HyperFile* hf)
	{
		AutoAlloc<BaseLink> bone_root_link;
		if (bone_root_link == nullptr)
			return false;
	...
	Bool OMMDRigidRoot::CopyTo(NodeData* dest, GeListNode* snode, GeListNode* dnode, COPYFLAGS flags, AliasTrans* trn)
	{
	...
	Bool OMMDRigidRoot::SetDParameter(GeListNode* node, const DescID& id, const GeData& t_data, DESCFLAGS_SET& flags)
	{
	...
	Bool OMMDRigidRoot::Message(GeListNode* node, Int32 type, void* data)
	{
		iferr_scope_handler{
			return true;
		};
