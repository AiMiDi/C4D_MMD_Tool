...
function(MaxonTargets_SetupPrebuildEvents)
	...
	set(hasPrebuildEvents FALSE)
	foreach(prebuild.DEBUG prebuild.RELEASE IN ZIP_LISTS arg_PrebuildEvents.DEBUG arg_PrebuildEvents.RELEASE)
		if(NOT CMAKE_GENERATOR MATCHES "Xcode")
			if("${prebuild.DEBUG}" MATCHES "typeviewer/xcode" OR "${prebuild.RELEASE}" MATCHES "typeviewer/xcode")
				continue()
			endif()
		endif()
		...
		foreach(config IN ITEMS DEBUG RELEASE)
			...
			string(REPLACE "\\\\;" " " params.${config} "${params.${config}}") # we had previously turned semi-colons into token dividers, which were then turned into spaces during appending. We transform them back now.
			string(REPLACE "\\\"" "\"" params.${config} "${params.${config}}") # we had previously turned semi-colons into token dividers, which were then turned into spaces during appending. We transform them back now.
			if(NOT CMAKE_GENERATOR MATCHES "Xcode" AND "${targetCommand.${config}}" STREQUAL "python" AND DEFINED g_pythonBin AND NOT "${g_pythonBin}" STREQUAL "")
				set(targetCommand.${config} "${g_pythonBin}")
			endif()
	...
	if(additionalPrebuildSuffix GREATER_EQUAL 1 AND NOT hasPrebuildEvents)
		...
		if(additionalPrebuildSuffix GREATER_EQUAL 2)
...
function (MaxonTargets_ConfigureLibrary)
	...
	set(sourceProcessorArgs ${divertPycFilesFlag} ${g_sourceProcessorPythonPy} --option stylecheck.aswarnings=true --option excludesplitregistercpps=true --generatedroot \"${sourceProcessorDirectory}\" --frameworkroot \"${g_frameworkRootDirectory}\" \"${arg_TargetRootDir}\")
	...
	if (maxon_hasPrebuildEvents)
		...
		add_custom_target(
			...
			COMMAND ${g_pythonBin} ${g_sourceProcessorPythonPy} --option stylecheck.aswarnings=true --option excludesplitregistercpps=true --generatedroot ${sourceProcessorDirectory} --frameworkroot ${g_frameworkRootDirectory} ${arg_TargetRootDir}
			BYPRODUCTS ${registerGeneratedFileNames} ${stampFilePath}
			)
